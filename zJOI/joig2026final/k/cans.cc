...
// @@ !! LIM(f:updMaxMin debug cmpNaive perm)
...
int naive(istream& cin, ostream& cout) {
  ...
  vector bag(T, vector<ll>());
  REP(i, 0, M) bag[B[i]].push_back(D[i]);
  REP(t, 0, T) ranges::sort(bag[t], greater<ll>());

  IntComb ic(N, M);
  ll ans = M;
  while (ic.get()) {
    vector cake(T, vector<ll>());
    REP(i, 0, M) cake[A[ic[i]]].push_back(C[ic[i]]);
    REP(t, 0, T) ranges::sort(cake[t], greater<ll>());
    ll got = 0;
    REP(t, 0, T) {
      ll j = 0;
      REP(i, 0, ssize(bag[t])) {
        for (; j < ssize(cake[t]) and cake[t][j] > bag[t][i]; j++);
        if (j == ssize(cake[t])) break;
        got++;
        j++;
      }
    }
...
int body(istream& cin, ostream& cout) {
  ...
  vector cake(T, vector<ll>());
  vector bag(T, vector<ll>());
  REP(i, 0, N) cake[A[i]].push_back(C[i]);
  REP(i, 0, M) bag[B[i]].push_back(D[i]);
  vector E(T, vector<pll>());
  REP(t, 0, T) {
    ranges::sort(cake[t], greater<ll>());
    ranges::sort(bag[t], greater<ll>());
    ll nc = ssize(cake[t]), nb = ssize(bag[t]);
    ll i = 0, j = 0;
    while (i < nc) {
      ll yield = 0, get = 0;
      for (; i < nc and j < nb and cake[t][i] <= bag[t][j]; i++, j++) yield++;
      for (; i < nc and (j == nb or cake[t][i] > bag[t][j]); i++) get++;
      E[t].emplace_back(yield, get);
    }
  }
  ll big = 1LL << 60;
  ...
  REP(t, 0, T) {
    ...
    REP(k, 0, M + 1) {
      if (prev[k] < big) {
        ...
        for (auto [y, g] : E[t]) {
          if (m >= M) break;
          if (m + y + g <= M) {
            if (updMin(tbl[m + y + g], val + y)) updMin(chgd, m + y + g);
          }
          else if (m + y <= M) {
            updMin(tbl[M], val + y);
          }
          else {
            updMin(tbl[M], val + M - m);
          }
          m += y + g;
          val += y;
        }
    ...
    if (chgd < M) {
      REP(k, chgd + 1, M + 1) updMin(tbl[k], tbl[k - 1] + 1);
    }
    DLOGK(t, E[t], tbl);
  }
  cout << tbl[M] << endl;
