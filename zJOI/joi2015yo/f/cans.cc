...
int naive(istream& cin, ostream& cout) {
  ...
  for (ll t = 0; t < (1LL << N); t++) {
	...
	if ((s >> i) & 1) {
      ...
      DLOG("t=", t, "s=", s, "ax=", ax, "ay=", ay);
      if (abs(ax) <= D) ans = max(ans, ay);
...
int body(istream& cin, ostream& cout) {
  ...
  for (ll i = 0; i < sz; i++) init_vec[i] = fh[i].second;
  ...
  for (ll t = 0; t < (1LL << K); t++) {
	...
	if ((s >> i) & 1) {
      ...
      // [-ax - D, -ax + D + 1)
      sta cmp1 = {-ax - D, LLONG_MIN};
      sta cmp2 = {-ax + D + 1, LLONG_MIN};
      ll idx1 = lower_bound(fh.begin(), fh.end(), cmp1) - fh.begin();
      ll idx2 = lower_bound(fh.begin(), fh.end(), cmp2) - fh.begin();
      ll vm = st.query(idx1, idx2);
      DLOG("ax=", ax, "ay=", ay, "idx1=", idx1, "idx2=", idx2, "vm=", vm);
      if (vm > LLONG_MIN) {
	ans = max(ans, ay + vm);
      }
