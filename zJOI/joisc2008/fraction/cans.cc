...
int naive(istream& cin, ostream& cout) {
  ...
  for (ll x = 2; x <= M; x++) {
    for (ll i = 1; i < x; i++) {
      ...
      vec.emplace_back((double)i / x, i, x);
    }
  }
  if (K - 1 >= (ll)vec.size()) {
    cout << -1 << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  ll M, K; cin >> M >> K;
  auto primes = sieve(200);

  vector<vector<ll>> divs(M + 1);
  for (ll x = 2; x <= M; x++) divs[x] = getDivisors(x, primes);
  ll s = 0;
  {
    vector<ll> tmp(M + 1);
    for (ll x = 2; x <= M; x++) {
      tmp[x] = x - 1;
      for (ll d : divs[x]) if (1 < d && d < x) tmp[x] -= tmp[d];
      s += tmp[x];
    }
  }
  DLOGK(s);
  ...
  for (ll x = 2; x <= M; x++) {
    ...
    DLOGK(x, t, v, vmax, max_r, max_n);
  }
  cout << max_r << " " << max_n << endl;
  return 0;
