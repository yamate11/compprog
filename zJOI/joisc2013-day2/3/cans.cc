...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  vector<vector<ll>> cldJ(N), cldI(N);
  ll rootJ = -1, rootI = -1;
  for (ll i = 0; i < N; i++) {
    ll p, q; cin >> p >> q; p--; q--;
    if (p == -1) rootJ = i;
    else cldJ[p].push_back(i);
    if (q == -1) rootI = i;
    else cldI[q].push_back(i);
  }
  vector<vector<ll>> spiedBy(N);
  vector<ll> rsBy(M);
  for (ll i = 0; i < M; i++) {
    ll r, s; cin >> r >> s; r--; s--;
    rsBy[i] = r;
    spiedBy[s].push_back(i);
  }
  vector<ll> et_in(N), et_out(N);
  {
    ll i = 0;
    auto dfs1 = [&](auto rF, ll nd) -> void {
      et_in[nd] = i++;
      for (ll c : cldJ[nd]) rF(rF, c);
      et_out[nd] = i++;
    };
    dfs1(dfs1, rootJ);
  }
  ...
  auto et = make_seg_tree(unit_dat, unit_op, xAdd, xComp, xAppl, false);
  et.set_data(vector<DAT>(2*N));
  ...
  auto dfs2 = [&](auto rF, ll nd) -> void {
    auto upd = [&](ll z) -> void {
      for (ll p : spiedBy[nd]) {
        ...
        ll out = et_out[r];
        et.update(in, in + 1, z);
        et.update(out, out + 1, -z);
      }
    };
    upd(1);
    ans[nd] = et.query(0, et_in[nd] + 1);
    for (ll c : cldI[nd]) rF(rF, c);
    upd(-1);
  };
  dfs2(dfs2, rootI);
