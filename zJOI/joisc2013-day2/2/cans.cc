...
int main(/* int argc, char *argv[] */) {
  ...
  ll R, C; cin >> R >> C;
  ll N; cin >> N;
  ll rmax = 0, cmax = 0, rmin = R, cmin = C;
  for (ll i = 0; i < N; i++) {
    ll r, c; cin >> r >> c; r--; c--;
    rmax = max(rmax, r);
    rmin = min(rmin, r);
  ...
  auto myfact = [&](ll x) -> Fp {
    Fp ret = 1;
    for (ll i = 1; i <= x; i++) ret *= Fp(i);
    return ret;
  };
  Comb cb(R + C + 10);
  auto tbl_skel = vector(R - r0 + 1, Fp(0));
  auto cur = tbl_skel;
  cur[0] = myfact(hole);
  for (ll i = 0; i < R - r0 + C - c0; i++) {
    auto prev = move(cur);
    cur = tbl_skel;
    for (ll r = 0; r <= R - r0; r++) {
      if (prev[r] == Fp(0)) continue;
      cur[r] += prev[r] * cb.fact(r0 + r);
      if (r+1 <= R - r0 && i - r >= 0) {
        cur[r+1] += prev[r] * cb.fact(c0 + i - r);
      }
    }
  }
  Fp ans = cur[R - r0] * cb.comb(R - r0, rmin) * cb.comb(C - c0, cmin);
  cout << ans << endl;
