...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = 0;
  REP(i, 0, N) REP(j, i + 1, i + N) REP(k, j + 1, i + N) {
    ll x = S[j] - S[i];
    ll y = S[k] - S[j];
    ll z = S[i + N] - S[k];
    ll cur = min(x, min(y, z));
    updMax(ans, cur);
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 0, N) {
    ...
    auto divIJ = [&]() -> ll {
      ...
      auto g = [&](ll kk) -> ll { return min(S[kk] - S[j], S[i0] - S[kk]); };
      DLOGKL("before", k, g(k), g(k + 1));
      while (k + 1 <= i0 and g(k) < g(k + 1)) {
        k++;
        DLOGKL("in loop", k, g(k), g(k + 1));
      }
      DLOGK(i, j, k, g(k));
      return g(k);
    };
    while(j < i0 and S[j] - S[i] <= divIJ()) j++;
    DLOGK(i, j);
    updMax(ans, S[j - 1] - S[i]);
  }
  cout << ans << endl;
