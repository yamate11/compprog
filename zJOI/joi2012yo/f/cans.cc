...
int main(/* int argc, char *argv[] */) {
  ...
  auto func = [&](string S) -> Fp {
    DLOGK(S);
    ll sz = S.size();
    auto tbl_init = vector(2, vector(3, vector(10, vector(M, Fp(0)))));
    auto tbl = tbl_init;
    ll m00 = 0, n00 = 0, r00 = 0;
    tbl[1][m00][n00][r00] = 1;
    for (ll i = 0; i < sz; i++) {
      auto prev = move(tbl);
      tbl = tbl_init;
      for (ll f = 0; f < 2; f++) {
        for (ll m = 0; m < 3; m++) {
          if (f == 1 && m != m00) continue;
          for (ll n = 0; n < 10; n++) {
            if (f == 1 && n != n00) continue;
            for (ll r = 0; r < M; r++) {
              Fp t = prev[f][m][n][r];
              if (t == Fp(0)) continue;
              for (ll x = 0; x < 10; x++) {
                ll f0 = f, m0 = m, n0 = n, r0 = r;
                if (f == 1) {
                  ll d = S[i] - '0';
                  if (x > d) continue;
                  if (x < d) f0 = 0;
    ...
    Fp ret = 0;
    for (ll m = 0; m < 3; m++) {
      for (ll n = 0; n < 10; n++) ret += tbl[0][m][n][0];
    }
    return ret;
  };
