...
int main(/* int argc, char *argv[] */) {
  ...
  ll n, m, s; cin >> n >> m >> s;
  for (ll i = 1; i <= n*n; i++) s -= i;
  m -= n*n;

  vector tbl(s + 1, vector(m + 1, Fp(0)));
  tbl[s][0] = 1;
  for (ll i = 0; i < n * n; i++) {
    vector prev(s + 1, vector(m + 1, Fp(0)));
    swap(tbl, prev);
    ll e = n * n - i;
    for (ll v = 0; v <= s; v++) {
      for (ll w = 0; w <= m; w++) {
	if (prev[v][w] == Fp(0)) continue;
	for (ll j = 0; v - j*e >= 0 && w + j <= m; j++) {
	  tbl[v - j*e][w + j] += prev[v][w];
	}
      }
    }
  }
  Fp ans = accumulate(tbl[0].begin(), tbl[0].end(), Fp(0));
  cout << ans << endl;
