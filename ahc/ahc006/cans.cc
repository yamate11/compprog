...
// @@ !! LIM(ipoint debug random sortedIndex forall)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpMVec(1000, ((A, IPoint), (B, IPoint))) [jchCh8Rv]
  auto A = vector(1000, IPoint());
  auto B = vector(1000, IPoint());
  for (int i = 0; i < 1000; i++) {
    IPoint v1; cin >> v1; A[i] = v1;
    IPoint v2; cin >> v2; B[i] = v2;
  }
  // @End [jchCh8Rv]

  auto dist = [&](const IPoint& p, const IPoint& q) -> ll { return abs(p.x - q.x) + abs(p.y - q.y); };

  IPoint origin(400, 400);

  vector<ll> bound(1000, 0LL);
  REP(i, 0, 1000) bound[i] = max(dist(origin, A[i]), dist(origin, B[i]));
  auto PB = sortedIndex(bound);

  auto func1 = [&](IPoint P, const auto& X) -> vector<ll> {
    vector<ll> ret;
    ...
    REP(i, 0, 50) {
      ll j0 = -1;
      ll val = big;
      REP(j, 0, 50) if (not done[j]) {
        ll v = dist(cur, X[PB[j]]);
        if (v < val) {
          j0 = j;
          val = v;
        }
  ...
  auto orderA = func1(origin, A);
  auto orderB = func1(A[orderA.back()], B);
  vector<pll> orderAB(100);
  REP(i, 0, 50) orderAB[i]      = pll(0, orderA[i]);
  REP(i, 0, 50) orderAB[50 + i] = pll(1, orderB[i]);

  auto get_point = [&](ll x) -> IPoint {
    if (x == -1 or x == 100) return origin;
    else {
      auto [tp, id] = orderAB[x];
      if (tp == 0) return A[id];
      else         return B[id];
    }
  };
...
#endif
  REP(_r, 0, rep) {
    double TempNow = pow(Temp1, (double)_r / rep) * pow(Temp0, 1.0 - (double)_r / rep);
    ll p, q;
    while (true) {
      p = rand.range(-1, 100);
      q = rand.range(0, 100);
      auto valid = [&]() -> bool {
        if (q == p or q == p + 1) return false;
        auto [tp, id] = orderAB[q];
        if (q < p) {
          if (tp == 0 and EXISTS(i, q + 1, p + 1, orderAB[i] == pll(1, id))) return false;
          return true;
        }else if (p < q) {
          if (tp == 1 and EXISTS(i, p + 1, q, orderAB[i] == pll(0, id))) return false;
          return true;
        }else assert(0);
      };
      if (valid()) break;
    }
    IPoint ptQ1 = get_point(q);
    IPoint ptQ0 = get_point(q - 1);
    IPoint ptQ2 = get_point(q + 1);
    IPoint ptP0 = get_point(p);
    IPoint ptP1 = get_point(p + 1);
    ll oldLen = dist(ptQ0, ptQ1) + dist(ptQ1, ptQ2) + dist(ptP0, ptP1);
    ll newLen = dist(ptQ0, ptQ2) + dist(ptP0, ptQ1) + dist(ptQ1, ptP1);
    ll cimpr = oldLen - newLen;
    double thr = exp(-abs(cimpr) / TempNow);
    if (cimpr >= 0 or rand.rand(0.0, 1.0) < thr) {
      impr += cimpr;
      pll valQ = orderAB[q];
      orderAB.erase(orderAB.begin() + q);
      ll eps = q < p ? 1 : 0;
      orderAB.insert(orderAB.begin() + (p + 1 - eps), valQ);
    }
  }
  cerr << "impr = " << impr << endl;
  ...
  REP(i, 0, 100) route.push_back(get_point(i));
  route.push_back(origin);

  ll totLen = 0;
  REP(i, 0, ssize(route) - 1) totLen += dist(route[i + 1], route[i]);
  cerr << "totLen = " << totLen << endl;

  cout << ssize(orderA) << " ";
  REPOUT(i, 0, ssize(orderA), orderA[i] + 1, " ");
  cout << ssize(route) << " ";
  REP(i, 0, ssize(route)) {
