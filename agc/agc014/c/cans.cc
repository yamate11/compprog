...
int main(int argc, char *argv[]) {
  ...
  for (ll r = 0; r < H; r++) {
    string s; cin >> s;
    for (ll c = 0; c < W; c++) {
      if (s.at(c) == 'S') { s_r = r; s_c = c; brd.set(r, c, '.'); }
      else                {                   brd.set(r, c, s.at(c)); }
    }
  }
  using Item = pair<BrdIdx, ll>;
  queue<Item> que;
  Board<char> checked(H, W, 0);
  que.emplace(BrdIdx(s_r, s_c), 0);
  ll dist0 = H+W+1;
  while (!que.empty()) {
    BrdIdx bi(-1,-1); ll d; tie(bi, d) = que.front(); que.pop();
    if (checked.get(bi)) continue;
    checked.set(bi, true);
    ll dist = min<ll>(bi.r, min<ll>(bi.c, min<ll>(H-1 - bi.r, W-1 - bi.c)));
    ...
    for (BrdIdx bd : BrdIdx::nbr4) {
      BrdIdx p = bi + bd;
      if (brd.get(p) == '#') continue;
      if (checked.get(p)) continue;
      que.emplace(p, d+1);
    }
  }
  DLOG("dist0=", dist0);
  cout << 2 + (dist0 - 1) / K << endl;
