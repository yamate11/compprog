...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, P, dec=1) [CjIovbbz]
  auto P = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; P[i] = v; }
  // @End [CjIovbbz]
  
  vector eff(N, vector(N, false));
  REP(i, 0, N) {
    REP(j, i, N) if (P[i] < P[j]) break; else eff[j][i] = true;
    REPrev(j, i, 0) if (P[i] < P[j]) break; else eff[j][i] = true;
  }
  vector tbl_init(N, Fp(0));
  auto tbl = tbl_init;
  REP(i, 0, N) if (eff[0][i]) tbl[i] = Fp(1);
  REP(i, 1, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    vector<Fp> acc(N + 1); REP(j, 0, N) acc[j + 1] = acc[j] + prev[j];
    REP(j, 0, N) if (eff[i][j]) tbl[j] = acc[j + 1];
  }
  Fp ans = accumulate(ALL(tbl), Fp(0));
  cout << ans << endl;
  return 0;
}
