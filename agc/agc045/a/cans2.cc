...
// @@ !! LIM(debug mod matrix)
// --> f:<< debug f:gcd mod f:power matrix
...
ll myRank(auto& X, ll k) {
  Matrix<Fp> mat(X.size(), k);
  for (int i = 0; i < (ll)X.size(); i++) {
    for (int j = 0; j < k; j++) {
      mat.at(i, j) = X.at(i).at(j);
    }
  }
  mat.self_sweepout();
  ll i = 0;
