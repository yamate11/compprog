...
int main(int argc, char *argv[]) {
  ...
  ll ans = 0;
  Board<ll> brd(N, N, 0);
  Board<char> open(N, N, 0);
  for (ll r = 0; r < N; r++) {
    for (ll c = 0; c < N; c++) {
      ll d = min(r, min(c, min(N-1-r, N-1-c)));
      brd.set(r, c, d);
    }
  }
  for (ll i = 0; i < N*N; i++) {
    ...
    que.push(here);
    while (!que.empty()) {
      BrdIdx cur = que.front(); que.pop();
      for (BrdIdx bd : BrdIdx::nbr4) {
	BrdIdx bnew = cur + bd;
	if (!brd.in(bnew)) continue;
	ll newDist = brd.get(cur);
	if (!open.get(cur)) newDist += 1;
	if (brd.get(bnew) <= newDist) continue;
	brd.set(bnew, newDist);
	que.push(bnew);
      }
    }
  }
