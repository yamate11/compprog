...
int main(/* int argc, char *argv[] */) {
  ...
  DLOGK(diam, nd0, nd1, ct0, ct1);
  if (ct0 == ct1) {
    tr.change_root(ct0);
    ll levs = diam / 2 + 1;
    vector<ll> vd(levs);
    REP(i, 0, N) updMax(vd[tr.depth(i)], SIZE(tr.children(i)));
    ll leaves = 1;
    REP(i, 0, levs - 1) leaves *= vd[i];
    cout << levs << " " << leaves << endl;
  }else {
    tr.change_root(ct0);
    Tree tr2(N + 1, N);
    REP(i, 0, N) {
      if (i == ct0 or i == ct1) tr2.add_edge(i, N);
      else tr2.add_edge(i, tr.parent(i));
    }
    tr2.set_parent_child();
    DLOGK(tr2._parent);
    ll levs = (diam + 1) / 2 + 1;
    vector<ll> vd(levs);
    REP(i, 0, N + 1) updMax(vd[tr2.depth(i)], SIZE(tr2.children(i)));
    DLOGK(levs, vd);
    ll leaves = 1;
    REP(i, 0, levs - 1) leaves *= vd[i];
    cout << levs - 1 << " " << leaves << endl;
  }
