...
// @@ !! LIM(UnionFind input debug cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((U, dec=1), (V, dec=1))) [EyeH7e4P]
  auto U = vector(M, ll());
  auto V = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  auto enc = [&](ll x, ll y) -> ll { return x * N + y; };

  UnionFind uf(N * N);
  REP(p, 0, M) REP(q, p, M) {
    uf.merge(enc(U[p], U[q]), enc(V[p], V[q]));
    uf.merge(enc(U[q], U[p]), enc(V[q], V[p]));
    uf.merge(enc(U[p], V[q]), enc(V[p], U[q]));
    uf.merge(enc(V[q], U[p]), enc(U[q], V[p]));
  }
  ll cnt = 0;
  REP(z, 0, N*N) if (uf.leader(z) == z) cnt++;
  cout << cnt << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, nbr, dec=1) [35ahYor3]
  ...
  auto dfs = [&](auto rF, ll i) -> ll {
    ...
    auto chk = [&](ll j) -> ll {
      ...
      if (visited[j] >= 0) return visited[j] == 1 - visited[i];
      else {
        
    };
    for (ll j : nbr[i]) ret = chk(j) and ret;
      if (visited[j] < 0) {
        visited[j] = 1 - visited[i];
        ret = rF(rF, j) and ret;
      }else ret = (visited[j] == 1 - visited[i]) and ret;
    }
    return ret;
  };
  ll nbp = 0;
  ...
  REP(i, 0, N) {
    if (visited[i] < 0) {
      visited[i] = 0;
      if (nbr[i].empty()) niso++;
      else if (dfs(dfs, i)) nbp++;
      else nnbp++;
    }
  }
  ll ans = niso * N * 2 - niso * niso + (nbp + nnbp) * (nbp + nnbp) + nbp * nbp;
  DLOGK(niso, nbp, nnbp, N, ans);
  cout << ans << endl;
