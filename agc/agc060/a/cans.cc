...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  string S; cin >> S;
  vector tbl_init(27, vector(27, Fp(0)));
  auto tbl = tbl_init;
  tbl[26][26] = 1;
  REP(i, 0, N) {
    auto prev = move(tbl);
    ...
    if (S[i] != '?') {
      char c = S[i] - 'a';
      REP(p, 0, 27) REP(q, 0, 27) {
        if (p != c and q != c) tbl[c][p] += prev[p][q];
      }
    }else {
      REP(c, 0, 26) REP(p, 0, 27) REP(q, 0, 27) {
        if (p != c and q != c) tbl[c][p] += prev[p][q];
      }
    }
  }
  Fp ans = 0;
  REP(p, 0, 27) REP(q, 0, 27) ans += tbl[p][q];
  cout << ans << endl;
