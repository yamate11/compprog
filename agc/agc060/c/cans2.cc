...
int main(/* int argc, char *argv[] */) {
  ...
  */
  ...
  vector<Fp> pow2(N + 1);
  pow2[0] = Fp(1);
  REP(i, 0, N) pow2[i + 1] = pow2[i] * Fp(2);
  vector tbl(N, vector(N, Fp(0)));
  tbl[0][0] = Fp(1);
  REP(sumab, 0, N-1 + N-1 + 1) {
    REP(a, 0, N) {
      ll b = sumab - a;
      if (b < a) break;
      if (b >= N) continue;
      Fp x = pow2[N - 1 - a] - 1;
      Fp y = pow2[N - 1 - b] - 1;
      Fp rx = x + y == Fp(0) ? Fp(0) : x / (x + y);
      Fp ry = Fp(1) - rx;
      if (a + 1 < N) tbl[a + 1][b] += tbl[a][b] * rx;
      if (b + 1 < N) tbl[a][b + 1] += tbl[a][b] * ry;
      if (a < b) {
        if (a + 1 < N) tbl[b][a + 1] += tbl[a][b] * rx;
        if (b + 1 < N) tbl[b + 1][a] += tbl[a][b] * ry;
      }
      DLOGK(a, b, tbl[a][b]);
    }
  }
