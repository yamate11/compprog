...
// @@ !! LIM(f:updMaxMin f:perfmeas mod)
...
inline ll better_get_time_msec() {
  static ll lc = 0;
  static ll prev = -1;
  if (prev < 0 or lc == 10000) {
    prev = get_time_msec();
    lc = 0;
  }else {
    lc++;
...
int main(/* int argc, char *argv[] */) {
  ...
  while (cnt < goal) {
    ...
    x += cnt * cnt;
    x += cnt + cnt;
    if (better_get_time_msec() - t > 2000) break;
#if 0
    if (++lc == 10000) {
      if (get_time_msec() - t > 2000) break;
      else lc = 0;
    }
#endif
  }
