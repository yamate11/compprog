...
// @@ !! LIM(intervalSet debug cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 0, N) {
    ...
    ll cnt = 0;
    while (p < big) {
      auto [x, y] = prev.get_itvl(p);
      // DLOGK(x, y);
      auto [_y, z] = prev.get_itvl(y);
      // DLOGK(_y, z);
      cur.put(A[i] + x, A[i] + y, 1);
      p = z;
      if (z < A[i]) cnt += z - y;
    }
    DLOGK(i, A[i], cnt, cur.impl);
    if (cnt > K * 2) break;
  }
  vector<ll> ans;
  ll p = 0;
  while (true) {
    auto [x, y] = cur.get_itvl(p);
    auto [_y, z] = cur.get_itvl(y);
    REP(i, y, z) {
      ans.push_back(i);
      if (SIZE(ans) == K) {
        REPOUT(j, 0, K, ans[j], " ");
