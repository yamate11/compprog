...
/*
  ...
  (1) Adds a line to S.
  (2) For given x_0, answers max { f(x_0) | f \in S }

  If you need min, use the library with -f's.

  IMPORTANT RESTRICTION
    In this implementation, you should know all (possible) x's in advance.

  Usage:
    vector<int> xs({0, 3, -2, 5});
    CHT<int> cht(xs);
    cht.put(4, -3);              // y = 4x - 3
    cout << cht.get(0) << endl;  // prints -3.  0 must be in xs.
    cht.put(10, 1);              // y = 10x + 1
    cout << cht.get(3) << endl;  // prints 31.  3 must be in xs.
 */
