...
/*
  Linear Transformation    f(x) = Ax + B
       In type of A, B and x, multiprecation should be defined.
       operator *  ... composition  (f * g)(x) = f(g(x))
  ...
  Usage:
    ...
    LinTr<ll> f2(3, -1);
    LinTr<ll> f3 = f1 * f2;
    assert(f1(f2(100)) == f3(100));
    LinTr<double> f4(M_PI/6.0);
    LinTr<double> f5 = power(f4, 11);
 */


// @@ !! LIM(mod f:power matrix)
// --> f:gcd f:intDiv mod f:power matrix
