...
/*
  Tree ...  0-indexed rooted tree
  Edge ...  its edge
  RerootTree ... reroot tree (全方位木DP)
  ...
  members
    ...
    vector<Edge> edge;
  constructors
    Tree(const auto& edge_, int root_ = 0);
    Tree(vector<Edge>&& edge_, int root_ = 0);
    // *** NOTE *** The size of edge should be N-1, where the nodes
    //              should be 0, 1, ..., N-1.
  member functions
    int lca(int x, int y);   // lowest common ancestor    
    vector<int> nnpath(int x, int y)  // path between two nodes
    int acestorDep(int x, int dep)   // the ancestor of x whose depth is dep
    int diameter()           // diameter
    int node2edgeID(int x, int y)   // the edge ID connecting x and y
  ...
  Typical usage:
    ...
    for (ll i = 0; i < N-1; i++) {
      ll a, b; cin >> a >> b; a--; b--;
      ve.emplace_back(a, b);
      // Or, if each input line is just two ints separated by a space
      // just like the above:
      //   ve.push_back(Edge(cin, 1)); // if input is 1-indexed.
      //   ve.push_back(Edge(cin, 0)); // if input is 0-indexed.
    }
    Tree tr(move(ve));
  ...
  Typical usage:
    ...
    for (int i = 0; i < tr.numNodes; i++) 
      cout << result.at(i) << endl; // print answers
