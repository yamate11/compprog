...
struct Matrix {
  ...
  optional<pair<Matrix, vector<Matrix>>> linSolution(const Matrix& bs) const {
    if (dimJ != bs.dimI or bs.dimJ != 1) throw domain_error("linSolution: invalid bs");
...
int main(/* int argc, char *argv[] */) {
  ...
  {
    using Pol = Polynomial<double, 0>;
    vector<double> vp = {1, 1};
    vector<double> vq = {1, 0, -2};
    Pol p(vp);
    Pol q(vq);
    Pol a = p.divFPS(Pol::SP(q), 9);
    DLOGK(a);
    auto optsol = fitFPS<Pol>(a.coefVec(), 3);
    assert(optsol);
    return 0;
  }
