...
struct DAT_add {
  ...
  static T unit_add(const T& a) { return ::zero<T>(a); }
};
...
struct OP_subst {
  ...
  using S = value_type;
  static S comp(const S& f, const S& g) { return f ? f : g; }
  static T op(const S& f, const T& a) { return f ? *f : a; }
  static S unit_op(const S& _dummy) { return nullopt; }
};

template<typename pDAT, typename pOP>
struct SEGT_add_subst {
  using DAT = pDAT;
  using OP = pOP;
  using T = typename pDAT::value_type;
  using S = typename pOP::value_type;
  static T mult_apply(int k, const T& a) { return k * a; }
};

template<typename SEGT>
struct SegTree {
  using DAT = typename SEGT::dat_type;
  using OP = typename SEGT::op_type;
...
*/
  ...
  SegTree(int size_) : built(false) {
    if (size_ <= 0) throw domain_error("size must be >= 1");
    if (size_ == 1) height = 0;
    else            height = sizeof(int) * 8 - __builtin_clz(size_ - 1);
    size = 1 << height;
    node.resize(2*size);
    susp.resize(size);
  }

  const DAT& at(int pos) { return node[size + pos]; }
  void set_data(int pos, const DAT& val) { node[size + pos] = val; }
  void set_data(int pos, DAT&& val) { node[size + pos] = move(val); }

  void chk_build() {
    if (not built) {
      built = true;
      for (int t = size - 1; t >= 1; t--) node[t] = DAT::add(node[t<<1|0], node[t<<1|1]);
    }
  }
