...
int main(/* int argc, char *argv[] */) {
  ...
  auto ok = [&](ll x) -> bool {
    ...
    Board<int> brd(6, 6, 0);
    REP(i, 4) REP(j, 4) if (x >> (i * 4 + j) & 1) brd.at(i + 1, j + 1) = 1;
    UnionFind uf(36);
    ll oneone = -1;
    REP(i, 6) REP(j, 6) {
      auto bc = BrdIdx(i, j);
      if (brd.at(bc)) oneone = bc.r * 6 + bc.c;
      for (auto d : BrdIdx::nbr4) {
        auto bi = bc + d;
        if (not brd.in(bi)) continue;
        if (brd.at(bi) == brd.at(bc)) uf.merge(bc.r * 6 + bc.c, bi.r * 6 + bi.c);
      }
    }
    assert(oneone >= 0);
    return uf.groupSize(0) + uf.groupSize(oneone) == 36;
  };

  ll ans = 0;
  REP(x, 1LL << 16) if (ok(x)) ans++;
  cout << ans << endl;
