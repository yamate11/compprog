...
// @@ !! LIM(f:updMaxMin debug perm cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  vector<int> vec;
  while (ip.get()) {
    ll d = dist(0, 1 + ip.at(0));
    REP(j, 0, N - 2) d += dist(1 + ip.at(j), 1 + ip.at(j + 1));
    d += dist(1 + ip.at(N - 2), 0);
    if (d < ans) {
      ans = d;
      vec = ip.vec;
    }
...
int body(istream& cin, ostream& cout) {
  ...
  ll big = 1e18;
  vector tbl(N, vector<ll>(1LL << N, big));
  tbl[0][0] = 0;
  REP(sz, 0, N) {
    REP(x, 0, 1LL << N) {
      if (__builtin_popcountll(x) == sz) {
        REP(i, 0, N) {
          if (tbl[i][x] < big) {
            REP(j, 0, N) {
              if (not (x >> j & 1)) {
                ll y = x | (1LL << j);
                updMin(tbl[j][y], tbl[i][x] + dist(i, j));
                DLOGK(j, y, i, x, tbl[j][y], tbl[i][x]);
              }
            }
          }
        }
