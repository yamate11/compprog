...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> pow10(12);
  pow10[0] = 1;
  REP2(i, 1, 12) pow10[i] = 10 * pow10[i - 1];
  vector tbl(50, vector<ll>(50, 0LL));
  ...
  REP(i, N) {
    ...
    if (q == -1) {
      ...
      ll a = stoll(s.substr(0, q));
      ll b = stoll(s.substr(q + 1));
      l = SIZE(s.substr(q + 1));
      x = pow10[l] * a + b;
    }
    ll j = 0;
  ...
  ll cntS = 0;
  ll cntD = 0;
  REP2(i2, -9, 40) REP2(i5, -9, 40) REP2(j2, max(-9LL, -i2), 40) REP2(j5, max(-9LL, -i5), 40) {
    if (i2 == j2 and i5 == j5) {
      ll t = tbl[i2 + 9][i5 + 9];
      cntS += t * (t - 1) / 2;
    }else {
      cntD += tbl[i2 + 9][i5 + 9] * tbl[j2 + 9][j5 + 9];
    }
  }
  ll ans = cntS + cntD / 2;
  cout << ans << endl;
