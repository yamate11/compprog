...
int main(/* int argc, char *argv[] */) {
  ...
  while (not deq.empty()) {
    ...
    for (BrdIdx dir : BrdIdx::nbr4) {
      ...
      if (brd.at(bnew) == '.') {
        if (dist.at(bnew) <= d) continue;
        dist.at(bnew) = d;
        deq.emplace_front(d, bnew);
      }else {
        for (BrdIdx dir2 : BrdIdx::nbr9) {
          ll dnew2 = d + 1;
          BrdIdx bnew2 = bnew + dir2;
          if (not brd.in(bnew2)) continue;
          if (dist.at(bnew2) <= dnew2) continue;
          dist.at(bnew2) = dnew2;
          deq.emplace_back(dnew2, bnew2);
        }
      }
    }
  }
