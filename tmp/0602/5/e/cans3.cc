...
// @@ !! LIM(mod sieve debug f:perfmeas)
...
template <template <class...> class Trait, class... Args>
using is_detected = typename my_detail::is_detected<Trait, void, Args...>::type;
...
ll my_gcd(ci a, ll b) {
  // ll tm0 = get_time_sec();
  ll ret = std::gcd((a % b).convert_to<ll>(), b);
  // ll tm1 = get_time_sec();
  // cerr << "my_gcd(" << a << ", " << b << ")   " << tm1 - tm0 << "\n";
  return ret;
}
*/
...
int main(/* int argc, char *argv[] */) {
  ...
  DLOGK(A, N, l);
  Fp ans = 0;
  REP(i, N) ans += (l / A[i] % primeA).convert_to<ll>();
