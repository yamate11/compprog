...
// @DefStruct(vl, (v, l), ord=v) [T5Xiwjmy]
struct vl_t {
  ll v;
  ll l;
  vl_t() {}
  vl_t(ll v_, ll l_) : v(v_), l(l_) {}
  friend istream& operator>>(istream& istr, vl_t& t) {
    istr >> t.v >> t.l;
    return istr;
  }
  friend ostream& operator<<(ostream& ostr, const vl_t& t) {
    ostr << "(" << t.v << ", " << t.l << ")";
    return ostr;
  }
  bool operator<(const vl_t& o) const {
...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [ANHrod5j]
  // @InpVec(M, VL, type=vl_t) [7ldvyRbe]
  auto VL = vector(M, vl_t());
  for (int i = 0; i < M; i++) { vl_t v; cin >> v; VL[i] = v; }
  // @End [7ldvyRbe]
  ll wmax = *max_element(ALL(W));
  if (EXISTS(i, 0, M, V[i] > wmax)) {
    cout << -1 << endl;
    return 0;
  }
  sort(ALL(VL));
