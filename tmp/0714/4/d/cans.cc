...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1e9;
  ll sz = H * W;
  vector dist(sz, vector(sz, big));
  REP(i, sz) dist[i][i] = 0;
  REP(r, H) REP(c, W) {
    if (brd.at(r, c) != '.') continue;
    for (BrdIdx dir : BrdIdx::nbr4) {
      auto br = BrdIdx(r, c) + dir;
      if (brd.at(br) == '.') dist[enc(r, c)][enc(br.r, br.c)] = 1;
    }
  }
  REP(k, sz) REP(i, sz) REP(j, sz) updMin(dist[i][j], dist[i][k] + dist[k][j]);
  ll ans = 0;
  REP(i, sz) REP(j, sz) {
