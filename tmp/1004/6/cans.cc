...
int main(/* int argc, char *argv[] */) {
  ...
  auto normalize = [&](const IPoint& p) -> IPoint {
    if (p.x == 0 and p.y == 0) return origin;
    else if (p.x == 0) return IPoint(0, 1);
    else if (p.y == 0) return IPoint(1, 0);
    IPoint ret = p;
    if (ret.x < 0) ret = -ret;
    ll g = gcd(ret.x, abs(ret.y));
    ret.x /= g;
    ret.y /= g;
    return ret;
  };
  ...
  for (auto& [p, n] : mp) {
    if (p != origin) {
      IPoint q = normalize(p.rotate());
      ll m = mapget(mp, q);
      if (m > 0 and p == q.rotate()) continue;
      ans *= power<Fp>(2, n) + power<Fp>(2, m) - 1;
    }
  }
  ans = ans + mapget(mp, origin) - 1;
  cout << ans << endl;
