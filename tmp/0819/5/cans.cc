...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  // @InpGrid(H, W, S, type=char) [gH2BBdiw]
  auto S = vector(H, vector(W, char()));
  for (int i = 0; i < H; i++) for (int j = 0; j < W; j++) { char v; cin >> v; S[i][j] = v; }
  // @End [gH2BBdiw]
  vector Vert(H, vector(W, 0LL));
  vector Horiz(H, vector(W, 0LL));
  REP(r, 0, H) {
    ll c = 0;
    while (c < W) {
      while (c < W and S[r][c] == '#') c++;
      ll st = c;
      while (c < W and S[r][c] == '.') c++;
      ll n = c - st;
      REP(i, st, c) Horiz[r][i] = n;
    }
  }
  REP(c, 0, W) {
    ll r = 0;
    while (r < H) {
      while (r < H and S[r][c] == '#') r++;
      ll st = r;
      while (r < H and S[r][c] == '.') r++;
      ll n = r - st;
      REP(i, st, r) Vert[i][c] = n;
    }
  }
  DLOGK(Horiz);
  DLOGK(Vert);
  ll K = 0;
  REP(r, 0, H) REP(c, 0, W) if (S[r][c] == '.') K++;
  Fp ans = 0;
  REP(r, 0, H) REP(c, 0, W) {
    if (S[r][c] == '#') continue;
    ll m = Horiz[r][c] + Vert[r][c] - 1;
    ans += (power<Fp>(Fp(2), m) - 1) * power<Fp>(Fp(2), K - m);
  }
  cout << ans << endl;
