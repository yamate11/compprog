...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, nbr, dec=1) [eSyv3acX]
  ...
  // @End [eSyv3acX]
  vector<ll> leaf;
  vector<ll> branching(N);
  vector<ll> deleted(N);
  REP(i, 0, N) {
    branching[i] = SIZE(nbr[i]);
    if (SIZE(nbr[i]) == 1) {
      leaf.push_back(i);
    }
  }
  auto getNext = [&](ll x) -> ll {
    while (true) {
      ll y = nbr[x].back();
      if (not deleted[y]) return y;
      nbr[x].pop_back();
    }
  };
