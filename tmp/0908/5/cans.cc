...
struct DoublingFRel { // from functional relation
  ...
  DoublingFRel(int n_, long long lim, auto frel) : n(n_) { _init(lim, frel); }

  int val(int i, long long x) { // Calculates frel^{(x)}(i).  Should be x <= lim.
...
struct DoublingCum {
  ...
  T val(int i, long long x) { // the monoid sum of x objs from i.  i.e. from i to i + x - 1.
    T ret = T();
...
int main(int argc, char *argv[]) {
  ...
  auto d1 = DoublingFRel(N, K, [&](int i) -> int { return (i + A[i]) % N; });
  auto d2 = DoublingCum<ll>(d1, [&](int i) -> ll { return A[i]; });
  DLOGK(d1.tbl);
  DLOGK(d2.tbl);
  cout << d2.val(0, K) << endl;
  return 0;
}
