...
int main(/* int argc, char *argv[] */) {
  ...
  ll ans = 0;
  auto dfs = [&](auto rF, BrdIdx bi) -> pll {
    DLOGKL("in", bi.r, bi.c);
    ll b = 0, w = 0;
    if (brd.at(bi) == '#') b = 1;
    else                   w = 1;
    for (auto dd : BrdIdx::nbr4) {
      BrdIdx bn = bi + dd;
      if (brd.in(bn) and not visited.at(bn) and brd.at(bn) != brd.at(bi)) {
        visited.at(bn) = 1;
        auto [bb, ww] = rF(rF, bn);
        b += bb;
        w += ww;
    ...
    DLOGKL("   out", bi.r, bi.c, b, w);
    return pll(b, w);
  };

  REP(i, H) REP(j, W) {
    BrdIdx bi(i, j);
    if (not visited.at(bi)) {
      visited.at(bi) = 1;
      auto [b, w] = dfs(dfs, bi);
      ans += b * w;
    }
