...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl(C + 1, vector(D + 1, optional<Fp>()));
  auto f = [&](auto rF, ll x, ll y) -> Fp {
    auto& ret = tbl[x][y];
    if (not ret) {
      if (x == A and y == B) ret = Fp(1);
      else if (x == A) ret = A * rF(rF, x, y - 1);
      else if (y == B) ret = B * rF(rF, x - 1, y);
      else {
        Fp t = rF(rF, x, y - 1) * x;
        Fp s = rF(rF, x - 1, y) * y;
        Fp u = rF(rF, x - 1, y - 1) * (x - 1) * (y - 1);
        ret = t + s - u;
      }
    }
    return *ret;
