...
int main(/* int argc, char *argv[] */) {
  ...
  auto len = [&](double t, auto& A, auto& B) -> double {
    double cur = 0;
    double s = 0;
    REP(i, 0, N) {
      double tt = (double)(A[i]) / B[i];
      if (cur + tt >= t) {
        DLOGK(t, A, B, s);
        return s + (t - cur) * B[i];
      }
  ...
  auto check = [&](double t) -> bool {
    return len(t, A, B) + len(t, C, D) >= (double)tot;
  };
  double t = binsearch_r<double>(check, 1e9, 0, 1e-6);
  cout << len(t, A, B) << endl;
