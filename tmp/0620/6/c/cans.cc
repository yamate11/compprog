...
// @@ !! LIM(board f:updMaxMin f:intDiv)
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, H) {
    string s; cin >> s;
    REP(j, W) {
      if (s[j] == 'S') { start.r = i; start.c = j; brd.at(i, j) = '.'; }
      else { brd.at(i, j) = s[j]; }
    }
  }
  const ll big = 1e18;
  Board<ll> dist(H, W, big);
  auto toBorder = [&](BrdIdx bi) -> ll {
    return divCeil(min((ll)bi.r, min((ll)bi.c, min(H - 1 - bi.r, W - 1 - bi.c))), K);
  };
  dist.at(start) = 0;
  using sta = pair<ll, BrdIdx>;
  queue<sta> que;
  que.emplace(0, start);
  ll ans = 1 + toBorder(start);
  while (not que.empty()) {
    auto [d, bi] = que.front(); que.pop();
    for (auto dir : BrdIdx::nbr4) {
      auto bn = bi + dir;
      if (brd.at(bn) == '#') continue;
      if (dist.at(bn) < big) continue;
      dist.at(bn) = d + 1;
      ans = min(ans, 1 + toBorder(bn));
      if (d + 1 + 1 <= K) que.emplace(d + 1, bn);
    }
  }
  cout << ans << endl;
