...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](auto rF, ll N, ll M, auto& mp) -> ll {
    DLOGKL("in f", N, mp);
    ll cmin = 1e18;
    ll amin = -1;
    for (auto [a, c] : mp) {
    ...
    ll g = gcd(N, amin);
    if (g == 1) return (N - 1) * cmin;
    ll bsize = N / g;
    ll cost1 = cmin * (bsize - 1) * g;
    map<ll, ll> newmp;
    for (auto [a, c] : mp) {
      ll gg = gcd(a, g);
      if (gg == g) continue;
      ll x = min(gg, g - gg);
      newmp[x] = min(c, mapget(newmp, x, 1e18));
    }
    if (newmp.empty()) return -1;
    ll cost2 = rF(rF, g, M, newmp);
    if (cost2 < 0) return -1;
    return cost1 + cost2;
