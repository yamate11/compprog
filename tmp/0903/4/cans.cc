...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  IPoint S, T; cin >> S >> T;
  // @InpMVec(N, ((P, type=IPoint), R)) [N3M1pZGv]
  auto P = vector(N, IPoint());
  auto R = vector(N, ll());
  for (int i = 0; i < N; i++) {
    IPoint v1; cin >> v1; P[i] = v1;
    ll v2; cin >> v2; R[i] = v2;
  }
  // @End [N3M1pZGv]
  ll sidx = -1, tidx = -1;
  auto dist2 = [&](IPoint u, IPoint v) -> ll {
    IPoint w = u - v;
    return w.x * w.x + w.y * w.y;
  };
  auto pon = [&](IPoint u, IPoint c, ll r) -> bool { return dist2(u, c) == r * r; };
  REP(i, 0, N) {
    if (pon(S, P[i], R[i])) sidx = i;
    if (pon(T, P[i], R[i])) tidx = i;
  }
  assert(sidx >= 0 and tidx >= 0);
  UnionFind uf(N);
  auto cross = [&](ll i, ll j) -> bool {
    IPoint u = P[i], v = P[j];
    ll ur = R[i], vr = R[j];
    ll d2 = dist2(u, v);
    return (ur - vr) * (ur - vr) <= d2 and d2 <= (ur + vr) * (ur + vr);
  };
  REP(i, 0, N) REP(j, i + 1, N) if (cross(i, j)) uf.merge(i, j);
  if (uf.leader(sidx) == uf.leader(tidx)) {
    cout << "Yes\n";
  }else {
    cout << "No\n";
  }
