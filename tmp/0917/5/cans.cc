...
int main(/* int argc, char *argv[] */) {
  ...
  auto containDigit = [&](ll c) -> bool {
    return EXISTS(d, 0, 10, appear[d] and uf.leader(d) == c);
  };

  vector<ll> leaders;
  REP(c, 10, 36) if (appear[c] and uf.leader(c) == c and not containDigit(c)) leaders.push_back(c);
  ll ans = 1;
  for (ll a : leaders) ans *= 10;
  if (f(S[0]) >= 10 and f(T[0]) >= 10 and not containDigit(uf.leader(f(S[0])))) ans = ans / 10 * 9;
  cout << ans << endl;
