...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpNbrList(N, M, nbr, dec=1, read=len) [FGaDjREO]
  struct nbr_t {
    int nd;
    ll len;
    nbr_t() {}
  ...
  REP(i, 0, N) {
    ...
    while (not pque.empty()) {
      ...
      if (dist[i][nd] < d) continue;
      for (auto [p, len] : nbr[nd]) {
        ll newD = d + len;
        if (newD >= dist[i][p]) continue;
        dist[i][p] = newD;
        pque.emplace(newD, p);
      }
    }
  ...
  REP(i, 0, N) {
    ...
    for (ll d : dist[i]) {
      hare.push_back(d * T);
      tortoise.push_back(d * R);
    }
    DLOGK(i, dist[i], hare, tortoise);
    ll t = 1;
    REP(j, 1, N) {
      for (; t < N and tortoise[t] < hare[j]; t++);
      ans += t - 1;
      if (T > R) ans--;
    }
  }
