...
int main(/* int argc, char *argv[] */) {
  ...
  Fp ss = 1, sd = 0, ds = 0, dd = 0;
  REP(i, 0, K) {
    Fp pss = ss, psd = sd, pds = ds, pdd = dd;
    ss = psd + pds;
    sd = pss * (W - 1) + psd * (W - 2) + pdd;
    ds = pss * (H - 1) + pds * (H - 2) + pdd;
    dd = psd * (H - 1) + pds * (W - 1) + pdd * (H - 2 + W - 2);
    DLOGK(ss, sd, ds, dd);
  }
  Fp ans = -1;
  if (x1 == x2 and y1 == y2) ans = ss;
  else if (x1 == x2) ans = sd / (W - 1);
  else if (y1 == y2) ans = ds / (H - 1);
  else ans = dd / ((W - 1) * (H - 1));
  cout << ans << endl;
