...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [UmgWWfrP]
  // @InpMVec(K, ((S, dec=1), (T, dec=1))) [vW5RKtxX]
  auto S = vector(K, ll());
  auto T = vector(K, ll());
  for (int i = 0; i < K; i++) {
  ...
  // @End [vW5RKtxX]
  CoordCompr cc;
  REP(d, 0, D) { cc.add(L[d]); cc.add(R[d]); cc.add(R[d] - 1); }
  REP(k, 0, K) { cc.add(S[k]); cc.add(T[k]); }
  ll sz = cc.size();

  vector<ll> vec(sz);
  REP(i, 0, sz) vec[i] = i;
  auto stA = make_seg_tree_lazy<ll, ll>(LLONG_MIN, LLONG_MIN, mymax, mymax, mymax, vec);
  auto stB = make_seg_tree_lazy<ll, ll>(LLONG_MAX, LLONG_MAX, mymin, mymin, mymin, vec);
  auto stC = stB;
  auto stD = stA;
  vector<ll> ans(K, -1LL);
  REP(d, 0, D) {
    ll l = cc.c(L[d]);
    ll r = cc.c(R[d] - 1);
    ll p = stB.query(l, l + 1);
    stA.update(p, r + 1, r);
    stB.update(p, r + 1, p);
    ll q = stD.query(r, r + 1);
    stC.update(l, q + 1, l);
    stD.update(l, q + 1, q);
    REP(k, 0, K) {
      if (ans[k] < 0) {
        ...
        ll b = cc.c(T[k]);
        if (a <= b) {
          if (stA.query(a, a + 1) >= b) ans[k] = d + 1;
        }else {
          if (stC.query(a, a + 1) <= b) ans[k] = d + 1;
        }
      }
    }
  }
