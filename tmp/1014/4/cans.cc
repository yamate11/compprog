...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> brd(H, W, '@');
  cin >> brd;
  Board<char> phase1(H, W, '#');
  REP(r, 0, H) REP(c, 0, W) {
    BrdIdx bi(r, c);
    if (brd.at(bi) == '#') continue;
    for (auto dir : BrdIdx::nbr9) {
      auto bd = bi + dir;
      if (phase1.in(bd)) phase1.at(bd) = '.';
    }
  }
  Board<char> phase2(H, W, '.');
  REP(r, 0, H) REP(c, 0, W) {
    BrdIdx bi(r, c);
    if (phase1.at(bi) == '.') continue;
    for (auto dir : BrdIdx::nbr9) {
      auto bd = bi + dir;
      if (phase2.in(bd)) phase2.at(bd) = '#';
    }
  }
  DLOGK(phase1);
  DLOGK(phase2);
  if (FORALL(r, 0, H, FORALL(c, 0, W, brd.at(r, c) == phase2.at(r, c)))) {
    cout << "possible\n";
    REP(r, 0, H) {
      REP(c, 0, W) cout << phase1.at(r, c);
      cout << "\n";
    }
  }else {
    cout << "impossible\n";
