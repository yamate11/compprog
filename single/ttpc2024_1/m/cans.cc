...
// @@ !! LIM(rollingHash cartesianTree debug segTree sortedIndex)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, A, dec=1) [6dUJ6KN3]
  auto A = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; A[i] = v; }
  // @End [6dUJ6KN3]
  ll Q; cin >> Q;
  // @InpMVec(Q, ((L, dec=1), (R, dec=0))) [ZtA5IxQP]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  vector<ll> revA(N);
  REP(i, 0, N) revA[A[i]] = i;

  CartesianTree ct(A);
  string s0;
  vector pos(3, vector<ll>(N));
  auto cts = [&](auto rF, ll x) -> void {
    if (x < 0) return;
    pos[0][x] = ssize(s0);
    s0 += '(';
    rF(rF, ct.left[x]);
  ...
  auto st_min = make_seg_tree(big, mymin, A);
  vector<ll> M(Q);
  REP(i, 0, Q) M[i] = revA[st_min.query(L[i], R[i])];

  using DAT = pair<RHVal, ll>;
  auto myadd = [&](DAT a, DAT b) -> DAT {
    auto [ha, la] = a;
    auto [hb, lb] = b;
    if (la == 0) return b;
    if (lb == 0) return a;
    return DAT(rh.hash_concat(ha, hb, lb), la + lb);
  };
  vector<DAT> init_vec(3*N, DAT(0, 0));
  auto st1 = make_seg_tree(DAT(0, 0), myadd, init_vec);

  vector<RHVal> LeftHash(Q);

  auto ordL = sortedIndex(L, greater<ll>());
  ll idx = N - 1;
  REP(ii, 0, Q) {
    ll i = ordL[ii];
    while (L[i] <= idx) {
      st1.rs(pos[0][idx]) = DAT('(', 1);
      st1.rs(pos[1][idx]) = DAT(',', 1);
      st1.rs(pos[2][idx]) = DAT(')', 1);
      idx--;
    }
    if (L[i] == M[i]) {
      LeftHash[i] = 0;
      ...
      ll lo = pos[0][M[i]] + 1;
      ll hi = pos[1][M[i]];
      LeftHash[i] = st1.query(lo, hi).first;
    }
  }
  using stb = pair<RHVal, RHVal>;
  safe_uset<stb> ss;
  auto st2 = make_seg_tree(DAT(0, 0), myadd, init_vec);
  auto ordR = sortedIndex(R);
  idx = 0;
  REP(ii, 0, Q) {
    ll i = ordR[ii];
    while (idx < R[i]) {
      st2.rs(pos[0][idx]) = DAT('(', 1);
      st2.rs(pos[1][idx]) = DAT(',', 1);
      st2.rs(pos[2][idx]) = DAT(')', 1);
      idx++;
    }
    RHVal righthash;
    if (R[i] == M[i]) {
      ...
      ll lo = pos[1][M[i]] + 1;
      ll hi = pos[2][M[i]];
      righthash = st2.query(lo, hi).first;
    }
    DLOGK(i, LeftHash[i], righthash);
    ss.emplace(LeftHash[i], righthash);
  }
  cout << ssize(ss) << endl;
