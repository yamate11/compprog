...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) REP(j, 0, N) REP(k, 0, N) {
    if (i == x0 or k == y0) continue;
    if (not f(i, j, k)) {
      DLOGKL("premature", i, j, k);
      cout << 0 << endl;
      return 0;
    }
  }
  ...
  REP(z, 0, N) {
    auto check = [&]() -> bool {
      REP(p, 0, N) REP(q, 0, N) {
        if (not f(x0, p, q)) return false;
        if (not f(p, q, y0)) return false;
      }
      return true;
    };
    A[x0][y0] = z;
