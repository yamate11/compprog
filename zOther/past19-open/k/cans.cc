...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl_incl(N, vector<ll>(K + 1, -1LL));
  vector tbl_excl(N, vector<ll>(K + 1, -1LL));
  auto dfs = [&](auto rF, ll nd) -> void {
    tbl_incl[nd][1] = A[nd];
    tbl_excl[nd][0] = 0;
    for (ll c : tr.children(nd)) {
      auto prev_incl = move(tbl_incl[nd]);
      auto prev_excl = move(tbl_excl[nd]);
      tbl_incl[nd] = vector<ll>(K + 1, -1LL);
      tbl_excl[nd] = vector<ll>(K + 1, -1LL);
      rF(rF, c);
      REP(i, 0, K + 1) REP(j, 0, K + 1) {
        if (i + j <= K and prev_incl[i] >= 0 and tbl_excl[c][j] >= 0) {
          updMax(tbl_incl[nd][i + j], prev_incl[i] + tbl_excl[c][j]);
        }
        if (i + j <= K and prev_excl[i] >= 0 and (tbl_excl[c][j] >= 0 or tbl_incl[c][j] >= 0)) {
          updMax(tbl_excl[nd][i + j], prev_excl[i] + max(tbl_excl[c][j], tbl_incl[c][j]));
        }
      }
    }
  };
  dfs(dfs, 0);
  if (tbl_incl[0][K] < 0 and tbl_excl[0][K] < 0) {
    cout << -1 << "\n";
  }else {
    cout << max(tbl_incl[0][K], tbl_excl[0][K]) << endl;
  }
