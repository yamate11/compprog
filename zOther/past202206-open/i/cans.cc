...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> brd(H, W, '#');
  cin >> brd;
  BrdIdx snk, apkg, goal;
  REP(i, 0, H) REP(j, 0, W) {
    if (brd.at(i, j) == 's') {
      snk = BrdIdx(i, j);
      brd.at(i, j) = '.';
    } else if (brd.at(i, j) == 'a') {
      apkg = BrdIdx(i, j);
      brd.at(i, j) = '.';
    }else if (brd.at(i, j) == 'g') {
      goal = BrdIdx(i, j);
      brd.at(i, j) = '.';
    }
  }
  ...
  queue<sta> que;
  dist[snk.r][snk.c][apkg.r][apkg.c] = 0;
  que.emplace(snk, apkg);
  while (not que.empty()) {
    auto [s, a] = que.front(); que.pop();
    ll d = dist[s.r][s.c][a.r][a.c];
    for (auto dir : BrdIdx::nbr4) {
      ...
      if (bi == a) {
        ...
        if (brd.at(bii) == '.') {
          ...
          ll dd = dist[bi.r][bi.c][bii.r][bii.c];
          if (dd == big) {
            dist[bi.r][bi.c][bii.r][bii.c] = d + 1;
            que.emplace(bi, bii);
          }
        }
      }else if (brd.at(bi) == '.') {
        ll dd = dist[bi.r][bi.c][a.r][a.c];
        if (dd == big) {
          dist[bi.r][bi.c][a.r][a.c] = d + 1;
          que.emplace(bi, a);
        }
      }
    }
