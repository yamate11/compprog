...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](string x) -> vector<vector<ll>> {
    ll n = ssize(x);
    vector ret(n, vector<ll>(26, big));
    vector<ll> p(26, big);
    REPrev(i, n - 1, 0) {
      p[x[i] - 'a'] = i;
      REP(d, 0, 26) ret[i][d] = p[d];
    }
    return ret;
  };
  ...
  auto g = [&](string x, const auto& pi) -> Fp {
    ll n = ssize(x);
    vector<Fp> tbl(n);
    REP(d, 0, 26) {
      ll q = pi[0][d];
  ...
  Fp com = 0;
  REP(i, 0, N) REP(j, 0, M) com += tbl[i][j];
  DLOGK(ns, nt, com);
  cout << ns + nt - com << endl;
