...
// @DefStruct(lrc, (L, R, C), ord=(L, (R, True))) [Rj1X8NaD]
struct lrc_t {
  ll L;
  ll R;
  ll C;
  // lrc_t() {}
  lrc_t(ll L_ = ll(), ll R_ = ll(), ll C_ = ll()) : L(L_), R(R_), C(C_) {}
  friend istream& operator>>(istream& istr, lrc_t& t) {
    istr >> t.L >> t.R >> t.C;
    return istr;
  }
  friend ostream& operator<<(ostream& ostr, const lrc_t& t) {
    ostr << "(" << t.L << ", " << t.R << ", " << t.C << ")";
    return ostr;
  }
  bool operator<(const lrc_t& o) const {
...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [ZoS50JXK]
  // @InpVec(M, lrc, type=lrc_t) [aUKirlcT]
  auto lrc = vector(M, lrc_t());
  for (int i = 0; i < M; i++) { lrc_t v; cin >> v; lrc[i] = v; }
  // @End [aUKirlcT]
  REP(i, 0, M) lrc[i].L--;
  sort(ALL(lrc));
  ...
  auto st = make_seg_tree(big, mymin, vector<ll>(N + 1, big));
  ll past = 0;
  ll m = 0;
  REP(i, 0, N) {
    past = min(past, st.at(i));
    for (; m < M and lrc[m].L == i; m++) {
      auto [l0, r0, c0] = lrc[m];
      ll vmin = st.query(l0, r0);
      vmin = min(vmin, past);
      st.rs(r0) = min(st.at(r0), vmin + c0);
    }
    past += A[i];
  }
  ll ans = min(past, st.at(N));
  cout << tot - ans << endl;
  return 0;
}
