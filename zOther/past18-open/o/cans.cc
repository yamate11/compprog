...
// @@ !! LIM(segTree ipoint debug cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ll N, Q; cin >> N >> Q;
  // @InpMVec(N, (X, (D, type=char))) [uzKyLKE9]
  auto X = vector(N, ll());
...
int body(istream& cin, ostream& cout) {
  ...
  vector<IPoint> vA, vB;
  REP(i, 0, N) {
    if (X[i] <= X[0] and D[i] == 'R') vA.emplace_back(X[i], -1LL);
    if (X[0] <  X[i] and D[i] == 'L') vB.emplace_back(X[i], -1LL);
  }

  vector<ll> QX(Q);
  vector<char> QD(Q);
  vector<ll> QL(Q, -1LL), QR(Q, -1LL);
  REP(t, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll x; cin >> x;
      ...
      QX[t] = x;
      QD[t] = d;
      if (x < X[0] and d == 'R') vA.emplace_back(x, t);
      if (X[0] < x and d == 'L') vB.emplace_back(x, t);
    }else if (tp == 2) {
      ll l, r; cin >> l >> r; l--; r--;
      QL[t] = l; QR[t] = r;
    }else assert(0);
  }
  sort(ALL(vA), greater<IPoint>());
  sort(ALL(vB), less<IPoint>());
  vector rev(Q, pll{-1, -1});
  REP(i, 0, ssize(vA)) {
    auto [x, t] = vA[i];
    if (t >= 0) rev[t] = {0, i};
  }
  ...
  vector<IPoint> initA(ssize(vA));
  vector<IPoint> initB(ssize(vB));
  REP(i, 0, ssize(vA)) {
    auto [x, t] = vA[i];
    initA[i] = IPoint(t < 0 ? 1 : 0, t < 0 ? x : 0);
  }
  REP(i, 0, ssize(vB)) {
    auto [x, t] = vB[i];
    initB[i] = IPoint(t < 0 ? 1 : 0, t < 0 ? x : 0);
  }
  DLOGK(vA, initA, vB, initB);
  auto segA = make_seg_tree(IPoint(), plus<IPoint>(), initA);
  auto segB = make_seg_tree(IPoint(), plus<IPoint>(), initB);
  REP(t, 0, Q) {
    if (QL[t] < 0) {  // tp == 1
      auto [side, idx] = rev[t];
      if (side == 0)      segA.rs(idx) = IPoint(1, QX[t]);
      else if (side == 1) segB.rs(idx) = IPoint(1, QX[t]);
      DLOGK(t, segA, segB);
    }else {           // tp == 2
      ll l = QL[t], r = QR[t];
      ...
      ll bL = (l + 1) / 2;
      ll bR = (r + 1) / 2;
      auto check_bL = [&](IPoint p) -> bool { return p.x >= bL; };
      ll bL0 = segB.binsearch_r_from(check_bL, 0);
      auto check_bR = [&](IPoint p) -> bool { return p.x <= bR; };
      ll bR0 = segB.binsearch_r_until(check_bR, 0);
      DLOGK(t, l, r, bL0, bR0, segB.query(bL0, bR0));
      ll sum_B = segB.query(bL0, bR0).y * 2;
      if (l % 2 != 0) {
        sum_B += segB.at(bL0 - 1).y;
      }
      if (r % 2 == 0) {
        sum_B += segB.at(bR0).y;
      }
      ...
      ll aL = (l + 2) / 2;
      ll aR = (r + 2) / 2;
      auto check_aL = [&](IPoint p) -> bool { return p.x >= aL; };
      ll aL0 = segA.binsearch_r_from(check_aL, 0);
      auto check_aR = [&](IPoint p) -> bool { return p.x <= aR; };
      ll aR0 = segA.binsearch_r_until(check_aR, 0);
      ll sum_A = segA.query(aL0, aR0).y * 2;
      if (l % 2 == 0) sum_A += segA.at(aL0 - 1).y;
      if (r % 2 != 0) sum_A += segA.at(aR0).y;
