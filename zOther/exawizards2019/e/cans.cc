...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Fp> ipow2(tot + 1);
  Fp x = 1 / power<Fp>(2, tot);
  for (ll i = tot; i >= 0; i--) {
    ipow2[i] = x;
    x *= 2;
  }

  auto calc_p = [&](ll k, auto& vec) {
    for (ll i = 0; i <= tot; i++) {
      if (i < k)       vec[i] = 0;
      else if (i == k) vec[i] = Fp(1);
      else             vec[i] = 2 * vec[i-1] + cmb.comb(i-1, i-k);
    }
    for (ll i = 0; i <= tot; i++) {
      vec[i] = vec[i] * ipow2[i];
    }
  };
  vector<Fp> pb(tot + 1), pw(tot + 1);
  calc_p(B, pb);
  calc_p(W, pw);
  DLOGK(pb);
  DLOGK(pw);
  Fp i2 = Fp(1) / 2;
  for (ll i = 1; i <= tot; i++) {
    Fp p = pw[i-1] + (Fp(1) - pb[i-1] - pw[i-1]) * i2;
    cout << p << "\n";
  }
