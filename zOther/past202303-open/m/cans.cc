...
int main(/* int argc, char *argv[] */) {
  ...
  using DAT = pll;
  auto mymax = [&](DAT px, DAT py) -> DAT {
    auto [vx, ix] = px;
    auto [vy, iy] = py;
    if (vx >= vy) return px;
    else return py;
  };
  vector init_vec(M, DAT());
  REP(i, 0, M) init_vec[i] = DAT(B[i], i);
  auto st = make_seg_tree(DAT(0LL, M), mymax, init_vec);
  ll big = 1e18;
  map<ll, ll> mp{{0, -1}, {big, M}};
  mp[B[0]] = 0;
  {
    ll prev = B[0];
    REP(i, 1, M) {
  ...
  REP(i, 0, N) {
    ...
    ll new_weight = st.at(m).first - A[i];
    st.rs(m) = DAT(new_weight, m);
    auto [w_prev, m_prev] = *(std::prev(it));
    auto [w_next, m_next] = *(std::next(it));
    mp.erase(it);
    if (new_weight <= w_prev) {
      new_weight = w_prev;
    }else {
      mp[new_weight] = m;
    }
    ll m_lim = m_next;
    while (m + 1 < m_lim) {
      auto [ww, ii] = st.query(m + 1, m_lim);
      if (ww <= new_weight) break;
      mp[ww] = ii;
      m_lim = ii;
    }
  }
