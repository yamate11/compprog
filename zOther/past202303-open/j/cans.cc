...
int main(/* int argc, char *argv[] */) {
  ...
  RollingHash rh;
  vector<u64> HA(2*W), HB(W);
  REP(j, 0, W) {
    HA[j] = rh.hashes(A[j]).get();
    HA[W + j] = HA[j];
    HB[j] = rh.hashes(B[j]).get();
  }
  
  RollingHashGen<u64, nullptr_t> rh2(0, 1000);
  u64 val_j = rh2.hashes(HB).get();
  auto hs = rh2.hashes(HA);
  REP(i, 0, W) {
    if (hs.get(i, W) == val_j) {
      cout << "Yes\n";
      return 0;
