...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpVec(N, R) [pUBo9i8q]
  auto R = vector(N, ll());
  for (int i = 0; i < N; i++) { int v; cin >> v; R[i] = v; }
  // @End [pUBo9i8q]
  ...
  double min_th = M_PI / (2 * N);

  auto check = [&](double x) -> bool {
    double sum = 0.0;
    for (auto [p, n] : mp) {
      if (p * cos(min_th) < x) sum += min_th * n;
      else {
        double th = acos(x / p);
        sum += th * n;
      }
    }
    return sum < M_PI * 2.0 / 3.0;
  };
  double prod = binsearch_r<double>(check, 1e7, 0.1, 1e-9, true);
  double ans = - 0.5 * R[0] * R[N - 1] * sin( M_PI * 2.0 / 3.0 );
  for (auto [p, n] : mp) {
    if (p * cos(min_th) < prod) {
      ans += 0.5 * p * sin(min_th) * n;
      DLOGKL("(min)", p, min_th / M_PI);
    }
    else {
      double th = acos(prod / p);
      ans += 0.5 * p * sin(th) * n;
      DLOGKL("(ord)", p, th / M_PI);
    }
  }
  cout << ans << endl;
