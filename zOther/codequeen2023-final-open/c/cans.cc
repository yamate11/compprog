...
int main(/* int argc, char *argv[] */) {
  ...
  vector onST(N, false);
  for (ll n = T; n >= 0; n = tr.parent(n)) onST[n] = true;
  vector<ll> ans(N);
  auto dfs = [&](auto rF, ll nd, ll bd) -> void {
    ans[nd] = tr.depth(nd) - bd + 1;
    for (ll cld : tr.children(nd)) {
      ll newBD = onST[cld] ? tr.depth(cld) : bd;
      rF(rF, cld, newBD);
    }
  };
  dfs(dfs, S, 0);
