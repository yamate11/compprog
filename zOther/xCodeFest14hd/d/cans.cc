...
int main(int argc, char *argv[]) {
  ...
  vector tbl(N, vector(2*lim + 1, Fp(0)));
  vector acc(N, vector(2*lim + 1, Fp(0)));
  ll prevP = -1;
  ll prevL = -1;
  for (ll i = 0; i < N; i++) {
    ll p, l; cin >> p >> l;
    if (i == 0) {
      for (ll x = -l; x <= l; x++) tbl.at(i).at(enc(x)) = 1;
    }else {
      for (ll x = -l; x <= l; x++) {
	ll y = x + (p - prevP);
	Fp w = 0;
	if (y > prevL) {
	  w = acc.at(i - 1).at(enc(prevL));
	}else if (y - 1 >= - prevL) {
	  w = acc.at(i - 1).at(enc(y - 1));
	}else {
	  w = 0;
	}
	tbl.at(i).at(enc(x)) = w;
    ...
    for (ll x = -l; x <= l; x++) {
      if (x == -l) acc.at(i).at(enc(x)) = tbl.at(i).at(enc(x));
      else acc.at(i).at(enc(x))
	     = acc.at(i).at(enc(x) - 1) + tbl.at(i).at(enc(x));
    }
    prevP = p;
    prevL = l;
  }
  cout << acc.at(N-1).at(enc(prevL)) << endl;
