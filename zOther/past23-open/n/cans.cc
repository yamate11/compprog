...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> B(N);
  REP(i, 0, N) B[i] = N - sa[i];
  vector<ll> C(N + 1, 0LL);
  REP(i, 0, N) C[i + 1] = C[i] + B[i];

  int big = N + 10;
  auto mymin = [&](int x, int y) -> int { return min(x, y); };
  auto st = make_seg_tree(big, mymin, lcp);
  ...
  for (ll m = 0; true; m++) {
    ...
    if (B[i0] == m) i0++;
    if (i1 == i0 + 1) {
      cout << S.substr(sa[i0], m + K + 1) << endl;
      return 0;
    }
    while (true) {
      assert(i0 < i1);
      auto check = [&](ll y) -> bool { return y <= m; };
      ll new_i0 = st.binsearch_r_from(check, i0);
      ll e = C[new_i0] - C[i0] - (m * (new_i0 - i0));
      if (K < e) {
        i1 = new_i0;
        break;
      }
