...
int main(/* int argc, char *argv[] */) {
  ...
  using DAT = vector<ll>;
  DAT zero(n_alph, 0LL);
  auto myadd = [&](const DAT& x, const DAT& y) -> DAT {
    DAT ret(n_alph, 0LL);
    REP(i, 0, n_alph) ret[i] = x[i] + y[i];
    return ret;
  };
  vector<DAT> init(N, DAT(26, 0LL));
  REP(i, 0, N) init[i][S[i] - 'a'] = 1;
  auto st = make_seg_tree(zero, myadd, init);
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll p; cin >> p; p--;
      ...
      ll l, r; cin >> l >> r; l--;
      auto vec = st.query(l, r);
      ll ans = *ranges::max_element(vec);
      cout << ans << "\n";
    }else assert(0);
  }
