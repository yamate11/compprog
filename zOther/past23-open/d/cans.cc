...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&]() -> bool {
    REP(i, 0, H) REP(j, 0, W - 1) {
      if (brd.at(i, j) == c1 and brd.at(i, j + 1) == c2) return true;
      if (brd.at(i, j) == c2 and brd.at(i, j + 1) == c1) return true;
    }
    REP(i, 0, H - 1) REP(j, 0, W) {
      if (brd.at(i, j) == c1 and brd.at(i + 1, j) == c2) return true;
      if (brd.at(i, j) == c2 and brd.at(i + 1, j) == c1) return true;
    }
    return false;
  };
  cout << (check() ? "Yes\n" : "No\n");
