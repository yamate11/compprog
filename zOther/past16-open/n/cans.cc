...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, Q) {
    ...
    ll x; cin >> x;
    auto [cur, mnum] = mp[x];
    if (c == '+') ops.emplace_back( 1, x, cur++);
    else          ops.emplace_back(-1, x, --cur);
    mnum = max(mnum, cur);
    mp[x] = pll(cur, mnum);
  }
  safe_umap<ll, ll> mp2;
  ...
  vector<Fp> kpow(idx2 + 1);
  kpow[0] = Fp(1);
  REP(i, 1, idx2 + 1) kpow[i] = kpow[i - 1] * Fp(K);
  using stb = pair<Fp, ll>;
  auto myadd = [&](stb x, stb y) -> stb {
    auto [x_val, x_len] = x;
    auto [y_val, y_len] = y;
    return stb(x_val + kpow[x_len] * y_val, x_len + y_len);
  };
  vector<stb> init_dat(idx2);
  REP(i, 0, idx2) init_dat[i] = stb(0, 0);
  auto st = make_seg_tree(stb(0, 0), myadd, init_dat);
  REP(_q, 0, Q) {
    auto [op, x, j] = ops[_q];
    ll i = mp2[x] + j;
    if (op > 0) st.rs(i) = stb(Fp(x), 1);
    else        st.rs(i) = stb(0, 0);
    auto [v, l] = st.query(0, idx2);
    cout << v << "\n";
  }
