...
// @@ !! LIM(power mod segTree debug f:perfmeas)
...
using OP = ll;
DAT plus_DAT(DAT x, DAT y) { return {x.first + y.first, x.second + y.second}; }
DAT unit_DAT() { return {Fp(0), 0}; }
OP compos(OP x, OP y) { return x + y; }
DAT apply(OP u, DAT yn) {
    auto [y, n] = yn;
    if (u == 0) {
      return {y, n};
    }else if (u > 0) {
...
int main(/* int argc, char *argv[] */) {
  ...
  ll Q; cin >> Q >> K;
  // @InpMVec(Q, ((op, type=char), X)) [sP0rqbwF]
  ...
  vector<ll> apidx(Q);
  {
    ...
    REP(q, 0, Q) {
      ll x = X[q];
      if (op[q] == '+') {
        apidx[q] = below[x] + num[x] - 1 - curPos[x];
        curPos[x]++;
      }else if (op[q] == '-') {
        apidx[q] = below[x] + num[x] - 1 - curNeg[x];
        curNeg[x]++;
      }
    }
  }
  ...
  vector<DAT> init_vec(totA);

  lazy_segtree<DAT, plus_DAT, unit_DAT, OP, apply, compos, unit_OP> st(init_vec);


#if DEBUG
    vector<DAT> tmp0(totA);
    REP(i, 0, totA) tmp0[i] = st.get(i);
    DLOGK(tmp0);
#endif

  double t4 = get_time_sec();

  double ft02 = 0,  ft03 = 0,  ft04 = 0,  ft05 = 0,  ft07 = 0, ft08 = 0, ft10 = 0;

  REP(q, 0, Q) {
    ll x = X[q];
    double et01, et02, et03, et04, et05, et06, et07, et08;
    if (op[q] == '+') {
      et01 = get_time_sec();
      auto [_d, bn] = st.prod(0, apidx[q]);
      et02 = get_time_sec();
      Fp pow = power<Fp>(K, bn);
      et03 = get_time_sec();
      DLOGK(bn, pow, x);
      st.set(apidx[q], {pow * x, 1});
      et04 = get_time_sec();
      st.apply(apidx[q] + 1, totA, 1);
      et05 = get_time_sec();
...
#if DEBUG
    vector<DAT> tmp1(totA);
    REP(i, 0, totA) tmp1[i] = st.get(i);
    DLOGK(tmp1);
#endif
    double et09 = get_time_sec();
    ll ans = st.prod(0, totA).first;
    double et10 = get_time_sec();
    cout << ans << "\n";
