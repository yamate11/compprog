...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Board<ll>> len(4, Board<ll>(4, 4, 0LL));
  {
    vector<BrdIdx> orig{{0, 0}, {(int)N - 1, 0}, {(int)N -1, (int)N - 1}, {0, (int)N - 1}};
    REP(d, 0, 4) {
      auto dir = BrdIdx::nbr4[d];
      auto sft = BrdIdx(-dir.c, dir.r);
      auto st = orig[d];
      REP(x0, 0, N) {
        ll y = 0, z = 0;
        while (true) {
          while (y < N and brd.at(st + y * dir) == 'X') y++;
          if (y == N) break;
          z = y;
          while (z < N and brd.at(st + z * dir) != 'X') z++;
          while (y < z) {
            len[d].rs(st + y * dir) = z - y;
            y++;
          }
        }
        st += sft;
  ...
  REP(k, 1, N) {
    auto func = [&]() -> ll {
      ...
      queue<BrdIdx> que;
      dist[brd.enc(start)] = 0;
      que.push(start);
      while (not que.empty()) {
        BrdIdx bi = que.front(); que.pop();
        if (brd.at(bi) == 'G') {
          return dist[brd.enc(bi)];
        }
        REP(d, 0, 4) {
          if (len[d].at(bi) > k) {
            BrdIdx peer = bi + k * BrdIdx::nbr4[d];
            if (dist[brd.enc(peer)] == big) {
              dist[brd.enc(peer)] = dist[brd.enc(bi)] + 1;
              que.push(peer);
            }
          }
        }
