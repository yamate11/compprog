...
// @@ !! LIM(board f:intDiv forall debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  Board<char> brd(N, N, 'X');
  cin >> brd;
  BrdIdx start;
  // vector<BrdIdx> goals;
  REP(i, 0, N) REP(j, 0, N) {
    if (brd.at(i, j) == 'S') start = BrdIdx(i, j);
    // else if (brd.at(i, j) == 'G') goals.emplace_back(i, j);
  }
  vector<ll> ans(N);
  REP(k, 1, N) {
    ll rem_r = start.r % k;
    ll rem_c = start.c % k;
    ll sz = divCeil(N, k);
    vector nbr(sz * sz, vector<ll>());
    vector isGoal(sz * sz, false);
    auto myenc = [&](const auto& bi) -> ll { return (bi.r - rem_r) / k * sz + (bi.c - rem_c) / k; };
    ll e_start;
    REP(i, 0, sz) REP(j, 0, sz) {
      BrdIdx bi(i * k + rem_r, j * k + rem_c);
      if (brd.at(bi) == 'G') isGoal[myenc(bi)] = true;
      if (brd.at(bi) == 'S') e_start = myenc(bi);
      if (brd.at(bi) != 'X') {
        for (auto dir : BrdIdx::nbr4) {
          if (FORALL(m, 1, k + 1, brd.at(bi + m * dir) != 'X')) {
            auto bj = bi + k * dir;
            nbr[myenc(bi)].push_back(myenc(bj));
          }
        }
      }
    }
    ...
    auto f = [&]() -> ll {
      ll big = N * N + 1;
      vector<ll> dist(sz * sz, big);
      queue<ll> que;
      dist[e_start] = 0;
      que.push(e_start);
      DLOGK(e_start, dist, que);
      while (not que.empty()) {
        auto e = que.front(); que.pop();
        if (isGoal[e]) return dist[e];
        for (ll p : nbr[e]) {
          if (dist[p] == big) {
            dist[p] = dist[e] + 1;
            que.push(p);
