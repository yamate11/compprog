...
// @@ !! LIM(segTree input unordered_map mod)
...
// @DefStruct(D, ((val, Fp), len, (mbase, Fp))) [m0nzrFHN]
struct D_t {
  Fp val;
  ll len;
  Fp mbase;
  D_t(Fp val_ = Fp(), ll len_ = ll(), Fp mbase_ = Fp()) : val(val_), len(len_), mbase(mbase_) {}
  friend ostream& operator<<(ostream& ostr, const D_t& t) {
    ostr << "(" << t.val << ", " << t.len << ", " << t.mbase << ")";
    return ostr;
  }
};
// @End [m0nzrFHN]
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<pll> vec;
  REP(i, 0, Q) {
    if (OP[i] == '+') vec.emplace_back(X[i], i);
  }
  ranges::sort(vec);
  vector<ll> pos_ins(Q);
  REP(j, 0, ssize(vec)) pos_ins[vec[j].second] = j;
  safe_umap<ll, vector<ll>> pos_val;

  D_t unit_dat(0, 0, 1);
  auto add = [&](D_t a, D_t b) -> D_t { return D_t(a.val + a.mbase * b.val, a.len + b.len, a.mbase * b.mbase); };
  vector<D_t> init_vec(ssize(vec), unit_dat);
  ...
  REP(q, 0, Q) {
    if (OP[q] == '+') {
      ll i = pos_ins[q];
      st.rs(i) = D_t(X[q], 1, K);
      pos_val[X[q]].push_back(i);
    }else {
      ll i = pos_val[X[q]].back(); pos_val[X[q]].pop_back();
      st.rs(i) = unit_dat;
    }
    cout << st.query(0, ssize(vec)).val << "\n";
  }
