...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [LXqlNGVS]
  // @InpVec(M - 1, D) [JrvvchS3]
  auto D = vector(M - 1, ll());
  for (int i = 0; i < M - 1; i++) { ll v; cin >> v; D[i] = v; }
  // @End [JrvvchS3]

  safe_umap<ll, ll> revS;
  REP(i, 0, N) revS[S[i]] = i;

  ll base = 0;
  vector<ll> plus;
  REP(i, 0, M - 1) {
    auto prev_plus = move(plus);
    plus = vector<ll>();
    if (not prev_plus.empty()) {
      base += 1;
      for (ll j : prev_plus) {
        ll x = S[j] + D[i];
        auto it = revS.find(x);
        if (it != revS.end()) plus.push_back(it->second);
        x = S[j] - D[i];
        it = revS.find(x);
        if (it != revS.end()) plus.push_back(it->second);
      }
    ...
    ranges::sort(plus);
    plus.erase(unique(ALL(plus)), plus.end());
  }
  if (not plus.empty()) base++;
  cout << base << endl;
