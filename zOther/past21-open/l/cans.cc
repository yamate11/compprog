...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, C, type=Point) [b6zpyQ1I]
  auto C = vector(N, Point());
  for (int i = 0; i < N; i++) { Point v; cin >> v; C[i] = v; }
  // @End [b6zpyQ1I]
  // @InpVec(N, P, type=Real) [IeIo3qxL]
  auto P = vector(N, Real());
  for (int i = 0; i < N; i++) { Real v; cin >> v; P[i] = v; }
  // @End [IeIo3qxL]

  using sta = tuple<Real, ll, ll>;
  vector<sta> vec;
  REP(i, 0, N) vec.emplace_back(P[i], i, N);
  REP(i, 0, N) REP(j, 0, N) vec.emplace_back((C[i] - C[j]).len(), i, j);
  ranges::sort(vec);

  Real ans = 0.0;
  UnionFind uf(N + 1);
  for (auto [cost, i, j] : vec) {
