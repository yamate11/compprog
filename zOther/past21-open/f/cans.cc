...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [701OIw3v]
  ll wmax = 0;
  REP(i, 0, N) updMax(wmax, du.width(A[i]));
  ll wmax2 = 0;
  REP(i, 0, N) if (du.width(A[i]) != wmax) updMax(wmax2, (ll)du.width(A[i]));

  vector<ll> vec1, vec2;
  REP(i, 0, N) {
    ll w = du.width(A[i]);
    if (w == wmax) vec1.push_back(A[i]);
    else if (w == wmax2) vec2.push_back(A[i]);
  }
  if (ssize(vec1) == 1) {
    ll ans = 0;
    for (ll x : vec2) {
      updMax(ans, vec1[0] * du.pow(wmax2) + x);
      updMax(ans, x * du.pow(wmax) + vec1[0]);
    }
    cout << ans << endl;
  }else {
    ranges::sort(vec1, greater<ll>());
    cout << vec1[0] * du.pow(wmax) + vec1[1] << endl;
  }
