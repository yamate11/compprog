...
int main(/* int argc, char *argv[] */) {
  ...
  // auto next_dir = [&](const BrdIdx& dir) -> BrdIdx { return BrdIdx(dir.c, -dir.r); };
  auto prev_dir = [&](const BrdIdx& dir) -> BrdIdx { return BrdIdx(-dir.c,  dir.r); };
  auto idx_dir = [&](const BrdIdx& dir) -> ll {
    if (dir.r == 1 and dir.c == 0) return 0;
    if (dir.r == 0 and dir.c == -1) return 1;
    if (dir.r == -1 and dir.c == 0) return 2;
    if (dir.r == 0 and dir.c == 1) return 3;
    assert(0);
  };
  ...
  queue<sta> que;
  visited.rs(1, 1) = 15;
  for (auto dir : BrdIdx::nbr4) que.emplace(BrdIdx(1, 1), dir);
  while (not que.empty()) {
    auto [bi, dir] = que.front(); que.pop();
    DLOGK(bi, dir);
    auto bj = bi - dir;
    if (brd.at(bj) == '.' and not (visited.at(bj) >> idx_dir(dir) & 1)) {
      visited.rs(bj) = (visited.at(bj) | (1LL << idx_dir(dir)));
      que.emplace(bj, dir);
      DLOGKL("  a", bj, dir);
    }
    auto pdir = prev_dir(dir);
    if (brd.at(bi + pdir) == '#') {
      if (not (visited.at(bi) >> idx_dir(pdir) & 1)) {
        visited.rs(bi) = (visited.at(bi) | (1LL << idx_dir(pdir)));
        que.emplace(bi, pdir);
        DLOGKL("  b", bi, pdir);
      }
    }
