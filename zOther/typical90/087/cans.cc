...
// @@ !! LIM(f:updMaxMin binsearch debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto func = [&](ll x) -> ll {
    ...
    for (ll k = 0; k < N; k++) {
      for (ll i = 0; i < N; i++) {
        for (ll j = 0; j < N; j++) {
          updMin(dist[i][j], dist[i][k] + dist[j][k]);
        }
      }
    }
    ll cnt = 0;
    for (ll i = 0; i < N; i++) {
      for (ll j = i + 1; j < N; j++) if (dist[i][j] <= P) cnt++;
    }
    return cnt;
  };
  ll extr = func(P + 1);
  ...
  auto check1 = [&](ll x) -> bool { return func(x) <= K; };
  auto check2 = [&](ll x) -> bool { return func(x) >= K; };
  ll x1 = binsearch_i<ll>(check1, P + 1, 0);
  ll x2 = binsearch_i<ll>(check2, 0, P + 1);
  DLOGK(x1, x2);
  if (x1 == P + 1 || x2 == 0) {
    cout << 0 << endl;
    return 0;
