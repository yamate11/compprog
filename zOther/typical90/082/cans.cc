...
int main(/* int argc, char *argv[] */) {
  ...
  auto sum = [&](ll from, ll to) -> Fp {
    return (Fp(from) + Fp(to)) * Fp(to - from + 1) / Fp(2);
  };

  ll L, R; cin >> L >> R;
  vector<ll> tenp(19);
  vector<Fp> rec(18), acc(19);
  tenp[0] = 1;
  for (ll i = 1; i <= 18; i++) tenp[i] = tenp[i - 1] * 10;
  for (ll i = 0; i < 18; i++)
    rec[i] = sum(tenp[i], tenp[i+1] - 1) * (i + 1);
  for (ll i = 0; i < 18; i++) acc[i + 1] = acc[i] + rec[i];
  DLOGK(rec);
  DLOGK(acc);
  auto func = [&](ll x) -> Fp {
    ll i = 0;
    for (; i + 1 <= 18 && x > tenp[i + 1] - 1; i++);
    Fp ret = acc[i] + sum(tenp[i], x) * (i + 1);
    DLOGK(x, i, ret);
    return ret;
  };
