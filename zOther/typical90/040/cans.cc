...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, W; cin >> N >> W;
  vector<ll> A(N); cin >> A;
  auto C = vector(N, vector<ll>());
  auto key = vector(N, vector<ll>());
  for (ll i = 0; i < N; i++) {
    ll k; cin >> k;
    C[i].resize(k);
    cin >> C[i];
    for (ll t : C[i]) { key[t - 1].push_back(i); }
  }
  Dinic dinic(2 + N + N, 0, 1);

  ll init_gain = 0;
  for (ll i = 0; i < N; i++) {
    if (W > A[i]) {
      DLOGK(2 + i, 1, W - A[i]);
      dinic.add_edge(2 + i, 1, W - A[i]);
    }else  {
      DLOGK(0, 2 + i, A[i] - W);
      dinic.add_edge(0, 2 + i, A[i] - W);
      init_gain += A[i] - W;
    }
  }
  DLOGK(init_gain);
  for (ll i = 0; i < N; i++) {
    ...
    if (key[i].size() == 1) {
      dinic.add_edge(2 + i, 2 + key[i][0], big);
      DLOGK(2 + i, 2 + key[i][0], big);
    }else {
      dinic.add_edge(2 + i, 2 + N + i, big);
      DLOGK(2 + i, 2 + N + i, big);
      for (ll j : key[i]) {
        dinic.add_edge(2 + N + i, 2 + j, big);
        DLOGK(2 + N + i, 2 + j, big);
      }
    }
  }
