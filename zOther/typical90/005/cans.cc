...
int main(/* int argc, char *argv[] */) {
  ...
  ll lim = 60;
#endif
  auto tbl = vector(lim, vector(B, Fp(0)));
  for (ll i = 0; i < K; i++) tbl[0][C[i] % B] += Fp(1);
  auto P = vector(lim, 0LL);
  P[0] = 10 % B;
  for (ll p = 0; p + 1 < lim; p++) {
    for (ll i = 0; i < B; i++) {
      for (ll j = 0; j < B; j++) {
        ll e = (i * P[p] + j) % B;
        tbl[p + 1][e] += tbl[p][i] * tbl[p][j];
        P[p + 1] = (P[p] * P[p]) % B;
      }
    }
  }
