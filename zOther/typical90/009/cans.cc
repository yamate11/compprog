...
int main(/* int argc, char *argv[] */) {
  ...
  for (ll i = 0; i < N; i++) {
    ...
    for (ll j = 0; j < N; j++) {
      if (i == j) continue;
      A.push_back((pt[j] - pt[i]).arg());
    }
    sort(A.begin(), A.end());
    DLOGK(A);
    for (ll j = 0; j < N-1; j++) A.push_back(A[j] + 2 * G_PI);
    ll k = 1;
    for (ll j = 0; j < N-1; j++) {
      while (A[k] < A[j] + G_PI) k++;
      Real p = A[j] + 2 * G_PI - A[k];
      Real q = A[k-1] - A[j];
      ans = max(ans, p);
      ans = max(ans, q);
      DLOGK(i, j, k, A[j], A[k], p, q);
    }
  }
  cout << 180.0 * ans / G_PI << endl;
