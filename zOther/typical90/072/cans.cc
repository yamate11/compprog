...
int main(/* int argc, char *argv[] */) {
  ...
  ll N = H * W;
  vector<bool> images(1LL << N);
  for (ll i = 0; i < H; i++) {
    for (ll j = 0; j < W; j++) {
      if (brd.at(i, j) == '#') continue;
      ll start = enc(i, j);
      auto visited = vector(1LL << N, false);
      auto dfs = [&](auto rF, const BrdIdx& bi0, ll x) -> void {
        if (visited[x]) return;
        visited[x] = true;
      ...
      dfs(dfs, BrdIdx(i, j), 1LL << start);
    }
  }
  ll ans = -1;
  for (ll x = 0; x < (1LL << N); x++) {
    if (!images[x]) continue;
    ll s = __builtin_popcountll(x);
    if (s <= 2) continue;
    ans = max(ans, s);
  }
  cout << ans << endl;
