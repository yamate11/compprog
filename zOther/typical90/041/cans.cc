...
// @@ !! LIM(geometry debug f:floor_sum)
...
// ---- inserted function f:floor_sum from util.cc
...
int main(/* int argc, char *argv[] */) {
  ...
  if (ch.back().x > ch.front().x) { ch.push_back(ch.front()); }
  DLOGK(ch);
  ll sum1 = 0;
  for (ll i = 0; true; i++) {
    ll x0 = lround(ch[i].x);
    ll y0 = lround(ch[i].y);
    ll x1 = lround(ch[i+1].x);
    ll y1 = lround(ch[i+1].y);
    if (x1 <= x0) {
      sum1 += y0;
      DLOGK(y0);
      break;
    ...
    assert(p > 0);
    DLOGK(p, q);
    ll s = y0 * p - floor_sum(p, p, -q, 0);
    DLOGK(s);
    sum1 += s;
  }
  DLOGK(sum1);
  ll sum2 = 0;
  for (ll i = (ll)ch.size() - 1; true; i--) {
    ll x0 = lround(ch[i].x);
    ll y0 = lround(ch[i].y);
    ll x1 = lround(ch[i-1].x);
    ll y1 = lround(ch[i-1].y);
