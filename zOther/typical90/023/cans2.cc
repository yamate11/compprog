...
int main(/* int argc, char *argv[] */) {
  ...
  // ll sz = poss.size();
  vector<vector<ll>> nxt(1 << len);
  ll cnt = 0;
  for (ll x: poss) {
    ll nar = ((1LL << len) - 1) & ~(x | (x << 1) | (x >> 1));
    ll y = nar;
    while (true) {
      if (!((y << 1) & y)) {
        nxt[x].push_back(y);
  ...
  auto cur = vector(1 << len, Fp(0));
  cur[0] = Fp(1);
  for (ll i = 0; i < H; i++) {
    auto prev = move(cur);
    cur = vector(1 << len, Fp(0));
    for (ll x: poss) {
      if (x & barrier[i]) continue;
      for (ll y: nxt[x]) cur[x] += prev[y];
    }
  }
  Fp ans = 0;
  for (ll x = 0; x < (1 << len); x++) { ans += cur[x]; }
  cout << ans << endl;
