...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  vector<string> barrier(H);
  for (ll i = 0; i < H; i++) { cin >> barrier[i]; }

  ll len = W;
  vector<vector<ll>> poss(len + 1);
  for (ll t = 0; t <= len; t++) {
    for (ll x = 0; x < (1LL << t); x++) {
      if ((x << 1) & x) continue;
      poss[t].push_back(x);
    }
  }
  ...
  auto cur = vector<Fp>(1 << (W + 1));
  auto prev = vector<Fp>(1 << (W + 1));
  auto cur_cont = vector<ll>();
  auto prev_cont = vector<ll>();

  auto add = [&](ll x, ll y) -> void {
    if (cur[y] == Fp(0)) cur_cont.push_back(y);
    cur[y] += prev[x];
  };

  auto placable = [&](ll x, ll c) -> bool {
    bool b0 = (c == 0) ? false : (x & 1);
    bool b1 = (x >> 1) & 1;
    bool b2 = (c == W - 1) ? false : ((x >> 2) & 1);
    bool b3 = (c == 0) ? false : ((x >> W) & 1);
    return ! (b0 || b1 || b2 || b3);
  };

  cur[0] = 1;
  cur_cont.push_back(0);
  for (ll r = 0; r < H; r++) {
    for (ll c = 0; c < W; c++) {
      for (ll x : prev_cont) prev[x] = Fp(0);
      DLOGKL("clean", prev);
      swap(cur, prev);
      prev_cont = vector<ll>();
      swap(cur_cont, prev_cont);
      for (ll x : prev_cont) {
        if (prev[x] == Fp(0)) continue;
        ll y = x >> 1;
        add(x, y);
        if (barrier[r][c] == '.' && placable(x, c)) {
          add(x, y | (1LL << W));
        }
      }
#if DEBUG
      for (ll x : cur_cont) { DLOGKL("  ", x, cur[x]); }
#endif
    }
  }
  Fp ans = Fp(0);
  for (ll x : cur_cont) ans += cur[x];
  cout << ans << endl;
