...
int main(/* int argc, char *argv[] */) {
  ...
  const OP unit_op = LLONG_MIN;
  auto xAdd = [](DAT x, DAT y) -> DAT { return max(x, y); };
  auto xAppl = [](int k, OP f, DAT x) -> DAT { return max(f, x); };
  auto xComp = [](OP h, OP g) -> OP { return max(h, g); };
  auto st = make_seg_tree(unit_dat, unit_op, xAdd, xComp, xAppl, false);
  vector<ll> init(W + 1, -big);
  init[0] = 0;
  st.set_data(init);
