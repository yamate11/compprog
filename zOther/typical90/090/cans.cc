...
// @@ !! LIM(mod f:power debug cmpNaive perm matrix)
...
int body(istream& cin, ostream& cout) {
  ...
  ll N, K; cin >> N >> K;
  if (K == 1) {
    Matrix<Fp> mat( {{ Fp(1), Fp(1) }, {Fp(1), Fp(0)}} );
    auto mat1 = mat.matpower(N);
    Matrix<Fp> vec = Matrix<Fp>::fromVec(vector<Fp>({1, 0}));
    auto vec1 = mat1 * vec;
    cout << vec1.at(0, 0) + vec1.at(1, 0) << endl;
    DLOGK(mat1, vec, vec1);
    return 0;
  }
  ...
  while (true) {
    ...
    v = vector<Fp>(t + 1);
    v[0] = 1;
    auto u = vector<Fp>(t + 1);
    u[0] = 1;
    ll longest = size(prev) - 1;
    for (ll x = 0; x < t; x++) {
      Fp coeff = 0;
      ll j0;
      if (x < longest) {
        ...
        j0 = longest;
        if (x - j0 - 2 + 1 >= 0) {
          u[x - j0 - 1 + 1] += u[x - j0 - 2 + 1];
          u[x - j0 - 2 + 1] = 0;
        }
        coeff = u[x - j0 - 1 + 1] * var;
      }
      DLOGKL("  ", x, j0, coeff);
      for (ll j = j0; j >= 1; j--) {
        u[x + 1] += coeff * prev[j];
        DLOGKL("  ", x, u[x + 1], coeff, prev[j]);
        if (x == j - 1) coeff = var;
        else coeff = (coeff + u[x - j + 1]) * var;
      }
      v[x + 1] = v[x] * var + u[x + 1];
    }
    DLOGK(t, k, v);
