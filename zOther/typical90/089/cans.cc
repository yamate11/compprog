...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> bd(N);
  {
    ...
    vector<DAT> init_vec = vector<DAT>(N);
    ...
    for (ll i = N - 1; i >= 0; i--) {
      for ( ; j >= 0; j--) {
        ...
        cnt += n;
        st.update(A[j], A[j] + 1, 1);
      }
      if (j < 0) bd[i] = 0;
      ll m = st.query(A[i] + 1, N);
      cnt -= m;
...

  {
    ...
    auto st = make_seg_tree(unit_dat, unit_op, xAdd, xComp, xAppl, false);
    vector<DAT> init_vec = vector<DAT>(N + 1);
    st.set_data(init_vec);

    st.update(0, 1, Fp(1));
    st.update(1, 2, Fp(1));
    for (ll i = 1; i < N; i++) {
      for (ll j = bd[i-1]; j < bd[i]; j++) st.update(j, j + 1, 0);
      Fp x = st.query(0, i + 1);
      st.update(i + 1, i + 2, x);
    }
