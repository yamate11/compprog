...
struct Polynomial {
  ...
  static Polynomial getX() { return Polynomial({(T)0, (T)1}); }
...
using PolyFpA = Polynomial<FpA, 1>;
...
int main(/* int argc, char *argv[] */) {
...

  {
    ...
    for (auto [vq, vd, vm] : tests1) {
      ...
      if (q.degree() == 1) {
        ...
        assert(ddd == q.coef[1] * d && PolyFpB(mmm) == m);
      }
    }
...

  {
    ...
    assert(PolyLL({1,2}) - SP::getXn(5) == PolyLL({1,2,0,0,0,-1}));
    assert(PolyLL({10,11,12}) - 12 * SP::getXn(2) == PolyLL({10,11}));
    assert(PolyLL() * SP() == PolyLL());
    assert(SP() * PolyLL() == PolyLL());
    assert(PolyLL() * SP::getXn(2) == PolyLL());
    assert(SP::getXn(2) * PolyLL() == PolyLL());
  }
  {
    using SP = SparsePoly<ll>;
    PolyLL p1({1, 2, 3});
    SP sp1 = 4LL - SP::getXn(2);
