...
int main(/* int argc, char *argv[] */) {
  ...
  auto f2 = [&](string s1, string s2) -> Fp {
    ll m1 = ssize(s1), m2 = ssize(s2);
    vector vec_init(m1 + 1, vector(m2 + 1, Fp(0)));
    auto vec = vec_init;
    vec[0][0] = 1;
    REP(i, 0, N) {
      auto prev = move(vec);
      vec = vec_init;
      REP(j1, 0, m1 + 1) REP(j2, 0, m2 + 1) {
        if (j1 < m1 and j2 < m2) {
          if (s1[j1] == s2[j2]) {
            vec[j1 + 1][j2 + 1] += prev[j1][j2];
            vec[j1][j2] += (n_alph - 1) * prev[j1][j2];
          }else {
            vec[j1 + 1][j2] += prev[j1][j2];
            vec[j1][j2 + 1] += prev[j1][j2];
            vec[j1][j2] += (n_alph - 2) * prev[j1][j2];
          }
        }else if (j1 < m1) {
          vec[j1 + 1][j2] += prev[j1][j2];
          vec[j1][j2] += (n_alph - 1) * prev[j1][j2];
  ...
  auto f3 = [&](string s1, string s2, string s3) -> Fp {
    ll m1 = ssize(s1), m2 = ssize(s2), m3 = ssize(s3);
    vector vec_init(m1 + 1, vector(m2 + 1, vector(m3 + 1, Fp(0))));
    auto vec = vec_init;
    vec[0][0][0] = 1;
    REP(i, 0, N) {
      auto prev = move(vec);
      vec = vec_init;
      REP(j1, 0, m1 + 1) REP(j2, 0, m2 + 1) REP(j3, 0, m3 + 1) {
        Fp pval = prev[j1][j2][j3];
        if (j1 < m1 and j2 < m2 and j3 < m3) {
          if (s1[j1] == s2[j2] and s2[j2] == s3[j3]) {
            vec[j1 + 1][j2 + 1][j3 + 1] += pval;
            vec[j1][j2][j3] += (n_alph - 1) * pval;
          }else if (s1[j1] == s2[j2]) {
            vec[j1 + 1][j2 + 1][j3] += pval;
            vec[j1][j2][j3 + 1] += pval;
            vec[j1][j2][j3] += (n_alph - 2) * pval;
          }else if (s2[j2] == s3[j3]) {
            vec[j1][j2 + 1][j3 + 1] += pval;
            vec[j1 + 1][j2][j3] += pval;
            vec[j1][j2][j3] += (n_alph - 2) * pval;
          }else if (s3[j3] == s1[j1]) {
            vec[j1 + 1][j2][j3 + 1] += pval;
            vec[j1][j2 + 1][j3] += pval;
            vec[j1][j2][j3] += (n_alph - 2) * pval;
          }else {
            vec[j1 + 1][j2][j3] += pval;
            vec[j1][j2 + 1][j3] += pval;
            vec[j1][j2][j3 + 1] += pval;
            vec[j1][j2][j3] += (n_alph - 3) * pval;
          }
        }else if (j1 < m1 and j2 < m2) {
          if (s1[j1] == s2[j2]) {
            vec[j1 + 1][j2 + 1][j3] += pval;
            vec[j1][j2][j3] += (n_alph - 1) * pval;
          }else {
  ...
  Fp a = f1(s1) + f1(s2) + f1(s3);
  DLOGK(a, f1(s1), f1(s2), f1(s3));
  Fp b = f2(s1, s2) + f2(s2, s3) + f2(s3, s1);
  DLOGK(b, f2(s1, s2), f2(s2, s3), f2(s3, s1));
  Fp c = f3(s1, s2, s3);
  DLOGK(c, f3(s1, s2, s3));
  Fp ans = power<Fp>(n_alph, N) - (a - b + c);
  cout << ans << endl;
