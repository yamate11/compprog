...
int main(/* int argc, char *argv[] */) {
  ...
  Mat some(3, 3);
  Mat unit_mat = some.one();
  vector<Mat> init_dat(N);
  auto mat_trans_ll = [&](ll a) -> Mat {
    if (a > 0) {
      return Mat{{a, big, big}, {0, 0, big}, {a, a, a}};
    }else {
      return Mat{{a, big, big}, {1, 1, big}, {a, a, a}};
    }
  };
  REP(i, 0, N) init_dat[i] = mat_trans_ll(A[i]);
  auto st = make_seg_tree(unit_mat, multiplies<Mat>(), init_dat);

  REP(_q, 0, Q) {
    ll i, v; cin >> i >> v; i--;
    st.rs(i) = mat_trans_ll(v);
    Mat mat = st.query(0, N);
    auto vals = mat * vector<MP>{0, 0, 0};
    DLOGK(mat, vals);
    // cout << vals[2] << "\n";
    cout << min(vals[0], min(vals[1], vals[2])) << "\n";
  }
