...
int naive(istream& cin, ostream& cout) {
  ...
  while (ip.get()) {
    ...
    if (cntL == L and cntR == R) {
      vector v(N, 0LL);
      REP(i, 0, N) v[i] = A[ip[i]];
      ss.insert(move(v));
    }
  }
  cout << ssize(ss) << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  auto vec = rle(A);
  auto [_x, n0] = vec[0];
  vector tbl_init(L + 1, vector(R + 1, Fp(0)));
  auto tbl = tbl_init;
  tbl[1][1] = 1;
  REP(i, 1, ssize(vec)) {
    auto prev = move(tbl);
    tbl = tbl_init;
    auto [_y, m] = vec[i];
    REP(l, 1, L + 1) REP(r, 1, R + 1) {
      tbl[l][r] += prev[l][r] * cb.binom(n0 - 2 + m, m);
      if (l + 1 <= L) tbl[l + 1][r] += prev[l][r] * cb.binom(n0 + m - 2, m - 1);
      if (r + 1 <= R) tbl[l][r + 1] += prev[l][r] * cb.binom(n0 + m - 2, m - 1);
      if (l + 1 <= L and r + 1 <= R) tbl[l + 1][r + 1] += prev[l][r] * cb.binom(n0 + m - 2, m - 2);
    }
    n0 += m;
  }
  cout << tbl[L][R] << endl;
