...
// @@ !! LIM(f:updMaxMin debug segTree cmpNaive forall)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<ll> ans(N + 1, big);
    REP(x, 1, 1LL << N) {
      auto check = [&]() -> bool {
        vector<ll> v;
        REP(i, 0, N) if (x >> i & 1) v.push_back(i);
        REP(i, 0, ssize(v) - 1) if (EXISTS(j, v[i] + 1, v[i + 1], P[j] < P[v[i]] and P[j] < P[v[i + 1]])) return false;
        return true;
      };
      if (check()) {
        ll cost = 0;
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    REP(i, 1, N) {
      ...
      while (not stack.empty() and P[stack.back()] > P[i]) {
        W[i].push_back(stack.back());
        stack.pop_back();
      }
      stack.push_back(i);
      DLOGK(i, W[i]);
    ...
    vector<ll> ans(N + 1, big);
    vector tbl = A;
    ans[1] = *ranges::min_element(tbl);
    REP(k, 2, N + 1) {
      auto stHi = make_seg_tree(big, mymin, tbl);
      auto stLo = stHi;
      tbl = vector<ll>(N, big);
      REP(i, k - 1, N) {
        ll ahi = stHi.query(R[i], i);
        ll alo = stLo.query(0, i);
        tbl[i] = min(alo, ahi) + A[i];
        for (ll j : W[i]) stLo.rs(j) = big;
      }
      ans[k] = *ranges::min_element(tbl);
    }
    REPOUT(k, 1, N + 1, ans[k], " ");
  };
