...
// @@ !! LIM(forall debug binsearch doubling)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, Q; cin >> N >> M >> Q;
  // @InpMVec(M, ((A, dec=1), (B, dec=1))) [mVX5f8IW]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  vector<ev_t> ev;
  REP(i, 0, M) ev.emplace_back(A[i], 1, i);
  REP(i, 0, N) ev.emplace_back(i, 0, i);
  ...
  DLOGK(vec);
  Doubling dobj(M, N + 2, vec);
  REP(_q, 0, Q) {
    ll l, r; cin >> l >> r; l--; r--;
    auto check = [&](ll m) -> bool {
      return dobj.val(m, l) <= r + 1;
    };
    ll m = binsearch<ll>(check, 0, M + 1);
    cout << m << "\n";
  }
