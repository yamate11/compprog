...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Fp> H(N + 1);
  REP(i, 1, N) {
    for (ll d : getDivisors(A[i - 1], primes)) G[d] += A[i - 1] * dp[i - 1];
    DLOGK(i, G);
    auto ds = getDivisors(A[i], primes);
    for (ll d: ds) H[d] = G[d];
    DLOGK(i, H, ds);
    auto pr = prfac(A[i], primes);
    for (auto [p, r] : pr) {
      ll x = 1;
      REP(j, 0, r) x *= p;
      ll y = A[i] / x;
      for (ll e: getDivisors(y, primes)) {
        ll z = e;
        REP(j, 0, r) {
          H[z] = H[z] - H[z * p];
          z *= p;
