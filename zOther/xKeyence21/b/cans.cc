...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = 0;
  DupIntPerm dip(K, N);
  vector box(K, vector<ll>());
  vector<bool> tmp(N + 10);
  do {
    const auto& vec = dip.get();
    for (ll j = 0; j < K; j++) box[j].resize(0);
    for (ll i = 0; i < N; i++) {
      ll j = vec[i];
      box[j].push_back(A[i]);
    }
    ll s = 0;
#if 1
    for (ll j = 0; j < K; j++) s += mex(box[j].begin(), box[j].end());
#else
    for (ll j = 0; j < K; j++) {
      fill(tmp.begin(), tmp.end(), false);
      for (ll a : box[j]) tmp[a] = true;
      for (ll i = 0; i < N + 1; i++) {
        if (!tmp[i]) {
          s += i;
          break;
