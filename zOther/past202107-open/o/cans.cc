...
int main(/* int argc, char *argv[] */) {
  ...
  auto st = make_seg_tree_lazy(-big, -big, mymax, mymax, mymax, vector<ll>(N, -big));
  REP(i, 0, N) {
    ll v;
    if (i > 0) {
      v = st.at(i - 1) - B[i - 1];
    ...
    ll idx = upper_bound(ALL(B), v + S[i + 1]) - B.begin();
    st.update(i, idx, v);
  }
  ll ans = st.at(N - 1) - B[N - 1] + S[N];
  if (ans < 0) {
    cout << -1 << endl;
  }else {
    cout << ans + acc << endl;
