...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ev_t> event;
  REP(i, Q) {
    event.emplace_back(A[i], 1, B[i], D[i]);
    event.emplace_back(C[i], 0, B[i], D[i]);
  }
  sort(ALL(event));

  CoordCompr cc;
  ll lim = 1e9;
  cc.add(-lim); cc.add(lim);
  REP(i, Q) {
    cc.add(B[i]); cc.add(D[i]);
  }
  ll lsz = cc.size() - 1;
  vector<ll> L(lsz);
  REP(i, lsz) L[i] = cc.d(i + 1) - cc.d(i);
