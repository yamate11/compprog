...
struct T_t {
  ...
  ll wt;
  ll upper;
  T_t() : w(0), p(0), p_prev(0), wt(0), upper(-1) {}

  ll fwd(const auto& B, const auto& D, ll k0, ll w0) {
    if (w < 0) return 0;
    p_prev = p;
    if (wt == 0) return 0;
    if (k0 < w) {
      while (p < ssize(B[w]) and take(B[w][p], k0) < D[k0]) p++;
      return (p - p_prev) * wt;
    }else {
      while (p < ssize(B[w0 - w]) and take(B[w0 - w][p], k0 - w) < D[k0]) p++;
      ll len = p - p_prev;
      if (p_prev <= upper and upper < p) len--;
      return len * wt;
    }
  }

  void bwd(ll k0, ll w0) {
    if (w < 0) return;
    if (k0 + 1 == w) {
      if (w == w0 - w) {
        upper = p_prev;
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(w, 1, 10) ranges::sort(B[w]);
  DLOGK(B);
  ll cnt_lim = N * (N - 1) / 2 - 1;
  ll cnt = 0;
  ll w0 = 1;
  for (; true; w0++) {
    ll num = 0;
    ...
    DLOGK(w0, cnt, num, cnt_lim);
    if (cnt + num > cnt_lim) break;
    cnt += num;
  }
  ...
  REP(k0, 0, w0) {
    REP(i, 1, 10) {
      ...
      ll num = 0;
      REP(w, 1, w0) {
        ll n = T[w].fwd(B, D, k0, w0);
        num += n;
        DLOGK(k0, i, w, n, num);
      }
      if (cnt + num > cnt_lim) {
        D[k0]--;
        REP(w, 1, w0) T[w].bwd(k0, w0);
        break;
      }
      cnt += num;
      DLOGK(k0, i, num, cnt);
