...
// @@ !! LIM(digit debug binsearch cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  vector<ll> v;
  REP(i, 0, N) REP(j, 0, N) {
    if (i != j) v.push_back(du.from_string(du.to_string(A[i]) + du.to_string(A[j])));
  }
  ranges::sort(v);
  cout << v[N * (N - 1) / 2 - 1] << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  /*
  vector B(10, vector<ll>());
  REP(i, 0, N) B[du.width(A[i])].push_back(A[i]);
  */
  ...
  auto check = [&](ll x) -> bool {
    ...
    REP(wa, 1, 10) {
      ll wx = du.width(x);
      if (wa >= wx) continue;
      string sx = du.to_string(x);
      ll y = du.from_string(sx.substr(0, wa));
      ll kL = ranges::lower_bound(B[wa], y) - B[wa].begin();
      ll kU = ranges::upper_bound(B[wa], y) - B[wa].begin();
      ll th1 = du.pow(wx - wa);
      ll th2 = du.from_string(sx.substr(wa));
      ll th3 = du.pow(wx - wa - 1);
      ll j1 = ranges::lower_bound(A, th1) - A.begin();
      ll j2 = ranges::lower_bound(A, th2) - A.begin();
      ll j3 = ranges::lower_bound(A, th3) - A.begin();
      ll dec1 = (wa <= wx - wa) ? 1 : 0;
      ll dec3 = (wa <= wx - wa - 1) ? 1 : 0;
      ll dec2 = (wa < wx - wa or (wa == wx - wa and y < th2)) ? 1 : 0;
      cnt += (j1 - dec1) * kL;
      cnt += (j2 - dec2) * (kU - kL);
      cnt += (j3 - dec3) * (ssize(B[wa]) - kU);
      return cnt < N * (N - 1) / 2;
    }
    */
    ...
    REP(i, 0, N) {
      ...
      ll y = du.from_string(sx.substr(0, wa));
      ll th;
      if (a < y) th = du.pow(wx - wa);
      else if (a == y) {
        if (sx[wa] == '0') th = du.pow(wx - wa - 1);
        else th = du.from_string(sx.substr(wa));
      }
      else th = du.pow(wx - wa - 1);
      while (j - 1 >= 0 and th <= A[j - 1]) j--;
      // ll j = ranges::lower_bound(A, th) - A.begin();
      DLOGK(x, i, A[i], j);
      if (i < j) cnt += j - 1;
      else       cnt += j;
    }
  ...
  ll ans = binsearch<ll>(check, 1, 1e18);
  cout << ans << endl;
