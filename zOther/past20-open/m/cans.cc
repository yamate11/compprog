...
struct digit_util {
  ...
  ll pow(ll i) const { return _pow[i]; }
  ...
  ll nd_min(ll i) const { return pow(i - 1); }
  ll nd_max(ll i) const { return nd_min(i + 1) - 1; }

  ll floor(ll x) const { return (x == 0) ? 0 : _pow[width(x) - 1]; }
  ...
  ll d_at(ll x, ll i) const { return (x / pow(i)) % base; }
  ll v_at(ll x, ll i) const { return d_at(x, i) * pow(i); }
...
#define use_string
    ...
    REP(i, 0, N) {
      ...
      ll z = du.from_string(sx.substr(wa));
#else
      ll y = x / du.pow(wx - wa);
      ll z = x % du.pow(wx - wa);
#endif      
...
#else
        if (z < du.pow(wx - wa - 1)) th = du.pow(wx - wa - 1);
#endif
        else th = z;
      }
