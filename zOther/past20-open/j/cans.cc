...
int main(/* int argc, char *argv[] */) {
  ...
  Board<bool> reg(N, N, false);
  vector<BrdIdx> stack;

  auto g = [&](BrdIdx bi) -> void {
    if (brd.at(bi) == '.' and not reg.at(bi) and f(bi) >= 3) {
      stack.push_back(bi);
      reg.rs(bi) = true;
    }
  };

  REP(i, 0, N) REP(j, 0, N) g(BrdIdx(i, j));
  ll ans = 0;
  while (not stack.empty()) {
    auto bi = stack.back(); stack.pop_back();
    brd.rs(bi) = '#';
    ans++;
    for (auto dir : BrdIdx::nbr4) g(bi + dir);
  }
  cout << ans << endl;
  return 0;
}
