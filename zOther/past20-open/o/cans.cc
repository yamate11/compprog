...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, T, Q; cin >> N >> T >> Q;
  // @InpMVec(T, ((L, dec=1), (R, dec=0), V)) [eEXHsIYk]
  auto L = vector(T, ll());
  auto R = vector(T, ll());
  auto V = vector(T, ll());
  for (int i = 0; i < T; i++) {
  ...
  // @End [eEXHsIYk]
  // @InpMVec(Q, ((LQ, dec=1), (RQ, dec=0), X)) [HY5NjV6L]
  auto LQ = vector(Q, ll());
  auto RQ = vector(Q, ll());
  auto X = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
    ll v1; cin >> v1; v1 -= 1; LQ[i] = v1;
    ll v2; cin >> v2; v2 -= 0; RQ[i] = v2;
    ll v3; cin >> v3; X[i] = v3;
  }
  // @End [HY5NjV6L]
  ...
  while (true) {
    DLOGK(lo);
    DLOGK(hi);
    vector to_check(T, vector<ll>());
    ll cnt = 0;
    REP(q, 0, Q) {
      if (lo[q] + 1 < hi[q]) {
        to_check[(lo[q] + hi[q]) / 2].push_back(q);
        cnt++;
      }
    }
    if (cnt == 0) break;
    using DAT = pll;
    auto myadd = [&](DAT x, DAT y) -> DAT { return DAT(x.first + y.first, x.second + y.second); };
    auto myapply = [&](ll f, DAT x) -> DAT { return DAT(x.first + f * x.second, x.second); };
    vector<DAT> init(N, DAT(0, 1));
    auto st = make_seg_tree_lazy(pll(), 0LL, myadd, plus<ll>(), myapply, init);
    REP(t, 0, T) {
      st.update(L[t], R[t], V[t]);
      DLOGK(t, st.vec_view());
      for (ll q : to_check[t]) {
        auto [v, len] = st.query(LQ[q], RQ[q]);
        DLOGK(t, q, LQ[q], RQ[q], X[q], v);
        if (X[q] <= v) hi[q] = t;
        else lo[q] = t;
      }
    }
