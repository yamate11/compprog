...
using Fp = FpB;
using sv = small_vector_u64<6>;
using mymap = safe_umap<sv, Fp>;
...
int main(/* int argc, char *argv[] */) {
  ...
  ll M; cin >> M;
  // @InpGrid(4, 4, A, dec=1) [nIFX0rWa]
  auto A = vector(4, vector(4, ll()));
  for (int i = 0; i < 4; i++) for (int j = 0; j < 4; j++) { ll v; cin >> v; v -= 1; A[i][j] = v; }
  // @End [nIFX0rWa]

  ll indet = M + 1;
  vector vecA(4, sv());
  REP(k, 0, 4) REP(i, 0, k + 1) {
    ll j = k - i;
    vecA[k][i] = A[i][j] < 0 ? indet : A[i][j];
  }
  vector vecB(4, sv());
  REP(k, 3, 7) REP(z, 0, 7 - k) {
    ll j = 3 - z;
    ll i = k - j;
    vecB[6 - k][z] = A[i][j] < 0 ? indet : M - 1 - A[i][j];
  }
  DLOGK(vecA);
  DLOGK(vecB);

  auto f = [&](const auto vec) -> mymap {
    auto compat = [&](ll i, sv v) -> bool {
      REP(j, 0, i + 1) {
        if (vec[i][j] != indet and vec[i][j] != v[j]) {
          DLOGKL("  compat", i, v, vec[i], j);
          return false;
        }
      }
      return true;
    };
    auto get_smaller = [&](ll len, sv v) -> pair<bool, sv> {
      sv ret;
      REP(i, 0, len) {
        ll x = min(v[i], v[i + 1]) - 1;
        if (x < 0) return {false, ret};
        ret[i] = x;
      }
      return {true, ret};
    };
    ...
    mp[sv()] = 1;
    REP(i, 0, 4) {
      mymap prevsum = move(mp);
      mp = mymap();
      REP(j, 0, i) {
        IntDirProd idi(vector<ll>(i, M));
        while (idi.get()) {
          sv v;
          REP(p, 0, i) v[p] = idi.at(p);
          if (v[j] + 1 < M) {
            sv w = v;
            w[j] = w[j] + 1;
            prevsum[w] += prevsum[v];
      ...
      IntDirProd idi(vector<ll>(i + 1, M));
      while (idi.get()) {
        sv v(idi, i + 1);
        auto [b, w] = get_smaller(i, v);
        if (b and compat(i, v)) mp[v] = prevsum[w];
        DLOGK(v, b, compat(i, v));
      }
      DLOGK(i, mp);
    }
    return mp;
