...
int main(/* int argc, char *argv[] */) {
  ...
  ll M; cin >> M;
  // @InpGrid(4, 4, A, dec=1) [jZYu1rmh]
  auto A = vector(4, vector(4, ll()));
  for (int i = 0; i < 4; i++) for (int j = 0; j < 4; j++) { ll v; cin >> v; v -= 1; A[i][j] = v; }
  // @End [jZYu1rmh]

  vector tbl_init(M, vector(M, vector(M, vector(M, Fp(0)))));
  auto tbl = tbl_init;
  if (A[0][0] < 0) REP(i, 0, M) tbl[0][0][0][i] = Fp(1);
  else tbl[0][0][0][A[0][0]] = 1;
  REP(idx, 1, 16) {
    auto prev = move(tbl);
    tbl = tbl_init;
    ll p = idx / 4;
    ll q = idx % 4;
    REP(m, 0, M) {
      if (A[p][q] >= 0 and A[p][q] != m) continue;
      REP(a, 0, M) REP(b, 0, M) REP(c, 0, M) REP(d, 0, M) {
        if (q == 0) {
          if (a < m) tbl[b][c][d][m] += prev[a][b][c][d];
        }else {
          if (a < m and d < m) tbl[b][c][d][m] += prev[a][b][c][d];
        }
      }
    }
  }
  Fp ans = 0;
  REP(a, 0, M) REP(b, 0, M) REP(c, 0, M) REP(d, 0, M) ans += tbl[a][b][c][d];
  cout << ans << endl;
