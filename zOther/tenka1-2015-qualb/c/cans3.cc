...
-   a-1 + a > b  ==>  b <= 2*a - 2
-   2*a + b - 1 <= L  ==>  b <= L - 2*a + 1
***  a + 2 <= b <= min( 2*a - 2,  L - 2*a + 1 )
            2*a - 2 <= L - 2*a + 1   <=> 4*a <= L + 3  <=> a <= (L + 3) / 4
     a + 2 <= 2*a - 2  <=> a >= 4
  first half:  sum from 1 to (L+3)/4 (diff=1)
  second half: sum from L - 2 * ((L+7) / 4) + 1 - (a + 2) + 1 (diff=3) while positive

(3) b, a-1, a; b <= a-3
-   b + a-1 > a  <=> b > 1
-   2*a + b - 1 <= L  ==> b <= L - 2*a + 1
        a - 3 <= L - 2*a + 1  <=>  3*a <= L + 4  <=>  a <= (L + 4) / 3
  first half: sum from 1 to (L+4)/3 (diff=1)
  second half: sum from L - 2 * ((L+4)/3) + 1 - 2 + 1 (diff=2) while positive
...
int main(/* int argc, char *argv[] */) {
  ...
  if (e2 >= 4) {
    ll x = 2 * e2 - 2 - (e2 + 2) + 1;
    cnt2a = Fp(x) * (x + 1) / 2;
    ll f2 = L - 2 * (e2 + 1) + 1 - (e2 + 1 + 2) + 1;
    if (f2 > 0) {
      ll h2 = f2 % 3;
      ll n2 = f2 / 3;
      cnt2b = Fp(n2) * (n2 + 1) / 2 * 3 + Fp(n2 + 1) * h2;
    }
  }
  ll e3 = (L + 4) / 3;
  Fp cnt3a = 0;
  ...
  if (e3 >= 5) {
    ll x = e3 - 3 - 2 + 1;
    cnt3a = Fp(x) * (x + 1) / 2;
    ll f3 = L - 2 * (e3 + 1) + 1 - 2 + 1;
    DLOG("f3=", f3);
    if (f3 > 0) {
      ll h3 = f3 % 2;
      ll n3 = f3 / 2;
      cnt3b = Fp(n3) * (n3 + 1) + Fp(n3 + 1) * h3;
    }
  }
  DLOG("cnt1=", cnt1, "cnt2a=", cnt2a, "cnt2b=", cnt2b,
       "cnt3a=", cnt3a, "cnt3b=", cnt3b);
  Fp ans = cnt1 + cnt2a + cnt2b + cnt3a + cnt3b;
  cout << ans << endl;
  return 0;
}
