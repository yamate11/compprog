...
int main(int argc, char *argv[]) {
  ...
  for (ll r = 0; r < H; r++) {
	...
	auto [rr, cc] = brd.dec(x - 1);
	pDist += abs(rr - r) + abs(cc - c);
      }
    }
  }
  using St1 = tuple<ll, Board<ll>, BrdIdx>;
  auto cmp = [](St1 st1, St1 st2) -> bool {
    return get<0>(st1) < get<0>(st2);
  };
  priority_queue<St1, vector<St1>, decltype(cmp)> pque;
  pque.emplace(pDist, brd, cur0);
  while (!pque.empty()) {
    auto&& [cPDist, cBrd, cCur] = pque.top(); pque.pop();
