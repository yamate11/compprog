...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Fp> pow2(2*N + 1), powA(2*N + 1), powB(2*N + 1);
  pow2[0] = powA[0] = powB[0] = Fp(1);
  REP(i, 1, 2 * N + 1) {
    pow2[i] = pow2[i - 1] * Fp(2);
    powA[i] = powA[i - 1] * a;
    powB[i] = powB[i - 1] * b;
  }

  DLOGK(pow2, powA, powB);
  Fp e = Fp(0);
  REP(x, 0, N) {
    Fp et = cb.binom(N - 1 + x, x) * (powA[N] * powB[x] + powA[x] * powB[N]) * (N + x);
    e += et;
    DLOGK(x, et);
  }
