...
int main(/* int argc, char *argv[] */) {
  ...
  Line sg(goal - start, start);
  REP(i, 0, N) {
    Point nv = nvec[i];
    Line l;
    if (may_eq(0.0, nv.x)) {
      Real y0 = R[i] / nv.y;
      l = Line(Point(-nv.y, nv.x), Point(0.0, y0));
    }else {
      Real x0 = R[i] / nv.x;
      l = Line(Point(-nv.y, nv.x), Point(x0, 0.0));
    }
    if (start.sim(goal)) {
      if (not (l.ptOn(start))) {
        W[i] = 0;
      ...
      auto [code, r1, r2] = sg.intersect_coeff(l);
      if (code == Line::IST_NONE) {
        W[i] = 0;
      }else if (code == Line::IST_ONE) {
        if (not (may_le(0.0, r1) and may_le(r1, 1.0))) {
          W[i] = 0;
        }
      }
    }
