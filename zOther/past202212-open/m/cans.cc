...
int main(/* int argc, char *argv[] */) {
  ...
  ll ans = 0;
  auto dfs = [&](auto rF, ll nd, ll vmin, ll vmax) -> void {
    if (nd == 0) ans += N * (N - 1) / 2;
    else ans += (vmin + 1) * (N - 2 - vmax + 1);
    for (auto [cld, e] : tr.children_pe(nd)) {
      rF(rF, cld, min(vmin, (ll)e), max(vmax, (ll)e));
    }
  };
  dfs(dfs, 0, LLONG_MAX, LLONG_MIN);
  cout << ans << endl;
