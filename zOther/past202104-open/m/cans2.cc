...
int main(/* int argc, char *argv[] */) {
  ...
  auto divide = [&](ll t) -> void {
    auto it = expr.lower_bound(sta(t, -1, -1));
    auto [l0, r0, y0] = *it;
    if (t == l0) return;
    it--;
    auto [l1, r1, y1] = *it;
    expr.erase(it);
    expr.emplace(l1, t, y1);
    expr.emplace(t, r1, y1);
  };
  ...
  for (ll q = 0; q < Q; q++) {
    ...
    divide(l);
    divide(r);
    auto it1 = expr.lower_bound(sta(l, -1, -1));
    auto it2 = expr.lower_bound(sta(r, -1, -1));
    for (auto it = it1; it != it2; it = expr.erase(it)) {
      auto [l0, r0, y0] = *it;
      cur -= comb(count[y0]) - comb(count[y0] - (r0 - l0));
      count[y0] -= r0 - l0;
    }
    expr.emplace(l, r, x);
