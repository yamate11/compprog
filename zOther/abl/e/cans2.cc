...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  vector<Fp> base(N), acc(N+1);
  base[0] = 1;
  for (ll i = 1; i < N; i++) base[i] = base[i-1] * 10;
  for (ll i = 0; i < N; i++) acc[i+1] = acc[i] + base[i];
  DLOG("base=", base);
  DLOG("acc=", acc);
  map<ll, ll> rec;
  rec[-1] = 0;
  ...
  for (ll _q = 0; _q < Q; _q++) {
    ...
    ll idx0 = N - r;
    ll idx1 = N - l + 1;
    DLOG("idx0=", idx0, "idx1=", idx1);
    ll lastD = 0;
    auto it = rec.lower_bound(idx0);
    if (idx0 < it->first) {
      auto itS = it; itS--;
      Fp len = min(idx1, it->first) - idx0;
      Fp x = d - itS->second;
      Fp t = base[idx0] * acc[len] * x;
      sum += t;
      lastD = itS->second;
    }
    while (it->first < idx1) {
      auto itP = it; itP++;
      Fp len = min(idx1, itP->first) - it->first;
      Fp x = d - it->second;
      Fp t = base[it->first] * acc[len] * x;
      sum += t;
      lastD = it->second;
      it = rec.erase(it);
    }
    cout << sum << "\n";
    rec[idx0] = d;
    if (idx1 < it->first) rec[idx1] = lastD;
  }
  return 0;
}
