...
// @@ !! LIM(f:intDiv f:updMaxMin segTree)
...
struct stA {
  ...
  ll v;
  vector<vector<ll>> d;
  stA() : r3(0), v(0), d(3, vector(3, -big)) {}
  void set_single(ll x, ll y) {
    r3 = x;
    v = y;
    REP(r, 0, 3) REP(c, 0, 3) d[r][c] = r == r3 ? v : -big;
  }
};
const int STANY = 0;
const int STLEFT = 1;
...
int main(/* int argc, char *argv[] */) {
  ...
  auto myadd = [&](const stA& a, const stA& b) -> stA {
    stA ret;
    ret.r3 = (a.r3 + b.r3) % 3;
    ret.v = a.v + b.v;
    REP(r, 0, 3) {
      updMax(ret.d[r][STANY], a.d[r][STANY]);
      updMax(ret.d[r][STANY], b.d[r][STANY]);
      updMax(ret.d[r][STLEFT], a.d[r][STLEFT]);
      updMax(ret.d[r][STRIGHT], b.d[r][STRIGHT]);
    }
    REP(ra, 0, 3) REP(rb, 0, 3) {
      if (a.d[ra][STRIGHT] > -big and b.d[rb][STLEFT] > -big) {
        updMax(ret.d[imod(ra + rb, 3)][STANY], a.d[ra][STRIGHT] + b.d[rb][STLEFT]);
      }
    }
    REP(rb, 0, 3) {
      if (b.d[rb][STLEFT] > -big) {
        updMax(ret.d[imod(a.r3 + rb, 3)][STLEFT], a.v + b.d[rb][STLEFT]);
      }
    }
    REP(ra, 0, 3) {
      if (a.d[ra][STRIGHT] > -big) {
        updMax(ret.d[imod(ra + b.r3, 3)][STRIGHT], a.d[ra][STRIGHT] + b.v);
      }
    }
    return ret;
  };
  vector<stA> init_vec(N);
  REP(i, 0, N) init_vec[i].set_single(A[i], B[i]);
  auto st = make_seg_tree(stA(), myadd, init_vec);
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ll p, x; cin >> p >> x; p--;
      stA s;
      s.set_single(x, st.at(p).v);
      st.rs(p) = s;
    }else if (tp == 2) {
      ll p, x; cin >> p >> x; p--;
      stA s;
      s.set_single(st.at(p).r3, x);
      st.rs(p) = s;
    }else if (tp == 3) {
