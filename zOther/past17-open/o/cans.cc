...
int main(/* int argc, char *argv[] */) {
  ...
  auto B = A;
  sort(ALL(B));
  auto st = make_seg_tree(0LL, plus<ll>(), B);
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ll k, d; cin >> k >> d; k--;
      ll a = A[k];
      ll lo = lower_bound(ALL(B), a) - B.begin();
      ll hi = upper_bound(ALL(B), a) - B.begin();
      ll idx = (d == 1) ? hi - 1 : lo;
      A[k] += d;
      B[idx] += d;
      st.rs(idx) = B[idx];
      ...
      ll x; cin >> x;
      ll idx = lower_bound(ALL(B), x) - B.begin();
      ll s1 = st.query(0, idx);
      ll s2 = st.query(idx, N);
      ll ans = x * idx - s1 + s2 - x * (N - idx);
      cout << ans << "\n";
    }else assert(0);
  }
