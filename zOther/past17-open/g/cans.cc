...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll idx, BrdIdx pos) -> bool {
    if (brd.at(pos) != S[idx]) return false;
    if (hist.contains(pos)) return false;
    hist.insert(pos);
    if (idx + 1 == N) return true;
    for (auto dir: BrdIdx::nbr8) {
      auto nxtpos = pos + dir;
      if (not hist.contains(nxtpos)) {
        bool b = rF(rF, idx + 1, nxtpos);
        if (b) {
          DLOGK(idx, pos, hist);
          return true;
        }
