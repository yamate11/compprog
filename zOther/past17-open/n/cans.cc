...
int main(/* int argc, char *argv[] */) {
  ...
  auto exper = [&]() -> void {
    ...
    while (ip.get()) {
      vector ST(N + 1, 0.0);
      REP(i, 0, N) ST[i + 1] = ST[i] + T[ip.at(i)];
      double e = 0.0;
      REP(i, 0, N) {
        double z1 = ST[i] * N / ((double)(i + 1) - 0.5) - S;
        double z2 = ST[i + 1] * N / ((double)(i + 1) - 0.5) - S;
        double thisE;
        if (z2 < 0) thisE = - (z1 + z2) / 2.0;
        else if (z1 < 0) thisE = (z1 * z1 + z2 * z2) / (z2 - z1) / 2.0;
        else thisE = (z1 + z2) / 2.0;
        e += thisE;
      }
      DLOGK(e, ip.vec_view());
      if (e < ans) {
  ...
  double big = 1e18;
  vector<double> tbl(1LL << N, big);
  tbl[0] = 0;
  REP(x, 0, 1LL << N) {
    double p = 0;
    REP(i, 0, N) if (x >> i & 1) p += T[i];
    ll k = popcount((u64)x);
    REP(i, 0, N) {
      if (x >> i & 1) continue;
      double q = p + T[i];
      double z1 = p * N / ((double)(k + 1) - 0.5) - S;
      double z2 = q * N / ((double)(k + 1) - 0.5) - S;
      double thisE;
      if (z2 < 0) thisE = - (z1 + z2) / 2.0;
      else if (z1 < 0) thisE = (z1 * z1 + z2 * z2) / (z2 - z1) / 2.0;
      else thisE = (z1 + z2) / 2.0;
      updMin(tbl[x | (1LL << i)], tbl[x] + thisE * T[i]);
    }
  }
  cout << tbl[(1LL << N) - 1] / S << endl;
