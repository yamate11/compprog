...
// @@ !! LIM(mod debug f:power)
// --> f:gcd f:intDiv mod f:<< debug f:power
...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W, K; cin >> H >> W >> K;
  vector<vector<char>> vec(H, vector(W, ' '));
  for (ll k = 0; k < K; k++) {
    ll h, w; char c; cin >> h >> w >> c; h--; w--;
    vec[h][w] = c;
  }
  vector tbl(H, vector(W, Fp(0)));
  Fp tot = power<Fp>(3, H * W - K);
  Fp b31 = Fp(1) / Fp(3);
  for (ll i = H - 1; i >= 0; i--) {
    for (ll j = W - 1; j >= 0; j--) {
      if (i == H-1 && j == W-1) {
      ...
      Fp& z = tbl[i][j];
      if (c == ' ') {
        if (i == H-1)      z = 2 * b31 * tbl[i][j + 1];
        else if (j == W-1) z = 2 * b31 * tbl[i + 1][j];
        else               z = 2 * b31 * (tbl[i][j + 1] + tbl[i + 1][j]);
      }else if (c == 'X') {
        if (i == H-1)      z = 1 * tbl[i][j + 1];
        else if (j == W-1) z = 1 * tbl[i + 1][j];
        else               z = 1 * (tbl[i][j + 1] + tbl[i + 1][j]);
      }else if (c == 'R') {
        if (i == H-1)      z = 1 * tbl[i][j + 1];
        else if (j == W-1) z = 0;
        else               z = 1 * tbl[i][j + 1];
