...
int main(/* int argc, char *argv[] */) {
  ...
  auto myadd = [&](pll x, pll y) -> pll { return pll(x.first + y.first, x.second + y.second); };
  vector init_vec(N + 1, pll(0, 0));
  // REP(i, 1, N + 1) init_vec[i] = pll(P[i], 1);
  auto st = make_seg_tree(pll(0, 0), myadd, init_vec);

  ll ans = 0;
  REP(i, 1, N + 1) {
    ll p = P[i - 1];
    auto [val, cnt] = st.query(p, N + 1);
    ans += val + cnt * p;
    st.rs(p) = pll(p, 1);
  }
  cout << ans << endl;
