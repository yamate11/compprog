...
// @@ !! LIM(coordCompr segTree debug cmpNaive perm)
...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = N;
  IntComb ic(P - (N - 1) + N, N);
  while (ic.get()) {
    vector<ll> vec;
    ll cur = -1;
    REP(i, 0, N) {
      ll x = ic.at(i);
      vec.push_back(x - cur - 1);
      cur = x;
    }
    vec.push_back(P - (N - 1) + N - cur - 1);
    vector<ll> R(N);
    R[0] = P - vec[0];
    REP(i, 1, N) R[i] = R[i - 1] - vec[i] - 1;
    // DLOGK(ic.vec_view(), vec, R);
    ll cnt = 0;
    REP(i, 0, N) if (R[i] != A[i]) cnt++;
    ans = min(ans, cnt);
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  CoordCompr cc;
  cc.add(0); cc.add(1); cc.add(P - (N - 1)); cc.add(P - (N - 1) - 1);
  REP(i, 0, N) REP(t, -1, 2) if (0 <= A[i] and A[i] <= P - (N - 1)) cc.add(A[i] + t);
  ll sz = cc.size();

  auto mymax = [&](ll a, ll b) -> ll { return max(a, b); };
  auto st = make_seg_tree_lazy<ll, ll>(0LL, 0LL, mymax, mymax, mymax, vector<ll>(sz, 0LL));
