...
int main(/* int argc, char *argv[] */) {
  ...
  auto valy = [&](Line& e, Real x) -> Real {
    auto [code, r1, r2] = e.intersect_coeff(Line(Point(0, 1), Point(x, 0)));
    if (code == Line::IST_ONE) return (e.base + r1 * e.dir).y;
    return 0.0;
  };

  auto func = [&](VP A, VP B) -> double {
    sort(ALL(A), cmp);
    sort(ALL(B), cmp);
    auto ev = vector<double>();
    vector<VP> AB{A, B};
    REP(i, 0, 2) {
      auto& C = AB[i];
      ev.emplace_back(C[0].x);
      ev.emplace_back(C[1].x);
      ev.emplace_back(C[2].x);
    }
    REP(i, 0, 3) REP(j, 0, 3) {
      ll ii = (i + 1) % 3;
      ll jj = (j + 1) % 3;
      Line e = Line::connect(A[i], A[ii]);
      Line f = Line::connect(B[j], B[jj]);
      auto [code, r1, r2] = e.intersect_coeff(f);
      if (code == Line::IST_ONE) {
        Point p = e.base + r1 * e.dir;
        ev.emplace_back(p.x);
      }
    }
    sort(ALL(ev));
    double xLo = max(A[0].x, B[0].x);
    double xHi = min(A[2].x, B[2].x);
    DLOGK(A, B, ev, xLo, xHi);
    Real ret = 0;
    REP(i, 0, SIZE(ev) - 1) {
      if (ev[i] < xLo - eps) continue;
      if (ev[i] > xHi - eps) break;
      double xt = (ev[i] + ev[i + 1]) / 2.0;
      Line e01 = Line::connect(A[0], A[1]);
      Line e02 = Line::connect(A[0], A[2]);
      Line e12 = Line::connect(A[1], A[2]);
      Line f01 = Line::connect(B[0], B[1]);
      Line f02 = Line::connect(B[0], B[2]);
      Line f12 = Line::connect(B[1], B[2]);
      Line aLo, aHi, bLo, bHi;
      if (xt < A[1].x) {
        if (valy(e01, xt) < valy(e02, xt)) { aLo = e01; aHi = e02; }
        else { aLo = e02; aHi = e01; }
      }else {
        if (valy(e12, xt) < valy(e02, xt)) { aLo = e12; aHi = e02; }
        else { aLo = e02; aHi = e12; }
      }
      if (xt < B[1].x) {
        if (valy(f01, xt) < valy(f02, xt)) { bLo = f01; bHi = f02; }
        else { bLo = f02; bHi = f01; }
      }else {
        if (valy(f12, xt) < valy(f02, xt)) { bLo = f12; bHi = f02; }
        else { bLo = f02; bHi = f12; }
      }
      Line lo = valy(aLo, xt) < valy(bLo, xt) ? bLo : aLo;
      Line hi = valy(aHi, xt) < valy(bHi, xt) ? aHi : bHi;
      DLOGK(i, xt, aHi, aLo, bHi, bLo);
      DLOGK(i, valy(aLo, xt), valy(bLo, xt), valy(aHi, xt), valy(bHi, xt));
      if (valy(lo, xt) > valy(hi, xt)) {
        continue;
      }
      double d1 = valy(hi, ev[i]) - valy(lo, ev[i]);
      double d2 = valy(hi, ev[i + 1]) - valy(lo, ev[i + 1]);
      double s = (d1 + d2) * (ev[i + 1] - ev[i]) / 2;
      DLOGK(i, ev[i], ev[i + 1], lo, hi, d1, d2, s);
      ret += s;
    }
    return ret;
  };
  ...
  double ans = 0;
  REP(i, 1, N - 1) REP(j, 1, M - 1) ans += func(VP{P[0], P[i], P[i + 1]}, VP{Q[0], Q[j], Q[j + 1]});
  cout << ans << endl;
