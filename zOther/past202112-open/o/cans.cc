...
int main(/* int argc, char *argv[] */) {
  ...
  ll c = *max_element(ALL(B));
  auto vec1 = vector(c + 1, Fp(0));
  auto vec2 = vector(c + 1, Fp(0));
  REP(i, 0, N) {
    vec1[B[i]] += 1;
    vec2[c - B[i]] += 1;
  }
  auto res = convolution(vec1, vec2);
  
  Comb<Fp> cb(N);
  Fp ans = Fp(0);
  REP(i, 1, N + 1) {
    if (c + i >= SIZE(res)) break;
    ans += res[c + i] * Fp(S[i - 1]) * cb.fact(N - 2) * Fp(N - i);
  }
  cout << ans << endl;
  return 0;
}
