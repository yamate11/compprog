...
int main(/* int argc, char *argv[] */) {
  ...
  ll sz = cc.size();
  auto stU = make_seg_tree<Fp>(Fp(0), myadd, vector<Fp>(sz));
  auto stD = make_seg_tree<Fp>(Fp(0), myadd, vector<Fp>(sz));
  Fp tot = Fp(0);
  REP(i, 0, N) {
    ll a = cc.c(A[i]);
    Fp d = stD.query(0, a);
    tot += d;
    stU.update(a, stU[a] + d + 1);
    Fp u = stU.query(a + 1, sz);
    tot += u;
    stD.update(a, stD[a] + u + 1);
  }
  cout << tot << endl;
