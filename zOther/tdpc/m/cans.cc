...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, R; cin >> H >> R;
  auto G = vector(R, vector(R, false));
  for (ll i = 0; i < R; i ++) {
    for (ll j = 0; j < R; j++) {
      ll x; cin >> x; G[i][j] = x == 1;
    }
  }
  Matrix<Fp> mat(R, R);
  // auto F = vector(R, vector(R, Fp(0)));
  for (ll i = 0; i < R; i++) {
    auto tsp = vector(1LL << R, vector(R, Fp(0)));
    tsp[1LL << i][i] = 1;
    for (ll x = 0; x < (1LL << R); x++) {
      for (ll j = 0; j < R; j++) {
        if (tsp[x][j] == 0) continue;
        for (ll k = 0; k < R; k++) {
          if (G[j][k] && (! (1LL & (x >> k)))) {
            tsp[x | (1LL << k)][k] += tsp[x][j];
            mat.at(i, k) += tsp[x][j];
          }
        }
      }
  ...
  auto mat1 = mat.matpower(H);
  Matrix<Fp> vec(R, 1);
  vec.at(0, 0) = 1;
  auto vec2 = mat1 * vec;
  cout << vec2.at(0, 0) << endl;
