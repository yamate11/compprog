...
namespace ddfa {  // digit DFA

  // The order matters in the following: max and min have been used in "operator*"
  constexpr int STABLE_UNACC = 0;    
  constexpr int UNACC = 1;
  constexpr int ACC = 2;
  constexpr int STABLE_ACC = 3;
  constexpr bool is_stable(int e) { return (e >> 1 & 1) == (e >> 0 & 1); }
  constexpr bool is_accepted(int e) { return e >= 2; }

  pair<int, int> dec(int s, int ns2) {
    int s1 = s / ns2;
    int s2 = s % ns2;
    return {s1, s2};
  }
  int enc(int s1, int s2, int ns2) {
    return s1 * ns2 + s2;
  }
  template<typename DFA1, typename DFA2>
  auto tr_comp(const DFA1& dfa1, const DFA2& dfa2) {
    return [&dfa1, &dfa2](int s, int a, int i) -> int {
      auto [s1, s2] = dec(s, dfa2.ns);
      int new_s1 = dfa1.trans(s1, a, i);
      int new_s2 = dfa2.trans(s2, a, i);
      return enc(new_s1, new_s2, dfa2.ns);
    };
  }
  template<typename DFA1, typename DFA2>
...
struct DFA {
  ...
  void run(int len) {
    ...
    result[init_state] = 1;
    for (int i = 0; i < len; i++) {
      vector<I> prev = move(result);
      result = vector<I>(ns);
      for (int s = 0; s < ns; s++) {
        if (ddfa::is_stable(acc[s])) result[s] += prev[s] * na;
        else for (int a = 0; a < na; a++) result[tr(s, a, i)] += prev[s];
      }
    }
  }
  I cnt_acc() const {
    I ret = 0;
    for (int s = 0; s < ns; s++) if (ddfa::is_accepted(acc[s])) ret += result[s];
    return ret;
  }

  void debug_run(int len, const auto& nss) const {
#if DEBUG
    IntDupPerm idp(na, len);
    int nc = ssize(nss);
    while (idp.get()) {
      vector rec(nc, vector<int>());
      auto do_rec = [&](int u) -> void {
        int t;
        for (int k = nc - 1; k >= 1; k--) {
          tie(u, t) = ddfa::dec(u, nss[k]);
          rec[k].push_back(t);
        }
        rec[0].push_back(u);
      };
      ...
      cerr << "<";
      for (int i = 0; i < len; i++) cerr << idp[i];
      cerr << "> ";
      for (int k = 0; k < nc; k++) {
        cerr << "[";
        for (int i = 0; i < ssize(rec[k]); i++) {
          if (i > 0) cerr << ",";
          cerr << rec[k][i];
        }
        cerr << "] ";
  ...
  template<typename DFA2>
  friend auto operator*(const DFA& dfa1, const DFA2& dfa2) -> DFA<na, I, ddfa::tr_comp_t<DFA, DFA2>> {
    auto tr_c = ddfa::tr_comp(dfa1, dfa2);
    vector<int> new_acc(dfa1.ns * dfa2.ns);
    for (int i = 0; i < dfa1.ns; i++) for (int j = 0; j < dfa2.ns; j++) {
        new_acc[ddfa::enc(i, j, dfa2.ns)] = min(dfa1.acc[i], dfa2.acc[j]);
      }
    return DFA<na, I, decltype(tr_c)> (dfa1.ns * dfa2.ns, ddfa::enc(dfa1.init_state, dfa2.init_state, dfa2.ns),
                                       move(new_acc), tr_c, dfa2.ns);
  }
};

namespace ddfa {
  template<int na, typename I>
  auto make_dfa_less_eq(const auto& S) {
    auto tr = [&S](int s, int a, int i) -> int {
      ll d = S[i] - '0';
      if (a < d) return 1;
      else if (a > d) return 2;
      else return 0;
    };
    return DFA<na, I, decltype(tr)>(3, 0, vector{ACC, STABLE_ACC, STABLE_UNACC}, tr);
  }

  template<int na, typename I>
  auto make_dfa_multiple(int n) {
    auto tr = [n](int s, int a, int i) -> int { return (10 * s + a) % n; };
    vector<int> acc(n, UNACC);
    acc[0] = ACC;
    return DFA<na, I, decltype(tr)>(n, 0, acc, tr);
  }
};
...
#if 1
  auto dfa = ddfa::make_dfa_multiple<10, ll>(3) * ddfa::make_dfa_less_eq<10, ll>(S);
#else
  auto dfa1 = ddfa::make_dfa_multiple<10, ll>(3);
  auto dfa2 = ddfa::make_dfa_less_eq<10, ll>(S);
  auto dfa = dfa1 * dfa2;
#endif

  dfa.run(len);
  cout << dfa.cnt_acc() << endl;

  dfa.debug_run(2, vector<int>{3, 3});
