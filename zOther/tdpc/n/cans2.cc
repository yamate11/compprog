...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  Comb comb(N);
  auto edge = vector(N-1, vector<ll>());
  using sta = tuple<ll, ll, ll>;
  auto nbr = vector(N, vector<sta>());
  for (ll i = 0; i < N-1; i++) {
    ll a, b; cin >> a >> b; a--; b--;
    edge[i] = {a, b};
    nbr[a].emplace_back(sta(b, i, 0));
    nbr[b].push_back(sta(a, i, 1));
  }
  using stb = pair<ll, Fp>;
  auto tbl = vector(N-1, vector(2, stb(-1, 0)));
  auto func = [&](auto rF, ll e, ll d) -> stb {
    auto [num, cmb] = tbl[e][d];
    if (num < 0) {
      ll nd = edge[e][1-d];
      ll pt = edge[e][d];
      num = 0;
      cmb = 1;
      for (auto [c, j, dd] : nbr[nd]) {
        if (c == pt) continue;
        auto [num2, cmb2] = rF(rF, j, dd);
        num += num2;
        cmb *= comb.comb(num, num2) * cmb2;
      }
      tbl[e][d] = stb(num + 1, cmb);
    }
    return tbl[e][d];
  };
  Fp ans = 0;
  for (ll e = 0; e < N-1; e++) {
    auto [num1, cmb1] = func(func, e, 0);
    auto [num2, cmb2] = func(func, e, 1);
    ans += comb.comb(N-2, num1 - 1) * cmb1 * cmb2;
  }
  cout << ans << endl;
