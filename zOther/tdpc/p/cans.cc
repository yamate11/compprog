...
int main(/* int argc, char *argv[] */) {
  ...
  using sta = vector<vector<Fp>>;
  sta ret_init = vector(3, vector<Fp>(K + 1, Fp(0)));
  auto dfs = [&](auto rF, ll nd, ll pt) -> sta {
    ...
    for (ll c : nbr[nd]) {
      ...
      for (ll k1 = 0; k1 <= K; k1++) {
        for (ll k2 = 0; k2 <= K; k2++) {
          auto g = [&](ll t3, ll k3, ll t1, ll t2) -> void {
            if (0 <= k3 && k3 <= K) ret[t3][k3] += prev[t1][k1] * cr[t2][k2];
          };
          for (ll t1 = 0; t1 <= 2; t1++) {
            for (ll t2 = 0; t2 <= 2; t2++) g(t1, k1 + k2, t1, t2);
          }
          g(1, k1 + k2 + 1, 0, 0);
          g(1, k1 + k2,     0, 1);
          g(2, k1 + k2,     1, 0);
  ...
  sta result = dfs(dfs, 0, -1);
  Fp ans = result[0][K] + result[1][K] + result[2][K];
  cout << ans << endl;
