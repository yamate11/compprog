...
int naive(istream& cin, ostream& cout) {
  ...
  do {
    string s;
    for (ll i = 0; i < cnt; i++) s += s0[v[i]];
    if (pool.find(s) != pool.end()) continue;
    pool.insert(s);
    bool succ = true;
...
int body(istream& cin, ostream& cout) {
  ...
  for (ll i = 0; i < 26; i++) {
    ...
    auto prev = move(cur);
    cur = vector<Fp>(charCnt + x);
    for (ll t = 0; t < (ll)prev.size(); t++) {
      Fp val = prev[t];
      if (val == 0) continue;
      for (ll g = 1; g <= x; g++) {
        ll delta = (x - 1) - (g - 1);
        for (ll p = max(0LL, g + t - 1 - charCnt); p <= min(g, t); p++) {
          cur[t + delta - p] +=
            val * cb.comb(x - 1, g - 1) *
            cb.comb(t, p) * cb.comb(charCnt + 1 - t, g - p);
        }
      }
    }
    charCnt += x;
