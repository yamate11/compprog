...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, L; cin >> N >> L;
  vector<vector<bool>> ws(9, vector<bool>(256));
  ll maxlen = 0;
  for (ll i = 0; i < N; i++) {
    string s; cin >> s;
    ll len = s.size();
  ...
  ll pow = 1LL << maxlen;
  auto cur_init = vector(pow, vector(pow, Fp(0)));
  auto cur = cur_init;
  cur[0][1] = 1;
  for (ll i = 0; i < L; i++) {
    auto prev = move(cur);
    cur = cur_init;
    for (ll x = 0; x < pow; x++) {
      ll x1 = (x << 1) & ~(1LL << maxlen);
      for (ll y = 0; y < pow; y++) {
        if (prev[x][y] == 0) continue;
        ll y1 = (y << 1) & ~(1LL << maxlen);
        for (ll j = 0; j <= 1; j++) {
          ll z = j;
          bool succ = false;
          for (ll t = 0; t < maxlen; t++) {
            if ((y >> t) & 1) {
              if (ws[t + 1][z]) {
                succ = true;
                break;
          ...
          if (succ) cur[x1 + j][y1 + 1] += prev[x][y];
          else      cur[x1 + j][y1    ] += prev[x][y];
        }
      }
    }
  }
