...
int main(/* int argc, char *argv[] */) {
  ...
  ll open = 0;
  for (ll i = 0; i < N; i++) {
    for (ll j = 0; j < M; j++) if (brd.at(i, j) == '.') open++;
  }
  
  auto check = [&](auto& cbrd, ll si, ll sj) -> bool {
    Board<int> visited(N, M, 0);
    ll nv = 0;
    auto sub = [&](auto rF, const BrdIdx bi) -> void {
      DLOGKL("sub called", bi);
      if (cbrd.at(bi) == '#') return;
  ...
  for (ll i = 0; i < N; i++) {
	...
	Board<char> cbrd(N, M, '#');
	for (ll ii = 0; ii < N; ii++) {
	  for (ll jj = 0; jj < M; jj++) cbrd.at(ii, jj) = brd.at(ii, jj);
	}
	cbrd.at(i, j) = '.';
	if (check(cbrd, i, j)) cnt++;
      }
    }
  }
  cout << cnt << endl;
