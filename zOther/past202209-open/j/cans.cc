...
int main(/* int argc, char *argv[] */) {
  ...
  Real H, W, D; cin >> H >> W >> D;
  if (H < W) swap(H, W);
  H /= 2.0;
  W /= 2.0;
  Real tot = H * W;
  Real diag = sqrt(W * W + H * H);
  if (D <= W) {
    Real a = G_PI * D * D / 4.0;
    cout << a / tot << endl;
  }else if (D <= H) {
    Real y  = sqrt(D * D - W * W);
    Real th = Point(W, y).angle(Point(0, 1));
    Real a0 = D * D * th / 2.0;
    Real a1 = W * y / 2.0;
    cout << (a0 + a1) / tot << endl;
  }else if (diag >= D) {
    Real y0 = sqrt(D * D - W * W);
    Real x0 = sqrt(D * D - H * H);
    Real th0 = Point(W, y0).angle(Point(x0, H));
    Real a0 = D * D * th0 / 2.0;
    Real a1 = W * y0 / 2.0;
    Real a2 = x0 * H / 2.0;
    cout << (a0 + a1 + a2) / tot << endl;
  }else {
    cout << 1.0 << endl;
  }
  return 0;
