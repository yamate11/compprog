...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](const auto& C, auto& zc) -> void {
    REP(x, 0, 1LL << N) zc[popcount((u64)x)][x] = C[x];
    REP(i, 0, N + 1) zeta_lower(zc[i]);
  };

  // @InpVec(1LL << N, A) [KP07YHNi]
  auto A = vector(1LL << N, ll());
  for (int i = 0; i < 1LL << N; i++) { ll v; cin >> v; A[i] = v; }
  // @End [KP07YHNi]
  // @InpVec(1LL << N, B) [shGsbPsZ]
  auto B = vector(1LL << N, ll());
  for (int i = 0; i < 1LL << N; i++) { ll v; cin >> v; B[i] = v; }
  // @End [shGsbPsZ]

  vector za(N + 1, vector(1LL << N, Fp(0)));
  vector zb(N + 1, vector(1LL << N, Fp(0)));
  f(A, za);
  f(B, zb);

  vector zq(2*N + 1, vector(1LL << N, Fp(0)));
  REP(x, 0, 1LL << N) {
    ll k = popcount((u64)x);
    REP(i, 0, k + 1) REP(j, 0, k + 1) {
      zq[i + j][x] += za[i][x] * zb[j][x];
    }
  }
  REP(i, 0, 2*N + 1) moebius_lower(zq[i]);
