...
int main(/* int argc, char *argv[] */) {
  ...
  vector<bool> idx_lose(N + 1);
  REP(i, 0, L) idx_lose[B[i]] = true;

  vector<Fp> v2b(N + 1);
  REP(i, 0, M) v2b[A[i]] = Fp(1) / Fp(M);
  Poly p2b(move(v2b));

  Fp p_lose;
  vector<Fp> tbl(N);
  tbl[0] = Fp(1);
  auto f = [&](auto rF, ll lo, ll hi) -> void {
    if (lo + A[0] >= hi) return;
    ll mid = (lo + hi) / 2;
    rF(rF, lo, mid);

    vector<Fp> v1(mid - lo);
    REP(i, lo, mid) v1[i - lo] = tbl[i];
    Poly p1(move(v1));
    Poly p2 = p2b.cutoff(hi - lo - 1);
    Poly p = p1 * p2;
    REP(i, mid - lo, hi - lo) {
      if (idx_lose[lo + i]) {
        p_lose += p.getCoef(i);
      }else {
        tbl[lo + i] += p.getCoef(i);
      }
    }
