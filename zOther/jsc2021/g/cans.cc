...
// @@ !! LIM(mod matrix UnionFind)
// --> f:gcd f:intDiv mod f:power matrix UnionFind
...
int main(/* int argc, char *argv[] */) {
  ...
  map<ll, ll> idx;
  for (ll i = 0; i < N; i++) {
    if (uf.leader(i) == i) leaders.push_back(i);
  }
  ll sz = leaders.size();
  sort(leaders.begin(), leaders.end());
  for (ll i = 0; i < sz; i++) idx[leaders[i]] = i;
  Matrix<Fp> mat(sz, sz);
  for (auto [i, j] : conn) {
    ll p = idx[uf.leader(i)];
    ll q = idx[uf.leader(j)];
    if (p == q) continue;
    mat.at(p, p) += 1;
    mat.at(q, q) += 1;
    mat.at(p, q) -= 1;
  ...
  Matrix<Fp> mminor(sz - 1, sz - 1);
  for (ll i = 0; i < sz - 1; i++) {
    for (ll j = 0; j < sz - 1; j++) mminor.at(i, j) = mat.at(i, j);
  }
  cout << mminor.determinant() << endl;
