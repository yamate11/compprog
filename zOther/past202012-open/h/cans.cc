...
int main(/* int argc, char *argv[] */) {
  ...
  que.emplace(r, c);
  while (!que.empty()) {
    BrdIdx p = que.front(); que.pop();
    for (BrdIdx d : BrdIdx::nbr4) {
      BrdIdx pd = p + d;
      if (ans.at(pd) >= 0) continue;
      if (brd.at(pd) == '#') ans.at(pd) = -1;
      else if (brd.at(pd) == '.') ans.at(pd) = 1;
      else if (d == BrdIdx(1, 0) && brd.at(pd) == '^') ans.at(pd) = 1;
      else if (d == BrdIdx(-1, 0) && brd.at(pd) == 'v') ans.at(pd) = 1;
      else if (d == BrdIdx(0, 1) && brd.at(pd) == '<') ans.at(pd) = 1;
      else if (d == BrdIdx(0, -1) && brd.at(pd) == '>') ans.at(pd) = 1;
      if (ans.at(pd) == 1) que.push(pd);
    }
  }
  for (ll i = 0; i < H; i++) {
    for (ll j = 0; j < W; j++) {
      if (brd.at(i, j) == '#') cout << '#';
      else if (ans.at(i, j) == 1) cout << 'o';
      else cout << 'x';
    }
    cout << "\n";
  }
