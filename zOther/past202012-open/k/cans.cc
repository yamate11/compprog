...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> brd(cin, 4, 4, '.');
  ll x0 = 0;
  for (ll i = 0; i < 4; i++) {
    for (ll j = 0; j < 4; j++) {
      if (brd.at(i, j) == '#') x0 |= (1LL << (4*i + j));
    }
  }
  DLOGK(x0);
  vector memo(1LL<<16, -1.0);
  memo[0] = 0.0;
  auto func = [&](auto rF, ll x) -> double {
    DLOGK(x);
    double& ret = memo[x];
    if (ret < 0) {
      double val = 1e10;
      for (ll i = 0; i < 4; i++) {
        for (ll j = 0; j < 4; j++) {
          ...
          if (outcnt == 5) continue;
          double exp = w / (1.0 - outcnt * 0.2);
          val = min(val, exp);
          DLOGK(w, outcnt, exp, val);
        }
      }
      ret = val;
    }
