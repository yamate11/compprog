...
ll resid(const auto& basis, ll x) {
  for (ll b : basis) x = min(x, x ^ b);
  return x;
}

vector<ll> get_basis(const auto& xs, ll mask) {
  vector<ll> ret;
  for (ll x : xs) {
    ll y = resid(ret, x & mask);
    if (y) ret.push_back(y);
  }
  return ret;
}

bool mask_contains(auto& basis, ll mask, ll t0) {
  auto vec = get_basis(basis, mask);
  return resid(vec, t0 & mask) == 0;
}
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, N) cin >> A[i] >> B[i];
  auto possible = [&]() -> bool {
    auto vec = get_basis(A, (1LL << width) - 1);
    if (SIZE(vec) < N) return true;
    ll vmin = 1LL << width;
    for (ll x : vec) vmin = min(vmin, x);
    return vmin <= K;
  };
  if (not possible()) {
    cout << -1 << endl;
  ...
  ll maskL = (1LL << width) - 1;
  vector<ll> FB;
  REP(i, N) {
    ll z = (A[i] << width) | B[i];
    for (ll x : FB) z = min(z, z ^ x);
    if (z) FB.push_back(z);
  }
  DLOGK(FB);
  auto func = [&](ll t1, ll mask1) -> ll {
    REP2R(i, width - 1, 0) {
      t1 |= 1LL << i;
      mask1 |= 1LL << i;
      auto vec = get_basis(FB, mask1);
      if (not mask_contains(vec, mask1, t1)) t1 ^= 1LL << i;
    }
    return t1 & maskL;
  };
  ll mask = 0;
  ...
  auto upd = [&]() -> void {
    ll mcb = mask_contains(FB, mask, t);
    DLOGK(mask, t, mcb);
    if (mcb) {
      ll tans = func(t, mask);
      DLOGK(t, mask, tans);
