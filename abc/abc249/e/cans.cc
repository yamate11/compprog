...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Fp> tenp = {Fp(1), Fp(10), Fp(100), Fp(1000), Fp(10000)};
  vector tbl(N + 1, vector(N + 1, Fp(0)));
  vector diff(N + 1, vector(N + 1, Fp(0)));
  tbl[0][0] = 1;
  diff[1][0] = -1;
  REP(j, N + 1) {
    REP(i, N + 1) {
      if (i - 1 >= 0) tbl[i][j] += tbl[i-1][j] + diff[i][j];
      REP(k, 4) {
        ll mul = i == 0 ? 26 : 25;
        if (i + tenp[k]   < N + 1 and j + 2 + k < N + 1) diff[i + tenp[k    ]][j + 2 + k] += tbl[i][j] * mul;
        if (i + tenp[k+1] < N + 1 and j + 2 + k < N + 1) diff[i + tenp[k + 1]][j + 2 + k] -= tbl[i][j] * mul;
      }
      DLOGK(i, j, tbl[i][j]);
    }
  }
