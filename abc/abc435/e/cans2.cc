...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  // @InpMVec(Q, ((L, dec=1), (R, dec=0))) [jaDMKJgz]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  ll sz = cc.size();
  vector<ll> init_vec(sz, 0LL);
  REP(i, 0, sz - 1) init_vec[i] = cc.d(i + 1) - cc.d(i);
  ...
  REP(i, 0, (ll)init_vec.size()) dat_init.emplace_back(init_vec[i], 1);
