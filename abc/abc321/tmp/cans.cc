...
int main(/* int argc, char *argv[] */) {
  ...
  while (dic.get()) {
    auto check = [&]() -> bool {
      ...
      while (dip2.get()) {
        vector vec(N, 0LL);
        REP(j, 0, N) vec[j] = (dic.at(dip2.at(j)) + j) % N;
        auto vec_s = vec;
        sort(ALL(vec_s));
        bool b = FORALL(j, 0, N, vec_s[j] == j);
        if (b) {
          DLOGKL("SUCCESS: ", dic.vec_view(), dip2.vec_view(), vec);
          return true;
        }
      }
      return false;
    ...
    ll s = 0;
    REP(i, 0, N) s += dic.at(i);
    if (s % N != 0) continue;
    if (not check()) {
      DLOGKL("FAIL: ", dic.vec_view());
      return 0;
    }
