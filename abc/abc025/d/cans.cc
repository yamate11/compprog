...
int main(/* int argc, char *argv[] */) {
  ...
  vector<BrdIdx> pos(sz * sz + 1, BrdIdx(-1, -1));
  REP(r, 0, sz) REP(c, 0, sz) {
    if (brd.at(r, c) > 0) pos[brd.at(r, c)] = BrdIdx(r, c);
  }
  vector<int> tbl(1LL << sz * sz, -1);
  auto f = [&](auto rF, u64 y) -> Fp {
    int& val = tbl[y];
    if (val < 0) {
      auto g = [&](auto& bi) -> Fp {
        auto sub = [&](auto dir) -> bool {
          bool ng = brd.in(bi - dir) and (y >> brd.enc(bi - dir) & 1)
            and brd.in(bi + dir) and (not (y >> brd.enc(bi + dir) & 1));
          return not ng;
        };

        bool b = FORALL_C(dir, BrdIdx::nbr4, sub(dir));
        if (b) {
          ll new_y = y | (1LL << brd.enc(bi));
          DLOGKL("rec", y, new_y);
          return rF(rF, new_y);
        }else {
          DLOGKL("nonrec", y);
      ...
      if (num == sz * sz) {
        ...
        if (brd.in(bi)) {
          ...
          REP(r, 0, sz) REP(c, 0, sz) {
            auto bj = BrdIdx(r, c);
            if (brd.at(bj) == 0 and not (y >> brd.enc(bj) & 1)) {
              sum += g(bj);
            }
          }
          val = (int)sum;
