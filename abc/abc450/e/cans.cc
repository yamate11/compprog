...
// @@ !! LIM(debug unordered_map cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  ll big = 4.1e18;
  string X, Y; cin >> X >> Y;
  vector<ll> len{ssize(X), ssize(Y)};
  for (ll i = 2; true; i++) {
    ll t = len[i - 2] + len[i - 1];
    len.push_back(t);
    if (t > big) break;
  }
  ...
  vector preX(ssize(X) + 1, vector(num_alph, 0LL));
  vector preY(ssize(Y) + 1, vector(num_alph, 0LL));
  REP(i, 0, ssize(X)) {
    preX[i + 1] = preX[i];
    preX[i + 1][X[i] - 'a']++;
  }
  ...
  vector pre(ssize(len), vector(num_alph, 0LL));
  pre[0] = preX.back();
  pre[1] = preY.back();
  REP(i, 2, ssize(len)) REP(d, 0, num_alph) pre[i][d] = pre[i - 2][d] + pre[i - 1][d];

  auto f = [&](char c, ll x) -> ll {
    ll n = 1;
    for (; true; n += 2) {
      if (len[n] >= x) break;
    }
    auto g = [&](auto rF, ll m, ll y) -> ll {
      ll ret;
      if (m == 0) { ret = preX[y][c - 'a']; }
      else if (m == 1) { ret = preY[y][c - 'a']; }
      else {
        ll k = len[m - 1];
        if (y <= k) ret = rF(rF, m - 1, y);
        else {
          ret = pre[m - 1][c - 'a'] + rF(rF, m - 2, y - k);
        }
      }
      DLOGK(m, y, ret);
      return ret;
