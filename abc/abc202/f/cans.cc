...
int main(/* int argc, char *argv[] */) {
  ...
  using Fp = FpA;
  ll N; cin >> N;
  vector<IPoint> P(N); cin >> P;

  auto inner = [&](ll a, ll b, ll c, ll d) -> bool {
    auto os = [&](ll aa, ll bb, ll cc, ll dd) -> bool {
      IPoint u = (P[cc] - P[dd]).rotate(1);
      IPoint base = P[cc];
      ll t1 = u.x * (P[aa].x - base.x) + u.y * (P[aa].y - base.y);
      ll t2 = u.x * (P[bb].x - base.x) + u.y * (P[bb].y - base.y);
      return (t1 > 0) == (t2 > 0);
    };
    return os(a, b, c, d) && os(a, c, d, b) && os(a, d, b, c);
  };

  auto inn = vector(N, vector(N, vector(N, 0LL)));
  auto parity = vector(N, vector(N, vector(N, (int8_t)-1)));

  auto getInn = [&](ll i, ll j, ll k) -> pair<ll, ll> {
    if (i > j) swap(i, j);
    if (j > k) swap(j, k);
    if (i > j) swap(i, j);
    return {inn[i][j][k], parity[i][j][k]};
  };
  ...
  for (ll i = 0; i < N; i++) {
    for (ll j = i + 1; j < N; j++) {
      ...
      for (ll k = j + 1; k < N; k++) {
        IPoint v2 = P[k] - P[j];
        ll s = abs(v1.x * v2.y - v1.y * v2.x);
        parity[i][j][k] = s % 2;
        for (ll l = 0; l < N; l++) {
          if (l == i || l == j || l == k) continue;
          if (inner(l, i, j, k)) inn[i][j][k] += 1;
        }
      }
    }
  }
  ...
  for (ll i = 0; i < N; i++) {
    ...
    for (ll j = 0; j < N; j++) {
      if (i == j || P[j].x < pi.x ||
          (P[j].x == pi.x && P[j].y < pi.y)) continue;
      Q.push_back(j);
    }
    Q.push_back(i);
    sort(Q.begin(), Q.end(),
         [&](ll i1, ll i2) -> bool {
           ...
           if (i1 == i) return true;
           const IPoint& q1 = P[i1];
           const IPoint& q2 = P[i2];
           ll a1 = q1.x - pi.x;
           ll a2 = q2.x - pi.x;
           ll b1 = q1.y - pi.y;
           ll b2 = q2.y - pi.y;
           ...
           ll v1 = a2 > 0 ? 1 : -1;
           ll v2 = a1 > 0 ? 1 : -1;
           ll v3 = b2 * a1 - a2 * b1 > 0 ? 1 : -1;
           return v1 * v2 * v3 > 0;
         });
    ll sz = Q.size();
    if (sz < 3) continue;
    vector tbl = vector(sz, vector(sz, vector(2, Fp(0))));
    for (ll j = 1; j < sz; j++) {
      tbl[0][j][0] = 1;
    }
    for (ll j = 0; j < sz; j++) {
      for (ll k = j + 1; k < sz; k++) {
        for (ll l = k + 1; l < sz; l++) {
          IPoint u = P[Q[k]] - P[Q[j]];
          IPoint v = P[Q[l]] - P[Q[k]];
          ll t = u.x * v.y - v.x * u.y;
          if (t < 0) continue;
          auto [m, pp] = getInn(i, Q[k], Q[l]);
          Fp x = 1LL << m;
          tbl[k][l][0] += tbl[j][k][pp] * x;
          tbl[k][l][1] += tbl[j][k][1-pp] * x;
        }
      }
    }
    for (ll j = 1; j < sz; j++) {
