...
int main(/* int argc, char *argv[] */) {
  ...
  Comb cb(K + 1);
  vector binom(K + 1, vector(K + 1, Fp(0)));
  REP(n, K + 1) REP(r, n + 1) binom[n][r] = cb.comb(n, r);

  vector tbl_init(M + 1, vector(K + 1, Fp(0)));
  tbl_init[0][0] = 1;
  auto tbl = tbl_init;
  REP(j, K + 1) tbl[0][j] = 1;
  Fp w_sum = 0;
  REP(i, N) {
    ...
    tbl = tbl_init;
    w_sum += W[i];
    Fp prob = Fp(W[i]) / w_sum;
    vector<Fp> p1(K + 1), p2(K + 1);
    p1[0] = p2[0] = Fp(1);
    REP(r, K) {
      p1[r + 1] = p1[r] * prob;
      p2[r + 1] = p2[r] * (1 - prob);
    }
    REP2(k, 1, M + 1) REP(j, K + 1) {
      DLOGKL("*** ", k, j);
      Fp val = 0;
      REP(r, j + 1) {
        ll kk = r == 0 ? k : k-1;
        Fp thisv = binom[j][r] * p1[r] * p2[j - r] * prev[kk][j - r];
        val += thisv;
        DLOGK(r, thisv, val, kk, prev[kk][j - r]);
      }
      tbl[k][j] = val;
    }
#if DEBUG
    REP2(k, 0, M + 1) REP(j, K+1) {
      DLOGK(k, j, tbl[k][j]);
    }
#endif
  }
