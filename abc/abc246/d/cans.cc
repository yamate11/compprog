...
int main(/* int argc, char *argv[] */) {
  ...
  auto check0 = [&](ll t) -> bool { return 4*t*t*t >= N; };
  ll t0 = binsearch_i<ll>(check0, ll(1e6), -1);
  ll ans = 4*t0*t0*t0;
  DLOGK(t0, ans);
  for (ll t = 2 * t0; t*t*t >= N; t--) {
    DLOGK(t, t*t*t);
    ll a1 = t / 2;
    ll b1 = t - a1;
    DLOGK(a1, b1);
    if (a1*a1*a1 + a1*a1*b1 + a1*b1*b1 + b1*b1*b1 == N) {
      DLOG("foo");
      cout << N << endl;
      return 0;
    }
    ll g;
    if (a1*a1*a1 + a1*a1*b1 + a1*b1*b1 + b1*b1*b1 > N) {
      g = a1*a1*a1 + a1*a1*b1 + a1*b1*b1 + b1*b1*b1;
    }else {
      auto check1 = [&](ll a) -> bool {
        ll b = t - a;
        return a*a*a + a*a*b + a*b*b + b*b*b >= N; 
      };
      ll a = binsearch_i<ll>(check1, t, a1);
      ll b = t - a;
      g = a*a*a + a*a*b + a*b*b + b*b*b;
      DLOGK(t, a, b, g);
    }
    ans = min(ans, g);
  }
