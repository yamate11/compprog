...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, L; cin >> N >> L;
  vector<Fp> pl(27);
  REP(i, 27) pl[i] = power<Fp>(i, L);
  DLOGK(pl);
  vector<bitset<26>> bss(1LL << N);
  REP(i, N) {
    string s; cin >> s;
    REP(j, SIZE(s)) bss[1LL << i][s[j] - 'a'] = true;
  }
  Fp ans = 0;
  REP2(x, 1, 1LL << N) {
    ll z = x & (-x);
    ll y = x & ~z;
    if (y != 0) bss[x] = bss[y] & bss[z];
    ll sgn = __builtin_popcountll(x) % 2 == 0 ? -1 : 1;
    ans += Fp(sgn) * pl[bss[x].count()];
    DLOGK(x, bss[x], bss[x].size(), pl[bss[x].count()], sgn);
  }
  cout << ans << endl;
