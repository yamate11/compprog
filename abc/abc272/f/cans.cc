...
int main(/* int argc, char *argv[] */) {
  ...
  auto la = lcp_array(X, sa);
  DLOGK(sa);
  DLOGK(la);
  
  vector rev_sa(2, vector(N, 0LL));
  REP(i, 0, 4*N) {
    ll p = sa[i] / N;
    ll q = sa[i] % N;
    if (p == 1 or p == 3) continue;
    if (p == 2) p = 1;
    rev_sa[p][q] = i;
  }
  DLOGK(rev_sa);
  ...
  step[0] = 0;
  REP(i, 0, 4*N - 1) {
    if (la[i] < N) step[i + 1] = step[i] + 1;
    else step[i + 1] = step[i];
  }
  
  auto cmp_sa = [&](ll i, ll j) -> ll {
    if (step[i] == step[j]) return 0;
    if (i < j) return -1;
    return 1;
  };
  ...
  auto cmp = [&](const pll& p, const pll& q) -> bool {
    auto [u, x] = p;
    auto [v, y] = q;
    ll rc = cmp_sa(rev_sa[u][x], rev_sa[v][y]);
    DLOGKL("cmp", p, q, rc);
    if (rc < 0) return true;
    if (rc > 0) return false;
    if (u == 0 and v == 1) return true;
    return false;
  };
  DLOGK(P);
  sort(ALL(P), cmp);
