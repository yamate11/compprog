...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  ll lim = llround(floor(sqrt(M)));
  vector<BrdIdx> move;
  REP(i, 0, lim + 1) {
    ll j = llround(floor(sqrt(M - i * i)));
    if (i * i + j * j != M) continue;
    move.emplace_back(i, j);
    if (j != 0) move.emplace_back(i, -j);
    if (i != 0) move.emplace_back(-i, j);
    if (i != 0 and j != 0) move.emplace_back(-i, -j);
  }
  ll big = 1e18;
  Board<ll> brd(N, N, big);
  queue<BrdIdx> que;
  que.emplace(0, 0);
  brd.at(0, 0) = 0;
  while (not que.empty()) {
  ...
  REP(i, 0, N) {
    REPOUT(j, 0, N, brd.at(i, j) == big ? -1 : brd.at(i, j), " ");
  }
