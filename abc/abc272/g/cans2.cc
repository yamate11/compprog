...
int main(/* int argc, char *argv[] */) {
  ...
  auto checkrow = [&](ll i) -> ll {
    vector<ll> P;
    vector B(N, 0LL);
    REP(j, 0, N) B[j] = abs(A[j] - A[i]);
    REP(jj, 0, min(N, 30LL)) {
      ll j = (N <= 30) ? jj : randrange(0, N);
      if (B[j] >= 3) {
        for (auto [p, r] : prfac(B[j], primes)) {
          if (p >= 3) P.push_back(p);
        }
      }
    }
    sort(ALL(P));
    ...
    for (ll p : P) {
      ll cnt = 0;
      REP(j, 0, N) if (B[j] % p == 0) cnt++;
      if (cnt > N - cnt) return p;
    }
    return -1;
  };
