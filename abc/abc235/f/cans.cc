...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl1_init(2, vector(2, vector(1LL << M, Fp(0))));
  vector tbl2_init(2, vector(2, vector(1LL << M, Fp(0))));
  auto tbl1 = tbl1_init;
  auto tbl2 = tbl2_init;
  tbl2[1][1][0] = 1;
  for (char c : N) {
    ...
    tbl1 = tbl1_init;
    tbl2 = tbl2_init;
    REP(eq, 2) REP(az, 2) REP(x, 1LL << M) {
      if (prev2[eq][az][x] == 0) continue;
      REP(d, 10) {
        if (eq && d > ic) continue;
        ll eqNew = eq && d == ic;
        ll azNew = az && d == 0;
        ...
        tbl1[eqNew][azNew][xNew] += 10 * prev1[eq][az][x] + d * prev2[eq][az][x];
        tbl2[eqNew][azNew][xNew] += prev2[eq][az][x];
      }
    }
#if DEBUG
    DLOG("");
    DLOGK(c);
    REP(eq, 2) REP(az, 2) REP(x, 1LL << M) DLOGK(eq, az, x, tbl2[eq][az][x]);
#endif

  }
  Fp ans = tbl1[1][0][(1LL << M) - 1] + tbl1[0][0][(1LL << M) - 1];
  ll num = tbl2[1][0][(1LL << M) - 1] + tbl2[0][0][(1LL << M) - 1];
  DLOGK(num);
  cout << ans << endl;
