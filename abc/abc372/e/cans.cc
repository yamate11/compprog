  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      if (a == b) {
        ...
        ll i = 0, j = 0;
        while (true) {
          if (i == ssize(rec[a]) and j == ssize(rec[b])) {
            break;
          }else if (i == ssize(rec[a])) {
            mg.push_back(rec[b][j]);
            j++;
          }else if (j == ssize(rec[b])) {
            mg.push_back(rec[a][i]);
            i++;
          }else if (rec[a][i] > rec[b][j]) {
            mg.push_back(rec[a][i]);
            i++;
          }else {
            mg.push_back(rec[b][j]);
