...
int main(/* int argc, char *argv[] */) {
  ...
  auto tbl = vector(N, vector<Fp>(T + 1));
  tbl[0][0] = 1;

  auto induce = [&](ll l, ll m, ll r) -> void {
    for (ll i = 0; i < M; i++) {
      ll x = A[i];
      ll y = B[i];
      vector<Fp> vx(m - l), vy(m - l);
      for (ll j = 0; j < m-l; j++) {
        vx[j] = tbl[x][l + j];
        vy[j] = tbl[y][l + j];
      }
      vector<Fp> w(r - l);
      for (ll j = 0; j < r-l; j++) w[j] = p[i][j];
      auto vec1 = convolution(vx, w);
      auto vec2 = convolution(vy, w);
      for (ll j = 0; j < r - m; j++) {
        tbl[y][m + j] += vec1[m - l + j];
        tbl[x][m + j] += vec2[m - l + j];
      }
    }
  };

  auto solve = [&](auto rF, ll l, ll r) -> void {
    if (l + 1 == r) return;
    ll m = (l + r) / 2;
    rF(rF, l, m);
    induce(l, m, r);
