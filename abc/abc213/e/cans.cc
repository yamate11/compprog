...
int main(/* int argc, char *argv[] */) {
  ...
  while (! deq.empty()) {
    ...
    DLOGKL("from deq", d, bi);
    if (d != dist.at(bi)) continue;
    if (bi == goal) {
      cout << d << endl;
      return 0;
    }
    for (BrdIdx diff : BrdIdx::nbr4) {
      BrdIdx bx = bi + diff;
      if (brd.at(bx) == '#') continue;
      if (dist.at(bx) <= d) continue;
      dist.at(bx) = d;
      deq.emplace_front(d, bx);
      DLOGKL("to deq front", d, bx);
    }
    for (ll r = -2; r <= 2; r++) {
      for (ll c = -2; c <= 2; c++) {
        if (abs(r) == 2 && abs(c) == 2) continue;
        if (r == 0 && c == 0) continue;
        BrdIdx bx = bi + BrdIdx(r, c);
        if (! brd.in(bx)) continue;
        ll newd = d + 1;
        if (dist.at(bx) <= newd) continue;
        dist.at(bx) = newd;
        deq.emplace_back(newd, bx);
        DLOGKL("to deq back", newd, bx);
      }
    }
  }
  assert(0);
