...
// @@ !! LIM(mod segTree cmpNaive)
// --> f:gcd f:intDiv mod f:<< segTree cmpNaive
...
int body(istream& cin, ostream& cout) {
  ...
  for (ll i = 0; i < N; i++) {
    auto [x, d] = XD[i];
    ll idx = lower_bound(XD.begin(), XD.end(), xd_t(x + d, -1)) - XD.begin();
    infl[i] = idx;
  }
  ...
  for (ll i = N-1; i >= 0; i--) {
    ll cl;
    if (infl[i] == i + 1) cl = i + 1;
    else {
      ll m = st.query(i + 1, infl[i]);
      cl = max(m, infl[i]);
    }
    st.update(i, i+1, cl);
    tbl[i] = tbl[cl] + tbl[i+1];
  }
