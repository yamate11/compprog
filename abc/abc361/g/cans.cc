...
int main(/* int argc, char *argv[] */) {
  ...
  ll lim = max(*max_element(ALL(X)), *max_element(ALL(Y))) + 1;
  vector pts(lim, vector<ll>{-2, lim + 2});
  REP(i, 0, N) {
    pts[X[i]].push_back(Y[i]);
  }
  REP(i, 0, lim) sort(ALL(pts[i]));
  vector itv(lim, vector<pll>());
  vector nseq(lim, vector<ll>());
  vector rseq(0, pll());
  REP(i, 0, lim) {
    auto& c_pts = pts[i];
    auto& c_nseq = nseq[i];
    REP(j, 0, ssize(c_pts) - 1) {
      if (c_pts[j] + 1 < c_pts[j + 1]) {
        itv[i].emplace_back(c_pts[j] + 1, c_pts[j + 1] - 1);
        ll seq = ssize(rseq);
        ll m = ssize(c_nseq);
        rseq.emplace_back(i, m);
        c_nseq.push_back(seq);
      }
    }
  }
  ll sz = ssize(rseq);
  ...
  REP(i, 0, lim - 1) {
    ...
    while (k1 < m1 and k2 < m2) {
      if (vec1[k1].second < vec2[k2].first) k1++;
      else if (vec2[k2].second < vec1[k1].first) k2++;
      else {
        uf.merge(nseq[i][k1], nseq[i + 1][k2]);
        if (vec1[k1].second < vec2[k2].second) k1++;
        else k2++;
      }
    }
  }
  ...
  ll ans = 0;
  REP(seq, 0, sz) {
    if (uf.leader(seq) == seq) {
      auto sub = [&]() -> ll {
        ll ret = 0;
        for (int s : gi.group(seq)) {
          auto [i, j] = rseq[s];
          if (i == 0 or i == lim - 1) return -1;
          auto [lo, hi] = itv[i][j];
          if (lo == -1 or hi == lim) return -1;
          ret += hi - lo + 1;
        }
        return ret;
      };
