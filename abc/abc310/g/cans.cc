...
// Graph derived from function fwd: N -> N
struct FtnGraph {
  int size;                     // number of nodes
  vector<int> fwd;              // forward function
  int numCC;                    // number of CC (connected component)
  vector<int> cc;               // cc[nd] is the index of the CC that nd belongs to
  vector<vector<int>> cycle;    // cycle[cc] is the list of nodes of the cycle of cc
  vector<int> cycleNode;        // cycleNode[nd] is the nearest node in the cycle of CC that nd belongs to
  vector<vector<int>> children; // the list of children in the tree rooted at the cycle
  vector<int> depth;            // depth[nd] is the distance from nd to the cycle

  FtnGraph(int size_ = 0) : size(size_), fwd(size, -1),
                            numCC(0), cc(size, -1), cycle(0), cycleNode(size, -1), children(size), depth(size, -1) {}
  void set_edge(int i, int j) { fwd[i] = j; }

  void build() {
    vector bwd(size, vector<int>());
    for (int i = 0; i < size; i++) bwd[fwd[i]].push_back(i);

    vector<int> stidx(size, -1);
    for (int i = 0; i < size; i++) {
      if (cc[i] >= 0) continue;
      vector<int> stack;
      int cur = i;
      for ( ; stidx[cur] < 0; cur = fwd[cur]) {
        stidx[cur] = SIZE(stack);
        stack.push_back(cur);
      }
      vector<int> new_cycle;
      copy(stack.begin() + stidx[cur], stack.end(), back_inserter(new_cycle));
      for (int rt : new_cycle) cycleNode[rt] = rt;
      auto dfs = [&](auto rF, int nd, int rt, int dp) -> void {
        cc[nd] = numCC;
        depth[nd] = dp;
        cycleNode[nd] = rt;
        for (int x : bwd[nd]) if (cycleNode[x] < 0) {
            children[nd].push_back(x);
            rF(rF, x, rt, dp + 1);
          }
      };
      for (int rt : new_cycle) dfs(dfs, rt, rt, 0);
      cycle.push_back(move(new_cycle));
      numCC++;
    }
  }
};
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(g, 0, fg.numCC) {
    Fp base = 0;
    ll clen = SIZE(fg.cycle[g]);
    auto func = [&](ll sidx, ll len, Fp val) -> void {
      DLOGKL("  func", sidx, len, val);
      Fp unit = val / K;
      ll p = len / clen;
      ll q = len % clen;
      base += unit * p;
      if (q > 0) {
        diff[fg.cycle[g][sidx]] += unit;
        if (sidx + q >= clen) {
          diff[fg.cycle[g][0]] += unit;
          diff[fg.cycle[g][sidx + q - clen]] -= unit;
        }else {
          diff[fg.cycle[g][sidx + q]] -= unit;
        }
      }
    };
    REP(i, 0, clen) {
      ...
      auto dfs = [&](auto rF, ll nd, ll dp) -> void {
        if (dp == 0) func((i + 1) % clen, K, B[nd]);
        else if (dp == 1) func(i, K, B[nd]);
        else {
          diff[fg.fwd[nd]] += Fp(B[nd]) / Fp(K);
          if (dp - (K + 1) >= 1) diff[stack[dp - (K + 1)]] -= Fp(B[nd]) / Fp(K);
          else func(i, K - (dp - 1), B[nd]);
        }
        for (ll cld : fg.children[nd]) {
          stack.push_back(nd);
          rF(rF, cld, dp + 1);
          stack.pop_back();
        }
      };
      dfs(dfs, fg.cycle[g][i], 0);
    }
    DLOGK(base);
    DLOGK(diff);
    REP(i, 0, clen) {
      auto dfs = [&](auto rF, ll nd) -> Fp {
        Fp t = 0;
        for (ll cld : fg.children[nd]) t += rF(rF, cld);
        if (nd != fg.cycle[g][i]) ans[nd] = t + diff[nd];
        return ans[nd];
      };
      dfs(dfs, fg.cycle[g][i]);
    }
    Fp curval = base;
    REP(i, 0, clen) {
      curval += diff[fg.cycle[g][i]];
      ans[fg.cycle[g][i]] = curval;
    }
  }
  REPOUT(i, 0, N, ans[i], " ");
