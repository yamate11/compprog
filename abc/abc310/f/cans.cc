...
int body(istream& cin, ostream& cout) {
  ...
  Fp div = 1;
  REP(i, 0, N) div *= A[i];
  vector tbl_init(1LL << (ten + 1), Fp(0));
  auto tbl = tbl_init;
  tbl[1] = 1;
  REP(i, 0, N) {
    auto prev = move(tbl);
    ...
    REP(x, 0, 1LL << (ten + 1)) {
      ...
      REP(t, 1, min(ten, A[i]) + 1) {
        ll y = x;
        REP(j, 0, (ten + 1)) if ((x >> j & 1) and (j + t <= ten)) y |= 1LL << (j + t);
        tbl[y] += prev[x];
      }
      if (ten < A[i]) tbl[x] += prev[x] * (A[i] - ten);

    }
    DLOGK(i, tbl);
  }
  Fp cnt = 0;
  REP(x, 0, 1LL << (ten + 1)) if (x >> ten & 1) cnt += tbl[x];
  DLOGK(cnt);
  cout << cnt / div << endl;
