...
// @@ !! LIM(segTree debug unordered_map cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  // @InpMVec(Q, ((A, dec=1), (B, dec=1))) [tk95ip6y]
  auto A = vector(Q, ll());
  auto B = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  auto intersect = [&](ll p, ll q) -> bool {
    if (A[p] < A[q] and A[q] < B[p] and B[p] < B[q]) return true;
    if (A[q] < A[p] and A[p] < B[q] and B[q] < B[p]) return true;
    return false;
  };
  ...
  REP(q, 0, Q) {
    auto check = [&]() -> bool {
      REP(p, 0, q) if (draw[p] and intersect(p, q)) return false;
      return true;
    };
    if (check()) {
      cout << "Yes\n";
...
int body(istream& cin, ostream& cout) {
  ...
  st.set_data(vector<ll>(N, 0));
  
  REP(_q, 0, Q) {
    ll a, b; cin >> a >> b; a--; b--;
    ll va = st.at(a), vb = st.at(b);
    if (va == vb and st.query(a, b + 1) == va) {
      cout << "Yes\n";
      st.update(a, b, 1);
    }else {
      cout << "No\n";
