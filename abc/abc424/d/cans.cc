  ...
  auto solve = [&]() -> ll {
    ...
    REP(i, 0, H) {
      REP(j, 0, W) {
        ...
        REP(x, 0, 1LL << len) {
          ll y = x << 1 & mask;
          updMin(tbl[y], prev[x] + 1);
          if (not (S[i][j] == '#' and j > 0 and (x >> W & 1) and (x >> (W - 1) & 1) and (x >> 0 & 1))) {
            updMin(tbl[y | (S[i][j] == '#')], prev[x]);
          }
        }
      }
    }
    return *min_element(ALL(tbl));
  };
