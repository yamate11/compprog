...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    REP(i, 0, H) {
      string s; cin >> s;
      REP(j, 0, W) T[i] |= (s[j] == '#' ? (1LL << j) : 0LL);
    }
    ll big = 1e18;
    vector<ll> tbl_init(1LL << W, big);
    auto tbl = tbl_init;
    tbl[0] = 0;
    REP(i, 0, H) {
      auto prev = move(tbl);
      ...
      REP(x, 0, 1LL << W) {
        ...
        while (true) {
          ...
          auto check = [&]() -> bool {
            REP(j, 0, W - 1) {
              if ((x >> j & 3) == 3 and (x1 >> j & 3) == 3) return false;
            }
            return true;
          };

          if (check()) updMin(tbl[x1], prev[x] + popcount((u64)y));
          if (y == 0) break;
          y = (y - 1) & T[i];
        }
      }
    ...
    ll ans = big;
    REP(x, 0, 1LL << W) updMin(ans, tbl[x]);
    return ans;
  };
