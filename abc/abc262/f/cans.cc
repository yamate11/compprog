...
int naive(istream& cin, ostream& cout) {
  auto myless = [&](auto& vec1, auto& vec2) {
    REP(i, SIZE(vec1)) {
      if (i == SIZE(vec2)) return false;
      if (vec1[i] < vec2[i]) return true;
      if (vec1[i] > vec2[i]) return false;
    }
    if (SIZE(vec1) < SIZE(vec2)) return true;
    return false;
  };
  ...
  REP(k, K + 1) {
    ...
    do {
      ...
      ll jj = 0;
      REP(i, N) {
        if (jj < k and v[jj] == i) jj++;
        else {
          w.push_back(P[i]);
        }
      }
      vector<ll> z;
      copy(w.begin() + (N - K), w.begin() + (N - k), back_inserter(z));
      copy(w.begin(), w.begin() + (N - K), back_inserter(z));
      if (myless(z, ret)) ret = move(z);
    }while (ic.next());
  }
  for (ll a : ret) cout << a << " ";
  cout << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  auto g = [&](auto& vec, ll st, ll L) -> vector<ll> {
    vector<ll> ret;
    vector<pll> w;
    REP2(i, st, SIZE(vec)) w.emplace_back(vec[i], i);
    sort(ALL(w));
    ll cur = -1;
    for (auto [val, idx] : w) {
      if (idx < cur) continue;
  ...
  auto f = [&](auto& vec, ll K, ll t) -> vector<ll> {
    vector<ll> ret;
    priority_queue<pll, vector<pll>, greater<pll>> pque;
    // is i < SIZE(vec)?????
    ll i = 0;
    for (; i < K + 1; i++) pque.emplace(vec[i], i);
    ll cur = -1;
    ll rest = t - K;
    while (not pque.empty()) {
      auto [val, idx] = pque.top(); pque.pop();
      if (idx < cur) continue;
      cur = idx;
      ret.push_back(val);
      rest--;
      if (rest == 0) break;
      if (i < t) {
        pque.emplace(vec[i], i); i++;
      }
    }
    DLOGKL("f", vec, K, t, ret);
    return ret;
  };
  ...
  ll M = N - varg;
  auto p2 = f(P, K - M, varg);
  DLOGKL("p2", p2);
  ll L = p2[0];
  auto p3 = g(P, varg, L);
  DLOGKL("p3", p3);
  copy(ALL(p2), back_inserter(p3));
  DLOGKL("p3 alter", p3);
  if (myless(p1, p3)) {
    for (ll a : p1) cout << a << " ";
  }else {
