...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  // @InpVec(K, S, type=string) [JWPmqwBd]
  auto S = vector(K, string());
  for (int i = 0; i < K; i++) { string v; cin >> v; S[i] = v; }
  // @End [JWPmqwBd]
  ...
  DLOGK(init, seq);
  mp[init] = seq++;
  auto f = [&](auto rF, const auto& vec) -> void {
    vector<int> tr(n_alph);
    REP(d, 0, n_alph) {
      auto new_vec = vec;
      bool succ = true;
      REP(k, 0, K) {
        if (S[k][vec[k]] - 'a' == d) {
          if (vec[k] + 1 < ssize(S[k])) new_vec[k]++;
          else {
            succ = false;
            break;
          }
          ...
          new_vec[k] = 0;
          REP(start, 1, vec[k] + 1) {
            if (S[k].substr(start, vec[k] - start) + (char)('a' + d) == S[k].substr(0, vec[k] - start + 1)) {
              new_vec[k] = vec[k] - start + 1;
              break;
            }
          }
        }
    ...
    trans[mp[vec]] = move(tr);
    DLOGK(vec, mp[vec], trans[mp[vec]]);
  };
  f(f, init);
  using MyMat = Matrix<Fp>;
  MyMat mat(seq, seq);
  REP(i, 0, seq) REP(d, 0, n_alph) {
    ll p = trans[i][d];
    if (p >= 0) {
      mat.rs(p, i) = mat.at(p, i) + Fp(1);
    }
  }
  DLOGK(mat);
  vector<Fp> ist(seq, Fp(0));
  ist[0] = Fp(1);
  auto V = power<MyMat>(mat, N) * ist;
  Fp ans = 0;
  REP(i, 0, seq) ans += V[i];
  cout << ans << endl;
