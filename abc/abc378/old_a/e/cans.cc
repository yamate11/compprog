...
// @@ !! LIM(segTree f:intDiv debug cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  REPrev(i, N - 1, 0) {
    ...
    if (r > 0) {
      ...
      ll k;
      if (start == 0) {
        k = st.query(start + y, M);
      }else if (M <= start + y) {
        k = st.query(start + y - M, start);
      }else {
        k = st.query(start + y, M) + st.query(0, start);
      }
      ll f = M * k;
      ll g = e - f;
      thisA += g;
      ans += thisA;
      DLOGK(i, e, f, g, k, thisA);

      st.rs(start) = st.at(start) + 1;

      start = imod(start - r, M);
    }else {
      ans += thisA;
      st.rs(start) = st.at(start) + 1;
    }
