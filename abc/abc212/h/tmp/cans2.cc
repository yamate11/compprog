...
int main(/* int argc, char *argv[] */) {
  ...
  auto put = [&](BrdIdx bi, ll pNew, auto& rec) -> void {
    ll diff = 0;
    for (auto [bp, p] : pieces) { diff -= erase(bi, bp, p, rec); }
    diff += place(bi, pNew, rec);
    // もともと利いているか?
  };
  ...
  vector<ll> cand = {R, B, Kn, Ki};
  auto dfs = [&](auto rF, ll lev, BrdIdx bi, ll fill) -> void {
    vector<rec_t> rec;
    fill += put(bi, cand[lev], rec);
    if (fill == 64) {
      for (ll i = 0; i < 8; i++) {
        auto [p, bp] = pieces[i];
        cout << p << " " << bp << "\n";
    ...
    for (ll r = 0; r < 8; r++) {
      for (ll c = 0; c < 8; c++) {
        BrdIdx bb(r, c);
        if (brd.at(bb) == 0) rF(rF, lev + 1, bb, fill);
      }
    }
    restore(rec);
  };
