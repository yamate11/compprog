...
int main(/* int argc, char *argv[] */) {
  ...
  Board<Fp> A(H, W, 0), R(H, W, 0), D(H, W, 0), RD(H, W, 0);
  D.at(H-1, W-1) = 1;
  R.at(H-1, W-1) = 1;
  RD.at(H-1, W-1) = 1;
  for (ll r = H - 1; r >= 0; r--) {
    for (ll c = W - 1; c >= 0; c--) {
      ...
      if (brd.at(r, c) == '#') continue;
      Fp val = D.at(r+1, c) + R.at(r, c+1) + RD.at(r+1, c+1);
      D.at(r, c) = val + D.at(r + 1, c);
      R.at(r, c) = val + R.at(r, c + 1);
      RD.at(r, c) = val + RD.at(r + 1, c + 1);
    }
  }
  cout << D.at(1,0) + R.at(0,1) + RD.at(1,1) << endl;
