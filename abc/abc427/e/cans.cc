...
// @@ !! LIM(board f:updMaxMin perm debug input forall unordered_map)
...
// @DefStruct(A, (r0, r1, c0, c1), istr=False) [no9cm66n]
struct A_t {
  ll r0;
  ll r1;
  ll c0;
  ll c1;
  A_t(ll r0_ = ll(), ll r1_ = ll(), ll c0_ = ll(), ll c1_ = ll()) : r0(r0_), r1(r1_), c0(c0_), c1(c1_) {}
  friend ostream& operator<<(ostream& ostr, const A_t& t) {
    ostr << "(" << t.r0 << ", " << t.r1 << ", " << t.c0 << ", " << t.c1 << ")";
    return ostr;
  }
};
// @End [no9cm66n]
...
int main(/* int argc, char *argv[] */) {
  ...
  cin >> brd;
  BrdIdx start;
  REP(i, 0, H) REP(j, 0, W) if (brd.at(i, j) == 'T') start = BrdIdx(i, j);

  auto encA = [&](A_t a) -> ll {
    return a.c1 + (W + 1) * (a.c0 + (W + 1) * (a.r1 + (H + 1) * a.r0));
  };

  ll szEncA = (W + 1) * (W + 1) * (H + 1) * (H + 1);
  vector<ll> done(szEncA);
  REP(r0, 0, H + 1) REP(r1, 0, H + 1) REP(c0, 0, W + 1) REP(c1, 0, W + 1) {
    done[encA(A_t(r0, r1, c0, c1))] = FORALL(r, r0, r1, FORALL(c, c0, c1, brd.at(r, c) != '#'));
    DLOGK(r0, r1, c0, c1, encA(A_t(r0, r1, c0, c1)), done[encA(A_t(r0, r1, c0, c1))]);
  }
  DLOGK(done);
  
  using sta = pair<BrdIdx, A_t>;
  queue<sta> que;
  sta init{{0,0}, {0, H, 0, W}};
  que.emplace(init);
  auto encP = [&](sta p) -> ll { return (H + p.first.r) + (2*H + 1) * (W + p.first.c + (2*W + 1) * encA(p.second)); };
  ll szEncP = (2*H + 1) * (2*W + 1) * szEncA;
  vector dist(szEncP, big);
  dist[encP(init)] = 0;
  while (not que.empty()) {
    auto p = que.front(); que.pop();
    auto [bi, a] = p;
    DLOGKL("popped", p, a, encA(a), done[encA(a)]);
    ll d = dist[encP(p)];
    if (done[encA(a)]) {
      cout << d << endl;
      return 0;
    ...
    for (auto dir : BrdIdx::nbr4) {
      auto bj = bi + dir;
      auto pos = start - bj;
      if (not (brd.in(pos) and a.r0 <= pos.r and pos.r < a.r1 and a.c0 <= pos.c and pos.c < a.c1
               and brd.at(pos) == '#')) {
        A_t newA;
        newA.r0 = max<ll>(a.r0, -bj.r);
        newA.r1 = min<ll>(a.r1, H - bj.r);
        newA.c0 = max<ll>(a.c0, -bj.c);
        newA.c1 = min<ll>(a.c1, W - bj.c);
        sta newP(bj, newA);
        DLOGK(newP);
        if (dist[encP(newP)] == big) {
          dist[encP(newP)] = d + 1;
          que.push(newP);
          DLOGKL("pushed", newP);
        }
