...
struct Vicinity {
  ...
  const Cont& cont;
  Vicinity(const Cont& cont_) : cont(cont_) {}
...
int main(/* int argc, char *argv[] */) {
  ...
  ll cent_lo = -lim - 1, cent_hi = 2 * lim + 1;
  CoordCompr cc;
  REP(i, 0, Q) cc.add(X[i]);
  cc.add(cent_lo);
  cc.add(cent_hi);
  ll sz = cc.size();
  auto stEx = make_seg_tree(0LL, plus<ll>(), vector<ll>(sz, 0LL));

  set<ll> ssMem{cent_lo, cent_hi};
  set<ll> ssCc{cent_lo, cent_hi};

  auto vmem = Vicinity(ssMem);
  auto vcc = Vicinity(ssCc);
  ...
  REP(q, 0, Q) {
    ...
    if (tp == 1) {
      ll lo = *vmem.lt(x);
      ll hi = *vmem.gt(x);
      if (not ssMem.contains(x)) {
        if (hi - x <= K) ssCc.erase(hi);
        if (x - lo >  K) ssCc.insert(x);
        ssMem.insert(x);
        stEx.rs(cc.c(x)) = 1;
      }else {
        stEx.rs(cc.c(x)) = 0;
        ssMem.erase(x);
        ssCc.erase(x);
        if (hi - lo > K) ssCc.insert(hi);
      }
    }else if (tp == 2) {
      cout << stEx.query(cc.c(*vcc.le(x)), cc.c(*vcc.gt(x))) << "\n";
    }else assert(0);
    DLOGK(q, ssMem, ssCc);
    DLOGK(stEx.vec_view());
  }
