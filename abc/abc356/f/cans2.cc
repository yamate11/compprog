...
int main(/* int argc, char *argv[] */) {
  ...
  REP(q, 0, Q) {
    ...
    ll x = X[q];
    if (tp == 1) {
      auto it = ssMem.find(x);
      if (it == ssMem.end()) {
        auto it2 = ssMem.lower_bound(x + 1);
        ll y = *it2;
        if (y - x <= K) {
          ssCc.erase(y);
        }
      ...
      auto it = ssCc.lower_bound(x);
      ll y = *it;
      ll lo, hi;
      if (x == y) {
        auto it2 = ssCc.lower_bound(x + 1);
        hi = *it2;
        lo = x;
      }else {
        hi = y;
      ...
      ll val = stEx.query(cc.c(lo), cc.c(hi));
      cout << val << "\n";
