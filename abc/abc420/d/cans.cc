...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, H) REP(j, 0, W) {
    ...
    else if (brd.at(i, j) == 'G') goal = BrdIdx(i, j);
  }
  ll big = 1e18;
  vector dist(2, vector(H, vector(W, big)));
  dist[0][start.r][start.c] = 0;
  using sta = pair<ll, BrdIdx>;
  queue<sta> que;
  que.emplace(0, start);
  while (not que.empty()) {
    auto [par, bi] = que.front(); que.pop();
    for (auto dir : BrdIdx::nbr4) {
      BrdIdx bj = bi + dir;
      
      auto cango = [&]() -> pair<bool, ll> {
        char c = brd.at(bj);
        if (c == '.' or c == 'S' or c == 'G') return {true, par};
        else if (c == '?') return {true, 1 - par};
        else if (c == 'o' and par == 0) return {true, par};
        else if (c == 'x' and par == 1) return {true, par};
        else return {false, -1};
      };

      auto [b, new_par] = cango();
      if (b and dist[new_par][bj.r][bj.c] == big) {
        dist[new_par][bj.r][bj.c] = dist[par][bi.r][bi.c] + 1;
        que.emplace(new_par, bj);
      }
    }
  }
  ll ans = min(dist[0][goal.r][goal.c], dist[1][goal.r][goal.c]);
  if (ans == big) ans = -1;
  cout << ans << endl;
