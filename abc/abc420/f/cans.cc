...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, K; cin >> N >> M >> K;
  // @InpVec(N, S, type=string) [Hy7RZ7N4]
  ...
  REP(i, 1, N + 1) REP(j, 1, M + 1) {
    ll w0 = min(j, K / i);
    f[i][j] = f[i - 1][j] + j * w0 - (w0 * (w0 - 1) / 2);
  }
  DLOGK(H);
  DLOGK(f);
  ll ans = 0;
  REP(i, 0, N) {
    CartesianProduct cp(H[i]);
    auto dfs = [&](auto rF, ll lo, ll hi, ll x, ll base) -> void {
      if (x < 0) return;
      ll w = hi - lo;
      ll h = H[i][x];
      ans += f[h][w] - f[base][w];
      rF(rF, lo, x, cp.left[x], h);
      rF(rF, x + 1, hi, cp.right[x], h);
    };
    dfs(dfs, 0, M, cp.root, 0);
  }
  cout << ans << endl;
