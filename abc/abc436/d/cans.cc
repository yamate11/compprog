...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  Board brd(H, W, '#');
  vector char_done(26, false);
  cin >> brd;
  ll nchar = 26;
  vector warp(nchar, vector<BrdIdx>());
  REP(i, 0, H) REP(j, 0, W) {
    char c = brd.at(i, j);
    if ('a' <= c and c <= 'z') { warp[c - 'a'].emplace_back(i, j); }
  }
  ll big = 1e18;
  ...
  while (not que.empty()) {
    ...
    if ('a' <= c and c <= 'z') {
      if (not char_done[c - 'a']) {
        char_done[c - 'a'] = true;
        for (auto bj : warp[c - 'a']) {
          if (dist[bj.r][bj.c] == big) {
            dist[bj.r][bj.c] = dist[bi.r][bi.c] + 1;
            que.push(bj);
          }
