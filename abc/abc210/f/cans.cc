...
int main(/* int argc, char *argv[] */) {
  ...
  vector<vector<sta>> rec(lim);
  ll cnt = 0;
  for (ll i = 0; i < N; i++) {
    for (auto [p, r] : prfac(A[i], primes)) {
      rec[p].emplace_back(i, false);
      cnt++;
    }
    for (auto [p, r] : prfac(B[i], primes)) {
      rec[p].emplace_back(i, true);
      cnt++;
    }
  }
  two_sat ts(N + cnt);
  ...
  for (ll p : primes) {
    ...
    for (ll i = 0; i < sz; i++) {
      auto [x, b] = rec[p][i];
      ts.add_clause(x, ! b, seq + i, false);
      if (i > 0) {
        ts.add_clause(x, ! b, seq + i-1, true);
        ts.add_clause(seq + i, false, seq + i-1, true);
      }
    }
    seq += sz;
  }
