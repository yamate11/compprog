...
int main(/* int argc, char *argv[] */) {
  ...
  REP(e, 0, 2) {
    dist[e][start.r][start.c] = 0;
    que.emplace(e, start);
  }
  auto horiz = vector<BrdIdx>{BrdIdx{0, 1}, BrdIdx{0, -1}};
  auto vert = vector<BrdIdx>{BrdIdx{1, 0}, BrdIdx{-1, 0}};
  while (not que.empty()) {
    auto [e, bi] = que.front(); que.pop();
    ll curdist = dist[e][bi.r][bi.c];
    DLOGK(e, bi, curdist);
    if (e == 0) {
      for (auto dir : horiz) {
        auto bj = bi + dir;
        if (brd.at(bj) != '#' and dist[1 - e][bj.r][bj.c] == big) {
          dist[1 - e][bj.r][bj.c] = curdist + 1;
          que.emplace(1 - e, bj);
          DLOGKL("pushedA", 1-e, bj, curdist + 1);
        }
      }
    }else {
      for (auto dir : vert) {
        ...
        if (brd.at(bj) != '#' and dist[1 - e][bj.r][bj.c] == big) {
          dist[1 - e][bj.r][bj.c] = curdist + 1;
          que.emplace(1 - e, bj);
          DLOGKL("pushedB", 1-e, bj, curdist + 1);
        }
      }
    }
  }
