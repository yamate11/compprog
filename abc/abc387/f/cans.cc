...
int body(istream& cin, ostream& cout) {
  ...
  vector rev(N, vector<ll>());
  REP(i, 0, N) {
    if (A[i] != i and not fg.is_on_cycle(i)) {
      rev[A[i]].push_back(i);
    }
  }

  vector<bool> done(N, false);
  vector tbl(N, vector(M, Fp(0)));
  vector stbl(N, vector(M, Fp(0)));
  auto calc = [&](auto rF, ll nd) -> void {
    if (done[nd]) return;
    for (ll j : rev[nd]) rF(rF, j);
    REP(i, 0, M) {
      Fp v = 1;
      for (ll j : rev[nd]) v *= stbl[j][i];
      tbl[nd][i] = v;
    }
    stbl[nd] = tbl[nd];
    REP(i, 1, M) stbl[nd][i] += stbl[nd][i - 1];
    done[nd] = true;
  };
  REP(nd, 0, N) calc(calc, nd);
  
  Fp ans = 1;
  vector<bool> circDone(fg.numcc(), false);
  REP(nd, 0, N) {
    if (not fg.is_on_cycle(nd)) continue;
    ll ccid = fg.ccid(nd);
    if (circDone[ccid]) continue;
    circDone[ccid] = true;
    Fp val = 0;
    REP(i, 0, M) {
      Fp cval = 1;
