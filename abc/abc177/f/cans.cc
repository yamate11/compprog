...
int main(/* int argc, char *argv[] */) {
  ...
  */
  ...
  DLOG("dist_en=", dist_en);
  DLOG("en_dist=", en_dist);
  for (ll i = 0; i < H; i++) {
    ll a, b; cin >> a >> b; a--;
    bool keep_last = b < W && en_dist.find(b) == en_dist.end();
    auto it = en_dist.lower_bound(a);
    ll last_e = -1, last_d = -1;
    while (true) {
      if (it == en_dist.end() || it->first >= b) break;
      auto [e, d] = *it;
      last_e = e, last_d = d;
      dist_en.erase({d, e});
      it = en_dist.erase(it);
    }
    if (keep_last && last_e >= 0) {
      ll new_d = last_d + (b - last_e);
      en_dist[b] = new_d;
      dist_en.emplace(new_d, b);
    }
    DLOG("i=", i, "dist_en=", dist_en);
    DLOG("en_dist=", en_dist);
    if (dist_en.empty()) {
      cout << -1 << "\n";
    }else {
      cout << i + 1 + dist_en.begin()->first << "\n";
    }
  }
