...
// @@ !! LIM(digit mod f:gcd debug cmpNaive f:intDiv)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    for (ll x = K; true; x += K) {
      ...
      for (ll i = 0; i + ssize(S) <= ssize(t); i++) {
        if (t.substr(i, ssize(S)) == S) {
          cout << x << endl;
          return;
        }
      }
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<Fp> pow10(sz + w + 10);
    pow10[0] = 1;
    REP(i, 1, sz + w + 10) pow10[i] = pow10[i - 1] * 10;
    DLOGK(pow10);
    ...
    sta ans = sta(w, 0, (ll)(-m0 * pow10[w]));
    DLOGKL("init ans", ans);
    
    auto zeropad = [&](ll width, ll y) -> string {
      string sy = to_string(y);
      if (width == 0) return string();
      // if (width <= ssize(sy)) return sy;
      return string(width - ssize(sy), '0') + sy;
    };

    auto comp = [&](sta z1, sta z2) -> bool {
      auto [b1, x1, y1] = z1;
      auto [b2, x2, y2] = z2;
      ll lx1 = x1 == 0 ? 0 : ssize(to_string(x1));
      ll lx2 = x2 == 0 ? 0 : ssize(to_string(x2));
      ll len1 = lx1 + sz + b1;
      ll len2 = lx2 + sz + b2;
      string sy1 = zeropad(b1, y1);
      string sy2 = zeropad(b2, y2);
      if (len1 != len2) return len1 < len2;
      REP(i, 0, len1) {
        char c1 = i < lx1 ? to_string(x1)[i] : i < lx1 + sz ? S[i - lx1] : sy1[i - lx1 - sz];
        char c2 = i < lx2 ? to_string(x2)[i] : i < lx2 + sz ? S[i - lx2] : sy2[i - lx2 - sz];
    ...
    REP(b, 0, w) {
      ...
      if (a < 6 and b > 0) {
        REP(x, 0, du.pow(a)) {
          ll t = -(x * pow10[b + sz] + m0 * pow10[b]);
          if (t < du.pow(b)) {
            DLOGK(a, b, x, t);
            if (comp(sta(b, x, t), ans)) ans = sta(b, x, t);
            DLOGK(ans);
            break;
          }
        }
        ...
        REP(y, 0, du.pow(b)) {
          ll m2 = -(pow10[b] * m0 + y);
          auto [g, s, t] = eGCD((ll)(pow10[sz + b]), K);
          DLOGKL("  ", b, y, m2, pow10[sz + b], g, s, t);
          if (m2 % g == 0) {
            ll x0 = imod(s * (m2 / g), K);
            ll p = K / g;
            ll x = x0 % p;
            DLOGK(a, b, x, y);
            if (comp(sta(b, x, y), ans)) ans = sta(b, x, y);
            DLOGK(ans);
          }
        }
      }
    ...
    auto [b, x, y] = ans;
    cout << (x == 0 ? "" : to_string(x)) << S << zeropad(b, y) << "\n";
  };
