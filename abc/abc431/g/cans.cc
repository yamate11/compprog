...
// @@ !! LIM(forall debug cmpNaive ipoint)
...
int naive(istream& cin, ostream& cout) {
  ...
  // @End [YgyeURko]
  // @InpVec(Q, K, dec=1) [fa93CT22]
  auto K = vector(Q, ll());
  for (int i = 0; i < Q; i++) { ll v; cin >> v; v -= 1; K[i] = v; }
  // @End [fa93CT22]
  ...
  vector<IPoint> tans(Q);
  REP(i, 0, Q) cin >> tans[i];
  REP(q, 0, Q) {
    auto vec = B[K[q]];
    swap(vec[tans[q].x - 1], vec[tans[q].y - 1]);
    if (vec != A) {
      cout << "NG\n";
      return 0;
    }
...
int body(istream& cin, ostream& cout) {
  ...
  pbds_set ps2;
  REP(i, 0, N) {
    ps1.insert(pll(A[i], i));
    ps2.insert(pll(A[i], -i));
  }
  vector<ll> cnt(N + 1, 0LL);
  REP(i, 0, N) cnt[A[i]]++;
  ll nochg1 = EXFIND_D(ii, 0, N, cnt[A[ii]] > 1, -1LL);
  ll nochg2 = -1;
  if (nochg1 >= 0) nochg2 = EXFIND(ii, nochg1 + 1, N, A[nochg1] == A[ii]);

  deque<pll> deq;
  REP(q, 0, Q) deq.emplace_back(K[q], q);
  ranges::sort(deq);
  ...
  ll carry = 0;
  REP(i, 0, N) {
    ll p = ps1.order_of_key(pll(A[i], i));
    DLOGK(i, shift, carry, p);
    while (not deq.empty()) {
      auto [k, q] = deq.front();
      k -= shift;
      if (k >= p) break;
      deq.pop_front();
      auto [a, j] = *ps2.find_by_order(k);
      ans[q] = pll(i, -j);
    }
    while (not deq.empty()) {
      auto [k, q] = deq.back();
      ll nxt_size = (N - i - 1) * (N - i - 2) / 2 + carry + cnt[A[i]] - 1;
      k -= shift + p + nxt_size;
      if (k < 0) break;
      deq.pop_back();
      auto [a, j] = *ps1.find_by_order(p + cnt[A[i]] + k);
      ans[q] = pll(i, j);
    }
    shift += p;
    carry += cnt[A[i]] - 1;
  ...
  if (not deq.empty()) {
    assert(nochg1 >= 0 and nochg2 >= 0);
    for (auto [k, q] : deq) ans[q] = pll(nochg1, nochg2);
  }
