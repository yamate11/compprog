...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, H) REP(j, 0, W) {
    if (brd2.at(i, j) == '#') {
      for (auto dir : BrdIdx::nbr8) {
        auto bj = BrdIdx(i, j) + dir;
        if (brd.in(bj)) brd.rs(bj) = '#';
      }
    }
  }
  REP(i, 0, H) REP(j, 0, W) {
    if (brd2.at(i, j) == '#') brd.rs(i, j) = '.';
  }

  // using sta = pair<BrdIdx, char>;
  ll big = 1LL << 60;
  ...
  while (not que.empty()) {
    ...
    for (auto dir : BrdIdx::nbr8) {
      auto bj = bi + dir;
      if (brd.in(bj) and dist[bj.r][bj.c] == big) {
        dist[bj.r][bj.c] = dist[bi.r][bi.c] + 1;
        que.emplace(BrdIdx(bj.r, bj.c));
      }
    }
  }
  REP(i, 0, H) {
