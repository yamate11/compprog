...
// @@ !! LIM(digit mod sieve debug cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto concat = [&](ll x, ll y) -> ll {
    return x * du.pow(du.width(y)) + y;
  };
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> Fp {
    ...
    auto f = [&](ll w) -> Fp {
      ...
      ll numX = N / m;
      ll numY;
      DLOGK(w, du.nd_max(w), du.nd_min(w), a);
      if (w == w0) numY = N - du.nd_min(w) + 1;
      else numY = du.nd_max(w) - du.nd_min(w) + 1;
      DLOGK(N, M, w, numX, numY);
      return Fp(numX) * Fp(numY);
    };

    Fp ret = 0;
    REP(w, 1, w0 + 1) ret += f(w);
    return ret;
  };
