...
// @@ !! LIM(tree f:updMaxMin input segTree debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @DefStruct(sta, (a, b, len)) [ZLvmRJtN]
  struct sta {
    ll a;
    ll b;
    ll len;
    string show() const {
      return format("({}, {}, {})", a, b, len);
    }
    auto operator<=>(const sta&) const = default;
  };
  // @End [ZLvmRJtN]
  ...
  sta sta_unit(-1, -1, 0);
  auto sta_add = [&](const sta& x, const sta& y) -> sta {
    if (x.a == -1) return y;
    if (y.a == -1) return x;
    sta ret = (x.len >= y.len) ? x : y;
    auto op = [&](ll c, ll d) -> void {
      assert(c >= 0 and d >= 0);
      ll z = tr.lca(c, d);
      ll new_len = tr.depth(c) - tr.depth(z) + tr.depth(d) - tr.depth(z);
      if (ret.len < new_len) ret = sta(c, d, new_len);
    };
    op(x.a, y.a);
    op(x.a, y.b);
    op(x.b, y.a);
  ...
  REP(i, 0, N) init_vec[i] = sta(i, i, 0);
  auto st = make_seg_tree(sta_unit, sta_add, init_vec);
  DLOGKL("init", st.query(0, N));
  ll Q; cin >> Q;
  REP(_q, 0, Q) {
    ll x; cin >> x; x--;
    auto [a, b, len] = st.at(x);
    if (a == -1) st.rs(x) = sta(x, b, len);
    else         st.rs(x) = sta(-1, b, len);
    cout << st.query(0, N).len << "\n";
  }
