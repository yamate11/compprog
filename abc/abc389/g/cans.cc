...
int main(/* int argc, char *argv[] */) {
  ...
  /*
  vector<Fp> pow2(N * N + 1);
  pow2[0] = Fp(1);
  REP(i, 1, N * N + 1) pow2[i] = pow2[i - 1] * Fp(2);
  */

  ll mne = N * (N - 1) / 2;
  ll half = N / 2;

  vector adj(half + 1, vector(half + 1, vector(0, Fp(0))));

  REP(n0, 1, half + 1) REP(n1, 1, half + 1) {
    vector atbl_init(mne + 1, Fp(0));
    auto atbl = atbl_init;
    atbl[0] = 1;
    REP(i, 0, n0) {
      auto prev = move(atbl);
    ...
    REP(j, 0, n0 * n1 + 1) {
      REP(k, 0, n0 * (n0 - 1) / 2 + 1) {
        atbl[j + k] += cb.binom(n0 * (n0 - 1) / 2, k) * prev[j];
      }
    }
    adj[n0][n1] = move(atbl);
  }
  ...
  auto tbl_init = vector(N, vector(half + 1, vector(half + 1, vector(mne + 1, Fp(0)))));
  auto tbl = tbl_init;
  tbl[1][1][0][0] = Fp(1);
  vector ans(mne + 1, Fp(0));
  REP(dist, 1, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(n0, 1, half + 1) {
      REP(n1, 1, half + 1) {
        REP(e0, 1, half + 1) REP(o0, 1, half + 1) {
          ll e1, o1;
          if (dist % 2 == 0) {
            e1 = e0 - n0;
            o1 = o0;
          ...
          if (e1 < 0 or o1 < 0) continue;
          REP(ae, 1, n0 + n1) {
            REP(edge0, 1, mne + 1) {
              ll edge1 = edge0 - ae;
              if (edge1 < 0) continue;
              tbl[n0][e0][o0][edge0] += prev[n1][e1][o1][edge1] * adj[n0][n1][ae];
            }
          }
        }
      }
      REP(edge, N - 1, mne + 1) ans[edge] += tbl[n0][half][half][edge];
    }
  }
  REPOUT(edge, N - 1, mne + 1, ans[edge], " ");
