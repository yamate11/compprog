...
int naive(istream& cin, ostream& cout) {
  ...
  REP(_q, 0, Q) {
    ll x; cin >> x;
    REP(i, 0, N) if (L[i] <= x and x <= R[i]) x++;
    cout << x << "\n";
  }
...
int body(istream& cin, ostream& cout) {
  ...
  // @End [Mph13prz]
  ll Q; cin >> Q;
  // @InpVec(Q, query, dec=1) [F06SV6GJ]
  auto query = vector(Q, ll());
  for (int i = 0; i < Q; i++) { ll v; cin >> v; v -= 1; query[i] = v; }
  // @End [F06SV6GJ]

  ll lim = max(*max_element(ALL(R)), *max_element(ALL(query))) + 3LL;

  ll big = 1e18;
  auto mymin = [](ll x, ll y) -> ll { return min(x, y); };
  vector<ll> init_vec(lim);
  REP(i, 0, lim) init_vec[i] = i;
  auto st = make_seg_tree_lazy(big, 0LL, mymin, plus<ll>(), plus<ll>(), init_vec);

  REP(i, 0, N) {
    auto check1 = [&](ll v) -> bool { return L[i] <= v; };
    ll lo = st.binsearch_l_until(check1, lim);
    auto check2 = [&](ll v) -> bool { return R[i] <= v; };
    ll hi = st.binsearch_l_until(check2, lim);
    if (lo < hi) st.update(lo, hi, 1);
    DLOGK(lo, hi, st);
  }
  
  REP(_q, 0, Q) {
    cout << st.at(query[_q]) + 1 << "\n";
  }
