...
// @@ !! LIM(mod debug cmpNaive perm forall)
...
int body(istream& cin, ostream& cout) {
  ...
  vector<bool> dup(numc);
  vector<ll> lastpos(numc, -1LL);
  REP(i, 0, N) {
    ll d = T[i] - 'a';
    if (lastpos[d] >= 0) {
      dup[d] = true;
  ...
  ll dupsz = 0;
  vector<ll> dupidx(N, -1LL);
  REP(i, 0, N) if (i < lastpos[T[i] - 'a']) dupidx[i] = dupsz++;
  vector<ll> revdupidx(dupsz);
  REP(i, 0, N) if (i < lastpos[T[i] - 'a']) revdupidx[dupidx[i]] = i;
  DLOGK(lastpos);
  DLOGK(dupidx);
  map<vector<ll>, Fp> mp_init;
  auto mp = mp_init;
  vector<ll> norest(dupsz, 0LL);
  mp[norest] = Fp(1);
  REP(i, 0, N) {
    auto prev = move(mp);
    mp = mp_init;
    for (const auto& [rest, num] : prev) {
      REP(k, 0, i + 1) {
        if (EXISTS(j, 0, i, T[j] == T[i] and k < rest[dupidx[j]])) continue;
        auto newrest = rest;
        REP(j, 0, dupsz) {
          ll i0 = revdupidx[j];
          if (i == i0) {
  ...
  Fp tot = 0;
  for (const auto& [rest, num] : mp) tot += num;
  cout << tot << endl;
  return 0;
}
