...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> pidx(lim, -1LL);
  ll pnum = 0;
  for (ll p : primes) pidx[p] = pnum++;
  vector<vector<ll>> fact(lim);
  fact[1] = vector<ll>(pnum);
  REP(n, 2, lim) {
    fact[n] = fact[n - 1];
    for (const auto& [p, r] : prfac(n, primes)) {
      fact[n][pidx[p]] += r;
    }
  }
  ...
  auto solve = [&]() -> void {
    ...
    auto vec = fact[tot];
    for (ll c : C) REP(i, 0, pnum) vec[i] -= fact[c][i];
    Fp ans = 1;
    REP(i, 0, pnum) {
      if (vec[i] > 0) ans *= power<Fp>(primes[i], vec[i]);
    }
    cout << ans << "\n";
