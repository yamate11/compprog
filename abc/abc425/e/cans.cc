...
int main(/* int argc, char *argv[] */) {
  ...
  vector<vector<ll>> fact_p(lim, vector(ssize(prM), 0LL));
  vector<Fp> fact_q(lim, Fp(1));

  REP(n, 2, lim) {
    ll x = n;
    fact_p[n] = fact_p[n - 1];
    REP(i, 0, ssize(prM)) {
      const auto& [p, r] = prM[i];
      ll cnt = 0;
      for ( ; x % p == 0; x /= p) cnt++;
      fact_p[n][i] += cnt;
    }
    fact_q[n] = fact_q[n - 1] * x;
  }

#if DEBUG
  REP(i, 1, 20) DLOGK(i, fact_p[i], fact_q[i]);
#endif  
  ...
  auto solve = [&]() -> void {
    ...
    Fp denom = 1;
    for (ll c : C) {
      REP(i, 0, ssize(prM)) vec[i] -= fact_p[c][i];
      denom = denom * fact_q[c];
    }
    Fp ans = fact_q[tot] / denom;
    REP(i, 0, ssize(prM)) {
      const auto& [p, r] = prM[i];
      ans *= power<Fp>(p, vec[i]);
    }
    cout << ans << "\n";
