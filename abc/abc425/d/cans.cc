...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  Board<int> brd(H, W, 0);
  vector<BrdIdx> added_init;
  auto added = added_init;
  ll ans = 0;
  REP(i, 0, H) {
    string s; cin >> s;
    REP(j, 0, W) {
      if (s[j] == '#') {
        brd.rs(i, j) = 1;
        added.push_back(BrdIdx(i, j));
        ans++;
      }
    }
  }
  for (ll stage = 2; true; stage++) {
    auto prev = move(added);
    added = added_init;
    for (const auto& bi : prev) {
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (not brd.in(bj)) continue;
        if (brd.at(bj) > 0) continue;
        ll cnt = 0;
        for (auto dir2 : BrdIdx::nbr4) {
          auto bk = bj + dir2;
          if (brd.at(bk) > 0 and brd.at(bk) < stage) cnt++;
        }
        if (cnt == 1) {
          brd.rs(bj) = stage;
          added.push_back(bj);
