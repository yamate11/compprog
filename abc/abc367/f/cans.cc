...
int main(/* int argc, char *argv[] */) {
  ...
  ll lim = 2e5;
  vector<u64> h(lim + 1);
  REP(i, 0, lim + 1) h[i] = rand.range(1LL << 40, rh_prime);
  vector<u64> sumA(N + 1), sumB(N + 1);
  REP(i, 0, N) {
    sumA[i + 1] = rh_add(sumA[i], h[A[i]]);
    sumB[i + 1] = rh_add(sumB[i], h[B[i]]);
  }
  
  REP(_q, 0, Q) {
    ll l, r, L, R; cin >> l >> r >> L >> R; l--; L--;
    if (rh_subt(sumA[r], sumA[l]) == rh_subt(sumB[R], sumB[L])) cout << "Yes\n";
    else cout << "No\n";
  }
