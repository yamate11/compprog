...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Fp> B(N + 1);
  REP(i, 0, N) B[i + 1] = B[i] + Fp(A[i]);
  vector BP(K + 1, vector(N + 1, Fp(0)));
  REP(i, 0, N + 1) BP[0][i] = Fp(1);
  REP(k, 1, K + 1) REP(i, 0, N + 1) BP[k][i] = BP[k - 1][i] * B[i];
  vector C(K + 1, vector(N + 2, Fp(0)));
  REP(k, 0, K + 1) REP(i, 0, N + 1) C[k][i + 1] = C[k][i] + BP[k][i];

  Fp ans = 0;
  Fp sign = 1;
  REP(k, 0, K + 1) {
    Fp t = 0;
    REP(i, 0, N) t += BP[k][i] * (C[K - k][N + 1] - C[K - k][i + 1]);
    ans += sign * cb.binom(K, k) * t;
    sign = -sign;
  }
  cout << ans << endl;
