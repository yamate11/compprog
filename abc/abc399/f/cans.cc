...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl_init(3, vector(K + 1, Fp(0)));
  auto tbl = tbl_init;
  tbl[0][0] = Fp(1);
  REP(i, 0, N + 1) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(j, 0, 3) {
      if (j + 1 < 3) REP(k, 0, K + 1) tbl[j + 1][k] += prev[j][k];
      REP(k, 0, K + 1) tbl[j][k] += prev[j][k];
    }
    DLOGKL("first", i);
    DLOGK(tbl[0]);
    DLOGK(tbl[1]);
    ...
    vector<Fp> mypow(K + 1);
    mypow[0] = Fp(1);
    REP(k, 1, K + 1) mypow[k] = mypow[k - 1] * A[i];
    prev = move(tbl);
    tbl = tbl_init;
    tbl[0] = prev[0];
    tbl[2] = prev[2];
    REP(k, 0, K + 1) {
      REP(a, 0, K - k + 1) {
        tbl[1][k + a] += cb.binom(K - k, a) * mypow[a] * prev[1][k];
      }
    }
    DLOGKL("second", i);
    DLOGK(tbl[0]);
