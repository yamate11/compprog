...
int main(/* int argc, char *argv[] */) {
  ...
  auto func = [&](ll k) -> Fp {
    vector rec(N + 1, vector(M + 1, Fp(0)));
    vector eff(N + 1, vector(M + 1, false));
    auto sub = [&](auto rF, ll n, ll m) -> Fp {
      if (n < m) return 0;
      if (m < 0) return 0;
      auto& ret = rec[n][m];
      if (!eff[n][m]) {
	...
	if (k == 0) {
	  ...
	  s += (n - 1) * rF(rF, n - 2, m - 2);
	  s += (n - 1) * rF(rF, n - 2, m - 1);
	  for (ll t = 3; t <= min(n, k); t++) {
	    s += cb.comb(n - 1, t - 1) * cb.fact(t - 1) / 2
	      * rF(rF, n - t, m - t);
	    s += cb.comb(n - 1, t - 1) * cb.fact(t) / 2
	      * rF(rF, n - t, m - (t - 1));
	  }
	  ret = s;
	}
      }
      DLOG("n=", n, "m=", m, "k=", k, "ret=", ret);
      return ret;
    };
