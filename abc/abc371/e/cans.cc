...
int main(/* int argc, char *argv[] */) {
  ...
  vector<pll> pinit(N);
  REP(i, 0, N) pinit[i] = pll(init[i], 1);
  ...
  auto appl = [](OP f, DAT x) -> DAT { return DAT(f * x.second + x.first, x.second); };
  auto st = make_seg_tree_lazy(DAT(), OP(), add, comp, appl, pinit);

  ll ans = 0;
  REP(i, 0, N) {
    auto [v, dummy] = st.query(i, N);
    ans += v;
    st.update(i, nextpos[i], -1);
  }
  cout << ans << endl;
