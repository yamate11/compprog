...
int main(/* int argc, char *argv[] */) {
  ...
  MyMat E2(2, 2, vector<Fp>{1, 0, 0, 1});
  vector<ll> A(N, -1LL);

  Comb<Fp> cb(N);
  auto g = [&](ll n, ll r) -> Fp { return cb.binom(n - r + 1, r); };

  auto f = [&](ll n, ll r) -> MyMat {
    MyMat M(2, 2);
    M.at(0, 0) = g(n - 3, r - 1);
    M.at(0, 1) = g(n - 2, r - 1);
    M.at(1, 0) = g(n - 2, r);
  ...
  auto calc = [&](ll x) -> MyMat {
    auto it = ss.find(x);
    if (it != ss.begin()) {
      ll k = *prev(it);
      return f(x - k, A[x] - A[k]);
    }else {
      return f(x + 1, A[x]);
    }
  };
  ...
  vector<MyMat> init_vec(N, E2);
  auto myadd = [&](const MyMat& a, const MyMat& b) -> MyMat { return b * a; };
  auto st = make_seg_tree(E2, myadd, init_vec);
  REP(_q, 0, Q) {
    ll x, y; cin >> x >> y; x--;
    if (A[x] == y) {
      ;
...
#if DEBUG
    DLOGK(_q, x, y, ss);
    REP(i, 0, N) DLOG(st.at(i));
#endif
    auto mat = st.query(0, N);
    auto it = ss.end();
    ll len;
    if (it == ss.begin()) {
      len = N;
    ...
    MyMat G{{0, 1}, {1, 1}};
    auto mat2 = G.matpower(len);
    auto iv = MyMat(2, 1, vector<Fp>{0, 1});
    auto ans = mat2 * mat * iv;
    DLOGK(len);
    DLOG("mat2");
    DLOG(mat2);
    DLOG("mat");
