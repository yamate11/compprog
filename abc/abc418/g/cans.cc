...
int main(/* int argc, char *argv[] */) {
  ...
  REP(a, 0, 2) REP(b, 0, 2) REP(c, 0, 2) REP(d, 0, 2) {
    auto f = [&](ll x, ll y) -> ll {
      ...
      else if (x == 0 and y == 1) return b;
      else if (x == 1 and y == 0) return c;
      else if (x == 1 and y == 1) return d;
      else assert(false);
    };
    auto check = [&]() -> vector<sta> {
      vector<sta> ret;
      REP(i, 0, 2) REP(j, 0, 2) REP(k, 0, 2) {
        if (f(f(i, j), k) != f(i, f(j, k))) ret.emplace_back(i, j, k);
      }
      return ret;
    };
    auto v = check();
