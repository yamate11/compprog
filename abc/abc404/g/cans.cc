...
struct BellmanFord {
  ...
  vector<ll> dist;
  BellmanFord(ll n_) : n(n_), fwd(n), dist(n, LLONG_MAX) {}
  void add_edge(ll from, ll to, ll cost) { fwd.emplace_back(from, to, cost); }
  bool _update() {
    bool ret = false;
    for (auto [from, to, cost] : fwd) {
      if (dist[from] < LLONG_MAX and dist[from] + cost < dist[to]) {
        ret = true;
  ...
  bool run(ll start) {
    dist[start] = 0;
    for(ll _r = 0; _r < ssize(fwd); _r++) if (not _update()) return true;
    return false;
  }
};
...
struct UshiGame {
  ...
  optional<ll> get_max(ll from, ll to) {
    bool b = bf.run(from);
  ...
  optional<ll> get_min(ll from, ll to) {
    bool b = bf.run(to);
    if (not b) return nullopt;
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  UshiGame ug(N + 1);
  REP(i, 0, N) ug.add_constrGE(i, i + 1, 1);
  REP(i, 0, M) {
    ll l, r, s; cin >> l >> r >> s; l--;
    ug.add_constrLE(l, r, s);
    ug.add_constrGE(l, r, s);
  }
  auto ans = ug.get_min(0, N);
  cout << (ans ? *ans : -1) << endl;
