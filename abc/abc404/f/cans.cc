...
vector<vector<ll>> partition(ll n) {
  vector<vector<ll>> res;
  vector<ll> prefix;
  auto sub = [&res, &prefix](auto rF, ll vmin, ll rem) -> void {
    if (rem == 0) {
      res.push_back(prefix);
    }else {
      for (ll i = vmin; i <= rem; i++) {
        prefix.push_back(i);
        rF(rF, i, rem - i);
        prefix.pop_back();
      }
...
int main(/* int argc, char *argv[] */) {
  ...
  IntPartition part(M);
  while (part.get()) {
    auto v = part.vec_view();
    if (ssize(v) <= N) vv.push_back(v);
  }
