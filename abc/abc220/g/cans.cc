...
// @@ !! LIM(f:<< ipoint f:itrange debug cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  vector<sta> xyc(N);
  REP(i,N) {
    IPoint p; ll c; cin >> p >> c;
    xyc[i] = sta(p, c);
  }
  
  auto para = [&](IPoint p, IPoint q) -> bool {
    return p.x * q.y == p.y * q.x;
  };

  auto d2 = [&](IPoint p) -> ll { return p.x * p.x + p.y * p.y; };

  auto ok = [&](ll i, ll j, ll k, ll l) {
    IPoint p1 = xyc[i].first - xyc[j].first;
    IPoint p2 = xyc[k].first - xyc[l].first;
    IPoint q1 = xyc[i].first - xyc[k].first;
    IPoint q2 = xyc[j].first - xyc[l].first;
    return para(p1, p2) && d2(q1) == d2(q2) && !para(q1, q2);
  };

  ll ans = -1;
  REP(i, N) REP2(j, i+1, N) REP2(k, j+1, N) REP2(l, k+1, N) {
    if (ok(i,j,k,l) || ok(i,j,l,k) || ok(i,k,j,l) || ok(i,k,l,j)
        || ok(i,l,j,k) || ok(i,l,k,j)) {
      ll c = xyc[i].second + xyc[j].second + xyc[k].second + xyc[l].second;
      ans = max(ans, c);
    }
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  map<stb, vector<stc>> mp;
  REP(i,N) REP2(j,i+1,N) {
    auto [pi, ci] = xyc[i];
    auto [pj, cj] = xyc[j];
    auto dir = pj - pi;
    if (dir.y < 0) dir = -dir;
    if (dir.y == 0 && dir.x < 0) dir = -dir;
    auto mid2 = pi + pj;
    ll t = - dir.x * mid2.x - dir.y * mid2.y;
    ll g = gcd(abs(t), gcd(abs(2 * dir.x), abs(2 * dir.y)));
    stb zb(2 * dir.x / g, 2 * dir.y / g, t / g);
    IPoint norm(dir.y, -dir.x);
    ll gg = gcd(norm.x, norm.y);
    ll s = norm.x / gg * pi.x + norm.y / gg * pi.y;
    stc zc(ci + cj, s);
    DLOGK(pi, pj, dir, mid2, zb, zc);
    mp[zb].push_back(zc);
  }
  ll ans = -1;
  for (auto& [zb, vec] : mp) {
