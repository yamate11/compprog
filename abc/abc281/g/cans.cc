...
int main(/* int argc, char *argv[] */) {
  ...
  REP(n, 0, N + 1) {
    ...
    REP(r, 1, n) binom[n][r] = binom[n - 1][r - 1] + binom[n - 1][r];
  }
  vector<Fp> powtwo(N * N + 1);
  powtwo[0] = Fp(1);
  REP(i, 1, N * N + 1) powtwo[i] = powtwo[i - 1] * 2;
  auto f = [&](auto rF, ll n, ll p) -> Fp {
    auto& ref = tbl[n][p];
    if (not ref) {
      if (n == 1) {
        if (p == 1) ref = Fp(1);
        ...
        if (p <= 0 or n <= p) ref = Fp(0);
        else {
          Fp c = binom[n - 1][p];
          Fp s = Fp(0);
          REP(q, 1, n - p + 1) {
            s += rF(rF, n - p, q) * power<Fp>(powtwo[q] - 1, p) * powtwo[p * (p - 1) / 2];
          }
          ref = c * s;
        }
      }
