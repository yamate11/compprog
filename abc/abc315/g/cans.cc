...
// @@ !! LIM(f:gcd f:intDiv debug cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  auto func = [&](ll Y) -> ll {
    ...
    ll i0 = p + q * s;
    ll j0 = q * t;
    ll up = min(divFloor(j0 - 1, a), divFloor(N - i0, b));
    ll down = max(divCeil(j0 - N, a), divCeil(1 - i0, b));
    ll ret = down > up ? 0 : up - down + 1;
    DLOGK(Y, up, down, g, i0, j0, ret);
    return ret;
  };
