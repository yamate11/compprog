...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  vector<ll> AA(N+1), BB(N+1), A(N+1), B(N+1);
  CoordCompr cc;
  REP(i, N) {
    cin >> AA[i];
    cc.add(AA[i]);
  ...
  DLOGK(pA);
  DLOGK(pB);
  vector<ll> lA(N, -1), lB(N, -1);
  ll k = 1;
  ll i = 0, j = 0;
  ll s = pA[B[0]], t = pB[A[0]];
  while (true) {
    while (true) {
      if (s == N or t == N) break;
      ll s0 = s, t0 = t;
      for ( ; i < s; i++) t = max(t, pB[A[i]]);
      if (t == N) break;
      for ( ; j < t; j++) s = max(s, pA[B[j]]);
      if (s == N) break;
      if (s == s0 and t == t0) break;
    }
    DLOGKL("1", s, t);
    if (s == N or t == N) break;
    for ( ; i < N and pB[A[i]] <= t; i++) lA[i] = k;
    for ( ; j < N and pA[B[j]] <= s; j++) lB[j] = k;
    if (i == N or j == N) break;
    k++;
    s = pA[B[j]];
    t = pB[A[i]];
  ...
  REP(q, Q) {
    ll x, y; cin >> x >> y;
    if (lA[x-1] >= 0 and lA[x - 1] == lB[y - 1]) cout << "Yes\n";
    else cout << "No\n";
  }
