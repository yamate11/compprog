...
int main(/* int argc, char *argv[] */) {
  ...
  list<ll> lst;
  lst.push_back(0);
  vector<decltype(lst.begin())> vec(Q + 1);
  vec[0] = lst.begin();
  ll num = 0;
  REP(i, 0, Q) {
    auto [tp, x, y] = QQ[i];
    if (tp == 1) {
      auto it = vec[x];
      vec[i + 1] = lst.insert(next(it), i + 1);
      num++;
    }
  }

  vector<ll> init;
  for (auto t : lst) init.push_back(t);
  vector Rinit(Q + 1, 0LL);
  REP(i, 0, ssize(init)) Rinit[init[i]] = i;
  ...
  REP(i, 0, num + 1) dat_init.emplace_back(0, 1);
  ...
  REP(i, 0, Q) {
    ...
    if (tp == 1) {
      ...
      ll hi = Rinit[y];
      if (lo > hi) swap(lo, hi);
      auto [ans, dummy] = st.query(lo + 1, hi);
      cout << ans << "\n";
      st.update(lo + 1, hi, 0);
    }
  }
