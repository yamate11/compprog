...
int main(/* int argc, char *argv[] */) {
  ...
  using DAT = pll;
  using OP = ll;
  DAT unit_dat(LLONG_MAX, 0);
  OP unit_op = 0;
  auto add = [&](DAT a, DAT b) -> DAT {
    auto [am, ac] = a;
    auto [bm, bc] = b;
    if (am == bm) return DAT(am, ac + bc);
    else if (am < bm) return a;
    else return b;
  };
  auto comp = [&](OP f, OP g) -> OP { return f + g; };
  auto apply = [&](OP f, DAT x) -> DAT { return DAT(x.first + f, x.second); };
  vector<DAT> init_vec(N + 1, DAT(0, 1));
  ...
  ll ans = 0;
  REP(i, 0, N) {
    auto [vmin, num] = st.query(i + 1, N + 1);
    if (vmin == 0) ans += (N - i) - num;
    else           ans += N - i;
    ll q = nxt[i];
    st.update(i + 1, q + 1, -1);
    if (q < N) {
