...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  string S, T; cin >> S >> T;
  const ll szS = ssize(S);
  const ll szT = ssize(T);
  vector pos(n_alph, vector<ll>());
  REP(i, 0, szS) pos[S[i] - 'a'].push_back(i);
  if (EXISTS(i, 0, szT, pos[T[i] - 'a'].empty())) {
    cout << 0 << endl;
    return 0;
  }
  vector nxt(n_alph, vector(szS, 0LL));
  REPrev(i, szS - 1, 0) {
    REP(d, 0, n_alph) nxt[d][i] = (i == szS - 1) ? ssize(pos[d]) : nxt[d][i + 1];
    nxt[S[i] - 'a'][i]--;
  }
