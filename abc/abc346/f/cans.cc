...
// @@ !! LIM(binsearch f:intDiv cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  for ( ; true; k++) {
    ...
    REP(i, 0, ssize(T)) {
      REP(j, 0, k) {
        p++;
        while (p < ssize(ss) and ss[p] != T[i]) p++;
        if (p == ssize(ss)) {
          cout << k - 1 << endl;
          return 0;
        }
...
int body(istream& cin, ostream& cout) {
  ...
  vector pos(asz, vector<ll>());
  REP(i, 0, szS) pos[S[i] - 'a'].push_back(i);

  auto check = [&](ll k) -> bool {
    ll cur = -1;
    REP(i0, 0, szT) {
      cur++;
      const auto& tpos = pos[T[i0] - 'a'];
      ll sz = ssize(tpos);
      if (sz == 0) return false;
      ll i = lower_bound(ALL(tpos), cur % szS) - tpos.begin();
      ll ii = k - 1 + i;
      ll p = ii / sz;
      ll q = ii % sz;
      cur = szS * (cur / szS) + szS * p + tpos[q];
      if (cur >= szS * N) return false;
    }
    return true;
  };
  ll ans = binsearch<ll>(check, 0, 1 + divCeil(N * szS, szT));
  cout << ans << endl;
