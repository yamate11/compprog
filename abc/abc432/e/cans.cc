...
int main(/* int argc, char *argv[] */) {
  ...
  IPoint unit_dat(0, 0);
  vector<IPoint> init_vec(lim + 1, IPoint(0, 0));
  REP(i, 0, N) init_vec[A[i]] += IPoint(1, A[i]);
  auto st = make_seg_tree(unit_dat, plus<IPoint>(), init_vec);
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      ll a = A[x];
      IPoint t = st.at(a);
      IPoint r0 = IPoint(t.x - 1, t.y - a);
      st.rs(a) = r0;
      A[x] = y;
      IPoint s = st.at(y);
      IPoint r1 = IPoint(s.x + 1, s.y + y);
      st.rs(y) = r1;
      DLOGK(x, y, a, r0, y, r1);
    }else if (tp == 2) {
      ll l, r; cin >> l >> r;
      if (l <= r) {
        auto p0 = st.query(0, l);
        auto p1 = st.query(l, r);
        auto p2 = st.query(r, lim + 1);
        DLOGK(p0, p1, p2);
        DLOGK(p0.x * l, p2.x * r, p1.y);
        ll ans = p0.x * l + p2.x * r + p1.y;
        cout << ans << "\n";
      }else {
        cout << l * N << "\n";
      }
