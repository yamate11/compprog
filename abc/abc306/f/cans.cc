...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpGrid(N, M, A) [t7Yns37H]
  auto A = vector(N, vector(M, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < M; j++) { ll v; cin >> v; A[i][j] = v; }
  // @End [t7Yns37H]
  CoordCompr cc;
  REP(i, 0, N) REP(j, 0, M) cc.add(A[i][j]);
  auto B = vector(N, vector(M, 0LL));
  REP(i, 0, N) REP(j, 0, M) B[i][j] = cc.c(A[i][j]);
  ...
  ll ans = 0;
  auto st = make_seg_tree<ll>(0LL, plus<ll>(), vector<ll>(N*M));
  REPrev(i, N - 1, 0) {
    ll thisInc = 0;
    REP(j, 0, M) {
      ll x = j == 0 ? 0 : B[i][j - 1] + 1;
      thisInc += st.query(x, B[i][j]) * (M - j);
    }
    ans += base * (N - 1 - i) + thisInc;
    REP(j, 0, M) st.update(B[i][j], 1);
  }
  DLOGK(ans);
  cout << ans << endl;
