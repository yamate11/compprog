...
int main(/* int argc, char *argv[] */) {
  ...
  REP(_q, 0, Q) {
    if (ng) {
      ...
      ll b = uf.leader(v);
      if (a == b) {
        if (*uf.pot(u) == *uf.pot(v)) ng = true;
      }else {
        ll new_leader = uf.merge(u, v, Ftwo(1));
        ll the_other = new_leader == a ? b : a;
        auto f = [&](pll p) -> ll { return min(p.first, p.second); };
        pll o1 = S[new_leader];
        pll o2 = S[the_other];
        pll nn = o1;
        if (*uf.pot(the_other) == Ftwo(0)) { nn.first += o2.first; nn.second += o2.second; }
        else                               { nn.first += o2.second; nn.second += o2.first; }
        S[new_leader] = nn;
        ans += f(nn) - (f(o1) + f(o2));
      }
    }
