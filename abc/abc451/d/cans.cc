...
int main(/* int argc, char *argv[] */) {
  ...
  vector pow2(32, 0LL);
  pow2[0] = 1;
  REP(i, 1, 32) pow2[i] = pow2[i - 1] * 2;
  vector w_pow2(15, vector<ll>());
  REP(i, 0, 32) w_pow2[du.width(pow2[i])].push_back(pow2[i]);
  
  ll K = 10;
  vector w_ans(K, vector<ll>());
  REP(w, 1, K) {
    for (ll x : w_pow2[w]) w_ans[w].push_back(x);
    REPrev(v, w - 1, 1) {
      ll u = w - v;
      for (ll x : w_pow2[v]) {
        for (ll y : w_ans[u]) {
    ...
    ranges::sort(w_ans[w]);
    w_ans[w].erase(unique(ALL(w_ans[w])), w_ans[w].end());
  }
  DLOGK(w_ans[0]);
  DLOGK(w_ans[1]);
  DLOGK(w_ans[2]);
