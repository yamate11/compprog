...
int body(istream& cin, ostream& cout) {
  ...
  pque.emplace(A[0] * K, 0, K, 0);
  while (true) {
    assert(not pque.empty());
    auto [t, i, a, b] = pque.top(); pque.pop();
    cout << t << "\n";
    if (--X == 0) break;
    if (i + 1 < N) pque.emplace(t - A[i] + A[i + 1], i + 1, 1, a - 1);
    if (b > 0)     pque.emplace(t - A[i - 1] + A[i], i, a + 1, b - 1);
  }
