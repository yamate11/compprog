...
// @@ !! LIM(forall cmpNaive perm debug)
...
int body(istream& cin, ostream& cout) {
  ...
  set<sta> ss;
  REP(i, 0, N) ss.emplace(A[i], B[i], i);

  auto itbd = ss.begin();
  ll sumLo = 0, sumHi = 0, num1Lo = 0, num2Lo = 0;
  {
    auto it = ss.begin();
    REP(i, 0, num1) {
      auto [a, b, ii] = *it;
  ...
  DLOGK(sumLo, sumHi, num1Lo, num2Lo, num1, num2);

  REP(_q, 0, Q) {
    ll w, x, y; cin >> w >> x >> y; w--;
    {
      auto it = ss.find(sta(A[w], B[w], w));
      assert(it != ss.end());
      if (B[w] == 1) num1--;
      else           num2--;
      if (itbd == ss.end() or *it < *itbd) {
        sumLo -= A[w];
        if (B[w] == 1) num1Lo--;
        else           num2Lo--;
      }else {
      ...
      if (it == itbd) itbd = ss.erase(it);
      else ss.erase(it);
    }
    A[w] = x;
    B[w] = y;
    {
      auto [it, b] = ss.emplace(A[w], B[w], w);
      if (itbd == ss.end() or *it < *itbd) {
        sumLo += A[w];
        if (B[w] == 1) num1Lo++;
        else           num2Lo++;
    ...
    assert(num1Lo + num2Lo == num1);
    DLOGK(_q, sumLo, sumHi, num1Lo, num2Lo, num1, num2);
    ll score;
    if (itbd == ss.begin()) {
      auto [a2, b2, w2] = *itbd;
      score = sumHi * 2 - a2;
      ...
      auto [a1, b1, w1] = *prev(itbd);
      auto [a2, b2, w2] = *itbd;
      score = sumLo + 2 * sumHi - a2 + a1;
    }
    cout << score << "\n";
  }
