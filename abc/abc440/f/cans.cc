...
// @@ !! LIM(forall cmpNaive perm debug coordCompr input segTree)
...
// @DefStruct(st, (sat, neat, ix), ord=(sat, neat, ix)) [u1SnKycB]
struct st_t {
  ll sat;
  ll neat;
  ll ix;
  st_t(ll sat_ = ll(), ll neat_ = ll(), ll ix_ = ll()) : sat(sat_), neat(neat_), ix(ix_) {}
  friend istream& operator>>(istream& istr, st_t& t) {
    istr >> t.sat >> t.neat >> t.ix;
    return istr;
  }
  friend ostream& operator<<(ostream& ostr, const st_t& t) {
    ostr << "(" << t.sat << ", " << t.neat << ", " << t.ix << ")";
    return ostr;
  }
  bool operator<(const st_t& o) const {
    if (sat < o.sat) return true;
    if (sat > o.sat) return false;
    if (neat < o.neat) return true;
    if (neat > o.neat) return false;
...
// @DefStruct(elem, (sat, nt1, nt2), ord=(sat, nt1, nt2)) [4xyjUUXa]
struct elem_t {
  ll sat;
  ll nt1;
  ll nt2;
  elem_t(ll sat_ = ll(), ll nt1_ = ll(), ll nt2_ = ll()) : sat(sat_), nt1(nt1_), nt2(nt2_) {}
  friend istream& operator>>(istream& istr, elem_t& t) {
    istr >> t.sat >> t.nt1 >> t.nt2;
    return istr;
  }
  friend ostream& operator<<(ostream& ostr, const elem_t& t) {
    ostr << "(" << t.sat << ", " << t.nt1 << ", " << t.nt2 << ")";
    return ostr;
  }
  bool operator<(const elem_t& o) const {
    if (sat < o.sat) return true;
    if (sat > o.sat) return false;
    if (nt1 < o.nt1) return true;
    if (nt1 > o.nt1) return false;
    if (nt2 < o.nt2) return true;
    if (nt2 > o.nt2) return false;
    return false;
  }
};
// @End [4xyjUUXa]
...
int body(istream& cin, ostream& cout) {
  ...
  // @End [YfEu48PU]
  // @InpMVec(Q, ((W, dec=1), X, Y)) [A72C6bPN]
  auto W = vector(Q, ll());
  auto X = vector(Q, ll());
  auto Y = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
    ll v1; cin >> v1; v1 -= 1; W[i] = v1;
    ll v2; cin >> v2; X[i] = v2;
    ll v3; cin >> v3; Y[i] = v3;
  }
  // @End [A72C6bPN]
  ...
  ll num1 = 0, num2 = 0;
  REP(i, 0, N) {
    cc.add(st_t(A[i], B[i], i));
    if (B[i] == 1) num1++; else num2++;
  }
  REP(q, 0, Q) cc.add(st_t(X[q], Y[q], W[q]));
  ll sz = cc.size();

  auto myadd = [&](const elem_t& a, const elem_t& b) -> elem_t {
    return elem_t(a.sat + b.sat, a.nt1 + b.nt1, a.nt2 + b.nt2);
  };
  vector<elem_t> init_vec(sz);
  REP(i, 0, N) init_vec[cc.c(st_t(A[i], B[i], i))] = elem_t(A[i], B[i] == 1, B[i] == 2);
  auto st = make_seg_tree(elem_t(0, 0, 0), myadd, init_vec);

  REP(q, 0, Q) {
    ll w = W[q], a = X[q], b = Y[q];
    st.rs(cc.c(st_t(A[w], B[w], w))) = elem_t(0, 0, 0);
    if (B[w] == 1) num1--; else num2--;
    A[w] = a;
    B[w] = b;
    if (B[w] == 1) num1++; else num2++;
    st.rs(cc.c(st_t(A[w], B[w], w))) = elem_t(A[w], B[w] == 1, B[w] == 2);
    auto check = [&](elem_t e) -> bool { return e.nt1 + e.nt2 >= num2; };
    ll y = st.binsearch_l_from(check, sz);
    ll satLo = st.query(0, y).sat;
    ll satHi = st.query(y, sz).sat;
    ll satAt = (y == sz) ? -1 : st.at(y).sat;
    if (num2 == 0) {
      cout << satLo << "\n";
    }else if (num1 == 0) {
      cout << 2 * satHi - satAt << "\n";
    }else {
      if (st.query(0, y).nt1 == num1) {
        auto check2 = [&](elem_t e) -> bool { return e.nt1 + e.nt2 >= 1; };
        ll z = st.binsearch_l_from(check2, y);
        cout << 2 * satHi - satAt + st.at(z).sat << "\n";
      }else {
        cout << 2 * satHi << "\n";
      }
    }
