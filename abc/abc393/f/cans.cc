...
int main(/* int argc, char *argv[] */) {
  ...
  */
  ...
  REP(i, 0, N) {
    // ll a = cc.c(A[i]);
    ll a = A[i];
    ll j = lower_bound(ALL(vec), a) - vec.begin();
    if (j == ssize(vec)) vec.push_back(a);
    else vec[j] = a;
    DLOGK(i, vec);
    while (idx < Q) {
      auto [r, x, q] = RXQ[idx];
      if (r != i) break;
      // ll y = cc.c(x);
      ll k = lower_bound(ALL(vec), x) - vec.begin();
      DLOGK(r, x, q, k);
      if (k < ssize(vec) and vec[k] == x) {
        ans[q] = k + 1;
      }else {
        ans[q] = k;
      }
