...
struct UnionFind {
  ...
  UnionFind(int size_)
    : size(size_), _gsize(size, 1), built_groups(false) {
    for (int i = 0; i < size; i++) _leader.push_back(i);
  }
  ...
  int leaderpot(int i) {
    ...
    vector<int> seen;
    {
      int nxt = _leader[cur];
      while (cur != nxt) {
        seen.emplace_back(cur);
        cur = nxt;
        nxt = _leader[cur];
      }
    }
    while (not seen.empty()) {
      int j = seen.back(); seen.pop_back();
      _leader[j] = cur;
    }
    return cur;
  }
