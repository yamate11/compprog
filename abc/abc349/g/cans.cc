...
int main(/* int argc, char *argv[] */) {
  ...
  auto mymerge = [&](ll x, u64 m) -> void {
    ...
    u64 t = countr_zero(s);
    sub(sub, t, x + 1, x - 1);
    if (s < m) sub(sub, t, x + 1 + (m - s), x - 1 - (m - s));
  };
  
  REP(i, 0, N) mymerge(i, A[i]);
  ...
  vector ans(N, -1LL);
  vector proh(N, set<ll>());
  REP(i, 0, N) {
    if (ans[i] >= 0) continue;
    ll v = 1;
    ll ld = uf[0].leader(i);
    for (; proh[ld].contains(v); v++);
    for (ll j : gi.group(ld)) ans[j] = v;
    for (ll x : diff[ld]) if (ans[x] < 0) proh[x].insert(v);
  }
