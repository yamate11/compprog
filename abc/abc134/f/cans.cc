...
int main(int argc, char *argv[]) {
  ...
  ll N, K; cin >> N >> K;
  // Vec3 tbl(N+1, Vec2(N+1, Vec1(K + 1)));
  
  tbl[0][0][0] = 1;
  for (ll i = 0; i < N; i++) {
    /*
    for (ll j = 0; j <= i; j++) {
      for (ll k = 0; k <= K; k++) tbl[i+1][j][k] = 0;
    }
    */
    for (ll j = 0; j <= i; j++) {
      for (ll k = 0; k <= K; k++) {
	Fp v = tbl[i][j][k];
	/*
	auto upd = [&](ll jj, ll dk, ll n) -> void {
	  if (0 <= jj && jj <= N && k + dk <= K) {
	    tbl[i+1][jj][k + dk] += Fp(n) * v;
	  }
	};
	*/
	// append * 
	if (j+1 <= N && k + 2*(j+1) <= K) {
	  tbl[i+1][j+1][k + 2*(j+1)] += Fp(1) * v;
	}
	// upd(j+1, 2*(j+1), 1);
	// append num  /and/  append *, place i+1
	if (k + 2*j <= K) {
	  tbl[i+1][j][k + 2*j] += Fp(2*j + 1) * v;
	}
	// upd(j, 2*j, 2 * j + 1);
	// append num, place i+1
	if (j-1 >= 0 && k + 2*(j-1) <= K) {
	  tbl[i+1][j-1][k + 2*(j-1)] += Fp(j*j) * v;
	}
	// upd(j-1, 2*(j-1), j*j);
      }
    }
