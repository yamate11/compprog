...
int main(/* int argc, char *argv[] */) {
  ...
  auto incomp = [&](ll i, ll m, ll a) -> bool {
    return S[i][m] != -1 and S[i][m] != a;
  };

  vector tbl(M, vector(10, vector(10, vector(N, vector(N, optional<Fp>())))));
  auto f = [&](auto rF, ll m, ll a, ll b, ll p, ll q) -> Fp {
    optional<Fp>& ret = tbl[m][a][b][p][q];
    if (not ret) {
      Fp v = Fp(0);
      if (EXISTS(i, p, q + 1, incomp(i, m, a))) v = Fp(0);
      else if (m == M - 1) {
        if (p == q) v = Fp(1);
        else v = Fp(0);
      }
      else v = rF(rF, m + 1, 0, 9, p, q);
      if (a < b) {
        v += rF(rF, m, a + 1, b, p, q);
        REP(i, p, q) {
          if (incomp(i, m, a)) break;
          Fp v1 = m == M - 1 ? (p == i ? Fp(1) : Fp(0)) : rF(rF, m + 1, 0, 9, p, i);
          Fp v2 = rF(rF, m, a + 1, b, i + 1, q);
          v += v1 * v2;
        }
      }
      ret = v;
  ...
  cout << f(f, 0, 0, 9, 0, N - 1) << endl;
  REP(m, 0, M) REP(a, 0, 10) REP(b, 0, 10) REP(p, 0, N) REP(q, 0, N) {
    auto r = tbl[m][a][b][p][q];
    if (r and *r) DLOGK(m, a, b, p, q, *r);
  }
