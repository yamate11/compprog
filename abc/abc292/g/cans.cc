...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpVec(N, S, type=string) [otPwCW1q]
  auto S = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; S[i] = v; }
  // @End [otPwCW1q]
  
  vector tbl(N, vector(N + 1, vector(M, vector(10, optional<Fp>()))));
  
  auto f = [&](auto rF, ll i, ll j, ll k, ll a) -> Fp {
    if (i == j) return 1;
    ...
    auto& ret = tbl[i][j][k][a];
    if (!ret) {
      Fp v = 0;
      REP(m, i, j + 1) {
        v += rF(rF, i, m, k + 1, 0) * rF(rF, m, j, k, a + 1);
        if (m < j and S[m][k] != '?' and S[m][k] != (char)('0' + a)) break;
      }
      ret = v;
      DLOGK(i, j, k, a, ret);
    }
