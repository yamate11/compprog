...
int main(/* int argc, char *argv[] */) {
  ...
  if (A > B) swap(A, B);
  double x = B / A;
  double lim = (double)2 / sqrtl((double)3);
  DLOGK(x, lim);
  if (x >= lim) {
    DLOG("foo");
    cout << lim * A << endl;
  ...
  DLOG("bar");
  auto check = [&](double beta) -> bool {
    DLOGKL("check", beta);
    double y = x * cosl(beta) - cosl(M_PIl / 6 - beta);
    bool b = y > (double)0;
    DLOGK(beta, y, b);
    return b;
  };
  double beta = binsearch_r<double>(check, (double)0, (double)(M_PIl / (double)6), 1e-10, true);
  double R = 1 / cosl(beta);
  cout << R * A << endl;
