...
int main(/* int argc, char *argv[] */) {
  ...
  auto st = make_seg_tree_lazy<ll, ll>(LLONG_MIN, 0LL, mymax, plus<ll>(), plus<ll>(), S);
  auto aout = [&](ll k, double v) -> void {
    cout << B + v / k << endl;
  };
  REP(_q, 0, Q) {
    ll c, x; cin >> c >> x; c--; x -= B;
    ll diff = x - A[c];
    A[c] = x;
    st.update(c, N, diff);
    DLOGK(_q, st);
    ll vm = st.query(0, N);
    if (vm < 0) {
      aout(N, st.query(N - 1, N));
    }else {
      auto check = [&](ll v) -> bool { return v >= 0; };
      ll ir = st.binsearch_r_from(check, 0);
      DLOGK(ir);
      aout(ir, st.query(0, ir));
    }
