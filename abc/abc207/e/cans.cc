...
int body(istream& cin, ostream& cout) {
  ...
  auto tbl = vector(N + 1, vector(N + 1, Fp(0)));
  tbl[0][0] = Fp(1);
  for (ll h = 1; h <= N; h++) {
    vector cls(h, vector<ll>());
    for (ll i = 0; i <= N; i++) cls[S[i] % h].push_back(i);
    for (ll r = 0; r < h; r++) {
      Fp s = 0;
      for (ll j = 0; j < (ll)cls[r].size(); j++) {
        tbl[h][cls[r][j]] = s;
        s += tbl[h - 1][cls[r][j]];
      }
    }
