...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll nd, ll pt) -> vector<vector<Fp>> {
    auto cur = vector(3, vector<Fp>());
    cur[0] = {1, 0};
    cur[1] = {0, 0};
    cur[2] = {0, 1};
    for (ll cld : nbr[nd]) {
      if (cld == pt) continue;
      auto cres = rF(rF, cld, nd);
      auto prev = move(cur);
      cur = vector(3, vector<Fp>(prev[0].size() + cres[0].size() - 1));
      for (ll p = 0; p < 3; p++) {
        for (ll q = 0; q < 3; q++) {
          for (ll i = 0; i < (ll)prev[p].size(); i++) {
            for (ll j = 0; j < (ll)cres[q].size(); j++) {
              ll inc = ((p == 0 && q == 2) || (p == 2 && q == 0)) ? 1 : 0;
              ll r = (p == 0 && q == 2) ? 1 : p;
              Fp comb = prev[p][i] * cres[q][j];
              if (comb != 0) {
                cur[r][i + j + inc] += comb;
              }
            }
