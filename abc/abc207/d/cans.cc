...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> bool {
    ...
    auto gcenter = [&](const auto& vec) -> Point {
      Point g(0,0);
      for (Point p : vec) g += p;
      return g / (double)vec.size();
    };
    Point gp = gcenter(P), gq = gcenter(Q);
    for (Point& p : P) p -= gp;
    for (Point& p : Q) p -= gq;
    Point up = P[0].len() < eps ? P[1] : P[0];
    return any_of(r0N.begin(), r0N.end(), [&](ll i) {
      if (!r_eq(Q[i].len(), up.len())) return false;
      return all_of(r0N.begin(), r0N.end(), [&](ll j) {
        return any_of(r0N.begin(), r0N.end(), [&](ll k) {
          return P[j].rotate(up.angle(Q[i])).sim(Q[k]);
        });
      });
    });
  };
