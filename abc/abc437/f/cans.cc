...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> initP(N);
  REP(i, 0, N) initP[i] = X[i] + Y[i];
  vector<ll> initM(N);
  REP(i, 0, N) initM[i] = X[i] - Y[i];
  auto stPmax = make_seg_tree(-big, mymax, initP);
  auto stMmax = make_seg_tree(-big, mymax, initM);
  auto stPmin = make_seg_tree( big, mymin, initP);
  auto stMmin = make_seg_tree( big, mymin, initM);
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      ll pmax = stPmax.query(L, R);
      ll pmin = stPmin.query(L, R);
      ll mmax = stMmax.query(L, R);
      ll mmin = stMmin.query(L, R);
      ll ans = 0;
      updMax(ans, pmax - (x + y));
      updMax(ans, (x + y) - pmin);
      updMax(ans, mmax - (x - y));
