    ...
    Doubling:
      calculating F(g, y):
          ...
          F(g, y) =      alpha(nxt^{(0)}(g)) 
                    ...
                    `op` alpha(nxt^{(y-1)}(g))
      where 
        - (T, unit, op) is a monoid
        - nxt : G -> G
        - alpha : G -> T          

      With O(|G| log(max(y))) precalculation, 
      F(g, y) can be calculated in O(log(y))


    Typical code:

      using T = ll;
      auto unit = 0LL;                                // monoid unit
      auto op = [](T x, T y) -> T { return x + y; }   // monoid operator
      ll size = 100;   // G = {0, 1, 2, ..., size - 1}
      vector<int> nxt(size);  // next element of the graph
      nxt = ....;
      vector<T> alpha(size);  // valuation
      alpha = ...;
      ll lim = ll(1e15);  // the maximum value of y for F(g, y)
      auto dbl = make_doubling(lim, move(nxt), move(alpha), unit, op);
      cout << dbl.calc(g, y)   // calculation for F(g, y)
