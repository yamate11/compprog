...
int main(int argc, char *argv[]) {
  ...
  Tree tr(move(ve));
  vector<ll> ans(N);
  auto dfs = [&](const auto& cmpF, auto& v, ll n) -> void {
    if (n == 0) v.push_back(A.at(0));
    else {
      ll x = A.at(n);
      ll k = v.size();
      auto check = [&](ll i) -> bool { return x <= v.at(i); };
      ll i = binsearch<ll>(check, k, -1);
      if (i == k) v.push_back(x);
      else if (x < v.at(i)) v.at(i) = x;
    }
    ans.at(n) = v.size();
    auto cld = tr.child.at(n);
    ll nc = cld.size();
    if (nc == 0) return;
    for (ll i = 0; i < nc - 1; i++) {
      vector<ll> w(v);
      cmpF(cmpF, w, cld.at(i));
    }
    cmpF(cmpF, v, cld.at(nc-1));
  };
  vector<ll> v0;
  dfs(dfs, v0, 0);
  for (ll i = 0; i < N; i++) {
