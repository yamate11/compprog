...
int main(/* int argc, char *argv[] */) {
  ...
  for (auto [nab, j] : app) {
    ...
    if (numB[j0] >= numA[j]) {
      type = 0;
      gap = max(0LL, N - (cov + numB[j]));
    }else {
      assert(numA[j0] >= numB[j]);
      type = 1;
      gap = max(0LL, N - (cov + numA[j]));
    }
    j1 = j;
    break;
  }
  vector<ll> vA(N), vB(N);
  {
    ...
    ll q = N - 1;
    for (auto [nab, j] : app) {
      ll nA = (type == 0 && j == j1) ? gap : numA[j];
      ll nB = (type == 1 && j == j1) ? gap : numB[j];
      for (ll t = 0; t < nA; t++) vA[p++] = j;
      for (ll t = 0; t < nB; t++) vB[q--] = j;
      DLOG("j=", j, "p=", p, "q=", q);
    }
    if (type == 0) for (ll t = 0; t < numA[j1] - gap; t++) vA[p++] = j1;
    if (type == 1) for (ll t = 0; t < numB[j1] - gap; t++) vB[q--] = j1;
  }
  DLOG("vA=", vA);
  DLOG("vB=", vB);
