...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpGrid(N, 6, A) [rwDms0Bo]
  auto A = vector(N, vector(6, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < 6; j++) { ll v; cin >> v; A[i][j] = v; }
  // @End [rwDms0Bo]

  REP(i, 0, N) sort(ALL(A[i]));
  ll vmax = 0;
  REP(i, 0, N) vmax = max(vmax, A[i][0]);
  Fp prob = 1;
  vector<ll> used(N);
  ...
  vector<Fp> fprob;
  face.push_back(vmax);
  fprob.push_back(prob);

  while (not pque.empty()) {
    auto [fc, idx] = pque.top(); pque.pop();
    ll y = used[idx];
    prob = prob * Fp(y + 1) / Fp(y);
    used[idx]++;
    auto check_end = [&]() -> bool {
      if (pque.empty()) return true;
      auto [ff, fi] = pque.top();
      return ff != fc;
    };
    if (check_end()) {
  ...
  Fp ans = face[0] * fprob[0];
  ll sz = ssize(face);
  REP(i, 1, sz) {
    ans += face[i] * (fprob[i] - fprob[i - 1]);
  }
  cout << ans << endl;
