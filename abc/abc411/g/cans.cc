...
int main(/* int argc, char *argv[] */) {
  ...
  Fp ans = 0;
  vector tbl(1LL << N, vector(N, Fp(0)));
  REP(i, 0, N) tbl[1LL << i][i] = Fp(1);
  REP(x, 1, 1LL << N) {
    REP(i, 0, N) {
      if (tbl[x][i] == Fp(0)) continue;
      ll t = countr_zero((u64)x);
      if (popcount((u64)x) == 2) {
        Fp a = cnt[i][t] * (cnt[i][t] - 1);
        if (a > 0) {
          DLOGKL("two", x, i, t, a);
          ans += a;
        }
      }else if (popcount((u64)x) >= 3) {
        Fp a = tbl[x][i] * cnt[i][t];
        if (a > 0) {
          DLOGKL("many", x, i, t, a);
          ans += a;
        }
