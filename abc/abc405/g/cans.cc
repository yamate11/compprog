...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  // @InpVec(N, A, dec=1) [yNl0M9vG]
  auto A = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; A[i] = v; }
  // @End [yNl0M9vG]
  // @InpMVec(Q, ((L, dec=1), R, (X, dec=1))) [YwhKVa5L]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  auto X = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  vector<Fp> divtbl(N + 1);
  divtbl[1] = 1;
  REP(i, 2, N + 1) divtbl[i] = cb.fact(i - 1) * cb.vInvFact[i];
  auto mydiv = [&](Fp& a, Fp b) { a *= divtbl[b]; };
  ...
  vector<Fp> ans(Q);
  auto in_left = [&](ll i) -> void {
    ll a = A[i];
    auto [b, _j] = srd.idx2pos(a);
    cnt[a]++;
    srd.data(b).sum += 1;
    mydiv(srd.data(b).invfactprod, cnt[a]);
  };
  auto out_left = [&](ll i) -> void {
    ll a = A[i];
    auto [b, _j] = srd.idx2pos(a);
    srd.data(b).sum -= 1;
    srd.data(b).invfactprod *= cnt[a];
    cnt[a]--;
  };
  auto calc = [&](ll q) -> void {
    Fp sum(0);
    ...
    auto fe = [&](ll idx, ll b) {
      DLOGK(idx, cnt[idx]);
      sum += cnt[idx];
      invfactprod *= cb.vInvFact[cnt[idx]];
    };
    auto fb = [&](int b) {
      S& s = srd.data(b);
      DLOGK(b, s.sum, s.invfactprod);
      sum += s.sum;
      invfactprod *= s.invfactprod;
    };
    DLOGK(cnt);
    srd.exec(0, X[q], fe, fb);
    DLOGK(sum, invfactprod);
    ans[q] = cb.fact(sum) * invfactprod;
  };
  mo.run(in_left, out_left, calc);
  REPOUT(q, 0, Q, ans[q], "\n");
