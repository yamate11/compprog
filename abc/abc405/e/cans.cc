...
int naive(istream& cin, ostream& cout) {
  ...
  cin >> A >> B >> C >> D;
  ll s = A + B + C + D;
  auto f = [&](const auto& vec, const auto& ic, ll t, ll num) -> vector<ll> {
    vector<ll> ret = vec;
    ll idx = 0;
    ll p = 0;
    REP(i, 0, s) {
  ...
  IntComb ic0(s, A);
  while (ic0.get()) {
    auto w1 = f(vec, ic0, 0, A);
    IntComb ic1(s - A, B);
    while (ic1.get()) {
      auto w2 = f(w1, ic1, 1, B);
      IntComb ic2(s - A - B, C);
      while (ic2.get()) {
        auto w3 = f(w2, ic2, 2, C);
        REP(i, 0, s) if (w3[i] < 0) w3[i] = 3;
        // DLOGK(w3);
        ll r0 = -1, l2 = -1, l3 = -1, r1 = -1;
        REP(i, 0, s) {
          if (w3[i] == 0) r0 = i;
          if (w3[i] == 1) r1 = i;
          if (w3[i] == 2 and l2 < 0) l2 = i;
          if (w3[i] == 3 and l3 < 0) l3 = i;
        }
        bool b1 = r0 < l2;
        bool b2 = r0 < l3;
        bool b3 = r1 < l3;
...
int body(istream& cin, ostream& cout) {
  ...
  Fp s1 = 0, s2 = 0;
  REP(a, 1, A + 1) {
    Fp x = cb.binom(A - a + B - 1, B - 1) * cb.binom(C + D, C);
    s1 += x;
  }
  REP(b, 1, B + 1) {
    Fp x = cb.binom(A - 1 + B - b, A - 1) * cb.binom(b + C + D, C);
    s2 += x;
  }
  Fp ans = s1 + s2;
  cout << ans << endl;
