...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N - 1, D) [Hr5PHNym]
  auto D = vector(N - 1, ll());
  for (int i = 0; i < N - 1; i++) { ll v; cin >> v; D[i] = v; }
  // @End [Hr5PHNym]

  auto add = [&](Fp x, Fp y) -> Fp { return x; };
  auto comp = [&](Fp a, Fp b) -> Fp { return a * b; };
  auto appl = [&](Fp a, Fp x) -> Fp { return a * x; };
  auto init_dat = vector<Fp>(N, Fp(0));
  Fp dat_unit = 0;
  auto st = make_seg_tree_lazy(dat_unit, Fp(1), add, comp, appl, init_dat);
  ranges::reverse(D);
  if (D[0] != 1) {
    cout << 0 << endl;
    return 0;
  ...
  REP(k, 3, N + 1) {
    ...
    if (a == D[k - 3]) {
      Fp t = st.at(k - 1 - a);
      st.update(0, k - 1, Fp(k - 2));
      DLOGK(st.vec_view());
      st.rs(k - 1) = t;
      DLOGK(st.vec_view());
      st.rs(k - 1 - a) = st.at(k - 1 - a) + t;
      DLOGK(st.vec_view());
    }else {
      if (k - 1 - a < 0) {
        cout << 0 << endl;
      ...
      Fp t = st.at(k - 1 - a);
      st.update(0, k - 1, Fp(0));
      st.rs(k - 1) = t;
      st.rs(k - 1 - a) = t;
    }
  }
  Fp ans = 0;
  REP(i, 0, N) ans += st.at(i);
  cout << ans << endl;
  return 0;
}
