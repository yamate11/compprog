...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((L, dec=1), (R, dec=1))) [z9Qqe05t]
  auto L = vector(M, ll());
  auto R = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  auto st = make_seg_tree(big, mymin, vector<ll>(N, big));

  vector fromL(N, vector<ll>());
  vector toR(N, vector<ll>());
  REP(i, 0, M) {
    fromL[L[i]].push_back(R[i]);
    toR[R[i]].push_back(L[i]);
  }
  REP(i, 0, N) {
    ranges::sort(fromL[i]);
    ranges::sort(toR[i]);
  ...
  REP(i, 0, N) {
    if (not fromL[i].empty()) st.rs(i) = fromL[i][0];
  }
  ...
  REP(_q, 0, Q) {
    auto check = [&]() -> bool {
      ll s, t; cin >> s >> t; s--; t--;
      ll p = ranges::lower_bound(fromL[s], t + 1) - fromL[s].begin() - 1;
      if (p < 0) return false;
      if (fromL[s][p] == t) {
        if (p - 1 >= 0) return true;
        ll v = st.query(s + 1, t + 1);
        if (v <= t) return true;
        return false;
      }else {
        ll q = ranges::lower_bound(toR[t], s) - toR[t].begin();
        if (q >= ssize(toR[t])) return false;
        if (toR[t][q] <= fromL[s][p] + 1) return true;
        else return false;
      }
    };
    bool b = check();
