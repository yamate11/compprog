...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, X; cin >> N >> X;
  ll Y = 2 * X - 1;
  vector tbl_init(1LL << Y, vector(N + 1, Fp(0)));
  auto tbl = tbl_init;
  tbl[0][0] = Fp(max(0LL, N - X));
  DLOGK(tbl);
  REP(i, 1, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(x, 0, 1LL << Y) REP(n, 0, N + 1) {
      if (prev[x][n] == Fp(0)) continue;
      ll szLo = max(0LL, (i - 1) - X + 1);  // for i-1
      ll szHi = max(0LL, N - ((i - 1) + X));  // for i-1
      ll m = ((i - 1) + 1) - (n + __builtin_popcountll(x));  // for i-1
      ll freeHi = max(0LL, szHi - m);
      ll freeLo = max(0LL, szLo - n);
      Fp alpha = (szHi > 0) ? prev[x][n] * Fp(m) / Fp(szHi) : Fp(0);
      Fp beta = prev[x][n] - alpha;
      for (ll t : vector<ll>{-1, 1}) {
        ll newN = n + (x & 1);
        ll da = -1, db = -1;
        ll newXa = (x >> 1) | (1LL << (Y - 1));
        ll newXb = x >> 1;
        if (t == -1) {
          if (i - X + 1 <= 0) da = db = 0;
          else                da = db = freeLo + (1 - (x & 1));
          newN++;
          DLOGK(t, newXa, newXb, newN, da, db, alpha, beta);
        }else {
          da = max(0LL, freeHi);
          db = max(0LL, freeHi - 1);
          DLOGK(t, newXa, newXb, newN, da, db, alpha, beta);
        }
        tbl[newXa][newN] += da * alpha;
        tbl[newXb][newN] += db * beta;
      }
    }
    DLOGK(i, tbl);
  }
  Fp ans = 0; REP(x, 0, 1LL << Y) REP(n, 0, N + 1) ans += tbl[x][n];
  cout << ans << endl;
