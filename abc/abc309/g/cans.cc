...
int main(/* int argc, char *argv[] */) {
  ...
  auto tbl_init = vector(N + 1, vector(1LL << Y, Fp(0)));
  auto tbl = tbl_init;
  tbl[0][0] = Fp(1);
  REP(i, 0, N) {
    auto prev = tbl;
    tbl = tbl_init;
    REP(k, 0, N + 1) REP(x, 0, 1LL << Y) if (prev[k][x] != Fp(0)) {
      ll newX = x >> 1;
      tbl[k][newX] += prev[k][x];
      REP(j, 0, Y) {
        ll v = i + (j - (X - 1));
        if (not (0 <= v and v < N)) continue;
        if (j < Y - 1 and (newX >> j & 1)) continue;
        tbl[k + 1][newX | (1LL << j)] += prev[k][x];
      }
    }
  }
  Comb<Fp> cb(N);
  Fp cnt = 0;
  REP(k, 1, N+1) REP(x, 0, 1LL << Y) {
    cnt += tbl[k][x] * (k % 2 == 1 ? 1 : -1) * cb.fact(N - k);
  }
