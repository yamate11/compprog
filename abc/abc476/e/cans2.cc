...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) init_vec[i] = pll(P[i], i);
  auto segt1 = make_seg_tree(unit1, add1, init_vec);
  auto segt2 = make_seg_tree(unit2, add2, init_vec);
  REP(_q, 0, M) {
    ll l, r; cin >> l >> r; l--;
    auto [v1, i1] = segt1.query(l, r);
    auto [v2, i2] = segt2.query(l, r);
    segt1.rs(i1) = pll(v2, i1);
    segt1.rs(i2) = pll(v1, i2);
    segt2.rs(i1) = pll(v2, i1);
    segt2.rs(i2) = pll(v1, i2);
  }
  bool first = true;
  REP(i, 0, N) {
    if (first) first = false;
    else cout << " ";
    cout << segt1.at(i).first;
  }
  cout << "\n";
  return 0;
}
