...
// @DefStruct(sta, (mx, ix, mn, in)) [uSfjL0gv]
struct sta {
  ll mx{};
  ll ix{};
  ll mn{};
  ll in{};
  string show() const {
    return "(" + to_string(mx) + ", " + to_string(ix) + ", " + to_string(mn) + ", " + to_string(in) + ")";
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1LL << 60;
  sta unit(-big, -1, big, -1);
  auto add = [&](const sta& a, const sta& b) -> sta {
    sta ret;
    if (a.mx > b.mx) { ret.mx = a.mx; ret.ix = a.ix; }
    else             { ret.mx = b.mx; ret.ix = b.ix; }
    if (a.mn < b.mn) { ret.mn = a.mn; ret.in = a.in; }
    else             { ret.mn = b.mn; ret.in = b.in; }
    return ret;
  };
  vector<sta> init_vec(N);
  REP(i, 0, N) init_vec[i] = sta(P[i], i, P[i], i);
  auto segt = make_seg_tree(unit, add, init_vec);
  REP(_q, 0, M) {
    ll l, r; cin >> l >> r; l--;
    auto v = segt.query(l, r);
    segt.rs(v.ix) = sta(v.mn, v.ix, v.mn, v.ix);
    segt.rs(v.in) = sta(v.mx, v.in, v.mx, v.in);
    DLOGK(_q, v, segt.vec_view());
  }
  bool first = true;
  REP(i, 0, N) {
