...
int main(/* int argc, char *argv[] */) {
  ...
  auto val = [&](ll i, ll j) -> ll { return A[i] * B[j] % M; };

  vector Ea(3*N + 2, vector(3*N + 2, 0LL));
  vector Eb(3*N + 2, vector(3*N + 2, 0LL));
  REP(p, 0, N) REP(q, 0, N) {
    auto set = [&](auto& E, ll x, ll y, ll v) -> void { E[N + 1 + p + x][N + 1 + q + y] += v * val(p, q); };
    set(Ea, -N, -N, N);
    set(Ea, -(N - 1), -(N - 1), -(N + 1));
    set(Ea, N + 1, N + 1, 1);
    set(Eb, -(N - 1), N, 1);
...
#if DEBUG
  DLOG("after set, Ea");
  REP(i, 0, 3*N + 2) DLOGK(i, Ea[i]);
  DLOG("after set, Eb");
  REP(i, 0, 3*N + 2) DLOGK(i, Eb[i]);
#endif
  auto f1 = [&](auto& E, ll a, ll b, ll da, ll db) -> void {
    while (true) {
      ll v = E[a][b];
      a += da; b += db;
      if (not (0 <= a and a < 3*N + 2 and 0 <= b and b < 3*N + 2)) break;
      E[a][b] += v;
    }
  };
  REP(a, 0, 3*N + 2) f1(Ea, a, 0, 1, 1);
  REP(b, 1, 3*N + 2) f1(Ea, 0, b, 1, 1);
  REP(a, 0, 3*N + 2) f1(Eb, a, 3*N + 1, 1, -1);
  REP(b, 0, 3*N + 1) f1(Eb, 0, b, 1, -1);
#if DEBUG
  DLOG("after exp, Ea");
  REP(i, 0, 3*N + 2) DLOGK(i, Ea[i]);
  DLOG("after exp, Eb");
  ...
  vector F(3*N + 2, vector(3*N + 2, 0LL));
  REP(i, 0, 3*N + 2) REP(j, 0, 3*N + 2) F[i][j] = Ea[i][j] + Eb[i][j];
  REP(i, 0, 3*N + 2) REP(j, 1, 3*N + 2) F[i][j] += F[i][j - 1];
  REP(i, 1, 3*N + 2) REP(j, 0, 3*N + 2) F[i][j] += F[i - 1][j];
  
#if DEBUG
  DLOG("f");
  REP(i, 0, N) {
    REP(j, 0, N) cerr << F[N + 1 + i][N + 1 + j] << " ";
    cerr << endl;
  }
#endif

  ll ans = 0;
  REP(i, 0, N) REP(j, 0, N) ans ^= F[N + 1 + i][N + 1 + j] + i * N + j;
  cout << ans << "\n";
