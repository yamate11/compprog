...
int main(/* int argc, char *argv[] */) {
  ...
  auto p = [&](ll i, ll j) -> ll { return A[i] * B[j] % M; };

  vector F(N, vector(N, 0LL));
  REP(i, 0, N) REP(j, 0, N) F[0][0] += max(i, j) * p(i, j);

  vector DW(N, vector(N, 0LL));
  vector DH(N, vector(N, 0LL));
  
  DW[0][0] += p(0, 0);
  REP(i, 0, N) REP(j, i + 1, N) DW[0][0] -= p(i, j);
  DH[0][0] += p(0, 0);
  REP(j, 0, N) REP(i, j + 1, N) DH[0][0] -= p(i, j);
  ...
  REP(i, 0, N) {
    if (i > 0) {
      DW[i][0] = DW[i - 1][0] + S[i + 0] - T[i - 1 - 0 + (N - 1)];
      DH[i][0] = DH[i - 1][0] + S[i + 0] + T[i - 0 + (N - 1)];
    }
    REP(j, 1, N) {
      DW[i][j] = DW[i][j - 1] + S[i + j] + T[i - j + (N - 1)];
      DH[i][j] = DH[i][j - 1] + S[i + j] + T[i - j + (N - 1)];
    }
  }
  ...
  REP(i, 0, N) {
    if (i > 0) {
      F[i][0] = F[i - 1][0] + DH[i - 1][0];
      DLOGK(i, F[i][0]);
    }
    REP(j, 1, N) {
      F[i][j] = F[i][j - 1] + DW[i][j - 1];
      DLOGK(i, j, F[i][j]);
    }
  }

  ll ans = 0;
  REP(i, 0, N) REP(j, 0, N) ans ^= F[i][j] + i * N + j;
  cout << ans << "\n";
