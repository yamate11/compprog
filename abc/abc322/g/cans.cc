...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, X; cin >> N >> X;
  auto primes = sieve(X);
  auto divs = getDivisors(X, primes);
  ...
  auto func1 = [&]() -> Fp {
    ...
    REP(s1, 1, 10) {
      if (X % s1 != 0) continue;
      ll y = X / s1;
      REP (b, s1 + 1, min(10LL, N - y + 1)) ret += b;
      if (10 <= N - y) ret += 10 * (N - y - 9);
    }
    DLOGKL("func1", ret);
    return ret;
  };
  auto func2 = [&]() -> Fp {
    Fp ret = 0;
    for (ll d : divs) REP(s2, 1, 10) REP(s1, 0, 10) {
        if ((d - s1) % s2 != 0) continue;
        ll z = (d - s1) / s2;
        if (z <= 0) continue;
        ll y = X / d;
        if ((z + y) % 2 != 0) continue;
        ...
        if (a > N) continue;
        ll b = a - y;
        if (s1 >= b or s2 >= b) continue;
        ll w = min(b, 10LL);
        ret += w;
        DLOGK(d, s2, s1, y, a, b, w);
      }
    DLOGKL("func2", ret);
    return ret;
  };
  auto func3 = [&](ll k) -> Fp {
    Fp ret = 0;
    REP(a, 2, min(N + 1, 300LL)) {
      if (mypow(a, k) - mypow(a - 1, k) > X) break;
      REP(b, 1, a) {
        auto check = [&]() -> bool {
          ll y = X;
#if DEBUG
...
#endif
          REPrev(m, k, 1) {
            ll t = mypow(a, m) - mypow(b, m);
            ll p = y / t;
            if (m == k and p == 0) return false;
            if (p >= b or p >= 10) return false;
#if DEBUG
            vec.push_back(p);
#endif
            y = y % t;
