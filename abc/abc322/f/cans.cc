...
struct T {
  ...
  T(ll v) {
    sz = 1;
    if (v == 0) len0 = left0 = right0 = 1;
    else len1 = left1 = right1 = 1;
  }
};
...
int main(/* int argc, char *argv[] */) {
  ...
  auto X = vector<T>();
  REP(i, 0, N) X.push_back(T(S[i] - '0'));
  auto add = [&](const T& t1, const T& t2) -> T {
    T ret;
    ret.sz = t1.sz + t2.sz;
    ret.len0 = max(t1.right0 + t2.left0, max(t1.len0, t2.len0));
    ret.left0 = t1.sz == t1.left0 ? t1.sz + t2.left0 : t1.left0;
    ret.right0 = t2.sz == t2.right0 ? t2.sz + t1.right0 : t2.right0;
    ret.len1 = max(t1.right1 + t2.left1, max(t1.len1, t2.len1));
    ret.left1 = t1.sz == t1.left1 ? t1.sz + t2.left1 : t1.left1;
    ret.right1 = t2.sz == t2.right1 ? t2.sz + t1.right1 : t2.right1;
    return ret;
  };
  auto apply = [&](Ftwo f, const T& t) -> T {
    T ret(t);
    if (f == Ftwo(1)) {
      swap(ret.len0, ret.len1);
      swap(ret.left0, ret.left1);
      swap(ret.right0, ret.right1);
    }
    return ret;
  };
  auto st = make_seg_tree_lazy<T, Ftwo>(T(), Ftwo(0), add, plus<Ftwo>(), apply, X);

  REP(_q, 0, Q) {
    ll c, L, R; cin >> c >> L >> R; L--;
    if (c == 1) st.update(L, R, Ftwo(1));
    else if (c == 2) cout << st.query(L, R).len1 << "\n";
    else assert(0);
  }
