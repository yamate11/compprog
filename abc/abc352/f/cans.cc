...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((A, dec=1), (B, dec=1), C)) [oD4jfWos]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  auto C = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  REP(i, 0, N) { DLOGK(i, *uf.pot(i)); }
  auto gi = uf.group_info();
  vector<vector<pll>> pis;
  vector<ll> pats;
  REP(i, 0, N) {
    if (uf.leader(i) == i) {
      vector<pll> pot_id_tmp;
      for (ll j : gi.group(i)) pot_id_tmp.emplace_back(*uf.pot(j), j);
      sort(ALL(pot_id_tmp));
      auto [p0, idx0] = pot_id_tmp[0];
      vector<pll> pot_id;
      for (auto [p, idx] : pot_id_tmp) pot_id.emplace_back(p - p0, idx);
      pis.push_back(pot_id);
      ll pat = 0;
      REP(j, 0, ssize(pot_id)) pat |= (1LL << pot_id[j].first);
      pats.push_back(pat);
    }
  }
  vector<ll> ans(N);
  ll outmask = ~((1LL << N) - 1);
  auto check = [&](ll pis_idx) -> ll {
    auto check_sub = [&](ll x0) -> bool {
      auto dfs = [&](auto rF, ll x, ll i) -> bool {
        if (i == ssize(pats)) return true;
        if (i == pis_idx) return rF(rF, x, i + 1);
        ll pat = pats[i];
        if (pat == 1) return rF(rF, x, i + 1);
        for (ll sft = 0; not ((pat << sft) & outmask); sft++) {
          ll new_pat = pat << sft;
          if (not (new_pat & x)) {
            bool b = rF(rF, new_pat | x, i + 1);
            if (b) return true;
          }
        }
        return false;
    ...
    REP(i, 0, ssize(pats)) {
      if (i != pis_idx and pats[i] == pats[pis_idx]) return -1;
    }

    ll pat = pats[pis_idx];
    if ((pat << 1) & outmask) return 0;
    ll ok_sft = -1;
    for (ll sft = 0; not ((pat << sft) & outmask); sft++) {
      bool b = check_sub(pat << sft);
      if (b) {
        if (ok_sft < 0) ok_sft = sft;
        else return -1;
      }
    }
    assert(ok_sft >= 0);
  ...
  REP(i, 0, ssize(pis)) {
    ll rc = check(i);
    if (rc < 0) {
      for (auto [p, idx] : pis[i]) ans[idx] = -1;
    }else {
      for (auto [p, idx] : pis[i]) ans[idx] = rc + p + 1;
    }
  }
