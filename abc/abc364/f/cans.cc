...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  // @InpMVec(Q, ((L, dec=1), (R, dec=0), C)) [PkxJ0R5P]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  auto C = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  REP(i, 0, N) dat_init.emplace_back(1, 1);
  ...
  REP(qq, 0, Q) {
    ...
    if (L[q] + 1 == R[q]) nc = 1;
      ...
      nc = st.query(L[q] + 1, R[q]).first + 1;
      st.update(L[q] + 1, R[q], 0);
    }
    ans += nc * C[q];
  }
  if (st.query(0, N).first > 1) {
    cout << -1 << endl;
  }else {
    cout << ans << endl;
  }
