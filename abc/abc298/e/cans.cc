...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl(2, vector(N, vector<optional<Fp>>(N)));
  auto f = [&](auto rF, int player, ll a, ll b) -> Fp {
    if (not tbl[player][a][b]) {
      Fp val;
      if (player == 0) {
        REP(p, 1, P + 1) {
          if (a + p >= N) val += Fp(1);
          else val += Fp(1) - rF(rF, 1, a + p, b);
        }
        val = val / P;
      }else if (player == 1) {
        REP(q, 1, Q + 1) {
          if (b + q >= N) val += Fp(1);
          else val += Fp(1) - rF(rF, 0, a, b + q);
        }
        val = val / Q;
      }else assert(0);
      tbl[player][a][b] = val;
    }
    return *tbl[player][a][b];
  };
  cout << f(f, 0, A, B) << endl;
