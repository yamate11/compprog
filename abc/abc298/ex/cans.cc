...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpMVec(N - 1, ((A, dec=1), (B, dec=1))) [gwZj0gyD]
  ...
  // @End [gwZj0gyD]
  ll Q; cin >> Q;
  // @InpMVec(Q, ((L, dec=1), (R, dec=1))) [Pddwm4Bb]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  Tree tr(N, 0);
  REP(i, 0, N - 1) tr.add_edge(A[i], B[i]);
  // (subtree_size, depth_sum)
  auto myadd = [&](pll a, pll b) -> pll { return {a.first + b.first, a.second + b.second}; };
  auto mymod = [&](pll v, int node, int child) -> pll {
    auto [ss, ds] = v;
    return {1 + ss, 1 + ss + ds};
  };
  auto ds = reroot(tr, pll(0, 0), myadd, mymod);
  REP(i, 0, N) ds[i].first += 1;
  DLOGK(ds);
