...
int main(/* int argc, char *argv[] */) {
  ...
  auto add = [](DAT x, DAT y) -> DAT {
    auto [xmax, xnum] = x;
    auto [ymax, ynum] = y;
    if (xmax == ymax) return DAT(xmax, xnum + ynum);
    else if (xmax < ymax) return y;
    else if (xmax > ymax) return x;
    else assert(0);
  };
  auto comp = [](OP f, OP g) -> OP { return f + g; };
  auto appl = [](OP f, DAT x) -> DAT {
    auto [xmax, xnum] = x;
    return DAT(xmax + f, xnum);
  };
  DAT unit_dat = DAT(0, 0);
  OP unit_op = 0LL;
  vector<DAT> dat_init(N);
  REP(i, 0, N) dat_init[i] = DAT(0, 1);
  auto st = make_seg_tree_lazy(unit_dat, unit_op, add, comp, appl, dat_init);

  ll K = 60;
  itv_set<ll> itv_init(0, N);
  vector<itv_set<ll>> itvs(K, itv_init);

  auto f = [&](ll L, ll R, ll x, ll vold, ll vnew) -> void {
    ll cur = L;
    while (cur < R) {
      auto [p, q, v] = itvs[x].get(cur);
      q = min(q, R);
      if (v == vold) {
        DLOGK(cur, q, vnew - vold);
        st.update(cur, q, vnew - vold);
      }
      cur = q;
    }
    itvs[x].put(L, R, vnew);
