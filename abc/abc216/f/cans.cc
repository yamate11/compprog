...
int main(/* int argc, char *argv[] */) {
  ...
  for (ll i = 0; i < N; i++) AB.emplace_back(A[i], B[i]);
  sort(AB.begin(), AB.end());
  auto tbl_init = vector(2, vector(maxA + 1, Fp(0)));
  auto tbl = tbl_init;
  const ll OK = 0;
  const ll NG = 1;
  tbl[OK][0] = 1;
  for (ll i = 0; i < N; i++) {
    ...
    auto [a, b] = AB[i];
    for (ll x = 0; x <= maxA; x++) {
      if (x + b <= a)         tbl[OK][x + b] += prev[OK][x] + prev[NG][x];
      else if (x + b <= maxA) tbl[NG][x + b] += prev[OK][x] + prev[NG][x];
    }
#if DEBUG
    DLOGK(i, a, b);
    DLOGKL("OK", tbl[OK]);
