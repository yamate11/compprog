...
int main(/* int argc, char *argv[] */) {
  ...
  ll M; cin >> M;
  vector traj(M + 1, vector<IPoint>(3));
  traj[0] = vector({IPoint(1, 0), IPoint(0, 1), IPoint(0, 0)});
  for (ll i = 0; i < M; i++) {
    ll tp; cin >> tp;
    vector<IPoint> cur(3);
    auto func = [&](auto g) -> void {
      vector<IPoint> v;
      for (ll j = 0; j < 3; j++) v.push_back(g(traj[i][j]));
      traj[i + 1] = move(v);
    };
    if (tp == 1) {
      func([](IPoint& pt) -> IPoint { return IPoint(pt.y, -pt.x); });
    }else if (tp == 2) {
      func([](IPoint& pt) -> IPoint { return IPoint(-pt.y, pt.x); });
    }else {
      ll p; cin >> p;
      if (tp == 3) {
        func([&p](IPoint& pt) -> IPoint { return IPoint(2*p - pt.x, pt.y); });
      }else if (tp == 4) {
        func([&p](IPoint& pt) -> IPoint { return IPoint(pt.x, 2*p - pt.y); });
      }
    }
  }
  DLOG(traj);
  ...
  for (ll i = 0; i < Q; i++) {
    ll a, b; cin >> a >> b; b--;
    auto [x, y] = XY[b];
    IPoint pt = x * traj[a][0] + y * traj[a][1] + (1 - x - y) * traj[a][2];
    cout << pt.x << " " << pt.y << "\n";
  }
