...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> bool {
    // IPoint: 整数座標を表す構造体
    IPoint P, Q, R, S; cin >> P >> Q >> R >> S;
    IPoint u = Q - P;  // ベクトルPQ
    IPoint v = S - R;  // ベクトルRS
    if (u.parallel(v)) {   // PQ と RS が平行なら
      IPoint w = (P + Q) - (R + S);   // PQ の中点と RS の中点を結ぶベクトル (の2倍)
      if (w.x * u.x + w.y * u.y == 0) return true;   // w と u の内積が0なら
      else return false;                             // そうでなければ
    }else {               // PQ と RS が平行でなければ
      return true;
    }
  };
