...
// @@ !! LIM(f:intDiv unordered_map debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [uTszvdRo]
  // @InpVec(N - 1, B) [GoEz2XBx]
  auto B = vector(N - 1, ll());
  for (int i = 0; i < N - 1; i++) { ll v; cin >> v; B[i] = v; }
  // @End [GoEz2XBx]

  safe_umap<ll, ll> mpE, mpO;
  ll z = 0;
  mpE[0] = 1;
  REP(i, 0, N - 1) {
    z = imod((B[i] - (A[i] + z)) - A[i + 1], M);
    if (i % 2 == 0) mpO[z]++;
    else            mpE[z]++;
  }
  DLOGK(mpO, mpE);
  ll t = 0;
  for (auto [k, num] : mpE) t += k * num;
  for (auto [k, num] : mpO) t += k * num;
  DLOGKL("init", t);
  ll ans = t;
  vector<ll> I;
  for (auto [k, num] : mpE) I.push_back(M - k);
  for (auto [k, num] : mpO) I.push_back(k + 1);
  ranges::sort(I);
  ...
  for (ll i : I) {
    if (i == M) break;
    t += (i - prev) * (divCeil(N, 2) - divFloor(N, 2)) - M * mpE[M - i] + M * mpO[i - 1];
    DLOGK(i, t);
    ans = min(ans, t);
    prev = i;
  }
