...
// @@ !! LIM(unordered_map segTree binsearch debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<pll> poss;
  REP(i, 0, N) poss.emplace_back(A[i], i);
  REP(q, 0, Q) poss.emplace_back(X[q], C[q]);
  auto comp2 = [](pll a, pll b) {
    if (a.second != b.second) return a.second < b.second;
    return a.first < b.first;
  };
  ranges::sort(poss, comp2);
  poss.erase(unique(ALL(poss)), poss.end());
  ll ht = bit_width(bit_ceil(poss.size()));
  ll szT = 1LL << (ht - 1);
  {
    ll z = poss.size();
    poss.resize(szT);
    REP(i, z, szT) poss[i] = pll((ll)1.01e9, i);
  }
  vector idx_oval(ht, poss);
  vector revIO(ht, safe_umap<pll, ll>());
  auto myplus = [](pll a, pll b) -> pll { return pll{a.first + b.first, a.second + b.second}; };
  auto sgt_base = make_seg_tree(pll{0, 0}, myplus, vector<pll>(1LL << (ht - 1)));
  vector cval(ht, sgt_base);

  REP(h, 0, ht) {
    ll len = 1LL << h;
    auto it = idx_oval[h].begin();
    if (h > 0) { REP(j, 0, szT >> h) sort(it + len * j, it + len * (j + 1)); }
    REP(i, 0, szT) revIO[h][idx_oval[h][i]] = i;
    REP(i, 0, N) cval[h].rs(revIO[h][pll(A[i], i)]) = pll{A[i], 1};
  }
  ...
  REP(q, 0, Q) {
    if (A[C[q]] != X[q]) {
      REP(h, 0, ht) {
        ll i_old = revIO[h][pll(A[C[q]], C[q])];
        ll i_new = revIO[h][pll(X[q], C[q])];
        cval[h].rs(i_old) = pll{0, 0};
        cval[h].rs(i_new) = pll{X[q], 1};
      }
    }
    A[C[q]] = X[q];
#if DEBUG
    REP(h, 0, ht) {
      DLOGK(q, h, idx_oval[h]);
      DLOGK(q, h, cval[h].vec_view());
    }
#endif

    ll rgL = ranges::lower_bound(idx_oval[0], pll{0, L[q]}, comp2) - idx_oval[0].begin();
    ll rgR = ranges::lower_bound(idx_oval[0], pll{0, R[q]}, comp2) - idx_oval[0].begin();

    auto left = [&](ll h, ll i) -> ll { return (1LL << h) * i; };
    auto right = [&](ll h, ll i) -> ll { return (1LL << h) * (i + 1); };
    auto f = [&](ll x) -> pll {
      auto sub = [&](auto rF, ll h, ll i) -> pll {
        if (rgR <= left(h, i) or right(h, i) <= rgL) return pll{0, 0};
        else if (rgL <= left(h, i) and right(h, i) <= rgR) {
          auto it0 = idx_oval[h].begin();
          ll j0 = ranges::lower_bound(it0 + left(h, i), it0 + right(h, i), pll{x, -1}) - it0;
          return cval[h].query(j0, right(h, i));
        }else {
          pll a = rF(rF, h - 1, 2 * i);
          pll b = rF(rF, h - 1, 2 * i + 1);
          return myplus(a, b);
        }
      };
      return sub(sub, ht - 1, 0);
    };
    auto check = [&](ll x) -> bool {
      auto [delsum, num] = f(x);
      return delsum >= K[q];
    };
    ll x = binsearch<ll>(check, 0, (ll)1.1e9);
    if (x == 0) cout << -1 << "\n";
    else {
      auto [delsum, num] = f(x);
      cout << num - (delsum - K[q]) / x << "\n";
    }
  }
