...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&](double x) -> bool {
    ...
    vector<double> vec;
    REP(j, 0, M) {
      vec.push_back((100.0 - x) * C[j] - D[j] * x);
    }
    sort(ALL(vec));
    DLOGK(vec);
    ...
    ll sum = 0;
    REP(i, 0, N) {
      double E = (A[i] + B[i]) * x - 100.0 * A[i];
      ll t = vec.end() - lower_bound(ALL(vec), E);
      DLOGK(i, E, t);
      sum += t;
    }
    DLOGK(sum);
  ...
  double x = binsearch_r<double>(check, 0, 100, 1e-10, true);
  cout << x << endl;
