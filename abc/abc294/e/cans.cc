...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N1) {
    ll v, l; cin >> v >> l;
    it1.put(a1, a1 + l, pll(v, 0));
    a1 += l;
  }
  ll b1 = 0;
  REP(i, 0, N2) {
    ll v, l; cin >> v >> l;
    it2.put(b1, b1 + l, pll(0, v));
    b1 += l;
  }
  assert(a1 == L);
  assert(b1 == L);
  auto f = [&](pll p1, pll p2) -> pll {
    auto [x1, y1] = p1;
    auto [x2, y2] = p2;
    return pll(x1 + x2, y1 + y2);
  };
  auto it0 = itv_apply(f, it1, it2);
  DLOGK(it1.impl);
  DLOGK(it2.impl);
  DLOGK(it0.impl);
  ll c1 = 0;
  ll ans = 0;
  while (c1 < LLONG_MAX) {
    auto [t, yy] = it0.get(c1);
    auto [a, b] = t;
    auto [l, r] = yy;
    if (r == LLONG_MAX) break;
    DLOGK(a, b, l, r);
    if (a == b) ans += r - l;
    c1 = r;
  }
