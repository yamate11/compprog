...
int main(/* int argc, char *argv[] */) {
  ...
  auto calc = [&](ll t, ll d) -> ll {
    double est = (3.0 + sqrt(9.0 - 4 * (2 - d))) / 2.0;
    ll iest = llround(est);
    ll vmin = big;
    if (iest + 100 <= t) {
      return t + (d / (t + 1));
    }
    for (ll i = max(t, iest - 100); i < iest + 100; i++) {
      vmin = min(vmin, i + (d / (i + 1)));
    }
    return vmin;
  };
  ...
  auto nbr = vector(N + 1, vector<sta>());
  for (ll i = 0; i < M; i++) {
    ll a, b, c, d; cin >> a >> b >> c >> d;
    nbr[a].emplace_back(b, c, d);
    nbr[b].emplace_back(a, c, d);
  }
