...
struct CartesianTree {
  ...
  CartesianTree() : root(-1) {}
  template<typename Comp = less<typename T::value_type>>
  CartesianTree(const T& vec, Comp comp = Comp()) { build(vec, comp); }

  void _build_lo_hi(int a, int b, int x) {
    if (x < 0) return;
    lo[x] = a;
    hi[x] = b;
    _build_lo_hi(a, x, left[x]);
    _build_lo_hi(x + 1, b, right[x]);
  }
  template<typename Comp = less<typename T::value_type>>
...
int main(/* int argc, char *argv[] */) {
  ...
  CartesianTree ctB(A, greater<ll>());
  CartesianTree ctC(A);

  vector<Fp> tbl0(N + 1, 0LL);
  vector<Fp> sum0(N + 2, 0LL);
  vector<Fp> tblB(N + 1, 0LL);
  vector<Fp> tblC(N + 1, 0LL);
  ...
  REP(i, 1, N + 1) {
    ll p = ctB.lo[i - 1];
    tblB[i] = tblB[p] + (sum0[i] - sum0[p]) * A[i - 1];
    ll q = ctC.lo[i - 1];
    tblC[i] = tblC[q] + (sum0[i] - sum0[q]) * A[i - 1];
    tbl0[i] = tblB[i] - tblC[i];
    sum0[i + 1] = sum0[i] + tbl0[i];
    DLOGK(i, p, q, tblB[i], tblC[i], tbl0[i]);
  }
  cout << tbl0[N] << endl;
