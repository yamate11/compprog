...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, N) cin >> A[i];
  vector<Fp> rec(N + 2);
  using sta = tuple<ll, ll, ll>; // val, pos_end, len
  vector<sta> vmax;
  vector<sta> vmin;
  rec[0] = 0;
  rec[1] = 1;
  rec[2] = 1;
  vmax.emplace_back(-1, -1, -1);
  vmax.emplace_back(A[0], 0, 1);
  vmin.emplace_back(-1, -1, -1);
  vmin.emplace_back(A[0], 0, 1);
  REP2(i, 1, N) {
    Fp inc = 0;
    while (true) {
      auto [val, pos_end, len] = vmax.back();
      if (pos_end >= 0 and A[i] > val) {
        inc += (A[i] - val) * (rec[pos_end + 1] - rec[pos_end - len + 1]);
        vmax.pop_back();
      }else {
        vmax.emplace_back(A[i], i, i - pos_end);
        break;
      }
    }
    while (true) {
      auto [val, pos_end, len] = vmin.back();
      if (pos_end >= 0 and A[i] < val) {
        inc += (val - A[i]) * (rec[pos_end + 1] - rec[pos_end - len + 1]);
        vmin.pop_back();
      }else {
        vmin.emplace_back(A[i], i, i - pos_end);
        break;
      }
    }
    rec[i + 2] = 2 * rec[i + 1] - rec[i] + inc;
    DLOGK(i, rec);
    DLOGK(vmax);
    DLOGK(vmin);
  }
  Fp ans = rec[N + 1] - rec[N];
  cout << ans << endl;
