...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, N) {
    ll x, y; cin >> x >> y;
    XY[i] = IPoint(x, y);
    mp[enc(x, y)].push_back(i);
  }
  auto distsq = [&](ll i, ll j) -> ll {
    IPoint d = XY[i] - XY[j];
    return d.x * d.x + d.y * d.y;
  };

  auto ans = vector(N, vector<ll>());
  ll m = 0;
  REP(i, N) {
    IPoint& ip = XY[i];
    REP2(j, -1, 2) REP2(k, -1, 2) {
      auto it = mp.find(enc(ip.x + j * K, ip.y + k * K));
      DLOGK(ip.x + j * K, ip.y + k * K);
      if (it == mp.end()) continue;
      DLOGK(i, it->second);
      for (ll l : it->second) if (i < l and distsq(i, l) <= K * K) {
          ans[i].push_back(l);
          m++;
        }
    }
