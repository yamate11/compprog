...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, H) REP(j, 0, W) {
    ...
    if (brd.at(i, j) == 'G') goal = BrdIdx(i, j);
  }
  cakes.push_back(start);
  cakes.push_back(goal);
  REP(i, 0, H) REP(j, 0, W) {
    if (brd.at(i, j) == 'o') {
      cakes.emplace_back(i, j);
    }
  }
  DLOGK(cakes);
  ll numCakes = SIZE(cakes);
  ll big = 1e18;
  vector dd(numCakes, vector(numCakes, big));
  REP(i, 0, numCakes) {
    queue<BrdIdx> que;
    vector dist(H, vector(W, big));
    dist[cakes[i].r][cakes[i].c] = 0;
    que.push(cakes[i]);
    while (not que.empty()) {
      BrdIdx nd = que.front(); que.pop();
      for (BrdIdx dir : BrdIdx::nbr4) {
        BrdIdx peer = nd + dir;
        if (brd.at(peer) == '#') continue;
        if (dist[peer.r][peer.c] < big) continue;
        dist[peer.r][peer.c] = dist[nd.r][nd.c] + 1;
        que.push(peer);
      }
    }
    REP(j, 0, numCakes) dd[i][j] = dist[cakes[j].r][cakes[j].c];
    DLOGK(i, dist);
    DLOGK(dd);
  }
  DLOGK(dd);
  vector tbl(1LL << numCakes, vector(numCakes, big));
  tbl[1][0] = 0;
  REP(x, 1, 1LL << numCakes) {
    REP(i, 0, numCakes) if (x >> i & 1) {
      if (tbl[x][i] == big) continue;
      REP(j, 0, numCakes) if (not (x >> j & 1)) {
        updMin(tbl[x | (1LL << j)][j], tbl[x][i] + dd[i][j]);
      }
    }
  }
#if DEBUG
  REP(x, 0, 1LL << numCakes) {
    DLOGK(x, tbl[x]);
  }
#endif
  ll ans = -1;
  REP(x, 0, 1LL << numCakes) {
    if (x >> 1 & 1) {
      if (tbl[x][1] <= T) ans = max(ans, (ll)(__builtin_popcountll(x) - 2));
    }
  }
  cout << ans << endl;
