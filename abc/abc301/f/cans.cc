...
int body(istream& cin, ostream& cout) {
  ...
  REP(j, 0, N) {
    ...
    if ('a' <= ch and ch <= 'z') {
      ...
      ph3 += ph2_prev;
      ph3 += ph3_prev;
      REP(i, 0, nalph) ph4[i] += ph4_prev[i];
    }else if ('A' <= ch and ch <= 'Z') {
      ph2 += ph1_prev;
      ph2 += ph2_prev;
      if (j > lim) ph4[avail[j]] += ph3_prev;
      REP(i, 0, nalph) ph4[i] += ph4_prev[i];
    }else if (ch == '?') {
      ph1 += ph1_prev * nalph;
      ph2 += ph1_prev * nalph;
      ...
      ph2 += ph2_prev * nalph;
      ph3 += ph3_prev * nalph;
      if (j > lim and avail[j] >= 1) ph4[avail[j] - 1] += ph3_prev * avail[j];
      REP(i, 0, nalph) ph4[i] += ph4_prev[i] * nalph;
      REP(i, 1, nalph) ph4[i - 1] += ph4_prev[i] * i;
    }else assert(0);
  }
  Fp ans = ph1 + ph2 + ph3;
  REP(i, 0, nalph) ans += ph4[i];
  cout << ans << endl;
