...
int main(/* int argc, char *argv[] */) {
  ...
  auto addA = [&](ll x, ll y) -> ll { return (x + y) % primeA; };
  auto addB = [&](ll x, ll y) -> ll { return (x + y) % primeB; };

  vector powA(N, 0LL);
  vector powB(N, 0LL);
  powA[0] = powB[0] = 1;
  REP(i, 1, N) powA[i] = (powA[i - 1] * 27) % primeA;
  REP(i, 1, N) powB[i] = (powB[i - 1] * 27) % primeB;
  
  using Fp = FpG<0>;
  vector invpowA(N, 0LL);
  Fp::setMod(primeA);
  REP(i, 0, N) {
    Fp x = Fp(1) / Fp(powA[i]);
    invpowA[i] = (ll)x;
  }
  vector invpowB(N, 0LL);
  Fp::setMod(primeB);
  REP(i, 0, N) {
    Fp x = Fp(1) / Fp(powB[i]);
    invpowB[i] = (ll)x;
  ...
  vector initSA(N, 0LL);
  vector initSB(N, 0LL);
  REP(i, 0, N) initSA[i] = ((S[i] - 'a' + 1) * powA[i]) % primeA;
  REP(i, 0, N) initSB[i] = ((S[i] - 'a' + 1) * powB[i]) % primeB;

  vector initTA(N, 0LL);
  vector initTB(N, 0LL);
  REP(i, 0, N) initTA[i] = ((T[i] - 'a' + 1) * powA[i]) % primeA;
  REP(i, 0, N) initTB[i] = ((T[i] - 'a' + 1) * powB[i]) % primeB;
  ...
  auto stSA = make_seg_tree<ll>(0LL, addA, initSA);
  auto stSB = make_seg_tree<ll>(0LL, addB, initSB);
  auto stTA = make_seg_tree<ll>(0LL, addA, initTA);
  auto stTB = make_seg_tree<ll>(0LL, addB, initTB);

  auto hashS = [&](ll i, ll j) -> pll {
    ll va = (stSA.query(i, j + 1) * invpowA[i]) % primeA;
    ll vb = (stSB.query(i, j + 1) * invpowB[i]) % primeB;
    return pll(va, vb);
  };
  auto hashT = [&](ll i, ll j) -> pll {
    ll wa = stTA.query(i, j + 1);
    ll va = (wa * invpowA[i]) % primeA;
    ll wb = stTB.query(i, j + 1);
    ll vb = (wb * invpowB[i]) % primeB;
    DLOGK(wa, va, wb, vb);
    return pll(va, vb);
  };
  ...
  REP(_q, 0, Q) {
    ...
    if (t == 1) {
      ll x; cin >> x; x--;
      char c; cin >> c;
      stSA.update(x, ((c - 'a' + 1) * powA[x]) % primeA);
      stSB.update(x, ((c - 'a' + 1) * powB[x]) % primeB);
      stTA.update(N - 1 - x, ((c - 'a' + 1) * powA[N - 1 - x]) % primeA);
      stTB.update(N - 1 - x, ((c - 'a' + 1) * powB[N - 1 - x]) % primeB);
    }else if (t == 2) {
      ll L, R; cin >> L >> R; L--; R--;
      ll L2 = N - 1 - R;
      ll R2 = N - 1 - L;
