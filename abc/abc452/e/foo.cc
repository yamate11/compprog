...
int main(/* int argc, char *argv[] */) {
  try {
    ...
    // std::locale::global(std::locale("POSIX.UTF-8"));
  }catch(exception&) {
  }
  ...
  auto f = [&](ll e) -> void {
    ...
    ll lim = 1LL << e;
#if 0
    auto primes = sieve(llround(1.1 * ceil(sqrt(lim)) + 10));
    auto getds = [&](ll n) { return getDivisors(n, primes); };
#else
    auto divSieve = divisorSieve(lim);
    auto getds = [&](ll n) { return getDivisorsDivSieve(n, divSieve); };
#endif
    ll cnt = 0;
    REP(n, 1, lim + 1) cnt += getds(n).size();
    double t2 = get_time_sec();
    cout << format("{:11L} {:11L} {:7.1f} {:7.3f}", lim, cnt, (double)cnt / lim, t2 - t1) << endl;
  };

  for (ll e = 8; e <= 24; e += 2) f(e);
