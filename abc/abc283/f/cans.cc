...
int main(/* int argc, char *argv[] */) {
  ...
  auto st2 = make_seg_tree<ll>(-big, mymax, vector<ll>(N, -big));
  auto st3 = make_seg_tree<ll>(big, mymin, vector<ll>(N, big));
  auto st4 = make_seg_tree<ll>(-big, mymax, vector<ll>(N, -big));
  REP(i, 0, N) {
    ll k1 = st1.query(P[i], N);
    DLOGK(i, k1);
    ll l1 = P[i] - i;
    if (k1 < big) {
      updMin(ans[i], k1 - l1);
      DLOGK(i, l1, k1 - l1, ans[i]);
    }
    st1.update(P[i], l1);
    ll k2 = st2.query(0, P[i]);
    ll l2 = P[i] + i;
    if (k2 < big) {
      updMin(ans[i], l2 - k2);
    }
  ...
  REPrev(i, N - 1, 0) {
    ll k3 = st3.query(P[i], N);
    ll l3 = P[i] + i;
    if (k3 < big) {
      updMin(ans[i], k3 - l3);
    }
    st3.update(P[i], l3);
    ll k4 = st4.query(0, P[i]);
    ll l4 = P[i] - i;
    if (k4 < big) {
      updMin(ans[i], l4 - k4);
    }
    DLOGK(i, k4, l4, l4 - k4, ans[i]);
    st4.update(P[i], l4);
  }
  REPOUT(i, 0, N, ans[i], " ");
