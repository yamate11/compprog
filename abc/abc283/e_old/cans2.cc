  ...
  REP(i, 0, H) {
    auto ok = [&]() -> bool {
      REP(j, 0, W) {
        BrdIdx bi(i, j);
        ll c = 0;
        for (auto dir : BrdIdx::nbr4) c += brd.at(bi) == brd.at(bi + dir) ? 1 : 0;
        if (c == 0) return false;
      }
      return true;
    };
    auto ok2 = [&]() -> pair<bool, bool> {
      bool b1 = ok();
      if (i + 1 == H) return pll(b1, b1);
      REP(j, 0, W) brd.at(i + 1, j) = 1 - brd.at(i + 1, j);
      bool b2 = ok();
      REP(j, 0, W) brd.at(i + 1, j) = 1 - brd.at(i + 1, j);
      return pll(b1, b2);
    };
