...
int main(/* int argc, char *argv[] */) {
  ...
  vector vec(H, vector(2, vector(2, false)));

  auto chk = [&](ll r, ll p, ll q) -> bool {
    auto val = [&](BrdIdx bi) -> ll {
      if (not brd.in(bi)) return -1;
      ll e = bi.r == r ? 0 : bi.r == r - 1 ? p : q;
      return brd.at(bi) ^ e;
    };
    return FORALL(c, 0, W, EXISTS_C(dir, BrdIdx::nbr4, val(BrdIdx(r, c) + dir) == brd.at(r, c)));
  };
  REP(i, 0, H) REP(p, 0, 2) REP(q, 0, 2) vec[i][p][q] = chk(i, p, q);
  ...
  REP(r, 0, H) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(p, 0, 2) REP(q, 0, 2) REP(s, 0, 2) if (vec[r][p ^ q][q ^ s]) updMin(tbl[q][s], prev[p][q] + s);
  }
  ll ans = big;
  REP(p, 0, 2) REP(q, 0, 2) ans = min(ans, tbl[p][q]);
  if (ans == big) ans = -1;
  cout << ans << endl;
