...
int body(istream& cin, ostream& cout) {
  ...
  vector<pll> init_vec(sz);
  REP(i, 0, N) {
    ll a = cc.c(A[i]);
    auto [za, zb] = init_vec[a];
    init_vec[a] = pll(za + A[i] * B[i], zb + B[i]);
  }
  auto myplus = [&](pll p1, pll p2) -> pll { return {p1.first + p2.first, p1.second + p2.second}; };
  auto st = make_seg_tree<pll>(pll(0, 0), myplus, init_vec);
  REP(i, 0, Q) {
    auto [tp, x, y] = QUE[i];
    if (tp == 1) {
      ll old_a = cc.c(A[x]);
      ...
      ll b = B[x];
      auto [za, zb] = st[old_a];
      st.update(old_a, pll(za - A[x] * b, zb - b));
      auto [wa, wb] = st[new_a];
      st.update(new_a, pll(wa + y * b, wb + b));
      A[x] = y;
    }else if (tp == 2) {
      ll a = cc.c(A[x]);
      ll diff = y - B[x];
      auto [za, zb] = st[a];
      st.update(a, pll(za + A[x] * diff, zb + diff));
      B[x] = y;
    }else if (tp == 3) {
      auto check = [&](pll p) -> bool {
        auto [za, zb] = p;
        return zb <= x;
      };
      ll il = st.binsearch_l_until(check, sz);
      auto [wa, wb] = st.query(il, sz);
      if (wb == x) {
        cout << wa << "\n";
      }else {
        il--;
