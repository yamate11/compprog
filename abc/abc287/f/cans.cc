...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpNbrList(N, N - 1, nbr, dec=1) [GDK77BWP]
  ...
  // @End [GDK77BWP]
  vector tbl(N, vector(2, vector<Fp>()));
  auto myplus = [&](auto& vec, ll idx, Fp val) -> void {
    if (idx < 0) return;
    while (idx >= SIZE(vec)) vec.push_back(Fp(0));
    vec[idx] += val;
  };
  auto dfs = [&](auto rF, ll nd, ll pt) -> void {
    vector cur(2, vector<Fp>());
    myplus(cur[0], 1, 1);
    myplus(cur[1], 0, 1);
    for (ll cld : nbr[nd]) {
      if (cld == pt) continue;
      rF(rF, cld, nd);
      auto prev = move(cur);
      cur = vector(2, vector<Fp>());
      auto& vec = tbl[cld];
      REP(i, 0, SIZE(prev[0])) REP(j, 0, SIZE(vec[0])) myplus(cur[0], i + j - 1, prev[0][i] * vec[0][j]);
      REP(i, 0, SIZE(prev[0])) REP(j, 0, SIZE(vec[1])) myplus(cur[0], i + j,     prev[0][i] * vec[1][j]);
      REP(i, 0, SIZE(prev[1])) REP(j, 0, SIZE(vec[0])) myplus(cur[1], i + j,     prev[1][i] * vec[0][j]);
      REP(i, 0, SIZE(prev[1])) REP(j, 0, SIZE(vec[1])) myplus(cur[1], i + j,     prev[1][i] * vec[1][j]);
    }
    tbl[nd] = move(cur);
    DLOGK(nd, tbl[nd][0]);
    DLOGK(nd, tbl[nd][1]);
  ...
  dfs(dfs, 0, -1);
  auto f = [&](const auto& vec, ll i) -> Fp {
    if (i >= SIZE(vec)) return Fp(0);
    else return vec[i];
  };
  REP(i, 1, N + 1) cout << f(tbl[0][0], i) + f(tbl[0][1], i) << "\n";
