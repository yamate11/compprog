...
// @@ !! LIM(sieve mod power perm debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  string S; cin >> S;
  auto primes = sieve(max(N, 100LL));
  auto divs = getDivisors(N, primes);
  sort(ALL(divs));
  vector<Fp> vec(N + 1), pvec(N + 1);
  for (ll d : divs) {
    vector w(d, false);
    REP(i, 0, N) if (S[i] == '.') w[i % d] = true;
    DLOGK(d, w);
    ll m = 0; REP(i, 0, d) if (not w[i]) m++;
    vec[d] = power<Fp>(2, m);
    pvec[d] = vec[d];
    for (ll e : getDivisors(d)) if (e < d) pvec[d] -= pvec[e];
    DLOGK(d, vec[d], pvec[d]);
  }
  Fp ans = 0;
  for (ll d : divs) if (d < N) ans += pvec[d];
  cout << ans << endl;
