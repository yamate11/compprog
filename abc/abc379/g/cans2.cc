...
// @@ !! LIM(mod forall debug f:perfmeas)
...
int main(/* int argc, char *argv[] */) {
  ...
  if (H1 >= W1) {
    ...
    W = H1;
    S = vector(H, string(W, ' '));
    REP(i, 0, H) REP(j, 0, W) S[i][j] = S1[j][i];
  }
  vector<string> alls;
  map<string, ll> enc;
  map<string, vector<string>> mp_raw;
  ll e = 0;
  REP(i0, 0, 3) {
    REP(x, 0, 1LL << (W - 1)) {
      string s(1, '0' + i0);
      ...
      enc[s] = e++;
      vector<string> res;
      auto dfs = [&](auto rF, string t) -> void {
        ll len = ssize(t);
        if (len == W) {
          res.push_back(t);
          return;
          ...
          REP(d, 0, 3) {
            char c = '0' + d;
            if (s[len] == c) continue;
            if (len >= 1 and t.back() == c) continue;
            rF(rF, t + c);
          }
        }
      };
  ...
  ll nume = e;
  vector mp(nume, vector<ll>());
  for (auto& [s, ts] : mp_raw) {
    ll e1 = enc[s];
    for (const string& t : ts) mp[e1].push_back(enc[t]);
  }
  DLOGK(alls);
  double t2 = get_time_sec();
  vector count_init(nume, Fp(0));
  auto count = count_init;
  for (const string& s : alls) {
    if (FORALL(i, 0, W, S[H - 1][i] == '?' or S[H - 1][i] == s[i])) count[enc[s]] = 1;
    else count[enc[s]] = 0;
  }
  DLOGKL("init", count);
  REPrev(r, H - 2, 0) {
    auto prev = move(count);
    count = count_init;
    for (const string& s : alls) {
      ll e0 = enc[s];
      if (FORALL(i, 0, W, S[r][i] == '?' or S[r][i] == s[i])) {
        Fp x = 0;
        for (ll f : mp[e0]) x += prev[f];
        count[e0] = x;
      }else {
        count[e0] = 0;
      }
  ...
  Fp ans = 0;
  for (const string& s : alls) ans += count[enc[s]];
