...
int main(/* int argc, char *argv[] */) {
  ...
  auto mymin = [&](pll x, pll y) -> pll {
    auto [x1, x2] = x;
    auto [y1, y2] = y;
    return pll(min(x1, y1), min(x2, y2));
  };
  auto comp = [&](OP f, OP g) -> OP { return f ? f : g; };
  auto appl = [&](OP f, pll x) -> pll { return f ? *f : x; };
  auto st = make_seg_tree_lazy(pll(big, big), OP(), mymin, comp, appl, vector(N, pll(-C, 0)));

  REP(i, 0, M) {
    ll t, p; cin >> t >> p; t--;
    auto [a, b] = st.at(t);
    ll val = a * t + b + p;
    auto check1 = [&](ll u) -> bool {
      auto [aa, bb] = st.at(u);
      return aa * u + bb <= -C * (u - t) + val;
    };
    ll u = binsearch(check1, t, N) + 1;
    auto check2 = [&](ll v) -> bool {
      auto [aa, bb] = st.at(v);
      return aa * v + bb <= C * (v - t) + val;
    };
    ll v = binsearch(check2, t, -1LL);
    // y = C(x - t) + val
    st.update(v, t, pll(C, -C * t + val));
    // y = -C(x - t) + val
    st.update(t, u, pll(-C, C * t + val));
  }
  ll ans = 0;
  REP(i, 0, N) {
    auto [a, b] = st.at(i);
