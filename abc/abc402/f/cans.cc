...
int main(/* int argc, char *argv[] */) {
  ...
  REP(a, 0, N) {
    ...
    while (ic1.get()) {
      ...
      val *= c0;
      upper.push_back((ll)val);
    }
    DLOGK(a, upper);
    IntComb ic2(N - 1, N - 1 - a);
    vector<ll> lower;
    while (ic2.get()) {
      ll i = a;
      ll j = N - 1 - a;
      ...
      lower.push_back((ll)val);
    }
    DLOGK(a, lower);
    sort(ALL(lower));
    for (ll p : upper) {
      ll j = lower_bound(ALL(lower), M - 1 - p) - lower.begin();
      ll q;
      if (j == ssize(lower)) {
        q = lower[j - 1];
      }else {
