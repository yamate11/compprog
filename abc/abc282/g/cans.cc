...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K, P; cin >> N >> K >> P;
  Fp::setMod(P);
  vector tbl(K + 1, vector(N, vector(N, Fp(0))));
  vector sum(K + 1, vector(N + 1, vector(N + 1, Fp(0))));
  auto mksum = [&]() -> void {
    REP(k, 0, K + 1) {
      REP(p, 0, N) REP(q, 0, N) sum[k][p + 1][q + 1] = tbl[k][p][q];
      REP(p, 1, N + 1) REP(q, 1, N + 1) sum[k][p][q] += sum[k][p - 1][q];
      REP(p, 1, N + 1) REP(q, 1, N + 1) sum[k][p][q] += sum[k][p][q - 1];
    }
  };
  auto fs = [&](ll k, ll p1, ll q1, ll p2, ll q2) -> Fp {
    return sum[k][p2][q2] - sum[k][p1][q2] - sum[k][p2][q1] + sum[k][p1][q1];
  };
  tbl[0][0][0] = Fp(1);
  REP(i, 1, N) {
    mksum();
    REP(k, 0, K + 1) {
      REP(p, 0, i + 1) REP(q, 0, i + 1) {
        tbl[k][p][q] = Fp(0);
        if (k >= 1) tbl[k][p][q] += fs(k - 1, 0, 0, p, q) + fs(k - 1, p, q, i, i);
        tbl[k][p][q] += fs(k, 0, q, p, i) + fs(k, p, 0, i, q);
      }
    }
  }
  mksum();
  cout << fs(K, 0, 0, N, N) << endl;
