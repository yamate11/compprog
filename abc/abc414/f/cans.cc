...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    vector dist(K, vector(2, vector(N - 1, big)));
    queue<sta> que;
    vector vcnt(K, vector(N, 0LL));
    for (auto [c, e] : tr.children_pe(0)) {
      dist[0][0][e] = 1;
      que.emplace(e, 0, 0);
      vcnt[0][c] = 1;
    }
    ...
    while (not que.empty()) {
      auto [e, dir, k] = que.front(); que.pop();
      auto [src, dst] = tr.nodes_of_edge(e, dir);
      DLOGKL("popped", e, dir, k, src, dst);
      ll new_d = dist[k][dir][e] + 1;
      ll new_k = (k + 1) % K;

      for (auto [peer, edge] : tr.neighbor_pe(dst)) {
        DLOGKL("    loop", dst, peer, edge);
        ll pdir = peer == tr.parent(dst);
        if (new_k == 0 or peer != src) {
          if (vcnt[new_k][peer] < 2 and new_d < dist[new_k][pdir][edge]) {
            dist[new_k][pdir][edge] = new_d;
            que.emplace(edge, pdir, new_k);
            vcnt[new_k][peer]++;
            DLOGKL("pushed", peer, edge, pdir, new_d, new_k);
          }
        }
      }
    ...
    vector<ll> ans(N, big);
    REP(e, 0, N - 1) REP(dir, 0, 2) {
      auto [src, dst] = tr.nodes_of_edge(e, dir);
      ll v = dist[K - 1][dir][e];
      if (v < big) updMin(ans[dst], v / K);
    }
    REPOUT(i, 1, N, ans[i] == big ? -1 : ans[i], " ");
