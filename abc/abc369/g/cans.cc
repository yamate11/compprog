...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> pls;
  auto dfs2 = [&](auto rF, ll nd, bool sw) -> void {
    if (nd == 0) {
      pls.push_back(stlen[nd]);
    }else if (sw) {
      ll pl = stlen[nd] - alen[tr.parent(nd)];
      pls.push_back(pl);
    }
    for (int c : tr.children(nd)) {
      if (c == stlen_cld[nd]) {
  ...
  dfs2(dfs2, 0, true);
  sort(ALL(pls), greater<ll>());
  ll s = 0;
  REP(i, 0, N) {
    if (i < ssize(pls)) s += 2 * pls[i];
    cout << s << "\n";
  }
