...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, cut, dec=1) [k2gF2KgV]
  auto cut = vector(N, vector(0, int()));
  for (int i = 0; i < M; i++) {
    int u, v; cin >> u >> v; u -= 1; v -= 1;
    cut[u].emplace_back(v);
    cut[v].emplace_back(u);
  }
  // @End [k2gF2KgV]

  if (not EXISTS_C(i, cut[0], i == N - 1)) {
    cout << 1 << endl;
    return 0;
  }

  vector nset(2, vector<ll>());
  vector isNset(2, vector(N, Fp(0)));
  vector tot(2, Fp(0));

  nset[0] = vector<ll>{0};
  nset[1] = vector<ll>{N - 1};
  isNset[0][0] = Fp(1);
  isNset[1][N - 1] = Fp(1);
  tot[0] = Fp(1);
  tot[1] = Fp(1);

  auto num_conn = [&](ll n, ll p) -> Fp {
    Fp ret = tot[p];
    for (ll y : cut[n]) ret -= isNset[p][y];
    return ret;
  };

  vector<ll> rest;
  REP(i, 1, N - 1) rest.push_back(i);

  ll len = 0;
  while (true) {
    ll sr = SIZE(rest);
    auto vec = vector(2, vector(sr, Fp(0)));
    Fp z = 0;
    REP(i, 0, sr) {
      REP(p, 0, 2) vec[p][i] = num_conn(rest[i], p);
      z += vec[0][i] * vec[1][i];
    }
    if (z != Fp(0)) {
      cout << z << endl;
    ...
    REP(p, 0, 2) {
      for (ll n : nset[p]) isNset[p][n] = Fp(0);
      nset[p].resize(0);
    }
    tot[0] = tot[1] = 0;
    auto new_rest = vector<ll>();
    REP(i, 0, sr) {
      ll cnt = 0;
      REP(p, 0, 2) {
        if (vec[p][i] != Fp(0)) {
          cnt++;
          nset[p].push_back(rest[i]);
          isNset[p][rest[i]] = vec[p][i];
          tot[p] += vec[p][i];
        }
      }
      assert(cnt <= 1);
      if (cnt == 0) new_rest.push_back(rest[i]);
    }
    if (nset[0].empty() or nset[1].empty()) {
      cout << -1 << endl;
      return 0;
    }
    rest = move(new_rest);
    Fp ans = 0;
    for (ll r : nset[0]) ans += isNset[0][r] * num_conn(r, 1);
    if (ans != Fp(0)) {
      cout << ans << endl;
      return 0;
    }
    len++;
    DLOGK(len, nset[0], nset[1], rest);
  }
