...
int main(/* int argc, char *argv[] */) {
  ...
  auto C = vector(3, vector(3, 0LL));
  REP(i, 0, 3) REP(j, 0, 3) cin >> C[i][j];

  auto dec = [&](ll e) -> pll { return pll(e / 3, e % 3); };
  IntPerm ip(9, 9);
  ll tot = 9 * 8 * 7 * 6 * 5 * 4 * 3 * 2;
  ll cnt = 0;
  while (ip.get()) {
    auto notgood = [&]() -> bool {
      auto know = vector(3, vector(3, false));
      REP(e, 0, 9) {
        auto [i0, j0] = dec(ip.at(e));
        if (not FORALL(i, 0, 3, i == i0 or know[i][j0])) {
          if (EXISTS(i, 0, 3, i != i0 and know[i][j0] and C[i][j0] == C[i0][j0])) return true;
        }
        if (not FORALL(j, 0, 3, j == j0 or know[i0][j])) {
          if (EXISTS(j, 0, 3, j != j0 and know[i0][j] and C[i0][j] == C[i0][j0])) return true;
        }
        if (i0 == j0 and not FORALL(i, 0, 3, i == i0 or know[i][i])) {
          if (EXISTS(i, 0, 3, i != i0 and know[i][i] and C[i][i] == C[i0][j0])) return true;
        }
        if (i0 + j0 == 2 and not FORALL(i, 0, 3, i == i0 or know[i][2 - i])) {
          if (EXISTS(i, 0, 3, i != i0 and know[i][2 - i] and C[i][2 - i] == C[i0][j0])) return true;
        }
        know[i0][j0] = true;
      }
      return false;
  ...
  cout << (double)cnt / (double)tot << endl;
