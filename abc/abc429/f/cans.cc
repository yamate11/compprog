...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(3, S, type=string) [Yk8JK88X]
  auto S = vector(3, string());
  for (int i = 0; i < 3; i++) { string v; cin >> v; S[i] = v; }
  // @End [Yk8JK88X]
  
  ll big = 1e18;
  using DAT = array<array<ll, 3>, 3>;
  DAT unit = DAT();
  REP(i, 0, 3) REP(j, 0, 3) unit[i][j] = 0;
  vector<DAT> one(8, unit);
  REP(x, 0, 8) {
    REP(i, 0, 3) one[x][i][i] = (x >> i & 1) ? big : 0;
    one[x][0][2] = one[x][2][0] = (x == 0) ? 2 : big;
    one[x][0][1] = one[x][1][0] = ((x >> 0 & 1) or (x >> 1 & 1)) ? big : 1;
    one[x][1][2] = one[x][2][1] = ((x >> 1 & 1) or (x >> 2 & 1)) ? big : 1;
  }
  auto myadd = [&](DAT a, DAT b) -> DAT {
    ...
    REP(i, 0, 3) REP(j, 0, 3) {
      ret[i][j] = big;
      REP(k, 0, 3) updMin(ret[i][j], a[i][k] + 1 + b[k][j]);
    }
    return ret;
  };
  auto f = [&](ll i) -> DAT {
    ll x = 0;
    REP(j, 0, 3) if (S[j][i] == '#') x |= (1LL << j);
    return one[x];
  };
  vector<DAT> init_vec(N);
  REP(i, 0, N) init_vec[i] = f(i);
  auto st = make_seg_tree(unit, myadd, init_vec);

#if DEBUG
  REP(i, 0, N) REP(j, i + 1, N + 1) DLOGK(i, j, st.query(i, j));
#endif
...
#if DEBUG
    DLOGK(_q);
    REP(ii, 0, N) REP(jj, ii + 1, N + 1) DLOGK(ii, jj, st.query(ii, jj));
#endif
