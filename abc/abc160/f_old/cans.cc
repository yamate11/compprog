...
public:
  ...
  T dfs1(int n) {
    vector<T> vec;
    for (ll c : child.at(n)) vec.push_back(mod(dfs1(c)));
    int k = child.at(n).size();
    ...
    rightN.resize(k+1);
    rightN.at(k) = unitT;
    DLOG("k=", k, "rightN.at(k)=", rightN.at(k), "unitT", unitT);
    for (int i = k-1; i >= 0; i--) {
      rightN.at(i) = add(vec.at(i), rightN.at(i+1));
    }
    DLOG("dfs1 n=", n, "rightN=", rightN);
    return rightN.at(0);
  }
  ...
  void dfs2(int n, const T& parRes) {
    ...
    res.at(n) = thisRes;
    DLOG("dfs2 n=", n, "thisRes=", thisRes);
    for (int i = 0; i < (int)child.at(n).size(); i++) {
      int c = child.at(n).at(i);
      T x = add(parRes, add(left.at(n).at(i), right.at(n).at(i+1)));
      dfs2(c, mod(move(x)));
    }
  }

  RerootTree(const auto& edge_, auto add_, auto mod_, 
	     T unitT_, int root_ = 0
	     ) : Tree(edge_, root_), unitT(unitT_), add(add_), mod(mod_), 
		 left(numNodes), right(numNodes), res(numNodes) {
    dfs1(root);
    dfs2(root, unitT);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  auto mod = [&](T t) -> T { return make_pair(t.first + 1, t.second); };
  DLOG("in main unitT=", unitT);
  RerootTree<T, decltype(add), decltype(mod)>
    tr(move(ve), add, mod, unitT, 0);

  for (ll i = 0; i < N; i++) cout << tr.res.at(i).second << "\n";
