...
// @@ !! LIM(mod debug f:updMaxMin)
// --> f:gcd mod f:<< debug f:updMaxMin
...
int main(int argc, char *argv[]) {
  ...
  using T = pair<ll, Fp>;
  const T unitT({0, 1});
  auto merge = [&](T t1, T t2) -> T {
    ll card1; Fp perm1; tie(card1, perm1) = t1;
    ll card2; Fp perm2; tie(card2, perm2) = t2;
    ll card = card1 + card2;
    Fp perm = perm1 * perm2 * cb.fact(card)
                  / (cb.fact(card1) * cb.fact(card2));
    T ret = make_pair(card, perm);
    DLOG("merge t1", t1, "t2", t2, "ret", ret);
    return ret;
  };
  auto divide = [&](const T& t0, const T& t1) -> T {
    ll card0; Fp perm0; tie(card0, perm0) = t0;
    ll card1; Fp perm1; tie(card1, perm1) = t1;
    ll card = card0 - card1;
    Fp perm = perm0 * cb.fact(card) * cb.fact(card1)
                   / (cb.fact(card0) * perm1);
    T ret = make_pair(card, perm);
    DLOG("divide t0", t0, "t1", t1, "ret", ret);
    return ret;
  };
  auto pre = [&](T t) -> T { return make_pair(t.first + 1, t.second); };

  vector<T> firstRes(N);
  vector<T> secondRes(N);
  auto dfs1 = [&](const auto& recF, ll n) -> T {
    vector<T> ts;
    T m = unitT;
    for (ll c : tr.child.at(n)) m = merge(m, pre(recF(recF, c)));
    firstRes.at(n) = m;
    DLOG("dfs1 n", n, "ret", m);
    return m;
  };
  auto dfs2 = [&](const auto& recF, ll n, const T& parRes) -> void {
    DLOG("dfs2 n", n, "parRes", parRes);
    T fr = firstRes.at(n);
    T thisRes = merge(fr, parRes);
    secondRes.at(n) = thisRes;
    for (ll c : tr.child.at(n)) {
      recF(recF, c, pre(divide(thisRes, pre(firstRes.at(c)))));
    }
  };
  dfs1(dfs1, tr.root);
  dfs2(dfs2, tr.root, unitT);
  for (ll i = 0; i < N; i++) cout << secondRes.at(i).second << "\n";
