...
int main() {
  ll H, W, Q; cin >> H >> W >> Q;
  itv_set<bool> h_full(-1, W + 1);
  h_full.put(0, W, true);
  itv_set<bool> v_full(-1, H + 1);
  v_full.put(0, H, true);
  ...
  REP(_q, 0, Q) {
    ...
    if (val == true) {
      ...
      if (lo >= 1) to_erase.emplace_back(r, lo - 1);  
      if (hi < W)  to_erase.emplace_back(r, hi);
      auto [loV, hiV, valV] = vert[c].get(r);
      if (loV >= 1) to_erase.emplace_back(loV - 1, c);
      if (hiV < H)  to_erase.emplace_back(hiV,     c);
      rem -= ssize(to_erase);
      for (auto [rr, cc] : to_erase) {
        horiz[rr].put(cc, false);
        vert[cc ].put(rr, false);
      }
    }
  }
  cout << rem << endl;
