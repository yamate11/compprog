...
int main(int argc, char *argv[]) {
  ...
  vector<ll> numBlock(N), numFriend(N);
  vector<ll> A(M), B(M);
  for (ll i = 0; i < M; i++) {
    ll a, b; cin >> a >> b; a--; b--;
    DLOG("a", a, "b", b);
  ...
  for (ll i = 0; i < K; i++) {
    ...
    DLOG("c", c, "d", d);
    ll ld = uf.leader(c);
    if (ld != uf.leader(d)) continue;
    numBlock.at(c)++;
    numBlock.at(d)++;
  }
  for (ll i = 0; i < N; i++) {
    if (i > 0) cout << " ";
    ll sz = uf.group(i).size();
    DLOG("i", i, "grp", uf.group(i), "sz", sz, "nF", numFriend.at(i), "nB", numBlock.at(i));
    cout << sz - 1 - numFriend.at(i) - numBlock.at(i);
  }
  cout << endl;
