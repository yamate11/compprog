...
struct Circle {
  ...
  tuple<bool, Point, Point> __intersect(const Circle& o) const {
    Real d = (o.c - c).len();
  ...
  tuple<bool, Point, Point> intersect(const Circle& o) const {
    Point w = o.c - c;
    Real d = w.len();
    Real a = (w.x*w.x + w.y*w.y + r*r - o.r*o.r) / 2;
    Real z = sqrt(d*d*r*r - a*a);
    Point p1((a * w.x + w.y * z) / (d*d), (a * w.y - w.x * z) / (d*d));
    Point p2((a * w.x - w.y * z) / (d*d), (a * w.y + w.x * z) / (d*d));
    return make_tuple(true, p1 + c, p2 + c);
  }
...
int main(int argc, char *argv[]) {
  ...
  Circle circ0(p0, tm / c0);
  Circle circ1(p1, tm / c1);
  bool b; Point q0, q1;
  tie(b, q0, q1) = circ1.intersect(circ0);
  cout << q0 << endl << q1 << endl;
  cout << tm / c0 << endl;
  cout << (q0 - p0).len() << endl;
  cout << (q1 - p0).len() << endl;
  ...
  tie(b, q0, q1) = circ1.__intersect(circ0);
  cout << q0 << endl << q1 << endl;
  cout << tm / c0 << endl;
  cout << (q0 - p0).len() << endl;
  cout << (q1 - p0).len() << endl;
