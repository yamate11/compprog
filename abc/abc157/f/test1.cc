...
bool r_sim(Real x, Real y, Real err = g_err) {
  return abs(y - x) < err || abs(y - x) < abs(x) * err;
}
...
struct Point {
  ...
  bool sim(const Point& p, Real err = g_err) const {
    return r_sim(x, p.x, err) && r_sim(y, p.y, err);
  }
...
int main(int argc, char *argv[]) {
  ...
  Point p0(-151.1, 218.1);
  vector<pair<double, ll>> di(N);
  for (ll i = 0; i < N; i++) {
    double d = (P.at(i) - p0).len() * C.at(i);
    di.at(i) = {d, i};
  }
  ...
  double vmin = DBL_MAX;
  double xmin, ymin;
  for (double y = 150; y < 250; y += 0.1) {
    for (double x = -200; x < -100; x += 0.1) {
      Point p0(x, y);
      vector<double> ds(N);
      for (ll i = 0; i < N; i++) ds.at(i) = (P.at(i) - p0).len() * C.at(i);
      sort(ds.begin(), ds.end());
      if (ds.at(K-1) < vmin) {
	vmin = min(vmin, ds.at(K-1));
	xmin = x;
	ymin = y;
      }
    }
  }
  cout << vmin << " " << xmin << " " << ymin << endl;
  */
