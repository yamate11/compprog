...
int main(/* int argc, char *argv[] */) {
  ...
  UnionFind uf(N);
  for (ll i = 0; i < N; i++) uf.merge(P[i], Q[i]);
  ll smax = 0;
  for (const auto& [dum, vec] : uf.groups()) smax = max(smax, (ll)vec.size());
  vector F(smax + 1, vector<ll>());
  F[1] = {1,1};
  auto tbl1_init = vector(3, vector(3, vector(smax + 1, Fp(0))));
  auto tbl1 = tbl1_init;
  for (ll a = 0; a < 3; a++) {
    for (ll b = 0; b < 3; b++) {
      if (a == 2 && b == 1) continue;
  ...
  for (ll i = 1; i < smax; i++) {
    ...
    for (ll t = 0; t <= i; t++) {
      for (ll a = 0; a < 3; a++) {
        for (ll b = 0; b < 3; b++) {
          for (ll b0 = 0; b0 < 3; b0++) {
            if (b == 2 && b0 == 1) continue;
            ll t0 = b0 == 0 ? t : t + 1;
            tbl1[a][b0][t0] += prev[a][b][t];
          }
        }
      }
    }
    F[i + 1].resize(i + 2);
    for (ll t = 0; t < i + 2; t++) {
      for (ll a = 0; a < 3; a++) F[i + 1][t] += tbl1[a][a][t];
    }
  }
  
  vector<Fp> tbl2({1});
  for (const auto& [dum, vec] : uf.groups()) {
    auto prev = move(tbl2);
    ll sz = vec.size();
    tbl2 = vector<Fp>(prev.size() + sz);
    for (ll i = 0; i < (ll)prev.size(); i++) {
      for (ll t = 0; t <= sz; t++) {
        tbl2[i + t] += prev[i] * F[sz][t];
      }
    }
  }
  ...
  vector<Fp> fact(N + 1);
  fact[0] = Fp(1);
  for (ll i = 1; i <= N; i++) fact[i] = fact[i - 1] * Fp(i);
  Fp ans = fact[N];
  ll sign = -1;
  for (ll k = 1; k <= N; k++) {
    ans += sign * tbl2[k] * fact[N - k];
    sign = -sign;
  }
  cout << ans << endl;
