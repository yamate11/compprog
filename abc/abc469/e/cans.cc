...
int main(/* int argc, char *argv[] */) {
  ...
  string S; cin >> S;
  vector<ll> A(N + 1);
  REP(i, 0, N) A[i + 1] = A[i] + (S[i] == 'o' ? 1 : 0);
  vector<ll> B(N);
  ll j = 0;
  REP(i, 0, N) {
    if (j < i) j = i;
    while (j < N + 1 and A[j] - A[i] < K) j++;
    B[i] = j;
  }

  auto check = [&](double p) -> bool {
    vector<double> V(N + 1);
    REP(i, 0, N) V[i + 1] = V[i] + (S[i] == 'o' ? (1 - p) : -p);
    vector<double> W(N + 1);
    W[N] = V[N];
    REPrev(i, N - 1, 0) W[i] = max(V[i], W[i + 1]);
    REP(i, 0, N) {
      if (B[i] > N) break;
      if (W[B[i]] >= V[i]) return true;
    }
    return false;
  };
  double p = binsearch<double>(check, 0.0, 1.0, 1e-7);
  cout << p << "\n";
