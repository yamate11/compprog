...
// @@ !! LIM(sieve unordered_map debug UnionFind)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [Ur3Op1g6]
  ll maxA = *ranges::max_element(A);
  auto divSieve = divisorSieve(maxA);
  vector dvs(maxA + 1, vector<ll>());
  REP(i, 0, ssize(A)) for (ll d : getDivisorsDivSieve(A[i], divSieve)) dvs[d].push_back(i);
  UnionFind uf(N);
  ll ans = 0;
  REPrev(d, maxA, 1) {
    DLOGK(d, dvs[d]);
    REP(i, 1, ssize(dvs[d])) {
      if (uf.leader(dvs[d][0]) != uf.leader(dvs[d][i])) {
        DLOGKL("merging", d, dvs[d][0], dvs[d][i]);
        uf.merge(dvs[d][0], dvs[d][i]);
        ans += d;
      }else {
        DLOGKL("already merged", dvs[d][0], dvs[d][i]);
      }
    }
  }
  cout << ans << "\n";
