...
int main(/* int argc, char *argv[] */) {
  ...
  auto divSieve = divisorSieve(maxA);
  vector dvs(maxA + 1, vector<ll>());
  REP(i, 1, ssize(A)) for (ll d : getDivisorsDivSieve(A[i], divSieve)) dvs[d].push_back(A[i]);
  priority_queue<ll> pque;
  for (ll d : getDivisorsDivSieve(A[0], divSieve)) pque.push(d);
  app[A[0]] = false;
  DLOGK(ans);
  DLOGK(pque);
  while (not pque.empty()) {
    ll d = pque.top();
    DLOGKL("popped from pque", d, pque);
    auto get_cand = [&]() -> ll {
      while (not dvs[d].empty()) {
        ll a = dvs[d].back(); dvs[d].pop_back();
        if (app[a]) {
          app[a] = false;
          return a;
        }
    ...
    if (a < 0) {
      ...
      ans += d;
      DLOGK(ans);
      for (ll d0 : getDivisorsDivSieve(a, divSieve)) pque.push(d0);
    }
  }
  cout << ans << "\n";
