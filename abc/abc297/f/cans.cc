...
int main(/* int argc, char *argv[] */) {
  ...
  auto prob = [&](ll r, ll c) -> Fp {
    Fp na = cb.binom(H * c, K);
    Fp nb = cb.binom(H * (W - c - 1), K);
    Fp nc = cb.binom(r * W, K);
    Fp nd = cb.binom((H - r - 1) * W, K);
    Fp nac = cb.binom(r * c, K);
    Fp nbc = cb.binom(r * (W - c - 1), K);
    Fp nad = cb.binom((H - r - 1) * c, K);
    Fp nbd = cb.binom((H - r - 1) * (W - c - 1), K);
    Fp x = (na + nb + nc + nd) - (nac + nbc + nad + nbd);
    Fp p = x / cb.binom(H * W, K);
    return 1 - p;
  };

  Fp e = 0;
  REP(i, 0, H) REP(j, 0, W) e += prob(i, j);
  cout << e << endl;
