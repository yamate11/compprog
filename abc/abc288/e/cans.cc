...
int main(/* int argc, char *argv[] */) {
  ...
  vector<bool> inX(N);
  REP(i, 0, M) inX[X[i]] = true;

  auto mymin = [&](ll x, ll y) -> ll { return min(x, y); };
  auto st = make_seg_tree<ll>(big, mymin, C);

  vector tbl(N, vector(N+1, big));
  REP(i, 0, N) {
    if (i == 0) {
      tbl[0][0] = A[0] + C[0];
      if (not inX[0]) {
      ...
      REP(j, 0, i + 2) {
        ll v = st.query(j, i + 1);
        updMin(tbl[i][j], tbl[i - 1][j] + A[i] + v);
        if (j > 0 and not inX[i]) {
