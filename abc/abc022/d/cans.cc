...
struct Line {
  ...
  static const int SIDE_ON = 1;
  static const int SIDE_P = 2;
  static const int SIDE_N = 4;
...
int main(int argc, char *argv[]) {
  ...
  ll N; cin >> N;
  vector<Point> vec0, vec1;
  for (ll i = 0; i < N; i++) {
    double x, y; cin >> x >> y;
    vec0.emplace_back(x, y);
  }
  for (ll i = 0; i < N; i++) {
    double x, y; cin >> x >> y;
    vec1.emplace_back(x, y);
  }
  double d0 = analyse(vec0);
  double d1 = analyse(vec1);
  cout << d1 / d0 << endl;
