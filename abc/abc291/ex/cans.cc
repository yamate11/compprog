...
// @@ !! LIM(forall debug cmpNaive tree)
...
int naive(istream& cin, ostream& cout) {
  ll N; cin >> N;
  // @InpMVec(N - 1, ((A, dec=1), (B, dec=1))) [6MHnvchy]
  ...
  Tree trT(N, 0);
  REP(i, 0, N - 1) trT.add_edge(A[i], B[i]);

  // @InpVec(N, newPt) [hrPL5540]
  auto newPt = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; newPt[i] = v; }
  // @End [hrPL5540]
  REP(i, 0, N) if (newPt[i] >= 0) newPt[i]--;

  ll root = EXFIND(i, 0, N, newPt[i] == -1);
  Tree trR(N, root);
  REP(i, 0, N) if (newPt[i] >= 0) trR.add_edge(i, newPt[i]);

  REP(i, 0, N) {
    if (i == root) continue;
    ll sts = trR.stsize(i);
    ll psts = trR.stsize(newPt[i]);
    if (sts * 2 > psts) {
      cout << "NG\n";
      DLOGKL("big subtree", i, newPt[i], sts, psts);
      return 0;
    }
  }
  ...
  REP(x, 0, N) REP(y, x + 1, N) {
    ll z = trR.lca(x, y);
    const auto path = trT.nnpath(x, y);
    bool b = EXISTS(i, 0, SIZE(path), path[i] == z);
    if (not b) {
      cout << "NG\n";
      DLOGKL("not on path", x, y, z, path);
      return 0;
    }
  }
  cout << "OK\n";
...
int body(istream& cin, ostream& cout) {
  ...
  ll N; cin >> N;
  // @InpNbrList(N, N - 1, nbr, dec=1) [AJVQl7KO]
  ...
  vector ssize(N, 0LL);
  auto calc_ssize = [&](auto rF, ll nd, ll pt) -> ll {
    if (parent[nd] >= 0) {
      ssize[nd] = 0;
      return 0;
    }
  ...
  auto dfs = [&](auto rF, ll nd, ll pt, ll sz, ll newPt) -> void {
    if (parent[nd] >= 0) return;
    DLOGKL("dfs in", nd, pt, sz, newPt);
    for (ll cld : nbr[nd]) {
      if (cld == pt) continue;
      ll s = ssize[cld];
      DLOGKL("  checking ssize", nd, cld, s, sz);
      if (2 * s > sz) {
        rF(rF, cld, nd, sz, newPt);
        return;
      }
    }
    DLOGKL("new node found", nd, newPt);
    parent[nd] = newPt;
    for (ll cld : nbr[nd]) {
      ll csz = calc_ssize(calc_ssize, cld, nd);
      rF(rF, cld, nd, csz, nd);
    }
  };
  calc_ssize(calc_ssize, 0, -1);
  dfs(dfs, 0, N, N, N);
  REPOUT(i, 0, N, parent[i] == N ? -1 : parent[i] + 1, " ");
