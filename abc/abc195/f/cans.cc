...
int main(/* int argc, char *argv[] */) {
  ...
  bset full;
  for (ll i = 0; i < sz; i++) full[i] = true;

  ll cnt = 0;
  using que_t = pair<bset, ll>; // remained set, isolated points
  queue<que_t> que;
  que.emplace(full, 0);
  while (!que.empty()) {
    auto [x, iso] = que.front(); que.pop();
    ll i = minv(x);
    if (i < 0) {
      cnt += 1 << cnt;
      return;
