...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> cur(1LL << len);
  cur[0] = 1;
  for (ll i = A; i <= B; i++) {
    auto prev = cur;
    // cur = vector<ll>(1LL << len);
    ll y = 0;
    for (ll j = 0; j < len; j++) if (i % primes[j] == 0) y |= (1LL << j);
    DLOGK(i, y);
    for (ll x = 0; x < (1LL << len); x++) {
      if (!(x & y)) cur[x | y] += prev[x];
    }
  }
  ll ans = 0;
  for (ll x = 0; x < (1LL << len); x++) ans += cur[x];
