...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    ...
    ll idx = 0;
    REP(j, 0, ssize(S[i])) {
      idx = trie.child_index(idx, S[i][j]);
      if (idx < 0) {
        break;
      }
      updMin(val, sz - (j + 1) + trie.nodes[idx].user.x);
    }
    cout << val << "\n";
    ...
    idx = 0;
    REP(j, 0, ssize(S[i]) + 1) {
      updMin(trie.nodes[idx].user.x, sz - j);
      if (j == ssize(S[i])) break;
      idx = trie.child_index(idx, S[i][j]);
    }
  }
