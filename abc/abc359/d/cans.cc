...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  string S; cin >> S;
  vector<bool> is_par(1LL << K);
  REP(x, 0, 1LL << K) {
    is_par[x] = FORALL(j, 0, K, (x >> j & 1) == (x >> (K - 1 - j) & 1));
  }
  ll mask = (1LL << K) - 1;
  ...
  REP(i, 0, N) {
    ...
    if (S[i] == '?') {
      REP(x, 0, 1LL << K) {
        ll y = (mask & (x << 1)) | 0;
        if (i < K - 1 or not is_par[y]) tbl[y] += prev[x];
        ll z = (mask & (x << 1)) | 1;
        if (i < K - 1 or not is_par[z]) tbl[z] += prev[x];
      }
    }else if (S[i] == 'A') {
      REP(x, 0, 1LL << K) {
        ll y = (mask & (x << 1)) | 0;
