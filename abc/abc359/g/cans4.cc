...
int main(/* int argc, char *argv[] */) {
  ...
  using um_t = unordered_map<ll, ll>;
  ll ans = 0;
  auto dfs = [&](auto rF, auto& tr, auto& cor) -> void {
    auto [root, _dummy] = tr.centroids();
    tr.change_root(root);
    REP(i, 0, tr.numNodes) {
      if (A[cor[root]] == A[cor[i]]) ans += tr.depth(i);
    }
    ll nc = tr.num_children(root);
    vector sum_dp(nc, um_t());
    um_t tot_sum_dp;
    vector num(nc, um_t());
    um_t tot_num;
    REP(j, 0, nc) {
      auto f = [&](auto rG, ll m) -> void {
        ll col = A[cor[m]];
        sum_dp[j][col] += tr.depth(m);
        tot_sum_dp[col] += tr.depth(m);
        num[j][col]++;
        tot_num[col]++;
        for (ll cld : tr.children(m)) rG(rG, cld);
      };
      f(f, tr.child(root, j));
    };
    REP(j, 0, nc) {
      for (auto [col, v] : sum_dp[j]) {
        ans += v * (tot_num[col] - num[j][col]);
      }
    }
    REP(j, 0, nc) {
      auto [new_tr, new_cor] = tr.subtree_as_obj(tr.child(root, j), cor);
      rF(rF, new_tr, new_cor);
    }
  };
  vector<ll> cor(N);
  iota(ALL(cor), 0);
