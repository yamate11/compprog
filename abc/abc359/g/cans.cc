...
int main(/* int argc, char *argv[] */) {
  ...
  ll L = countr_zero(bit_ceil((u64)(2*N - 1 + 1)));
  vector tbl(L, vector(2*N - 1, 0LL));
  REP(i, 0, 2*N - 1) {
  ...
  REP(k, 1, L) {
    REP(i, 0, 2*N - (1LL << k)) {
      tbl[k][i] = min(tbl[k - 1][i], tbl[k - 1][i + (1LL << (k - 1))]);
    }
    DLOGK(k, tbl[k]);
  }
  ...
  */
  auto case2 = [&](ll x, ll y) -> ll {
    ...
    u64 w = bit_floor(len);
    ll k = countr_zero(w);
    ll z = min(tbl[k][a], tbl[k][b + 1 - w]);
    ll ret = tr.depth(x) - z + tr.depth(y) - z;
    DLOGKL("case2", x, y, a, b, z, ret);
    return ret;
  };
