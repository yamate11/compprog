  ...
  auto dfs = [&](auto rF, ll nd) -> a_t {
    ...
    for (ll cld : tr.children(nd)) {
      ...
      if (sz >= tr.stsize(cld)) {
        for (auto& [g, info] : um) {
          if (ret.contains(g)) {
            auto [v1, dp1, d1, n1] = ret[g];
            auto [v2, dp2, d2, n2] = info;
            ll td1 = (dp1 - tr.depth(nd)) * n1 + d1;
            ll td2 = (dp2 - tr.depth(nd)) * n2 + d2;
            ret[g] = s_t(v1 + v2 + td1 * n2 + n1 * td2, tr.depth(nd), td1 + td2, n1 + n2);
          }else {
            ret[g] = info;
          }
        }
        ...
        for (auto& [g, info] : ret) {
          if (um.contains(g)) {
            auto [v1, dp1, d1, n1] = info;
            auto [v2, dp2, d2, n2] = um[g];
            DLOGKL("part2", v1, dp1, d1, n1, v2, dp2, d2, n2);
            ll td1 = (dp1 - tr.depth(nd)) * n1 + d1;
            ll td2 = (dp2 - tr.depth(nd)) * n2 + d2;
            um[g] = s_t(v1 + v2 + td1 * n2 + n1 * td2, tr.depth(nd), td1 + td2, n1 + n2);
          }else {
            um[g] = info;
          }
        }
