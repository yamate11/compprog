...
int body(istream& cin, ostream& cout) {
  ...
  vector tbl(N + 1, vector(N + 1, vector(2, vector(L + 1, -2.0))));
  auto f = [&](auto rF, ll c0, ll c1, ll c2, ll l) -> double {
    double& r = tbl[c0][c1][c2][l];
    if (r < -1.0) {
      if (l == 0) r = 0.0;
      else if (c2 >= 1) r = 1.0 + rF(rF, c0, c1, c2 - 1, l);
      else if (c0 == 0 and c1 == 0) r = 0.0;
      else {
        ll n = c0 * 2 + c1;
        double e1 = c1 == 0 ? 0.0 : (double)c1 / n * (1.0 + rF(rF, c0, c1 - 1, c2, l));
        auto calc_e2 = [&]() -> double {
          double e21 = (double)1 / (n - 1) * (1.0 + rF(rF, c0 - 1, c1, c2, l));
          double e22 = (double)c1 / (n - 1) * (0.0 + rF(rF, c0 - 1, c1, c2 + 1, l - 1));
          double e23 =
            c0 == 1 ? 0.0
            : (double)((c0 - 1) * 2) / (n - 1) * (0.0 + rF(rF, c0 - 2, c1 + 2, c2, l - 1));
          DLOGKL("  ", e21, e22, e23);
          return e21 + e22 + e23;
        };
        double e2 = c0 == 0 ? 0.0 : (double)(c0 * 2) / n * calc_e2();
        r = e1 + e2;
        DLOGK(c0, c1, c2, l, r, e1, e2);
      }
    }
    return r;
  };
  double exp = f(f, N, 0, 0, L);
  double ans = exp * tot / N;
  cout << ans << "\n";
  return 0;
}
