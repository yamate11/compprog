...
// @@ !! LIM(UnionFind mod forall power)
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    if (uf.leader(i) == i) {
      ll num = uf.group_size(i);
      if (num == 1) continue;
      m++;
      vector cnt(sigma, 0LL);
      for (int j : gi.group(i)) {
        cnt[S[j] - 'a']++;
      }
      Fp div = 1;
      REP(d, 0, sigma) div *= cb.fact(cnt[d]);
      Fp permN = cb.fact(num) / div;
      numP *= permN;
      if (EXISTS(d, 0, sigma, cnt[d] >= 2)) dup = true;
    }
  }
  if (not dup) numP /= 2;
  cout << numP << "\n";
