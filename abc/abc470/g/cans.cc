...
// @DefStruct(DAT, (sum, min, len)) [3fUpUyZg]
struct DAT {
  ll sum;
  ll min;
  ll len;
  string show() const {
    return "(" + to_string(sum) + ", " + to_string(min) + ", " + to_string(len) + ")";
  }
  bool operator==(const DAT&) const = default;
};
// @End [3fUpUyZg]
...
int body(istream& cin, ostream& cout) {
  ...
  auto add = [](DAT x, DAT y) -> DAT { return DAT(x.sum + y.sum, min(x.min, y.min), x.len + y.len); };
  auto comp = [](OP f, OP g) -> OP { return f ? f : g; };
  auto appl = [](OP f, DAT x) -> DAT { return f ? DAT(*f * x.len, *f, x.len) : x; };
  auto dat_init = vector<DAT>();
  REP(i, 0, ssize(init_mex)) dat_init.emplace_back(init_mex[i], init_mex[i], 1);
  auto segt = make_seg_tree_lazy(DAT(0, N + 1, 0), OP(), add, comp, appl, dat_init);

  ll ans = segt.query(0, N).sum;
  DLOGKL("init", ans, segt.vec_view());
  REP(i, 0, N - 1) {
    ll x = A[i];
    auto check = [&](DAT v) -> bool {
      return x < v.min;
    };
    ll j = segt.binsearch_l_until(check, N);
    ll idx = (lower_bound(ALL(pos[x]), i + 1) - pos[x].begin());
    DLOGK(i, j, idx);
    if (idx == ssize(pos[x])) segt.update(j, N, x);
    else if (j < pos[x][idx]) segt.update(j, pos[x][idx], x);
    ll val = segt.query(i + 1, N).sum;
    DLOGK(i, val, segt.vec_view());
    ans += val;
  }
  cout << ans << "\n";
