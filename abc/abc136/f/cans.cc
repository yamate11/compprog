...
// @@ !! LIM(debug segTree mod f:power)
// --> f:<< segTree f:gcd mod f:power debug
...
struct Fp {
  ...
  Fp operator -(const Fp& t) const {
    Fp xx = (Fp(val) -= t);
    DLOG("operator -", "this", *this, "t", t, "res", xx);
    return xx;
  }
  Fp operator *(const Fp& t) const { return Fp(val) *= t; }
  Fp operator /(const Fp& t) const { return Fp(val) /= t; }
...
int main(int argc, char *argv[]) {
  ...
  for (ll i = 0; i < N; i++) {
    ll x, y; cin >> x >> y;
    X.at(i) = make_pair(x, i);
    Y.at(i) = make_pair(y, i);
  }
  sort(X.begin(), X.end());
  sort(Y.begin(), Y.end());
  vector<ll> cX(N), cY(N);
  for (ll i = 0; i < N; i++) {
    cX.at(X.at(i).second) = i;
    cY.at(X.at(i).second) = i;
  }
  ...
  vector<ll> cLD(N), cLU(N), cRD(N), cRU(N);
  for (ll i = 0; i < N; i++) {
    ll y = cY.at(i);
    ll c = st.query(0, y);
    cLD.at(i) = c;
  ...
  for (ll i = 0; i < N; i++) {
    ...
    ll c = st.query(0, y);
    cRD.at(i) = c - cLD.at(i);
    cRU.at(i) = N - 1 - c - cLU.at(i);
  }
  Fp cnt(0);
  auto pwr = [](ll x) -> Fp { return power<Fp>(Fp(2), x); };
  for (ll i = 0; i < N; i++) {
    ll cU = cLU.at(i) + cRU.at(i);
    ll cD = cLD.at(i) + cRD.at(i);
    ll cL = cLU.at(i) + cLD.at(i);
    ll cR = cRU.at(i) + cRD.at(i);
    Fp num = pwr(cU) + pwr(cD) + pwr(cL) + pwr(cR)
      - (pwr(cLU.at(i)) + pwr(cLD.at(i)) + pwr(cRU.at(i)) + pwr(cRD.at(i)));
    cnt += num;
    DLOG("i", i, "cLD", cLD.at(i), "cLU", cLU.at(i), "cRD", cRD.at(i),
	 "cRU", cRU.at(i), "num", num, "cnt", cnt);
  }
  Fp y = pwr(N);
  DLOG("***");
  Fp z = y - cnt;
  DLOG("y", y, "cnt", cnt, "z", z);
  cout << (ll)(z) << endl;
