...
int main(/* int argc, char *argv[] */) {
  ...
  BrdIdx down(1, 0);
  BrdIdx right(0, 1);
  auto dfs = [&](auto rF, BrdIdx bi) -> ll {
    if (res.at(bi) >= 0) return res.at(bi);
    ll v = 0;
    if (brd.at(bi + down) == '.') v = max(v, 1 + rF(rF, bi + down));
    if (brd.at(bi + right) == '.') v = max(v, 1 + rF(rF, bi + right));
    res.at(bi) = v;
    return v;
  };
  ll n = dfs(dfs, BrdIdx(0, 0));
  cout << n + 1 << endl;
