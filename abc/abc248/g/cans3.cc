...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs1 = [&](auto rF, ll nd, ll pt) -> void {
    for (ll cld : nbr[nd]) {
      if (cld == pt) continue;
      rF(rF, cld, nd);
      for (auto [k, v] : rec1[cld]) rec1[nd][gcd(A[nd], k)] += v;
    }
    rec1[nd][A[nd]] += 1;
  };
  dfs1(dfs1, 0, -1);
#if DEBUG  
  REP(i, N) DLOGK(i, rec1[i]);
#endif
  vector<Fp> vans(N);
  auto subt = [&](auto& mp, ll key, Fp d) -> void {
    if (mp[key] == d) mp.erase(key);
    else mp[key] -= d;
  };
  auto dfs2 = [&](auto rF, ll nd, ll pt, const T& mppt) -> void {
    DLOGKL("dfs2", nd, mppt);
    Fp ta = 0;
    T all = rec1[nd];
    subt(all, A[nd], 1);
    if (pt >= 0) {
      for (auto [k, v] : mppt) {
        ll kk = gcd(A[nd], k);
        all[kk] += v;
        for (auto [k2, v2] : rec1[nd]) {
          if (k2 == A[nd]) ta += gcd(kk, k2) * v * (v2 - 1);
          else             ta += gcd(kk, k2) * v * v2;
        }
      }
      DLOGKL("pt positive", nd, ta);
    }
    for (auto [k, v] : all) ta += k * v;
    DLOGK(all, nd, ta);
    T rem = rec1[nd];
    subt(rem, A[nd], 1);
    for (ll cld : nbr[nd]) {
      if (cld == pt) continue;
      T tmp = all;
      for (auto [k, v] : rec1[cld]) {
        subt(tmp, gcd(A[nd], k), v);
        subt(rem, gcd(A[nd], k), v);
      }
      DLOGK(nd, rec1[cld], rem, ta);
      for (auto [k, v] : rec1[cld]) for (auto [k2, v2] : rem) ta += gcd(k, k2) * v * v2;
      DLOGK(nd, ta);
      tmp[A[nd]] += 1;
      rF(rF, cld, nd, tmp);
    }
