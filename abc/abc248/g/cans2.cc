...
int main(/* int argc, char *argv[] */) {
  ...
  auto add = [&](T& t1, T& t2) -> T {
    for (auto [k, v] : t2) t1[k] += v;
    return t1;
  };
  auto mod = [&](T t, int node, int child) -> T {
    T ret;
    for (auto [k, v] : t) ret[gcd(k, A[node])] += v;
    ret[gcd(A[child], A[node])] = 1;
    return ret;
  };
  vector<T> result = reroot(tr, unitT, add, mod);
#if DEBUG
  REP(i, N) {
    DLOGK(i, result[i]);
  }
  ...
  REP(node, N) {
    ll thisa = 0;
    for (auto [k, v] : result[node]) thisa += k * v;
    const auto& cld = tr.child(node);
    ll sz = SIZE(cld);
    REP(i, sz) REP2(j, i + 1, sz) {
      for (auto [k1, v1] : result[cld[i]]) {
        for (auto [k2, v2] : result[cld[j]]) {
          ll g = gcd(k1, k2);
          thisa += g * v1 * v2;
        }
        if (node != 0) {
          ll par = tr.parent(node);
          for (auto [k2, v2] : result[par]) {
            ll g = gcd(k1, k2);
            thisa += g * v1 * v2;
          }
        }
