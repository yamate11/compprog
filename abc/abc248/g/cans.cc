...
// @@ !! LIM(mod debug sieve f:perfmeas)
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<vector<ll>> nodes(lim + 1);
  vector<ll> A(N);
  REP(i, N) {
    cin >> A[i];
    for (ll d : getDivisors(A[i], primes)) nodes[d].push_back(i);
  }
  REP(i, N - 1) {
    ll u, v; cin >> u >> v; u--; v--;
    nbr[u].push_back(v);
  ...
  vector<ll> subsize(N);
  auto f = [&](ll d) -> Fp {
    if (SIZE(nodes[d]) <= 1) return 0;
    for (ll i : nodes[d]) toCheck[i] = true;
    auto dfs = [&](auto rF, ll nd, ll pt) -> void {
      toCheck[nd] = false;
      ll cnt = 1;
      for (ll cld : nbr[nd]) {
        if (cld == pt or A[cld] % d != 0) continue;
        rF(rF, cld, nd);
        cnt += subsize[cld];
      }
      subsize[nd] = cnt;
    ...
    Fp ret = 0;
    for (ll i : nodes[d]) {
      if (not toCheck[i]) continue;
      dfs(dfs, i, -1);
      ll tot = subsize[i];
      auto dfs2 = [&](auto rF, ll nd, ll pt) -> void {
        if (pt >= 0) ret += subsize[nd] * (tot - subsize[nd]);
        for (ll cld : nbr[nd]) {
          if (cld == pt or A[cld] % d != 0) continue;
          rF(rF, cld, nd);
        }
  ...
  REP2R(p, lim, 1) {
    Fp t = f(p);
    for (ll q = 2 * p; q <= lim; q += p) t -= D[q];
    D[p] = t;
  }
#if DEBUG
  REP2(p, 1, lim + 1) if (D[p] != 0) DLOGK(p, D[p]);
#endif
  Fp ans = 0;
  REP2(i, 1, lim + 1) ans += i * D[i];
