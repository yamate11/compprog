...
int main(/* int argc, char *argv[] */) {
  ...
  const ll SOFT = 1;
  const ll YET = 2;
  vector tbl_init(3, vector(N, Fp(0)));
  auto tbl = tbl_init;
  tbl[YET][0] = 1;
  REP(i, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(k, N) {
      tbl[SOFT][k] += prev[HARD][k];
      if (k + 1 < N) tbl[HARD][k + 1] += prev[HARD][k];
      tbl[SOFT][k] += prev[SOFT][k];
      if (k + 1 < N) tbl[SOFT][k + 1] += prev[SOFT][k];
      tbl[SOFT][k] += prev[YET][k];
      if (k + 1 < N) tbl[YET][k + 1] += prev[YET][k];
    }
    DLOGKL("fst", i);
    REP(k, N) DLOGK(k, tbl[SOFT][k]);
    REP(k, N) DLOGK(k, tbl[HARD][k]);
    REP(k, N) DLOGK(k, tbl[YET][k]);
    if (i == N - 1) break;
    prev = move(tbl);
    tbl = tbl_init;
    REP(k, N) {
      tbl[HARD][k] += prev[HARD][k];
      tbl[SOFT][k] += prev[SOFT][k];
      tbl[YET][k] += prev[YET][k];
      if (k + 1 < N) tbl[HARD][k + 1] += 2 * prev[SOFT][k];
    }
    DLOGKL("snd", i);
    REP(k, N) DLOGK(k, tbl[SOFT][k]);
    REP(k, N) DLOGK(k, tbl[HARD][k]);
