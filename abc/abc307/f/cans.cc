...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, nbr, dec=1, read=W) [wBwYpo1H]
  struct nbr_t {
    int nd;
    ll W;
    nbr_t() {}
  ...
  // @InpVec(D, X) [qxYexcBs]
  auto X = vector(D, ll());
  for (int i = 0; i < D; i++) { ll v; cin >> v; X[i] = v; }
  // @End [qxYexcBs]

  auto mymax = [&](ll a, ll b) -> ll { return max(a, b); };
  auto st = make_seg_tree<ll>(-1, mymax, X);

  auto func = [&](ll w, ll d) -> ll {
    auto check = [&](ll x) -> bool { return x < w; };
    ll p = st.binsearch_r_until(check, d);
    return p;
  };

  vector ans(N, -1LL);
  vector cand(D + 1, vector<pll>());
  REP(i, 0, K) {
    ans[A[i]] = 0;
    for (auto [peer, w] : nbr[A[i]]) {
      ll e = func(w, 0);
      cand[e].emplace_back(peer, w);
    }
  }
  ll big = 1e18;
  ...
  REP(d, 0, D) {
    ...
    for (auto [nd, len] : cand[d]) {
      if (ans[nd] >= 0) continue;
      if (len >= dist[nd]) continue;
      dist[nd] = len;
      mod_dist.push_back(nd);
      pque.emplace(len, nd);
    }
    while (not pque.empty()) {
      ...
      for (auto [peer, w] : nbr[nd]) {
        if (ans[peer] >= 0) continue;
        ll newD = dist[nd] + w;
        if (newD >= dist[peer]) continue;
        if (newD <= X[d]) {
          dist[peer] = newD;
          mod_dist.push_back(peer);
    ...
    for (ll nd : mod_dist) dist[nd] = big;
  }
  REPOUT(i, 0, N, ans[i], "\n");
