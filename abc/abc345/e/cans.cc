...
// @DefStruct(T, (max1, max2, col1)) [DX2Vr69A]
struct T_t {
  ll max1;
  ll max2;
  ll col1;
  // T_t() {}
  T_t(ll max1_ = ll(), ll max2_ = ll(), ll col1_ = ll()) : max1(max1_), max2(max2_), col1(col1_) {}
  friend istream& operator>>(istream& istr, T_t& t) {
    istr >> t.max1 >> t.max2 >> t.col1;
    return istr;
  }
  friend ostream& operator<<(ostream& ostr, const T_t& t) {
    ostr << "(" << t.max1 << ", " << t.max2 << ", " << t.col1 << ")";
    return ostr;
  }
};
// @End [DX2Vr69A]
...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl(K + 1, T_t(-big, -big, -big));
  tbl[0] = T_t(0, 0, 0);
  REP(i, 0, N) {
    REPrev(k, K, 0) {
      ll v = (tbl[k].col1 == C[i] ? tbl[k].max2 : tbl[k].max1) + V[i];
      if (k == 0) {
        tbl[k] = T_t(v, -big, C[i]);
      }else {
        if (v < 0) {
          ...
          auto [w1, w2, c] = tbl[k - 1];
          if (w1 < 0) {
            tbl[k] = T_t(v, -big, C[i]);
          }else if (c == C[i]) {
            tbl[k] = T_t(max(v, w1), w2, C[i]);
          }else if (v >= w1) {
            tbl[k] = T_t(v, w1, C[i]);
          }else {
            tbl[k] = T_t(w1, max(v, w2), c);
          }
        }
      }
    }
