...
using mymap = unordered_map<ll, ll>;

int main(){
  int n,k;
  cin >> n >> k;

  auto enc = [&](ll cnt, ll col) -> ll { return cnt * (n + 10) + col; };
  auto dec = [&](ll e) -> S { return S(e / (n + 10), e % (n + 10)); };
  ...
  rep(i,n){
    ...
    mp.clear();
    for(auto& [key,val] : dp){
      auto [cnt, last] = dec(key);
      auto chmax = [](auto &a, auto b)
      { a = max(a, b); };
      if(cnt+1 <= k){
        chmax(mp[enc(cnt + 1, last)], val);
      }
      if(last != c){
        chmax(mp[enc(cnt, c)], val + v);
      }
    }
