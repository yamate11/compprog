...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> brd(N, M, '#');
  cin >> brd;
  vector<Board<bool>> visited(4, Board<bool>(N, M, false));
  using sta = pair<ll, BrdIdx>;
  vector<sta> stack;
  REP(d, 0, 4) {
    visited[d].at(1, 1) = true;
    stack.emplace_back(d, BrdIdx(1, 1));
  }
  while (not stack.empty()) {
    auto [dir, bi] = stack.back(); stack.pop_back();
    auto new_bi = bi + BrdIdx::nbr4[dir];
    if (brd.at(new_bi) == '#') {
      REP(d, 0, 4) {
        if (d == dir) continue;
        if (visited[d].at(bi)) continue;
        visited[d].at(bi) = true;
        stack.emplace_back(d, bi);
      }
    }else if (visited[dir].at(new_bi)) {
      // do nothing
    }else {
      visited[dir].at(new_bi) = true;
      stack.emplace_back(dir, new_bi);
    }
  }
  ll ans = 0;
  REP(i, 0, N) REP(j, 0, M) {
    if (EXISTS(d, 0, 4, visited[d].at(i, j))) ans++;
  }
  cout << ans << endl;
