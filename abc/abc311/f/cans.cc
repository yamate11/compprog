...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpVec(N, S, type=string) [5nC1zgvS]
  auto S = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; S[i] = v; }
  // @End [5nC1zgvS]

  vector top(M, N);
  REP(i, 0, N) REP(j, 0, M) if (S[i][j] == '#') updMin(top[j] , i);

  vector tbl(N + 1, vector(M, Fp(0)));
  vector acc(N + 2, vector(M, Fp(0)));
  REP(i, 0, top[M - 1] + 1) tbl[i][M - 1] = Fp(1);
  acc[0][M - 1] = 0;
  REP(i, 0, N + 1) acc[i + 1][M - 1] = acc[i][M - 1] + tbl[i][M - 1];
  REPrev(m, M - 2, 0) {
    REP(i, 0, top[m] + 1) {
      if (i == N) tbl[i][m] = acc[N + 1][m + 1];
      else tbl[i][m] = acc[i + 2][m + 1];
    }
    REP(i, 0, N + 1) acc[i + 1][m] = acc[i][m] + tbl[i][m];
  }
  Fp ans = 0;
  REP(i, 0, N + 1) ans += tbl[i][0];
  cout << ans << endl;
