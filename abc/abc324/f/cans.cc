...
// @@ !! LIM(input binsearch f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, fwd, dir=True, dec=1, read=(B, C)) [EqsaFX8O]
  struct fwd_t {
    int nd;
    ll B;
    ll C;
    fwd_t() {}
    fwd_t(int nd_, ll B_, ll C_) : nd(nd_), B(B_), C(C_) {}
  };
  ...
  auto check = [&](double a0) -> bool {
    DLOGKL("*** ", a0);
    auto vec = vector(N, -1e18);
    vec[0] = 0.0;
    REP(i, 0, N) {
      if (vec[i] < -1e17) continue;
      for (auto [j, b, c] : fwd[i]) {
        updMax(vec[j], vec[i] + b - c * a0);
      }
      DLOGK(i, vec);
    }
    return vec[N - 1] >= 0;
  };
  double ans = binsearch_r<double>(check, 0, 1e18, 1e-11, false);
  cout << ans << endl;
