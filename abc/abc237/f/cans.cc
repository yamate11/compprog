...
struct BwAO { // body with AO
  ...
  bool operator !=(const BwAO& o) { return v != o.v; }
  BwAO zero() const { return BwAO(::zero<T>(v)); }
  BwAO one() const { return BwAO(::one<T>(v)); }
  BwAO& operator +=(const BwAO& o) { AO().subst_add(v, o.v); return *this; }
  BwAO& operator -=(const BwAO& o) { AO().subst_subt(v, o.v); return *this; }
  BwAO& operator *=(const BwAO& o) { AO().subst_mult(v, o.v); return *this; }
  BwAO operator -() const { BwAO ret = zero(); return ret -= *this; }
  BwAO inverse() const { return BwAO(AO().inverse(v)); }
  BwAO& operator /=(const BwAO& o) { AO().subst_mult(v, AO().inverse(o.v)); return *this; }
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, M) op(1LL << i);
  REP(i, M) REP2(j, i+1, M) op((1LL << i) | (1LL << j));
  REP(i, M) REP2(j, i+1, M) REP2(k, j+1, M) op((1LL << i) | (1LL << j) | (1LL << k));
  ll K = SIZE(pick);

  vector<Fp> tbl_init(K);
  auto tbl = tbl_init;
  tbl[0] = 1;
  REP(i, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(k, K) {
      if (prev[k] == Fp(0)) continue;
      ll x = pick[k];
      REP(t, M) {
        if (x >> t & 1) { tbl[k] += prev[k]; }
        else {
          ll s = t + 1;
          for (; s < M; s++) if (x >> s & 1) break;
          if (s == M) {
            ll popc = __builtin_popcountll(x);
            if (popc < 3) {
              ll y = x | (1LL << t);
              tbl[idx[y]] += prev[k];
            }
  ...
  Fp ans = 0;
  REP(k, K) {
    if (__builtin_popcountll(pick[k]) == 3) ans += tbl[k];
  }
  cout << ans << endl;
