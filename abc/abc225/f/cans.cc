...
int body(istream& cin, ostream& cout) {
  ...
  vector<string> S(N); REP(i, N) cin >> S[i];
  sort(ALL(S));
  using tpa = tuple<ll, ll, ll>; // idx, pos, finished_set
  vector<tpa> cands_init;
  ...
  [&]() {
    while (true) {
      ...
      if (first) {
        ...
        for (auto [idx, pos, fs] : prev) {
          if (pos < SIZE(S[idx]))  c = min(c, S[idx][pos]);
          else {
            ll pc = __builtin_popcountll(fs) + 1;
            if (pc == K) return;
            if (pc > max_pc) {
              max_pc = pc;
              max_fs = fs | (1LL << idx);
      ...
      if (max_pc >= 0) REP(i, N) if (not ((max_fs >> i) & 1)) c = min(c, S[i][0]);
      cout << c;
      for (auto [idx, pos, fs] : prev) if (pos < SIZE(S[idx]) && S[idx][pos] == c) cands.emplace_back(idx, pos + 1, fs);
      if (max_pc >= 0) REP(i, N) if (not ((max_fs >> i) & 1) and c == S[i][0]) cands.emplace_back(i, 1, max_fs);
    }
  }();
  cout << endl;
