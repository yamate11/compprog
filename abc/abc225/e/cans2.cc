...
// @@ !! LIM(ipoint binsearch debug cmpNaive)
...
bool small(const IPoint& p, const IPoint& q) {
  return p.x * q.y > p.y * q.x;
}

bool smallEq(const IPoint& p, const IPoint& q) {
  return p.x * q.y >= p.y * q.x;
}
...
IPoint beg(const IPoint& p) { return p - unity; }
IPoint en(const IPoint& p) { return p - unitx; }
...
int naive(istream& cin, ostream& cout) {
  ...
  auto check = [&](ll x) -> bool {
    REP(i, N) REP2(j, i + 1, N) {
      ...
      if (smallEq(P[i] - unitx, P[j] - unity) or smallEq(P[j] - unitx, P[i] - unity)) {
      }else {
        return false;
      }
    }
  ...
  ll ans = 0;
  REP(x, 1LL << N) {
    if (check(x)) ans = max(ans, (ll)(__builtin_popcountll(x)));
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, N) {
    ...
    auto check = [&](ll j0) -> bool {
      ll k = E[j0];
      bool b = smallEq(en(P[k]), beg(P[E[i]]));
      DLOGK(j0, k, P[k], beg(P[E[i]]), b);
      return b;
    };
    ll j = binsearch_i<ll>(check, -1, i);
    ll val = 0;
    if (j == -1) val = 0;
    else val = tbl[j];
    tbl[i] = max(val + 1, tbl[i - 1]);
    DLOGK(i, j);
  }
  cout << tbl[N - 1] << endl;
