...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, D, R; cin >> N >> D >> R;
  // @InpVec(N, H, dec=1) [Vg9MjbNh]
  auto H = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; H[i] = v; }
  // @End [Vg9MjbNh]

  vector<ll> revH(N);
  REP(i, 0, N) revH[H[i]] = i;

  auto mymax = [&](ll a, ll b) -> ll { return max(a, b); };
  auto st = make_seg_tree(0LL, mymax, vector<ll>(N));
  ...
  REP(i, 0, N) {
    ...
    if (i + D < N) {
      ll k = revH[i + D];
      ll x = st.query(max(0LL, k - R), min(N, k + R + 1));
      rec[i + D] = x + 1;
    }
    DLOGK(i, st);
    DLOGK(i, rec);
