...
int main(/* int argc, char *argv[] */) {
  ...
  REP(_q, 0, Q) {
    ...
    B--; C--;
    if (A == 1) {
      if (uf.group_size(B) < uf.group_size(C)) swap(B, C);
      auto rev = [&](auto rF, ll p) -> void {
        if (parent[p] == p) return;
        ll q = parent[p];
        rF(rF, q);
        parent[q] = p;
      };
      ...
      ll ans;
      if (B == C or parent[B] == C or parent[C] == B) ans = 0;
      else if (parent[B] == parent[C]) ans = parent[B] + 1;
      else if (parent[parent[B]] == C) ans = parent[B] + 1;
      else if (parent[parent[C]] == B) ans = parent[C] + 1;
      else ans = 0;
      cout << ans << "\n";
      xx = ans;
    }else assert(0);
