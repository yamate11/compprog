...
tuple<ll, ll, ll> mut_div_old(ll a, ll b, ll c, bool eff_c = true) {
  ...
  if (a == 0) {
    ...
    auto [g, t, u] = mut_div_old(b % a, a, c, eff_c);
    // DLOGK(b%a, a, c, g, t, u);
    if (g == -1) return {-1, -1, -1};
    return {g, u - (b / a) * t, t};
  }
...
tuple<ll, ll, ll> mut_div(ll a, ll b) {
  // auto [g, s, t] = mut_div(a, b)
  //        g == gcd(|a|, |b|) and as + bt == g           
  // a and b must be less than 2^31.
  array<ll, 100> vec;  // In fact, 50 is sufficient for a,b < 2^31.
  ll idx = 0;
  while (a != 0) {
    ll x = b / a;
    ll y = b % a;
...
tuple<ll, ll, ll> eGCD_old(ll a, ll b) { return mut_div_old(a, b, 0, false); }

pair<ll, ll> crt_sub(ll a1, ll x1, ll a2, ll x2) {
  // DLOGKL("crt_sub", a1, x1, a2, x2);
  a1 = a1 % x1;
  a2 = a2 % x2;
  auto [g, s, t] = mut_div_old(x1, -x2, a2 - a1);
  // DLOGK(g, s, t);
  if (g == -1) return {-1, -1};
  ll z = x1 / g * x2;
  // DLOGK(z);
...
int main(/* int argc, char *argv[] */) {
  ...
  ll rep = 1 << 16;
  u64 a0 = 0x890234979801;
  u64 b0 = 0x434908024129;
  u64 c0 = 0xe823df405349;
  u64 d0 = 0xf3423450abc7;
  u64 a, b;
