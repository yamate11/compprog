...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  auto func = [&](auto& AA) {
    vector tbl(N, vector(N, vector<ll>()));
    tbl[0][0] = {AA[0][0]};
    REP(k, 0, N - 1) {
      REP(r, 0, k + 1) {
        ll c = k - r;
        for (ll x : tbl[r][c]) {
          tbl[r][c + 1].push_back(x ^ AA[r][c + 1]);
          tbl[r + 1][c].push_back(x ^ AA[r + 1][c]);
        }
      }
    }
    DLOGKL("func", tbl);
  ...
  REP(i, 0, N) {
    ...
    for (ll x : tbl1[i][j]) { mp[x]++; }
    for (ll y : tbl2[i][j]) {
      ans += mapget(mp, y ^ A[i][j], 0);
    }
  }
  cout << ans << endl;
