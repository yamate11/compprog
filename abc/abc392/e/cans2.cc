...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  UnionFind uf(N);
  // @InpMVec(M, ((A, dec=1), (B, dec=1))) [v8ms8fqH]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  vector<ll> leaders;
  REP(i, 0, N) if (uf.leader(i) == i) leaders.push_back(i);
  ll i0 = leaders[0];
  vector<ll> eld(M);
  vector lde(N, vector<ll>());
  for (ll t : rem) {
    ll i = uf.leader(A[t]);
    eld[t] = i;
    lde[i].push_back(t);
  }
  using st_t = tuple<ll, ll, ll>;
  vector<st_t> ans;
  ...
  for (ll j0 : leaders) {
    ...
    for (ll t : lde[j0]) {
      if (not connected[j0]) {
        DLOGKL("A", t, j0);
        ans.emplace_back(t, A[t], i0);
        connected[j0] = true;
      }
      else {
        while (idx < N and (uf.leader(idx) != idx or connected[idx])) idx++;
        DLOGKL("B", t, j0, idx);
        if (idx == N) break;
        ans.emplace_back(t, A[t], idx);
        connected[idx] = true;
      }
    }
    if (idx == N) break;
