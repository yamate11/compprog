...
int main(/* int argc, char *argv[] */) {
  ...
  for (ll t : rem) vec[uf.leader(A[t])].push_back(t);
  vector<pll> nl;
  REP(i, 0, N) {
    if (uf.leader(i) == i) nl.emplace_back(ssize(vec[i]), i);
  }
  sort(ALL(nl), greater<pll>());
  vector<pll> ans;
  vector<ll> ww;
  bool first = true;
  REP(i, 0, ssize(nl)) {
    auto [_n, idx] = nl[i];
    if (i > 0) {
      ll t = ww.back(); ww.pop_back();
      ans.emplace_back(t, idx);
    }
    for (ll t : vec[idx]) ww.push_back(t);
  }
  cout << ssize(ans) << endl;
  for (auto [t, i] : ans) {
    cout << t + 1 << " " << A[t] + 1 << " " << i + 1 << "\n";
