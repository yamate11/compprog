...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) init_vec[i] = i;
  auto st = make_seg_tree_lazy(big, 0, mymin, plus<ll>(), plus<ll>(), init_vec);
  
  vector<ll> ans(N);
  REPrev(i, N - 1, 0) {
    auto check = [&](ll t) -> bool { return t <= P[i]; };
    ll x = st.binsearch_l_from(check, N);
    ans[x] = i;
    st.update(x + 1, N, -1LL);
  }
