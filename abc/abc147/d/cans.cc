...
int main(int argc, char *argv[]) {
  ...
  Fp ans = 0;
  vector<vector<ll>> cntO(61, vector<ll>(N+1));
  for (int b = 0; b < 60; b++) {
    cntO.at(b).at(0) = 0;
    for (int i = 0; i < N; i++) {
      if ((A.at(i) >> b) & 1LL) cntO.at(b).at(i+1) = cntO.at(b).at(i) + 1;
      else                      cntO.at(b).at(i+1) = cntO.at(b).at(i);
    }
    Fp s = 0;
    for (int i = 1; i < N; i++) {
      ll t = 0;
      if ((A.at(i) >> b) & 1LL) t = i - cntO.at(b).at(i);
      else                      t = cntO.at(b).at(i);
      s += t;
    }
    ans = ans + s * Fp(1LL << b);
  }
