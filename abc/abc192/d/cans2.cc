...
int main(/* int argc, char *argv[] */) {
  ...
  ll D = 0;
  for (ll i = 0; i < sz; i++) D = max(D, vec[i]);
  D++;
  auto f_d = [&](double n) -> double {
    double ans = 0.0;
    for (ll i = 0; i < sz; i++) {
      ans += vec[i] * pow(n, i);
    }
  ...
  auto check_d = [&](double n) -> bool {
    return f_d(n) <= M;
  };
  double t = binsearch<double>(check_d, 0, M, 0.001);
  DLOGK(t);
  double d = abs(round(t) - t);
  ll x;
  if (d > 0.01) {
    x = llround(floor(t));
  }else {
