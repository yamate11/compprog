...
// @@ !! LIM(f:intDiv coordCompr binsearch debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpVec(N, S, type=string) [pZd4DRqM]
  auto S = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; S[i] = v; }
  // @End [pZd4DRqM]

  auto pos = vector(10, vector(N, vector<ll>()));
  REP(i, 0, N) REP(j, 0, M) pos[S[i][j] - '0'][i].push_back(j);

  auto func = [&](ll d) -> ll {
    DLOGKL("func", d);
    auto W = vector(N, vector<ll>());
    REP(i, 0, N) {
      ll k = SIZE(pos[d][i]);
      if (k == 0) return big;
      ll cnt = 0;
      REP(p, 0, divCeil(N, k)) REP(q, 0, k) {
        W[i].push_back(p * M + pos[d][i][q]);
        cnt++;
        if (cnt == N) break;
      }
      DLOGK(d, i, W[i]);
    ...
    CoordCompr cc;
    REP(i, 0, N) REP(j, 0, N) cc.add(W[i][j]);
    ll sz = cc.size();
    DLOGK(cc.rev);
    auto check = [&](ll t) -> bool {
      mf_graph<ll> graph(2 + N + t);
      const ll source = 0;
      const ll sink = 1;
      auto reel = [&](ll i) -> ll { return 2 + i; };
      auto tsec = [&](ll i) -> ll { return 2 + N + i; };
      REP(i, 0, N) graph.add_edge(source, reel(i), 1);
      REP(i, 0, t) graph.add_edge(tsec(i), sink, 1);
      REP(i, 0, N) {
        REP(j, 0, N) {
          ll z = cc.c(W[i][j]);
          if (z < t) graph.add_edge(reel(i), tsec(z), 1);
        }
      }
    ...
    ll t = binsearch_i<ll>(check, sz, N - 1);
    ll ret = cc.d(t - 1);
    DLOGK(d, ret);
    return ret;
  };
