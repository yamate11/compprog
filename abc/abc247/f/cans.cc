...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, N) cin >> P[i];
  REP(i, N) cin >> Q[i];
  REP(i, N) PQ[i] = sta(P[i] - 1, Q[i] - 1);
  sort(ALL(PQ));
  vector<bool> hasCache(N + 1);
  vector<Fp> cache(N + 1);
  hasCache[1] = true;
  cache[1] = Fp(1);
  auto func = [&](ll m) -> Fp {
    if (hasCache[m]) return cache[m];
    Fp ret = 0;
    vector tbl_init(2, vector(m + 1, Fp(0)));
    auto tbl = tbl_init;
    tbl[0][0] = 1;
    REP2(i, 1, m) {
      tbl[0][i] = tbl[1][i - 1];
      tbl[1][i] = tbl[0][i - 1] + tbl[1][i - 1];
    }
    ret = tbl[1][m - 1];
    tbl = tbl_init;
    tbl[1][0] = 1;
