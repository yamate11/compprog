...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<Fp> L(N);
    vector<Fp> R(N);
    safe_umap<ll, ll> mpL, mpR;
    L[0] = 1;
    REP(i, 1, N) {
      L[i] = L[i - 1];
      for (auto [p, r] : prfac(A[i - 1], primes)) {
        ll r0 = mpL[p];
        if (r > r0) {
          REP(j, 0, r - r0) L[i] *= p;
          change[i].emplace_back(p, r0);
          mpL[p] = r;
        }
      }
    }
    ...
    REPrev(i, N - 2, 0) {
      ...
      for (auto [q, s] : change[i + 1]) {
        mpL[q] = s;
        ch.push_back(q);
      }
      ranges::sort(ch);
      auto itt = unique(ALL(ch));
      ch.erase(itt, ch.end());
      ans[i] = ans[i + 1];
      for (ll p : ch) {
        ll new_b0 = max(mpL[p], mpR[p]);
        ll b0 = mpB[p];
        if (new_b0 > b0) {
          REP(j, 0, new_b0 - b0) ans[i] *= p;
        }else if (new_b0 < b0) {
          REP(j, 0, b0 - new_b0) ans[i] /= p;
        }
        mpB[p] = new_b0;
      }
      DLOGK(i, mpR, mpL, mpB);
