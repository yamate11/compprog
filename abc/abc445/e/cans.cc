...
// @@ !! LIM(sieve mod unordered_map debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto divSieve = divisorSieve((ll)1e7 + 10);
  ...
  auto solve = [&]() -> void {
    ...
    safe_umap<ll, ll> max1, max2;
    safe_umap<ll, Fp> invP;

    REP(i, 0, N) {
      for (auto [p, r] : prfacDivSieve(A[i], divSieve)) {
        auto [it1, b1] = max1.try_emplace(p, r);
        if (not b1) {
          if (it1->second < r) {
            ll tmp = r;
            r = it1->second;
          ...
          auto [it2, b2] = max2.try_emplace(p, r);
          if (not b2) {
            if (it2->second < r) {
              it2->second = r;
            }
    ...
    Fp x = 1;
    for (auto [p, r] : max1) REP(j, 0, r) x *= p;
    REP(i, 0, N) {
      Fp y = x;
      for (auto [p, r] : prfacDivSieve(A[i], divSieve)) {
        if (r == max1[p]) REP(j, 0, r - max2[p]) y *= invP[p];
      }
      cout << y << "\n";
    }
