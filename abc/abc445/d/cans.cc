...
int main(/* int argc, char *argv[] */) {
  ...
  safe_umap<ll, vector<ll>> mpX, mpY;
  REP(i, 0, N) {
    mpX[X[i]].push_back(i);
    mpY[Y[i]].push_back(i);
  }
  ...
  auto myget = [&](ll z, auto& mp) -> vector<ll> {
    auto it = mp.find(z);
    if (it == mp.end()) return vector<ll>();
    vector<ll> ret;
    for (ll i : it->second) {
      if (rem[i]) {
        ret.push_back(i);
    ...
    if (not vecX.empty()) {
      ...
      for (ll i: vecX) {
        if (rem[i]) {
          ...
          y0 -= Y[i];
          cnt++;
          DLOGKL("x", i, X[i], Y[i], y0, cnt);
        }
      }
    }else {
      auto vecY = myget(y0, mpY);
      assert(not vecY.empty());
      DLOGK(y0, vecY);
      for (ll i : vecY) {
        if (rem[i]) {
          ord.push_back(i);
