...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> double {
    ...
    Point st, gt, sa, ga; cin >> st >> gt >> sa >> ga;
    Point dt = gt - st;
    Point da = ga - sa;
    Real lent = dt.len();
    Real lena = da.len();
    if (lent > lena) { swap(st, sa); swap(gt, ga); swap(dt, da); swap(lent, lena); }
    Point vt = dt / lent;
    Point va = da / lena;
    auto func2t = [&](double t) -> Point {
      if (t > lent) return gt;
      else return st + vt * t;
    };
    auto func2a = [&](double t) -> Point { return sa + va * t; };
    auto func = [&](double t) -> double {
      return (func2t(t) - func2a(t)).len();
    };
    Real t0 = 0, t1 = lent;
    REP(rep, 0, 60) {
      if (func(t0) < func(t1)) t1 = t0 + 2.0 / 3.0 * (t1 - t0);
      else                     t0 = t0 + 1.0 / 3.0 * (t1 - t0);
    }
    Real t2 = lent, t3 = lena;
    REP(rep, 0, 60) {
      if (func(t2) < func(t3)) t3 = t2 + 2.0 / 3.0 * (t3 - t2);
      else                     t2 = t2 + 1.0 / 3.0 * (t3 - t2);
    }
    return min(func(t0), func(t2));
  };
