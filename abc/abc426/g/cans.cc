...
// @@ !! LIM(unordered_map f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [DyR4JpvX]
  ll Q; cin >> Q;
  // @InpMVec(Q, ((L, dec=1), (R, dec=0), C)) [7Q4UP7fe]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  auto C = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  ll maxC = *ranges::max_element(C);

  vector<ll> ans(Q, 0LL);
  auto f = [&](auto rF, ll lo, ll hi, const auto& qs) -> void {
    if (lo + 1 == hi) {
      for (ll q : qs) ans[q] = W[lo] <= C[q] ? V[lo] : 0;
      return;
    }
    ll mid = (lo + hi) / 2;
    vector<ll> qsLo, qsHi, qsCur;
    for (ll q : qs) {
      if (R[q] <= mid) qsLo.push_back(q);
      else if (mid <= L[q]) qsHi.push_back(q);
      else qsCur.push_back(q);
    }
    if (not qsLo.empty()) rF(rF, lo, mid, qsLo);
    if (not qsHi.empty()) rF(rF, mid, hi, qsHi);
    if (not qsCur.empty()) {
      auto g = [&](ll st, ll len, ll d) -> vector<vector<ll>> {
        vector tbl(len + 1, vector(maxC + 1, 0LL));
        REP(i, 0, len) {
          ll w = W[st + i * d];
          ll v = V[st + i * d];
          REP(j, 0, maxC + 1) {
            if (j + w <= maxC) updMax(tbl[i + 1][j + w], tbl[i][j] + v);
            updMax(tbl[i + 1][j], tbl[i][j]);
          }
          REP(j, 1, maxC + 1) updMax(tbl[i + 1][j], tbl[i + 1][j - 1]);
        }
        return tbl;
      };
      auto vecHi = g(mid, hi - mid, +1);
      auto vecLo = g(mid - 1, mid - lo, -1);
      for (ll q : qsCur) {
        ll ridx = R[q] - mid;
        ll lidx = mid - L[q];
        REP(j, 0, C[q] + 1) updMax(ans[q], vecLo[lidx][j] + vecHi[ridx][C[q] - j]);
      }
    }
  };
  vector<ll> qs0(Q);
