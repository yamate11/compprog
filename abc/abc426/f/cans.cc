...
// @DefStruct(dat, (min, min_idx, sum, len)) [85dPgVyZ]
struct dat_t {
  ll min;
  ll min_idx;
  ll sum;
  ll len;
  dat_t(ll min_ = ll(), ll min_idx_ = ll(), ll sum_ = ll(), ll len_ = ll()) : min(min_), min_idx(min_idx_), sum(sum_), len(len_) {}
  friend istream& operator>>(istream& istr, dat_t& t) {
    istr >> t.min >> t.min_idx >> t.sum >> t.len;
    return istr;
  }
  friend ostream& operator<<(ostream& ostr, const dat_t& t) {
    ostr << "(" << t.min << ", " << t.min_idx << ", " << t.sum << ", " << t.len << ")";
    return ostr;
  }
};
// @End [85dPgVyZ]
...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1e18;
  using OP = ll;
  dat_t dat_t_unit(big, -1, 0, 0);
  auto add = [](dat_t a, dat_t b) -> dat_t {
    if (a.min > b.min) return dat_t(b.min, b.min_idx, a.sum + b.sum, a.len + b.len);
    else               return dat_t(a.min, a.min_idx, a.sum + b.sum, a.len + b.len);
  };
  auto comp = [](OP f, OP g) -> OP { return f + g; };
  auto appl = [](OP f, dat_t x) -> dat_t { return dat_t(x.min + f, x.min_idx, x.sum + f * x.len, x.len); };
  auto dat_init = vector<dat_t>(N);
  REP(i, 0, N) dat_init[i] = dat_t(A[i], i, A[i], 1);
  auto st = make_seg_tree_lazy(dat_t_unit, OP(), add, comp, appl, dat_init);
  
  ll Q; cin >> Q;
  REP(_q, 0, Q) {
    ll l, r, k; cin >> l >> r >> k; l--;
    auto v0 = st.query(l, r);
    ll ans = v0.len * k;
    st.update(l, r, -k);
    while (true) {
      auto v = st.query(l, r);
      if (st.at(v.min_idx).min > 0) break;
      ans += v.min;
      st.rs(v.min_idx) = dat_t(big, v.min_idx, 0, 0);
    }
    cout << ans << "\n";
  }
