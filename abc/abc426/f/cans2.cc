...
int main(/* int argc, char *argv[] */) {
  ...
  ll Q; cin >> Q;
  // @InpMVec(Q, ((L, dec=1), (R, dec=0), K)) [hpSzMIxm]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  auto K = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  vector exh(Q + 1, vector<ll>());
  vector<ll> exh_val(Q + 1);
  vector<ll> pts0(N);
  vector<ll> vals0(N);
  REP(i, 0, N) pts0[i] = i;
  REP(i, 0, N) vals0[i] = A[i];
  auto f = [&](auto rF, const auto& pts, const auto& vals, ll lo, ll hi) -> void {
    DLOGK(pts, vals, lo, hi);
    if (pts.empty()) return;
    if (hi == lo + 1) {
      REP(i, 0, ssize(pts)) {
        exh[lo].push_back(pts[i]);
        exh_val[lo] += vals[i];
      }
      return;
    }
    ll mid = (hi + lo) / 2;
    vector<ll> diff(ssize(pts) + 1);
    REP(q, lo, mid) {
      ll a = lower_bound(ALL(pts), L[q]) - pts.begin();
      ll b = lower_bound(ALL(pts), R[q]) - pts.begin();
      diff[a] -= K[q];
      diff[b] += K[q];
    }
    DLOGK(lo, hi, diff);
    vector<ll> pts_lo, pts_hi, vals_lo, vals_hi;
    ll d = 0;
    REP(i, 0, ssize(pts)) {
      d += diff[i];
      ll v = vals[i] + d;
      DLOGK(lo, hi, i, pts[i], vals[i], d, v);
      if (v <= 0) {
        pts_lo.push_back(pts[i]);
        vals_lo.push_back(vals[i]);
      }else {
        pts_hi.push_back(pts[i]);
        vals_hi.push_back(v);
      }
    }
    rF(rF, pts_lo, vals_lo, lo, mid);
    rF(rF, pts_hi, vals_hi, mid, hi);
  };
  f(f, pts0, vals0, 0, Q + 1);
  
  auto st = make_seg_tree(0LL, plus<ll>(), vector<ll>(N, 0LL));
  REP(q, 0, Q) {
    ll zero = st.query(L[q], R[q]);
    ll ans = (R[q] - L[q] - zero - ssize(exh[q])) * K[q] + exh_val[q];
    cout << ans << "\n";
    for (ll i : exh[q]) st.rs(i) = 1;
  }
