...
int body(istream& cin, ostream& cout) {
  ...
  auto func = [&](vector<ll>& ls, vector<ll>& rs) -> Fp {
    auto init_tbl = vector(2, vector(2, vector(2, Fp(0))));
    auto tbl = init_tbl;
    tbl[1][1][1] = 1;
    for (ll i = maxidx; i >= 0; i--) {
      ll bl = ls[i], br = rs[i];
      auto prev = move(tbl);
      tbl = init_tbl;
      for (ll az = 0; az < 2; az++) {
        for (ll ix = 0; ix < 2; ix++) {
          for (ll iy = 0; iy < 2; iy++) {
            for (ll vx = 0; vx < 2; vx++) {
              for (ll vy = 0; vy < 2; vy++) {
                ll aw = az, jx = ix, jy = iy;
                if (az == 1) {
                  if (vx != vy) continue;
                  if (vx == 1) aw = 0;
                }
                if (vx == 1 && vy == 0) continue;
                if (ix == 1) {
                  if (vx < bl) continue;
                  if (bl < vx) jx = 0;
                }
    ...
    return tbl[0][0][0] + tbl[0][0][1] + tbl[0][1][0] + tbl[0][1][1];
  };
  ll L, R; cin >> L >> R;
  auto mkvec = [&](ll t) {
    vector<ll> ret(maxidx + 1);
    for (ll i = 0; i <= maxidx; i++) {
      ret[i] = t % 2;
      t /= 2;
    }
    return ret;
