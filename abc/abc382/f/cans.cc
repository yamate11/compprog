...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W, N; cin >> H >> W >> N;
  // @InpMVec(N, ((R, dec=1), (C, dec=1), L)) [pTNCBvjZ]
  auto R = vector(N, ll());
  auto C = vector(N, ll());
  auto L = vector(N, ll());
  for (int i = 0; i < N; i++) {
  ...
  vector<ll> ord(N, 0LL);
  REP(i, 0, N) ord[i] = i;
  sort(ALL(ord), [&](ll i, ll j) -> bool { return R[i] > R[j]; });
  ...
  auto st = make_seg_tree_lazy(H, OP(), mymin, comp, appl, vector<ll>(W, H));
  
  vector<ll> ans(N);
  REP(ii, 0, N) {
    ll i = ord[ii];
    ll v = st.query(C[i], C[i] + L[i]);
    ans[i] = v - 1;
    st.update(C[i], C[i] + L[i], v - 1);
  }
