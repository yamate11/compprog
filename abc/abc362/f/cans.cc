...
int main(/* int argc, char *argv[] */) {
  ...
  auto [g1, g2] = tr.centroids();
  tr.change_root(g1);
  auto getst = [&](ll nd) -> vector<ll> {
    vector<ll> ret;
    auto dfs0 = [&](auto rF, ll m) -> void {
      ret.push_back(m);
      for (ll cld : tr.children(m)) rF(rF, cld);
    };
    dfs0(dfs0, nd);
    return ret;
  };
  ...
  vector<ll> arr;
  for (int i : getst(cc)) arr.push_back(i);
  for (int i : tr.children(g1)) {
    if (i == cc) continue;
    for (int j : getst(i)) arr.push_back(j);
  }
  if (N % 2 == 0) {
    arr.push_back(g1);
    assert(ssize(arr) == N);
    REP(i, 0, N/2) {
      pout(arr[i], arr[N/2 + i]);
    }
  }else {
    assert(ssize(arr) == N - 1);
    REP(i, 0, (N - 1)/2) {
      pout(arr[i], arr[(N - 1)/2 + i]);
    }
  }
  return 0;
}
