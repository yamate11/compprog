...
// @@ !! LIM(forall cmpNaive debug binsearch)
...
int body(istream& cin, ostream& cout) {
  ...
  ll N = ssize(S);
  ll Q; cin >> Q;
  // @InpVec(Q, T, type=string) [jqMrGq6R]
  auto T = vector(Q, string());
  for (int i = 0; i < Q; i++) { string v; cin >> v; T[i] = v; }
  // @End [jqMrGq6R]

  string C = S + '9';
  vector<pll> Z(N, pll(0, 0));
  Z.emplace_back(-1, -1);
  REP(i, 0, Q) {
    C += T[i] + '0' + T[i] + '~';
    Z.emplace_back(1, i);
    REP(j, 0, ssize(T[i])) Z.emplace_back(-1, -1);
    Z.emplace_back(2, i);
    REP(j, 0, ssize(T[i])) Z.emplace_back(-1, -1);
  }
  ...
  REP(i, 0, ssize(C)) {
    auto [tp, idx] = Z[sa[i]];
    if (tp == 0) seq++;
    else if (tp == 1) ans[idx] -= seq;
    else if (tp == 2) ans[idx] += seq;
  }
  REPOUT(i, 0, Q, ans[i], "\n");
