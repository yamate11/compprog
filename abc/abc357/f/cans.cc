...
int body(istream& cin, ostream& cout) {
  ...
  using DAT = tuple<Fp, Fp, Fp, ll>;
  DAT unit_dat(0, 0, 0, 0);
  using OP = pair<Fp, Fp>;
  OP unit_op = pll(Fp(0), Fp(0));
  auto myadd = [&](DAT x, DAT y) -> DAT {
    auto [xab, xa, xb, xlen] = x;
    auto [yab, ya, yb, ylen] = y;
    return DAT(xab + yab, xa + ya, xb + yb, xlen + ylen);
  };
  auto myappl = [&](OP p, DAT x) -> DAT {
    auto [pa, pb] = p;
    auto [xab, xa, xb, xlen] = x;
    return DAT(xab + pa * xb + pb * xa + pa * pb * xlen, xa + pa * xlen, xb + pb * xlen, xlen);
  };
  vector<DAT> init_vec(N, DAT());
  REP(i, 0, N) {
    init_vec[i] = DAT(Fp(A[i]) * Fp(B[i]), Fp(A[i]), Fp(B[i]), 1LL);
  }
  auto mycomp = [&](OP p, OP q) -> OP {
    auto [pa, pb] = p;
    auto [qa, qb] = q;
    return OP(pa + qa, pb + qb);
  };
  auto st = make_seg_tree_lazy(unit_dat, unit_op, myadd, mycomp, myappl, init_vec);
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll l, r, x; cin >> l >> r >> x; l--;
      st.update(l, r, OP(Fp(x), 0));
    }else if (tp == 2) {
      ll l, r, x; cin >> l >> r >> x; l--;
      st.update(l, r, OP(0, Fp(x)));
    }else if (tp == 3) {
      ll  l, r; cin >> l >> r; l--;
      auto [xab, xa, xb, xlen] = st.query(l, r);
      cout << xab << "\n";
    }else assert(0);
  }
