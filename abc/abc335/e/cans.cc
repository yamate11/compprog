...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, M) {
    ll u, v; cin >> u >> v; u--; v--;
    if (A[u] <= A[v]) scc.addEdge(u, v);
    if (A[v] <= A[u]) scc.addEdge(v, u);
  }
  scc.build();
  ll cc0 = scc.ccForNode(0);
  ll ccN = scc.ccForNode(N - 1);
  ll sz = scc.numComp();
  
  ll big = 1e18;
  using OL = optional<ll>;
  vector<OL> tbl(sz, OL());
  auto rank = [&](auto rF, ll n) -> ll {
    OL& ret = tbl[n];
    if (not ret.has_value()) {
      if (n == ccN) ret = 0;
      else {
        if (scc.cFwd(n).empty() or (SIZE(scc.cFwd(n)) == 1 and scc.cFwd(n)[0] == n)) {
          ret = -big;
        }else {
          ret = -big;
          for (ll p : scc.cFwd(n)) {
            if (p == n) continue;
            ret = max(*ret, rF(rF, p) + 1);
          }
        }
      }
    }
