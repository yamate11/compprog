...
int main(/* int argc, char *argv[] */) {
  ...
  auto f1 = [&](ll n, ll x) -> Fp {
    if (abs(x) > n) return 0;
    if ((n - x) % 2 != 0) return 0;
    return cm.comb(n, (n - x) / 2);
  };
  auto f2 = [&](ll n, ll x, ll y) -> Fp { return f1(n, x + y) * f1(n, x - y); };
  auto f3 = [&](ll n, ll x, ll y, ll z) -> Fp {
    Fp ret = 0;
    REP(m, N + 1) ret += cm.comb(n, m) * f1(m, z) * f2(N - m, x, y);
    return ret;
  };
  cout << f3(N, X, Y, Z) << endl;
  return 0;
}
