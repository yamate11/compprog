...
// @@ !! LIM(sieve debug f:intDiv f:updMaxMin)
...
int main(/* int argc, char *argv[] */) {
  ...
  REPrev(n, N, 1) {
    if      (P[n] > 0) {
      updMax(lo, divCeil( -Q[n], P[n]));
      double lo_cand = (double)(-Q[n]/P[n]);
      DLOGK(n, -Q[n], P[n], lo_cand);
    }else if (P[n] < 0) {
      updMin(hi, divFloor(-Q[n], P[n]));
      double hi_cand = (double)(-Q[n]/P[n]);
      DLOGK(n, -Q[n], P[n], hi_cand);
    }else {
      DLOGKL("P == 0", n, Q[n]);
      if (Q[n] < 0) {
        DLOG("impossible");
