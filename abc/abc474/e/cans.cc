...
// @@ !! LIM(cmpNaive sortedIndex debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> ll {
    ...
    REP(ex, 0, N + 1) {
      ...
      vector o2i(N, 0LL);
      REP(i, 0, N) o2i[i] = i;
      ranges::sort(o2i, [&](ll i, ll j) -> bool { return A[i] - B[i] < A[j] - B[j]; });
      REP(ii, 0, N) {
        ll i = o2i[ii];
        DLOGK(ii, i, A[i], B[i]);
        if (N - ii <= coupon) {
  ...
  auto solve = [&]() -> ll {
    ...
    REP(ii, 0, N) {
      ...
      if (ii < N / 2) {
        ...
        ll fact = (2 * ii + 1 == N) ? 1 : 2;
        if (A[i] - B[i] > fact * d0) {
          DLOGKL("  B2", B[i] + fact * d0);
          ans += B[i] + fact * d0;
        }else {
          DLOGKL("  A", A[i]);
          ans += A[i];
