...
int main(/* int argc, char *argv[] */) {
  ...
  myset bar;
  REP(i, 0, K) bar.insert(enc(R[i], C[i]));
  
  queue<pll> que;
  auto init_in = [&](ll r, ll c) -> void {
    if (bar.contains(enc(r, c))) {
      que.emplace(r, c);
      bar.erase(enc(r, c));
    }
  };
  REP(i, 1, H) init_in(i, 0);
  REP(i, 1, W) init_in(H - 1, i);
