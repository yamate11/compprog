...
// @@ !! LIM(unordered_map debug UnionFind)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W, K; cin >> H >> W >> K;
  // @InpMVec(K, ((R, dec=1), (C, dec=1))) [jzPSQmmu]
  auto R = vector(K, ll());
  auto C = vector(K, ll());
  for (int i = 0; i < K; i++) {
  ...
  UnionFind uf(K + 2);
  vector<unordered_map<ll, ll>> bar(H);
  REP(i, 0, K) bar[R[i]][C[i]] = i;
  REP(i, 0, K) {
    if (R[i] == 0) uf.merge(i, K + 1);
    if (R[i] == H - 1) uf.merge(i, K);
    if (C[i] == 0) uf.merge(i, K);
    if (C[i] == W - 1) uf.merge(i, K + 1);
    REP(dr, -1, 2) REP(dc, -1, 2) {
      ll nr = R[i] + dr;
      ll nc = C[i] + dc;
      if (dr == 0 and dc == 0) continue;
      if (0 <= nr and nr < H and 0 <= nc and nc < W) {
        auto it = bar[nr].find(nc);
        if (it != bar[nr].end()) uf.merge(i, it->second);
      }
    }
  }
  if (uf.leader(K) == uf.leader(K + 1)) {
    cout << "No\n";
  }else {
    cout << "Yes\n";
  }
