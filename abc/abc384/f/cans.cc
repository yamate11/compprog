...
// @@ !! LIM(unordered_map cmpNaive debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto f = [&](ll x) -> ll { return x >> countr_zero((u64)x); };
  ll ans = 0;
...
int body(istream& cin, ostream& cout) {
  ...
  ll diag = 0;
  REP(i, 0, N) diag += (A[i] >> countr_zero((u64)A[i]));
  ...
  REP(k, 0, K) {
    ...
    safe_umap<ll, pll> mp;
    REP(i, 0, N) {
      ll key = A[i] & (pow_k_1 - 1);
      auto [it, b] = mp.try_emplace(key, pll(0, 0));
      it->second.first++;
      it->second.second += A[i];
    }
    DLOGK(k, mp);
    ll lksum = 0;
    for (auto [a, ns] : mp) {
      ...
      auto it2 = mp.find(b);
      if (it2 != mp.end()) {
        auto [nb, sb] = it2->second;
        ll lsum = sa * nb + sb * na;
        assert(countr_zero((u64)lsum) >= k);
        lksum += (lsum >> k);
      }
    }
    DLOGK(k, lksum);
