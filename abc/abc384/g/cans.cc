...
struct GenSegTree {
  ...
  DAT query(int l, int r) {
    ...
    while (l < r) {
      if (l & 1) {
	ret_l = add(move(ret_l), node[l]);
	l++;
      }
      if (r & 1) {
	ret_r = add(node[r-1], move(ret_r));
      }
      l >>= 1;
      r >>= 1;
    }
    DAT ret = add(move(ret_l), move(ret_r));
    return ret;
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  auto op_x = [&](ll x, ll k) -> void {
    ...
    cnt_x += k;
    sum_x += k * A[x];
    DLOGKL("op_x", x, k, a, cnt_lo, sum_lo, cnt_hi, sum_hi, cval, st_x.at(a));
  };
  auto op_y = [&](ll y, ll k) -> void {
    ...
    cnt_y += k;
    sum_y += k * B[y];
    DLOGKL("op_y", y, k, a, cnt_lo, sum_lo, cnt_hi, sum_hi, cval, st_y.at(a));
  };
