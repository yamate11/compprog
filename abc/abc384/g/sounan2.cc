...
int main() {
    ...
    atcoder::segtree<ll, myplus, myunit> fta0(sz), ftb0(sz);
    atcoder::segtree<ll, myplus, myunit> fta1(sz), ftb1(sz);
    ...
    for (const auto& [xi, yi, i] : query) {
        while (x < xi) {
            ll c = ftb0.prod(0, a[x] + 1);
            ll s = ftb1.prod(0, a[x] + 1);
            now += y * A[x] + bsum - 2 * ((y - c) * A[x] + s);
            asum += A[x];
            fta0.set(a[x], fta0.get(a[x]) + 1);
            // fta0.add(a[x], 1);
            fta1.set(a[x], fta1.get(a[x]) + A[x]);
            // fta1.add(a[x], A[x]);
            x++;
        }
        while (x > xi) {
        ...
        while (y < yi) {
            ll c = fta0.prod(0, b[y] + 1);
            ll s = fta1.prod(0, b[y] + 1);
            now += x * B[y] + asum - 2 * ((x - c) * B[y] + s);
            bsum += B[y];
            ftb0.set(b[y], ftb0.get(b[y]) + 1);
            // ftb0.add(b[y], 1);
            ftb1.set(b[y], ftb1.get(b[y]) + B[y]);
            // ftb1.add(b[y], B[y]);
            y++;
        }
        while (y > yi) {
