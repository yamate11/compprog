...
// @@ !! LIM(mo coordCompr segTree debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  */
  ...
  auto op_x = [&](ll x, ll k) -> void {
    ll a = cc.c(A[x]);
    ll cnt_lo = st_y_cnt.query(0, a);
    ll sum_lo = st_y_sum.query(0, a);
    // auto [cnt_lo, sum_lo] = st_y.query(0, a);
    ll cnt_hi = cnt_y - cnt_lo;
    ll sum_hi = sum_y - sum_lo;
    ll cval = k * ((sum_hi - A[x] * cnt_hi) + (A[x] * cnt_lo - sum_lo));
    val += cval;
    // st_x_cnt.rs(a) = st_x_cnt.at(a) + k;
    st_x_cnt.set_single(a, st_x_cnt.at(a) + k);
    // st_x_sum.rs(a) = st_x_sum.at(a) + k * A[x];
    st_x_sum.set_single(a, st_x_cnt.at(a) + k * A[x]);
    // st_x.rs(a) = st_x.at(a) + k * MyPair(1, A[x]);
    cnt_x += k;
    sum_x += k * A[x];
  ...
  auto op_y = [&](ll y, ll k) -> void {
    ll a = cc.c(B[y]);
    ll cnt_lo = st_x_cnt.query(0, a);
    ll sum_lo = st_x_sum.query(0, a);
    // auto [cnt_lo, sum_lo] = st_x.query(0, a);
    ll cnt_hi = cnt_x - cnt_lo;
    ll sum_hi = sum_x - sum_lo;
    // auto [cnt_hi, sum_hi] = st_x.query(a, sz);
    ll cval = k * ((sum_hi - B[y] * cnt_hi) + (B[y] * cnt_lo - sum_lo));
    val += cval;
    // st_y_cnt.rs(a) = st_y_cnt.at(a) + k;
    st_y_cnt.set_single(a, st_y_cnt.at(a) + k);
    // st_y_sum.rs(a) = st_y_sum.at(a) + k * B[y];
    st_y_sum.set_single(a, st_y_sum.at(a) + k * B[y]);
    // st_y.rs(a) = st_y.at(a) + k * MyPair(1, B[y]);
    cnt_y += k;
    sum_y += k * B[y];
