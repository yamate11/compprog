...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&](ll p) -> bool {
    double v = double(n) * b - double(a) * p * (p + 1) / 2;
    DLOGK(p, v);
    return v <= 0;
  };
  DLOGK(n);
  ll p0 = binsearch_i<ll>(check, t, 0);
  // cout << p0 << endl;
  if (s == t) {
    cout << 0 << endl;
  }else if (s <= t - p0 or t < s) {
    double e = (double(n) * double(b) + double(a) * double(p0) * double(p0 - 1) / 2) / double(p0);
    cout << e << endl;
  }else {
    double e = (t - s) * a;
    cout << e << endl;
