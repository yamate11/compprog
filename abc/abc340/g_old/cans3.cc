...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll nd, ll pt) -> pair<sta, Fp> {
    Fp thisComplete = 0;
    sta thisCont;
    for (ll cld : nbr[nd]) {
      if (cld == pt) continue;
      auto [subCont, subComplete] = rF(rF, cld, nd);
      thisComplete += subComplete;
      auto it2 = subCont.find(A[nd]);
      if (it2 != subCont.end()) thisComplete += it2->second;
      if (SIZE(thisCont) > SIZE(subCont)) swap(thisCont, subCont);
