...
int main(/* int argc, char *argv[] */) {
  ...
  using sta = unordered_map<ll, Fp>;
  // vector<unordered_map<ll, Fp>> cont(N);
  // vector<Fp> complete(N, Fp(0));
  auto dfs = [&](auto rF, ll nd) -> pair<sta, Fp> {
    const auto& cldr = tr.children(nd);
    Fp thisComplete = 0;
    sta thisCont;
    ll nc = SIZE(cldr);
    vector<ll> ord(nc); REP(i, 0, nc) ord[i] = i;
    sort(ALL(ord), [&](ll i, ll j) -> bool { return tr.stsize(cldr[i]) > tr.stsize(cldr[j]); });
    REP(i, 0, nc) {
      ll cld = cldr[ord[i]];
      const auto& [subCont, subComplete] = rF(rF, cld);

      thisComplete += subComplete;

      if (i == 0) {
        thisCont = move(subCont);
        auto it = thisCont.find(A[nd]);
        if (it != thisCont.end()) thisComplete += it->second;
      }else {
        for (auto [c2, v2] : subCont) {
          if (c2 == A[nd]) thisComplete += v2;
          auto it = thisCont.find(c2);
          if (it == thisCont.end()) {
            thisCont[c2] = v2;
          }else {
            thisComplete += v2 * it->second;
            it->second = v2 + it->second + v2 * it->second;
          }
        }
      }
    }
    thisComplete += 1;
    thisCont[A[nd]] += 1;
    return {move(thisCont), thisComplete};
  };
  const auto& [_, ans] = dfs(dfs, 0);
  cout << ans << endl;
