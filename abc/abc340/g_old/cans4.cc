...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll nd) -> sta {
    ...
    ll nc = ssize(cldr);
    vector<int> ord(nc); iota(ALL(ord), 0);
    sort(ALL(ord), [&](ll i, ll j) -> bool { return tr.stsize(cldr[i]) > tr.stsize(cldr[j]); });
    // vector<ll> foo;
    REP(i, 0, nc) {
      ll cld = cldr[ord[i]];
      // foo.push_back(cld);
      sta cldCont = rF(rF, cld);
      if (i == 0) {
        auto it = cldCont.find(A[nd]);
        if (it != cldCont.end()) ans += it->second;
        ret = move(cldCont);
      }else {
        for (auto [c, v] : cldCont) {
          // cnt++;
          if (A[nd] == c) ans += v;
          auto it = ret.find(c);
          if (it == ret.end()) {
...
#if 0
    assert(ssize(foo) == 2 or ssize(foo) == 0);
    if (ssize(foo) == 2) {
      if (foo[0] < foo[1]) {
        cerr << "violated " << foo[0] << " " << foo[1] << endl;
      }
    }
#endif
    ans += 1;
