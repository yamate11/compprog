...
int main(/* int argc, char *argv[] */) {
  ...
  vector cpt(N, unordered_map<ll, ll>());
  vector crt(N, -1LL);
  ...
  auto dfs1 = [&](auto rF, ll nd) -> void {
    ...
    if (rec[c].empty()) {
      ...
      if (m == p) {
        ...
        while (true) {
          rec[c].pop_back();
          if (rec[c].empty()) break;
          ll newcur = rec[c].back();
          if (tr.depth(newcur) <= tr.depth(m)) {
            if (tr.depth(newcur) == tr.depth(m)) rec[c].pop_back();
            break;
          }
          cpt[c][cur] = newcur;
          cur = newcur;
  ...
  REP(c, 0, N) {
    ll cur = -1;
    while (not rec[c].empty()) {
      if (cur >= 0) cpt[c][cur] = rec[c].back();
      cur = rec[c].back();
      rec[c].pop_back();
    }
    crt[c] = cur;
  ...
  REP(c, 0, N) {
    if (crt[c] < 0) continue;
    unordered_map<ll, vector<ll>> ccldr;
    for (auto [cd, pt] : cpt[c]) ccldr[pt].push_back(cd);
    auto dfs2 = [&](auto rF, ll nd) -> Fp {
      Fp r = 0;
      for (ll cld : ccldr[nd]) {
        Fp z = rF(rF, cld);
        if (A[nd] == c) ans += z;
