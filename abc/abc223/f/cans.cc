  ...
  REP(_q, Q) {
    ...
    if (tp == 1) {
      ...
      if (l == 0) {
        ...
        ll q1 = st.query(l-1, l);
        ll q2 = st.query(l, l+1);
        vl = q2 - q1;
      }
      ll q3 = st.query(r-1, r);
      ll q4 = st.query(r, r+1);
      vr = q4 - q3;
      if (vl == vr) {
      }else {
        st.update(l, r, vr - vl);
      ...
      ll left = l == 0 ? 0 : st.query(l - 1, l);
      ll vmin = st.query(l, r + 1);
      ll right = st.query(r, r + 1);
      DLOGK(l, r, left, vmin, right);
      bool b = left == right && left == vmin;
      cout << (b ? "Yes\n" : "No\n");
    }
  }
