...
int main(/* int argc, char *argv[] */) {
  ...
  Tree tr(move(ve));
  auto add = [&](int t1, int t2) -> int { return t1 or t2; };
  auto mod = [&](int t, int node, int child) -> int { return not t; };
  vector<int> result = reroot(tr, 0, add, mod);
  ll ans = 0;
  REP(i, N) if (not result[i]) ans++;
  cout << ans << endl;
