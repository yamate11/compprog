...
int body(istream& cin, ostream& cout) {
  ...
  auto f = [&](ll K, const auto& A, const auto& B, ll shift) -> Fp {
    ll N = ssize(A);
    ll M = ssize(B);
    ll g = gcd(N, M);
    ll NN = N / g;
    ll MM = M / g;
    vector<ll> ord(MM);
    REP(i, 0, MM) ord[i] = (NN * i) % MM;
    vector<ll> rev(MM);
    REP(i, 0, MM) rev[ord[i]] = i;
    DLOGK(ord);
    DLOGK(rev);
    ll rep = K / (NN * MM * g);
    ll rem = K % (NN * MM * g);
    ...
    REP(r, 0, g) {
      vector<ll> B1(MM);
      REP(i, 0, MM) B1[i] = B[i * g + r];
      ranges::sort(ALL(B1));
      REP(i, 0, NN) {
        ll num1 = B1.end() - ranges::lower_bound(B1, A[i * g + r] + shift);
        ret1 += A[i * g + r] * num1;
      }
      DLOGK(r, ret1);
    }
    ...
    REP(r, 0, g) {
      ...
      auto st = make_seg_tree(0LL, plus<ll>(), vector<ll>(2*MM));

      using ev_t = tuple<ll, ll, ll>;
      ll EVA = 1;
      ll EVB = 2;
      vector<ev_t> ev;
      REP(i, 0, NN) ev.emplace_back(A[i * g + r] + shift, EVA, i);
      REP(i, 0, MM) ev.emplace_back(B[i * g + r], EVB, i);
      ranges::sort(ev, greater<>());
      ...
      for (auto [v, tp, i] : ev) {
        if (tp == EVB) {
          ...
          if (i < p1) {
            ...
            ll q2 = p1 % NN;
            if (i < q2) p2++;
            ll thisval = (v - shift) * st.query(i0, i0 + p2);
            DLOGK(r, v, i, i0, p2, thisval);
            ret2 += thisval;
