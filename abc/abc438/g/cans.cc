...
// @@ !! LIM(mod segTree debug cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  auto h2 = [&](ll K, const auto& A, const auto& B, ll shift) -> Fp {
    ll N = ssize(A);
    ll M = ssize(B);

    vector<ll> ord(M);
    REP(i, 0, M) ord[i] = (i * N) % M;
    vector<ll> rev(M);
    REP(i, 0, M) rev[ord[i]] = i;

    ll p0 = K / N;
    ll q0 = K % N;
    auto st = make_seg_tree(0LL, plus<ll>(), vector<ll>(2*M, 0LL));

    using ev_t = tuple<ll, ll, ll>;
    vector<ev_t> ev;
    ll EVA = 1;
    ll EVB = 2;
    REP(i, 0, N) ev.emplace_back(A[i] + shift, EVA, i);
    REP(i, 0, M) ev.emplace_back(B[i], EVB, i);
    ranges::sort(ev, greater<ev_t>());
    Fp ret = 0;
    for (auto [val, tp, i] : ev) {
      if (tp == EVA) {
        if (i < K) {
          ll j = i % M;
          Fp thisval = Fp(A[i]) * st.query(rev[j], rev[j] + p0 + (i < q0 ? 1 : 0));
          ret += thisval;
        }
      }else if (tp == EVB) {
        st.rs(rev[i]) = st.rs(M + rev[i]) = 1;
      }
    }
    return ret;
  };

  auto gg = [&](ll K, const auto& A, const auto& B, ll shift) -> Fp {
    ll N = ssize(A);
    ll M = ssize(B);
    ...
    auto h1 = [&]() -> Fp {
      ...
      Fp ans = 0;
      for (ll a : A) {
        ll n = BB.end() - ranges::lower_bound(BB, a + shift);
        ans += a * n;
      }
      return ans;
    };
    ...
    ll Kq = K % (N * M);
    Fp ans1 = Kp * h1();
    Fp ans2 = h2(Kq, A, B, shift);
    return ans1 + ans2;
  };

  auto f = [&](ll K, auto& A, auto& B) -> Fp {
    Fp ans1 = gg(K, A, B, 0);
    Fp ans2 = gg(K, B, A, 1);
    return ans1 + ans2;
  };
  ...
  REP(r, 0, g) {
    vector<ll> A1(N / g);
    vector<ll> B1(M / g);
    REP(p, 0, N / g) A1[p] = A[p * g + r];
    REP(p, 0, M / g) B1[p] = B[p * g + r];
    ll Kp = K / g;
    ll Kq = K % g;
    if (r < Kq) Kp++;
    ans += f(Kp, A1, B1);
