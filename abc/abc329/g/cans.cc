...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, K; cin >> N >> M >> K;
  // @InpVec(N - 1, P, dec=1) [K5ThYFHe]
  auto P = vector(N - 1, ll());
  for (int i = 0; i < N - 1; i++) { ll v; cin >> v; v -= 1; P[i] = v; }
  // @End [K5ThYFHe]
  ...
  vector<ll> trns(N);
  {
    // @InpMVec(M, ((S, dec=1), (G, dec=1))) [05llqRlO]
    auto S = vector(M, ll());
    auto G = vector(M, ll());
    for (int i = 0; i < M; i++) {
      ll v1; cin >> v1; v1 -= 1; S[i] = v1;
      ll v2; cin >> v2; v2 -= 1; G[i] = v2;
    }
    // @End [05llqRlO]

    REP(i, 0, N - 1) tr.add_edge(P[i], i + 1);
    REP(i, 0, M) {
      ll ta = tr.lca(S[i], G[i]);
      ll d = tr.depth(ta);
      if (ta == S[i]) {
        gammaE[G[i]]++;
        ll tb = tr.ancestorDep(G[i], d + 1);
        if (tb == tr.children(S[i])[0]) sigmaP[S[i]]++;
        else sigmaQ[S[i]]++;
      }else if (ta == G[i]) {
        sigmaE[S[i]]++;
        ll tb = tr.ancestorDep(S[i], d + 1);
        if (tb == tr.children(G[i])[0]) gammaP[G[i]]++;
        else gammaQ[G[i]]++;
      }else {
        ll tb = tr.ancestorDep(S[i], d + 1);
        sigmaE[S[i]]++;
        gammaE[G[i]]++;
        if (tb == tr.children(ta)[0]) {
          if (dir[ta] == 1) {
            cout << 0 << endl;
            return 0;
          }
  ...
  vector tbl(N, vector<Fp>());
  vector GG(N, 0LL), SS(N, 0LL);
  auto dfs = [&](auto rF, ll nd) -> void {
    ll numc = SIZE(tr.children(nd));
    if (numc == 0) {
      GG[nd] = gammaE[nd];
      SS[nd] = sigmaE[nd];
      tbl[nd].push_back(Fp(1));
      DLOGK(nd, tbl[nd]);
      return;
    }
    if (numc == 1) {
      ll cld = tr.children(nd)[0];
      rF(rF, cld);
      GG[nd] = gammaE[nd] + GG[cld] - sigmaP[nd];
      SS[nd] = sigmaE[nd] + SS[cld] - gammaP[nd];
      REPrev(a, SIZE(tbl[cld]) - 1, 0) {
        ll aa = max(a, max(SS[cld], GG[cld]));
        if (aa > K) continue;
        if (aa >= SIZE(tbl[nd])) tbl[nd].resize(aa + 1);
        tbl[nd][aa] += tbl[cld][a];
      }
      DLOGK(nd, tbl[nd]);
      return;
    ...
    assert(numc == 2);
    auto func = [&](ll cld1, ll cld2, const auto& sigma1, const auto& sigma2,
                    const auto& gamma1, const auto& gamma2) -> void {
      GG[nd] = gammaE[nd] + GG[cld1] - sigma1[nd] + GG[cld2] - sigma2[nd] - trns[nd];
      SS[nd] = sigmaE[nd] + SS[cld1] - gamma1[nd] + SS[cld2] - gamma2[nd] - trns[nd];
      ll va = max(GG[cld1] + GG[cld2] - sigma2[nd],
                  max(SS[cld1] + GG[cld2] - sigma2[nd],
                      max(SS[cld1] + GG[cld2] - gamma1[nd],
                          SS[cld1] + SS[cld2] - gamma1[nd])));
      DLOGK(nd, cld1, cld2, va);
      REPrev(a1, SIZE(tbl[cld1]) - 1, 0) REPrev(a2, SIZE(tbl[cld2]) - 1, 0) {
        ll aa = max(a1 + GG[cld2] - sigma2[nd],
                    max(a2 + SS[cld1] - gamma1[nd], va));
        aa -= trns[nd];
        DLOGK(nd, cld1, cld2, a1, a2, aa);
        if (aa > K) continue;
        if (aa >= SIZE(tbl[nd])) tbl[nd].resize(aa + 1);
        tbl[nd][aa] += tbl[cld1][a1] * tbl[cld2][a2];
      }
    };
    ll cldP = tr.children(nd)[0];
    ll cldQ = tr.children(nd)[1];
    rF(rF, cldP);
    rF(rF, cldQ);
    if (dir[nd] != 1) func(cldP, cldQ, sigmaP, sigmaQ, gammaP, gammaQ);
    if (dir[nd] != 0) func(cldQ, cldP, sigmaQ, sigmaP, gammaQ, gammaP);
    DLOGK(nd, tbl[nd]);
  };
