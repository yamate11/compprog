...
int main(/* int argc, char *argv[] */) {
  ...
  auto st = make_seg_tree(0LL, plus<ll>(), A);
  itv_set<int> itvB(0, N);
  REP(i, 0, N) {
    ll val = B[i] >= 2 ? 1 : 0;
    itvB.put(i, val);
  }
  itv_set<int> itvA(0, N);
  REP(i, 0, N) {
    ll val = A[i] == 0 ? 0 : 1;
    itvA.put(i, val);
  }
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      auto f = [&]() -> ll {
        ...
        while (p < r) {
          if (B[p] <= 1) {
            auto [loB, hiB, tB] = itvB.get(p);
            if (r <= hiB) hiB = r;
            ans += st.query(p, hiB);
            p = hiB;
          }else {
            ans = max(ans + A[p], ans * B[p]);
            p++;
          }
        }
        return ans;
