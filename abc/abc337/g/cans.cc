...
int main(/* int argc, char *argv[] */) {
  ...
  vector rec(N, map<ll, ll>());
  auto dfs0 = [&](auto rF, ll nd) -> ordered_set {
    ordered_set ret;
    ll t = 0;
    for (ll cld: tr.children(nd)) {
      ordered_set ss = rF(rF, cld);
      ll tt = ss.order_of_key(nd);
      rec[nd][cld] = tt;
      t += tt;
      if (SIZE(ret) > SIZE(ss)) for (ll x : ss) ret.insert(x);
      else {
        for (ll x : ret) ss.insert(x);
        ret = move(ss);
      }
    }
    if (nd != 0) rec[nd][tr.parent(nd)] = nd - t;
    ret.insert(nd);
    return ret;
  };
  dfs0(dfs0, 0);
  
  auto add = [&](pll a, pll b) -> pll { return pll(a.first + b.first, a.second + b.second); };
  auto mod = [&](pll p, ll nd, ll cld) -> pll {
    return pll(p.first + p.second, rec[nd][cld]);
  };
  auto result = reroot(tr, pll(0, 0), add, mod);
  REPOUT(i, 0, N, result[i].first + result[i].second, " ");
