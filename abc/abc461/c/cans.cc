...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [mdmVwpng]
  vector<vector<ll>> perc(N);
  REP(i, 0, N) {
    perc[C[i]].push_back(V[i]);
  }
  REP(i, 0, N) ranges::sort(perc[i], greater<ll>());
  vector<pll> P;
  REP(i, 0, N) {
    if (not perc[i].empty()) P.emplace_back(perc[i][0], i);
  }
  ranges::sort(P, greater<pll>());
  ...
  REP(i, 0, N) {
    if (D[i]) {
      ans += perc[i][0];
      REP(j, 1, ssize(perc[i])) others.push_back(perc[i][j]);
    }else {
      REP(j, 0, ssize(perc[i])) others.push_back(perc[i][j]);
    }
  }
  ranges::sort(others, greater<ll>());
  DLOGK(perc);
  DLOGK(others);
  REP(i, 0, K - M) ans += others[i];
  cout << ans << endl;
