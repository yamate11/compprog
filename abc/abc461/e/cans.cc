...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  auto row = make_seg_tree(0LL, plus<ll>(), vector<ll>(Q));
  auto col = make_seg_tree(0LL, plus<ll>(), vector<ll>(Q));
  vector<ll> row_prev(N, -1LL);
  vector<ll> col_prev(N, -1LL);
  ll ans = 0;
  REP(q, 0, Q) {
    ll tp, idx; cin >> tp >> idx; idx--;
    if (tp == 1) {
      ll p = row_prev[idx];
      if (p < 0) ans += N;
      else {
        ll num = col.query(p, Q);
        ans += num;
        row.rs(p) = 0;
      }
      row.rs(q) = 1;
      ...
      ll p = col_prev[idx];
      if (p < 0) {
        ll num = row.query(0, Q);
        ans -= num;
      }else {
        ll num = row.query(p, Q);
        ans -= num;
        col.rs(p) = 0;
      }
      col.rs(q) = 1;
