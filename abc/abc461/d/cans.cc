...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i0, 0, H) {
    ...
    REP(i1, i0, H) {
      vector<ll> C(W + 1, 0LL);
      REP(j, 0, W) C[j + 1] = C[j] + A[i1][j];
      REP(j, 0, W) B[j + 1] += C[j + 1];
      DLOGK(i0, i1, B);
      ll j1 = 1;
      REP(j0, 0, W) {
        if (j1 <= j0) j1 = j0 + 1;
        while (j1 < W + 1 and B[j1] - B[j0] < K) {
          j1++;
        }
        if (j1 == W + 1) break;
        if (B[j1] - B[j0] == K) {
          ll j2 = lower_bound(ALL(B), B[j1] + 1) - B.begin();
          ans += j2 - j1;
        }
        DLOGK(j0, j1, B[j1] - B[j0]);
      }
      DLOGK(i0, i1, ans);
   }
  }
