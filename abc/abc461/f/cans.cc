...
// @@ !! LIM(sieve mod unordered_map debug cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  Fp ans = 0;
  auto dfs = [&](auto rF, ll idx, ll len, ll sum, ll prod) -> void {
    assert(N % prod == 0);
    ll r = N / prod;
    assert(idx < sz and ds[idx] <= r);
    ans += cb.fact(len + 1) * (sum + r);
    for (ll i = idx; i + 1 < sz; i++) {
      if (ds[i + 1] > r / ds[i]) break;
      if (N % (prod * ds[i]) == 0) {
        rF(rF, i + 1, len + 1, sum + ds[i], prod * ds[i]);
      }
    }
  };
  dfs(dfs, 0, 0, 0, 1);
