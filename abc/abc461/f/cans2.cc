...
int body(istream& cin, ostream& cout) {
  ...
  using sta = pair<ll, Fp>;
  auto f = [&](auto rF, ll x, ll n) -> safe_umap<ll, sta> {
    safe_umap<ll, sta> ret;
    if (x < n) ret[1] = sta(1, Fp(n));
    for (ll d : ds) {
      if (d <= x) continue;
      if (n <= d * d) break;
      if (n % d != 0) continue;
