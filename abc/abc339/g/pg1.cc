...
int main(/* int argc, char *argv[] */) {
  ...
  using DAT = vector<ll>;
  auto merge = [&](const DAT& a, const DAT& b) -> DAT {
    ll i = 0, j = 0;
    DAT ret;
    while (i < SIZE(a) or j < SIZE(b)) {
  ...
  auto st = make_seg_tree(DAT(), merge, init_vec);

  ll M = st.size;
  vector C(2*M, vector<ll>());
  REP(i, 1, 2*M) {
    C[i].resize(SIZE(st.node[i]) + 1);
    REP(j, 0, SIZE(st.node[i])) C[i][j + 1] = C[i][j] + st.node[i][j];
  }

  auto ask = [&](ll p, ll q, ll x) -> ll {
    ll ans = 0;
    auto vec = st.node_seq(p, q);
    for (ll i : vec) {
