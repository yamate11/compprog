...
struct GenSegTree {
  ...
  pair<int, int> node_range(u64 idx) {
    u64 sft = idx - bit_floor(idx);
    u64 unit = bit_width((u64)size) - bit_width(idx);
    u64 lo = rotl(sft, unit);
    u64 hi = rotl(sft + 1, unit);
    return {lo, hi};
  }

  vector<int> node_seq(int l, int r) {
    vector<int> ret;
    int x = l + size;
    while (node_range(x).second < r) {
      if (not (x & 1)) x >>= 1;
      else {
        ret.push_back(x);
        x = (x >> 1) + 1;
...
int main() {
  ...
  using DAT = vector<ll>;
  auto merge = [&](const DAT& a, const DAT& b) -> DAT { // 順序を保ってマージ
    DAT ret;
    for (ll i = 0, j = 0; i < SIZE(a) or j < SIZE(b); ) {
      if (j == SIZE(b) or (i < SIZE(a) and a[i] <= b[j])) ret.push_back(a[i++]);
      else ret.push_back(b[j++]);
    }
    return ret;
  };
  vector<DAT> init_vec(N); REP(i, 0, N) init_vec[i] = DAT{A[i]};
  auto st = make_seg_tree(DAT(), merge, init_vec);   // セグメント木を作成．葉は { A[i] }

  ll M = st.size;    // セグメント木のサイズ (bit_ceil(N))
  vector S(2*M, vector<ll>());   // S[i] は，ノード i のベクトルの累積和
  REP(i, 1, 2*M) {   // 累積和を計算
    S[i].resize(SIZE(st.node[i]) + 1);
    REP(j, 0, SIZE(st.node[i])) S[i][j + 1] = S[i][j] + st.node[i][j];
  }
  ...
  REP(_q, 0, Q) {
    ll a, b, c; cin >> a >> b >> c;
    ll L = a ^ B, R = b ^ B, X = c ^ B;
    B = 0;
    for (ll i : st.node_seq(L - 1, R)) {   // L - 1 から R までのノードを列挙する
      // 各ノードで該当する和を作成
      ll idx = upper_bound(ALL(st.node[i]), X) - st.node[i].begin();
      B += S[i][idx];
    }
    cout << B << "\n";
  }
