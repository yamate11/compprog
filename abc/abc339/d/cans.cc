...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> brd(N, N, '#');
  cin >> brd;
  BrdIdx pl1(-1, -1), pl2(-1, -1);
  REP(i, 0, N) REP(j, 0, N) {
    if (brd.at(i, j) == 'P') {
      if (pl1.r < 0) pl1 = BrdIdx(i, j);
      else           pl2 = BrdIdx(i, j);
    }
  }
  ...
  queue<sta> que;
  ll big = 1e18;
  vector dist(N, vector(N, vector(N, vector(N, big))));
  auto r_dist = [&](const BrdIdx& x1, const BrdIdx& x2) -> ll& { return dist[x1.r][x1.c][x2.r][x2.c]; };
  REP(i, 0, N) REP(j, 0, N) {
    que.emplace(BrdIdx(i, j), BrdIdx(i, j));
    r_dist(BrdIdx(i, j), BrdIdx(i, j)) = 0;
  }
  while (not que.empty()) {
    auto [bi1, bi2] = que.front(); que.pop();
    ll curdist = r_dist(bi1, bi2);
    for (auto dir : BrdIdx::nbr4) {
      auto new1 = brd.at(bi1 + dir) == '#' ? bi1 : bi1 + dir;
      auto new2 = brd.at(bi2 + dir) == '#' ? bi2 : bi2 + dir;
      if (new1 == pl1 and new2 == pl2) {
        cout << curdist + 1 << endl;
        return 0;
      }
      if (r_dist(new1, new2) >= big) {
        r_dist(new1, new2) = curdist + 1;
        que.emplace(new1, new2);
      }
    }
  }
  cout << -1 << endl;
