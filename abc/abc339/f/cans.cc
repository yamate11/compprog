...
// @@ !! LIM(random mod sieve power debug)
...
bool is_prime_MR(ll n, int k = 20) {
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, AA, type=string) [cBtLA4O4]
  auto AA = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; AA[i] = v; }
  // @End [cBtLA4O4]
  ...
  ll start = 0;
  REP(i, 0, N) {
    if (i == N - 1 or AA[i] != AA[i + 1]) {
      A.push_back(AA[i]);
      D.push_back(i + 1 - start);
      start = i + 1;
    }
  }
  N = ssize(A);
  ...
  REP(k, 0, 20) {
    ll p;
    while (true) {
      p = rand.range((ll)1e9, (ll)2e9);
      if (_is_prime_MR(p)) break;
    }
    P[k] = p;
  }
  using Fp = Fp<0>;
  ...
  vector<Fp> C(1010);
  C[0] = Fp(1);
  REP(i, 1, 1010) C[i] = C[i - 1] * Fp(10);
  vector<Fp> B(N);
  unordered_map<ll, vector<ll>> mp;
  REP(i, 0, N) {
    REP(j, 0, N) B[i] += (A[i][j] - '0') * C[j];
    mp[(ll)B[i]].push_back(i);
  }
  REP(i, 0, N) REP(j, 0, N) {
    Fp x = B[i] * B[j];
  }
