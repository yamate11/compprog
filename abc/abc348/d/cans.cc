...
int main(/* int argc, char *argv[] */) {
  ...
  Board<ll> medicine(H, W, 0LL);
  REP(i, 0, N) medicine.at(R[i], C[i]) = E[i];
  Board<ll> energy(H, W, -1LL);
  ...
  while (not que.empty()) {
    ...
    e = max(e, medicine.at(bi));
    if (e > energy.at(bi)) {
      energy.at(bi) = e;
      if (e > 0) {
        REP(i, 0, 4) {
          auto bj = bi + BrdIdx::nbr4[i];
          if (brd.at(bj) != '#' and energy.at(bj) < e - 1) {
            que.emplace(e - 1, bj);
          }
        }
      }
