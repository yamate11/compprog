...
int main(/* int argc, char *argv[] */) {
  ...
  cin >> brd;
  ll N; cin >> N;
  // @InpMVec(N, ((R, dec=1), (C, dec=1), E)) [5kTuwbeM]
  auto R = vector(N, ll());
  auto C = vector(N, ll());
  auto E = vector(N, ll());
  for (int i = 0; i < N; i++) {
  ...
  REP(i, 0, H) REP(j, 0, W) {
    ...
    if (brd.at(i, j) == 'T') end = BrdIdx(i, j);
  }

  int start_idx = -1;
  int end_idx = -1;
  REP(i, 0, N) {
    if (start.r == R[i] and start.c == C[i]) start_idx = i;
    if (end.r == R[i] and end.c == C[i]) end_idx = i;
  }
  if (start_idx == -1) {
    cout << "No\n";
    return 0;
  ...
  ll big = 1e18;
  vector gdist(N, vector(N, big));
  REP(i, 0, N) {
    queue<BrdIdx> que;
    que.push(BrdIdx(R[i], C[i]));
    ...
    while (not que.empty()) {
      ...
      for (BrdIdx dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (brd.at(bj) == '#' or dist[bj.r][bj.c] < big) continue;
        dist[bj.r][bj.c] = dist[bi.r][bi.c] + 1;
        que.push(bj);
      }
    }
    REP(j, 0, N) {
      gdist[i][j] = dist[R[j]][C[j]];
    }
  }
  queue<int> bque;
  vector used(N, false);
  bque.push(start_idx);
  used[start_idx] = true;
  while (not bque.empty()) {
    ll i = bque.front(); bque.pop();
    REP(j, 0, N) {
      if (not used[j] and gdist[i][j] <= E[i]) {
        if (j == end_idx) {
          cout << "Yes\n";
          return 0;
        }
