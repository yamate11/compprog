...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](pll a, ll cld, ll nd) -> pll { return pll(a.first + a.second, a.second); };
  auto g = [&](pll a, ll nd) -> pll { return pll(a.first, a.second + C[nd]); };
  auto X = reroot(tr, pll(0, 0), myadd, f, g);

  ll ans = LLONG_MAX;
  REP(i, 0, N) ans = min(ans, X[i].first);
  cout << ans << endl;
