...
int body(istream& cin, ostream& cout) {
  ...
  using DAT = pll;
  using OP = enum { NONE, LEFT, RIGHT };
  auto add = [&](DAT a, DAT b) { return DAT(a.first + b.first, a.second + b.second); };
  auto comp = [&](OP f, OP g) { return f != NONE ? f : g; };
  auto appl = [&](OP f, DAT a) -> DAT {
    if (f == NONE) return a;
    if (f == LEFT) return DAT(0, a.first + a.second);
    if (f == RIGHT) return DAT(a.first + a.second, 0);
    assert(0);
  };
  vector<DAT> init_vec(N - 1);
  REP(i, 0, N - 1) init_vec[i] = DAT(0, W[i + 1] - W[i]);
  auto st = make_seg_tree_lazy(DAT(), OP(), add, comp, appl, init_vec);
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      ll ans;
      if (x < 0) ans = 0;
      else if (W[N - 1] <= x) ans = 0;
      else {
        auto [yl, yr] = st.query(0, N - 1);
        if (yl <= x and x < W[N - 1] - yr) ans = N;
        else if (x < yl) {
          auto check = [&](DAT a) -> bool { return x < a.first; };
          ll t = st.binsearch_l_from(check, N - 1);
          DLOGKL(" a ", t);
          ans = N - t - 1;
        }else if (W[N - 1] - yr <= x) {
          auto check = [&](DAT a) -> bool { return W[N - 1] - a.second <= x; };
          ll t = st.binsearch_l_from(check, N - 1);
          DLOGKL(" b ", t);
          ans = N - t - 1;
        }else assert(0);
