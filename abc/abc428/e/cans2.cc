...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> ans(N);
  auto [diam, _nd0, _nd1, ct0, ct1] = tr.diameter();
  if (ct0 == ct1) { // diam: even
    tr.change_root(ct0);
    // ll d0 = diam / 2;
    vector<ll> party(N);
    vector<ll> farnode(N, -1LL);
    vector<ll> pdepth(N);
    auto dfs = [&](auto rF, ll nd, ll pt) -> void {
      party[nd] = pt;
      ll thisD = tr.depth(nd);
      if (thisD > pdepth[pt]) {
        pdepth[pt] = thisD;
        farnode[pt] = nd;
      }else if (thisD == pdepth[pt] and nd > farnode[pt]) {
        farnode[pt] = nd;
      }
      for (ll c : tr.children(nd)) rF(rF, c, pt);
    };
    for (ll c : tr.children(ct0)) dfs(dfs, c, c);
    DLOGK(party, farnode, pdepth);
    vector<pll> tmp;
    REP(i, 0, N) {
      if (party[i] == i) tmp.emplace_back(pdepth[i], farnode[i]);
    }
    ranges::sort(tmp, greater<pll>());
    DLOGK(tmp);
    REP(i, 0, N) {
      if (i == ct0) ans[i] = tmp[0].second;
      else {
        ll p = party[i];
        if (farnode[p] == tmp[0].second) ans[i] = tmp[1].second;
        else ans[i] = tmp[0].second;
      }
    }
  }else { // diam: odd
    tr.change_root(ct0);
    ...
    DLOGK(ct0, ct1, d0);
    vector<ll> party(N);
    ll farnode1 = -1, farnode2 = -1;
    auto dfs1 = [&](auto rF, ll nd) -> void {
      party[nd] = 1;
      if (tr.depth(nd) == d0 + 1 and nd >= farnode1) farnode1 = nd;
      for (ll c : tr.children(nd)) rF(rF, c);
    };
    dfs1(dfs1, ct1);
    auto dfs2 = [&](auto rF, ll nd) -> void {
      party[nd] = 2;
      if (tr.depth(nd) == d0 and nd >= farnode2) farnode2 = nd;
      for (ll c : tr.children(nd)) rF(rF, c);
    };
    party[ct0] = 2;
    for (ll c : tr.children(ct0)) if (c != ct1) dfs2(dfs2, c);
    DLOGK(party, farnode1, farnode2);
    REP(i, 0, N) {
      if (party[i] == 1) ans[i] = farnode2;
      else ans[i] = farnode1;
    }
  }
