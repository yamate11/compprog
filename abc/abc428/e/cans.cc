...
int main(/* int argc, char *argv[] */) {
  ...
  pll unit(-1, -1);
  auto myadd = [](pll x, pll y) -> pll {
    auto [distx, ndx] = x;
    auto [disty, ndy] = y;
    return (distx > disty) or (distx == disty and ndx > ndy) ? x : y;
  };
  auto mod1 = [](pll x, ll cld, ll nd) -> pll { return pll(x.first + 1, x.second); };
  auto mod2 = [&](pll x, ll nd) -> pll { return x == unit ? pll(0, nd) : x; };
  auto ans = reroot(tr, unit, myadd, mod1, mod2);
  REP(i, 0, N) cout << ans[i].second + 1 << "\n";
