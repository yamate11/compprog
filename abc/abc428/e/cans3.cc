...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpNbrList(N, N - 1, nbr, dec=1) [5Ja6LaSt]
  ...
  auto func = [&](ll nd0) -> vector<ll> {
    vector<ll> dist(N);
    auto dfs = [&](auto rF, ll nd, ll d, ll pt) -> void {
      dist[nd] = d;
      for (ll c : nbr[nd]) {
        if (c == pt) continue;
        rF(rF, c, d + 1, nd);
  ...
  auto vec1 = func(0);
  ll nd1 = -1, vmax1 = 0;
  REP(i, 0, N) if (vmax1 <= vec1[i]) { vmax1 = vec1[i]; nd1 = i; }
  auto vec2 = func(nd1);
  ll nd2 = -1, vmax2 = 0;
  REP(i, 0, N) if (vmax2 <= vec2[i]) { vmax2 = vec2[i]; nd2 = i; }
  auto vec3 = func(nd2);
  vector<ll> ans(N);
  REP(i, 0, N) ans[i] = vec2[i] > vec3[i] ? nd1 : vec2[i] < vec3[i] ? nd2 : nd1 > nd2 ? nd1 : nd2;
