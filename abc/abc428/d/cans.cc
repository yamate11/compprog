...
int main(/* int argc, char *argv[] */) {
  ...
  auto func1 = [&](ll y) -> ll {
    auto sat = [&](ll q) -> bool { return y <= q * q; };
    long double yy = y;
    ll p = llround(ceil(sqrt(yy)));
    if (sat(p) and not sat(p - 1)) return p;
    if (sat(p - 1) and not sat(p - 2)) return p - 1;
    if (sat(p + 1) and not sat(p)) return p + 1;
    assert(0);
  };
  auto func2 = [&](ll y) -> ll {
    auto sat = [&](ll q) -> bool { return q * q <= y; };
    long double yy = y;
    ll p = llround(floor(sqrt(yy)));
    if (sat(p) and not sat(p + 1)) return p;
    if (sat(p - 1) and not sat(p)) return p - 1;
    if (sat(p + 1) and not sat(p + 2)) return p + 1;
    assert(0);
  };
  auto func3 = [&](ll a, ll b) -> ll {
    ll p1 = func1(a);
    ll p2 = func2(b);
    ll ret;
    if (p2 < p1) {
  ...
  auto solve = [&]() -> ll {
    ...
    ll ans = 0;
    if (w0 == w1) {
      ll a = C * du.pow(w0) + C + 1;
      ll b = C * du.pow(w0) + C + D;
      ans = func3(a, b);
      DLOGKL("w0==w1", a, b, ans);
      return ans;
    }
    if (C != du.pow(w0) - 1) {
      ll a = C * du.pow(w0) + C + 1;
      ll b = (C + 1) * du.pow(w0) - 1;
      ans += func3(a, b);
    }
    for (ll w = w0 + 1; w < w1; w++) {
      ll a = (C * 10 + 1) * du.pow(w - 1);
      ll b = (C + 1) * du.pow(w) - 1;
      ans += func3(a, b);
    }
    {
      ll a = (C * 10 + 1) * du.pow(w1 - 1);
      ll b = C * du.pow(w1) + C + D;
      ans += func3(a, b);
    }
    return ans;
  };
