  ...
  auto get = [&](ll a, ll c, ll W) -> Fp {
    ...
    REP(ex, a + c) {
      REP(x, ex + 1) {
        ...
        ll remC = c - y;
        DLOGKL("  .", remA, remC, W);
        if (remA * remC < W) continue;
        Fp thisv = cb.comb(a, remA) * cb.comb(c, remC) * cb.comb(remA * remC, W);
        DLOGKL("  ", a, c, remA, remC, thisv);
        ret += sgn * thisv;
      }
      sgn = -sgn;
    }
    DLOGKL("get", a, c, W, ret);
    return ret;
  };
  ll N, M, B, W; cin >> N >> M >> B >> W;
  vector recW(N + 1, vector(M + 1, Fp(0)));
  vector recB(N + 1, vector(M + 1, Fp(0)));
  REP2(a, 1, N + 1) REP2(c, 1, M + 1) recW[a][c] = get(a, c, W);
  REP2(b, 1, N + 1) REP2(d, 1, M + 1) recB[b][d] = get(b, d, B);
  Fp ans = 0;
  REP2(a, 1, N) {
    REP2(b, 1, N - a + 1) {
      REP2(c, 1, M) {
        REP2(d, 1, M - c + 1) {
          if (a * c < W or b * d < B) continue;
          ans += cb.comb(N, a) * cb.comb(N - a, b) * cb.comb(M, c) * cb.comb(M - c, d) * recW[a][c] * recB[b][d];
        }
      }
    }
  }
