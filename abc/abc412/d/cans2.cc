...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](ll x)  {
    ...
    IntPerm ip(m - 1, m - 1);
    while (ip.get()) {
      if (ip.at(0) > ip.at(m - 2)) continue;
      vector<ll> cand{i0};
      REP(i, 0, m - 1) cand.push_back(rec[ip.at(i)]);
      ret.push_back(move(cand));
    }
    return ret;
  };
  ...
  // DLOGK(all, f(all));
  for (auto& p : cand0) {
    vector c0(N, vector(N, false));
    REP(i, 0, ssize(p) - 1) {
      c0[p[i]][p[i + 1]] = c0[p[i + 1]][p[i]] = true;
    }
    c0[p[0]][p.back()] = c0[p.back()][p[0]] = true;

    ll cnt = 0;
    REP(i, 0, N) REP(j, i + 1, N) {
      if (conn[i][j] != c0[i][j]) cnt++;
    }
    ans = min(ans, cnt);
    // DLOGKL("aaa", p, cnt);
  }
  ...
  REP(x, 0, 1LL << N) {
    ...
    if (a >= 3 and b >= 3 and a <= b) {
      ...
      auto cand1 = f(x);
      for (auto& p : cand1) {
        vector c1(N, vector(N, false));
        REP(i, 0, ssize(p) - 1) {
          c1[p[i]][p[i + 1]] = c1[p[i + 1]][p[i]] = true;
        }
        c1[p[0]][p.back()] = c1[p.back()][p[0]] = true;
        auto cand2 = f(y);
        DLOGK(x, y);
        for (auto& q : cand2) {
          DLOGK(p, q);
          vector c2(N, vector(N, false));
          REP(i, 0, ssize(q) - 1) {
            c2[q[i]][q[i + 1]] = c2[q[i + 1]][q[i]] = true;
          }
          c2[q[0]][q.back()] = c2[q.back()][q[0]] = true;
          ll cnt = 0;
          REP(i, 0, N) REP(j, i + 1, N) {
