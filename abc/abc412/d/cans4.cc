...
int main(/* int argc, char *argv[] */) {
  ...
  while (ip.get()) {
    auto check = [&]() -> bool {
      REP(i, 0, N) if (ip.at(i) == i or ip.at(ip.at(i)) == i) return false;
      return true;
    };
    if (check()) {
      DLOGK(ip.vec_view());
      vector c0(N, vector(N, false));
      REP(i, 0, N) c0[i][ip.at(i)] = c0[ip.at(i)][i] = true;
      ll cnt = 0;
      REP(i, 0, N) REP(j, i + 1, N) if (conn[i][j] != c0[i][j]) {
        DLOGKL("  ", i, j, conn[i][j], c0[i][j]);
        cnt++;
      }
      ans = min(ans, cnt);
    }
