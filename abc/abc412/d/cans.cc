...
int main(/* int argc, char *argv[] */) {
  ...
  auto g = [&](ll x) {
    vector<ll> ret;
    REP(i, 0, N) if (x >> i & 1) ret.push_back(i);
    return ret;
  };

  auto f = [&](auto& ip, ll n) {
    vector ret(N, vector(N, false));
    REP(i, 0, n) {
      ll a = ip[i];
      ll b = ip[(i + 1) % n];
  ...
  REP(x, 0, 1LL << N) {
    ...
    if (nx == N or
      ...
      auto tx = g(x);
      auto ty = g(y);
      IntPerm ipx(nx, nx, tx);
      while (ipx.get()) {
        if (ipx[0] != tx[0] or ipx[1] > ipx[nx - 1]) continue;
        auto cx = f(ipx, nx);
        IntPerm ipy(ny, ny, ty);
        while (ipy.get()) {
          auto cy = f(ipy, ny);
          ll cnt = 0;
          REP(i, 0, N) REP(j, i + 1, N) if (conn[i][j] != (cx[i][j] or cy[i][j])) cnt++;
          ans = min(ans, cnt);
        }
      }
    }
