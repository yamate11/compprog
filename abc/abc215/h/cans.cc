...
int main(/* int argc, char *argv[] */) {
  ...
  DLOGK(A, B);
  auto C = vector(N, vector(M, 0LL));
  for (ll i = 0; i < N; i++) cin >> C[i];
  ll totA = 0; for (ll i = 0; i < N; i++) { totA += A[i]; }
  ll powN = 1LL << N;
  vector<ll> prep(powN), req(powN);
  for (ll x = 0; x < powN; x++) {
    for (ll i = 0; i < N; i++) if ((x >> i) & 1) prep[x] += A[i];
  }
  for (ll j = 0; j < M; j++) {
    ll x = 0;
    for (ll i = 0; i < N; i++) if (C[i][j]) x |= (1LL << i);
    req[x] += B[j];
  }
  zeta_lower(req);
  DLOGK(prep, req);
  ...
  ll eat = vmin + 1;
  vector<ll> sat(powN);
  for (ll x = 0; x < powN; x++) if (prep[x] - req[x] == vmin) sat[x] = 1;
  auto subsat(sat); zeta_upper(subsat);
  Comb cb(totA);
  vector<Fp> cmbnt(powN);
  for (ll x = 0; x < powN; x++) {
    if (prep[x] < eat) continue;
    cmbnt[x] = cb.comb(prep[x], eat);
  }
  moebius_lower(cmbnt);
  Fp ans = 0;
  for (ll x = 0; x < powN; x++) if (subsat[x]) ans += cmbnt[x];
  cout << eat << " " << ans << endl;
  return 0;
}
