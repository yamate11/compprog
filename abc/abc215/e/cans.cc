...
int main(/* int argc, char *argv[] */) {
  ...
  string S; cin >> S;
  ll numC = 10;
  auto cur_init = vector(1LL << numC, vector<Fp>(numC, Fp(0)));
  auto cur = cur_init;
  for (ll i = 0; i < N; i++) {
    auto prev = cur;
    cur = cur_init;
    ...
    // firstly chosen
    cur[1LL << c][c] += 1;
    for (ll x = 0; x < (1LL << numC); x++) {
      for (ll d = 0; d < numC; d++) {
        cur[x][d] += prev[x][d];
        if (d == c) cur[x][c] += prev[x][c];
        else {
          if (!((x >> c) & 1)) cur[x | (1LL << c)][c] += prev[x][d];
        }
      }
    }
  }
  Fp ans = 0;
  for (ll x = 0; x < (1LL << numC); x++) {
    for (ll y = 0; y < numC; y++) ans += cur[x][y];
  }
  cout << ans << endl;
