...
// @@ !! LIM(ipoint binsearch cmpNaive debug)
...
int naive(istream& cin, ostream& cout) {
  ll N; cin >> N;
  vector<IPoint> pts(N);
  for (ll i = 0; i < N; i++) cin >> pts[i];

  ll vmax = 0;
  for (ll i = 0; i < N; i++) {
    for (ll j = i + 1; j < N; j++) {
      ll d = min(abs(pts[i].x - pts[j].x), abs(pts[i].y - pts[j].y));
      vmax = max(vmax, d);
    }
  }
  cout << vmax << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  auto check = [&](ll k) -> bool {
    ...
    ll ymin = big;
    for (;;) {
      for (; p < N && pts[p].x <  pts[q].x + k; p++) {
        bool b = ymax >= pts[p].y + k || ymin <= pts[p].y - k;
        DLOGK(p, q, ymax, ymin, pts[q], b);
        if (b) return true;
      }
      if (p >= N) break;
      for (; q < p && pts[p].x >= pts[q].x + k; q++) {
        ymax = max(ymax, pts[q].y);
        ymin = min(ymin, pts[q].y);
      }
    }
    return false;
  };
  ll ans = binsearch_i<ll>(check, 0, llround(1.1e9));
  cout << ans << endl;
