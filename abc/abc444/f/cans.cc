...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    auto check = [&](ll x) -> bool {
      map<ll, ll, greater<ll>> upper, lower;
      ll num_ge = 0;
      for (ll a : A) {
        if (x <= a) { upper[a]++; num_ge++; }
        else        lower[a]++;
      }
      ll rem = M;
      DLOGK(x, upper, lower, rem);
      while (rem > 0) {
        if (not upper.empty() and upper.begin()->first >= 2 * x - 1) {
          auto& [v, n] = *upper.begin();
          assert(2 * x - 1 <= v);
          ll k = min(rem, n);
          upper[v / 2 + 1] += k;
          if (x <= v / 2) { upper[v / 2] += k; num_ge += k; }
          else            lower[v / 2] += k;
          rem -= k;
          n -= k;
          if (n == 0) upper.erase(upper.begin());
        }else if (not lower.empty()) {
          auto& [v, n] = *lower.begin();
          rem -= (v - 1) * n;
          lower.erase(lower.begin());
        }else if (not upper.empty()) {
          auto& [v, n] = *upper.begin();
          ll m = min(n, divCeil(rem, v - 1));
          num_ge -= m;
          rem -= (v - 1) * m;
          n -= m;
          if (n == 0) upper.erase(upper.begin());
      ...
      bool b = num_ge >= (N + M + 1) / 2;
      DLOGK(x, b);
      return b;
    };

    ll amax = *ranges::max_element(A);
    ll ans = binsearch<ll>(check, 1, amax + 1);
    return ans;
  };
