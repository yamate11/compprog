...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  Comb cb(2*N);
  vector likes(2*N, vector(2*N, false));
  for (ll i = 0; i < M; i++) {
    ll a, b; cin >> a >> b; a--; b--;
    likes[a][b] = true;
    likes[b][a] = true;
  }
  vector eff(2*N + 1, vector(2*N + 1, false));
  vector memo(2*N + 1, vector(2*N + 1, Fp(0)));
  auto func = [&](auto rF, ll x, ll y) -> Fp {
    Fp& ret = memo[x][y];
    if (! eff[x][y]) {
      eff[x][y] = true;
      ret = Fp(0);
      if (x == y) {
        ...
        for (ll z = x + 2; z <= y; z++) {
          if (! likes[x][z - 1]) continue;
          Fp v1 = rF(rF, x + 1, z - 1);
          Fp v2 = rF(rF, z, y);
          Fp n1 = (z - x) / 2;
          Fp n2 = (y - z) / 2;
          Fp c = cb.comb(n1 + n2, n1);
          ret += c * v1 * v2;
        }
      }
    }
