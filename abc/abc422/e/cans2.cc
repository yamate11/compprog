...
// @@ !! LIM(ipoint random unordered_map sortedIndex)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, P, type=IPoint) [GcZcY1Nz]
  auto P = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; P[i] = v; }
  // @End [GcZcY1Nz]

  using sta = tuple<ll, ll, ll>;
  auto check = [&](ll i0) -> sta {
    auto ord = sortedIndex(P, [&](const IPoint& a, const IPoint& b) -> bool {
      return IPoint::lt_arg(a - P[i0], b - P[i0]);
    });
    ll cur = 0;
    REP(ii, 0, N - 1) {
      if (ii == 0 or not (P[ord[ii - 1]] - P[i0]).parallel(P[ord[ii]] - P[i0])) {
        cur = 1;
      }else {
        cur++;
      }
      if (2 * cur >= N - 1) {
        IPoint v = P[ord[ii]] - P[i0];
        ll a = v.y;
        ll b = -v.x;
        ll c = v.x * P[i0].y - v.y * P[i0].x;
        return {a, b, c};
      }
    }
    return {0, 0, 0};
