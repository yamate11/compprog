...
// @@ !! LIM(mod f:power matrix debug)
// --> f:gcd mod f:power matrix f:<< debug
...
int main(int argc, char *argv[]) {
  ...
  Fp cur = 0;
  Fp seq = A;
  for (ll d = 0; d <= 18; d++) st.at(d) = (power(10LL, d) - A + (B-1)) / B;
  DLOG("st", st);
  for (ll d = 1; d <= 18; d++) {
    if (st.at(d) <= 0) continue;
    ll b = max(0LL, st.at(d-1));
    ll e = min(L, st.at(d));
    if (b > e) break;
    Matrix<Fp> conv({{power<Fp>(10, d), 1, 0}, {0, 1, Fp(B)}, {0, 0, 1}});
    Matrix<Fp> tconv = power(conv, e-b);
    Matrix<Fp> calced = tconv * Matrix<Fp>({{cur}, {seq}, {1}});
    cur = calced.at(0, 0);
    seq = calced.at(1, 0);
    assert(calced.at(2, 0) == Fp(1));
    DLOG("d=", d, "b=", b, "e=", e, "cur=", cur, "seq=", seq);
  }
  cout << cur << endl;
