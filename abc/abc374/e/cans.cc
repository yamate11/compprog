...
// @@ !! LIM(binsearch f:intDiv cmpNaive debug)
  ...
  auto check = [&](ll W) -> bool {
    ...
    REP(i, 0, N) {
      ...
      REP(a, 0, 1e10) {
        ll b;
        if (A[i] * a >= W) b = 0;
        else b = divCeil(W - A[i] * a, B[i]);
        ll thisCost = P[i] * a + Q[i] * b;
        cost = min(cost, thisCost);
        if (b == 0) break;
      }
      totCost += cost;
    }
    bool ret = totCost <= X;
    DLOGK(W, ret, totCost, X);
    return totCost <= X;
  };
...
int body(istream& cin, ostream& cout) {
  ...
  auto check = [&](ll W) -> bool {
    ...
    REP(i, 0, N) {
      ...
      REP(b, 0, A[i]) {
        ll rem = W - B[i] * b;
        ll a = 0;
        if (rem > 0) a = divCeil(rem, A[i]);
        cost2 = min(cost2, P[i] * a + Q[i] * b);
      }
      cost += cost2;
  ...
  ll W = binsearch<ll>(check, 0, (ll)1e9 + 1);
  cout << W << endl;
