...
int main(/* int argc, char *argv[] */) {
  ...
  cin >> F;
  ll Q; cin >> Q;
  // @InpMVec(Q, ((A, dec=1), (B, dec=1), (Y, dec=0), (C, dec=1), (D, dec=1), (Z, dec=0))) [bOR4DQ24]
  auto A = vector(Q, ll());
  auto B = vector(Q, ll());
  auto Y = vector(Q, ll());
  auto C = vector(Q, ll());
  ...
  for (int i = 0; i < Q; i++) {
    ...
    ll v3; cin >> v3; v3 -= 0; Y[i] = v3;
    ll v4; cin >> v4; v4 -= 1; C[i] = v4;
    ll v5; cin >> v5; v5 -= 1; D[i] = v5;
    ll v6; cin >> v6; v6 -= 0; Z[i] = v6;
  }
  // @End [bOR4DQ24]
  ...
  vector<ll> yeses(Q, 1LL);
  vector<ll> noes(Q, maxht + 1);
  vector<ll> ans(Q, -1LL);
  while (true) {
    UnionFind uf(H * W);
    vector mids(maxht + 1, vector<ll>());
    bool done = true;
    REP(q, 0, Q) {
      ll y = yeses[q], n = noes[q];
      if (y + 1 < n) {
        mids[(y + n) / 2].push_back(q);
        done = false;
      }
    }
    if (done) break;
    REPrev(ht, maxht, 1) {
      for (auto bi : Ht[ht]) {
        for (auto dir : BrdIdx::nbr4) {
          auto bj = bi + dir;
          if (F.at(bj) >= F.at(bi)) {
            uf.merge(F.enc(bi), F.enc(bj));
            DLOGKL("merge", ht, bi, bj);
          }
        }
      }
      for (ll q : mids[ht]) {
        if (uf.leader(F.enc(A[q], B[q])) == uf.leader(F.enc(C[q], D[q]))) yeses[q] = ht;
        else noes[q] = ht;
      }
    }
  }
  DLOGK(yeses, noes);
  REPOUT(q, 0, Q, (Y[q] >= yeses[q] and Z[q] >= yeses[q]) ? Y[q] - yeses[q] + Z[q] - yeses[q] : abs(Y[q] - Z[q]), "\n");
