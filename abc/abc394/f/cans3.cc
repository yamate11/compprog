...
// @@ !! LIM(input tree forall f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  vector done(N, false);
  vector<ll> cands;
  vector<vector<ll>> wt(N);
  REP(i, 0, N) {
    if (ssize(nbr[i]) <= 3) {
      for (ll j : nbr[i]) {
        if (done[j]) continue;
        if (ssize(nbr[j]) <= 3) continue;
        wt[j].push_back(1);
        if (ssize(wt[j]) >= ssize(nbr[j]) - 1) cands.push_back(j);
      }
      done[i] = true;
    }
  }
  ...
  while (not cands.empty()) {
    ll i = cands.back(); cands.pop_back();
    sort(ALL(wt[i]), greater<ll>());
    if (ssize(wt[i]) >= 4) {
      updMax(ans, 1 + wt[i][0] + wt[i][1] + wt[i][2] + wt[i][3]);
    }
    if (ssize(wt[i]) == ssize(nbr[i]) - 1) {
      ll j0 = EXFIND_C(j, nbr[i], not done[j]);
      wt[j0].push_back(1 + wt[i][0] + wt[i][1] + wt[i][2]);
      if (ssize(wt[j0]) >= ssize(nbr[j0]) - 1) cands.push_back(j0);
    }
    done[i] = true;
  }
  cout << ans << endl;
