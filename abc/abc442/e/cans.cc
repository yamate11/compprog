...
// @@ !! LIM(ipoint sortedIndex debug binsearch)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  // @InpVec(N, P, type=IPoint) [QTjeUXP4]
  auto P = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; P[i] = v; }
  // @End [QTjeUXP4]
  auto A = P;
  ranges::sort(A, IPoint::lt_arg);

  REP(i, 0, Q) {
    ll a, b; cin >> a >> b; a--; b--;
    ll ans = ranges::upper_bound(A, P[a], IPoint::lt_arg) - ranges::lower_bound(A, P[b], IPoint::lt_arg);
    if (ans <= 0) ans += N;
    cout << ans << "\n";
  }
  return 0;
