...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> bool {
    ll H, W; cin >> H >> W;
    // @InpGrid(H, W, A, type=ll) [Cakx2pSJ]
    ...
    REP(i, 0, H) {
      ...
      if (vmax == LLONG_MIN) continue;
      rec.emplace_back(vmin, vmax);
    }
    if (rec.empty()) return true;
    sort(ALL(rec));
    REP(i, 1, SIZE(rec)) if (rec[i - 1].second > rec[i].first) return false;
    vector<pll> edges;
    ll seq = W;
    ll top = W;
    seq = top + 1;
    REP(i, 0, H) {
      ...
      for (auto& [e, v] : mp) {
        if (SIZE(v) == 1) {
          edges.emplace_back(prev, v[0]);
          prev = v[0];
        }else {
          ll f = seq; seq++;
          ll b = seq; seq++;
          edges.emplace_back(prev, f);
          for (ll j : v) {
            edges.emplace_back(f, j);
            edges.emplace_back(j, b);
          }
          prev = b;
        }
      }
    ...
    SCC scc(seq);
    for (auto& [j1, j2] : edges) {
      DLOGKL("addedge", j1, j2);
      scc.addEdge(j1, j2);
    }
    REP(j, 0, W) {
      auto& vvv = scc.nodesInCC(scc.ccForNode(j));
      DLOGK(j, vvv);
      if (vvv.size() > 1) {
        return false;
      }
