...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpVec(N, A, dec=1) [mv4XBeNX]
  auto A = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; A[i] = v; }
  // @End [mv4XBeNX]
  ll Q; cin >> Q;
  // @InpVec(Q, X, dec=1) [5E1mCpH3]
  auto X = vector(Q, ll());
  ...
  vector<ll> cnt(M);
  REP(i, 0, N) cnt[A[i]]++;
  ll K = *ranges::max_element(cnt);
  vector bc(K + 1, vector<ll>());
  REP(m, 0, M) bc[cnt[m]].push_back(m);
  vector T(K + 2, 0LL);
  REP(k, 0, K + 1) T[k + 1] = T[k] + ssize(bc[k]);
  vector TT(K + 3, 0LL);
  REP(k, 0, K + 2) TT[k + 1] = TT[k] + T[k];
  DLOGK(cnt);
  DLOGK(bc);
  DLOGK(T);
  DLOGK(TT);
  ...
  ll K_BB = 0;
  ll K_Q = 1;
  REP(k, 0, K + 1) ev.emplace_back(TT[k + 1], K_BB, k);
  REP(q, 0, Q) {
    ll x = X[q];
    if (x < N) ans[q] = A[x];
    else if (x >= K * M) ans[q] = x % M;
    else ev.emplace_back(x - N, K_Q, q);
  }
  ranges::sort(ev);
  DLOGK(ev);
  auto st = make_seg_tree(0LL, plus<ll>(), vector<ll>(M, 0LL));
  ll boundary = -1;
  for (auto [t, kd, idx] : ev) {
    if (kd == K_BB) {
      for (ll y : bc[idx]) st.rs(y) = 1;
      boundary = t;
    }else if (kd == K_Q) {
      ll i = t - boundary;
      auto check = [&](ll val) -> bool { return val <= i; };
      ll a = st.binsearch_r_until(check, 0);
      ans[idx] = a;
    }else assert(0);
  }
  REPOUT(q, 0, Q, ans[q] + 1, "\n");
  return 0;
}
