...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) cnt[A[i]]++;
  vector<pll> P(M);
  REP(i, 0, M) P[i] = pll(cnt[i], i);
  ranges::sort(P);
  vector<ll> tmp(M);
  REP(i, 0, M) tmp[i] = P[i].first;
  auto T = rle(tmp);
  vector<ll> vec;
  ll acc = 0;
  REP(i, 0, ssize(T) - 1) {
    ...
    acc += n0;
    assert(c1 > c0);
    REP(j, 0, c1 - c0) vec.push_back(acc);
  }
  ll K = T.back().first;
  DLOGK(P);
  DLOGK(T);
  DLOGK(vec);
  ll sz = ssize(vec);
  vector Svec(sz + 1, 0LL);
  REP(i, 0, sz) Svec[i + 1] = Svec[i] + vec[i];
  assert(Svec[sz] + N == K * M);
  ...
  for (auto [x, _q] : qs) {
    ll ans;
    if (x < N) ans = A[x];
    else if (x >= K * M) ans = (x - K * M) % M;
    else {
      ll x0 = x - N;
      ll j0 = ranges::lower_bound(Svec, x0 + 1) - Svec.begin() - 1;
      ll y0 = x0 - Svec[j0];
      REP(k, prev_added + 1, vec[j0]) {
        DLOGKL("  ", x, k, P[k].second);
        os.insert(P[k].second);
      }
      prev_added = vec[j0] - 1;
      ans = *os.find_by_order(y0);
      DLOGK(x, x0, j0, y0, Svec);
    }
    vans[_q] = ans;
  }
  REPOUT(i, 0, Q, vans[i] + 1, "\n");
