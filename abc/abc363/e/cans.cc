...
int main(/* int argc, char *argv[] */) {
  ...
  ll lim = 1e5;
  Board<ll> brd(H, W, 0);
  vector rev(lim + 1, vector<BrdIdx>());
  REP(i, 0, H - 2) {
    REP(j, 0, W - 2) {
      ll a; cin >> a;
      brd.rs(i + 1, j + 1) = a;
      rev[a].push_back(BrdIdx(i + 1, j + 1));
    }
  }
  UnionFind uf(H * W);
  REP(i, 0, H - 1) {
    uf.merge(brd.enc(i, 0), brd.enc(i + 1, 0));
    uf.merge(brd.enc(i, W - 1), brd.enc(i + 1, W - 1));
  }
  REP(i, 0, W - 1) {
    uf.merge(brd.enc(0, i), brd.enc(0, i + 1));
    uf.merge(brd.enc(H - 1, i), brd.enc(H - 1, i + 1));
  }
  BrdIdx bi00(0, 0);
  ll base_size = uf.group_size(brd.enc(bi00));
  REP(y, 1, Y + 1) {
    DLOGK(y, rev[y]);
    for (auto bi : rev[y]) {
      DLOGK(y, bi);
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (brd.at(bj) <= brd.at(bi)) uf.merge(brd.enc(bj), brd.enc(bi));
      }
    }
    ll sunk = uf.group_size(brd.enc(bi00)) - base_size;
    ll rem = (H - 2) * (W - 2) - sunk;
    cout << rem << "\n";
  }
