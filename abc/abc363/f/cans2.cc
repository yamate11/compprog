...
using vll = vector<ll>;
// @DefStruct(foo, (val, (tp1, vll), (tp2, vll), (tp3, vll)), ord=val, istr=False, ostr=True) [9pVe0fYV]
struct foo_t {
  ll val;
  vll tp1;
  vll tp2;
  vll tp3;
  // foo_t() {}
  foo_t(ll val_ = ll(), vll tp1_ = vll(), vll tp2_ = vll(), vll tp3_ = vll()) : val(val_), tp1(tp1_), tp2(tp2_), tp3(tp3_) {}
  bool operator<(const foo_t& o) const {
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> cat1;
  vector<pll> cat2;
  for (ll d : divs) ssdivs.insert(d);
  for (ll d : divs) {
    if (d == 1) continue;
    string s = du.to_string(d);
    if (EXISTS(j, 0, ssize(s), s[j] == '0')) continue;
    reverse(ALL(s));
    ll r = du.from_string(s);
    if (d == r) {
      cat1.push_back(d);
    }
    if (d <= r and ssdivs.contains(r)) {
      if (d <= N / r) {
        cat2.emplace_back(d, r);
      }
    }
  }
  auto pout = [&](foo_t& q) {
    DLOGKL("pout", q.val, q.tp1, q.tp2, q.tp3);
    vector<ll> aa;
    for (ll x : q.tp2) aa.push_back(x);
    for (ll x : q.tp1) aa.push_back(x);
    reverse(ALL(q.tp3));
    for (ll x : q.tp3) aa.push_back(x);
    REP(i, 0, ssize(aa) - 1) {
      cout << aa[i] << '*';
    }
    cout << aa.back() << "\n";
  ...
  foo_t unit;
  unit.val = 1;
  set<foo_t> cands{unit};
  for (ll d: cat1) {
    auto prev = cands;
    for (auto& p : prev) {
      if (p.val <= N / d and p.tp1.empty()) {
        foo_t q = p;
        q.val = p.val * d;
        q.tp1.push_back(d);
        DLOGK(d, q.val, q.tp1, q.tp2, q.tp3);
        if (q.val == N) {
          pout(q);
          return 0;
        }
