...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> bool {
    ...
    cin >> brd;
    BrdIdx start;
    REP(i, 0, H) REP(j, 0, W) if (brd.at(i, j) == 'S') { start = BrdIdx(i, j); break; }
    auto func = [&](BrdIdx s, BrdIdx g) -> void {
      Board<bool> visited(H, W, false);
      auto dfs = [&](auto rF, BrdIdx x) -> void {
        if (x == g) throw myexc();
        if (visited.at(x)) return;
        visited.at(x) = true;
        for (BrdIdx d : BrdIdx::nbr4) if (brd.at(x + d) == '.') rF(rF, x + d);
      };
      dfs(dfs, s);
    };

    vector<BrdIdx> sg;
    for (BrdIdx d : BrdIdx::nbr4) if (brd.at(start + d) == '.') sg.push_back(start + d);
    ll sz = SIZE(sg);
    if (sz <= 1) return false;
    try {
      REP(i, 0, sz) REP(j, i + 1, sz) func(sg[i], sg[j]);
    }catch( myexc& e) {
      return true;
    }
    return false;
