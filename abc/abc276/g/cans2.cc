...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Fp> vec1(deg0 + 1);
  REP(i, 0, deg0 + 1) vec1[i] = cb.binom(i + 2 - 1, 2 - 1);
  Pol p1(move(vec1));
  
  vector<Fp> vec2(deg0 + 1);
  REP(i, 0, deg0 + 1) {
    if (3*i >= deg0 + 1) break;
    vec2[3 * i] = cb.binom(i + N - 1 - 1, N - 1 - 1);
  }
  Pol p2(move(vec2));

  vector<Fp> vec3(N - 1 + 1);
  REP(i, 0, N - 1 + 1) {
    vec3[i] = cb.binom(N - 1, i);
  }
  Pol p3(move(vec3));
  p3.selfCutoff(deg0);
  
  // Pol pp = ((p2 * p3).cutoff(deg0) * p1).cutoff(deg0);

  Pol q = p2;

  cerr << "    " << q.getCoef(0) << " " << p3.getCoef(0) << " " << q.degree() << " " << p3.degree() << endl;

  q *= p3;

  cerr << q.degree() << endl;
  REP(i, 0, 10) cerr << q.getCoef(i) << " ";
  cerr << endl;
  REP(i, 0, 10) cerr << q.getCoef(deg0 - i) << " ";
  cerr << endl;
  ...
  cerr << "  " << q.getCoef(0) << " " << p1.getCoef(0) << endl;

  q *= p1;

  cerr << "  " << q.getCoef(0) << endl;
