...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    vert[X[i]].insert(Y[i]);
    horiz[Y[i]].insert(X[i]);
  }

  ll ans = 0;

  auto doit = [&](ll p, ll q, auto& vvec, ll t, auto& wvec) -> void {
    auto it = vvec.find(t);
    if (it == vvec.end()) return;
    auto& dset = vvec[t];
    if (p > q) swap(p, q);
    auto it2 = dset.lower_bound(p);
    while (it2 != dset.end() and *it2 <= q) {
      ll e = *it2;
      it2 = dset.erase(it2);
      wvec[e].erase(t);
      ans++;
  ...
  REP(i, 0, M) {
    ...
    ll c; cin >> c;
    IPoint dir;
    if (d == 'U') dir = IPoint(0, 1);
    if (d == 'D') dir = IPoint(0, -1);
    if (d == 'L') dir = IPoint(-1, 0);
    if (d == 'R') dir = IPoint(1, 0);
    IPoint nxt = cur + dir * c;
    if (cur.x == nxt.x) {
      doit(cur.y, nxt.y, vert, cur.x, horiz);
    }else {
      doit(cur.x, nxt.x, horiz, cur.y, vert);
    }
    cur = nxt;
  }
  cout << cur.x << " " << cur.y << " " << ans << endl;
