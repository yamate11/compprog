...
int main(/* int argc, char *argv[] */) {
  ...
  ll maxv = 0;
  auto dfs = [&](auto rF, ll nd, ll cforp) -> void {
    vector<ll> vec;
    if (nd > 0 and cforp > 0) vec.push_back(cforp);
    for(ll c : tr.children(nd)) {
      ll k = tr.num_children(c);
      if (k > 0) vec.push_back(k);
      ll e;
      if (nd == 0) e = tr.num_children(nd) - 1;
      else         e = tr.num_children(nd);
      rF(rF, c, e);
    }
    if (ssize(vec) > 0) {
      sort(ALL(vec));
      ll vv = 0;
      REP(i, 0, ssize(vec)) vv = max(vv, 1 + (ssize(vec) - i) * (1 + vec[i]));
      maxv = max(maxv, vv);
    }
  };
  dfs(dfs, 0, 0);
