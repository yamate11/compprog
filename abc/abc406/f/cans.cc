...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> initw(2*N);
  REP(i, 0, N) {
    initw[tr.euler_in(i)] = 1;
  }
  auto st = make_seg_tree(0LL, plus<ll>(), initw);
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      ll y; cin >> y; y--;
      auto [pt, cld] = tr.nodes_of_edge(y);
      if (tr.parent(pt) == cld) swap(pt, cld);
      ll i0 = tr.euler_in(cld);
      ll i1 = tr.euler_out(cld);
      ll w_sub = st.query(i0, i1);
      ll w_oth = totw - w_sub;
      cout << abs(w_sub - w_oth) << "\n";
    }
  }
