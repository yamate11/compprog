...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    vector tbl_init(2, vector(K + 1, sta()));
    auto tbl = tbl_init;
    tbl[1][0] = sta(Fp(1), Fp(0));
    REPrev(j, sz - 1, 0) {
      ll d = vec[j];
      auto prev = move(tbl);
      tbl = tbl_init;
      REP(eq, 0, 2) REP(k, 0, K + 1) {
        auto [num, sum] = prev[eq][k];
        REP(x, 0, 2) {
          if (eq and x > d) continue;
          ll new_eq = eq and x == d;
          ll new_k = (x == 1) ? k + 1 : k;
          if (new_k > K) continue;
          auto [onum, osum] = tbl[new_eq][new_k];
          tbl[new_eq][new_k] = sta(onum + num, osum + sum * 2 + num * x);
        }
      }
    }
    auto [num0, sum0] = tbl[0][K];
    auto [num1, sum1] = tbl[1][K];
    cout << sum0 + sum1 << "\n";
  };
