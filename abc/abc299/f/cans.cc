...
int main(/* int argc, char *argv[] */) {
  ...
  vector<int> S;
  string _SS; cin >> _SS;
  for (char c : _SS) S.push_back(c - 'a');
  ll N = SIZE(S);
  ll nc = 26;
  if (N == 1) { cout << 0 << endl; return 0; }
  vector nxt(nc, vector(N + 1, N));
  REP(c, 0, nc) REPrev(i, N - 1, 0) nxt[c][i] = c == S[i] ? i : nxt[c][i + 1];
  
  auto func = [&](ll sep) -> Fp {
    ll t = S[sep];
    vector tbl(N, vector(N, Fp(0)));
    ll a0 = nxt[t][0];
    if (a0 >= sep) return Fp(0);
    tbl[a0][sep] = Fp(1);
    REP(i, a0, sep) REP(j, sep, N) REP(c, 0, nc) {
      ll a = nxt[c][i + 1];
      ll b = nxt[c][j + 1];
      if (a >= sep or b >= N) continue;
      tbl[a][b] += tbl[i][j];
    }
    Fp ret = 0;
    REP(i, a0, sep) REP(j, sep, N) {
      DLOGK("  ", sep, i, j, a0, t, nxt[t][a0 + 1]);
      if (nxt[t][i + 1] == sep) {
        DLOGK("   yes", i, j, tbl[i][j]);
        ret += tbl[i][j];
      }else {
        DLOGK("    no", i, j);
      }
