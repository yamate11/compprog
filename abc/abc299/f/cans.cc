...
int main(/* int argc, char *argv[] */) {
  ...
  ll N = ssize(S);
  ll sigma = 26;
  vector na(N, vector(sigma, 0LL));
  REP(i, 0, sigma) na[N - 1][i] = N;
  REPrev(i, N - 2, 0) {
    na[i] = na[i + 1];
    na[i][S[i + 1] - 'a'] = i + 1;
  }
  ...
  REP(st, 1, N) {
    ...
    ll fst = 0;
    while (S[fst] != S[st]) fst++;
    if (fst < st) {
      tbl[fst][st] = 1;
      REP(i, fst, st) REP(j, st, N) {
        if (tbl[i][j] != Fp(0)) {
          REP(c, 0, sigma) {
            ll p = na[i][c];
            ll q = na[j][c];
            if (p < st and q < N) tbl[p][q] += tbl[i][j];
          }
        }
      }
      ll th = st - 1;
      while (S[th] != S[st]) th--;
      REP(i, th, st) REP(j, st, N) ans += tbl[i][j];
    }
  }
  cout << ans << endl;
  return 0;
