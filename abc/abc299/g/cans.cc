...
int body(istream& cin, ostream& cout) {
  ...
  vector poss(M + 1, vector<ll>());
  REP(i, 0, N) poss[A[i]].push_back(i);

  pll unit{LLONG_MAX, LLONG_MAX};
  vector<pll> init_vec;
  REP(i, 0, N) init_vec.emplace_back(A[i], i);
  auto myadd = [&](pll a, pll b) -> pll { return min(a, b); };
  auto st = make_seg_tree<pll>(unit, myadd, init_vec);

  vector<pll> vec;
  vector<bool> appear(M + 1);
  REPrev(i, N - 1, 0) {
    if (not appear[A[i]]) {
