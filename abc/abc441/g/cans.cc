...
// @DefStruct(DAT, ((head, bool), (tail, bool), (num, ll)), istr=False, ostr=False) [8URPlMgz]
struct DAT_t {
  bool head;
  bool tail;
  ll num;
  DAT_t(bool head_ = bool(), bool tail_ = bool(), ll num_ = ll()) : head(head_), tail(tail_), num(num_) {}
};
// @End [8URPlMgz]
// @DefStruct(OP, (num, flip), istr=False, ostr=False) [XdOKx8dx]
struct OP_t {
  ll num;
  ll flip;
  OP_t(ll num_ = ll(), ll flip_ = ll()) : num(num_), flip(flip_) {}
};
// @End [XdOKx8dx]
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  auto add = [&](DAT x, DAT y) -> DAT { return DAT(x.head or y.head, x.tail or y.tail, max(x.num, y.num)); };
  auto comp = [&](OP f, OP g) -> OP {
    ll k = f.flip + g.flip;
    if (k >= 3) k -= 2;
    return OP(f.num, k);
  };
  auto apply = [&](OP f, DAT x) -> DAT {
    if (f.flip >= 1) {
      x.num = 0;
      if (f.flip == 1) swap(x.head, x.tail);
    }
    if (x.head > 0) x.num += f.num;
    return x;
  };
  vector init_vec(N, DAT(true, false, 0));
  auto st = make_seg_tree_lazy(DAT(false, false, 0, OP(0, 0), add, comp, apply, init_vec);
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll L, R, X; cin >> L >> R >> X; L--;
      st.update(L, R, OP(X, 0));
    }else if (tp == 2) {
      ll L, R; cin >> L >> R; L--;
      st.update(L, R, OP(0, 1));
    }else if (tp == 3) {
      ll L, R; cin >> L >> R; L--;
      auto [head, tail, num] = st.query(L, R);
      if (head) cout << num << "\n";
      else cout << 0 << "\n";
    }
    DLOGK(_q, st.vec_view());
  }
