...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpNbrList(N, M, fwd, dir=True, dec=1, read=C) [PJJtfCYQ]
  struct fwd_t {
    int nd;
    ll C;
  ...
  // @End [PJJtfCYQ]
  IntDupPerm idp(4, L);
  vector<ll> rec(N, false);
  while (idp.get()) {
    ll cur = 0;
    ll len = 0;
    auto f = [&]() -> void {
      DLOGKL("in", idp.vec_view());
      REP(i, 0, L) {
        if (ssize(fwd[cur]) <= idp[i]) {
          // DLOGK(i, idp[i], cur, fwd[cur]);
          return;
        }
        auto [j, d] = fwd[cur][idp[i]];
        len += d;
        cur = j;
      }
      DLOGK(idp.vec_view(), cur, len);
      if (S <= len and len <= T) {
        rec[cur] = true;
        DLOGKL("  ", cur, true);
      }else {
  ...
  vector<ll> ans;
  REP(i, 0, N) if (rec[i]) ans.push_back(i);
