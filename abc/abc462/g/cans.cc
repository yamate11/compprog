...
// published at https://github.com/yamate11/compprog-clib/blob/master/convolution.cc
...
// published at https://github.com/yamate11/compprog-clib/blob/master/polynomial.cc
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, C, dec=1) [YlnuLgIb]
  auto C = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; C[i] = v; }
  // @End [YlnuLgIb]
  // @InpVec(N, G, dec=1) [JeVxslm7]
  auto G = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; G[i] = v; }
  // @End [JeVxslm7]
  vector<ll> E(N);
  REP(i, 0, N) E[C[i]]++;
  vector<ll> H(N);
  ...
  vector<Pol> F(N);
  REP(i, 0, N) {
    vector<Fp> v(min(E[i], H[i]) + 1);
    REP(j, 0, ssize(v)) {
      // v[j] = Fp(j % 2 == 0 ? 1 : -1) * cb.binom(E[i], j) * cb.binom(H[i], j) * cb.fact(j);
      v[j] = cb.binom(E[i], j) * cb.binom(H[i], j) * cb.fact(j);
    }
    F[i] = Pol(move(v));
  }
  auto comp = [&](ll i, ll j) -> bool { return F[i].degree() > F[j].degree(); };
  priority_queue<ll, vector<ll>, decltype(comp)> pque(comp);
  REP(i, 0, N) pque.push(i);
  while (ssize(pque) >= 2) {
    ll i = pque.top(); pque.pop();
    ll j = pque.top(); pque.pop();
    F[i] *= F[j];
    pque.push(i);
  }
  const auto& f = F[pque.top()];
  Fp ans = 0;
  REP(k, 0, N + 1) {
    ans += Fp(k % 2 == 0 ? 1 : -1) * f.getCoef(k) * cb.fact(N - k);
  }
  ans /= cb.fact(N);
  cout << ans << endl;
  return 0;
