...
int main(/* int argc, char *argv[] */) {
  ...
  auto primes = sieve(M);
  ll sz = ssize(primes);
  // ll lim = llround(floor(sqrt(N)));
  REP(i, 0, sz) {
    ll j = upper_bound(ALL(primes), llround(floor(sqrt(N) / primes[i]))) - primes.begin();
    // if (not (j < sz and primes[i] * primes[j] <= lim)) j--;
    if (i < j) {
      DLOGK(i, j, primes[i], primes[j]);
      cnt += j - i - 1;
      // if (j < sz and lim * lim == N and primes[i] * primes[j] == lim) cnt +
    }
  }
  cout << cnt << endl;
