...
int main(/* int argc, char *argv[] */) {
  ...
  sort(ALL(WUV));
  // @InpVec(K, A, dec=1) [wP6yTTpQ]
  auto A = vector(K, ll());
  for (int i = 0; i < K; i++) { ll v; cin >> v; v -= 1; A[i] = v; }
  // @End [wP6yTTpQ]
  // @InpVec(K, B, dec=1) [MgR9ISMu]
  auto B = vector(K, ll());
  for (int i = 0; i < K; i++) { ll v; cin >> v; v -= 1; B[i] = v; }
  // @End [MgR9ISMu]
  vector<pair<multiset<ll>, multiset<ll>>> vec(N);
  REP(i, 0, K) {
    vec[A[i]].first.insert(A[i]);
    vec[B[i]].second.insert(B[i]);
  }
  UnionFind uf(N);
  ll ans = 0;
  for(auto [w, u, v] : WUV) {
    ...
    auto& [a1, b1] = vec[nl];
    auto& [a2, b2] = vec[ol];
    while (not a1.empty() and not b2.empty()) {
      a1.erase(a1.begin());
      b2.erase(b2.begin());
      ans += w;
    }
    while (not b1.empty() and not a2.empty()) {
      b1.erase(b1.begin());
      a2.erase(a2.begin());
      ans += w;
    }
    for (ll x : a2) a1.insert(x);
    for (ll y : b2) b1.insert(y);
  }
  cout << ans << endl;
