...
// @@ !! LIM(f:updMaxMin debug cmpNaive unordered_map)
...
int naive(istream& cin, ostream& cout) {
  ll H, W; cin >> H >> W;
  // @InpVec(H, S, type=string) [DpMmTKSm]
  auto S = vector(H, string());
  for (int i = 0; i < H; i++) { string v; cin >> v; S[i] = v; }
  // @End [DpMmTKSm]

  auto enc = [&](ll i, ll j) -> ll { return i * W + j; };
  auto dec = [&](ll e) -> pll { return {e / W, e % W}; };

  ll ans = 0;
  vector<ll> vecA;
  REP(i, 0, H) REP(j, 0, W) if (S[i][j] != '#' and (j == 0 or S[i][j - 1] == '#')) vecA.push_back(enc(i, j));
  ll sz = ssize(vecA);
  DLOGK(vecA);
  REP(x, 0, 1LL << sz) {
    safe_uset<ll> ss;
    ...
    while (not que.empty()) {
      auto op = [&](ll i, ll j) -> void {
        if (i < H and 0 <= j and j < W and S[i][j] != '#' and not ss.contains(enc(i, j))) {
          ss.insert(enc(i, j));
          que.push(enc(i, j));
        }
      };
...
int body(istream& cin, ostream& cout) {
  ...
  vector A(H + 1, vector(W, -1LL));
  vector B(H + 1, vector<ll>());
  REP(i, 0, H) {
    ll seq = -1;
    ll j;
    for (j = 0; j < W and S[i][j] == '#'; j++);
    while (j < W) {
      seq++;
      for (; j < W and S[i][j] != '#'; j++) {
        A[i][j] = seq;
        if (ssize(B[i]) == seq) B[i].push_back(0);
        if (S[i][j] == '+') B[i][seq]--;
        else                B[i][seq]++;
      }
      for (; j < W and S[i][j] == '#'; j++);
  ...
  vector D(H + 1, vector<ll>());
  D[H].resize(1);
  D[H][0] = 0;
  REPrev(i, H - 1, 0) {
    ll sz = ssize(B[i]);
    C[i] = vector<ll>(1LL << sz);
    vector E(sz, 0LL);
    REP(j, 0, W) {
      ll a = A[i][j], b = A[i + 1][j];
      if (a >= 0 and b >= 0) E[a] |= 1LL << b;
    }
    REP(x, 0, 1LL << sz) {
      ll v = 0;
      ll s = 0;
