...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> brd(H, W, '#');
  cin >> brd;
  vector<ll> safeR, safeC;
  REP(i, 0, H) if (FORALL(j, 0, W, brd.at(i, j) == '.')) safeR.push_back(i);
  REP(j, 0, W) if (FORALL(i, 0, H, brd.at(i, j) == '.')) safeC.push_back(j);
  ll big = 1LL << 60;
  vector dist(H, vector(W, big));
  queue<BrdIdx> que;
  for (ll i : safeR) for (ll j : safeC) {
      dist[i][j] = 0;
      que.emplace(i, j);
    }
  while (not que.empty()) {
    ...
    for (auto dir : BrdIdx::nbr4) {
      auto bj = bi + dir;
      if (brd.at(bj) == '.' and dist[bj.r][bj.c] == big) {
  ...
  ll ans = 0;
  REP(i, 0, H) REP(j, 0, W) if (dist[i][j] <= K) ans++;
  cout << ans << "\n";
