...
// published at https://github.com/yamate11/compprog-clib/blob/master/rerror.cc
...
// published at https://github.com/yamate11/compprog-clib/blob/master/geometry.cc
...
int main(/* int argc, char *argv[] */) {
  ...
  auto outprod = [&](IPoint a, IPoint b) -> ll { return a.x * b.y - a.y * b.x; };

  ll N, Q; cin >> N >> Q;
  // @InpVec(N, P, type=IPoint) [PenOhJ3z]
  ...
  vector<IPoint> SA(2 * N + 1);
  REP(i, 0, 2*N) SA[i + 1] = SA[i] + A[i];
  vector<ll> SC(2 * N + 1);
  REP(i, 0, 2*N) SC[i + 1] = SC[i] + C[i];
  ...
  REP(_q, 0, Q) {
    ll u, v; cin >> u >> v; u--; v--;
    if (v < u) v += N;
    IPoint B = outprod(P[u], P[v]) * (P[u] + P[v]);
    DLOGK(B);
    DLOGK(SA[v] - SA[u]);
    IPoint X = SA[v] - SA[u] - B;
    double x = X.x;
    double y = X.y;
    ll t = SC[v] - SC[u] - outprod(P[u], P[v]);
    DLOGK(X, t);
    x /= (t * 3);
    y /= (t * 3);
    cout << x << " " << y << "\n";
