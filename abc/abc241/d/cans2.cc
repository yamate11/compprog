...
int main(/* int argc, char *argv[] */) {
  ...
  REP(q, Q) {
    ...
    if (tp[q] == 1) {
      ...
      auto it = mp.lower_bound(v);
      ll i = K[q];
      DLOGKL("type3 ", q, v, it->first, it->second, i);
      while (i > 0 and cc.d(it->first) < big) {
        if (it->second >= i) break;
        i -= it->second;
        it++;
        DLOGKL("   ", i, it->first, it->second);
      }
      if (cc.d(it->first) == big) {
        cout << -1 << "\n";
      }else {
        cout << cc.d(it->first) << "\n";
      }
