...
int main() {
  ...
  for (BrdIdx dir : BrdIdx::nbr8) REP(i, 0, H) REP(j, 0, W) {
      vector<BrdIdx> vec(5);
      REP(k, 0, 5) vec.push_back(BrdIdx(i, j) + k * dir);
      if (FORALL(k, 0, 5, brd.at(vec[k]) == "snuke"[k])) {
        REP(k, 0, 5) cout << vec[k].r + 1 << " " << vec[k].c + 1 << "\n";
        return 0;
      }
    }
  return 0;
