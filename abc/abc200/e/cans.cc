...
// @@ !! LIM(debug polynomial binsearch)
...
int main(/* int argc, char *argv[] */) {
  ...
  PolyLL X = PolyLL::get_X();
  vector<ll> v(3*N + 1);
  v[0] = 1; v[N] = -3; v[2*N] = 3; v[3*N] = -1;
  PolyLL f(move(v));
  PolyLL g = 1LL - X;
  auto f1 = f.divFormalSeries(g, 3*N);
  auto f2 = f1.divFormalSeries(g, 3*N);
  auto f3 = f2.divFormalSeries(g, 3*N);

  ll rem = K - 1;
  ll p = 0;
  for (; p <= 3*(K-1) && f3.coef[p] <= rem; p++) {
    rem -= f3.coef[p];
  }
  DLOGK(p, rem);

  vector<ll> v2(2*N + 1);
  v2[0] = 1; v2[N] = -2; v2[2*N] = 1;
  PolyLL h(move(v2));
  auto h1 = h.divFormalSeries(g, 3*N);
  auto h2 = h1.divFormalSeries(g, 3*N);
  DLOGK(h2);
  // ll a = max(0LL, p - 2*(N-1));
  ll a = 0;
  for (; a <= 2*(N-1) && h2.coef[p - a] <= rem; a++) {
    rem -= h2.coef[p - a];
  }
  ll q = p - a;
  ll b_min = q <= (N - 1) ? 0 : q - (N - 1);
  ll b = b_min + rem;
  ll c = p - a - b;
