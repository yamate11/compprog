...
int main(/* int argc, char *argv[] */) {
...

  {
    ...
    Fp::MOD = primeB;
    vector<pair<PolyFp, PolyFp>> tests3({
        {PolyFp({1, 2, 3, 4}), PolyFp({2, 1})},
        {PolyFp({2, 1}), PolyFp({1, 2, 3, 4})},
        {PolyFp({0, 0, 0, 4}), PolyFp({2, 1})},
        {PolyFp({2, 1}), PolyFp({10, 0, 0, 0})},
        {PolyFp({4, 3, 2, 1}), PolyFp({1, 2, 3, 4})},
        {PolyFp({4}), PolyFp({1, 2, 3, 4})},
        {PolyFp({0}), PolyFp({1, 2, 3, 4})},
        {PolyFp({4, 3, 2, 1}), PolyFp({4})},
      });
    for (auto [p, q] : tests3) {
      auto [d, m] = p.divmod(q);
      assert(p == q * d + m &&
             (m.degree() < q.degree() || (q.degree() == 0 && m == PolyFp(0.0))));
    }
...

  {
    ...
    /*
    Pol p3 = Pol(1), p4 = (Fp(1) - X) * (Fp(1) - X) * (Fp(1) - X*X*X*X);
    assert(bostanMori(p3, p4, 10000000000) == 452641873);
    Pol p5 = (Fp(1) - X) * (Fp(1) - X*X);
    p5 *= p5;
    assert(bostanMori(p3, p5, 10000000000) == 134866529);
    */
