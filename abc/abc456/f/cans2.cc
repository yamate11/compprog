...
// @@ !! LIM(matrix segTree debug cmpNaive forall)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    ll ans = big;
    REP(x, 1, 1LL << N) {
      ll lo = countr_zero((u64)x);
      ll hi = 64 - countl_zero((u64)x);
      if (hi - lo < K) continue;
      if (EXISTS(i, lo, hi - 1, not (x >> i & 1) and not (x >> (i + 1) & 1))) continue;
      ll cost = 0;
      REP(i, lo, hi) if (x >> i & 1) cost += A[i];
      ans = min(ans, cost);
    }
    cout << ans << endl;
  };
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    const Mat zero(2, 2);
    const Mat unit = zero.one();
    auto myadd = [](const Mat& a, const Mat& b) -> Mat { return b * a; };
    vector<Mat> init_dat;
    REP(i, 0, N) init_dat.push_back(Mat{{A[i], A[i]}, {0, big}});
    auto st = make_seg_tree(unit, myadd, init_dat);

    ll ans = big;
    REP(i, 0, N - K + 1) {
      vector<MP> init{A[i], big};
      if (i < N - K) {
        auto x = st.query(i + 1, i + K + 1) * init;
        DLOGK(i, st.query(i + 1, i + K + 1), init, x);
        ans = min(ans, min((ll)x[0], (ll)x[1]));
      }else {
        auto x = st.query(i + 1, i + K) * init;
        DLOGK(i, st.query(i + 1, i + K), init, x);
        ans = min(ans, (ll)x[0]);
      }
    }
    cout << ans << endl;
