...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    // @End [Oz59timT]
    vector tbl(N, safe_umap<ll, ll>());
    auto f = [&](auto rF, ll s, ll t) -> ll {
      auto [it, b] = tbl[s].try_emplace(t, 0);
      ll& r = it->second;
      if (b) {
        if (s == t) r = A[s];
        else if (s + 1 == t) r = A[s] + A[t];
        else if (s + 2 == t) r = A[s] + A[t];
        else {
          ll m = (s + t) / 2;
          ll r1 = rF(rF, s, m    ) + rF(rF, m,     t) - A[m];
          ll r2 = rF(rF, s, m + 1) + rF(rF, m + 1, t) - A[m + 1];
          r = min(r1, r2);
        }
      }
      return r;
    ...
    ll big = 1LL << 60;
    ll ans = big;
    REP(i, 0, N - K + 1) ans = min(ans, f(f, i, i + K - 1));
    if (K < N) REP(i, 0, N - (K + 1) + 1) ans = min(ans, f(f, i, i + (K + 1) - 1));
    return ans;
  };
