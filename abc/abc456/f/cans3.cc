...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    using sta = array<ll, 4>;
    sta unit{-1, -1, -1, -1};
    auto add = [&](const sta& a, const sta& b) -> sta {
      sta ret;
      if (a[0] < 0) return b;
      if (b[0] < 0) return a;
      REP(i, 0, 4) ret[i] = min(a[(i & 2) | 0] + b[2 | (i & 1)],
                                a[(i & 2) | 1] + b[0 | (i & 1)]);
      return ret;
    };
    vector<sta> init_vec(N);
    REP(i, 0, N) { init_vec[i] = sta{0, A[i], A[i], A[i]}; }
    auto segt = make_seg_tree(unit, add, init_vec);

#if DEBUG
    DLOGK(add(unit, init_vec[0]));
    DLOGK(add(init_vec[0], unit));
    REP(i, 0, N) REP(j, i + 1, N + 1) {
      DLOGK(i, j, segt.query(i, j));
    }
#endif
    ...
    ll big = 1LL << 60;
    ll ans = big;
    REP(i, 0, N - K + 1) ans = min(ans, segt.query(i, i + K)[3]);
    if (K < N) REP(i, 0, N - (K + 1) + 1) ans = min(ans, segt.query(i, i + K + 1)[3]);
    return ans;
  };
