...
// @DefStruct(SS2, (a, (c, char)), ord=((c, True), a, iii), idx=iii, istr=True) [QT2Bblj9]
struct SS2 {
  ll a;
  char c;
  int iii;
...
// @InpNbrList(N, M, nbr1, dec=1) [znvFMqjQ]
...
// @InpNbrList(N, N - 1, fwd, dec=1, dir=True) [mtYF05yO]
...
// @InpNbrList(N, M, nbr3, read=(cost, (desc, string)), idx=eid, dec=1) [BptrEsOt]
struct nbr3_t {
  int nd;
  ll cost;
  string desc;
...
// @DefStruct(A, (x, y), ord=(y,(x, True))) [C9dqpCfN]
struct A {
  ll x;
  ll y;
  ostream& operator<<(ostream& ostr) const {
  ...
  strong_ordering operator<=>(const A& o) const {
    if (auto r_ = y <=> o.y; r_ != 0) return r_;
    if (auto r_ = o.x <=> x; r_ != 0) return r_;
    return std::strong_ordering::equal;
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, nbr, dec=1, read=C) [KwlPNZJc]
  struct nbr_t {
    int nd;
    ll C;
    ostream& operator<<(ostream& ostr) const {
  ...
  REP(i, 0, N) {
    for (auto m : nbr[i]) DLOGK(i, m.nd, m.C);
  }
