...
int main(/* int argc, char *argv[] */) {
  ...
  auto enc = [&](ll r, ll c) -> ll { return 4 * r + c; };
  // auto dec = [&](ll e) -> pair<ll, ll> { return { e / 4, e % 4 }; };

  ll houses = 0;
  for (ll r = 0; r < 4; r++) {
    for (ll c = 0; c < 4; c++) {
      ll a; cin >> a;
      if (a) houses |= (1LL << enc(r, c));
    }
  }
  auto count = [&](auto& brd, BrdIdx start, int v) -> ll {
    ll cnt = 0;
    Board<int> visited(6, 6, 0);
    auto dfs1 = [&](auto rF, BrdIdx bi) -> void {
      if (! brd.in(bi)) return;
      if (brd.at(bi) != v) return;
      if (visited.at(bi)) return;
      cnt++;
      visited.at(bi) = true;
      for (auto& bd : BrdIdx::nbr4) {
        rF(rF, bi + bd);
      }
    };
    dfs1(dfs1, start);
  ...
  auto conn = [&](auto& brd, ll t) -> bool {
    for (ll r = 0; r < 4; r++) {
      for (ll c = 0; c < 4; c++) {
        if (brd.at(1 + r, 1 + c)) {
          ll z = count(brd, BrdIdx(1 + r, 1 + c), 1);
          DLOGKL("conn", z, t);
          return z == t;
        }
      }
  ...
  auto check = [&](ll x) -> bool {
    if ((houses & x) != houses) return false;
    Board<int> brd(6, 6, 0);
    for (ll r = 0; r < 4; r++) {
      for (ll c = 0; c < 4; c++) {
        if ((x >> enc(r, c)) & 1) brd.at(1 + r, 1 + c) = 1;
      }
    }
    ll t = __builtin_popcountll(x);
    DLOG(brd);
    DLOGK(t);
    if (! conn(brd, t)) return false;
    ll ct = count(brd, BrdIdx(0, 0), 0);
    DLOGK(ct);
    if (ct + t != 36) return false;
    return true;
  };

  ll ans = 0;
  for (ll x = 0; x < 1LL << 16; x++) {
    if (check(x)) ans++;
  }
  cout << ans << endl;
