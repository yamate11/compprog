...
int main(/* int argc, char *argv[] */) {
  ...
  auto myadd = [&](sta a, sta b) -> sta {
    auto [amax, amaxnum, asnd, asndnum] = a;
    auto [bmax, bmaxnum, bsnd, bsndnum] = b;
    if (amax < bmax) {
      swap(amax, bmax);
      swap(amaxnum, bmaxnum);
      swap(asnd, bsnd);
      swap(asndnum, bsndnum);
    }
    ll cmax = amax;
    ll cmaxnum = -1;
    ll csnd = -1;
    ll csndnum = -1;
    if (amax == bmax) {
      cmaxnum = amaxnum + bmaxnum;
      if (asnd >= bsnd) {
        csnd = asnd;
        if (asnd == bsnd) csndnum = asndnum + bsndnum;
        else csndnum =asndnum;
      }else {
        csnd = bsnd;
        csndnum = bsndnum;
      }
    ...
    return sta(cmax, cmaxnum, csnd, csndnum);
  };

  ll N, Q; cin >> N >> Q;

  ll big = 1e18;
  sta unit(-big, 0LL, -big, 0LL);
  vector<sta> init_vec(N, unit);
  REP(i, 0, N) {
    ll a; cin >> a;
    init_vec[i] = sta(a, 1LL, -big, 0LL);
  }
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ll p, x; cin >> p >> x; p--;
      st.update(p, sta(x, 1LL, -big, 0LL));
    }else if (tp == 2) {
      ll l, r; cin >> l >> r; l--;
      auto [v, vn, w, wn] = st.query(l, r);
      cout << wn << "\n";
    }else assert(0);
  }
