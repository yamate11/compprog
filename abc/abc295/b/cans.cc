...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> res(R, C, '.');
  cin >> brd;
  REP(i, 0, R) REP(j, 0, C) res.at(i, j) = brd.at(i, j);
  REP(i, 0, R) REP(j, 0, C) {
    if (brd.at(i, j) == '.') {
    }else if (brd.at(i, j) == '#') {
    }else {
      ll d = brd.at(i, j) - '0';
      REP(ii, i - d, i + d + 1) REP(jj, j - d, j + d + 1) {
        if (abs(ii - i) + abs(jj - j) <= d and brd.in(ii, jj)) res.at(ii, jj) = '.';
      }
    }
  }
  REP(i, 0, R) {
    REP(j, 0, C) cout << res.at(i, j);
    cout << "\n";
  }
