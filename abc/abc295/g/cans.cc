...
template<typename T = ll,
         typename prop_t = ll,
         typename prop_merge_t = prop_t (*)(prop_t, prop_t),
         typename oplus_t = decltype(plus<T>()), typename onegate_t = decltype(negate<T>())>
struct UnionFindProp : UnionFind<T, oplus_t, onegate_t> {
  vector<prop_t> _prop;
  prop_merge_t _prop_merge;

  UnionFindProp(int size_, prop_merge_t prop_merge_,
                T zero_ = (T)0, oplus_t oplus_ = plus<T>(), onegate_t onegate_ = negate<T>())
    : UnionFind<T, oplus_t, onegate_t>(size_, zero_, oplus_, onegate_), _prop(size_), _prop_merge(prop_merge_) {}

  int merge(int i, int j, const T& p = (T)0) {
    ll li = this->leader(i), lj = this->leader(j);
    prop_t pi = _prop[li], pj = _prop[lj];
    ll new_ld = UnionFind<T, oplus_t, onegate_t>::merge(i, j, p);
    if (new_ld == -1) return -1;
    if (li == lj) return new_ld;
    _prop[new_ld] = _prop_merge(pi, pj);
    return new_ld;
  }

  prop_t& prop(int i) { return _prop[this->leader(i)]; }
};
...
int main(/* int argc, char *argv[] */) {
  ...
  auto prop_merge = [&](ll x, ll y) -> ll { return x; };
  UnionFindProp<ll, ll, decltype(prop_merge)> uf(N, prop_merge);
  REP(i, 0, N) uf.prop(i) = i;
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      ll cur = uf.prop(u);
      while (cur != goal) {
        ll nxt = uf.prop(bwd[cur]);
        uf.merge(nxt, cur);
        cur = nxt;
      }
      DLOGK(u, v, uf._leader, uf._prop);
    }else if (tp == 2) {
      ll x; cin >> x; x--;
      cout << uf.prop(x) + 1 << "\n";
    }else assert(0);
