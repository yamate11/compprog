...
int main(/* int argc, char *argv[] */) {
  ...
  using sta = pair<ll, int>;
  vector<vector<sta>> A(N);
  REP(i, N) {
    ll a; cin >> a;
    A[i] = prfac(a);
  }
  using stb = tuple<ll, ll, ll>;
  vector<vector<stb>> LR(M);
  REP(i, Q) {
    ll l, r; cin >> l >> r; l--;
    ll j = l / C;
    LR[j].emplace_back(r, l, i);
  }
  REP(i, M) sort(ALL(LR[i]));
  vector<bool> ans(Q);
  vector<ll> rec(lim);
  ll x = 0, y = 0;
  ll nz = 0;
