...
int main(/* int argc, char *argv[] */) {
  ...
  auto P = vector(X + 1, vector(N, Fp(0)));
  auto Q = vector(X + 1, vector(N, Fp(0)));
  REP(i, 0, N) {
    P[0][i] = Fp(1) / Fp(N);
    if (T[i] <= X) Q[T[i]][i] = Fp(1) / Fp(N);
  }
  REP(t, 1, X + 1) {
    Fp allQ = Fp(0);
    REP(i, 0, N) allQ += Q[t][i];
    REP(i, 0, N) P[t][i] = P[t - 1][i] + allQ / Fp(N) - Q[t][i];
    REP(i, 0, N) if (t + T[i] <= X) Q[t + T[i]][i] += allQ / N;
  }
  cout << P[X][0] << endl;
