...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  string S; cin >> S;
  auto myadd = [&](ll x, ll y) -> ll { return x + y; };
  auto dummy_comp = [](void* x, void* y) -> void* { return nullptr; };
  auto dummy_appl = [](void* f, ll x) -> ll { return 0LL; };
  using SEG = GenSegTree<ll, void*, decltype(myadd), decltype(dummy_comp), decltype(dummy_appl), false>;
  auto init_vec = vector(N, 0LL);
  vector<SEG> st(26, SEG(0LL, nullptr, myadd, dummy_comp, dummy_appl, init_vec));
  REP(i, 0, N) st[S[i] - 'a'].update(i, 1);
  ll Q; cin >> Q;
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ...
      if (el > er) b = false;
        ...
        REP(e, el, er + 1) {
          ll k = st[e].query(l, r);
          if (e != el and e != er) {
            ll tot = st[e].query(0, N);
            if (k != tot) {
              b = false;
              break;
            }
          }
          ll kk = st[e].query(p, p + k);
          if (kk != k) {
            b = false;
            break;
          }
