...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  string S; cin >> S;
  auto st = make_seg_tree<ll>(0LL, plus<ll>(), vector(N - 1, 0LL));
  REP(i, 0, N - 1) if (S[i] > S[i + 1]) st.update(i, 1);
  vector<set<ll>> lm(26);
  REP(i, 0, N) lm[S[i] - 'a'].insert(i);
  auto chg_st = [&](bool b1, bool b2, ll j) -> void {
    if (b1 != b2) st.update(j, b2 ? 1 : 0);
  };
  ll Q; cin >> Q;
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ...
      if (x - 1 >= 0) chg_st(S[x - 1] > S[x], S[x - 1] > c, x - 1);
      if (x + 1 < N)  chg_st(S[x] > S[x + 1], c > S[x + 1], x);
      lm[S[x] - 'a'].erase(x);
      S[x] = c;
      lm[c - 'a'].insert(x);
    }else if (tp == 2) {
      ll l, r; cin >> l >> r; l--; r--;
      auto ok = [&](ll e) -> bool { return lm[e].empty() or (l < *lm[e].begin() and *prev(lm[e].end()) < r); };
      bool b = st.query(l, r) == 0 and FORALL(c, S[l] + 1, S[r], ok(c - 'a'));
      cout << (b ? "Yes\n" : "No\n");
    }
  }
  return 0;
