...
int main(/* int argc, char *argv[] */) {
  ...
  DAT unit_dat(-big, 0);
  auto mymax = [&](DAT a, DAT b) -> DAT {
    auto [av, ai] = a;
    auto [bv, bi] = b;
    if (av < bv) return b;
    else return a;
  };
  auto myappl = [&](ll x, DAT a) -> DAT {
    auto [av, ai] = a;
    return DAT(av + x, ai);
  };
  vector<DAT> init_vec(Q);
  REP(q, 0, Q) init_vec[q] = DAT(-big, q);
  auto segt = make_seg_tree_lazy(unit_dat, 0LL, mymax, plus<ll>(), myappl, init_vec);
  ll prev_t = 0;
  REP(q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll t, w; cin >> t >> w;
      segt.update(0, Q, t - prev_t);
      segt.rs(q) = DAT(w, q);
      prev_t = t;
    }else if (tp == 2) {
      ll t; cin >> t;
      segt.update(0, Q, t - prev_t);
      auto [a, i] = segt.query(0, Q);
      if (a < 0) {
        cout << "-1\n";
      }else {
        if (a >= V) a = V;
