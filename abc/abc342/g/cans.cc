...
int naive(istream& cin, ostream& cout) {
  ...
  REP(t, 0, Q) {
    ...
    if (tp == 1) {
      ...
      REP(p, 0, t) {
        auto [a, b, c] = op[p];
        if (a >= 0 and a <= i and i < b) val = max(val, c);
      }
      cout << val << "\n";
    }else assert(0);
  }
...
int body(istream& cin, ostream& cout) {
  ...
  ll M = bit_ceil((u64)N);
  vector S(2*M, multiset<ll>());
  using sta = tuple<ll, ll, ll>;
  auto roof = [&](ll a, ll b) -> vector<ll> {
    vector<ll> fst, snd;
    for (a += M, b += M - 1; a < b; a >>= 1, b >>= 1) {
      if (     a & 1 ) fst.push_back(a++);
      if (not (b & 1)) snd.push_back(b--);
    }
    if (a == b) fst.push_back(a);
    while (not snd.empty()) {
  ...
  ll Q; cin >> Q;
  vector opr(Q, sta(-1, -1, -1));
  REP(t, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll a, b, c; cin >> a >> b >> c; a--;
      for (ll i : roof(a, b)) S[i].insert(c);
      opr[t] = sta(a, b, c);
    }else if (tp == 2) {
      ll i; cin >> i; i--;
      auto [a, b, c] = opr[i];
      ...
      ll i; cin >> i; i--;
      ll val = A[i];
      for (ll a = M + i; a > 0; a >>= 1) {
        if (not S[a].empty()) val = max(val, *prev(S[a].end()));
      }
      cout << val << "\n";
    }else assert(0);
  }
