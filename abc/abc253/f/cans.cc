...
int main(/* int argc, char *argv[] */) {
  ...
  const ll TP1 = 1, TP2 = 2, TP3 = 3;

  ll N, M, Q; cin >> N >> M >> Q;
  vector<ll> tp2recT(N);
  vector<ll> tp2recX(N);
  using ev_t = tuple<ll, ll, ll, ll, ll, ll>;
  vector<ev_t> event;
  ll numSeq = -1;
  {
    ll seq = 0;
    REP2(t, 1, Q + 1) {
      ...
      if (tp == 1) {
        ...
        event.emplace_back(t, TP1, l, r, x, -1);
      }else if (tp == 2) {
        ll i, x; cin >> i >> x; i--;
        tp2recT[i] = t;
        tp2recX[i] = x;
        ...
        ll i, j; cin >> i >> j; i--; j--;
        event.emplace_back(t, TP3, i, j, seq, -1);
        event.emplace_back(tp2recT[i], TP2, i, j, seq, tp2recX[i]);
        seq++;
      }
    }
    numSeq = seq;
  ...
  vector<ll> val_set(numSeq);
  vector<ll> val_start(numSeq);
  vector<ll> ans(numSeq);
  for (auto [t, tp, a1, a2, a3, a4] : event) {
    if (tp == TP1) {
      st.update(a1, a2, a3);
    }else if (tp == TP2) {
      ll j = a2, seq = a3, x = a4;
      val_set[seq] = x;
      val_start[seq] = st.query(j, j + 1);
    }else if (tp == TP3) {
      ll j = a2, seq = a3;
      ll val_end = st.query(j, j + 1);
      ans[seq] = val_set[seq] + (val_end - val_start[seq]);
    }
  }
  for (ll v : ans) cout << v << "\n";
