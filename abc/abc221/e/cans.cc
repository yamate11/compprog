...
// @@ !! LIM(mod segTree coordCompr debug)
...
#if DEBUG
    DLOG("   ** 1");
    for (ll x = 0; x < sz; x++) DLOGK(x, st.query(x, x+1));
#endif
    st.update(0, sz, OP(2, 0));
#if DEBUG
    DLOG("   ** 2");
    for (ll x = 0; x < sz; x++) DLOGK(x, st.query(x, x+1));
#endif
    st.update(a, a+1, OP(1, 1));
#if DEBUG
    DLOG("   ** 3");
    for (ll x = 0; x < sz; x++) DLOGK(x, st.query(x, x+1));
#endif
