...
int main(/* int argc, char *argv[] */) {
  ...
  ll maxdep = 0;
  vector<ll> parent(N);
  auto dfs1 = [&](auto rF, ll nd, ll pt, ll dep) -> void {
    parent[nd] = pt;
    if (dep > maxdep) {
      maxdep = dep;
      remote = nd;
  ...
  dfs1(dfs1, rem1, -1, 0);
  ll rem2 = remote;
  auto count2 = [&](ll subtop, ll stpar, ll goal) -> ll {
    ll ret = 0;
    auto dfs2 = [&](auto rF, ll nd, ll pt, ll g) -> void {
      if (g == 0) ret++;
      for (ll cld : nbr[nd]) {
        if (cld == pt) continue;
        assert(g > 0);
  ...
  if (maxdep == 1) {
    ...
    ll radius = maxdep / 2;
    ll center = rem2;
    for (ll i = 0; i < radius; i++) center = parent[center];
    Fp ans = 1;
    ll w = 0;
    for (ll subtop : nbr[center]) {
      ll val = count2(subtop, center, radius - 1);
      w += val;
      ans *= Fp(val + 1);
    }
    ans -= Fp(w + 1LL);
    ...
    ll rad = maxdep / 2;
    ll cent = rem2;
    for (ll i = 0; i < rad; i++) cent = parent[cent];
    ll cent2 = parent[cent];
    ll val1 = count2(cent, cent2, rad);
    ll val2 = count2(cent2, cent, rad);
    Fp ans = Fp(val1) * Fp(val2);
    cout << ans << endl;
  }
