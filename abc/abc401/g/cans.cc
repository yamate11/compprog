...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, start, type=Point) [lp0etzTK]
  auto start = vector(N, Point());
  for (int i = 0; i < N; i++) { Point v; cin >> v; start[i] = v; }
  // @End [lp0etzTK]
  // @InpVec(N, goal, type=Point) [kJrGxD2q]
  auto goal = vector(N, Point());
  for (int i = 0; i < N; i++) { Point v; cin >> v; goal[i] = v; }
  // @End [kJrGxD2q]

  auto check = [&](Real t) -> bool {
    mf_graph<ll> graph(2*N + 2);
    REP(i, 0, N) graph.add_edge(2*N, i, 1);
    REP(i, 0, N) graph.add_edge(N + i, 2*N + 1, 1);
    REP(i, 0, N) REP(j, 0, N) {
      if ((start[i] - goal[j]).len() <= t)  graph.add_edge(i, N + j, 1);
    }
    return graph.flow(2*N, 2*N + 1) == N;
  };

  Real t = binsearch<Real>(check, 1.5e18, 0.0, 1e-7);
  cout << t << endl;
