...
int naive(istream& cin, ostream& cout) {
  ...
  REP(x, 0, 1LL << numq) {
    ...
    ll z = 0;
    REP(i, 0, N) if (t[i] == 'o') z++;
    if (z != K) continue;
    if (EXISTS(i, 0, N - 1, t[i] == 'o' and t[i + 1] == 'o')) continue;
    REP(i, 0, N) {
      if (ans[i] == ' ') ans[i] = t[i];
      else if (ans[i] != t[i]) ans[i] = '?';
    }
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 0, sz) {
    ...
    if (c == '?') {
      auto [cp, np] = vec[i - 1];
      auto [cn, nn] = vec[i + 1];
      if      (cp == '.' and cn == '.') vmax += (n + 1) / 2;
      else if (cp == 'o' and cn == 'o') vmax += (n - 1) / 2;
      else                              vmax += n / 2;
    }
  }
  DLOGK(numo, vmax);
  string ans;
  auto [c0, n0] = vec[0];
  if (c0 == '.' and n0 > 1) ans += string(n0 - 1, '.');
  REP(i, 1, sz - 1) {
    auto [c, n] = vec[i];
    auto [cp, np] = vec[i - 1];
    auto [cn, nn] = vec[i + 1];
    if (c != '?') REP(j, 0, n) ans += c;
    else {
      if (numo == K) {
        REP(j, 0, n) ans += '.';
      }else if (vmax == K) {
  ...
  auto [cF, nF] = vec[sz - 1];
  if (cF == '.' and nF > 1) ans += string(nF - 1, '.');
  cout << ans << endl;
