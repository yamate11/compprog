...
int naive(istream& cin, ostream& cout) {
  ll N; cin >> N;
  // @InpVec(N, P, type=double) [RfkXhLsj]
  auto P = vector(N, double());
  for (int i = 0; i < N; i++) { double v; cin >> v; P[i] = v; }
  // @End [RfkXhLsj]

  vector<double> pow09(N + 1);
  pow09[0] = 1.0;
  REP(i, 1, N + 1) pow09[i] = pow09[i - 1] * 0.9;
  ...
  REP(k, 1, N + 1) {
    ...
    while (ic.get()) {
      ...
      double rate = a / b - 1200.0 / sqrt(k);
      if (rate > ans) {
        DLOGK(ic.vec_view(), rate, a, b, k);
        ans = rate;
      }
    }
  }
...
int body(istream& cin, ostream& cout) {
  ...
  vector tbl_init(N + 1, 0.0);
  auto tbl = tbl_init;
  REP(i, 0, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    tbl[1] = max(prev[1], P[i]);
    // REP(j, 2, N + 1) {
    REP(j, 2, i + 2) {
      tbl[j] = max(prev[j], 0.9 * prev[j - 1] + P[i]);
    }
  }
  ...
  double ans = -1e18;
  REP(j, 1, N + 1) {
    ans = max(ans, tbl[j] / alpha[j] - 1200.0 / sqrt(j));
  }
  cout << ans << endl;
