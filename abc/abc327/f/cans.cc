...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpMVec(N, ((T, dec=1), (X, dec=1))) [vgLdBbyD]
  auto T = vector(N, ll());
  auto X = vector(N, ll());
  for (int i = 0; i < N; i++) {
  ...
  ll lim = max(*max_element(ALL(T)), *max_element(ALL(X))) + 1;

  vector tp(lim, vector<ll>());
  REP(i, 0, N) tp[T[i]].push_back(X[i]);
  DLOGK(tp);

  auto lo = [&](ll x) -> ll { return max(0LL, x - W); };
  auto hi = [&](ll x) -> ll { return min(lim - W, x + 1); };

  vector<ll> vd(lim - W + 1);
  REP(t, 0, D + 1) {
    for (ll x : tp[t]) {
      vd[lo(x)]++;
      vd[hi(x)]--;
  ...
  DLOGK(vd);
  vector<ll> init_vec(lim - W + 1);
  init_vec[0] = vd[0];
  REP(i, 1, lim - W) init_vec[i] = init_vec[i - 1] + vd[i];
  DLOGK(init_vec);
  auto mymax = [&](ll a, ll b) -> ll { return max(a, b); };
  auto st = make_seg_tree_lazy<ll, ll>(0LL, 0LL, mymax, plus<ll>(), plus<ll>(), init_vec);
  ll ans = st.query(0, lim - W + 1);
  REP(t, 0, lim - (D + 1)) {
    for (ll x : tp[t])         st.update(lo(x), hi(x), -1);
    for (ll x : tp[t + D + 1]) st.update(lo(x), hi(x), 1);
    ans = max(ans, st.query(0, lim - W + 1));
  }
  cout << ans << endl;
