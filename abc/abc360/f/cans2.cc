...
int main(/* int argc, char *argv[] */) {
  ...
  auto mymax = [](pll x, pll y) -> pll {
    ...
    auto [vy, iy] = y;
    if (vx > vy) return x;
    else if (vx < vy) return y;
    else if (ix < iy) return x;
    else return y;
  };
  auto myappl = [&](ll f, pll x) -> pll { return pll(x.first + f, x.second); };
  auto st = make_seg_tree_lazy(pll(0, 0), 0LL, mymax, plus<ll>(), myappl);
  vector<ll> di(sz);
  vector<pll> init(sz);
  REP(i, 0, N) {
    di[cc.c(L[i] + 1)] += 1;
  ...
  init[0] = pll(di[0], 0);
  REP(i, 1, sz) {
    init[i] = pll(init[i - 1].first + di[i], i);
  }
  st.set_data(init);

  enum kind { P1, P2, P3 };
  using ev_t = pair<kind, ll>;
  vector ev(sz, vector<ev_t>());
  REP(i, 0, N) {
    if (L[i] + 2 <= R[i]) {
      ev[cc.c(L[i])].emplace_back(P1, i);
      ev[cc.c(L[i] + 1)].emplace_back(P2, i);
      ev[cc.c(R[i])].emplace_back(P3, i);
    }
  }
  ll vmax = -1, ans_l = -1, ans_r = -1;
  REP(t, 0, sz) {
    for (auto [kd, i] : ev[t]) {
      if (kd == P1) {
        st.update(cc.c(L[i] + 1), cc.c(R[i]), -1);
      }else if (kd == P2) {
        st.update(cc.c(R[i] + 1), sz, 1);
      }else if (kd == P3) {
        st.update(cc.c(R[i] + 1), sz, -1);
      }
    }
    auto [vcur, rr] = st.query(t + 1, sz);
    if (vcur > vmax) {
      vmax = vcur;
      ans_l = t;
      ans_r = rr;
