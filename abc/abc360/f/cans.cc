...
// @@ !! LIM(coordCompr segTree cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto intersect = [&](ll la, ll ra, ll lb, ll rb) -> bool {
    if (la < lb and lb < ra and ra < rb) return true;
    if (lb < la and la < rb and rb < ra) return true;
    return false;
  };
  ...
  ll l0 = -1;
  ll r0 = -1;
  REP(l, 0, lim + 1) REP(r, l + 1, lim + 1) {
    ll cnt = 0;
    REP(i, 0, N) {
      if (intersect(l, r, L[i], R[i])) cnt++;
    }
    if (vmax < cnt) {
      vmax = cnt;
      l0 = l;
...
int body(istream& cin, ostream& cout) {
  ...
  using DAT = pll;
  using OP = ll;
  auto myadd = [&](DAT x, DAT y) -> DAT { return (x.first >= y.first) ? x : y; };
  auto mycomp = [&](OP x, OP y) -> OP { return x + y; };
  auto myapply = [&](OP x, DAT y) -> DAT { return pll(x + y.first, y.second); };
  vector<DAT> init_vec(sz);
  REP(i, 0, sz) init_vec[i] = DAT(0LL, i);
  auto st = make_seg_tree_lazy(pll(-10, 0), 0LL, myadd, mycomp, myapply, init_vec);
  enum kind { IN, OUT };
  using sta = tuple<kind, ll, ll>;
  vector<vector<sta>> ev(sz);
  auto f = [&](ll x, kind kd, ll lo, ll hi) { ev[cc.c(x)].emplace_back(kd, cc.c(lo), cc.c(hi)); };
  REP(i, 0, N) {
    f(0,        IN,  L[i] + 1, R[i]);
    f(L[i],     OUT, L[i] + 1, R[i]);
    f(L[i] + 1, IN,  R[i] + 1, lim + 1);
    f(R[i],     OUT, R[i] + 1, lim + 1);
  }
  ll vmax = -1;
  ll xidx = -1;
  ll yidx = -1;
  REP(i, 0, sz) {
    for (auto [kd, lo, hi] : ev[i]) {
      st.update(lo, hi, kd == IN ? 1 : -1);
    }
    auto [val, jy] = st.query(0, sz);
    if (vmax < val) {
      vmax = val;
      xidx = i;
      yidx = jy;
