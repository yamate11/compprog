  ...
  REPrev(len, N, 1) {
    ...
    while (ic.get()) {
      auto check = [&]() -> bool {
        ll j = -1;
        REP(i, 0, len - 1) {
          if (A[ic.at(i)] >= A[ic.at(i + 1)]) {
            if (j >= 0) {
              DLOGK(j, i, ic.vec_view());
              return false;
            }
            j = i;
          }
        ...
        DLOGK(j, ic.vec_view());
        if (j < 0) return true;
        if (j == 0 or j == len - 2) return true;
        if (A[ic.at(j - 1)] + 1 < A[ic.at(j + 1)] or A[ic.at(j)] + 1 < A[ic.at(j + 2)]) return true;
        return false;
      };
      if (check()) {
        cout << len << "\n";
...
int body(istream& cin, ostream& cout) {
  ...
  vector<ll> vec;
  vector<vector<pll>> rec;

  bool add1 = false;
  REP(i, 0, N) {
    ll idx = lower_bound(ALL(vec), A[i]) - vec.begin();
    if (idx == ssize(vec)) {
      vec.push_back(A[i]);
      vector<pll> adding = {{i, A[i]}};
      rec.push_back(move(adding));
    }else {
      if (i == N - 1) add1 = true;
      vec[idx] = A[i];
      rec[idx].emplace_back(i, A[i]);
    }
  }
  ll orig = ssize(vec);
  DLOGK(orig, add1);
  ...
  auto [i0, a0] = rec.back().back();
  queue<sta> que;
  que.emplace(i0, a0, orig - 1);
  seen[i0] = true;
  while (not que.empty()) {
    auto [i, a, lev] = que.front(); que.pop();
    auto check = [&](ll j) -> bool {
      auto [ii, aa] = rec[lev - 1][j];
      if (ii + 1 < i and aa + 1 < a) {
        DLOGK(i, a, lev, ii, aa);
        return true;
      }
      if (aa < a and not seen[ii]) {
        seen[ii] = true;
        que.emplace(ii, aa, lev - 1);
      }
      return false;
    };
    if (lev > 0) {
      ll j = lower_bound(ALL(rec[lev - 1]), pll(i, 0)) - rec[lev - 1].begin() - 1;
      if (check(j) or (j - 1 >= 0 and check(j - 1))) {
        cout << orig + 1 << endl;
        return 0;
      }
    }else {
