...
// @@ !! LIM(forall cmpNaive perm sortedIndex sieve random)
...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ll N; cin >> N;
    // @InpVec(N, S, type=string) [dFwS3ZXp]
...
int body(istream& cin, ostream& cout) {
  ...
  /*
  auto g = [&](const string& s, const string& t) -> int {
    ll i = ssize(s);
    assert(i < ssize(t));
    if (s[0] < t[i]) return 1;
    if (s[0] > t[i]) return 0;
  ...
  */
  ...
  auto mycomp = [&](const string& s, const string& t) -> bool {
    auto ps = &s;
    auto pt = &t;
    bool sw = false;
    if (ssize(*ps) > ssize(*pt)) {
      sw = true;
      swap(ps, pt);
    }
    ll a = ssize(*ps);
    ll b = ssize(*pt);
    assert(a <= b);
    REP(i, 0, a) if ((*ps)[i] != (*pt)[i]) return ((*ps)[i] < (*pt)[i]) != sw;
    REP(i, 0, b - a) if ((*pt)[i] != (*pt)[a + i]) return ((*pt)[i] < (*pt)[a + i]) != sw;
    REP(i, 0, a) if ((*pt)[b - a + i] != (*ps)[i]) return ((*pt)[b - a + i] < (*ps)[i]) != sw;
    return false;
  ...
  auto solve = [&]() -> void {
    ...
    REP(i, 0, N) {
      auto ds = getDivisors(ssize(S[i]), primes);
      sort(ALL(ds));
      for (ll d : ds) {
        ll e = ssize(S[i]) / d;
        if (FORALL(k, 0, d, FORALL(j, 1, e, S[i][d * j + k] == S[i][k]))) {
          save[i] = S[i];
          S[i] = S[i].substr(0, d);
          break;
        }
    ...
    */
    ...
    auto ord = sortedIndex(S, mycomp);

    /*
    vector<ll> ord(N);
    REP(i, 0, N) ord[i] = i;
    auto f = [&](auto rF, auto it, ll len) -> void {
      if (len == 1) return;
      ll vlen = (len + 1) / 2;
      ll wlen = len - vlen;
      vector<ll> v(vlen);
      REP(i, 0, vlen) v[i] = *(it + i);
      auto itv = v.begin();
      auto itw = it + vlen;
      rF(rF, itv, vlen);
      rF(rF, itw, wlen);
      ll j = 0, jv = 0, jw = 0;
      while (jv < vlen or jw < wlen) {
        auto opv = [&]() { *(it + j++) = *(itv + jv++); };
        auto opw = [&]() { *(it + j++) = *(itw + jw++); };
        if (jv == vlen) opw();
        else if (jw == wlen) opv();
        else if (mycomp(S[*(itv + jv)], S[*(itw + jw)])) opv();
        else opw();
      }
    };
    f(f, ord.begin(), N);
    */
    ...
    // REP(i, 0, N) if (ssize(save[i]) > 0) S[i] = save[i];

    REP(i, 0, N - 1) if (S[ord[i]] + S[ord[i + 1]] == S[ord[i + 1]] + S[ord[i]]) {
      REP(j, 0, N) cout << S[ord[j]];
      cout << "\n";
      return;
    }
    string cand1;
    REP(i, 0, N - 2) cand1 += S[ord[i]];
    cand1 += S[ord[N - 1]];
    cand1 += S[ord[N - 2]];
    if (N <= 2) {
      cout << cand1 << "\n";
