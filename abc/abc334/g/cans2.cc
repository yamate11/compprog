...
// @@ !! LIM(board UnionFind mod debug bridge forall)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto enc = [&](ll x, ll y) -> ll { return x * W + y; };

  UnionFind uf(H * W);
  articulation au(H * W);
  REP(i, 0, H) REP(j, 0, W - 1) if(brd.at(i, j) == '#' and brd.at(i, j + 1) == '#') {
    ll e1 = enc(i, j);
    ll e2 = enc(i, j + 1);
    au.add_edge(e1, e2);
    uf.merge(e1, e2);
  }
  REP(i, 0, H - 1) REP(j, 0, W) if(brd.at(i, j) == '#' and brd.at(i + 1, j) == '#') {
    ll e1 = enc(i, j);
    ll e2 = enc(i + 1, j);
    au.add_edge(e1, e2);
    uf.merge(e1, e2);
  ...
  REP(i, 0, H) REP(j, 0, W) if (brd.at(i, j) == '#') {
    cnt = cnt + Fp(1);
    ll e = enc(i, j);
    if (uf.leader(e) == e) tot++;
  }

  Fp sum = 0;
  REP(i, 0, H) REP(j, 0, W) if (brd.at(i, j) == '#') {
    BrdIdx bi(i, j);
    if (FORALL_C(dir, BrdIdx::nbr4, brd.at(bi + dir) == '.')) sum -= Fp(1);
  }

  auto bctree = au.make_bctree();
  vector<ll> deg(bctree->size());
  for (auto [a, b] : bctree->edges()) {deg[a]++; deg[b]++;}

  REP(a, 0, bctree->size()) {
    auto [w, e] = bctree->what(a);
    if (w == articulation::CUT) { sum += deg[a] - 1; }
  }
  Fp ans = Fp(tot) + (sum / cnt);
  cout << ans << endl;
