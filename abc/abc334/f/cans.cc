...
int main(/* int argc, char *argv[] */) {
  ...
  long double big = 1e18;
  auto mymin = [](long double x, long double y) -> long double { return min(x, y); };
  enum opkind { ADD, SUBST };
  using OP = pair<opkind, long double>;
  auto mycomp = [](OP f, OP g) -> OP {
    auto [fk, fd] = f;
    auto [gk, gd] = g;
    if (fk == SUBST) return f;
    if (gk == SUBST) return OP(SUBST, fd + gd);
    return OP(ADD, fd + gd);
  };
  auto myappl = [](OP f, long double x) -> long double {
    auto [fk, fd] = f;
    if (fk == SUBST) return fd;
    return fd + x;
  };
  auto st = make_seg_tree_lazy<long double, OP>(big, OP(ADD, 0.0), mymin, mycomp, myappl);
  ...
  REP(i, 1, N) {
    ...
    OP op(ADD, (P[i] - P[i - 1]).len());
    st.update(0, K, op);
    long double e1 = (P[i - 1] - S).len();
    long double e0 = (P[i] - S).len();
    st.update(j0, j0 + 1, OP(SUBST, vmin + e0 + e1));
  }
