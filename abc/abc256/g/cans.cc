...
int main(/* int argc, char *argv[] */) {
  ...
  REP2(alpha, 1, D + 1) {
    Matrix<Fp> mat(2, 2);
    mat.at(0, 0) = cb.comb(D - 1, alpha - 2);
    mat.at(0, 1) = cb.comb(D - 1, alpha - 1);
    mat.at(1, 0) = cb.comb(D - 1, alpha - 1);
    mat.at(1, 1) = cb.comb(D - 1, alpha);
    auto matpow = mat.matpower(N);
    DLOGK(mat);
    DLOGK(matpow);
    ans += matpow.at(0, 0) + matpow.at(1, 1);
  }
  cout << ans << endl;
  return 0;
}
