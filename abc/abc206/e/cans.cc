...
auto divSieve = divisorSieve(1000010);
vector<ll> reduce(1000010);
...
void build() {
  for (ll i = 2; i <= 1000000; i++) {
    ...
    if (p == i) { reduce[i] = i; }
      ...
      ll y = p / i;
      if (divSieve[y] > p) reduce[i] = reduce[y] * p;
      else reduce[i] = reduce[y];
    }
  }
}
...
int main(/* int argc, char *argv[] */) {
  ...
  ll L, R; cin >> L >> R;
  ll ans = func(R, R) - func(L-1, R) - func(R, L-1) + func(L-1, L-1);
  cout << ans << endl;
