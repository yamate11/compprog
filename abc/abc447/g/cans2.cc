...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpMVec(N, ((K, dec=1), A)) [iXCNwHtb]
  auto K = vector(N, ll());
  auto A = vector(N, ll());
  for (int i = 0; i < N; i++) {
    ll v1; cin >> v1; v1 -= 1; K[i] = v1;
    ll v2; cin >> v2; A[i] = v2;
  }
  // @End [iXCNwHtb]

  // using d_t = vector<pll>;
  using d_t = array<pll, 4>;
  d_t unit_d_t;
  REP(j, 0, 4) unit_d_t[j] = pll(-1, -1);
  auto single_d_t = [&](ll a, ll k) -> d_t {
    d_t ret = unit_d_t;
    ret[0] = pll(a, k);
    return ret;
  };
  auto add_d_t = [&](const d_t& d1, const d_t& d2) -> d_t {
    DLOGK(d1, d2);
    d_t ret = unit_d_t;
    ll i = 0, j = 0;
    REP(k, 0, 4) {
      DLOGK(k, ret, i, j);
      assert (i < 4 and j < 4);
      if (d1[i].first < 0 and d2[j].first < 0) break;
      auto op = [&](auto p1, auto p2, auto x, auto y) -> bool {
        DLOGKL("  op", *x, *y);
        bool ret_op;
        assert (*x < 4 and *y < 4);
        if ((*p1)[*x].first < (*p2)[*y].first) {
          swap(p1, p2);
          swap(x, y);
        }
        DLOGKL("    before", *x, *p1, ret);
        ret_op = not EXISTS(p, 0, k, (*p1)[*x].second == ret[p].second);
        if (ret_op) ret[k] = (*p1)[*x];
        (*x)++;
        DLOGKL("  op ret", *x, ret_op);
        return ret_op;
      };
      while (not op(&d1, &d2, &i, &j)) {
        DLOGKL("body-f", i, j);
      }
      DLOGKL("body-t", i, j);
    }
  ...
  L[0] = R[N - 1] = unit_d_t;
  REP(i, 0, N - 1)    L[i + 1] = add_d_t(L[i], single_d_t(A[i], K[i]));
  REPrev(i, N - 1, 1) R[i - 1] = add_d_t(R[i], single_d_t(A[i], K[i]));
  ...
  REP(i, 0, N) REP(j, 0, 4) {
    auto [a, k] = L[i][j];
    if (k >= 0) revL[k].push_back(i);
  }
  vector revK(N, vector<ll>());
  REP(i, 0, N) revK[K[i]].push_back(i);
  
  auto calc = [&](ll i, ll c) -> d_t {
    ll a = A[i];
    ll cnt = 0;
    REP(j, 0, 4) {
      auto [aa, kk] = L[i][j];
      if (kk >= 0 and kk != K[i] and kk != c) {
        a += aa;
        if (++cnt == 2) break;
      }
    }
    if (cnt == 2) return single_d_t(a, K[i]);
    else          return unit_d_t;
  };

  vector<d_t> init_vec(N);
  REP(i, 0, N) init_vec[i] = calc(i, 0);
  DLOGK(init_vec);
  auto st = make_seg_tree(unit_d_t, add_d_t, init_vec);
  ll ans = -1;
  REP(c, 0, N) {
    for (ll i : revK[c]) {
      if (i >= 1) {
        ...
        REP(p, 0, 4) {
          ...
          REP(j, 0, 4) {
            auto [ar, kr] = R[i][j];
            if (kr >= 0 and kr != c and kr != kz) {
              vcur += ar;
              if (++cnt == 2) break;
            }
          }
          if (cnt == 2) vmax = max(vmax, vcur);
        }
        if (vmax >= 0) ans = max(ans, vmax + A[i]);
      }
    }

    if (c + 1 < N) {
      for (ll i : revL[c])     st.rs(i) = calc(i, c + 1);
      for (ll i : revL[c + 1]) st.rs(i) = calc(i, c + 1);
    }
  }
  cout << ans << endl;
  return 0;
