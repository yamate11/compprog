...
int naive(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> ll {
    ...
    auto g = [&](ll i) -> void {
      ...
      auto f = [&](auto rF, ll nd, ll len) -> void {
        visited[nd] = true;
        ll d = ssize(nbr[nd]);
        if (d <= 2) return;
        if (d == 3) {
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> ll {
    ...
    auto dfs = [&](auto rF, ll nd) -> pll {
      ll d = tr.num_children(nd) + (nd == 0 ? 0 : 1);
      if (d <= 2) {
        ll acmax = 0;
      ...
      if (d == 3) {
        ...
        for (ll c : tr.children(nd)) {
          ...
          if (bcmax2 < bc) {
            bcmax2 = bc;
            if (bcmax < bcmax2) swap(bcmax, bcmax2);
          }
        }
        ll a = max(acmax, bcmax + bcmax2 + 1);
        ll b = bcmax + 1;
        DLOGK(nd, d, a, b);
        return pll(a, b);
      }
