...
int main(/* int argc, char *argv[] */) {
  ...
  ll K = bit_width(ull(N));
  auto tbl_init = vector(2, vector(2, vector(2, vector(2, vector(2, vector(2, vector(A1, vector(A2, vector(A3, Fp(0))))))))));
  auto tbl = tbl_init;
  tbl[1][1][1][1][1][1][0][0][0] = Fp(1);
  // tbl[eq1][eq2][eq3][az1][az2][az3][r1][r2][r3];
  REPrev(k, K - 1, 0) {
    ll d = N >> k & 1;
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(eq1, 0, 2) REP(eq2, 0, 2) REP(eq3, 0, 2) REP(az1, 0, 2) REP(az2, 0, 2) REP(az3, 0, 2) REP(r1, 0, A1) REP(r2, 0, A2) REP(r3, 0, A3) {
      Fp prev_val = prev[eq1][eq2][eq3][az1][az2][az3][r1][r2][r3];
      if (prev_val == Fp(0)) continue;
      REP(x1, 0, 2) REP(x2, 0, 2) {
        ll x3 = x1 ^ x2;
        if ((eq1 and x1 > d) or (eq2 and x2 > d) or (eq3 and x3 > d)) continue;
        ll new_eq1 = eq1 and x1 == d;
        ll new_eq2 = eq2 and x2 == d;
        ll new_eq3 = eq3 and x3 == d;
        ll new_az1 = az1 and x1 == 0;
        ll new_az2 = az2 and x2 == 0;
        ll new_az3 = az3 and x3 == 0;
        ll new_r1 = (r1 * 2 + x1) % A1;
        ll new_r2 = (r2 * 2 + x2) % A2;
        ll new_r3 = (r3 * 2 + x3) % A3;
        tbl[new_eq1][new_eq2][new_eq3][new_az1][new_az2][new_az3][new_r1][new_r2][new_r3] += prev_val;
      }
    }
  }
  Fp ans = 0;
  REP(eq1, 0, 2) REP(eq2, 0, 2) REP(eq3, 0, 2) ans += tbl[eq1][eq2][eq3][0][0][0][0][0][0];
  cout << ans << endl;
