...
int main(/* int argc, char *argv[] */) {
  ...
  auto stop_cell = [&](char c) -> bool {
    return (c == '>' or c == '<' or c == '^' or c == 'v' or c == '#');
  };
  ...
  REP(i, 0, H) {
    ...
    REP(j, 0, W) {
      char c = brd.at(i, j);
      if (c == '>') on = true;
      else if (stop_cell(c)) on = false;
      else if (c == '.' and on) seen[i][j] = true;
    }
    on = false;
    REPrev(j, W - 1, 0) {
      char c = brd.at(i, j);
  ...
  REP(i, 0, H) REP(j, 0, W) if (seen[i][j]) brd.at(i, j) = '#';
  ll big = 1e18;
  ...
  while (not que.empty()) {
    ...
    for (BrdIdx dir : BrdIdx::nbr4) {
      ...
      if (brd.at(bb) == '.' and dist[bb.r][bb.c] == big) {
        dist[bb.r][bb.c] = d + 1;
        que.push(bb);
      }
    }
