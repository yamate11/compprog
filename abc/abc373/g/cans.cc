...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, B, type=Point) [GkFzMtSP]
  auto B = vector(N, Point());
  for (int i = 0; i < N; i++) { Point v; cin >> v; B[i] = v; }
  // @End [GkFzMtSP]
  // @InpVec(N, W, type=Point) [snExoCe3]
  auto W = vector(N, Point());
  for (int i = 0; i < N; i++) { Point v; cin >> v; W[i] = v; }
  // @End [snExoCe3]
  
  vector<ll> ans(N);

  auto f = [&](auto& vecB, auto& vecW) -> void {
    ll n = ssize(vecB);
