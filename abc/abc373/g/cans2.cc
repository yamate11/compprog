...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, B, type=IPoint) [GkFzMtSP]
  auto B = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; B[i] = v; }
  // @End [GkFzMtSP]
  // @InpVec(N, W, type=IPoint) [snExoCe3]
  auto W = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; W[i] = v; }
  // @End [snExoCe3]
  ...
  auto f = [&](ll i, ll j) -> ll {
    IPoint p = B[i], q = W[R[i]], r = B[j], s = W[R[j]];
    IPoint v = (q - p).rotate(1);
    auto g = [&](IPoint z) -> ll {
      ll val = v.x * (z.x - p.x) + v.y * (z.y - p.y);
