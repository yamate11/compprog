...
int main(/* int argc, char *argv[] */) {
  ...
  auto prog = [&](Fp aa, Fp dd, Fp nn) -> Fp {
    return (2 * aa + dd * (nn - 1)) * nn / 2;
  };
  ll N, M; cin >> N >> M;
  ll Q; cin >> Q;
  REP(_q, 0, Q) {
    ...
    if ((a + c) % 2 == 0) {
      ...
      u = prog(1 + a * M + c, 2, p);
      q = Fp((d - c) / 2);
      v = prog(1 + a * M + c + M + 1, 2, q);
    }else {
      p = Fp((d - c) / 2);
      u = prog(1 + a * M + c + 1, 2, p);
      q = Fp((d - c + 1) / 2);
      v = prog(1 + a * M + c + M, 2, q);
    }
    Fp t = Fp((b - a + 1) / 2);
    Fp s = Fp((b - a) / 2);
    Fp sum1 = prog(u, 2 * M * p, t);
    Fp sum2 = prog(v, 2 * M * q, s);
    cout << sum1 + sum2 << "\n";
