...
int main() {
  ...
  auto nxtchar = [&](char c) -> char { return c == '9' ? 'a' : (c + 1); };
  ...
  REP(i, 0, N) {
    string t; cin >> t;
    ll val; cin >> val;
    REP(j, 0, 6) t[j] = nxtchar(t[j]);
    A[du11.from_string(t)] += val;
  }
  REP(i, 0, 6) REP(x, 0, asz) if (du11.d_at(x, i) != 0) A[x] += A[x - du11.pow(i)];
  ...
  REP(_q, 0, Q) {
    ...
    auto setp = [&]() -> bool {
      REP(i, 0, 6) {
        px[i] = (x[i] - '0') * du11.pow(5 - i);
        py[i] = (y[i] - '0' + 1) * du11.pow(5 - i);
    ...
    if (not setp()) ans = 0;
      ...
      REP(z, 0, 1LL << 6) {
        ll p = 0;
        REP(i, 0, 6) p += (z >> i & 1) ? py[i] : px[i];
        ans += (popcount((u64)z) % 2 == 0 ? 1 : -1) * A[p];
      }
    }
    cout << ans << "\n";
  }
