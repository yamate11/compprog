...
int main(/* int argc, char *argv[] */) {
  ...
  ll w = 11;
#endif
  vector A(w, vector(w, vector(w, vector(w, vector(w, vector(w, 0LL))))));

  auto myshowA = [&]() -> void {
#if DEBUG
    REP(i0, 0, w) REP(i1, 0, w) REP(i2, 0, w) REP(i3, 0, w) REP(i4, 0, w) REP(i5, 0, w) {
      DLOGK(i0, i1, i2, i3, i4, i5, A[i0][i1][i2][i3][i4][i5]);
    }
#endif
  };
  auto gete = [&](ll d, string s) -> vector<ll> {
    vector<ll> e(6);
    REP(i, 0, 6) e[i] = s[i] - '0' + d;
    return e;
  };
  REP(i, 0, N) {
    auto e = gete(1, S[i]);
    A[e[0]][e[1]][e[2]][e[3]][e[4]][e[5]] += V[i];
  }
  myshowA();
  REP(i0, 1, w) REP(i1, 1, w) REP(i2, 1, w) REP(i3, 1, w) REP(i4, 1, w) REP(i5, 1, w) {
    A[i0][i1][i2][i3][i4][i5] += A[i0 - 1][i1 - 0][i2 - 0][i3 - 0][i4 - 0][i5 - 0];
  }
  REP(i0, 1, w) REP(i1, 1, w) REP(i2, 1, w) REP(i3, 1, w) REP(i4, 1, w) REP(i5, 1, w) {
    A[i0][i1][i2][i3][i4][i5] += A[i0 - 0][i1 - 1][i2 - 0][i3 - 0][i4 - 0][i5 - 0];
  }
  REP(i0, 1, w) REP(i1, 1, w) REP(i2, 1, w) REP(i3, 1, w) REP(i4, 1, w) REP(i5, 1, w) {
    A[i0][i1][i2][i3][i4][i5] += A[i0 - 0][i1 - 0][i2 - 1][i3 - 0][i4 - 0][i5 - 0];
  }
  REP(i0, 1, w) REP(i1, 1, w) REP(i2, 1, w) REP(i3, 1, w) REP(i4, 1, w) REP(i5, 1, w) {
    A[i0][i1][i2][i3][i4][i5] += A[i0 - 0][i1 - 0][i2 - 0][i3 - 1][i4 - 0][i5 - 0];
  }
  REP(i0, 1, w) REP(i1, 1, w) REP(i2, 1, w) REP(i3, 1, w) REP(i4, 1, w) REP(i5, 1, w) {
    A[i0][i1][i2][i3][i4][i5] += A[i0 - 0][i1 - 0][i2 - 0][i3 - 0][i4 - 1][i5 - 0];
  }
  REP(i0, 1, w) REP(i1, 1, w) REP(i2, 1, w) REP(i3, 1, w) REP(i4, 1, w) REP(i5, 1, w) {
    A[i0][i1][i2][i3][i4][i5] += A[i0 - 0][i1 - 0][i2 - 0][i3 - 0][i4 - 0][i5 - 1];
  }
  myshowA();
  ll Q; cin >> Q;
  REP(q, 0, Q) {
    string x, y; cin >> x >> y;
    if (EXISTS(i, 0, 6, x[i] > y[i])) cout << 0 << "\n";
    else {
      ll ans = 0;
      REP(z, 0, 64) {
        ll sgn = (popcount((u64)z) % 2 == 0) ? 1 : -1;
        vector<ll> e(6);
        REP(i, 0, 6) {
          if (z >> i & 1) e[i] = y[i] - '0' + 1;
          else            e[i] = x[i] - '0';
        }
        ans += sgn * A[e[0]][e[1]][e[2]][e[3]][e[4]][e[5]];
      }
      cout << ans << "\n";
    }
  }
