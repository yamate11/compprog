...
// @@ !! LIM(forall debug digit f:perfmeas)
...
int main() {
  ...
  */
  ...
  REP(i, 0, N) A[S[i]] += V[i];
  REP(i, 0, 6) REP(x, 0, lim) if (du.d_at(x, i) != 0) A[x] += A[x - du.pow(i)];
    ...
    */
    ...
    if (not trivial) {
      for (int z = 0; z < 64; z++) {
        ...
        bool edge = false;
        for (int i = 0; i < 6; i++) {
          if (z >> i & 1) t = t * 10 + (y[i] - '0');
          else if (x[i] > '0') t = t * 10 + (x[i] - '0' - 1);
          else { edge = true; break; }
        }
        if (not edge) {
          if (__builtin_popcount((u64)z) & 1) ans -= A[t];
          else                                ans += A[t];
        }
      }
    }
