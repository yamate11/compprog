...
int main(/* int argc, char *argv[] */) {
  ...
  string N; cin >> N;
  vector tbl_init(2, vector(2, vector(3, vector(1024, Fp(0)))));
  auto tbl = tbl_init;
  tbl[1][1][0][0] = Fp(1);
  for (char ct : N) {
    int t = ct - '0';
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(eq, 0, 2) REP(az, 0, 2) REP(m, 0, 3) REP(y, 0, 1024) {
      if (prev[eq][az][m][y] == 0) continue;
      REP(d, 0, 10) {
        if (eq and d > t) continue;
        int new_eq = eq and d == t;
        int new_az = az and d == 0;
        int new_m = new_az ? 0 : (m * 10 + d) % 3;
        int new_y = new_az ? 0 : (y | (1LL << d));
        tbl[new_eq][new_az][new_m][new_y] += prev[eq][az][m][y];
      }
    }
  }
  Fp t1 = 0;
  Fp t2 = 0;
  Fp t3 = 0;
  REP(eq, 0, 2) REP(m, 0, 3) REP(y, 0, 1024) {
    if (m == 0 and (not (y >> 3 & 1)) and popcount((u64)y) != 3) t1 += tbl[eq][0][m][y];
    if (m != 0 and      (y >> 3 & 1)  and popcount((u64)y) != 3) t2 += tbl[eq][0][m][y];
    if (m != 0 and (not (y >> 3 & 1)) and popcount((u64)y) == 3) t3 += tbl[eq][0][m][y];
  }
  Fp ans = t1 + t2 + t3;
  cout << ans << endl;
