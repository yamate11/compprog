...
int main(/* int argc, char *argv[] */) {
  ...
  ll glim = cc.size();
  vector vec(glim, vector<ll>());
  REP(i, 0, N) {
    vec[cc.c(A[i])].push_back(cc.c(B[i]));
  }
  auto mymax = [&](ll x, ll y) -> ll { return max(x, y); };
  auto st = make_seg_tree(0LL, mymax, vector<ll>(glim, 0LL));
  REP(i, 0, glim) {
    if (not vec[i].empty()) {
      sort(ALL(vec[i]), greater<>());
      for (ll b : vec[i]) {
        ll x = st.query(0, b);
        st.rs(b) = x + 1;
      }
