...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> W(N);
  {
    auto st1 = make_seg_tree(0LL, plus<ll>(), vector<ll>(N, 0LL));
    REP(i, 0, N) {
      W[i] = st1.query(0, P[i]);
      st1.rs(P[i]) = 1;
    }
  ...
  vector<ll> V(N);
  {
    auto st2 = make_seg_tree(0LL, plus<ll>(), vector<ll>(N, 0LL));
    REPrev(i, N - 1, 0) {
      V[i] = st2.query(0, P[i]);
      st2.rs(P[i]) = 1;
    }
  ...
  vector<Fp> pow2(N + 1);
  pow2[0] = Fp(1);
  REP(i, 1, N + 1) pow2[i] = Fp(2) * pow2[i - 1];
  vector<Fp> R(N);
  R[N - 1] = 0;
  REPrev(i, N - 2, 0) R[i] = R[i + 1] + pow2[i] * Fp(V[i + 1]);
  Fp ans4 = 0;
  REP(i, 0, N) ans4 += Fp(W[i]) * R[i] / pow2[i];
  DLOGK(W, R, V);
  Fp ans3 = 0;
  REP(i, 0, N) ans3 += W[i] * V[i];
  Fp ans = ans3 + ans4;
