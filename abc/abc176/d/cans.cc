...
int main(/* int argc, char *argv[] */) {
  ...
  deque<sta> deq;
  deq.emplace_back(0, BrdIdx(ch, cw));
  dist.set(ch, cw, 0);
  while (!deq.empty()) {
    ...
    if (dist.get(bi) < d) continue;
    for (auto dir : BrdIdx::nbr4) {
      if (brd.get(bi + dir) == '.' && dist.get(bi + dir) > d) {
	dist.set(bi + dir, d);
	deq.emplace_front(d, bi + dir);
      }
    }
    for (ll h = -2; h <= 2; h++) {
      for (ll w = -2; w <= 2; w++) {
	BrdIdx bj = bi + BrdIdx(h, w);
	if (brd.get(bj) == '.' && dist.get(bj) > d + 1) {
	  dist.set(bj, d + 1);
