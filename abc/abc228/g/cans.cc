...
int main(/* int argc, char *argv[] */) {
  ...
  vector Ctr(W, vector(H, 0LL));
  REP(i, H) REP(j, W) Ctr[j][i] = C[i][j];
  auto pre = [&](const auto& D) -> vector<vector<ll>> {
    ll dh = SIZE(D);
    ll dw = SIZE(D[0]);
    vector ret(1LL << dh, vector<ll>());
    REP(x, 1LL << dh) {
      vector<ll> q(10);
      REP(i, dh) {
        if ((x >> i) & 1) {
          REP(j, dw) q[D[i][j]] |= (1LL << j);
        }
      }
      REP2(c, 1, 10) if (q[c]) ret[x].push_back(q[c]);
    }
    return ret;
  };
  vector<vector<ll>> T1 = pre(C);
  vector<vector<ll>> T2 = pre(Ctr);
  vector initH(1LL << H, Fp(0));
  auto tblH = initH;
  tblH[(1LL << H) - 1] = Fp(1);
  REP(t, N) {
    vector tblW(1LL << W, Fp(0));
    REP(x, 1LL << H) for (ll y: T1[x]) tblW[y] += tblH[x];
    tblH = initH;
    REP(y, 1LL << W) for (ll x: T2[y]) tblH[x] += tblW[y];
  }
  Fp ans = accumulate(ALL(tblH), Fp(0));
  cout << ans << endl;
