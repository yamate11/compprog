...
int main(/* int argc, char *argv[] */) {
  ...
  if (S != accumulate(ALL(C), 0LL)) {
    cout << "0\n";
    return 0;
  }

  ll x0 = COUNT(i, 0, N, C[i] == 2);
  auto tbl_init = vector<Fp>(x0 + 1, Fp(0));
  auto tbl = tbl_init;
  tbl[x0] = Fp(1);
  DLOGK(tbl);
  ll Si = 0;  
  REP(i, 0, N) {
    ...
    REP(x, 0, x0 + 1) {
      ...
      DLOGK(i, x, y, prev[x]);
      if (R[i] == 2) {
        if (y >= 2) tbl[x] += prev[x] * y * (y - 1) / 2;
        if (x >= 1 and y >= 1) tbl[x - 1] += prev[x] * x * y;
        if (x >= 2) tbl[x - 2] += prev[x] * x * (x - 1) / 2;
        if (x >= 1) tbl[x - 1] += prev[x] * x;
      }else if (R[i] == 1) {
        if (y >= 1) tbl[x] += prev[x] * y;
        if (x >= 1) tbl[x - 1] += prev[x] * x;
      }
    }
    DLOGK(i, tbl);
