...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    ...
    if      (A[2*i] == vmax) {
      if (A[2*i + 1] == vmax) a++;
      else if (A[2*i + 1] == vmax - 1) b++;
      else c++;
    }else if (A[2*i] == vmax - 1) {
      if (A[2*i + 1] == vmax - 1) d++;
      else e++;
    }
  }
  DLOGK(swapped, a, b, c, d, e);
  vector<Fp> pow2(N + 1);
  pow2[0] = 1;
  REP(i, 1, N + 1) pow2[i] = pow2[i - 1] * 2;
  vector<Fp> pow2R(N + 1);
  pow2R[N] = Fp(1) / pow2[N];
  REPrev(i, N - 1, 0) pow2R[i] = pow2R[i + 1] * 2;
  DLOGK(pow2, pow2R);
  Comb<Fp> cb(N);
  Fp pa = 0;
  REP(M, a, b + c + a + 1) {
    if (M != 0) pa += pow2R[b + c + 1] * cb.binom(b + c, M - a) / M;
  }
  Fp pb = 0;
  REP(M, a + 1, b + c + a + 1) {
    if (M != 0) pb += pow2R[b + c] * cb.binom(b + c - 1, M - a - 1) / M;
  }
  Fp pc = pb;
  Fp qb = 0, qd = 0, qe = 0 ;
  if (a == 0) {
    REP(x, 0, e + 1) qb += pow2R[b + c + e] * cb.binom(e, x) / (2*b + c + d + x);
    qd = qb / 2;
    REP(x, 0, e - 1 + 1) qe += pow2R[b + c + e] * cb.binom(e - 1, x) / (2*b + c + d + x + 1);
  }
  DLOGK(pa, pb, pc, qb, qd, qe);
  vector<Fp> ans(2*N, Fp(0));
  REP(i, 0, N) {
    auto myset = [&](Fp x, Fp y) -> void {
      ans[2*i] = x;
      ans[2*i + 1] = y;
    };
    if      (A[2*i] == vmax) {
      if (A[2*i + 1] == vmax)          myset(pa, pa);
      else if (A[2*i + 1] == vmax - 1) myset(pb + qb, qb);
      else                             myset(pc + qb, Fp(0));
    }else if (A[2*i] == vmax - 1) {
      if (A[2*i + 1] == vmax - 1)      myset(qd, qd);
      else                             myset(qe, Fp(0));
    }
    if (swapped[i]) {
