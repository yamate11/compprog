...
// @@ !! LIM(sortedIndex coordCompr binsearch f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&](ll dist) -> bool {
    ...
    REP(i, 0, N + 1) {
      ...
      if (i == N) break;
      auto [l, r] = LR[i];
      ll j0 = ranges::lower_bound(LR, pll(r + dist, 0)) - LR.begin();
      updMax(tbl[j0], tbl[i] + 1);
    }
    DLOGK(dist, tbl);
    return tbl[N] >= K;
  };
  ll dist = binsearch<ll>(check, -1, (ll)1.1e9);
  if (dist == 0) dist = -1;
  cout << dist << endl;
  return 0;
}
