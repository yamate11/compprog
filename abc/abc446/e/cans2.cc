...
// @@ !! LIM(FuncGraph f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto dec = [&](ll e) -> pll { return pll(e / M, e % M); };

  FuncGraph fg(M * M);
  vector fwd(M * M, 0LL);
  vector bwd(M * M, vector<ll>());
  REP(x, 0, M) REP(y, 0, M) {
    ll z = (B * x + A * y) % M;
    ll e0 = enc(x, y);
    ll e1 = enc(y, z);
  ...
  fg.build();
  ll numcc = fg.numcc();
  vector<bool> contCycle(numcc);
  REP(x, 0, M) REP(y, 0, M) {
    ll e = enc(x, y);
    ll cc = fg.cc(e);
    if (x == 0) {
      if (fg.is_on_cycle(e)) contCycle[cc] = true;
    }
  }
  DLOGK(contCycle);
  ll ans = 0;
  REP(cc, 0, numcc) {
    if (not contCycle[cc]) {
      ans += fg.cycle_size(cc);
      ll e0 = fg.repr(cc);

      auto func = [&](auto rF, ll z) -> void {
        auto [x, y] = dec(z);
        if (x == 0) return;
        ans++;
        for (ll e : bwd[z]) rF(rF, e);
      };
      ...
      while (true) {
        for (ll z : bwd[e]) {
          if (not fg.is_on_cycle(z)) { func(func, z); }
        }
        e = fwd[e];
        if (e == e0) break;
      }
