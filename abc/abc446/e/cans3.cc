...
// @@ !! LIM(f:updMaxMin debug doubling)
...
int main(/* int argc, char *argv[] */) {
  ...
  vector vecF(M * M, 0LL);
  REP(x, 0, M) REP(y, 0, M) vecF[enc(x, y)] = enc(y, (B * x + A * y) % M);
  vector vecM(M * M, 0LL);
  REP(y, 0, M) vecM[enc(0, y)] = 1;

  auto dobj = make_doubling_with_monoid<ll>(M * M + 110, M * M, vecF, vecM);

  ll ans = 0;
  REP(x, 0, M) REP(y, 0, M) {
    auto [_dummy, v] = dobj.val(M * M + 100, enc(x, y));
    if (v == 0) ans++;
  }
  cout << ans << endl;
