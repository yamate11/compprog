...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [jqncClSR]
  // @InpMVec(M, ((L, dec=1), R, X)) [rmaFEGF0]
  auto L = vector(M, ll());
  auto R = vector(M, ll());
  auto X = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  using OP = pair<Fp, Fp>;
  auto dummy_add = [&](DAT a, DAT b) -> DAT { return a == DAT() ? b : a; };
  auto mycomp = [&](OP p1, OP p2) -> OP {
    auto [a, b] = p1;
    auto [c, d] = p2;
    return OP(a * c, a * d + b);
  };
  auto myappl = [&](OP f, DAT x) -> DAT {
    if (not x) return x;
    auto [a, b] = f;
    return a * (*x) + b;
  };
  vector<DAT> AA(N); REP(i, 0, N) AA[i] = Fp(A[i]);
  auto st = make_seg_tree_lazy(DAT(), OP(1, 0), dummy_add, mycomp, myappl, AA);

  REP(i, 0, M) {
    Fp p = Fp(1) / Fp(R[i] - L[i]);
    st.update(L[i], R[i], OP(Fp(1) - p, p * X[i]));
  }
  vector ans(N, 0.0);
  REP(i, 0, N) ans[i] = *st.query(i, i + 1);
  REPOUT(i, 0, N, ans[i], " ");
