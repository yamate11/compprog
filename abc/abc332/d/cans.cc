...
int main(/* int argc, char *argv[] */) {
  ...
  IntPerm iph(H, H);
  while (iph.get()) {
    ll numH = calcnum(H, iph);
    IntPerm ipw(W, W);
    while (ipw.get()) {
      ll numW = calcnum(W, ipw);
      auto check = [&]() -> bool {
        REP(i, 0, H) REP(j, 0, W) if (A[iph.at(i)][ipw.at(j)] != B[i][j]) return false;
        return true;
      };
      if (check()) ans = min(ans, numH + numW);
    }
  }
  if (ans == big) ans = -1;
  cout << ans << endl;
