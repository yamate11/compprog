...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl(x2 + 1, vector(x3 + 1, vector(x5 + 1, optional<Fp>())));
  tbl[0][0][0] = Fp(1);
  auto f = [&](auto rF, ll y2, ll y3, ll y5) -> Fp {
    if (y2 < 0 or y3 < 0 or y5 < 0) return Fp(0);
    optional<Fp>& ret = tbl[y2][y3][y5];
    if (not ret) {
      ret =  (    rF(rF, y2 - 1, y3,     y5)
                + rF(rF, y2,     y3 - 1, y5)
                + rF(rF, y2 - 2, y3,     y5)
                + rF(rF, y2,     y3,     y5 - 1)
                + rF(rF, y2 - 1, y3 - 1, y5)
