...
int main(/* int argc, char *argv[] */) {
  ...
  for (ll p : primes) {
    vector<ll> pow;
    for (ll q = 1; q <= N; q *= p) pow.push_back(q);
    auto& vec = (SIZE(vec1) <= SIZE(vec2)) ? vec1 : vec2;
    vector<ll> new_vec;
    for (ll x : vec) {
      for (ll q : pow) {
        if (q > N / x) break;
        new_vec.push_back(q * x);
      }
    }
    vec = move(new_vec);
  }
  ...
  /*
  ll sep = M / 3;
  auto func = [&](ll a, ll b) -> vector<ll> {
    if (a == b) return vector<ll>{1};
    vector<ll> ret;
    auto g = [&](auto rF, ll i, ll val) -> void {
      if (i == b) {
        DLOGK(val);
        ret.push_back(val);
      }
  ...
  auto vec1 = func(0, sep);
  auto vec2 = func(sep, M);
  */
  ...
  ll ans = 0;
  for (ll x1 : vec1) {
    ll m = upper_bound(ALL(vec2), N / x1) - vec2.begin();
    DLOGK(x1, m);
    ans += m;
  }
  cout << ans << endl;
