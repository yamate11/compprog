...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> sign(lim + 1);
  REP2(k, 2, lim + 1) {
    sign[k] = -1;
    for (auto [p, r] : prfacDivSieve(k, divSieve)) {
      if (r >= 2) { sign[k] = 0; break; }
      sign[k] = -sign[k];
    }
  }
  vector<vector<ll>> dlist(N + 1);
  REP2(i, 1, N + 1) {
    for (ll d : getDivisorsDivSieve(i, divSieve)) if (d > 1 and sign[d] != 0) dlist[i].push_back(d);
  }
  DLOGK(dlist);
  ...
  auto do_cnt = [&](ll k) -> ll {
    ...
    for (ll i = k; i <= N; i += k) {
      ll x = P[i];
      DLOGKL("  ", i, x);
      for (ll y : dlist[x]) mp[y]++;
    }
    DLOGK(k, mp);
    ll ret = 0;
    for (auto& [y, c] : mp) ret += sign[y] * c * (c + 1) / 2;
    return ret;
  };
  ll ans = 0;
  REP2(k, 2, N + 1) {
