...
struct small_vector_string {
  ...
  char& operator[](ll i) { return impl[i]; }
...
int main(/* int argc, char *argv[] */) {
  ...
  using myvec = small_vector_string;

  ll N, L; cin >> N >> L;
  // @InpVec(N, S, type=string) [11vQ4Gc3]
  auto S = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; S[i] = v; }
  // @End [11vQ4Gc3]

  myvec start(N), goal(N);
  REP(i, 0, N) {
    start[i] = 0;
    goal[i] = ssize(S[i]);
  }

  auto match = [&](const string& cur, const string& target) -> ll {
    ll k0 = ssize(target);
    ll sz = ssize(cur);
    REPrev(k, k0, 1) if (sz - k >= 0 and target.substr(0, k) == cur.substr(sz - k, k)) {
      DLOGKL("match", cur, target, k);
      return k;
    }
    DLOGKL("match=0", cur, target);
    return 0;
  };

  vector<vector<vector<ll>>> et(N);
  REP(i, 0, N) {
    ll sz = ssize(S[i]);
    et[i] = vector(sz + 1, vector(sigma, 0LL));
    REP(j, 0, sz) REP(d, 0, sigma) et[i][j][d] = match(S[i].substr(0, j) + (char)('a' + d), S[i]);
    REP(d, 0, sigma) et[i][sz][d] = sz;
    DLOGK(i, et[i]);
  }

  auto trans = [&](const myvec& x, ll d) -> myvec {
    myvec ret(N);
    REP(i, 0, N) ret[i] = et[i][x[i]][d];
    DLOGKL("trans", x, d, ret);
    return ret;
  };

  my_umap<myvec, vector<myvec>> t_fwd;
  auto fwd = [&](const myvec& x) -> const vector<myvec>& {
    auto it = t_fwd.find(x);
    if (it == t_fwd.end()) {
      vector<myvec> vec(sigma, myvec(N));
      REP(d, 0, sigma) vec[d] = trans(x, d);
      it = t_fwd.emplace(x, move(vec)).first;
    }
    return it->second;
  };

  my_umap<myvec, Fp> tbl_init;
  auto tbl = tbl_init;
  tbl[start] = Fp(1);
  REP(i, 0, L) {
    auto prev = tbl;
    tbl = tbl_init;
    for (auto [x, n] : prev) {
      const auto& vec = fwd(x);
      REP(d, 0, sigma) tbl[vec[d]] += n;
    }
    DLOGK(i, tbl);
  }
  DLOGK(goal);
