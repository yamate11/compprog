...
int main(/* int argc, char *argv[] */) {
  ...
  vector ev(N + 1, vector(2, vector(2, 0LL)));
  REP(i, 0, N) {
    using sta = tuple<ll, ll, ll>;
    vector<sta> vec{{L[i], 1, 0}, {R[i] + 1, -1, 0}, {N - R[i], 0, 1}, {N - L[i] + 1, 0, -1}};
    ranges::sort(vec);
    pll st(0, 0);
    for (auto [t, da, db] : vec) {
      ev[t][st.first][st.second]--;
      st.first += da;
      st.second += db;
      ev[t][st.first][st.second]++;
    }
  ...
  Fp ans = 0;
  vector st(2, vector(2, 0LL));
  st[0][0] = N;
  REP(i, 0, N + 1) {
    REP(a, 0, 2) REP(b, 0, 2) st[a][b] += ev[i][a][b];
    // DLOGK(i, st[0][0], st[0][1], st[1][0], st[1][1]);
    if (st[0][0] == 0 and st[1][0] <= i and st[0][1] <= N - i) {
      ans += cb.binom(st[1][1], i - st[1][0]);
    }
  }
  cout << ans << endl;
