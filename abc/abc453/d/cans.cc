...
int main(/* int argc, char *argv[] */) {
  ...
  BrdIdx start, goal;
  REP(i, 0, H) REP(j, 0, W) {
    if (brd.at(i, j) == 'S') { start = BrdIdx(i, j); brd.rs(i, j) = '.'; }
    if (brd.at(i, j) == 'G') { goal  = BrdIdx(i, j); brd.rs(i, j) = '.'; }
  }
  string dirchar = "DRUL";

  using Node = pair<BrdIdx, ll>;
  ...
  /*
  vector visited(H, vector(W, vector(5, false)));
  visited[start.r][start.c][4] = true;
  */

  vector prev(H, vector(W, vector(5, Node(BrdIdx(), -1))));

  auto nbr = [&](Node nd) -> vector<Node> {
    vector<Node> ret;
    auto [bi, k_prev] = nd;
    REP(k, 0, 4) {
      if (brd.at(bi) == 'o' and k != k_prev) continue;
      if (brd.at(bi) == 'x' and k == k_prev) continue;
      auto dir = BrdIdx::nbr4[k];
      auto bj = bi + dir;
      if (brd.at(bj) == '#') continue;
      ret.emplace_back(bj, k);
    }
    return ret;
  };
  ...
  auto bfs = [&]() -> ll {
    while (not que.empty()) {
      ...
      if (nd.first == goal) return nd.second;
      for (auto peer : nbr(nd)) {
        auto [bj, kj] = peer;
        if (prev[bj.r][bj.c][kj].second < 0) {
          prev[bj.r][bj.c][kj] = nd;
          que.emplace(bj, kj);
        }
      }
    }
  ...
  if (k >= 0) {
    ...
    string ans;
    while (cur.second < 4) {
      ans += dirchar[cur.second];
      cur = prev[cur.first.r][cur.first.c][cur.second];
    }
    ranges::reverse(ans);
    cout << "Yes\n" << ans << endl;
  }else {
