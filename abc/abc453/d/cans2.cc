...
int main(/* int argc, char *argv[] */) {
  ...
  vector rec(H, vector(W, vector(4, -1LL)));
  using sta = pair<BrdIdx, ll>;
  queue<sta> que;
  que.emplace(start, 4);
  while (not que.empty()) {
    auto [bi, odir] = que.front(); que.pop();
    DLOGKL("popped", bi, odir, dirchar[odir]);
    if (bi == goal) {
      cout << "Yes\n";
      string ans;
      ll k = odir;
    ...
    REP(k, 0, 4) {
      ...
      if (brd.at(bi) == 'o' and k != odir) continue;
      if (brd.at(bi) == 'x' and k == odir) continue;
      auto bj = bi + dir;
      if (bj != start and brd.at(bj) != '#' and rec[bj.r][bj.c][k] == -1) {
        DLOGKL("pushed", bj, k, odir);
        rec[bj.r][bj.c][k] = odir;
        que.emplace(bj, k);
      }
    }
  }
