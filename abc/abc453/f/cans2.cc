...
// @@ !! LIM(tree forall cmpNaive debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto judge = [&]() -> bool {
    ...
    REP(i, 0, N) ans[i]--;
    REP(i, 0, N) {
      if (not (0 <= ans[i] and ans[i] < K)) return false;
      cnt[ans[i]]++;
    }
    REP(i, 0, K) if (cnt[i] > C[i]) return false;
    REP(e, 0, N - 1) {
      set<ll> ss0, ss1;
      auto dfs = [&](auto rF, ll nd, ll mode) -> void {
        if (mode == 0) ss0.insert(ans[nd]);
        else ss1.insert(ans[nd]);
        for (auto [cld, ec] : tr.children_pe(nd)) {
          if (ec == e) rF(rF, cld, 1);
          else rF(rF, cld, mode);
        }
      };
      dfs(dfs, 0, 0);
      auto check = [&]() -> bool {
        for (ll i : ss0) if (ss1.contains(i)) return true;
        return false;
      };
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> vector<ll> {
    ll N, K; cin >> N >> K;
    // @InpNbrList(N, N - 1, nbr, dec=1) [LMiLuUkw]
    ...
    if (N == 2) {
      ll k0 = EXFIND_D(k, 0, K, C[k] >= 2, -1LL);
      if (k0 < 0) return vector<ll>();
      return vector<ll>{k0, k0};
    }

    ll root = EXFIND(i, 0, N, ssize(nbr[i]) >= 2);
    Tree tr(N, root);
    REP(i, 0, N) for (ll j : nbr[i]) if (i < j) tr.add_edge(i, j);
    ll nl = 0;
    REP(i, 0, N) if (tr.num_children(i) == 0) nl++;
    ll nc = 0;
    REP(k, 0, K) if (C[k] >= 2) nc += C[k];
    if (nc < nl) return vector<ll>();
    vector<ll> ans(N, -1LL);
    vector<ll> cLeaf;
    vector<ll> cNode;
    {
      ...
      for (auto [k, w] : tmp) cLeaf.push_back(k);
      for (auto [k, w] : tmp) REP(i, 0, w - 1) cLeaf.push_back(k);
      ll sz = N - nl - (ans[root] >= 0 ? 1 : 0);
      REP(k, 0, K) {
        if (ssize(cNode) == sz) break;
        ll t = 0;
        if (k == k0) t = rem0;
        else if (C[k] == 1) t = 1;
        else if (k0 >= 0 and k > k0) t = C[k];
        else t = 0;
        t = min(t, sz - ssize(cNode));
        REP(i, 0, t) cNode.push_back(k);
      }
    }
    DLOGK(cLeaf);
    DLOGK(cNode);
    ...
    auto dfs = [&](auto rF, ll nd) -> void {
      if (tr.num_children(nd) == 0) ans[nd] = cLeaf[idxL++];
      else {
        if (ans[nd] < 0) ans[nd] = cNode[idxN++];
        for (ll cld : tr.children(nd)) rF(rF, cld);
      }
    };
    dfs(dfs, root);
    return ans;
