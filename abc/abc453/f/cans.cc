...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> vector<ll> {
    ...
    queue<ll> cLeaf1, cLeaf2;
    ll z = 0;
    REP(k, 0, K) {
      if (C[k] >= 2) {
        ll m = max(2LL, min(C[k], nl - z));
        cLeaf1.push(k);
        REP(j, 0, m - 1) cLeaf2.push(k);
        z += m;
        if (z >= nl) break;
      }
    }
    if (z < nl) return vector<ll>();
    while (not cLeaf2.empty()) { cLeaf1.push(cLeaf2.front()); cLeaf2.pop(); }
    vector<ll> used(K, 0LL);
    vector<ll> ans(N, -1LL);
    auto dfs = [&](auto rF, ll nd, ll pt) -> void {
      if (ssize(nbr[nd]) == 1) {
        ll k = cLeaf1.front(); cLeaf1.pop();
        ans[nd] = k;
        used[k]++;
      }else {
        for (ll cld : nbr[nd]) {
    ...
    dfs(dfs, root, -1LL);
    if (not cLeaf1.empty()) {
      ans[root] = cLeaf1.front(); cLeaf1.pop();
    }
    assert(cLeaf1.empty());
    ll idx = 0;
    REP(nd, 0, N) if (ans[nd] < 0) {
      while (idx < K and used[idx] == C[idx]) idx++;
      assert(idx < K);
      ans[nd] = idx;
    }
    return ans;
