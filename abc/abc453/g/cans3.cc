...
int main(/* int argc, char *argv[] */) {
  ...
  vector<vector<sta>> fwd(1);
  using stb = tuple<ll, ll, ll>;
  vector<vector<stb>> query(1);
  vector tnode(N, 0LL);
  ll seq = 1;
  ll qidx = 0;
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      fwd[tnode[x]].emplace_back(seq, y, z);
      tnode[x] = seq;
      fwd.push_back(vector<sta>());
      query.push_back(vector<stb>());
      seq++;
    }else if (tp == 3) {
      ll x, l, r; cin >> x >> l >> r; x--; l--;
      query[tnode[x]].emplace_back(l, r, qidx++);
    }
  }
  ll numQ = qidx;
  ...
  auto dfs = [&](auto rF, ll nd) -> void {
    for (auto [l, r, qi] : query[nd]) ans[qi] = st.query(l, r);
    for (auto [cld, i, z] : fwd[nd]) {
      ll a = st.at(i);
      st.rs(i) = z;
      rF(rF, cld);
      st.rs(i) = a;
