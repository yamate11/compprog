...
int main() {
  ...
  using C = BwAO<AO_def<FpA>>;
  C y1(1), y5(5), y10(10);
  assert(y1 + y5 == C(6));
  assert(FpA(y1 - y10) == FpA(-9));
  assert(y5 * y10 == C(50));
  assert(y10 / y5 == C(2));
  assert(FpA(y5.zero()) == FpA(0));
  assert(FpA(y10.one()) == FpA(1));

  using D = BwAO<AO_vec<double>>;
  D d1(vector<double>({0.0, 8.0}));
  D d2(vector<double>({16.0, 2.0}));
  D d3(vector<double>({2.0, 32.0}));
  D d4 = d1;
  d4 += d2;
  assert(d4 == D(vector<double>({16.0, 10.0})));
  D d5 = d2;
  d5 *= d3;
  assert(vector<double>(d5) == vector<double>({32.0, 64.0}));
