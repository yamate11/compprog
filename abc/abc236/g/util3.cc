...
// AO = Algebra Operations

template<typename T>
struct AO_def { // default operations
  using ult = T;
  ult zero() { return (T)0; }
  ult one() { return (T)1; }
  void subst_add(T& v, const T w) { v += w; }
  void subst_subt(T& v, const T w) { v -= w; }
  void subst_mult(T& v, const T w) { v *= w; }
};

struct AO_double : AO_def<double> {
  ult inverse(ult w) { return 1.0 / w; }
}


struct AO_maxmin { // plus = min, mult = max
  using ult = ll;
  ult zero() { return LLONG_MIN; }
  ult one() { return LLONG_MAX; }
  void subst_add(ult& v, const ult w) { if (v < w) v = w; }
...
template<typename AO>
ult_t<AO> power(ult_t<AO> a, ll b, AO ao) {
  auto two_pow = a;
  auto ret = ao.one();
  while (b > 0) {
...
template<typename T>
T power(T a, ll b) { return power<AO_def<T>>(a, b, AO_def<T>()); }

/*
template<typename AO>
ult_t<AO> power(ult_t<AO> a, ll b) {
  ult_t<AO> two_pow = a;
  ult_t<AO> ret = AO().one();
  while (b > 0) {
    if (b & 1LL) AO().subst_mult(ret, two_pow);
...
template<typename T>
T power(T a, ll b) { return power<AO_def<T>>(a, b); }
*/

int main() {
  using Fp = FpB;
  Fp x = 3;
  cout << power(x, 1000000) << endl;
}
