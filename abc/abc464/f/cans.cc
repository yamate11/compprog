...
int body(istream& cin, ostream& cout) {
  ...
  ll L = N / 2;
  ll H = N - L;
  vector vec(L + 1, vector<ll>());
  REP(x, 0, 1LL << L) {
    ll s = 0;
    REP(i, 0, L) if (x >> i & 1) s += A[i];
    vec[popcount((u64)x)].push_back(s);
  }
  vector acc(L + 1, vector<Fp>{0});
  REP(i, 0, L + 1) {
    ranges::sort(vec[i]);
    for (ll t : vec[i]) acc[i].push_back(acc[i].back() + t);
  }
  DLOGK(vec);
  DLOGK(acc);
  vector ssnum(N + 1, Fp(0));
  vector sssum(N + 1, Fp(0));
  REP(x, 0, 1LL << H) {
    ll s = 0;
    REP(i, 0, H) if (x >> i & 1) s += A[L + i];
    ll pc = popcount((u64)x);
    REP(n, 0, L + 1) {
      ll t = ranges::lower_bound(vec[n], X - s) - vec[n].begin();
      ssnum[n + pc] += t;
      sssum[n + pc] += Fp(t) * s + Fp(acc[n][t]);
    }
  };
  Fp tot = accumulate(ALL(A), Fp(0));
  DLOGK(ssnum);
  DLOGK(sssum);
  Fp ans = 0;
  REP(i, 0, N) {
    ans += (tot * ssnum[i] - sssum[i]) / (N - i) / cb.binom(N, i);
  }
  cout << ans << endl;
  return 0;
