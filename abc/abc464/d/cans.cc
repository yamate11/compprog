...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    // @End [6BykvXWR]
    // @InpVec(N - 1, Y) [9cHaOLnf]
    auto Y = vector(N - 1, ll());
    for (int i = 0; i < N - 1; i++) { ll v; cin >> v; Y[i] = v; }
    // @End [9cHaOLnf]
    ...
    DLOGK(sunny, rainy);
    REP(i, 0, N - 1) {
      auto prev_sunny = sunny;
      auto prev_rainy = rainy;
      if (S[i + 1] == 'S') {
        sunny = max(prev_rainy + Y[i], prev_sunny);
        rainy = max(prev_rainy - X[i + 1], prev_sunny - X[i + 1]);
      }else if (S[i + 1] == 'R') {
        sunny = max(prev_rainy + Y[i] - X[i + 1], prev_sunny - X[i + 1]);
        rainy = max(prev_rainy, prev_sunny);
      }
      DLOGK(i, sunny, rainy);
    }
    return max(sunny, rainy);
  };
