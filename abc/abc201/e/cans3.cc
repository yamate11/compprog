...
int main(/* int argc, char *argv[] */) {
  ...
  vector<vector<sta>> child(N + 1);
  vector<ll> ord;
  auto idfs = [&](auto rF, int nd, int pt) -> void {
    for (auto [cld, wt] : nbr[nd]) {
      if (cld == pt) continue;
      child[nd].emplace_back(cld, wt);
      rF(rF, cld, nd);
    }
    ord.push_back(nd);
  };
  ...
  for (ll sft = 0; sft < sftlim; sft++) {
    auto dfs = [&]() -> void {
      for (ll nd : ord) {
        ...
        Fp csum_even = 0;
        Fp csum_pair = 0;
        for (auto [cld, wt] : child[nd]) {
          auto [o, e, p] = rec[cld];
          if ((wt >> sft) & 1) {
            swap(o, e);
          }
          info.emplace_back(o, e, p);
          csum_odd += o;
          csum_even += e;
          csum_pair += p;
        }
        ...
        for (auto [o, e, p] : info) {
          csum_pair += o;
          cnt_two += e * (csum_odd - o);
          cnt_two += o * (csum_even - e);
        }
        csum_pair += cnt_two / 2;
        rec[nd] = {csum_odd, 1 + csum_even, csum_pair};
      }
    };
    rec = vector<dfs_t>(N + 1);
    dfs();
    auto [_dumo, _dume, npair] = rec[1];
    ans += npair * Fp(1LL << sft);
  }
  cout << ans << endl;
