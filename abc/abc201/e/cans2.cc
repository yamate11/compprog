...
int main(/* int argc, char *argv[] */) {
  ...
  vector<vector<sta>> nbr(N + 1);
  for (ll i = 0; i < N-1; i++) {
    ll u, v, w; cin >> u >> v >> w;
    nbr[u].emplace_back(v, w);
    nbr[v].emplace_back(u, w);
  }
  using dfs_t = tuple<Fp, Fp, Fp>;
#if DEBUG
  const ll sftlim = 10;
#else
  const ll sftlim = 60;
#endif
  auto v_pair = vector(N + 1, vector(sftlim, Fp(0)));
  auto v_odd = vector(N + 1, vector(sftlim, Fp(0)));
  auto v_even = vector(N + 1, vector(sftlim, Fp(0)));

  auto dfs = [&](auto rF, ll nd, ll pt) -> void {
    vector<dfs_t> info;
    for (auto [cld, wt] : nbr[nd]) {
      if (cld == pt) continue;
      rF(rF, cld, nd);
      for (ll sft = 0; sft < sftlim; sft++) {
        if ((wt >> sft) & 1) {
          v_odd[nd][sft] += v_even[cld][sft];
          v_even[nd][sft] += v_odd[cld][sft];
        }else {
          v_odd[nd][sft] += v_odd[cld][sft];
          v_even[nd][sft] += v_even[cld][sft];
        }
        v_pair[nd][sft] += v_pair[cld][sft];
      }
    }
    for (ll sft = 0; sft < sftlim; sft++) {
      Fp cnt_two = 0;
      for (auto [cld, wt] : nbr[nd]) {
        if (cld == pt) continue;
        if ((wt >> sft) & 1) {
          cnt_two += v_even[cld][sft] * (v_even[nd][sft] - v_odd[cld][sft]);
          cnt_two += v_odd[cld][sft] * (v_odd[nd][sft] - v_even[cld][sft]);
        }else {
          cnt_two += v_odd[cld][sft] * (v_even[nd][sft] - v_even[cld][sft]);
          cnt_two += v_even[cld][sft] * (v_odd[nd][sft] - v_odd[cld][sft]);
        }
      }
      v_pair[nd][sft] += cnt_two / 2;
      v_pair[nd][sft] += v_odd[nd][sft];
      v_even[nd][sft] += 1;
    }
  };
  dfs(dfs, 1, -1);
  ...
  DLOGK(v_even);
  Fp ans = 0;
  for (ll i = 0; i < sftlim; i++) {
    ans += v_pair[1][i] * Fp(1LL << i);
  }
  cout << ans << endl;
