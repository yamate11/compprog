...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> D(N);
  REP(i, N) cin >> D[i];
  vector<ll> et(N), etrev(N);
  vector<ll> dist(N);
  vector<vector<sta>> child(N);
  {
    ll seq = -1;
    auto dfs = [&](auto rF, ll nd, ll pt, ll dis) -> void {
      seq++;
      et[seq] = nd;
      etrev[nd] = seq;
      dist[seq] = dis;
      for (auto [cld, d] : nbr[nd]) {
        if (cld == pt) continue;
        child[nd].emplace_back(cld, d);
        rF(rF, cld, nd, dis + d);
      }
    };
    dfs(dfs, 0, -1, 0);
  }
  REP2(i, 1, N) dist[i] += D[et[i]];
  ...
  auto st = [&](){
    ...
    auto st1 = make_seg_tree(unit_dat, unit_op, xAdd, xComp, xAppl, true);
    st1.set_data(dist);
    auto st2 = SegTree(unit_dat, unit_op, xAdd, xComp, xAppl, true);
    return st1;
  }();
  vector<ll> ans(N);
  ans[0] = st.query(1, N);
  st.update(0, 1, D[0]);
  auto dfs2 = [&](auto rF, ll nd, ll pt, ll dis, ll lim) -> void {
    ll myseq;
    if (nd == 0) {
    }else {
      st.update(0, N, dis);
      myseq = etrev[nd];
      st.update(myseq, lim, -2 * dis);
      st.update(myseq, myseq + 1, -D[nd]);
      ll v = st.query(0, N);
      ans[nd] = v;
      st.update(myseq, myseq + 1, D[nd]);
    }
    ll sz = SIZE(child[nd]);
    REP(k, sz - 1) {
      auto [cld, d] = child[nd][k];
      auto [cld2, d2] = child[nd][k+1];
      rF(rF, cld, nd, d, etrev[cld2]);
    }
    if (sz-1 >= 0) {
      auto [cld, d] = child[nd][sz-1];
      rF(rF, cld, nd, d, lim);
    }
    if (nd == 0) {
    }else {
