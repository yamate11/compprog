...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Edge> ve;
  // vector<vector<ll>> n2e(N, vector<ll>(N));
  ...
  vector<ll> dup(N-1);
  REP(j, M-1) {
    auto path = tr.nnpath(A[j], A[j+1]);
    REP(k, SIZE(path) - 1) {
      ll e = tr.node2edgeID(path[k], path[k+1]);
      dup[e]++;
    }
  }
  ...
  if (K + base < 0 || thesz <= K + base) {
    cout << 0 << endl;
  }else {
    cout << cur[K + base] << endl;
  }
