...
template<typename T>
T binsearch_r(auto check, T yes, T no, T err) {
...
int main(/* int argc, char *argv[] */) {
  ...
  CHT<long double, long double, true> cht;
  REPrev(k, N, 1) {
    cht.insert(-k, S[k]);
    auto check = [&](long double m) -> bool {
      long double y = cht.query(m);
      return y >= S[k - 1] + m * (1 - k);
    };
    long double m = binsearch_r<long double>(check, 1.0, 1.1e6, 5e-7);
    ans[k - 1] = m;
  }
  REPOUT(k, 0, N, ans[k], "\n");
