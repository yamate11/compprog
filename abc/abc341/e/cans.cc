...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  string S; cin >> S;
  vector diff(N - 1, false);
  REP(i, 0, N - 1) diff[i] = S[i] != S[i + 1];

  auto myand = [&](bool a, bool b) -> bool { return a and b; };
  auto st = make_seg_tree(true, myand, diff);
