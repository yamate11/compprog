...
int main(/* int argc, char *argv[] */) {
  ...
  auto ok = [&](ll i, ll j) -> bool {
    BrdIdx cur(i, j);
    if (brd.at(cur) == '#') return false;
    REP(k, 0, N) {
      BrdIdx dir;
      if (T[k] == 'L') dir = BrdIdx(0, -1);
      if (T[k] == 'R') dir = BrdIdx(0,  1);
      if (T[k] == 'U') dir = BrdIdx(-1, 0);
      if (T[k] == 'D') dir = BrdIdx(1, 0);
      cur += dir;
      if (brd.at(cur) == '#') return false;
    }
    return true;
  ...
  ll cnt = 0;
  REP(i, 0, H) REP(j, 0, W) if (ok(i, j)) cnt++;
  cout << cnt << endl;
