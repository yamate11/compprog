...
int main(int argc, char *argv[]) {
  ...
  vector<ll> H(N);
  for (ll i = 0; i< N; i++) H.at(i) = xh.at(i).second;

  auto xAdd = [](ll x, ll y) -> ll { return x + y; };
  auto xMod = [](ll g, ll x) -> ll { return x - g; };
  auto xComp = [](ll h, ll g) -> ll { return h + g; };
  auto xMult = [](int k, ll g) -> ll { return k * g; };
  SegTree<ll, ll> st(H, xAdd, xMod, xComp, xMult, 0, 0);
  ...
  for (ll i = 0; i < N; i++) {
    ...
    cnt += thiscnt;
    ll amount = thiscnt * A;
    for (; j < N && xh.at(j).first - xh.at(i).first <= 2*D; j++);
    st.update(i, j, amount);
  }
  cout << cnt << endl;
