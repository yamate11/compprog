...
int main(/* int argc, char *argv[] */) {
  ...
  vector<atcoder::modint998244353> vecm1(lim), vecm2(lim);
  for (ll i = 0; i < lim; i++) {
    vecf1[i] = vec1[i];
    vecf2[i] = vec2[i];
    vecm1[i] = vec1[i];
...
#ifdef USE_CONSTANT_MOD
  auto conved1 = convolution(vecf1, vecf2);
#else
  auto conved1 = convolution_ll(vec1, vec2);
#endif

  double et2 = get_time_sec();

#ifdef USE_CONSTANT_MOD
  auto conved2 = atcoder::convolution(vecm1, vecm2);
#else
  auto conved2 = atcoder::convolution_ll(vec1, vec2);
#endif
