...
int main(/* int argc, char *argv[] */) {
  ...
  vector<bool> hasPow(N + M);
  vector<ll> numTown(N + M);
  REP2(i, 0, N) numTown[i] = 1;
  REP2(i, N, N + M) hasPow[i] = true;
  ll Q; cin >> Q;
  vector<ll> X(Q);
  vector<bool> Ev(E);
  REP(i, Q) {
  ...
  REP(i, E) {
    if (not Ev[i]) {
      ...
      hasPow[c] = hp;
      numTown[c] = nt;
      DLOGK(a, b, c, hasPow[c], numTown[c]);
    }
  }
  ll cnt = 0;
  REP(i, N + M) {
    if (uf.leader(i) == i and hasPow[i]) cnt += numTown[i];
  }
  ans[Q - 1] = cnt;
  REP2R(i, Q - 1, 1) {
    ll e = X[i];
    ...
    if (a == b) {
      ...
      if (hasPow[a]) {
        if (hasPow[b]) {
          ...
          ans[i - 1] = ans[i] + numTown[b];
          hp = true;
        }
      }else {
        if (hasPow[b]) {
          ans[i - 1] = ans[i] + numTown[a];
          hp = true;
        }else {
          ans[i - 1] = ans[i];
          hp = false;
