...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, S, type=string) [gbFS6id0]
  auto S = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; S[i] = v; }
  // @End [gbFS6id0]
  using sta = pair<string, ll>;
  vector<sta> T(N);
  REP(i, 0, N) T[i] = sta(S[i], i);
  sort(ALL(T));
  vector<Fp> rec(N);
  auto dfs = [&](auto rF, ll st, ll en, ll idx, Fp wt) -> void {
    vector<ll> cat(26);
    if (SIZE(T[st].first) <= idx) {
      rec[st] = wt;
      wt += Fp(1);
      st ++;
    }
    ...
    while (p < en) {
      ll q = p + 1;
      for (; q < en and T[q].first[idx] == T[p].first[idx]; q++);
      ll k = q - p;
      rF(rF, p, q, idx + 1, wt + Fp(en - st - k) / Fp(2));
      p = q;
    }
  };
  dfs(dfs, 0, N, 0, 0);
  ...
  vector<Fp> ans(N);
  REP(i, 0, N) ans[T[i].second] = Fp(1) + rec[i];
  REPOUT(i, 0, N, (ll)(ans[i]), "\n");
