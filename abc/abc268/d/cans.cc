...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpVec(N, S, type=string) [JFxYwQOY]
  ...
  ll sz = 0; REP(i, 0, N) sz += SIZE(S[i]);

  ll max_extra = 16 - sz - (N - 1);
  if (max_extra < 0) {
    cout << -1 << endl;
    return 0;
  }
  ...
  while (ip.get()) {
    REP(e, 0, max_extra + 1) {
      IntDupComb idc(N - 1, e);
      while (idc.get()) {
        vector<ll> nus(N - 1, 1);
        REP(j, 0, e) nus[idc.at(j)]++;
        string cand = "";
        REP(i, 0, N - 1) cand += S[ip.at(i)] + string(nus[i], '_');
        cand += S[ip.at(N - 1)];
        if (SIZE(cand) >= 3 and T.find(cand) == T.end()) {
          cout << cand << endl;
          return 0;
        }
      }
