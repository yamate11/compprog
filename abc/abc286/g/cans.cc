...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((U, dec=1), (V, dec=1))) [65utGMPN]
  auto U = vector(M, ll());
  auto V = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  // @End [65utGMPN]
  ll K; cin >> K;
  // @InpVec(K, X, dec=1) [LyLx3YMh]
  auto X = vector(K, ll());
  for (int i = 0; i < K; i++) { ll v; cin >> v; v -= 1; X[i] = v; }
  // @End [LyLx3YMh]
  vector isin(M, false);
  REP(i, 0, K) isin[X[i]] = true;
  
  UnionFind uf1(N);
  REP(i, 0, M) {
    if (not isin[i]) uf1.merge(U[i], V[i]);
  }
