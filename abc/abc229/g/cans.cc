...
int naive(istream& cin, ostream& cout) {
  ...
  ll N = SIZE(S);
  vector<ll> P;
  REP(i, N) if (S[i] == 'Y') P.push_back(i);
  ll szP = SIZE(P);
  REP(m, szP + 1) {
    bool b = [&]() -> bool {
      if (m == 0) return true;
      REP(st, szP - m + 1) {
        REP2(i, P[st], P[st + m -1] - m + 1 + 1) {
          ll due = 0;
          REP(j, m) {
            due += abs(i + j - P[st + j]);
              }
          if (due <= K) return true;
        }
      }
      return false;
    }();
...
int body(istream& cin, ostream& cout) {
  ...
  auto check = [&](ll m) -> bool {
    ...
    ll c = (m - 1) / 2;
    ll due = 0;
    REP(i, c)         due += P[c] - P[i] - (c - i);
    REP2(i, c + 1, m) due += P[i] - P[c] - (i - c);
    DLOGKL("start due", due);
    if (due <= K) {
      DLOG("ret true");
      return true;
    }
    REP2(i, 1, szP - m + 1) {
      ...
      ll c_old = c;
      c++;
      ll dec = P[c_old] - P[i_old] - (c_old - i_old);
      ll right = i_old + m;
      ll inc = P[right] - P[c] - (right - c);
      ll shorten = 0;
      if (m % 2 == 0) {
        shorten = P[c] - P[c_old] - 1;
      }
      due = due + inc - dec - shorten;
      DLOGK(i, due, inc, dec, shorten);
      if (due <= K) {
        DLOG("ret true");
        return true;
      }
  ...
  ll ans = binsearch_i<ll>(check, 0, szP+1);
  cout << ans << endl;
