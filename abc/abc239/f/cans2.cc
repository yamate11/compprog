...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, M) {
    ll a, b; cin >> a >> b; a--; b--;
    D[a]--; D[b]--;
    if (uf.leader(a) == uf.leader(b) or D[a] < 0 or D[b] < 0) { cout << "-1\n";  return 0; }
    uf.merge(a, b);
  }
  using sta = pair<ll, vector<pair<ll, ll>>>;
  vector<sta> rec(N);
  REP(i, N) {
    ll p = uf.leader(i);
    rec[p].first += D[i];
    rec[p].second.emplace_back(i, D[i]);
  }
  DLOGK(rec);
  vector<sta> rec2;
  REP(i, N) {
    if (uf.leader(i) != i) continue;
    if (rec[i].first == 0) { cout << "-1\n"; return 0; }
    rec2.push_back(rec[i]);
  }
  sort(ALL(rec2), greater<sta>());
  DLOGK(rec2);
  using stc = pair<ll, vector<ll>>;
  vector<stc> rec3;
  REP2(i, 1, SIZE(rec2)) {
    stc foo;
    bool fst = true;
    for (auto& [node, rem] : rec2[i].second) {
      REP(j, rem) {
        if (fst) {
          foo.first = node;
          fst = false;
        }else {
          foo.second.push_back(node);
        }
      }
    }
    rec3.push_back(foo);
  ...
  DLOGK(rec3);
  queue<ll> que;
  for (auto& [node, rem] : rec2[0].second) {
    REP(i, rem) que.push(node);
  }
  DLOGKL("init que", que);
  ll idx = 0;
  while (not que.empty()) {
    ll out = que.front(); que.pop();
    auto& [in, outs] = rec3[idx++];
    cout << out << " " << in << "\n";
    for (ll o : outs) que.push(o);
  }
  return 0;
}
