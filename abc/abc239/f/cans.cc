...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> vector<pair<ll, ll>> {
    ...
    REP(i, M) {
      ...
      D[a]--; D[b]--;
      if (D[a] < 0 or D[b] < 0) {
        DLOGKL("pt3 ", a, b, D[a], D[b]);
        return ans;
      }
    }
    vector<ll> cnt(N);
    vector<vector<ll>> rem_nodes(N);
    REP(i, N) {
      ll p = uf.leader(i);
      cnt[p] += D[i];
      if (D[i] > 0) rem_nodes[p].push_back(i);
    }
    DLOGK(cnt);
    DLOGK(rem_nodes);
    vector<ll> bots;
    ...
    REP(i, N) {
      ...
      if (cnt[i] == 1) bots.push_back(rem_nodes[i][0]);
      else ints.push_back(i);
    }
    for (ll i : ints) {
      ll out = -1;
      bool fst = true;
      for (ll j : rem_nodes[i]) {
        REP(k, D[j]) {
          if (fst) {
            out = j;
            fst = false;
          }else {
            ll p = bots.back(); bots.pop_back();
    ...
    assert(SIZE(bots) == 2);
    ans.emplace_back(bots[0], bots[1]);
    return ans;
  };
