...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&](ll L) -> bool {
    auto g = [&](const auto& Z) -> ll {
      auto check2 = [&](ll t) -> bool {
        ll w1 = Z.end() - lower_bound(ALL(Z), t + L + 1);
        ll w2 = upper_bound(ALL(Z), t) - Z.begin();
        return w1 - w2 <= 0;
      };
      ll t = binsearch_i<ll>(check2, (ll)1e9 + 10, -1LL);
      ll ret = 0;
      for (ll z : Z) {
        if (z <= t) ret += t - z;
        else if (z >= t + L) ret += z - (t + L);
      }
      return ret;
    };
    ll mx = g(X);
  ...
  ll L = binsearch_i<ll>(check, (ll)1e9 + 10, -1LL);
  cout << L << endl;
