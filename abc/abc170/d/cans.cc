...
vector<ll> myGetDivisors(ll n, auto& primes) {
...
int main(int argc, char *argv[]) {
  ...
  auto primes = sieve(int(1e3) + 10);
  ll cnt = 0;

  auto chk = [&](ll x) -> bool {
    auto divss = getDivisors(x);
    DLOG("chk: x:", x, "div:", divss);
    for (ll y : divss) {
      if (y == x) continue;
      if (vec.at(y) > 0) return false;
    }
    return true;
  };

  for (ll x = 1; x <= lim; x++) {
    if (vec.at(x) != 1) continue;
    if (chk(x)) cnt++;
  }
  cout << cnt << endl;
