...
// @DefStruct(info, (len, l_len, (l_c, char), r_len, (r_c, char), (unif, bool))) [gAwNp86w]
struct info_t {
  ll len;
  ll l_len;
  char l_c;
  ...
  char r_c;
  bool unif;
  info_t(ll len_ = ll(), ll l_len_ = ll(), char l_c_ = char(), ll r_len_ = ll(), char r_c_ = char(), bool unif_ = bool()) : len(len_), l_len(l_len_), l_c(l_c_), r_len(r_len_), r_c(r_c_), unif(unif_) {}
  friend istream& operator>>(istream& istr, info_t& t) {
    istr >> t.len >> t.l_len >> t.l_c >> t.r_len >> t.r_c >> t.unif;
    return istr;
  }
  friend ostream& operator<<(ostream& ostr, const info_t& t) {
    ostr << "(" << t.len << ", " << t.l_len << ", " << t.l_c << ", " << t.r_len << ", " << t.r_c << ", " << t.unif << ")";
    return ostr;
  }
};
// @End [gAwNp86w]
...
int main(/* int argc, char *argv[] */) {
  ...
  auto my_add = [&](info_t a, info_t b) -> info_t {
    ll new_len = max(a.len, b.len);
    ll new_l_len = a.l_len;
    ll new_r_len = b.r_len;
    bool new_unif = false;
    if (a.r_c == b.l_c) {
      updMax(new_len, a.r_len + b.r_len);
      if (a.unif and b.unif) new_unif = true;
      if (a.unif) new_l_len = a.l_len + b.l_len;
      if (b.unif) new_r_len = a.r_len + b.r_len;
    }
    return info_t(new_len, new_l_len, a.l_c, new_r_len, b.r_c, new_unif);
  };
  vector<info_t> init_vec(N);
  REP(i, 0, N) init_vec[i] = info_t(1, 1, S[i], 1, S[i], true);
  auto st = make_seg_tree(info_t(0, 0, '.', 0, '.'), my_add, init_vec);
