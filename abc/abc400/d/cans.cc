...
int main(/* int argc, char *argv[] */) {
  ...
  BrdIdx start, goal;
  {
    ...
    start = BrdIdx(a - 1, b - 1);
    goal = BrdIdx(c - 1, d - 1);
  }
  ll big = 1e18;
  vector<ll> dist(H * W, big);
  using sta = pair<BrdIdx, ll>;
  deque<sta> deq;
  deq.emplace_back(start, 0);
  dist[brd.enc(start)] = 0;
  while (true) {
    auto [bi, d] = deq.front(); deq.pop_front();
    if (dist[brd.enc(bi)] == d) {
      ...
      for (auto dir : BrdIdx::nbr4) {
        ...
        if (brd.in(bj)) {
          if (brd.at(bj) == '.') {
            ll new_d = d + 0;
            if (new_d < dist[brd.enc(bj)]) {
              dist[brd.enc(bj)] = new_d;
              deq.emplace_front(bj, new_d);
            }
          }else {
            ll new_d = d + 1;
            if (new_d < dist[brd.enc(bj)]) {
              dist[brd.enc(bj)] = new_d;
              deq.emplace_back(bj, new_d);
            }
            bj += dir;
            if (brd.in(bj)) {
              if (new_d < dist[brd.enc(bj)]) {
