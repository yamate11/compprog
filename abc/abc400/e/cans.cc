...
int main(/* int argc, char *argv[] */) {
  ...
  vector<bool> many(lim + 1, false);
  vector<pll> factor(lim + 1, pll(-1, -1));
  REP(i, 2, lim + 1) {
    ll p = divSieve[i];
    ll a = i / p;
    if (many[a]) many[i] = true;
    else if (a == 1) factor[i] = pll(p, -1);
    else {
      auto [q, r] = factor[a];
      if (r < 0) {
        if (q == p) factor[i] = pll(q, -1);
        else        factor[i] = pll(q, p);
      }else if (q == p or r == p) factor[i] = pll(q, r);
      else many[i] = true;
    }
  }
  vector<ll> vec(lim + 1);
  ll cur = 6;
  REP(i, 6, lim + 1) {
    if (not many[i] and factor[i].second >= 0) cur = i;
    vec[i] = cur;
  }
  ...
  REP(_q, 0, Q) {
    ll A; cin >> A;
    ll x = llround(floor(sqrt((long double)A)));
    cout << vec[x] * vec[x] << "\n";
  }
