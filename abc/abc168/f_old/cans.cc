...
int main(int argc, char *argv[]) {
  ...
  for (ll i = 0; i < M; i++) cin >> D.at(i) >> E.at(i) >> F.at(i);

  vector cR(1, 0LL), cC(1, 0LL);
  cR.insert(cR.end(), A.begin(), A.end());
  cR.insert(cR.end(), B.begin(), B.end());
  cR.insert(cR.end(), D.begin(), D.end());
  cC.insert(cC.end(), C.begin(), C.end());
  cC.insert(cC.end(), E.begin(), E.end());
  cC.insert(cC.end(), F.begin(), F.end());
  CoordCompr ccR(move(cR)), ccC(move(cC));
  Board brd(ccR.size * 2 - 1, ccC.size * 2 - 1, '.');
  for (ll i = 0; i < N; i++) {
    for (ll y = 2 * ccR.c(A.at(i)); y <= 2 * ccR.c(B.at(i)); y++) {
      brd.set(y, 2 * ccC.c(C.at(i)), '#');
    }
  }
  for (ll i = 0; i < M; i++) {
    for (ll x = 2 * ccC.c(E.at(i)); x <= 2 * ccC.c(F.at(i)); x++) {
      brd.set(2 * ccR.c(D.at(i)), x, '#');
    }
  }
  DLOG("ccR.rev=", ccR.rev);
  DLOG("ccC.rev=", ccC.rev);
  DLOG(brd);


  Board<bool> visited(ccR.size*2 - 1, ccC.size*2 - 1, false);
  queue<BrdIdx> que;
  que.emplace(2 * ccR.c(0), 2 * ccC.c(0));
  ll ans = 0;

  while (!que.empty()) {
    BrdIdx bi = move(que.front()); que.pop();
    if (visited.get(bi)) continue;
    visited.set(bi, true);
    if (bi.r == 0 || bi.c == 0 || bi.r == brd.nR - 1 || bi.c == brd.nC - 1) {
      cout << "INF" << endl;
      return 0;
    }
    if (bi.r % 2 == 1 && bi.c % 2 == 1) {
      ll r1 = bi.r / 2;
      ll c1 = bi.c / 2;
      ans += (ccR.d(r1+1) - ccR.d(r1)) * (ccC.d(c1+1) - ccC.d(c1));
    }
    for (BrdIdx d : BrdIdx::nbr4) {
      BrdIdx bi0 = bi + d;
      if (brd.get(bi0) == '#') continue;
      if (visited.get(bi0)) continue;
      que.push(move(bi0));
    }
  }
