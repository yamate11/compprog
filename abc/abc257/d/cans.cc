...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&](ll s) -> bool {
    ...
    REP(i, N) REP(j, N) {
      ...
      if (P[i] * s >= abs(X[i] - X[j]) + abs(Y[i] - Y[j])) dist[i][j] = 0;
    }
    REP(k, N) REP(i, N) REP(j, N) {
      if (dist[i][k] == 0 and dist[k][j] == 0) dist[i][j] = 0;
    }
    return EXISTS(i, 0, N, FORALL(j, 0, N, dist[i][j] == 0));
  };
  ll s = binsearch_i<ll>(check, 5e9, 0);
  cout << s << endl;
