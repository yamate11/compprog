...
void impl(istream& cin, ostream& cout, bool isNaive) {
  ...
  auto impl_body = [&]() -> void {
    ...
    using sta = pair<ll, bool>;
    vector<ll> stsize(N);
    auto dfsB = [&](auto rF, ll nd) -> ll {
      stsize[nd] = 1;
      for (ll c : children[nd]) stsize[nd] += rF(rF, c);
      return stsize[nd];
    };
    dfsB(dfsB, 0);
    REP(nd, 0, N) {
      if (ssize(children[nd]) >= 2) {
        auto i0 = ranges::max_element(children[nd], [&](ll i, ll j) { return stsize[i] < stsize[j]; }) - children[nd].begin();
        if (i0 > 0) swap(children[nd][0], children[nd][i0]);
      }
    }
    vector<sta> et_order;
    auto dfsA = [&](auto rF, ll nd) -> void {
      et_order.emplace_back(nd, true);
      for (ll c : children[nd]) rF(rF, c);
      et_order.emplace_back(nd, false);
    };
    dfsA(dfsA, 0);
    vector<ll> et_rev_in(N);
    vector<ll> et_rev_out(N);
    REP(i, 0, 2*N) {
      auto [nd, b] = et_order[i];
      if (b) et_rev_in[nd] = i;
      else   et_rev_out[nd] = i;
    }
#if DEBUG
    REP(nd, 0, N) DLOGK(nd, children[nd]);
    DLOGK(et_order);
    DLOGK(et_rev_in);
    DLOGK(et_rev_out);
#endif
    vector<ll> vec(N);
    auto append = [&](ll nd, ll col) -> void {
      ll x = ++vec[col];
      if (x > ans_m[nd]) {
        ans_m[nd] = x;
        ans_k[nd] = 1;
    ...
    auto dfs = [&](auto rF, ll nd) -> void {
      const auto& cvec = children[nd];
      if (ssize(cvec) > 0) {
        ll cld0 = cvec[0];
        if (ssize(cvec) >= 2) {
          REP(i, 1, ssize(cvec)) {
            rF(rF, cvec[i]);
            REP(j, et_rev_in[cvec[i]], et_rev_out[cvec[i]]) {
              auto [n, b] = et_order[j];
              if (b) vec[C[n]] = 0;
            }
          }
          DLOGK(nd, cvec[1], et_rev_in[cvec[1]], et_rev_out[nd]);
        }
        DLOGKL("after clear", nd, vec);
        rF(rF, cld0);
        ans_m[nd] = ans_m[cld0];
        ans_k[nd] = ans_k[cld0];
        if (ssize(cvec) >= 2) {
          REP(i, et_rev_in[cvec[1]], et_rev_out[nd]) {
            auto [n, b] = et_order[i];
            if (b) append(nd, C[n]);
          }
        }
