...
void test01(auto gen, int repNum, int vecSizeMax, int iMax) {
  auto xAdd = [](int x, int y) -> int { return min(x, y); };
  auto xMod = [](int g, int x) -> int {
    if (g < 0) return x;
    return g;
  };
  auto xComp = [](int h, int g) -> int {
    if (h < 0) return g;
    return h;
  };
  auto xMult = [](int k, int g) -> int { return g; };
  SegTree<int, int> st0(vector<int>(), xAdd, xMod, xComp, xMult, INT_MAX, -1);
  ...
  st0.compareNaive(gen, genInt, genInt, repNum, vecSizeMax, iMax);
}
...
// Update: adds a number uniformly onto the range
void test02(auto gen, int repNum, int vecSizeMax, int iMax) {
  auto xAdd = [](Test1 a,  Test1 b) -> Test1 {
    return Test1(a.first + b.first, a.second + b.second); };
  auto xMod = [](ll g, Test1 a) -> Test1 {
  ...
  Test1 unit1(0, 0);
  ll unit2 = 1;
  SegTree<Test1, ll> st0(vector<Test1>(), xAdd, xMod, xComp, xMult,
			 unit1, unit2);
  ...
  st0.compareNaive(gen, genTest1, genInt, repNum, vecSizeMax, iMax);
}

void test03(auto gen, int repNum, int vecSizeMax, int iMax) {
  auto xAdd = [](int x, int y) -> int { return x + y; };
  auto xMod = [](int g, int x) -> int { return g + x; };
  auto xComp = [](int h, int g) -> int { return h + g; };
  auto xMult = [](int k, int g) -> int { return k * g; };
  SegTree<int, int> st0(vector<int>(), xAdd, xMod, xComp, xMult, 0, 0);
...
int main(int argc, char *argv[]) {
  ...
  test01(gen, 1000, 30, 30);
  test02(gen, 1000, 30, 30);
  test03(gen, 1000, 30, 30);
