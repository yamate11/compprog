...
int main(/* int argc, char *argv[] */) {
  ...
  while (ipa.get()) {
    IntPerm ipb(N, N);
    while (ipb.get()) {
      if (EXISTS(i, 0, N, ipa.at(i) == ipb.at(i))) continue;
      IntPerm ipc(N, N);
      while (ipc.get()) {
        auto leftmost = [&](ll i) -> char {
          if (ipa.at(i) < ipb.at(i) and ipa.at(i) < ipc.at(i)) return 'A';
          if (ipb.at(i) < ipc.at(i)) return 'B';
          return 'C';
        };
        auto topmost = [&](ll i) -> char {
          ll aj = EXFIND(j, 0, N, ipa.at(j) == i);
          ll bj = EXFIND(j, 0, N, ipb.at(j) == i);
          ll cj = EXFIND(j, 0, N, ipc.at(j) == i);
          if (aj < bj and aj < cj) return 'A';
          if (bj < cj) return 'B';
          return 'C';
        };
        if (EXISTS(i, 0, N, ipa.at(i) == ipc.at(i))) continue;
        if (EXISTS(i, 0, N, ipb.at(i) == ipc.at(i))) continue;
        if (not FORALL(i, 0, N, R[i] == leftmost(i))) continue;
        if (not FORALL(i, 0, N, C[i] == topmost(i))) continue;
        cout << "Yes\n";
