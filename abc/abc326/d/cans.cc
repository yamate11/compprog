...
int main(/* int argc, char *argv[] */) {
  ...
  auto brd = vector(N, string(N, '.'));
  auto dfs = [&](auto rF, ll i, ll j) -> void {
    if (i == N) {
      if (not FORALL(c, 0, 3, FORALL(m, 0, N, EXISTS(k, 0, N, brd[k][m] == (char)('A' + c))))) return;
      cout << "Yes\n";
      REP(m, 0, N) cout << brd[m] << "\n";
      throw myexc();
    }else if (j == N) {
      if (not FORALL(c, 0, 3, EXISTS(k, 0, N, brd[i][k] == (char)('A' + c)))) return;
      rF(rF, i + 1, 0);
    }else {
      for (char c = 'A'; c <= 'C'; c++) {
        if (EXISTS(k, 0, j, brd[i][k] == c) or EXISTS(k, 0, i, brd[k][j] == c)) continue;
        if (FORALL(k, 0, j, brd[i][k] == '.') and c != R[i]) continue;
        if (FORALL(k, 0, i, brd[k][j] == '.') and c != C[j]) continue;
        brd[i][j] = c;
        rF(rF, i, j + 1);
        brd[i][j] = '.';
      }
