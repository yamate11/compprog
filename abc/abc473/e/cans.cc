...
// @@ !! LIM(f:updMaxMin debug unordered_map cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  // @End [PFeF4bm4]
  vector<ll> B(N + 1);
  REP(i, 0, N) B[i + 1] = (B[i] + A[i]) % K;
  safe_umap<ll, vector<ll>> mp;
  REP(i, 0, N + 1) mp[B[i]].push_back(i);
  vector<ll> tbl(N + 1);
  REP(i, 0, N + 1) {
    if (i + 1  < N + 1) updMax(tbl[i + 1],  tbl[i]);
    const auto& vec = mp[B[i]];
    auto it = ranges::lower_bound(vec, i + 1);
    if (it != vec.end()) updMax(tbl[*it], tbl[i] + 1);
  }
  cout << tbl[N] << "\n";
  return 0;
}
