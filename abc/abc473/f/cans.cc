...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1LL << 60;
  pll unit_dat(0LL, big);
  auto add = [&](pll a, pll b) -> pll {
    auto [sa, ma] = a;
    auto [sb, mb] = b;
    return pll(sa + sb, min(ma, sa + mb));
  };
  vector<pll> init_vec(N);
  REP(i, 0, N) {
    if (S[i] == 'A') init_vec[i] = pll(1, 1);
    else             init_vec[i] = pll(-1, -1);
  }
  auto segt = make_seg_tree(unit_dat, add, init_vec);
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ll i; cin >> i; i--;
      char c; cin >> c;
      if (c == 'A') segt.rs(i) = pll(1, 1);
      else          segt.rs(i) = pll(-1, -1);
    }else if (tp == 2) {
      ll l, r; cin >> l >> r; l--;
      auto [s, m] = segt.query(l, r);
      if (m >= 0) cout << "Yes\n";
      else cout << "No\n";
