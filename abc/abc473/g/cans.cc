...
Poly pts(const Poly& f, Fp c, Comb<Fp>& cb) {
  ll d = f.degree();
  vector<Fp> x(d + 1), y(d + 1);
  Fp cpow = 1;
  REP(i, 0, d + 1) {
    x[d - i] = cb.fact(i) * f.getCoef(i);
  ...
  z.resize(d + 1);
  vector<Fp> g(d + 1);
  REP(i, 0, d + 1) g[i] = z[d - i] * cb.inv_fact(i);
  return Poly(move(g));
}
...
int main(/* int argc, char *argv[] */) {
  ...
  Comb<Fp> cb(N);
  auto calc = [&](auto rF, ll d) -> Poly {
    if (d == 1) return Poly::X;
    else if (d % 2 == 0) {
      auto f = rF(rF, d / 2);
      return f * pts(f, d / 2, cb);
    }else {
      return rF(rF, d - 1) * (Poly::X + (d - 1));
      // return rF(rF, d - 1) * ((d - 1) * Poly::X + 1);
    }
  };
  auto f = calc(calc, N);
  Fp ans = f.getCoef(L) / cb.fact(N);
  cout << ans << "\n";
