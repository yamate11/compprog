...
// @DefStruct(sta, (pidx, cval, sum)) [tOqfatYH]
struct sta {
  ll pidx{};
  ll cval{};
  ll sum{};
  string show() const {
    return "(" + to_string(pidx) + ", " + to_string(cval) + ", " + to_string(sum) + ")";
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  vector res(N + 1, vector(K + 1, vector<ll>()));

  for (ll i = 0; true; i++) {
    if (i * N > K) break;
    res[N][i * N].push_back(i);
  }
  REPrev(n, N - 1, 2) {
    REP(s, 0, K + 1) {
      for (ll i = 0; true; i++) {
        if (s - i * n < 0) break;
        if (not res[n + 1][s - i * n].empty()) res[n][s].push_back(i);
      }
      DLOGK(n, s, res[n][s]);
    }
  }
  vector<ll> vec;
  auto showres = [&](auto rF, ll n, ll s) -> void {
    DLOGKL("showres", n, s);
    if (res[n][s].empty()) return;
    if (n == N) {
      vec.push_back(res[n][s][0]);
