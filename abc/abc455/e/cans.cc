...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  string S; cin >> S;
  vector cnt(3, vector(N + 1, 0LL));
  REP(i, 0, N) {
    REP(d, 0, 3) {
      cnt[d][i + 1] = cnt[d][i] + (S[i] == 'A' + d ? 1 : 0);
    }
  }
  DLOGK(cnt);
  auto f = [&](ll d1, ll d2) -> ll {
    safe_umap<ll, ll> mp;
    REP(i, 0, N + 1) mp[cnt[d2][i] - cnt[d1][i]]++;
    DLOGK(d1, d2, mp);
    ll ret = 0;
    for (auto [_diff, n] : mp) {
      ret += n * (n - 1) / 2;
  ...
  auto g = [&]() -> ll {
    safe_umap<pll, ll> mp;
    REP(i, 0, N + 1) {
      ll t01 = cnt[1][i] - cnt[0][i];
      ll t02 = cnt[2][i] - cnt[0][i];
      mp[pll(t01, t02)]++;
    }
    ll ret = 0;
    for (auto [_diff, n] : mp) {
  ...
  ll x20 = f(2, 0);
  ll x012 = g();
  DLOGK(x01, x12, x20, x012);
  ll x = x01 + x12 + x20 - 2 * x012;
  ll ans = N * (N + 1) / 2 - x;
  cout << ans << endl;
