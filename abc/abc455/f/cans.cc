...
struct DAT {
  ...
  friend ostream& operator<<(ostream& ostr, const DAT& t) {
    ostr << "(" << t.sum << ", " << t.sqsum << ", " << t.len << ")";
    return ostr;
  }
  static DAT forOne(ll a) { return DAT(a, a * a, 1); }
};
// @End [seTjtLYV]
...
int main(/* int argc, char *argv[] */) {
  ...
  using OP = Fp;
  auto add = [&](const DAT& a, const DAT& b) -> DAT { return DAT(a.sum + b.sum, a.sqsum + b.sqsum, a.len + b.len); };
  auto comp = [&](OP f, OP g) -> OP { return f + g; };
  auto appl = [&](OP f, const DAT& a) -> DAT {
    return DAT(a.sum + f * a.len, a.sqsum + 2 * f * a.sum + a.len * f * f, a.len);
  };
  DAT forzero = DAT::forOne(0);
  vector<DAT> init_vec(N, forzero);
  auto st = make_seg_tree_lazy(DAT(), Fp(0), add, comp, appl, init_vec);

  REP(_q, 0, Q) {
    ll l, r, a; cin >> l >> r >> a; l--;
    st.update(l, r, a);
    auto [sum, sqsum, len] = st.query(l, r);
    Fp ans = (sum * sum - sqsum) / 2;
    cout << ans << "\n";
  }
