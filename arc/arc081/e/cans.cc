...
int main(/* int argc, char *argv[] */) {
  ...
  DLOGK(P, Q);
  using sta = tuple<ll, char, ll>;
  sta unit = {LLONG_MAX, 'z', 0};
  auto myadd = [&](sta x, sta y) -> sta { return min(x, y); };
  auto st = make_seg_tree<sta>(unit, myadd, vector(N + 1, unit));
  vector R(N, 0LL);
  st.update(0, {0, 'z', 0});
  REP(i, 0, N) {
    auto [v, c, j] = st.query(Q[i + 1], i + 1);
    R[i] = j;
    st.update(i + 1, {v + 1, A[i], i + 1});
  }
  DLOGK(R);
  DLOGK(st);
  ll vmin = LLONG_MAX, jmin = -1, dmin = -1;
  REP(d, 0, numalpha) {
    auto [v, c, j] = st.query(P[d], N + 1);
    if (v < vmin) {
      vmin = v;
      jmin = j;
      dmin = d;
