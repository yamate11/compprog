...
int main(/* int argc, char *argv[] */) {
  ...
  vector<bool> eble(N, false);
  REP(i, 0, N) {
    if (i + 1 < N and A[i] > A[i + 1]) eble[i] = true;
    if (i > 0 and A[i - 1] < A[i]) eble[i] = true;
  }
  ...
  REP(i, 0, N) init_vec[i] = DAT(A[i], i);
  auto st = make_seg_tree(pll(0, -1), myadd, init_vec);

  vector<ll> ans(N);
  auto func = [&](auto rF, ll lft, ll rgt, ll bar, ll sval) -> void {
    if (lft == rgt) return;
    auto [v, idx] = st.query(lft, rgt);
    if (not eble[idx]) {
      REP(i, lft, rgt) ans[i] = v;
      return;
    }
    ll t = S[rgt] - S[lft];
    if (t > bar) {
      ans[idx] = sval;
      rF(rF, lft, idx, v, sval);
      rF(rF, idx + 1, rgt, v, sval);
    }else {
      ans[idx] = t;
      rF(rF, lft, idx, v, t);
      rF(rF, idx + 1, rgt, v, t);
