...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [5395O6t8]
  // @InpVec(N, P, dec=1) [I4lkAWQj]
  auto P = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; P[i] = v; }
  // @End [I4lkAWQj]
  // @InpVec(N, Q, dec=1) [fEtdewrz]
  auto Q = vector(N, ll());
  ...
  auto f = [&](auto& C, auto& revR) {
    ...
    if (cnt != accumulate(ALL(C), 0LL)) throw myexc();
    while (ssize(cC) >= 1 and C[cC.back()] == 0) cC.pop_back();
    return cC;
  };
  ...
  try {
    ...
    auto cB = f(B, revQ);
    vector revCB(N, -1LL);
    REP(i, 0, ssize(cB)) revCB[cB[i]] = i;

    auto mymax = [&](ll a, ll b) -> ll { return max(a, b); };
    auto st = make_seg_tree(0LL, mymax, vector<ll>(ssize(cB), 0LL));
    REP(i, 0, ssize(cA)) {
      ll j = revCB[cA[i]];
      if (j < 0) continue;
      ll x = st.query(0, j);
    ...
    cout << ssize(cA) + ssize(cB) - st.query(0, ssize(cB)) << endl;
  }catch(myexc& e) {
    cout << -1 << endl;
    return 0;
  }
