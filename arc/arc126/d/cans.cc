...
int naive(istream& cin, ostream& cout) {
  ...
  do {
    ...
    DLOGK(nrev);
    if (nrev < 0) continue;
    for (ll i = 0; i < K - 1; i++) nrev += min(i + 1, K - i - 1) * (v[i + 1] - v[i] - 1);
    ans = min(ans, nrev);
  }while (ic.next());
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, N) {
    ...
    tbl[1LL << a] = 0;
    REP(x, 1<<K) {
      if (prev[x] >= big) continue;
      if (not (x >> a & 1)) {
        ll sft = __builtin_popcountll(x & ~((1LL << a) - 1));
        ll newX = x | (1LL << a);
        if (tbl[newX] > prev[x] + sft) {
          DLOGKL("update1", newX, x, tbl[newX], prev[x], sft, prev[x] + sft);
          tbl[newX] = prev[x] + sft;
        }
      }
      ll pc = __builtin_popcount(x);
      if (pc * 2 <= K)  {
        if (tbl[x] > prev[x] + pc) {
          DLOGKL("update2", x, tbl[x], prev[x], pc, prev[x] + pc);
          tbl[x] = prev[x] + pc;
        }
      }else if (pc < K)  {
        if (tbl[x] > prev[x] + K - pc) {
          DLOGKL("update3", x, tbl[x], prev[x], prev[x] + K - pc);
          tbl[x] = prev[x] + K - pc;
        }
      }else if (pc == K) {
        if (tbl[x] > prev[x]) {
          DLOGKL("update4", x, tbl[x], prev[x]);
          tbl[x] = prev[x];
        }
      }
    }
