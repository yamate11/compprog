...
int naive(istream& cin, ostream& cout) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((L, dec=1), (R, dec=0), (X, dec=1))) [zY4KoISC]
  auto L = vector(M, ll());
  auto R = vector(M, ll());
  auto X = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  while (ip.get()) {
    auto check = [&]() -> bool {
      auto vec = ip.vec_view();
      REP(i, 0, M) {
        ll j = max_element(vec.begin() + L[i], vec.begin() + R[i]) - vec.begin();
...
int body(istream& cin, ostream& cout) {
  ...
  vector<sta> RLX;
  REP(i, 0, M) RLX.emplace_back(R[i], L[i], X[i]);
  sort(ALL(RLX));
  
  Comb<Fp> cb(N);

  vector tbl(N + 1, vector(N + 1, Fp(1)));
  REPrev(l, N - 1, 0) {
    vector<bool> proh(N);
    ll idx = 0;
    REP(r, l + 1, N + 1) {
      while (idx < ssize(RLX)) {
        auto [r0, l0, x0] = RLX[idx];
        if (r < r0) break;
        if (l <= l0) proh[x0] = true;
        idx++;
      }
      Fp val = 0;
      REP(x, l, r) if (not proh[x]) {
        val += cb.binom(r - l - 1, x - l) * tbl[l][x] * tbl[x + 1][r];
      }
      tbl[l][r] = val;
    }
  }
