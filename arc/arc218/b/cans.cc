...
// @@ !! LIM(unordered_map forall debug perm cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> bool {
    ...
    auto check = [&](auto rF, const vector<ll>& vec) -> bool {
      auto sub = [&]() -> bool {
        if (FORALL(i, 0, K, vec[i] == 0)) return true;
        if (vec[0] > 0) {
          REP(n, 1, vec[0] + 1) {
            auto w = vec;
            w[0] -= n;
            if (rF(rF, w)) return false;
          }
          return true;
        }else {
          for (ll n = 1; true; n++) {
      ...
      auto [it, b] = mp.try_emplace(vec, false);
      if (b) {
        it->second = sub();
        // DLOGK(vec, it->second);
      }
  ...
  ll T; cin >> T;
  REP(t, 0, T) cout << (solve() ? "Bob" : "Alice") << "\n";
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> bool {
    ...
    auto check = [&](auto rF, auto& vec, ll base) -> bool {
      if (vec.empty()) return true;
      if (ssize(vec) == 1) {
        if (vec[0] == base + 1) return true;
        else return false;
      }
      if (vec.back() == base) {
        vec.pop_back();
        if (vec.back() == base) return false;
        return not rF(rF, vec, base);
      }
      else if (vec.back() == base + 1) {
        return not rF(rF, vec, base + 1);
      }
      else {
        return false;
      }
