...
// @@ !! LIM(mod unordered_map debug power)
  ...
  */
  ...
  REP(i, 0, N) {
    ...
    REP(j, 0, M) {
      ...
      auto it = mp.find(a);
      if (it == mp.end()) t += base;
      else t += base * it->second;
    }
    S = M * S + t;
    safe_umap<ll, ll> submp;
    REP(j, 0, M) submp[A[i][j]]++;
    for (auto [a, n] : submp) {
      Fp r = Fp(M - n) / Fp(M);
      auto [it, b] = mp.try_emplace(a, r);
      if (not b) it->second *= r;
    }
    base *= M;
  }
  cout << S << endl;
