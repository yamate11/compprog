...
int main(/* int argc, char *argv[] */) {
  ...
  auto rec = vector(p + 1, vector(M+1, Fp(0)));
  rec[0] = vector(M+1, Fp(1));
  for (ll i = 1; i <= p; i++) {
    for (ll j = 1; j <= M; j++) {
      for (ll k = 1; j * k <= M; k ++) {
        rec[i][j * k] += rec[i-1][j] * rec[i-1][k];
      }
    }
  }
  DLOGK(rec);
  auto cur = vector(M+1, Fp(0));
  ll x = N;
  bool first = true;
  while (q > 0) {
    if (q <= x) {
