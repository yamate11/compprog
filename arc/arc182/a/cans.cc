...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> Fp {
    ...
    vector<bool> prohLo(Q, false);
    vector<bool> prohHi(Q, false);
    REP(i, 0, Q) REP(j, i + 1, Q) {
      if (V[i] > V[j]) {
        if (P[i] == P[j]) return 0;
        if (P[i] < P[j]) {
          if (prohLo[i]) return 0;
          if (prohHi[j]) return 0;
          prohHi[i] = prohLo[j] = true;
        }
        if (P[i] > P[j]) {
          if (prohHi[i]) return 0;
          if (prohLo[j]) return 0;
          prohLo[i] = prohHi[j] = true;
        }
      }
    }
    Fp ret = 1;
    REP(i, 0, Q) {
      if (not prohHi[i] and not prohLo[i]) ret *= 2;
    }
    return ret;
  };
  cout << solve() << endl;
