...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> ll {
    ...
    while (icA.get()) {
      vector<ll> vec(n, 2);
      REP(i, 0, a) vec[icA[i]] = 0;
      vector<ll> rem;
      REP(i, 0, n) if (vec[i] == 2) rem.push_back(i);
      IntComb icB(n - a, b, rem);
      while (icB.get()) {
        auto vec2 = vec;
        REP(i, 0, b) vec2[icB[i]] = 1;
        ll cur = 0;
        REP(i, 0, n) {
          ll j = (i == n - 1) ? 0 : i + 1;
          ll k = (i == 0)     ? n - 1 : i - 1;
          if (vec2[j] == vec2[k] and (vec2[i] + 1 - vec2[j]) % 3 == 0) cur++;
        }
        DLOGK(icB.vec_view(), vec2, cur);
        updMax(ans, cur);
      }
    }
    return ans;
...
int body(istream& cin, ostream& cout) {
  auto solve = [&]() -> ll {
    ...
    ll xC = c - (C1 + B1 + 1);
    if (xA + xB + xC >= 2) {
      if (A1 <= B1 and A1 <= C1) A1++;
      else if (B1 <= C1 and B1 <= A1) B1++;
      else C1++;
    }
    ll ret = 0;
    if (A1 >= 0 and B1 >= 0 and C1 >= 0) ret = A1 + B1 + C1;
    updMax(ret, min(a - 1, c));
    updMax(ret, min(b - 1, a));
    updMax(ret, min(c - 1, b));
    updMax(ret, min(a - 1, b + c - 1));
    updMax(ret, min(b - 1, c + a - 1));
    updMax(ret, min(c - 1, a + b - 1));
    return ret;
  };
