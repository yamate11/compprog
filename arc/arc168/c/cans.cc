...
int main(/* int argc, char *argv[] */) {
  ...
  Fp ans = 0;
  REP(ab, 0, 1 + K) REP(bc, 0, 1 + K - ab) REP(ca, 0, 1 + K - (ab + bc)) {
    if (ab + ca > numA or ab + bc > numB or bc + ca > numC) continue;
    Fp tmp = comb.binom(numA, ab) * comb.binom(numA - ab, ca)
      * comb.binom(numB, ab) * comb.binom(numB - ab, bc)
      * comb.binom(numC, ca) * comb.binom(numC - ca, bc);
    ans += tmp;
  }
  REP(circ, 1, 1 + K / 2) REP(ab, 0, 1 + K - 2 * circ) REP(bc, 0, 1 + K - 2 * circ - ab) REP(ca, 0, 1 + K - 2 * circ - ab - bc) {
    if (circ + ab + ca > numA or circ + ab + bc > numB or circ + bc + ca > numC) continue;
    Fp tmp1 = comb.binom(numA, ab + circ) * comb.binom(numA - (ab + circ), ca)
            * comb.binom(numB, bc + circ) * comb.binom(numB - (bc + circ), ab)
            * comb.binom(numC, ca + circ) * comb.binom(numC - (ca + circ), bc);
    Fp tmp2 = comb.binom(numA, ca + circ) * comb.binom(numA - (ca + circ), ab)
            * comb.binom(numB, ab + circ) * comb.binom(numB - (ab + circ), bc)
            * comb.binom(numC, bc + circ) * comb.binom(numC - (bc + circ), ca);
    ans += tmp1 + tmp2;
  }
  cout << ans << endl;
