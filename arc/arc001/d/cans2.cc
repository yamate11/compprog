...
int main(int argc, char *argv[]) {
  ...
  auto calc = [&](const auto& recF, Point pu, Point pv) -> double {
    ll u = pu.y;
    ll v = pv.y;
    vector<Point> vec;
    vec.push_back(pu);
    if (u < v) {
      for (ll i = u + 1; i < v; i++) vec.push_back(left.at(i));
    }else {
      for (ll i = u - 1; i > v; i--) vec.push_back(right.at(i));
    }
    vec.push_back(pv);
    DLOG("vec", vec);
    auto ch = cvxHull(vec);
    DLOG("ch", ch);
    if (ch.size() == 2) return (ch.at(0) - ch.at(1)).len();
    double rv = 0;
    for (ll i = 0; i < (ll)ch.size() - 1; i++) {
      rv += recF(recF, ch.at(i+1), ch.at(i));
    }
    return rv;
  };
  vector<Point> lCv = cvxHull(left);
  DLOG("lCv", lCv);
  double ans = 0;
  for (ll i = 0; i < (ll)lCv.size() - 1; i++) {
    ans += calc(calc, lCv.at(i+1), lCv.at(i));
  }
  cout << ans << endl;
  return 0;
}
