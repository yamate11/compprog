...
// counter-clockwise
vector<Point> cvxHull(const auto& pts) {
  vector<Point> ret;
  ll j = -1;
  for (Point p : pts) {
    while (j > 0) {
      Point pj = ret.at(j);
      Point pk = ret.at(j-1);
      if ((p - pj).outerProd(pk - pj) > 0) break;
      ret.resize(j);
      j--;
    }
    ret.push_back(p);
...
int main(int argc, char *argv[]) {
  ...
  for (ll i = 0; i <= N; i++) {
    ...
    if (i == 0) {
      ...
      right.emplace_back(b, i);
    }
  }

  double ans = 0.0;
  vector<Point> lCvx, rCvx;
  lCvx.push_back(start);
  rCvx.push_back(start);
  lCvx.push_back(left.at(1));
  rCvx.push_back(right.at(1));
  ll lb = 0;
  ll rb = 0;
  
  auto cvxRan = [&](ll rot, const auto& cvx, ll b, const Point& pt) -> ll {
    ll j = cvx.size() - 1;
    while (j-1 >= b) {
      Point pivot = cvx.at(j);
      Point dir1 = pt          - pivot;
      Point dir2 = cvx.at(j-1) - pivot;
      if (rot * dir1.outerProd(dir2) > 0) return j;
      j--;
    }
    return j;
  };
  auto part = [&](int side, ll& ub, ll& vb,
		  auto& uCvx, auto& vCvx, Point newP) -> void {
    DLOG("part", "side", side, "ub", ub, "vb", vb,
	 "uCvx", uCvx, "vCvx", vCvx, "newP", newP);
    auto far = [&]() -> bool {
      if ((ll)vCvx.size() <= vb + 1) return false;
      Point base = vCvx.at(vb);
      Point sec = vCvx.at(vb+1);
      Line lTh = Line::connect(base, sec);
      Line lHor(Point(1,0), newP);
      double th = lTh.intersect(lHor).x;
      return side * (newP.x - th) >= 0;
    };
    if (far()) {
      ll j = cvxRan(-side, vCvx, vb, newP);
      for (ll k = vb; k < j; k++) {
	ans += (vCvx.at(k+1) - vCvx.at(k)).len();
      }
      uCvx.resize(0);
      uCvx.push_back(vCvx.at(j));
      ub = 0;
      vb = j;
    }else {
      ll j = cvxRan(side, uCvx, ub, newP);
      uCvx.resize(j + 1);
    }
    DLOG("out part", "uCvx", uCvx, "vb", vb);
  };

  for (ll i = 2; i <= N; i++) {
    DLOG("i", i);
    part( 1, lb, rb, lCvx, rCvx, left.at(i));
    part(-1, rb, lb, rCvx, lCvx, right.at(i));
    lCvx.push_back(left.at(i));
    rCvx.push_back(right.at(i));
    DLOG("i", i, "lCvx", lCvx, "lb", lb, "rCvx", rCvx, "rb", rb);
  }

  assert((ll)lCvx.size() == lb + 2 && (ll)rCvx.size() == rb + 2);
  ans += (lCvx.at(lb+1) - lCvx.at(lb)).len();
  cout << ans << endl;
