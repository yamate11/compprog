...
int main(/* int argc, char *argv[] */) {
  ...
  REP(m, 0, M + 1) {
    if (m == 0) {
      if (N % 3 == 1) ans += Fp(X) * Fp(X);
    }else if (m == X) {
      if (N % 3 == 2) ans += Fp(X) * Fp(X);
    }else {
      DLOGK(m);
      ll t = 2;
      auto f = [&](ll& c, ll& d) -> void {
        ll p = c / d;
        if (p % 2 != 0) p--;
        ll k = min(p / 2, (N - t) / 3);
        c -= 2 * k * d;
      ...
      ll a = m, b = X;
      while (t < N and a != b and a != 0 and b != 0) {
        if (a >= 2 * b) {
          f(a, b);
          DLOGKL("A ", a, b, t);
        } else if (2 * a <= b) {
        ...
        if (t == N or a == b or a == 0 or b == 0) break;
        if (N - t < 3 or (a < 2 * b and b < 2 * a)) {
          ll tmp = abs(a - b);
          b = a;
          a = tmp;
          t++;
      ...
      if (t == N) {
        ...
        if ((N - t) % 3 == 0) ans += Fp(a) * Fp(b);
      }else if (a == 0) {
        if ((N - t) % 3 == 2) ans += Fp(b) * Fp(b);
      }else if (b == 0) {
        if ((N - t) % 3 == 1) ans += Fp(a) * Fp(a);
      }else assert(0);
    }
  }
  cout << ans << endl;
