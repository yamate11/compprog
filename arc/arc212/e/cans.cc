...
// @@ !! LIM(cartesianTree mod debug cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  set<vector<ll>> ss;
  auto f = [&](auto rF, auto pre, auto rem) -> void {
    if (ssize(rem) == 1) {
      ss.insert(pre);
      return;
    }
    REP(i, 0, ssize(rem) - 1) {
      if (rem[i] < rem[i + 1]) {
        pre.push_back(rem[i]);
        auto rrr = rem;
        rrr.erase(rrr.begin() + i);
        rF(rF, pre, rrr);
        pre.pop_back();
      }else {
        pre.push_back(rem[i + 1]);
        auto rrr = rem;
        rrr.erase(rrr.begin() + i + 1);
        rF(rF, pre, rrr);
        pre.pop_back();
      }
    }
...
int body(istream& cin, ostream& cout) {
  ...
  // @End [p3Apntt7]
  Comb<Fp> cb(N);
  CartesianTree ct(P, greater<ll>());
  auto f = [&](auto rF, ll lob, ll rob, ll nd) -> Fp {
    if (nd < 0) return Fp(1);
    ll vL = rF(rF, lob, nd, ct.left[nd]);
    ll vR = rF(rF, nd, rob, ct.right[nd]);
    ll lenL = nd - lob - 1;
    ll lenR = rob - nd - 1;
    ll len = lenL + lenR + 1;
    DLOGK(nd, lob, rob, vL, vR);
    if (len == 1) return Fp(1);
    if (lob == -1 and rob == N) {
      Fp v = cb.binom(len - 1, lenL) * vL * vR;
      DLOGKL("a: ", v);
      return v;
    } else if (lob == -1) {
      Fp v = cb.binom(len, lenR + 1) * vL * vR;
      DLOGKL("b: ", v);
      return v;
    }else if (rob == N) {
      Fp v = cb.binom(len, lenL + 1) * vL * vR;
      DLOGKL("c: ", v);
      return v;
    }else {
      Fp case1 = cb.binom(len - 1, lenL) * vL * vR;
      Fp case2 = cb.binom(len - 1, lenR + 1) * vL * vR;
      Fp case3 = cb.binom(len - 1, lenL + 1) * vL * vR;
      Fp v = case1 + case2 + case3;
      DLOGKL("d: ", v, case1, case2, case3);
      return v;
    }
  };
  Fp ans = f(f, -1, N, ct.root);
  cout << ans << endl;
