...
int main(/* int argc, char *argv[] */) {
  ...
  rep(i, lim-1) rep(j, lim-1) S[i + 1][j + 1] += S[i][j + 1];
  rep(i, lim-1) rep(j, lim-1) S[i + 1][j + 1] += S[i + 1][j];

  auto check = [&](ll t) -> bool {
    rep(i, lim - t) rep(j, lim - t) {
      ll n = S[i + t][j + t] - S[i + t][j] - S[i][j + t] + S[i][j];
      if (n >= K) return true;
    }
    return false;
  };
  ll t = binsearch_i<ll>(check, lim, 0);
  Comb cb(N);
  Fp cnt = 0;
  rep(i, lim - t) rep(j, lim - t) {
    ll n = S[i + t][j + t] - S[i + t][j] - S[i][j + t] + S[i][j];
    if (n >= K) cnt += cb.comb(n, K);
  }

  cout << t - 1 << endl << cnt << endl;
