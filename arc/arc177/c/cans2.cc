...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](auto& brd, char ok, char ng, auto start, auto goal) -> ll {
    ll dist = 0;
    vector<BrdIdx> cands;

    auto proc_ok = [&](auto rF, auto bi) -> void {
      brd.at(bi) = fin;
      if (bi == goal) throw myexc();
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (brd.at(bj) == ok) {
          rF(rF, bj);
        }else if (brd.at(bj) == ng) {
          cands.push_back(bj);
          brd.at(bj) = fin;
        }
      }
  ...
  ll N; cin >> N;
  Board<char> brd1(N, N, fin);
  cin >> brd1;
  Board<char> brd2(N, N, fin);
  REP(i, 0, N) REP(j, 0, N) brd2.at(i, j) = brd1.at(i, j);
  
  ll x = f(brd1, 'R', 'B', BrdIdx(0, 0), BrdIdx(N - 1, N - 1));
  ll y = f(brd2, 'B', 'R', BrdIdx(0, N - 1), BrdIdx(N - 1, 0));
  DLOGK(x, y);
  cout << x + y << endl;
