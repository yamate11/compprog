...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1e18;
  auto f = [&](char me, char you, auto start, auto goal) -> ll {
    deque<pair<ll, BrdIdx>> deq;
    vector dist(N * N, big);
    dist[brd.enc(start)] = 0;
    deq.emplace_back(0, start);
    while (not deq.empty()) {
      auto [d, bi] = deq.front(); deq.pop_front();
      if (dist[brd.enc(bi)] == d) {
        if (bi == goal) return d;
        for (auto dir : BrdIdx::nbr4) {
          auto bj = bi + dir;
          if (brd.at(bj) == me) {
            if (d < dist[brd.enc(bj)]) {
              dist[brd.enc(bj)] = d;
              deq.emplace_front(d, bj);
            }
          }else if (brd.at(bj) == you) {
            if (d + 1 < dist[brd.enc(bj)]) {
              dist[brd.enc(bj)] = d + 1;
              deq.emplace_back(d + 1, bj);
            }
          }
        }
      }
  ...
  ll x = f('R', 'B', BrdIdx(0, 0), BrdIdx(N - 1, N - 1));
  ll y = f('B', 'R', BrdIdx(0, N - 1), BrdIdx(N - 1, 0));
  cout << x + y << endl;
