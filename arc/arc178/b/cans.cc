...
int main(/* int argc, char *argv[] */) {
  ...
  /*
  vector pow10(19, Fp(0));
  pow10[0] = Fp(1);
  REP(i, 1, 19) pow10[i] = pow10[i - 1] * Fp(10);
  */

  auto dmn = [&](ll k) -> Fp {
    return power<Fp>(Fp(10), k - 1);
  };
  auto dmx = [&](ll k) ->Fp {
    return dmn(k + 1) - Fp(1);
  };

  auto init_diff_num = [&](Fp a, Fp d, Fp n) -> Fp {
    return (a + (a + (n - 1) * d)) * n / Fp(2);
  };
  ...
  auto solve = [&]() -> Fp {
    ...
    if (A1 > A2) swap(A1, A2);
    if (A2 == A3) {
      Fp a = dmx(A2) - dmn(A2) - dmn(A1) + 1;
      if (A1 < A2) {
        return init_diff_num(a, Fp(-1), dmx(A1) - dmn(A1) + 1);
      }else {
        return init_diff_num(a, Fp(-1), dmx(A2) - dmn(A2) - dmn(A2) + 1);
      }
    }else if (A2 + 1 == A3) {
      Fp a = dmn(A1);
      if (A1 < A2) {
        ...
        Fp x = init_diff_num(a, Fp(1), dmn(A2 + 1) - dmn(A2) - dmn(A1) + 1);
        Fp y = (dmn(A2 + 1) - dmn(A2)) * (dmx(A2) - (dmn(A2 + 1) - dmn(A2)));
        return x + y;
      }
    }else return Fp(0);
  };
