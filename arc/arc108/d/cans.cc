...
// @@ !! LIM(mod f:power cmpNaive)
// --> f:gcd f:intDiv mod f:power cmpNaive
...
int naive(istream& cin, ostream& cout) {
  ...
  for (ll i = 2; i < N; i++) {
    ...
    for (string s : vs) {
      for (ll j = 0; j < i - 1; j++) {
	char c;
	if (s[j] == 'A' && s[j + 1] == 'A') c = caa;
	if (s[j] == 'A' && s[j + 1] == 'B') c = cab;
	if (s[j] == 'B' && s[j + 1] == 'A') c = cba;
	if (s[j] == 'B' && s[j + 1] == 'B') c = cbb;
	string t = s.substr(0, j + 1) + c + s.substr(j + 1);
	newvs.push_back(t);
      }
    }
    sort(newvs.begin(), newvs.end());
    auto it = unique(newvs.begin(), newvs.end());
    newvs.erase(it, newvs.end());
    vs = move(newvs);
  }
  cout << vs.size() << endl;
