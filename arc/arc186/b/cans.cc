...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> parent(N + 1, N);
  vector<ll> bigger_child(N + 1, N);
  bigger_child[N] = 0;
  REP(i, 1, N) {
    ll y = (A[i] == 0) ? N : A[i] - 1;
    ll x = bigger_child[y];
    ...
    parent[i] = y;
    parent[x] = i;
    DLOGK(i, parent, bigger_child);
  }
  ll root = bigger_child[N];
  DLOGK(parent);
  Tree tr(N, root);
  REP(i, 0, N) {
    if (i != root) tr.add_edge(parent[i], i);
  }
  Comb<Fp> cb(N);
  auto dfs = [&](auto rF, ll nd) -> Fp {
    if (tr.num_children(nd) == 0) return Fp(1);
    if (tr.num_children(nd) == 1) {
      ll c = tr.child(nd, 0);
      return rF(rF, c);
    }
    assert(tr.num_children(nd) == 2);
    ll c0 = tr.child(nd, 0);
    ll c1 = tr.child(nd, 1);
    ll s0 = tr.stsize(c0);
    ll s1 = tr.stsize(c1);
    Fp a0 = rF(rF, c0);
    Fp a1 = rF(rF, c1);
    return cb.binom(s0 + s1, s0) * a0 * a1;
  };
  Fp ans = dfs(dfs, root);
  cout << ans << endl;
