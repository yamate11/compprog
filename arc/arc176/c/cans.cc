...
int naive(istream& cin, ostream& cout) {
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((A, dec=1), (B, dec=1), (C, dec=1))) [BZMA7fnW]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  auto C = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  IntPerm ip(N, N);
  while (ip.get()) {
    if (FORALL(i, 0, M, max(ip.at(A[i]), ip.at(B[i])) == C[i])) ans++;
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  vector<ll> sbcvals(N + 1, 0);
  REP(i, 0, N) sbcvals[i + 1] = sbcvals[i] + bcvals[i];

  vector nbr(N, vector<pll>());
  REP(i, 0, M) {
    nbr[A[i]].emplace_back(C[i], B[i]);
    nbr[B[i]].emplace_back(C[i], A[i]);
  }

  vector<ll> val(N, -1LL);

  vector<ll> rest(N, N);
  auto chkset = [&](ll i, ll v) -> void {
    if (val[i] >= 0 and val[i] != v) throw myexc();
    val[i] = v;
    rest[i] = -1;
  };

  auto restrict = [&](ll i, ll v) -> void {
    assert(rest[i] >= 0);
    rest[i] = min(rest[i], v);
  };
  ...
  try {
    ...
    REP(i, 0, N) {
      ...
      if (sz > 1) {
        ll j = 1;
        while (j < sz and nbr[i][0].first == nbr[i][j].first) j++;
        if (j > 1) chkset(i, nbr[i][0].first);
        REP(k, j, sz) chkset(nbr[i][k].second, nbr[i][k].first);
      }
    }
    REP(i, 0, M) {
      ll a = A[i], b = B[i], c = C[i];
      if (val[a] >= 0 and val[b] >= 0) {
        if (max(val[a], val[b]) != c) throw myexc();
      }else if (val[a] >= 0) {
        if (val[a] == c) restrict(b, c);
        else if (val[a] < c) chkset(b, c);
        else throw myexc();
      }else if (val[b] >= 0) {
        if (val[b] == c) restrict(a, c);
        else if (val[b] < c) chkset(a, c);
        else throw myexc();
      }
    }
