...
/*
  DigitUtil du(10) ... 10進表記
  DigitUtil du(2) ... 2進表記
  du.numDigits(x) ... 何桁ある?
  du.digits(x) ... 長さ numDigits の配列．du.digits(x)[q] は，下からq桁めの数．
  du.digitL(x) ... 下からq桁めの数
  du.digitH(x) ... 上からq桁めの数
  du.powL(x) ... b^X <= x < b^(X+1) なるX
  du.powH(x) ... b^X < x <= b^(X+1) なるX
 */

struct DigitUtil {
  int base;
  DigitUtil(int base_) : base(base_) { assert(base_ == 2); }
  const int numDigits(ll x) {
    if (x == 0) return 1;
    if (base == 2) {
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Fp> nums(nd + 1);
  nums[1] = 1;
  REP2(i, 2, nd) nums[i] = nums[i - 1] * 2;
  nums[nd] = M - (1LL << (nd - 1)) + 1;
  vector<Fp> tbl_init(nd + 1);
  auto tbl = nums;
  REP2(i, 1, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
