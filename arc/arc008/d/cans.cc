...
using Info = pair<double, double>;

bool isUnit(Info g) { return g.first > 100; }
...
int main(int argc, char *argv[]) {
  ...
  auto xAdd = [](Info x, Info y) -> Info {
    return make_pair(x.first * y.first, x.second * y.first + y.second);
  };
  auto xMod = [](Info g, Info x) -> Info {
    return isUnit(g) ? x : g; };
  auto xComp = [](Info h, Info g) -> Info {
    return isUnit(h) ? g : h; };
  auto xMult = [](int k, Info g) -> Info { return g; };

  ll N, M; cin >> N >> M;
  vector<ll> P(M);
  vector<double>A(M), B(M);
  for (ll i = 0; i < M; i++) cin >> P.at(i) >> A.at(i) >> B.at(i);
  vector<ll> ord(M);
  iota(ord.begin(), ord.end(), 0);
  sort(ord.begin(), ord.end(),
       [&](ll i, ll j) { return P.at(i) < P.at(j); });
  map<ll, ll> conv;
  ll prev = -1;
  ll idx = -1;
  for (int i = 0; i < M; i++) {
    if (P.at(ord.at(i)) != prev) {
      idx++;
      conv[P.at(ord.at(i))] = idx;
      prev = P.at(ord.at(i));
    }
  }

  Info unitT(1.0, 0.0);
  Info unitS(1000.0, 0.0);
  vector<Info> initVec(M, Info(1.0, 0.0));
  SegTree<Info, Info> st(initVec, xAdd, xMod, xComp, xMult, unitT, unitS);

  double vMax = 1.0;
  double vMin = 1.0;
  for (ll i = 0; i < M; i++) {
    ll p = conv.at(P.at(i));
    st.update(p, p+1, Info(A.at(i), B.at(i)));
    Info x = st.query(0, M);
    double val = x.first + x.second;
    vMax = max(vMax, val);
    vMin = min(vMin, val);
  }
  cout << vMin << endl << vMax << endl;
