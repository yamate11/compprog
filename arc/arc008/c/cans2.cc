...
int main(int argc, char *argv[]) {
  ...
  while (!pque.empty()) {
    ...
    double t = p.first;
    ll i = p.second;
    // if (time.at(i) < big) continue;
    if (time.at(i) < t) continue;
    //// time.at(i) = t;
    for (ll j = 0; j < N; j++) {
      // if (time.at(j) < big) continue;
      double vmin = min(T.at(i), R.at(j));
      double tt = hypot(X.at(i) - X.at(j), Y.at(i) - Y.at(j)) / vmin;
      //// if (time.at(j) <= tt + t) continue;
      double newT = time.at(i) + tt;
