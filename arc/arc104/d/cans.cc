...
int main(/* int argc, char *argv[] */) {
  ...
  auto SP1 = [&](ll i) { return 1 - Pol::Xn((K + 1) * i); };
  auto SP2 = [&](ll i) { return 1 - Pol::Xn(i); };

  Pol bigp = Fp(1);
  for (ll i = 1; i <= N - 1; i++) {
    bigp.selfMult(SP1(i));
    bigp.selfDivFPS(SP2(i));
  }
  Pol smallp = Fp(1);
  vector<Fp> ans(N + 1);
  for (ll x = 1; x <= (N + 1) / 2; x++) {
    if (x > 1) {
      smallp.selfMult  (SP1(x - 1));
      smallp.selfDivFPS(SP2(x - 1));
      bigp.  selfMult  (SP2(N - x + 1));
      bigp.  selfDivFPS(SP1(N - x + 1));
    }
    DLOGK(x, smallp, bigp);
    Fp sum = 0;
    for (ll i = 0; i <= smallp.degree(); i++) {
      sum += smallp.getCoef(i) * bigp.getCoef(i);
    }
    sum *= K + 1;
    sum -= 1;
    ans[x] = ans[N + 1 - x] = sum;
