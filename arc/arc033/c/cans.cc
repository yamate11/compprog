...
int main(int argc, char *argv[]) {
  ...
  ll Q; cin >> Q;
  vector<ll> T(Q), X(Q), coord;
  for (ll i = 0; i < Q; i++) {
    cin >> T.at(i) >> X.at(i);
    if (T.at(i) == 1) coord.push_back(X.at(i));
  }

  sort(coord.begin(), coord.end());
  map<ll, ll> mp;
  for (ll i = 0; i < (ll)coord.size(); i++) mp[coord.at(i)] = i;

  auto xAdd = [](ll x, ll y) -> ll { return x + y; };
  auto xMod = [](ll g, ll x) -> ll { return g == -1 ? x : g; };
  auto xComp = [](ll h, ll g) -> ll { return h == -1 ? g : h; };
  auto xMult = [](int k, ll g) -> ll { return g; };
  SegTree<ll, ll> st(vector<ll>(Q), xAdd, xMod, xComp, xMult, 0, -1);
  ...
  for (ll i = 0; i < Q; i++) {
    if (T.at(i) == 1) {
      ...
      ll x = X.at(i);
      // min z s.t. query(0, z) >= x
      ll ng = 0;
      ll ok = Q;
      while (ng + 1 < ok) {
	ll mid = (ng + ok) / 2;
	if (st.query(0, mid) >= x) ok = mid;
	else                       ng = mid;
      }
      cout << X.at(oX[ok-1]) << "\n";
      st.update(ok-1, ok, 0);
