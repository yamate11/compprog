...
// public:
  ...
  */
  ...
  // Sorry, only implemented for vertical.
  // Use transpose twice for horizontal.
  void self_sweepout() {
...
int main(int argc, char *argv[]) {
  ...
  for (ll i = 0; i < N; i++) {
    string s; cin >> s;
    for (ll j = 0; j < N; j++) mat.at(i,j) = s.at(j) == '1' ? 1 : 0;
  }
  cout << ((int)(mat.determinant()) == 0 ? "Even" : "Odd") << endl;
