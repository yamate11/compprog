...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = big;
  while (dip.get()) {
    if (FORALL(i, 0, N, L[i] + dip.at(i) <= R[i])) {
      ll sum = 0;
      REP(j, 0, N) REP(k, j + 1, N) {
        sum += abs(L[j] + dip.at(j) - (L[k] + dip.at(k)));
      }
      ans = min(ans, sum);
    }
  }
...
int body(istream& cin, ostream& cout) {
  ...
  enum kind_t { LEFT, RIGHT};
  using ev_t = tuple<ll, kind_t, ll>;
  vector<ev_t> ev;
  REP(i, 0, N) {
    ev.emplace_back(L[i], LEFT, i);
    ev.emplace_back(R[i], RIGHT, i);
  }
  sort(ALL(ev));
  ll wr = 0, wl = N;
  ll t0 = -1;
