...
template <typename T, typename S, typename BINSCH>
struct SegTree {
...
auto makePred(ll x) { return [&](ll q) -> bool { return q >= x; }; }
...
int main(int argc, char *argv[]) {
  ...
  SegTree<ll, ll, decltype(makePred(0))>
    st(vector<ll>(N+1, 1), xAdd, xMod, xComp, 0LL, -1LL);
  st.update(0, 1, 0);
  ...
  while (true) {
    ...
    ll rNew = (n * r) % K;
    if (rNew != 0) xxx++;
    ll t = st.binsch(makePred(xxx));
    DLOG("xxx, t", xxx, t, st.query(0, t), st.query(0, t-1));
    xxx = t - 1;
    st.update(xxx, xxx+1, 0);
    pos.at(xxx) = true;
    cout << xxx << "\n";
    if (rNew == 0) break;
    n--;
  ...
  for (ll i = N; i >= 1; i--) if (!pos.at(i)) cout << i << "\n";
