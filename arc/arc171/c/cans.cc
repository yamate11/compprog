...
int main(/* int argc, char *argv[] */) {
  ...
  Comb<Fp> cb(N);
  vector P(N + 1, Fp(0));
  REP(n, 0, N + 1) REP(r, 0, n + 1) P[n] += cb.perm(n, r);
   vector tblNC(N, Fp(0));
   vector tblC(N, Fp(0));
  auto dfs = [&](auto rF, ll nd) -> void {
    ll k = SIZE(tr.children(nd));
    if (k == 0) {
      tblNC[nd] = 1;
      tblC[nd] = 1;
    }
    else {
      auto v_init = vector(k + 1, Fp(0));
      auto v = v_init;
      v[0] = 1;
      REP(i, 0, k) {
        auto prev = move(v);
        v = v_init;
        ll cld = tr.children(nd)[i];
        rF(rF, cld);
        REP(j, 0, i + 1) {
          v[j] += prev[j] * tblNC[cld];
          v[j + 1] += prev[j] * (j + 1) * tblC[cld];
        }
      }
      tblNC[nd] = accumulate(ALL(v), Fp(0));
      tblC[nd] = 0; REP(j, 0, k + 1) tblC[nd] += v[j] * (j + 1);
    }
  };
  dfs(dfs, 0);
  cout << tblNC[0] << endl;
