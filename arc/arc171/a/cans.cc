  ...
  auto solve = [&]() -> bool {
    ...
    REP(x, 0, 1LL << N) {
      ...
      while (ipi.get()) {
        IntComb ipj(N, A);
        while (ipj.get()) {
          vector brd(N, vector<bool>(N, false));
          REP(i, 0, A) REP(j, 0, N) brd[ipi.at(i)][j] = true;
          REP(i, 0, A) REP(j, 0, N) brd[j][ipj.at(i)] = true;
          ll cnt = 0;
          REP(i, 0, N) REP(j, 0, N) {
            if (not brd[i][j]) {
              cnt++;
              brd[i][j] = true;
              if (i + 1 < N) brd[i + 1][j] = true;
            }
          }
          ans = max(ans, cnt);
          if (B <= ans) return true;
        }
      }
    }
    return false;
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> bool {
    ll N, A, B; cin >> N >> A >> B;
    if (2*A - 1 < N) {
      if (B <= A * A + A * divCeil(N - 2*A, 2) + (N - 2 * A) * divCeil(N, 2)) return true;
      return false;
    }else if (A <= N) {
      if (B <= (N - A) * (N - A)) return true;
      return false;
    }else return false;
  };
