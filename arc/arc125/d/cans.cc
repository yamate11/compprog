...
int main(/* int argc, char *argv[] */) {
  ...
  vector rec(N + 1, queue<ll>());
  REP(i, 0, N) rec[A[i]].push(i);
  vector<ll> tbl(N);

  using DAT = optional<Fp>;
  using OP = pair<bool, Fp>;
  auto myadd = [&](DAT x, DAT y) -> DAT {
    Fp xx = x ? *x : Fp(0);
    Fp yy = y ? *y : Fp(0);
    return xx + yy;
  };
  auto mycomp = [&](OP x, OP y) -> OP {
    auto [xf, xa] = x;
    auto [yf, ya] = y;
    return OP(xf or yf, xa + ya);
  };
  auto myapply = [&](OP x, DAT y) -> DAT {
    auto [xf, xa] = x;
    if (xf) return xa;
    else if (not y) return y;
    else return xa + *y;
  };
  auto st = make_seg_tree_lazy<DAT, OP>(Fp(0), pll(false, Fp(0)), myadd, mycomp, myapply,
                                        vector<DAT>(N + 1, optional<Fp>()));
  st.update(N, N + 1, OP(true, Fp(0)));
  REP(k, 1, N + 1) if (not rec[k].empty()) {
    ll i = rec[k].front();
    // DLOGKL("in_init", i);
    st.update(i, i + 1, OP(true, Fp(1)));
    // DLOGKL("  val", st.query(i, i + 1));
  }
  REP(i, 0, N) {
    DLOGKL("top loop", i);
    DAT c = st.query(i, i + 1);
    if (not c) c = Fp(0);
    auto& que = rec[A[i]];
    ...
    if (que.empty()) {
      ...
      ll k = que.front();
      st.update(k, k + 1, OP(true, Fp(0)));
      rt = k + 1;
    }
    DLOGKL("update", i + 1, rt, *c);
    st.update(i + 1, rt, OP(false, *c));
#if DEBUG    
    REP(m, 0, N + 1) DLOGKL("    ", m, st.query(m, m + 1));
#endif
  }
  DAT ans = st.query(N, N + 1);
  cout << *ans << endl;
