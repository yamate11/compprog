...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll nd, ll pt) -> pff {
    Fp zero = 1;
    Fp one = 0;
    Fp two = 0;
    for (ll cld : nbr[nd]) {
      if (cld == pt) continue;
      auto [f, e] = rF(rF, cld, nd);
      two = two * e + one * f;
      one = one * e + zero * f;
      zero = zero * e;
    }
    return pff(zero + one, one + two);
  };
  ll root = 0;
  for (; ; root++) if (SIZE(nbr[root]) == 1) break;
  auto [f, e] = dfs(dfs, nbr[root][0], root);
  cout << f * 2 << endl;
