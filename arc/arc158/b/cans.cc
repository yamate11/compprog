...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 0, N) {
    ll x; cin >> x;
    if (x > 0) pos.push_back((double)x);
    else if (x < 0) neg.push_back((double)x);
    else assert(0);
  }
  ll szp = SIZE(pos);
  ll szn = SIZE(neg);
  ...
  double vmin = 1e18;
  if (szp >= 3) {
    updMax(vmax, f(pos[0], pos[1], pos[2]));
    updMin(vmin, f(pos[szp - 1], pos[szp - 2], pos[szp - 3]));
  }
  if (szn >= 3) {
    updMax(vmax, f(neg[szn - 1], neg[szn - 2], neg[szn - 3]));
    updMin(vmin, f(neg[0], neg[1], neg[2]));
  }
  if (szp >= 2 and szn >= 1) {
    updMax(vmax, f(pos[0], pos[1], neg[0]));
    updMax(vmax, f(pos[0], pos[szp - 1], neg[0]));
    updMax(vmax, f(pos[szp - 1], pos[szp - 2], neg[0]));
    updMin(vmin, f(pos[0], pos[1], neg[szn - 1]));
    updMin(vmin, f(pos[0], pos[szp - 1], neg[szn - 1]));
    updMin(vmin, f(pos[szp - 1], pos[szp - 2], neg[szn - 1]));
  }
  if (szn >= 2 and szp >= 1) {
    updMax(vmax, f(neg[0], neg[1], pos[szp - 1]));
    updMax(vmax, f(neg[0], neg[szn - 1], pos[szp - 1]));
    updMax(vmax, f(neg[szn - 1], neg[szn - 2], pos[szp - 1]));
    updMin(vmin, f(neg[0], neg[1], pos[0]));
    updMin(vmin, f(neg[0], neg[szn - 1], pos[0]));
    updMin(vmin, f(neg[szn - 1], neg[szn - 2], pos[0]));
  }
  cout << vmin << endl;
  cout << vmax << endl;
