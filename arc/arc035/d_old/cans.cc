...
using cbf = number<cpp_bin_float<25>>;
...
int main(int argc, char *argv[]) {
  ...
  ll vf_len = (ll)1e6 + 10;
  vector<cbf> vfact(vf_len);
  vfact.at(0) = 1.0;
  for (ll i = 1; i < vf_len; i++) vfact.at(i) = i * vfact.at(i-1);
  auto comb = [&](ll i) -> cbf {
    ll dx = p.at(i+1) - p.at(i);
    ll dy = q.at(i+1) - q.at(i);
    return vfact.at(dx + dy) / vfact.at(dx) / vfact.at(dy);
  };
  ...
  DAT unit1 = 1.0;
  OP unit2 = -1.0; // assigned number is non-negative
  vector<DAT> init_vec(N);
  for (ll i = 0; i < N-1; i++)  init_vec.at(i) = comb(i);
  ...
  for (ll z = 0; z < Q; z++) {
    ll t; cin >> t;
    if (t == 1) {
      ll k, a, b; cin >> k >> a >> b; k--;
      p.at(k) = a;
      q.at(k) = b;
      if (k-1 > 0) st.update(k-1, k, comb(k-1));
      if (k+1 < N) st.update(k, k+1, comb(k));
    }else {
	...
	continue;
      ...
      if (l2 < r1) swap(l2, r1);
      DAT v1 = st.query(l1, r1);
      DAT v2 = st.query(l2, r2);
      if (rev) swap(v1, v2);
      cout << (v1 > v2 ? "FIRST\n" : "SECOND\n");
    }
  }
