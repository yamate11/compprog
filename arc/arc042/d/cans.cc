...
ll solve() {
  ...
  if (B - A < ll(1e7)) return straight();
  ll m = ceil(sqrt(P));
  vector<Fp> vec1(m);
  map<Fp, ll> mp2;
  Fp pow = 1;
  for (ll j = 0; j < m; j++) {
    if (mp2.find(pow) != mp2.end()) { return straight(); }
    mp2[pow] = j;
    pow *= Fp(X);
  }
  DLOG("mp2", mp2);
  ...
  for (ll k = 1; true; k++) {
    Fp t = power<Fp>(y, A / m);
    for (ll p = m * (A / m); p <= B; p += m, t *= y) {
      auto it = mp2.find(t * Fp(k));
      if (it == mp2.end()) continue;
      ll j = it->second;
      if (p + j < A) continue;
      if (p + j >= B) break;
      DLOG("k", k, "p", p, "j", j, "t", t, "t*k", t * Fp(k));
      return k;
    }
  }
}
