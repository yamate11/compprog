...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> Fp {
    ...
    vector<ll> oneP, oneM, neither;
    REP(i, 0, N) {
      if (A[i] >= 0) {
        if (A[i] == i + 1) oneP.push_back(i);
        else if (A[i] == i - 1) oneM.push_back(i);
        else neither.push_back(i);
      }
    }
    if (ssize(neither) >= 2) return 0;
    if (ssize(neither) == 1) {
      ll i0 = neither[0];
      REP(i, 0, i0) {
        if (A[i] >= 0 and A[i] != i + 1) return 0;
      }
      REP(i, i0 + 1, N) {
        if (A[i] >= 0 and A[i] != i - 1) return 0;
      }
    ...
    if (not oneP.empty()) {
      ll i0 = oneP.back();
      if (oneM.empty()) return f(N - 1 - i0);
      if (oneM[0] < i0) return 0;
      return f(oneM[0] - i0 - 1);
    }else {
      if (not oneM.empty()) return f(oneM[0]);
      else return f(N);
    }
  };
  cout << solve() << "\n";
