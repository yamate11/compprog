...
int body(istream& cin, ostream& cout) {
  ...
  auto func = [&](const auto& L, const auto& R) -> vector<ll> {
    ll M = ssize(L);
    vector<ll> ret(M);
    ret[0] = L[0];
    vector<pll> init_vec(M);
    REP(i, 0, M) init_vec[i] = pll(L[i], i);
    auto st = make_seg_tree(pll(-1, -1), mymax, init_vec);
    ll i1 = 0;
    ll t = L[i1];
    while (true) {
      ll i2 = i1;
      while (i2 < M and L[i2] <= t and t <= R[i2]) i2++;
      if (i2 == M) {
        REP(i, i1, M) ret[i] = t;
        return ret;
      }else if (L[i2] > t) {
        ...
        ll i3 = i1;
        while (i3 + 1 < i2) {
          auto [v, i4] = st.query(i3 + 1, i2);
          t = L[i4];
          REP(i, i3 + 1, i4 + 1) ret[i] = t;
          i3 = i4;
        }
        t = ret[i2] = R[i2];
      }
  ...
  ll i0 = max_element(ALL(L)) - L.begin();
  ...
  auto vec1 = func(L1, R1);
  auto vec2 = func(L2, R2);
  auto vec = vec2;
  reverse(ALL(vec));
  vec.pop_back();
  for (ll v : vec1) vec.push_back(v);
  REPOUT(i, 0, N, vec[i], " ");
