...
void test1(ll N) {
  vector<pll> edges;
  REP(i, 0, N) REP(j, i + 1, N) edges.emplace_back(i, j);
  ll M = ssize(edges);
  REP(x, 1, 1LL << M) {
    UnionFind uf(N);
    vector dist(N, vector(N, 3));
    REP(e, 0, M) if (x >> e & 1) {
      auto [a, b] = edges[e];
      dist[a][b] = dist[b][a] = 1;
      uf.merge(a, b);
    }
    if (uf.group_size(0) != N) continue;
    ...
    auto f = [&](ll i) -> ll {
      ll ret = 0;
      REP(j, 0, N) if (dist[i][j] == 1 or dist[i][j] == 2) ret += j + 1;
      return ret;
    };
    ...
    // DLOG(dist);
    ll v = f(0);
    if (FORALL(i, 1, N, f(i) == v)) {
      cout << N << " " << popcount((u64)x) << "\n";
      REP(j, 0, M) if (x >> j & 1) {
        auto [a, b] = edges[j];
        cout << a + 1 << " " << b + 1 << "\n";
      }
