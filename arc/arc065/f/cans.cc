...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> n_one(N + 1);
  for (ll i = 0; i < N; i++) n_one[i + 1] = n_one[i] + (S[i] == '1' ? 1 : 0);
  ll tot_one = n_one[N];
  using tbl_t = vector<Fp>;
  tbl_t cur(tot_one + 1);
  cur[0] = 1;
  ll p = 0;
  ll far = 0;
  for (ll i = 0; i < N; i++) {
    far = max(far, i + 1);
    for ( ; p < M && L[p] == i; p++) far = max(far, R[p]);
    tbl_t prev = cur;
    cur = tbl_t(tot_one + 1);
    for (ll j = 0; j <= tot_one; j++) {
      if (i + 1 - j <= (far - n_one[far])) cur[j] += prev[j];
      if (j + 1 <= n_one[far])             cur[j + 1] += prev[j];
    }
  }
  Fp ans = 0;
  for (ll i = 0; i <= tot_one; i++) ans += cur[i];
  cout << ans << endl;
