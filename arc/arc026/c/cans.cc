...
int main(int argc, char *argv[]) {
  ...
  auto xMod = [](int k, OP f, DAT x) -> DAT { return min(f, x); };
  auto xComp = [](OP f, OP g) -> OP { return min(f, g); };
  //auto mkPred = [](DAT lim) { return [&](DAT z)->bool { return z < lim; }; };
  DAT unit1 = LLONG_MAX;
  OP unit2 = LLONG_MAX;
  vector<DAT> init_vec(L+1, LLONG_MAX);
  SegTree<DAT, OP, decltype(xAdd), decltype(xMod), decltype(xComp)>
    st(init_vec, xAdd, xMod, xComp, unit1, unit2);

  st.update(0, 1, 0);
  for (ll i = 0; i < N; i++) {
    ll l, r, c; tie(l, r, c) = vec.at(i);
    ll x = st.query(l, l+1);
    st.update(l+1, r+1, x+c);
  }
