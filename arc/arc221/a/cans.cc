...
// @@ !! LIM(mod f:gcd sieve perm unordered_map debug cmpNaive)
...
int body(istream& cin, ostream& cout) {
#if DEBUG
  auto primes = sieve((ll)1000);
#else
  auto primes = sieve((ll)4e6);
#endif
  ...
  auto solve = [&]() -> Fp {
    ...
    /*
      ...
      if (C % A == 0 and C / A * B == D) return N0 * (N0 + 1) * A / 2 + N0 * B;
    */
    auto [g, s, t] = eGCD(A, C);
    ll chat = s * B + t * D;
    ll dhat;
    ...
    ll b = bb / a;
    ll d = dd / a;
    // sum of gcd(abi + c, ad)  multiplied by z
    DLOGK(g, s, t, c, dhat, a);
    DLOGK(z, a, b, c, d);
    if (d == 0) return (N0 * (N0 + 1) * b / 2 + N0 * c) * z;
    else if (d == 1) return N0 * z;
    auto prD = prfac(d, primes);
    auto dsD = getDivisors(d, primes);
    ranges::sort(dsD);

    auto func = [&](ll x) -> Fp {
      safe_umap<ll, ll> mp;
      for (auto e : dsD) mp[e] = x / e;
      for (auto [p, r] : prD) for (auto e : dsD) if (e * p <= d) mp[e] -= mp[e * p];
      Fp ret = 0;
      for (auto [e, n] : mp) ret += e * n;
      return ret;
    };

    using tmpFp = FpG<0>;
    tmpFp::setMod(d);
    ll i0 = tmpFp(-c) / tmpFp(a * b);
    ll k = (d + N - i0) / d;
    ll n = (d + N - i0) % d;
    DLOGK(d, i0, k, n, func(d), func(n), func(d - i0));

    Fp ans = z * (k * func(d) + func(n) - func(d - i0));
    return ans;
  };
