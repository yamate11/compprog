...
int naive(istream& cin, ostream& cout) {
  ...
  IntDupPerm dip(M + 1, N);
  ll ans = 0;
  while (dip.get()) {
    set<ll> ss;
    auto mex = [&]() -> ll {
      REP(i, 0, M + 2) if (not ss.contains(i)) return i;
      assert(0);
    };
    auto check = [&]() -> bool {
      REP(i, 0, N) {
        ll m = mex();
        if (S[i] == 0 and dip.at(i) == m) return false;
        if (S[i] == 1 and dip.at(i) != m) return false;
        ss.insert(dip.at(i));
      }
      return true;
    };
