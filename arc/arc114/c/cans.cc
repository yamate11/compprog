...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  vector pow(M + 1, vector(N + 1, Fp(0)));
  REP(i, M + 1) {
    pow[i][0] = Fp(1);
    REP(j, N) pow[i][j + 1] = Fp(i) * pow[i][j];
  }
  Fp sum = 0;
  REP2(k, 1, N) {
    Fp s = Fp(N - k) * pow[M][N - k - 1]; // power<Fp>(M, N - k - 1);
    Fp t = 0;
    REP2(x, 1, M + 1) t += pow[M - x][k - 1]; // power<Fp>(M - x, k - 1);
    sum += s * t;
  }
  DLOG(sum);
  Fp ans = Fp(N) * power<Fp>(M, N) - sum;
  cout << ans << endl;
