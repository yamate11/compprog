...
int main(/* int argc, char *argv[] */) {
  ...
  auto func = [&](ll a, ll b) -> void {
    ...
    REP(k, 1, N + 1) {
      if (k <= 2) pow4 = 1;
      if (k >= 3) pow4 *= 4;
      auto new_f = [&](Fp ff, ll x) -> Fp {
        Fp ret = ff;
        if (N - k >= x) ret += cb.binom(N - k, x);
        if (x - k + 1 >= 0) ret += cb.binom(N - k, x - k + 1);
        ret *= half;
        return ret;
      };
      if (k == 1) {
      ...
      Fp bna = cb.binom(N - k, a);
      Fp bnb = cb.binom(N - k, b);
      DLOGK(a, b, k, fa, fb, bna, bnb);
      ans[k] += pow4 * (bna * fb + bnb * fa - bna * bnb);
    }
  };

  ll a, b; cin >> a >> b; a--; b--;
  auto rot = [&](ll x, ll y) -> pll {
    ll new_x = N - 1 - y;
    ll new_y = x;
    return {new_x, new_y};
  };
