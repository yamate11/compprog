...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  auto eff = vector(N + 1, vector(N + 1, false));
  auto memo = vector(N + 1, vector(N + 1, Fp(0)));
  auto func = [&](auto rF, ll n, ll k) -> Fp {
    if (n == k) return 1;
    if (k == 0 || n < k) {
      return 0;
    }
    ...
    if (!eff[n][k]) {
      eff[n][k] = true;
      ret = rF(rF, n - 1, k - 1) + rF(rF, n, 2 * k);
    }
    return ret;
  };
  cout << func(func, N, K) << endl;
