...
--}    
  ...
  bwTbl0 = VU.fromList $ concatMap (\(a,b) -> a++b) ls where
    (_, ls) = mapAccumL f ([0..n], [0..n]) cas
      where
      f (bc, wc) (c, a) = ((bcN, wcN), (bcN, wcN))
        where bcN = if color c == 0 then zipWith (+) bc as else bc
              wcN = if color c == 1 then zipWith (+) wc as else wc
              as = replicate a 0 ++ replicate (n-a+1) (-1)
  enc  k col i = k * (2*(n+1)) + col * (n+1) + i
  bwT k col i = trace ("bwT " ++ show (k,col,i,ans)) ans
    where ans = bwT0 k col i
  bwT0 k col i = bwTbl VU.! (enc k col i)
