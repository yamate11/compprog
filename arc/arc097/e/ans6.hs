...
--}    
  ...
  revNums = foldl op (VU.fromList [0]) [1..2*n]
    where
    op prev m = trace ("op " ++ show (prev, m, a)) a
      where a = op0 prev m
    op0 prev m = VU.fromListN (n+1) $ lst
      where
      lst | m <= n = w1 : body ++ [b1]
          | otherwise = body
      body = [min (putBlack i) (putWhite i) | i <- indices]
      w1 = putWhite 0
      b1 = putBlack m
      indices | m <= n = [1..m-1]
              | otherwise = [m-n..n]
      vPrev i | m <= n = prev VU.! i
              | otherwise = prev VU.! (i - (m-n-1))
      putBlack i = (vPrev (i-1)) + (right (posT 0 i) (i-1) (m-i))
      putWhite i = (vPrev i) + (right (posT 1 (m-i)) i (m-i-1))
