...
--}    
  ...
  revNums = foldl op [0] [1..2*n]
    ...
    op0 rs m
      | m <= n
      = zipWith min (inf : zipWith f_b [1..m] rs)
                    ((zipWith f_w [m,m-1..1] rs) ++ [inf])
      | otherwise
      = zipWith min (zipWith f_b [m-n..n] rs)
                    (zipWith f_w [n,n-1..m-n] (tail rs))
      where
      f_b i r = r + (right (posT 0 i) (i-1) (m-i))
      f_w i r = r + (right (posT 1 i) (m-i) (i-1))
