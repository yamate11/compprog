...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  // @InpVec(H, S, type=string) [kd6StRv2]
  auto S = vector(H, string());
  for (int i = 0; i < H; i++) { string v; cin >> v; S[i] = v; }
  // @End [kd6StRv2]
  vector tbl0(H, vector(W, Fp(0)));
  vector tbl1(H, vector(W, Fp(0)));
  vector tbl2(H, vector(W, Fp(0)));
  tbl0[0][0] = 1;
  REP(i, 0, H) REP(j, 0, W) {
    if (i > 0) {
      tbl0[i][j] += tbl0[i - 1][j];
      if (S[i][j] == 'Y' and S[i - 1][j] == 'Y') {
        tbl1[i][j] += tbl1[i - 1][j] + tbl0[i - 1][j];
        tbl2[i][j] += tbl2[i - 1][j] + 2 * tbl1[i - 1][j] + tbl0[i - 1][j];
      }else {
        tbl1[i][j] += tbl1[i - 1][j];
        tbl2[i][j] += tbl2[i - 1][j];
      }
    ...
    if (j > 0) {
      tbl0[i][j] += tbl0[i][j - 1];
      if (S[i][j] == 'Y' and S[i][j - 1] == 'Y') {
        tbl1[i][j] += tbl1[i][j - 1] + tbl0[i][j - 1];
        tbl2[i][j] += tbl2[i][j - 1] + 2 * tbl1[i][j - 1] + tbl0[i][j - 1];
      }else {
        tbl1[i][j] += tbl1[i][j - 1];
        tbl2[i][j] += tbl2[i][j - 1];
      }
