...
int main(/* int argc, char *argv[] */) {
  ...
  auto funcH = [&](ll n, ll r) -> Fp {
    if (n == 0) return Fp(0);
    return cb.binom(n + r - 1, r);
  };
  const ll lim = 2e5;
  vector<bool> appear(lim + 1);
  REP(i, 0, N) appear[A[i]] = true;
  vector<ll> gap;
  REP(i, 0, lim + K + 10) {
    if (i >= lim + 1 or not appear[i]) gap.push_back(i);
    if (SIZE(gap) == K) break;
  }
  ll mxGap = gap[K - 1];
  Fp tot = funcH(mxGap + 1, K);
  Fp exc = 0;
  REP(k, 0, K) {
    exc += funcH(mxGap + 1 - 1, K - k) - funcH(gap[k], K - k);
  }
  cout << tot - exc << endl;
