...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, S, type=string) [UQ4gt3hJ]
  ...
  REP(i, 0, N) {
    ...
    ll ca = trie.child_index(idx, 'A');
    ll cb = trie.child_index(idx, 'B');
    trie.nodes[idx].user.inS = true;
    trie.nodes[idx].user.num = 1;
    if (ca >= 0) {
      trie.nodes[idx].user.num += trie.nodes[ca].user.num;
    }
    if (cb >= 0) {
      trie.nodes[idx].user.num += trie.nodes[cb].user.num;
    }
    trie.nodes[idx].user.val = power<Fp>(2, trie.nodes[idx].user.num - 1);
    if (ca >= 0 and cb >= 0) {
      trie.nodes[idx].user.val += trie.nodes[ca].user.val * trie.nodes[cb].user.val;
    }
    while (true) {
      idx = trie.parent_index(idx);
      ll ca0 = trie.child_index(idx, 'A');
      ll cb0 = trie.child_index(idx, 'B');
      trie.nodes[idx].user.num += 1;
      if (trie.nodes[idx].user.inS) {
        trie.nodes[idx].user.val = power<Fp>(2, trie.nodes[idx].user.num - 1);
      }else {
        trie.nodes[idx].user.val = 0;
      }
      if (ca0 >= 0 and cb0 >= 0) {
        trie.nodes[idx].user.val += trie.nodes[ca0].user.val * trie.nodes[cb0].user.val;
      }
      if (idx == 0) break;
    }
    cout << trie.nodes[0].user.val << "\n";
  }
