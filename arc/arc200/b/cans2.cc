...
int main(/* int argc, char *argv[] */) {
  ...
  vector lst(18, vector(3, 0LL));
  lst[1] = vector<ll>{7, 8, 9};
  REP(k, 2, 18) {
    ll x = du.nd_max(k);
    REP(i, 0, 3) {
      while (not is_prime_MR(x)) x--;
      lst[k][i] = x;
      x--;
    }
  }
  ...
  auto solve = [&]() -> pll {
    ...
    if (x == 1 and y == 1 and z == 1) {
      ...
      if (x == y) {
        ret = pll(lst[x][0], lst[x][1]);
      }else {
        ret = pll(lst[x][0], lst[y][0]);
      }
    }else if (c == 0) {
      ret = pll(du.pow(x - 1), du.pow(y - 1));
    }else {
      // a > 0, b > 0, c > 0
      ll sa = lst[a][0];
      ll sb;
    ...
    if (swapped) {
      return pll(ret.second, ret.first);
    }else {
      return ret;
    }
  };
