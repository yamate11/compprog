...
// @@ !! LIM(sieve digit debug f:intDiv)
...
int main(/* int argc, char *argv[] */) {
  ...
  vector ans(18, vector(18, vector(18, pll(-1, -1))));
  ...
  vector<ll> pow2(sz2);
  pow2[0] = 1;
  REP(i, 1, sz2) pow2[i] = pow2[i - 1] * 2;
  vector<ll> pow3(sz3);
  pow3[0] = 1;
  REP(i, 1, sz3) pow3[i] = pow3[i - 1] * 3;
  REP(a1, 0, sz2) {
    REP(a2, 0, sz2) {
      ll a = max(a1, a2);
      REP(b1, 0, sz3) {
        if (pow2[a] >= divCeil(lim, pow3[b1])) break;
        REP(b2, 0, sz3) {
          ll b = max(b1, b2);
          if (pow2[a] >= divCeil(lim, pow3[b])) break;
          ll y = pow2[a] * pow3[b];
          ll x1 = pow2[a1] * pow3[b1];
          ll x2 = pow2[a2] * pow3[b2];
          ll p1 = du.width(x1);
          ll p2 = du.width(x2);
          ll p = du.width(y);
          ans[p1][p2][p] = pll(x1, x2);
          ans[p2][p1][p] = pll(x2, x1);
          DLOGK(p1, p2, p, x1, x2);
        }
      }
    }
