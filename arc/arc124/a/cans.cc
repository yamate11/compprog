...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  auto rec = vector(N, vector(K, true));
  for (ll i = 0; i < K; i++) {
    char c; cin >> c;
    ll k; cin >> k; k--;
    if (c == 'L') {
      for (ll j = 0; j < k; j++) rec[j][i] = false;
    }else {
      for (ll j = k + 1; j < N; j++) rec[j][i] = false;
    }
    for (ll j = 0; j < K; j++) if (j != i) rec[k][j] = false;
#if DEBUG
    cerr << "** " << i << endl;
    for (ll n = 0; n < N; n++) {
      for (ll j = 0; j < K; j++) {
  ...
  for (ll i = 0; i < N; i++) {
    ll cnt = 0;
    for (ll j = 0; j < K; j++) if (rec[i][j]) cnt += 1;
    ans *= cnt;
    DLOGK(i, cnt);
  }
  cout << ans << endl;
