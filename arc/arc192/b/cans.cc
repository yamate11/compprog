...
int main(/* int argc, char *argv[] */) {
...
#if 0
  auto judge = [&](auto rF, const auto& S, const auto& A) -> bool {
    ll n = ssize(S);
    if (FORALL(i, 0, n, S[i])) return true;
    REP(j, 0, n) {
      if (A[j] == 0) continue;
      auto S1 = S;
      auto A1 = A;
  ...
  while (dic.get()) {
    vector<ll> A(5);
    vector<ll> S(5);
    REP(j, 0, 5) A[j] = dic.at(j) + 1;
    bool b = judge(judge, S, A);
    cout << A << " " << b << endl;
  }
#endif
