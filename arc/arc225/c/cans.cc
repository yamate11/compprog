...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> vector<bool> {
    ll N, M, K; cin >> N >> M >> K;
    // @InpMVec(M, ((A, dec=1), (B, dec=1), C)) [hogrDfon]
    auto A = vector(M, ll());
    auto B = vector(M, ll());
    auto C = vector(M, ll());
    for (int i = 0; i < M; i++) {
    ...
    vector uf(2, UnionFind(N));
    vector cnt(2, 0LL);
    vector use(2, vector(M, false));
    auto connect = [&](ll tp, ll i) -> void {
      if (uf[tp].leader(A[i]) != uf[tp].leader(B[i])) {
        uf[tp].merge(A[i], B[i]);
        use[tp][i] = true;
        if (C[i] == 1) cnt[tp]++;
      }
    ...
    for (ll i : white) connect(0, i);
    for (ll i : black) connect(0, i);
    if (cnt[0] > K) return vector<bool>{};
    for (ll i : black) if (use[0][i]) connect(1, i);
    for (ll i : black) if (cnt[1] < K) connect(1, i);
    if (cnt[1] < K) return vector<bool>{};
    for (ll i : white) connect(1, i);
    return use[1];
  };
  ...
  REP(t, 0, T) {
    ...
    if (use.empty()) {
      ...
      REP(i, 0, ssize(use)) if (use[i]) cout << i + 1 << " ";
      cout << "\n";
    }
  }
  return 0;
