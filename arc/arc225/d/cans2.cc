...
// @@ !! LIM(smallVec perm debug forall)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @DefStruct(sta, (rn, val, nxt)) [Z5qo65dI]
  struct sta {
    ll rn;
    ll val;
    svec nxt;
    string show() const {
      return "(" + to_string(rn) + ", " + to_string(val) + ", " + g_show(nxt) + ")";
    }
    bool operator==(const sta&) const = default;
  };
  // @End [Z5qo65dI]

  safe_umap<svec, sta> mp;

  ll big = N * N + 10;

  IntPerm ip0(N, N);
  while (ip0.get()) {
    svec v; REP(i, 0, N) v[i] = ip0[i];
    ll rn = 0;
    REP(i, 0, N) REP(j, i + 1, N) if (v[i] > v[j]) rn++;
    mp[v] = sta{rn, big, svec{}};
  }
  svec v0; REP(i, 0, N) v0[i] = i;
  mp[v0] = sta{0, 0, svec{}};

  auto f = [&](auto rF, svec v) -> ll {
    sta& s = mp[v];
    if (s.val >= big) {
      ll xm = big;
      svec arg;
  ...
  */
  ...
  ll N; cin >> N;
  // @InpVec(N, P, dec=1) [fyqbgqYG]
  auto P = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; P[i] = v; }
  // @End [fyqbgqYG]
  ll ans = 0;
  REP(i, 0, N) if (P[i] > i) ans += P[i] - i;
  cout << ans << endl;
