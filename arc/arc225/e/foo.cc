...
// @@ !! LIM(smallVec perm debug forall segTree)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @DefStruct(sta, (val, (nxt, svec))) [Z5qo65dI]
  struct sta {
    ll val;
    svec nxt;
    string show() const {
      return "(" + to_string(val) + ", " + g_show(nxt) + ")";
    }
  ...
  REP(n, 1, N + 1) {
    svec v0;
    REP(i, 0, n) v0[i] = i;
    safe_umap dist<svec, ll>;
    using stb = pll<ll, svec>;
    ...
    while (not pque.empty()) {
      ...
      if (dist[nd] == d) {
        REP(i, 0, n) REP(j, i + 1, n) {
          if (FORALL(k, i + 1, j, nd[k] == k)) {
            auto vv = nd;
            vv[i] = nd[j];
            vv[j] = nd[i];
            auto [it, b] = dist.try_emplace(vv, 0);
            if (b or d + 1 < it->second) {
              it->second = d + 1;
              pque.emplace(d + 1, vv);
            }
          }
    ...
    ostream outfile(fname);
    for (auto [v, d] : dist) outfile << v << " " << d << "\n";
  }
