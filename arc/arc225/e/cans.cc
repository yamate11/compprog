...
// @@ !! LIM(smallVec perm debug forall segTree cmpNaive sortedIndex)
...
constexpr ll ra_sz = 10;

using svec = small_vector_u64<4>;
using stb = pair<svec, ll>;

vector<safe_umap<svec, ll>> read_ans;

int naive(istream& cin, ostream& cout) {
  if (read_ans.empty()) {
    read_ans.resize(ra_sz + 1);
    REP(n, 1, ra_sz + 1) {
      string fname = "o" + to_string(n) + ".txt";
      ifstream infile(fname);
      while (true) {
        ll x, d;
        if (not (infile >> x >> d)) break;
        read_ans[n][(svec)x] = d;
      }
    }
  }
  ...
  cout << read_ans[N][v] << endl;
  return 0;
}
int body(istream& cin, ostream& cout) {
  ll N; cin >> N;
  // @InpVec(N, P, dec=1) [0wtAAeG8]
  auto P = vector(N, ll());
  ...
  vector<pll> L;
  REP(i, 0, N) {
    if (P[i] == i) C.push_back(i);
    else if (i < P[i]) R.emplace_back(P[i], i);
    else if (P[i] < i) L.emplace_back(P[i], i);
    else assert(0);
  }
  DLOGK(C, R, L);

  vector U(N, vector<ll>());
  auto segt1 = make_seg_tree(0LL, plus<ll>(), vector<ll>(N));
  auto segt2 = make_seg_tree(0LL, plus<ll>(), vector<ll>(N));
  for (ll i : C) segt1.rs(i) = 1;

  vector<ll> revRS(N, -1LL);
  REP(i, 0, ssize(L)) revRS[L[i].second] = i;
  auto ord = sortedIndex(L);

  ll ii = 0;
  ranges::sort(R);
  for (auto [dest, src] : R) {
    ll b = segt1.query(src, dest);
    for (; ii < ssize(L) and L[ord[ii]].first < dest; ii++) segt2.rs(L[ord[ii]].second) = 1;
    ll th = dest - src - b - 1;
    ll y = segt2.binsearch_r_until([&](ll z) -> bool { return z <= th; }, src);
    U[revRS[y]].push_back(dest);
    segt1.rs(src) = 1;
  }
  DLOGK(U);
  
  ll ans = 0;
  auto segt3 = make_seg_tree(0LL, plus<ll>(), vector<ll>(N));
  for (ll i : C) segt3.rs(i) = 1;
  REP(j, 0, ssize(L)) {
    auto [dest, src] = L[j];
    ll a = segt3.query(dest, src);
    ans += src - dest - a;
    segt3.rs(dest) = 1;
    for (ll i : U[j]) segt3.rs(i) = 1;
  }
  cout << ans << endl;
