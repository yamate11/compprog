...
// @@ !! LIM(mod segTree debug cmpNaive perm rle)
...
int naive(istream& cin, ostream& cout) {
  ...
  ll cnt = 0; REP(i, 0, N) if (A[i] == -1) cnt++;
  IntDupPerm dip(N, cnt);
...
int body(istream& cin, ostream& cout) {
  ...
  B.push_back(0);
  vector<pair<ll, ll>> rlenc;
  rle(B.begin(), B.end(), back_inserter(rlenc));
  for (auto [val, num] : rlenc) {
  ...
  auto dummy_max = [&](Fp x, Fp y) -> Fp { return Fp(max((ll)x, (ll)y)); };

  auto func = [&](ll lval, auto& rec, ll num, ll rval, ll rnum) {
    DLOGKL("func", lval, rec, num, rval, rnum);
    vector ret(N + 1, Fp(0));
    /*
    auto st_pat = make_seg_tree_lazy(Fp(0), Fp(0), dummy_max, plus<Fp>(), plus<Fp>());
    st_pat.set_data(vector(num, Fp(0)));
    */
    vector diff(N + 1, vector(N + 1, Fp(0)));
    
    Fp sum0 = accumulate(ALL(rec), Fp(0));
    REP(k, 1, N + 1) {
      if (k != lval) {
        if (k != rval) {
          diff[k][0] += sum0;
          diff[k][min(num, 0 + k)] -= sum0;
          // st[k].update(0, min(num, 0 + k), sum0);
        }else {
          diff[k][0] += sum0;
          diff[k][min(num - 1, 0 + k)] -= sum0;
          // st[k].update(0, min(num - 1, 0 + k), sum0);
          if (num + rnum <= k) ret[num + rnum] += sum0;
        }
      }else {
        Fp tmp = sum0;
        REP(j, 0, min(k + 1, num)) {
          tmp -= rec[k - j];
          if (k == rval and j == num - 1) {
            REP(t, 1, k + 1) if (t + num + rnum <= k) ret[t + num + rnum] += rec[t];
          }else {
            diff[k][j] += tmp;
            diff[k][j + 1] -= tmp;
            // st[k].update(j, j + 1, tmp);
          }
        }
      }
    }
#if DEBUG
    // REP(k, 1, N + 1) DLOGK(k, st[k]);
#endif
    vector cache(N + 1, Fp(0));
    REP(i, 0, num - 1) {
      REP(k, 1, N + 1) {
        cache[k] += diff[k][i];
        // cache[k] = st[k].query(i, i + 1);
      }
      Fp sum1 = Fp(0);
      REP(k, 1, N + 1) sum1 += cache[k];
      REP(k, 1, N + 1) {
        if (k != rval) {
          diff[k][i + 1] += sum1 - cache[k];
          diff[k][min(num, i + 1 + k)] -= sum1 - cache[k];
          // st[k].update(i + 1, min(num, i + 1 + k), sum1 - cache[k]);
        }else {
          diff[k][i + 1] += sum1 - cache[k];
          diff[k][min(num - 1, i + 1 + k)] -= sum1 - cache[k];
          // st[k].update(i + 1, min(num - 1, i + 1 + k), sum1 - cache[k]);
          if (num - i - 1 + rnum <= k) ret[num - i - 1 + rnum] += sum1 - cache[k];
        }
      }
    }

    REP(k, 1, N + 1) cache[k] += diff[k][num - 1];
    REP(k, 1, N + 1) {
      if (k != rval) ret[rnum] += cache[k];
    }
  ...
  ll lval = 0;
  while (ei < SIZE(rlenc)) {
    auto [val, num] = rlenc[ei];
    DLOGK(val, num);
    if (val != -1) {
      vector new_rec(N + 1, Fp(0));
      new_rec[num] = accumulate(ALL(rec), Fp(0));
      rec = move(new_rec);
      lval = val;
      ei++;
      DLOGK(rec);
    }else {
      auto [rval, rnum] = rlenc[ei + 1];
      rec = func(lval, rec, num, rval, rnum);
      lval = rval;
      ei += 2;
      DLOGK(rec);
    }
