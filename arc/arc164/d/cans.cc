...
int main(/* int argc, char *argv[] */) {
  ...
  vector num_init(2*N + 1, Fp(0));
  vector val_init(2*N + 1, Fp(0));
  auto num = num_init;
  auto val = val_init;
  num[N] = Fp(1);
  REP(i, 0, 2*N) {
    auto num_prev = move(num);
    auto val_prev = move(val);
    num = num_init;
    val = val_init;
    if (T[i] == '+' or T[i] == '?') {
      REP(k, 0, 2*N) {
        if (k < N)       val[k + 1] += val_prev[k] -     num_prev[k];
        else if (k >= N) val[k + 1] += val_prev[k] + 2 * (k - N + 1) * num_prev[k];
        num[k + 1] += num_prev[k];
      }
    }
    if (T[i] == '-' or T[i] == '?') {
      REP(k, 1, 2*N + 1) {
        if (k > N)       val[k - 1] += val_prev[k] -     num_prev[k];
        else if (k <= N) val[k - 1] += val_prev[k] + 2 * (N - k + 1) * num_prev[k];
        num[k - 1] += num_prev[k];
      }
    }
    DLOGK(i, T[i], num, val);
