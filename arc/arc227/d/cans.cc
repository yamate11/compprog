...
int main(/* int argc, char *argv[] */) {
  ...
  ll M = 6;
  vector<string> init = {"101001", "000110", "111010", "000101", "011010"};
  vector<ll> di(ssize(init));
  REP(i, 0, ssize(init)) di[i] = du.from_string(init[i]);
  vector<ll> V{di[0], di[1]};
  vector<bool> A(1LL << M);
  REP(i, 0, ssize(init))  A[di[i]] = true;
  queue<ll> que;
  REP(i, 2, ssize(init)) que.push(di[i]);
  while (not que.empty()) {
    ...
    REP(i, 0, sz) REP(j, i + 1, sz) {
      ll y = f(V[i], V[j], x);
      if (not A[y]) {
        DLOGK(du.to_string(V[i]), du.to_string(V[j]), du.to_string(x), du.to_string(y));
        A[y] = true;
        que.push(y);
      }
    }
  ...
  REP(i, 0, 1LL << M) {
    if (A[i]) cout << du.to_string_padding(i, M) << "\n";
  }
