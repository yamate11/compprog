...
// @@ !! LIM(forall f:intDiv debug sieve unordered_map cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  auto f = [&](const string& S) -> string {
    ...
    vector fwd(sigma, vector<vector<ll>>());
    {
      vector pos(sigma, vector<ll>());
      vector rpos(N, pll());
      REP(i, 0, N) {
        ll d = S[i] - 'a';
        pos[d].push_back(i);
      ...
      vector ifwd(N, vector(sigma, -1LL));
      REPrev(i, 2*N - 1, 0) {
        ifwd[i % N] = ifwd[(i + 1) % N];
        ll d = S[(i + 1) % N] - 'a';
        ifwd[i % N][d] = (i + 1) % N;
      }
      REP(d, 0, sigma) if (cnt[d] > 0) {
        fwd[d] = vector(cnt[d], vector(sigma, -1LL));
        REP(a, 0, cnt[d]) REP(e, 0, sigma) if (cnt[e] > 0) {
          fwd[d][a][e] = rpos[ifwd[pos[d][a]][e]].second;
        }
      }
    }

    auto get_gap = [&](ll d, ll a, ll b) -> pll {
      if (a == b) return {a, 0};
      if (a > b) swap(a, b);
      if (b - a <= cnt[d] - (b - a)) return {a, b - a};
      return {b, cnt[d] - (b - a)};
    };

    vector idc(sigma, vector(sigma, vector<pll>()));
    REP(d, 0, sigma) REP(e, 0, sigma) if (cnt[d] > 0 and cnt[e] > 0) idc[d][e] = vector<pll>(cnt[d] / 2 + 1, pll(-1, -1));
    vector prv(sigma, vector<vector<pll>>());
    REP(d, 0, sigma) prv[d] = vector(cnt[d] / 2 + 1, vector<pll>());
    REP(d, 0, sigma) if (cnt[d] > 0) {
      REP(e, 0, sigma) if (d != e and cnt[e] > 0) {
        REP(gap, 1, cnt[d] / 2 + 1) {        
          ll a0 = -1, gap0 = N + 1, r0 = -1;
          REP(a, 0, cnt[d]) {
            ll b = (a + gap) % cnt[d];
            ll p = fwd[d][a][e];
            ll q = fwd[d][b][e];
            auto [r1, gap1] = get_gap(e, p, q);
            if (gap1 < gap0) {
              gap0 = gap1;
              a0 = a;
              r0 = r1;
          ...
          prv[e][gap0].emplace_back(d, gap);
          idc[d][e][gap] = pll(a0, r0);
          DLOGKL("  prv", d, gap, e, gap0);
        }
      }
    }
    vector nxt(sigma, vector<pll>());
    REP(d, 0, sigma) nxt[d] = vector(cnt[d], pll(-2, 0));
    queue<pll> que;
    REP(e, 0, sigma) if (cnt[e] > 0) {
      nxt[e][0] = pll(-1, 0);
      que.emplace(e, 0);
    }
    while (not que.empty()) {
      auto [e, gap1] = que.front(); que.pop();
      for (auto [d, gap0] : prv[e][gap1]) {
        if (nxt[d][gap0].first == -2) {
          nxt[d][gap0] = pll{e, gap1};
          que.emplace(d, gap0);
          DLOGKL("  nxt", d, gap0, e, gap1);
        }
      }
    }
    ...
    ll mn = N + 1;
    ll d0 = -1;
    REP(d, 0, sigma) if (0 < cnt[d] and cnt[d] < mn) { mn = cnt[d]; d0 = d; }
    string ans(1, 'a' + d0);
    if (cnt[d0] == 1) return ans;
    vector<ll> poss(cnt[d0]);
    REP(i, 0, cnt[d0]) poss[i] = i;
    ll d = d0;
    while (ssize(poss) >= 2) {
      ll gap = N + 1;
      REP(i, 0, ssize(poss)) {
        ll g = imod(poss[(i + 1) % ssize(poss)] - poss[i], cnt[d]);
        if (g < gap) {
          gap = g;
        }
      }
      while (gap > 0) {
        auto [d_new, gap_new] = nxt[d][gap];
        DLOGK(d, gap, poss, d_new, gap_new);
        assert(d_new >= 0);
        auto [p, r] = idc[d][d_new][gap];
        ll fs = N + 1;
        REP(i, 0, ssize(poss)) {
          ll g = imod(poss[(i + 1) % ssize(poss)] - poss[i], cnt[d]);
          if (g == gap) {
            ll x = imod(p - poss[i], cnt[d]);
            if (x < fs) fs = x;
          }
        }
        assert(fs < cnt[d]);
        DLOGKL("  ", p, fs);
        for (ll& x : poss) x = fwd[d][(x + fs) % cnt[d]][d_new];
        if (fs > 0) ans += string(fs, 'a' + d);
        ans += string(1, 'a' + d_new);
        d = d_new;
        gap = gap_new;
      }
      ranges::sort(poss);
  ...
  ranges::sort(divs);
  ll per = -1;
  for (ll p : divs) if (FORALL(i, 1, N/p, FORALL(j, 0, p, S[i * p + j] == S[j]))) { per = p; break; }
  assert(per >= 1);
  string op = f(S.substr(0, per));
  cout << N / per << "\n" << ssize(op) << "\n" << op << "\n";
