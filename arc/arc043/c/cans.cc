...
int main(int argc, char *argv[]) {
  ...
  ll N; cin >> N;
  vector<ll> A(N), B(N), revA(N), sftB(N);
  for (ll i = 0; i < N; i++) {
    cin >> A.at(i); A.at(i)--;
    revA.at(A.at(i)) = i;
  }
  for (ll i = 0; i < N; i++) {
    cin >> B.at(i); B.at(i)--;
    sftB.at(i) = revA.at(B.at(i));
  }

  auto xAdd = [](int x, int y) -> int { return x + y; };
  auto xMod = [](int k, int g, int x) -> int { return k * g; };
  auto xComp = [](int h, int g) -> int { return h; };
  SegTree<int, int> st(vector<int>(N), xAdd, xMod, xComp, 0, -1);
  
  ll tot = 0;
  vector<ll> leftLarge(N);
  for (ll i = 0; i < N; i++) {
    ll x = sftB.at(i);
    ll y = st.query(x+1, N);
    leftLarge.at(x) = y;
  ...
  ll r = leftLarge.at(i) - goal;
  ll j = 0;
  for ( ; j < N; j++) {
    if (sftB.at(j) <= i) continue;
    pre.push_back(sftB.at(j));
    r--;
    if (r == 0) {
      pre.push_back(i);
    }
