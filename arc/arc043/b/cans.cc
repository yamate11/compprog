...
int main(int argc, char *argv[]) {
  ...
  ll sizeD = 1e5 + 10;
#endif
  vector<vector<Fp>> D(4, vector<Fp>(sizeD));
  for (ll i = 0; i < N; i++) {
    ll d; cin >> d;
    D.at(0).at(d) += Fp(1);
  }
  for (ll j = 1; j <= 3; j++) { 
    Fp sum = 0;
    for (ll i = sizeD-1; i >= 3; i -= 2) {
      sum += D.at(j-1).at(i) + D.at(j-1).at(i-1);
      D.at(j).at(i / 2) = D.at(0).at(i / 2) * sum;
    }
    DLOG("j, D.at(j)", j, D.at(j));
  }
  Fp ans = 0;
  for (ll i = 0; i < sizeD; i++) ans += D.at(3).at(i);
  cout << ans << endl;
