...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> Fp {
    ...
    auto calc = [&](ll i0, ll j0) -> Fp {
      ll t = A[i0] % K;
      if (t == 0 or t * 2 == K) {
        safe_umap<ll, ll> mp;
        REP(i, i0, j0) mp[A[i]]++;
        Fp x = 1;
        for (auto [a, n] : mp) x *= cb.fact(n);
        return cb.fact(j0 - i0) / x;
      }else {
        ll a = 0, b = 0;
        REP(i, i0, j0) {
          if (A[i] % K == t) a++;
    ...
    ll j = 0;
    REP(i, 0, N + 1) {
      if (i == N or (not same_grp(A[j], A[i]))) {
        Fp t = calc(j, i);
        DLOGK(j, i, t);
        ans *= t;
        if (i == N) break;
