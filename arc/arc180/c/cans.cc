...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> work(21, -1LL);
  vector<ll> prevpos(N);
  REP(i, 0, N) {
    prevpos[i] = work[A[i] + 10];
    work[A[i] + 10] = i;
  }

  ll lim = 10 * N;
  ll tsz = 2 * lim + 1;
  vector tblA(N, vector(tsz, Fp(0)));
  vector tblB(N, Fp(0));
  
  REP(i, 0, N) {
    if (prevpos[i] < 0) {
      if (A[i] != 0) tblB[i] += Fp(1);
    }
  }
  ...
  Fp ans = 1;
  REP(i, 0, N) REP(a, -lim, lim + 1) ans += tblA[i][lim + a];
  cout << ans << endl;
