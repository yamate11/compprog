...
int main(/* int argc, char *argv[] */) {
  ...
  oL.emplace_back(N + 1, N + 1);
  oR.emplace_back(N + 1, N + 1);
  sort(oL.begin(), oL.end());
  sort(oR.begin(), oR.end());
  vector cur(N, vector(N, Fp(0)));
  cur[0][0] = Fp(1);
  set<sta> one, two;
  auto itl = oL.begin();
  auto itr = oR.begin();
  for (ll i = 0; i < N; i++) {
    auto prev = move(cur);
    cur = vector(N, vector(N, Fp(0)));

    ll onelim = one.empty() ? i : one.begin()->first;
    ll twolim = min(onelim, two.empty() ? i : two.begin()->first);

    if (i == 0) {
      cur[0][0] = 3;
    }else {
      cur[0][0] += prev[0][0];
      if (i <= onelim) cur[i][0] += 2 * prev[0][0];
      for (ll p = 0; p < i; p++) {
        for (ll q = 0; q < p; q++) {
          cur[p][q] += prev[p][q];
          if (i <= onelim) cur[i][q] += prev[p][q];
          if (i <= onelim && p <= twolim) cur[i][p] += prev[p][q];
        }
      }
    }
    ...
    for ( ; itl->first == i; itl++) {
      ll j = itl->second;
      if (X[j] == 1) one.emplace(i, itl->second);
      else if (X[j] == 2) two.emplace(i, itl->second);
    }
    for ( ; itr->first == i; itr++) {
      ll j = itr->second;
      if (X[j] == 2) {
        for (ll p = L[j]; p >= 0; p--) {
          for (ll q = p; q >= 0; q--) cur[p][q] = 0;
        }
      }else if (X[j] == 3) {
        for (ll q = L[j]; q >= 0; q--)  {
          for (ll p = i; p >= q; p--) cur[p][q] = 0;
        }
      }
      if (X[j] == 1) one.erase({L[j], j});
      else if (X[j] == 2) two.erase({L[j], j});
    }
  }
  Fp ans = 0;
  for (ll p = 0; p < N; p++) {
    for (ll q = 0; q < N; q++) ans += cur[p][q];
  }
  cout << ans << endl;
