...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  string S; cin >> S;
  // dp[i][eb][ec][(p0,p1,p2,p3)] = i まで見て #B - #A \equiv eb (mod 2), #C - #A \equiv ec (mod 2),
  //                 いままでの 00 01 10 11 の数が p0,p1,p2,p3 である文字列の数
  using tpA = tuple<ll, ll, ll, ll>;
  // auto myenc = [&](ll x, ll y, ll z, ll w) -> ll { return w + (N + 1) * (z + (N + 1) * (y + (N + 1) * x)); };
  auto myenc = [&](const tpA& v) -> ll {
    auto [x, y, z, w] = v;
    return w + (N + 1) * (z + (N + 1) * (y + (N + 1) * x)); };
  auto mydec = [&](ll e) -> tpA {
    ll w = e % (N + 1);
    ll zz = e / (N + 1);
    ll z = zz % (N + 1);
  ...
  auto tbl_init = vector(2, vector(2, unordered_map<ll, Fp>()));
  auto tbl = tbl_init;
  tbl[0][0][myenc(tpA{1, 0, 0, 0})] = 1;
  REP(i, 0, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    char c = S[i];
    REP(eb, 0, 2) REP(ec, 0, 2) {
      for (auto [xx, n] : prev[eb][ec]) {
        auto vec = mydec(xx);
        auto op = [&](ll zb, ll zc) -> void {
          ll z = zb * 2 + zc;
          auto vv = vec;
          if (z == 0) get<0>(vv) += 1;
          else if (z == 1) get<1>(vv) += 1;
          else if (z == 2) get<2>(vv) += 1;
          else if (z == 3) get<3>(vv) += 1;
          tbl[zb][zc][myenc(vv)] += n;
        };
        if (c == 'A' or c == '?') op(not eb, not ec);
        if (c == 'B' or c == '?') op(not eb, ec);
        if (c == 'C' or c == '?') op(eb, not ec);
      }
    }
#if DEBUG
    REP(eb, 0, 2) REP(ec, 0, 2) {
      for (auto [xx, n] : tbl[eb][ec]) {
        auto vvv = mydec(xx);
        DLOGK(i, eb, ec, vvv, n);
      }
    }
  ...
  REP(eb, 0, 2) REP(ec, 0, 2) {
    for (auto [xx, n] : tbl[eb][ec]) {
      auto [x, y, z, w] = mydec(xx);
      ll m = x * (x - 1) / 2 + y * (y - 1) / 2 + z * (z - 1) / 2 + w * (w - 1) / 2;
      if (m >= K) ans += n;
    }
  }
  cout << ans << endl;
