...
void add(vector<Fp>& vec1, const vector<Fp>& vec2) {
  ll sz1 = vec1.size();
  ll sz2 = vec2.size();
  if (sz1 < sz2) vec1.resize(sz2);
  for (ll i = 0; i < sz2; i++) vec1[i] += vec2[i];
}

vector<Fp> conv(const vector<Fp>& vec1, const vector<Fp>& vec2) {
  ll sz1 = vec1.size();
  ll sz2 = vec2.size();
  vector<Fp> ret(sz1 + sz2 - 1);
  for (ll p = 0; p < sz1 + sz2 - 1; p++) {
    for (ll i = max(0LL, p - sz2 + 1); i < min(sz1, p + 1); i++) {
      ret[p] += vec1[i] * vec2[p - i];
    }
  }
  return ret;
}
...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll nd) -> pair<ll, vector<Fp>> {
    auto tv0 = vector<Fp>({1});
    auto tv1 = vector<Fp>();
    auto csize = 1;
    for (ll cld : child[nd]) {
      auto [m, vec] = rF(rF, cld);
      csize += m;
      auto w0 = conv(tv0, vec);
      auto w1 = conv(tv1, vec);
      ll sz = vec.size();
      auto vec2 = vector<Fp>(sz + 1);
      for (ll i = 0; i < sz; i++) {
        vec2[i + 1] = vec[i] * (m - i);
      }
      auto w2 = conv(tv0, vec2);
      tv0 = move(w0);
      tv1 = move(w1);
      add(tv1, w2);
    }
    add(tv0, tv1);
    return {csize, move(tv0)};
  };
  auto [_, vec] = dfs(dfs, 1);
  Fp cnt = 0;
  ll sgn = 1;
  CombB cb(N);
  for (ll i = 1; i <= N; i++) {
    Fp w = i < (ll)vec.size() ? vec[i] : Fp(0);
    cnt += sgn * w * cb.fact(N - i);
    sgn = -sgn;
  }
  Fp ans = cb.fact(N) - cnt;
  cout << ans << endl;
