...
int main(int argc, char *argv[]) {
  ...
  ll N; cin >> N;
  vector<vector<Fp>> tbl(62, vector<Fp>(3));
  tbl.at(61).at(0) = 1;
  
  for (int i = 60; i >= 0; i--) {
    ll bit = 1LL << i;
    if (N & bit) {
      tbl.at(i).at(0) += 1 * tbl.at(i+1).at(0);
      tbl.at(i).at(1) += 1 * tbl.at(i+1).at(0);
      tbl.at(i).at(1) += 1 * tbl.at(i+1).at(1);
      tbl.at(i).at(2) += 2 * tbl.at(i+1).at(1);
      tbl.at(i).at(2) += 3 * tbl.at(i+1).at(2);
    }else {
      tbl.at(i).at(0) += 1 * tbl.at(i+1).at(0);
      tbl.at(i).at(0) += 1 * tbl.at(i+1).at(1);
      tbl.at(i).at(1) += 1 * tbl.at(i+1).at(1);
      tbl.at(i).at(2) += 1 * tbl.at(i+1).at(1);
      tbl.at(i).at(2) += 3 * tbl.at(i+1).at(2);
    }
  }
  Fp ans = tbl.at(0).at(0) + tbl.at(0).at(1) + tbl.at(0).at(2);
  cout << ans << endl;
  return 0;
}
