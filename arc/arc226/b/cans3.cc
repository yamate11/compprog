...
// @@ !! LIM(debug binsearch digit forall)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    auto check = [&](ll X) -> bool {
      if (X == 0) return FORALL(i, 0, M, A[i] == 0);
      auto vec = du2.to_vector(X);
      for (ll& t : vec) t *= N;
      DLOGKL("check called", X, vec);
      if (ssize(vec) < M) return false;
      ll i = ssize(vec) - 1;
      for (; i >= M; i--) {
        if (vec[i] * 2 >= big) {
          DLOGKL("ret1", X, i, vec[i]);
          return true;
        }
        vec[i - 1] += vec[i] * 2;
        vec[i] = 0;
      ...
      for (; i >= 0; i--) {
        if (A[i] > vec[i]) {
          DLOGKL("  return false", i, A[i], vec[i]);
          return false;
        }
        vec[i] -= A[i];
        if (vec[i] * 2 >= big) {
          DLOGKL("  ret2", X, i, vec[i]);
          return true;
        }
        if (i > 0) vec[i - 1] += vec[i] * 2;
      }
      DLOGKL("  return true", i);
      return true;
    };
    ll X = binsearch<ll>(check, big, -1);
    return X;
