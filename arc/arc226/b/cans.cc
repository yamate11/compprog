...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    ll big = 1LL << 60;
#endif
    vector<pll> vec{{big, N}};
    REPrev(i, M - 1, 0) {
      ll x = A[i];
      while (true) {
        DLOGK(i, x, vec);
        auto [ht, wd] = vec.back(); vec.pop_back();
        ll htc = ht / (1LL << i);
        if (wd <= x / htc) {
          x -= htc * wd;
        }else {
          ...
          ll q = x % wd;
          if (q == 0) {
            vec.emplace_back(ht - p * (1LL << i), wd);
          }else {
            if (ht - (p + 1) * (1LL << i) > 0) vec.emplace_back(ht - (p + 1) * (1LL << i), wd);
            vec.emplace_back(1LL << i, wd - q);
          }
          break;
        }
      }
