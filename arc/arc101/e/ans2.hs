...
solve n xys = (oTbl ! 0) `mSub` (eTbl ! 0)
  ...
  calc v0 | trace ("calc_IN " ++ show v0) False = undefined
  calc v0 = trace ("calc " ++ show (v0, a)) a
    where a = calc0 v0

  calc0 v0 = partZero partOnePlus
    where
    partOnePlus = foldl op (VU.fromList [1], VU.fromList [])
                           (map calc (children V.! v0))
      where
...
--      conv2 v1 v2 v3 v4 = zipWithMAddL (mConvolve v1 v2) (mConvolve v3 v4)
    ...
    partZero (eL, oL) = (f eL oL, f oL eL)
      where f ls1 ls2 = VU.cons (vmSum (VU.zipWith mMul vcmbPair ls2)) ls1
--      where f ls1 ls2 = mSum (zipWith mMul cmbPair ls2) : ls1

  vcmbPair = VU.fromListN (n+1) cmbPair
...
vzWMA v1 v2 | trace ("vzWMA_IN " ++ show (v1,v2)) False = undefined
vzWMA v1 v2 = trace ("vzWMA_OUT " ++ show a) a
  where a = vzWMA0 v1 v2
vzWMA0 v1 v2 = VU.fromListN (max len1 len2) (zipWith mAdd p1 p2)
  where len1 = VU.length v1
        len2 = VU.length v2
        u1 = VU.toList v1
        u2 = VU.toList v2
        (p1,p2) | len1 >= len2 = (u1, u2 ++ repeat 0)
                | otherwise    = (u1 ++ repeat 0, u2)
...
vmConv v1 v2 = VU.fromListN
               (VU.length v1 + VU.length v2 - 1)
               (mConvolve (VU.toList v1) (VU.toList v2))
