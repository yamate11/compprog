...
int body(istream& cin, ostream& cout) {
  ...
  for (ll i = 0; i < N; i++) CR[i] = -C[i];
  auto check = [&](ll b0) -> bool {
    ll c0 = A[0] - b0;
    ll iB = upper_bound(B.begin(), B.end(), -b0) - B.begin();
    ll iC = upper_bound(CR.begin(), CR.end(), c0) - CR.begin();
    ll j2 = max(iB, iC);
    ll j1 = min(iB, iC);
    DLOGK(b0, j1, j2);
    return (N - j2) >= j1;
  };
  ll b0 = binsearch_i<ll>(check, max(1LL, A[0] + 1), -B[N-1]-1);
  ll c0 = A[0] - b0;
  DLOGKL("binsearch_res", b0, c0);
  ll ans = LLONG_MAX;
  for (ll dd = -2; dd <= 2; dd++) {
    ll b1 = b0 + dd;
    ll c1 = c0 - dd;
    ll sum = abs(b1) + abs(c1);
    for (ll i = 1; i < N; i++) sum += abs(b1 + B[i]) + abs(c1 + C[i]);
    ans = min(ans, sum);
  }
  cout << ans << endl;
