...
vector<ll> knapsack(vector<ll> wt, vector<ll> val, vector<ll> num, ll maxVal) {
  ...
  vector<ll> results(maxVal + 1, LLONG_MAX);
  results.at(0) = 0;
  for (ll i = 0; i < (ll) mWt.size(); i++) {
	...
	ll w0 = results.at(v);
	ll w1 = results.at(v1);
	if (w1 != LLONG_MAX)  results.at(v) = min(w0, w1 + mWt.at(i));
      }
    }
  }
  return results;
