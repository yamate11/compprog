...
int body(istream& cin, ostream& cout) {
  ...
  vector<Fp> Q0(K, Fp(0)), Q1(K, Fp(0));
  Q0[K - 1] = Fp(1);
  REPrev(m, K - 2, 0) Q0[m] = (N - m - 2) * Q0[m + 1];
  Q1[K - 1] = Fp(1);
  REPrev(m, K - 2, 0) Q1[m] = (N - m - 1) * Q1[m + 1];
  
  auto stA = make_seg_tree(0LL, plus<ll>(), vector(N, 0LL));
  vector<Fp> alpha(K);
  REP(i, 0, K) {
    alpha[i] = P[i] - stA.query(0, P[i]);
    stA.set_single(P[i], 1);
  }

  Fp tot(0);
  REP(m, 0, K) tot += alpha[m] * Q1[m];
  tot += Fp(1); // for K
  
  vector<Fp> init_vec(K + 1);
  REP(m, 0, K) {
    init_vec[m] = (alpha[m] - 1) * Q0[m];
  }
  init_vec[K] = Fp(1);
  auto stV = make_seg_tree(Fp(0), plus<Fp>(), init_vec);

  REP(t, 0, N) {
    ll z = revP[t];
    if (z < K) stV.set_single(z, stV.get_single(z) + Q0[z]);
    Fp c = stV.query(0, z + 1);
    cout << tot - c << "\n";
  }
