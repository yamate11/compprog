...
pair<Fp, Fp> eqsolve(Fp a1, Fp b1, Fp c1, Fp a2, Fp b2, Fp c2) {
  Fp v = a1 * b2 - b1 * a2;
  Fp x0 = b2 * c1 - b1 * c2;
  Fp y0 = -a2 * c1 + a1 * c2;
  return {x0 / v, y0 / v};
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  Fp a(0), b(0), abar(0), bbar(0);
  REP(k, 1, N) {
    Fp prev_a = a;
    Fp prev_b = b;
    Fp prev_abar = abar;
    Fp prev_bbar = bbar;
    tie(a, abar) = eqsolve(N, -(N - k), k * prev_abar + N - k,
                           -(N - k), N, k * prev_a);
    tie(b, bbar) = eqsolve(N, -(N - k), k * prev_bbar,
                           -(N - k), N, k * prev_b + N - k);
  }
  cout << abar << " " << bbar << endl;
