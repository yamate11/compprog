...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<Fp> pow2(N);
    pow2[0] = Fp(1);
    REP(i, 1, N) pow2[i] = Fp(2) * pow2[i - 1];
    DLOGK(pow2);
    ...
    REP(i, 0, K) {
      ...
      if (amax < A[i]) {
        ...
        ans += pow2[A[i]] * cb.fact(M);
      }
    }
    DLOGK(amax, ans, app);
    REP(i, 0, N) if (not app[i]) {
      if (i < amax) ans += pow2[i] * cb.fact(M);
      else ans += pow2[i] * cb.fact(M) * (Fp(N - i - 1) / Fp(N - i));
      DLOGK(i, ans);
    }
    cout << ans << endl;
