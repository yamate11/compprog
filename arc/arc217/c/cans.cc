...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ranges::sort(A);
    vector<Fp> P(N + 1);
    REP(i, 0, N + 1) P[i] = Fp(A[i + 1] - A[i]) / Fp(C - A[i]);
    vector PPP(N + 1, vector(N + 1, Fp(1)));
    vector PPN(N + 1, vector(N + 1, Fp(1)));
    REP(r, 1, N + 1) {
      REP(i, 0, N + 1) {
        PPP[i][r] = PPP[i][r - 1] * P[i];
        PPN[i][r] = PPN[i][r - 1] * (Fp(1) - P[i]);
      }
    }
    DLOGK(P);
    DLOGK(PPP);
    ...
    REP(k, 0, N + 1) {
      ...
      REP(i, 0, k + 1) {
        ...
        REP(j, 0, N - k + i + 1) {
          tbl[j] = Fp(0);
          REP(x, 0, j + 1) {
            Fp s = prev[x] * cb.binom(N - x, j - x) * PPP[i][j - x] * PPN[i][N - j];
            tbl[j] += s;
          }
        }
        DLOGK(k, i, tbl);
      ...
      aup[k] = 0;
      REP(j, 0, N + 1) aup[k] += tbl[j];
    }
    vector<Fp> ans(N + 1);
    REP(i, 0, N) ans[i] = aup[i] - aup[i + 1];
    ans[N] = aup[N];
