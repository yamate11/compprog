...
// @@ !! LIM(segTree sieve debug)
// --> f:<< segTree sieve debug
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  vector<ll> L(N), R(N);
  using ev_t = tuple<ll, ll, ll>; // station, on/off, idx
  vector<ev_t> ev;
  for (ll i = 0; i < N; i++) {
    ll l, r; cin >> l >> r; r++;
  ...
  ll ei = 0;
  for (ll i = 1; i <= M; i++) {
    for ( ; ei < (ll)ev.size(); ei++) {
      auto [t, op, idx] = ev[ei];
      if (t > i) break;
      assert(i == t);
      if (op == 0) {
	DLOG("removing", L[idx], "for", idx);
	st.update(L[idx], L[idx] + 1, -1);
      }else if (op == 1) {
	st.update(i, i + 1, 1);
	DLOG("adding", i, "for", idx);
      }else assert(0);
    }
    for (ll x : getDivisors(i, primes)) {
      ll adding = st.query(i - x + 1, i + 1);
      DLOG("i=", i, "x=", x, "adding=", adding);
      ans[x] += adding;
    }
  }
  for (ll i = 1; i <= M; i++) {
