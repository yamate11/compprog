...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  ll N; cin >> N;
  // @InpMVec(N, ((A, dec=1), (B, dec=1))) [JX1TSber]
  auto A = vector(N, ll());
  auto B = vector(N, ll());
  for (int i = 0; i < N; i++) {
  ...
  // @End [JX1TSber]
  safe_umap<ll, vector<ll>> mp;
  REP(i, 0, N) {
    mp[A[i]].push_back(B[i]);
  }
  for (auto& [_i, vec] : mp) ranges::sort(vec);
  ll cand1 = 0;
  for (auto& [_i, vec] : mp) cand1 += 2 * vec.back();
  ...
  for (auto& [_i, vec] : mp) {
    ll mincost = big;
    REP(j, -1, ssize(vec)) {
      if (j == -1) updMin(mincost, 2 * (W - vec[0] - 1));
      else if (j == ssize(vec) - 1) updMin(mincost, 2 * vec.back());
      else updMin(mincost, 2 * (W - (vec[j + 1] - vec[j]) - 1));
    }
    costs.push_back(mincost);
    DLOGK(vec, _i, mincost);
  }
  ranges::sort(costs);
  ll sz = ssize(costs);
  auto it = ranges::lower_bound(costs, W - 1);
  ll t = costs.end() - it;
  ll w2 = t / 2;
  ll w3 = (t + 1) / 2;
  if (w2 == 0) w2 = 1;
  if (w3 == 0) w3 = 1;
  cand2 = w2 * 2 * (W - 1) + (sz <= w2 * 2 ? 0 : accumulate(costs.begin(), costs.begin() + (sz - w2 * 2), 0LL));
  cand3 = w3 * 2 * (W - 1) + (sz <= w3 * 2 ? 0 : accumulate(costs.begin(), costs.begin() + (sz - w3 * 2), 0LL));
