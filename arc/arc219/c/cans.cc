...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> LR;
  for (auto& [_i, vec] : mp) {
    ll mn = *ranges::min_element(vec);
    ll mx = *ranges::max_element(vec);
    L.push_back(2 * mx);
    
    ll imax = 0;
    REP(i, 0, ssize(vec) - 1) imax = max(imax, vec[i + 1] - vec[i]);
    LR.push_back(2 * min(W - 1 - imax, min(mx, W - 1 - mn)));
  }
  ll cand1 = accumulate(ALL(L), 0LL);
  ranges::sort(LR);
  ll cand2;
  ll sz = ssize(LR);
  DLOGK(sz);
  if (sz <= 2) cand2 = (W - 1) * 2;
    ...
    ll p = ranges::lower_bound(LR, W - 1) - LR.begin();
    ll t = sz - p;
    DLOGK(p, t, LR);
    if (t <= 2) {
      DLOG("case A");
      cand2 = (W - 1) * 2 + accumulate(LR.begin(), LR.begin() + (sz - 2), 0LL);
    }else if (t % 2 == 0) {
      DLOG("case B");
      cand2 = (W - 1) * (t / 2) + accumulate(LR.begin(), LR.begin() + p, 0LL);
    }else {
      ll a = (W - 1) * (t / 2) + accumulate(LR.begin(), LR.begin() + p + 1, 0LL);
      ll b;
      if (p >= 1) {
        b = (W - 1) * (t / 2 + 1) + accumulate(LR.begin(), LR.begin() + (p - 1), 0LL);
      }else {
        b = (W - 1) * (t / 2 + 1);
      }
      DLOGKL("case C", a, b);
