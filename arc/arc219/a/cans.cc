...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpVec(N, S, type=string) [3NwLaV3W]
  ...
  REP(i, 0, M) {
    vector<ll> vec0, vec1;
    for (ll j: vec) {
      if (S[j][i] == '0') vec0.push_back(j);
      else vec1.push_back(j);
    }
    if (ssize(vec0) < ssize(vec1)) {
      y += '1';
      vec = vec0;
    }else {
      y += '0';
  ...
  REP(x, 0, 1LL << k) {
    if (not ss.contains(x)) {
      string y = du.to_string(x);
      y = string(k - ssize(y), '0') + y;
      REP(i, 0, k) y[i] = y[i] == '0' ? '1' : '0';
      REP(i, k, M) y += '0';
      cout << "Yes\n";
      cout << y << endl;
      return 0;
