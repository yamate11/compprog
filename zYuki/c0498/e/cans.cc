...
int main(/* int argc, char *argv[] */) {
  ...
  auto report = [&](ll x) -> void { cout << "! " << x << endl; };
  ...
  auto primes = sieve((ll)3.5e4);
  auto euler_phi = [&](ll y) -> ll {
    ll beta = 1;
  ...
  auto f = [&](auto rF, ll x, ll k, ll y) -> ll {
    if (y == 1) return 0;
    ...
    ll beta = euler_phi(y);
    Fp::setMod(beta);
    ll r = power<Fp>(M, L) - Fp(k);
    Fp::setMod(y);
    ll t = power<Fp>(x / g, r);
    ll s = 1;
    if (g > 1) s = rF(rF, g, k + 1, y / g);
    ll ret = g * s * t % y;
    DLOGK(x, k, y, t, s);
    return ret;
  };
