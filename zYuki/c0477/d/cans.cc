...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    ...
    A.push_back(Mat(2, 2, vector<Fp>{a1, a2, a3, a4}));
  }
  Mat unitMat(2, 2, vector<Fp>{1, 0, 0, 1});

  auto st = make_seg_tree(unitMat, multiplies<Mat>(), A);

  ll Q; cin >> Q;
  REP(_q, 0, Q) {
    ll i, L, R, y1, y2, y3, y4; cin >> i >> L >> R >> y1 >> y2 >> y3 >> y4; i--; L--;
    st.rs(i) = Mat(2, 2, vector<Fp>{y1, y2, y3, y4});
    auto ans = st.query(L, R);
    cout << ans.at(0, 0) << " " << ans.at(0, 1) << "\n" << ans.at(1, 0) << " " << ans.at(1, 1) << "\n";
  }
