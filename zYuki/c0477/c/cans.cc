...
int main(/* int argc, char *argv[] */) {
  ...
  ll a, b, c, d, s, t, n, k; cin >> a >> b >> c >> d >> s >> t >> n >> k;
  Fp::setMod(k);

  Matrix<Fp> M(2, 2, vector<Fp>{a, b, c, d});
  Matrix<Fp> X(2, 1, vector<Fp>{s, t});
  Matrix<Fp> ans = M.matpower(n) * X;
  cout << ans.at(0, 0) << " " << ans.at(1, 0) << endl;
