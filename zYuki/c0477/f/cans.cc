...
// @@ !! LIM(matrix ftwo mod power debug forall)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpVec(M, S, type=string) [hUrzmQn2]
  auto S = vector(M, string());
  for (int i = 0; i < M; i++) { string v; cin >> v; S[i] = v; }
  // @End [hUrzmQn2]

  using MyBitset = bitset<2600>;
  vector<MyBitset> vec(M);
  REP(i, 0, M) REP(j, 0, N) vec[i][j] = S[i][j] == '1';
  ll idx = 0;
  REP(j, 0, N) {
    ll i0 = EXFIND_D(i, idx, M, vec[i][j], M);
    if (i0 < M) {
      if (idx < i0) swap(vec[idx], vec[i0]);
      REP(i, idx + 1, M) if (vec[i][j]) vec[i] ^= vec[idx];
      idx++;
    }
  }
