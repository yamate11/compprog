...
int body(istream& cin, ostream& cout) {
  ...
  ll ans = 0;
  vector B(K0 + 1, vector<ll>());
  B[K0] = A;
  REPrev(k, K0 - 1, 0) {
    if (k % 2 != 0) {
      for (ll a : A) if (a >> k & 1) B[k].push_back(a);
      if (not B[k].empty()) ans |= (1LL << k);
    }else {
      vector new_B(K0 + 1, vector<ll>());
      auto check = [&]() -> bool {
        bool progr = false;
        REPrev(l, K0, 0) {
          if (not B[l].empty()) {
            for (ll a : B[l]) if (a >> k & 1) new_B[l].push_back(a);
            if (new_B[l].empty()) return false;
            progr = true;
          }
        }
        return progr;
      ...
      if (check()) {
        ...
        ans |= (1LL << k);
        vector<ll> new_A;
        for (ll a : A) if (a >> k & 1) new_A.push_back(a);
        A = move(new_A);
      }
    }
    DLOGK(k, A, B);
