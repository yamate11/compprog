...
int naive(istream& cin, ostream& cout) {
  ...
  REP(_q, 0, Q) {
    ...
    while (ip.get()) {
      ...
      if (t % 8 == 0) {
        ll rnum = 0;
        REP(i, 0, len) REP(j, i + 1, len) if (ip[i] > ip[j]) rnum++;
        ans = min(ans, rnum);
      }
    }
    if (ans == big) ans = -1;
...
int body(istream& cin, ostream& cout) {
  ...
  ll N, Q; cin >> N >> Q;
  string S; cin >> S;
  vector pos(N + 1, vector<ll>(10));
  pos[0] = vector<ll>(10, -1LL);
  REP(i, 0, N) {
    pos[i + 1] = pos[i];
    ll d = S[i] - '0';
    pos[i + 1][d] = i;
  ...
  REP(i, 0, Q) {
    ...
    if (l + 1 == r) {
      ...
      ll x = 10 * (S[l] - '0') + 1 * (S[l + 1] - '0');
      ll y = 1 * (S[l] - '0') + 10 * (S[l + 1] - '0');
      ll ans = -1;
      if (x % 8 == 0) ans = 0;
      else if (y % 8 == 0) ans = 1;
      cout << ans << "\n";
      ...
      REP(xx, 0, 125) {
        ...
        auto f = [&]() -> ll {
          ...
          ll y0 = (x / 100) % 10;
          ll pos2 = pos[r][y2];
          if (pos2 < l) return big;
          ll pos1;
          if (y1 == y2) pos1 = pos[pos2][y1];
          else          pos1 = pos[r][y1];
          if (pos1 < l) return big;
          ll pos0;
          if (y0 == y1) pos0 = pos[pos1][y0];
          else if (y0 == y2) pos0 = pos[pos2][y0];
          else pos0 = pos[r][y0];
          if (pos0 < l) return big;
          vector<ll> w{pos0, pos1, pos2};
          sort(ALL(w));
          ll ret = (r - 1 - w[2]) + (r - 2 - w[1]) + (r - 3 - w[0]);
          DLOGK(ret);
          ret += (pos0 < pos1 ? 0 : 1) + (pos0 < pos2 ? 0 : 1) + (pos1 < pos2 ? 0 : 1);
          DLOGK(i, l, r, x, pos0, pos1, pos2, ret);
          return ret;
        };
        ans = min(ans, f());
      }
