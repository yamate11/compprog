...
int main(/* int argc, char *argv[] */) {
  ...
  string N; cin >> N;
  vector tbl_init(2, vector(2, vector(2, Fp(0))));
  auto tbl = tbl_init;
  tbl[1][1][0] = 1;
  for (char c : N) {
    ll d = c - '0';
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(eq, 0, 2) REP(az, 0, 2) REP(has, 0, 2) {
      if (prev[eq][az][has] == 0) continue;
      REP(x, 0, 10) {
        if (eq and x > d) continue;
        ll new_eq = eq && x == d;
        ll new_az = az && x == 0;
        ll new_has = has or x == 8;
        tbl[new_eq][new_az][new_has] += prev[eq][az][has];
      }
    }
  }
  Fp ans = 0;
  REP(eq, 0, 2) ans += tbl[eq][0][1];
  cout << ans << endl;
