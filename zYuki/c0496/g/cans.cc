...
int body(istream& cin, ostream& cout) {
  ...
  for (int i = 0; i < N; i++) { ll v; cin >> v; A[i] = v; }
  // @End [NZU667sY]
  // @InpMVec(Q, ((I, dec=1), X, (L, dec=1), R)) [96Bk0dNV]
  auto I = vector(Q, ll());
  auto X = vector(Q, ll());
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  ...
  vector SpecR(Q, 0LL);
  vector SpecI(Q, 0LL);
  auto st = make_seg_tree(0LL, plus<ll>(), vector<ll>(N + Q, 1LL));

  auto getIndex = [&](ll i) -> ll {
    auto check = [&](ll val) -> bool { return val <= i; };
    return st.binsearch_r_until(check, 0);
  };
  REPrev(q, Q - 1, 0) {
    SpecL[q] = getIndex(L[q]);
    SpecR[q] = getIndex(R[q]);
    SpecI[q] = getIndex(I[q] + 1);
    st.rs(SpecI[q]) = 0;
  }
  DLOGK(SpecL, SpecR, SpecI);
  vector<ll> init_vec(N + Q, 0LL);
  REP(i, 0, N) init_vec[getIndex(i)] = A[i];
  auto st2 = make_seg_tree(0LL, plus<ll>(), init_vec);
  REP(q, 0, Q) {
    st2.rs(SpecI[q]) = X[q];
    cout << st2.query(SpecL[q], SpecR[q]) << "\n";
  }
  return 0;
}
