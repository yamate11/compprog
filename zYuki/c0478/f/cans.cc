...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpMVec(N -1, ((U, dec=1), (V, dec=1))) [ddBeA6SQ]
  ...
  vector<ll> vec1;
  REP(i, 0, N - 1) {
    if (tr.depth(U[i]) % 2 == 0) vec1.push_back(U[i]);
    else                         vec1.push_back(V[i]);
  }
  bool b1 = ask(vec1);
  
  ll k0 = bit_width(u64(N - 1));
  ll x = 0;
  REP(k, 0, k0) {
    vector<ll> vec;
    REP(i, 0, N - 1) {
      ll p, c;
      if (tr.parent(U[i]) == V[i]) { p = V[i]; c = U[i]; }
      else                         { p = U[i]; c = V[i]; }
      if (b1 and (tr.depth(c) % 2 == 0)) {
        if ((i + 1) >> k & 1) vec.push_back(c);
        else            vec.push_back(p);
      }else if (b1 and (tr.depth(c) % 2 != 0)) {
        vec.push_back(c);
      }else if (not b1 and (tr.depth(c) % 2 == 0)) {
        vec.push_back(c);
      }else if (not b1 and (tr.depth(c) % 2 != 0)) {
        if ((i + 1) >> k & 1) vec.push_back(c);
        else            vec.push_back(p);
  ...
  if (x == 0) ans = 0;
    ...
    if (tr.parent(U[x - 1]) == V[x - 1]) ans = U[x - 1];
    else                                 ans = V[x - 1];
  }
  report(ans);
  return 0;
