...
int naive(istream& cin, ostream& cout) {
  ll N, K, L; cin >> N >> K >> L;
  // @InpVec(L, B, dec=1) [zitgbrFT]
  auto B = vector(L, ll());
  for (int i = 0; i < L; i++) { ll v; cin >> v; v -= 1; B[i] = v; }
  // @End [zitgbrFT]
  vector T(N, vector(L, 0LL));
  ...
  ll ansnum = -1;
  vector<ll> anspwd(L, 0LL);
  IntDirProd idp(vector<ll>(L, K));
  while (idp.get()) {
    if (EXISTS(j, 0, L, B[j] >= 0 and B[j] != idp.at(j))) continue;
    IntPerm ip(N, N);
    while (ip.get()) {
      ll cnt = 0;
      vector<ll> C(K);
      ...
      REP(ii, 0, N) {
        ll i = ip.at(ii);
        if (FORALL(j, 0, L, C[idp.at(j)] == T[i][j])) cnt++;
        vector<ll> new_C(K);
        REP(j, 0, K) new_C[j] = A[i][C[j]];
        C = move(new_C);
      }
      auto idp_smaller = [&]() -> bool {
        REP(j, 0, L) if (anspwd[j] != idp[j]) return idp[j] < anspwd[j];
        return false;
      };
      if (cnt > ansnum or (cnt == ansnum and idp_smaller())) {
        ansnum = cnt;
        REP(i, 0, L) anspwd[i] = idp[i];
      }
    }
  }
  cout << ansnum << endl;
  REPOUT(i, 0, L, anspwd[i] + 1, " ");
...
int body(istream& cin, ostream& cout) {
  ...
  ll ansnum = 0;
  vector<ll> anspwd(L, 0LL);
  REP(i, 0, L) if (B[i] >= 0) anspwd[i] = B[i];
  auto comp = [&](const auto& vec1, const auto& vec2) -> bool {
    REP(i, 0, L) if (vec1[i] != vec2[i]) return vec1[i] < vec2[i];
    return false;
  };
  IntPerm ip(N, N);
  while (ip.get()) {
    vector R(N, vector(L, 0LL));
    vector C(K, 0LL);
    REP(i, 0, K) C[i] = i;
    REP(ii, 0, N) {
      ll i = ip.at(ii);
      vector D(K, 0LL);
      REP(j, 0, K) D[C[j]] = j;
      REP(j, 0, L) R[i][j] = D[T[i][j]];
      vector new_C(K, 0LL);
      REP(j, 0, K) new_C[j] = A[i][C[j]];
      C = move(new_C);
    }
    REP(i, 0, N) {
      if (FORALL(j, 0, L, B[j] < 0 or R[i][j] == B[j])) {
        ll cnt = 0;
        REP(ii, 0, N) {
          if (FORALL(j, 0, L, R[i][j] == R[ii][j])) cnt++;
        }
        if (cnt > ansnum or (cnt == ansnum and comp(R[i], anspwd))) {
          ansnum = cnt;
          anspwd = R[i];
        }
      }
