...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [PVi5RWRZ]
  // @InpMVec(M, ((L, dec=1), (R, dec=0))) [jxup97Go]
  auto L = vector(M, ll());
  auto R = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  vector sl(N, vector<ll>());
  REP(i, 0, M) sl[L[i]].push_back(i);
  DLOGK(sl);
  /*
  vector el(N + 1, false);
  REP(i, 0, M) el[R[i]] = true;
  */

  vector tbl(N + 1, vector<ll>(K + 1, -1LL));
  tbl[0][0] = 0;
  ll maxe = -1;
  REP(i, 0, N + 1) {
    if (i > 0) {
      REP(k, 0, K + 1) updMax(tbl[i][k], tbl[i - 1][k]);
      if (i == N) break;
    }
    for (ll p : sl[i]) maxe = max(maxe, R[p]);
    DLOGK(i, maxe);
    if (maxe > i) {
      REP(k, 0, K) {
        if (tbl[i][k] >= 0) {
          updMax(tbl[maxe][k + 1], tbl[i][k] + SV[maxe] - SV[i]);
        }
      }
    }
  }
  ll ans = 0;
  REP(k, 0, K + 1) ans = max(ans, tbl[N][k]);
  cout << ans << endl;
