...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((U, dec=1), (V, dec=1))) [HOwsdxfi]
  auto U = vector(M, ll());
  auto V = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  UnionFind<Ftwo> uf(N);
  REP(i, 0, M) {
    uf.merge(U[i], V[i], Ftwo(1));
  }
  vector<ll> side(N);
  REP(i, 0, N) {
    ll a = uf.leader(i);
    assert(uf.pot(a) and uf.pot(i));
    if (*uf.pot(a) == *uf.pot(i)) side[i] = 0;
    else side[i] = 1;
  }
  ...
  mf_graph<ll> g(N + 2);
  REP(i, 0, N) {
    if (side[i] == 0) g.add_edge(source, i, 1);
    else g.add_edge(i, sink, 1);
  }
  REP(i, 0, M) {
    assert(side[U[i]] != side[V[i]]);
    if (side[U[i]] == 0) g.add_edge(U[i], V[i], 1);
    else g.add_edge(V[i], U[i], 1);
  }
  ll m = g.flow(source, sink);
  cout << m << endl;
