...
// @@ !! LIM(f:updMaxMin debug digit input)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, K, valT; cin >> N >> M >> K >> valT;
  // @InpNbrList(N, M, nbr, dir=True, dec=1, read=W) [WkoWpFqa]
  struct nbr_t {
    int nd;
    ll W;
    string show() const {
    ...
    while (not pque.empty()) {
      ...
      if (dist[start][nd] == d) {
        for (auto [peer, len] : nbr[nd]) {
          ll new_d = d + len;
          if (dist[start][peer] > new_d) {
            dist[start][peer] = new_d;
  ...
  vector tbl(1LL << szT, vector(szT, big));
  ll r0 = revT[0];
  tbl[1LL << r0][r0] = 0;
  REP(x, 0, 1LL << szT) REP(i, 0, szT) {
    if (tbl[x][i] < big) {
      REP(j, 0, szT) {
        if (i == j) continue;
        if (x >> j & 1) continue;
        ll y = x | (1LL << j);
        updMin(tbl[y][j], tbl[x][i] + dist[T[i]][T[j]]);
      }
    }
  }
  ll ans = 0;
  REP(x, 0, 1LL << szT) REP(i, 0, szT) {
    if (tbl[x][i] <= valT) {
      ll rew = 0;
      REP(j, 0, K) if ((pat[j] & ~x) == 0) rew += P[j];
      if (ans < rew) {
        DLOGK(x, du2.to_string(x), rew, i, tbl[x][i]);
        ans = rew;
      }
