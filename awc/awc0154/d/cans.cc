...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  /*
  // @InpNbrList(N, M, fwd, dir=True, dec=1, read=W) [tb44PPeU]
  struct fwd_t {
    int nd{};
    ll W{};
    string show() const {
  ...
  // @End [tb44PPeU]
  */
  // @InpMVec(M, ((U, dec=1), (V, dec=1), (W, dec=0))) [13QTPxWL]
  auto U = vector(M, ll());
  auto V = vector(M, ll());
  auto W = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  REP(i, 0, M) updMin(dist[U[i]][V[i]], W[i]);
  REP(k, 0, N) REP(i, 0, N) REP(j, 0, N) updMin(dist[i][j], dist[i][k] + dist[k][j]);

  /*
  auto f = [&](ll start, ll goal) -> vector<bool> {
    ll big = 1LL << 60;
    ...
    dist[start] = 0;
    pque.emplace(0, start, -1);
    while (not pque.empty()) {
      auto [d, nd, prv] = pque.top(); pque.pop();
      if (dist[nd] == d) {
        for (auto [nxt, len] : fwd[nd]) {
          ll new_d = d + len;
          if (new_d < dist[nxt]) {
            dist[nxt] = new_d;
            pque.emplace(new_d, nxt, nd);
            bwd[nxt] = vector<ll>{nd};
          }else if (new_d == dist[nxt]) {
            bwd[nxt].push_back(nd);
          }
        }
      }
    }
    if (dist[goal] == big) return vector<bool>{};
    vector<bool> seen(N);
    vector<ll> stack;
    for (ll nd : bwd[goal]) {
      if (nd != start) {
    ...
    while (not stack.empty()) {
      ...
      for (ll p : bwd[nd]) {
        if (p != start and not seen[p]) {
          seen[p] = true;
          stack.push_back(p);
        }
      }
    }
    DLOGK(start, goal, seen);
    return seen;
  };
  */
  DLOGK(dist);
  auto f = [&](ll start, ll goal) -> vector<bool> {
    if (dist[start][goal] == big) return vector<bool>();
    vector<bool> ret(N);
    REP(i, 0, N) {
      if (i != start and i != goal and dist[start][i] + dist[i][goal] == dist[start][goal]) {
        DLOGK(start, goal, i);
        ret[i] = true;
      }
    }
