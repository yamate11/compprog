...
int main(/* int argc, char *argv[] */) {
  ...
  BrdIdx start, goal;
  REP(i, 0, H) REP(j, 0, W) {
    if (brd.at(i, j) == '@') start = BrdIdx(i, j);
    if (brd.at(i, j) == 'G') goal = BrdIdx(i, j);
    if (brd.at(i, j) == 'F') F[idx++] = BrdIdx(i, j);
  }
  assert(idx == K + 1);
  F[0] = start;
  F[K + 1] = goal;

  auto func = [&](BrdIdx base) {
    vector locD(H * W, big);
    locD[brd.enc(base)] = 0;
    queue<BrdIdx> que;
    que.push(base);
    while (not que.empty()) {
      ...
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (brd.at(bj) != '#' and locD[brd.enc(bj)] == big) {
          locD[brd.enc(bj)] = locD[brd.enc(bi)] + 1;
          que.push(bj);
        }
      }
    }
  ...
  vector dist(K + 2, vector(K + 2, big));
  REP(i, 0, K + 2) {
    auto locD = func(F[i]);
    REP(j, 0, K + 2) dist[i][j] = locD[brd.enc(F[j])];
  }
  if (EXISTS(i, 0, K + 2, dist[0][i] >= big)) {
    cout << -1 << endl;
    return 0;
  }
  vector tbl(1LL << (K + 2), vector(K + 2, big));
  tbl[1LL << 0][0] = 0;
  REP(x, 0, 1LL << (K + 2)) REP(i, 0, K + 2) if (tbl[x][i] < big) {
    REP(j, 0, K + 2) if (not (x >> j & 1)) updMin(tbl[x | (1LL << j)][j], tbl[x][i] + dist[i][j]);
  }
  cout << tbl[(1LL << (K + 2)) - 1][K + 1] << endl;
  return 0;
}
