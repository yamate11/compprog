...
int main(/* int argc, char *argv[] */) {
  ...
  // ll big = 1LL << 60;
  using DAT = vector<pll>;
  auto add = [&](const DAT& a, const DAT& b) -> DAT {
    DAT ret(C + 1);
    REP(i, 0, C + 1) {
      auto [cnt1, rem1] = a[i];
      auto [cnt2, rem2] = b[rem1];
      ret[i] = pll(cnt1 + cnt2, rem2);
    }
    return ret;
  };
  DAT unit(C + 1);
  REP(i, 0, C + 1) unit[i] = pll(0, i);
  auto single = [&](ll d) -> DAT {
    DAT ret(C + 1);
    REP(i, 0, d) ret[i] = pll(0, i);
    REP(i, d, C + 1) ret[i] = pll(1, i - d);
    return ret;
  };
  vector init(N, DAT(C + 1));
  REP(i, 0, N) init[i] = single(A[i]);
  auto st = make_seg_tree(unit, add, init);
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll p, x; cin >> p >> x; p--;
