...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl_init(2, vector(2, vector(10, vector(2, sta(0, 0)))));
  auto tbl = tbl_init;
  tbl[1][1][0][0] = sta(Fp(1), Fp(0));
  for (ll t : vec) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(eq,0,2) REP(az,0,2) REP(c,0,10) REP(hz,0,2) {
      auto [prev_num, prev_tot] = prev[eq][az][c][hz];
      if (prev_num == 0 and prev_tot == 0) continue;
      REP(d, 0, 10) {
        if (eq and d > t) continue;
        int new_eq = eq and d == t;
        int new_az = az and d == 0;
        int new_c = d;
        int new_hz = hz or (not az and d == 0);
        auto& [cur_num, cur_tot] = tbl[new_eq][new_az][new_c][new_hz];
        cur_num += prev_num;
        ll hdiff = az ? 0 : abs(c - d);
        cur_tot += prev_tot + prev_num * hdiff;
        DLOGK(t, eq, az, c, hz, new_eq, new_az, new_c, new_hz, hdiff, prev_num, prev_tot);
      }
    }
  }
  Fp ans = 0;
  REP(eq, 0, 2) REP(c, 0, 10) REP(hz, 0, 2) {
    auto [n, t] = tbl[eq][0][c][hz];
    ans += hz == 1 ? t * 2 : t;
  }
  cout << ans << endl;
