...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, Q; cin >> N >> M >> Q;
  // @InpMVec(N, ((S, type=string), (C, type=string))) [hDtSienZ]
  auto S = vector(N, string());
  auto C = vector(N, string());
  for (int i = 0; i < N; i++) {
    string v1; cin >> v1; S[i] = v1;
    string v2; cin >> v2; C[i] = v2;
  }
  // @End [hDtSienZ]
  // @InpVec(N, T, type=string) [l57UOrvY]
  auto T = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; T[i] = v; }
  // @End [l57UOrvY]

  vector A(N, vector(N, vector(M, 0LL)));
  REP(i, 0, N) {
    REP(j, 0, N) {
      REP(r, 0, M) {
        ll val = 0;
        REP(p, 0, M) {
          if (C[i][p] == '0' and S[i][p] != T[j][(p + r) % M]) val++;
        }
        A[i][j][r] = val;
      }
    }
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      REP(j, 0, N) {
        REP(r, 0, M) {
          if (C[i][p] == '0' and S[i][p] != T[j][(p + r) % M]) A[i][j][r]--;
          if (C[i][p] == '0' and x       != T[j][(p + r) % M]) A[i][j][r]++;
        }
      }
      S[i][p] = x;
      ...
      REP(j, 0, N) {
        REP(r, 0, M) {
          if (C[i][p] == '0' and S[i][p] != T[j][(p + r) % M]) A[i][j][r]--;
          if (b       == '0' and S[i][p] != T[j][(p + r) % M]) A[i][j][r]++;
        }
      }
      C[i][p] = b;
      ...
      REP(i, 0, N) {
        REP(r, 0, M) {
          ll q = imod(p - r, M);
          if (C[i][q] == '0' and S[i][q] != T[j][p]) A[i][j][r]--;
          if (C[i][q] == '0' and S[i][q] != x      ) A[i][j][r]++;
        }
      }
      T[j][p] = x;
