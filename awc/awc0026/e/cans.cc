...
int main(/* int argc, char *argv[] */) {
  ...
  ll vmin = mp.begin()->first;
  itv_set<ll> is(0, N);
  REP(i, 0, N) if (vmin <= A[i] and A[i] <= vmin + K) is.put(i, 1);
  ll ans = 0;
  auto it1 = mp.begin();
  for (auto& [a, va] : mp) {
    for (; it1 != mp.end() and it1->first <= a + K; it1++) for (ll i : it1->second) is.put(i, 1);
    DLOGK(a, va, is.impl);

    auto itH = mp.find(a + K);
    if (itH != mp.end()) {
      auto& vb = itH->second;
      DLOGK(va, vb);
      ll i = 0, j = 0;
      while (i < ssize(va) and j < ssize(vb)) {
        ll x = min(va[i], vb[j]);
        auto [l, r, t] = is.get(x);
        DLOGK(l, r);
        while (true) {
          if (i >= ssize(va) or r < va[i]) break;
          if (j >= ssize(vb) or r < vb[j]) break;
          if (va[i] < vb[j]) {
            ans += (va[i] - l + 1) * (r - vb[j]);
            DLOGK(i, j, va[i], vb[j], ans);
            l = va[i] + 1;
            i++;
          }else {
            ans += (vb[j] - l + 1) * (r - va[i]);
            DLOGK(j, i, vb[j], va[i], ans);
            l = vb[j] + 1;
            j++;
          }
        }
        while (i < ssize(va) and va[i] < r) i++;
        while (j < ssize(vb) and vb[j] < r) j++;
        DLOGK(i, j);
      }
    }
    for (ll i : va) is.put(i, 0);
  }
  cout << ans << endl;
