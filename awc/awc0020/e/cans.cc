...
int main(/* int argc, char *argv[] */) {
  ...
  auto mymax = [&](pll x, pll y) -> pll {
    ...
    auto [vy, iy] = y;
    if (vx < vy) return y;
    else if (vx > vy) return x;
    if (ix > iy) return y;
    return x;
  };
  auto st = make_seg_tree(pll(-big, -1), mymax, init_vec);
  DLOGK(st.vec_view());
  ...
  REP(i, 0, N) {
    ...
    if (i0 < M) {
      ...
      ll j = iR;
      while (i0 + 1 < j) {
        auto [vm, im] = st.query(i0 + 1, j);
        if (cL >= vm) break;
        mp[vm] = im;
        st.rs(im) = pll(-1LL, im);
        j = im;
      }
      DLOGK(ans, mp, st.vec_view());
    }
  }
  cout << ans << endl;
  return 0;
