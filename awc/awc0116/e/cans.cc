...
int naive(istream& cin, ostream& cout) {
  ...
  auto ok = [&](ll x) -> bool {
    ...
    REP(i, 0, ssize(s) - 2) {
      if (abs(s[i + 1] - s[i]) > abs(s[i + 2] - s[i + 1])) return false;
    }
    return true;
  };
...
int body(istream& cin, ostream& cout) {
  ...
  auto f = [&](const string& x) -> Fp {
    vector tbl_init(2, vector(2, vector(10, vector(10, Fp(0)))));
    auto tbl = tbl_init;
    tbl[1][1][0][0] = 1;
    for (char ct : x) {
      int t = ct - '0';
      auto prev = move(tbl);
      tbl = tbl_init;
      REP(eq, 0, 2) REP(az, 0, 2) REP(p, 0, 10) REP(q, 0, 10) {
        if (prev[eq][az][p][q] == 0) continue;
        REP(d, 0, 10) {
          ...
          if (new_az) {
            ...
            if (abs(d - p) < q) continue;
            new_p = d;
            new_q = abs(d - p);
          }
          tbl[new_eq][new_az][new_p][new_q] += prev[eq][az][p][q];
        }
      }
    }
    Fp ret = 0;
    REP(eq, 0, 2) REP(p, 0, 10) REP(q, 0, 10) ret += tbl[eq][0][p][q];
    return ret;
  };

  auto check = [&](const string& x) -> bool {
    REP(i, 0, ssize(x) - 2) {
      if (abs(x[i + 1] - x[i]) > abs(x[i + 2] - x[i + 1])) return false;
    }
    return true;
  };
