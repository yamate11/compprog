...
int main(/* int argc, char *argv[] */) {
  ...
  vector<pll> WT;
  REP(i, 0, N) WT.emplace_back(W[i], T[i]);
  ranges::sort(WT, greater<pll>());
  if (P[0] > WT[0].first) {
    cout << -1 << endl;
    return 0;
  }
  ...
  auto check = [&](ll t0) -> bool {
    ...
    ll i = 0;
    REP(j, 0, M) {
      for (; i < N and WT[i].first >= P[j]; i++) cap += t0 / WT[i].second;
      if (cap == 0) return false;
      cap--;
    }
    return true;
  };
  ll t0 = binsearch<ll>(check, WT[0].second * M, 0);
  cout << t0 << endl;
