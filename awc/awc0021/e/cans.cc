...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, a, b; cin >> N >> a >> b;
  vector mp(2, vector(2, safe_umap<ll, ll>()));
  auto f = [&](auto rF, ll p, ll q, ll x) -> ll {
    auto [it, bb] = mp[p][q].try_emplace(x, 0);
    ll& r = it->second;
    if (bb) {
      if (x == 1) {
        if      (p == 0 and q == 0) r = a / 2;
        else if (p == 0 and q == 1) r = b / 2;
        else if (p == 1 and q == 0) r = a;
        else if (p == 1 and q == 1) r = b;
        else assert(0);
      }else {
        ll y = x / 2;
        ll z = x - y;
        r = max(rF(rF, p, 0, y) + rF(rF, 0, q, z), rF(rF, p, 1, y) + rF(rF, 1, q, z));
      }
    }
    return r;
  };
  cout << max(f(f, 1, 0, N), f(f, 1, 1, N)) << endl;
