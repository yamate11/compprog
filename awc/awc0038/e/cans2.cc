...
// @@ !! LIM(UnionFind f:updMaxMin unordered_map)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [QoC9K4hx]
  // @InpNbrList(N, K, nbr, dec=1) [2kEmbFb6]
  ...
  UnionFind uf(N);
  REP(i, 0, N) {
    for (ll j : nbr[i]) uf.merge(i, j); 
  }
  ll ans = 0;
  auto gi = uf.group_info();
  REP(a, 0, N) {
    if (uf.leader(a) == a) {
      ll sz = uf.group_size(a);
      vector<ll> rev(N);
      auto mem = gi.group(a);
      REP(i, 0, sz) rev[mem[i]] = i;
      vector<ll> mask(sz);
      REP(i, 0, sz) {
        mask[i] = (1LL << i);
        for (ll b : nbr[mem[i]]) mask[i] |= (1LL << rev[b]);
      }

      safe_umap<ll, ll> mp;
      auto dfs = [&](auto rF, ll rem) -> ll {
        auto [it, b] = mp.try_emplace(rem, 0);
        if (b) {
          if (rem == 0) return it->second = 0;
          else {
            ll val = 0;
            REP(i, countr_zero((u64)rem), sz) {
              if (rem >> i & 1) {
                ll new_rem = rem & ~mask[i];
                updMax(val, P[mem[i]] + rF(rF, new_rem));
              }
            }
            it->second = val;
          }
