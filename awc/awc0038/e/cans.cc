...
// @@ !! LIM(f:updMaxMin unordered_map debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](ll lo, ll hi) {
    ...
    REP(x, 1, sz) {
      REP(i, 0, hi - lo) if (x >> i & 1) {
        ll exc = (1LL << i) | (nbr[lo + i] >> lo);
        updMax(ret[x], P[lo + i] + ret[x & ~exc]);
      }
    }
    return ret;
  };
  ...
  REP(x, 0, 1LL << H1) {
    ll y = 0;
    REP(i, 0, H1) if (x >> i & 1) y |= nbr[i];
    updMax(ans, res1[x] + res2[((1LL << H2) - 1) & ~(y >> H1)]);
  }
  cout << min(M, ans) << endl;
