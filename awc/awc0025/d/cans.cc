...
int main(/* int argc, char *argv[] */) {
  ...
  vector<pll> P(N);
  REP(i, 0, N) P[i] = pll(X[i], i);
  ranges::sort(P);

  vector<ll> vecF(N, 0LL);
  REP(i, 0, N) {
    auto [x, j] = P[i];
    if (i == 0) vecF[j] = P[1].second;
    else if (i == N - 1) vecF[j] = P[N - 2].second;
    else {
      auto [xL, jL] = P[i - 1];
      auto [xH, jH] = P[i + 1];
      if (x - xL == xH - x) vecF[j] = min(jL, jH);
      else if (x - xL < xH - x) vecF[j] = jL;
      else vecF[j] = jH;
    }
  }
  Doubling dobj(Q, N, vecF);
  ll ans = dobj.val(Q, S);
  cout << ans + 1 << endl;
