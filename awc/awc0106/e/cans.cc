...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [3XIwFgAz]
  // @InpMVec(Q, ((L, dec=1), (R, dec=0))) [LO1eslpH]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  vector<ll> ordA(N);
  REP(i, 0, N) ordA[i] = i;
  ranges::sort(ordA, [&](ll i, ll j) -> bool {
  ...
  REP(ii, 0, N) {
    ll i = ordA[ii];
    B[i] = *ss.lower_bound(i);
    ss.insert(i);
  }
  vector<ll> SB(N + 1);
  REP(i, 0, N) SB[i + 1] = SB[i] + B[i];
  auto segtN = make_seg_tree(0LL, plus<ll>(), vector<ll>(N, 0LL));
  auto segtS = make_seg_tree(0LL, plus<ll>(), vector<ll>(N, 0LL));
  using ev_tp = tuple<ll, ll, ll>;
  vector<ev_tp> ev;
  const int VAL = 2;
  const int QUERY = 1;
  REP(q, 0, Q) ev.emplace_back(R[q], QUERY, q);
  REP(i, 0, N) ev.emplace_back(B[i], VAL, i);
  ranges::sort(ev, greater<ev_tp>());
  vector<ll> ans(Q);
  for (auto [v, kd, idx] : ev) {
    if (kd == VAL) {
      segtN.rs(idx) = 1;
      segtS.rs(idx) = B[idx];
    }else if (kd == QUERY) {
      ll l = L[idx], r = R[idx];
      ans[idx] = (SB[r] - SB[l]) - segtS.query(l, r) + r * segtN.query(l, r) - (r - l) * (l + r - 1) / 2;
    }else assert(0);
  }
  REPOUT(i, 0, Q, ans[i], "\n");
  return 0;
