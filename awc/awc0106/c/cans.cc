...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [B13GVxy7]
  Tree tr(N, 0, true);
  vector elen(2, vector<ll>(N - 1));
  REP(i, 0, N - 1) {
    ll u, v; cin >> u >> v; u--; v--;
    ll e = tr.add_edge(u, v);
    elen[0][e] = abs(X[u] - X[v]) + abs(Y[u] - Y[v]);
    elen[1][e] = abs(X[u] + X[v]) + abs(Y[u] + Y[v]);
  }
  DLOG(tr);

  vector<ll> init_vec(2 * N, 0LL);
  REP(e, 0, N - 1) init_vec[tr.euler_in(tr.nodes_of_edge(e).second)] = elen[0][e];
  auto segt = make_seg_tree(0LL, plus<ll>(), init_vec);

  vector<bool> reversed(N, false);
  auto cur_elen = [&](ll e) -> ll {
    auto [p, c] = tr.nodes_of_edge(e);
    return elen[reversed[p] ^ reversed[c]][e];
  };
  auto rev_modified = [&](ll p, ll c) -> void {
    ll k = tr.euler_in(c);
    segt.rs(k) = cur_elen(tr.edge_idx(p, c));
  };
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ll c; cin >> c; c--;
      reversed[c] = not reversed[c];
      if (tr.num_children(c) >= 1) rev_modified(c, tr.child(c, 0));
      if (c != 0) {
        ll p = tr.parent(c);
        if (c == tr.child(p, 0)) rev_modified(p, c);
      }
    }else if (tp == 2) {
      ll w; cin >> w;
      S += w;
      ...
      ll a, b; cin >> a >> b; a--; b--;
      ll val = 0;
      for (auto [x, y] : tr.hl_path(a, b)) {
        val += cur_elen(tr.euler_elem_edge(x));       // light or heavy edge
        if (x + 1 < y) val += segt.query(x + 1, y);   // heavy edges
      }
      ll l = tr.lca(a, b);
      ll val2 = (tr.depth(a) - tr.depth(l) + tr.depth(b) - tr.depth(l) + 1) * S;
      val += val2;
      cout << val << "\n";
    }else assert(0);
  }
