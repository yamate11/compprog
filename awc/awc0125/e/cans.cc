...
*/
ll next_basis_elem(const vector<ll>& tri_basis, ll x) {
  ll e = x;
  for (ll b : tri_basis) e = min(e, e ^ b);
  return e;
}
...
vector<ll> getF2Basis(const vector<ll>& vs) {
  ...
  for (ll v : vs) {
    ll e = next_basis_elem(aux, v);
    if (e) {
      if constexpr (mode == 0) ret.push_back(v);
      aux.push_back(e);
    }
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(x, 0, 1LL << N) {
    vector<ll> A(N);
    REP(i, 0, N) A[i] = (x >> i & 1) ? V[i] : W[i];
    auto B = getF2Basis<1>(A);
    auto check = [&]() -> bool {
      REP(i, 0, M) {
        ll e = next_basis_elem(B, T[i]);
        if (e != 0) return false;
      }
      return true;
    };
