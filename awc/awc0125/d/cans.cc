...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    ll c, p; cin >> c >> p;
    A.emplace_back(p, p - c);
    if (p - c >= 0) npos++;
    else nneg++;
  }
  if (npos == 0) {
    ranges::sort(A, [&](pll x, pll y) -> bool { return x.second > y.second; });
    cout << A[0].second << "\n";
    return 0;
  }
  ranges::sort(A);
  ...
  ll val = 0;
  REP(i, 0, N) {
    while (j < N and A[j].first - A[i].first <= D) {
      if (A[j].second >= 0) val += A[j].second;
      j++;
    }
    DLOGK(i, j, val);
    ans = max(ans, val);
    if (A[i].second >= 0) val -= A[i].second;
  }
  cout << ans << "\n";
