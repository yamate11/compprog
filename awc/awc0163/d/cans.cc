...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, P, Q, M; cin >> N >> P >> Q >> M;
  // @InpMVec(N, (L, R)) [JByaUmFG]
  auto L = vector(N, ll());
  auto R = vector(N, ll());
  for (int i = 0; i < N; i++) {
    ll v1; cin >> v1; L[i] = v1;
  ...
  REP(_q, 0, M) {
    ll x, a, b; cin >> x >> a >> b; x--;
    L[x] = a;
    R[x] = b;
    vector tbl_init(121, vector(P + 1, vector(Q + 1, Fp(0))));
    auto tbl = tbl_init;
    tbl[60][0][0] = 1;
    REP(i, 0, N) {
      auto prev = tbl;
      REP(d, 0, 121) REP(p, 0, P + 1) REP(q, 0, Q + 1) {
        if (Fp pv = prev[d][p][q]; pv != Fp(0)) {
          REP(y, L[i], R[i] + 1) {
            DLOGKL("    ", i, d, p, q, y);
            if (p + 1 <= P) {
              tbl[d + y][p + 1][q] += pv;
              // DLOGK(d, y, p, q, tbl[d + y][p + 1][q]);
            }
            if (q + 1 <= Q) {
              tbl[d - y][p][q + 1] += pv;
              // DLOGK(d, y, p, q, tbl[d - y][p][q + 1]);
            }
          }
        }
      }
#if DEBUG
      REP(d, 0, 121) REP(p, 0, P + 1) REP(q, 0, Q + 1) if (tbl[d][p][q] != 0) DLOGK(i, d - 60, p, q, tbl[d][p][q]);
#endif
    }
    cout << tbl[60][P][Q] << "\n";
  }
