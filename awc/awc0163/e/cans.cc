...
// @@ !! LIM(power debug unordered_map f:intDiv)
...
int main(/* int argc, char *argv[] */) {
  ...
  safe_umap<pll, vector<pll>> mp;
  REP(i, 0, N + 1) {
    ll xx = divFloor(X[i], D);
    ll yy = divFloor(Y[i], D);
    mp[pll(xx, yy)].emplace_back(X[i], Y[i]);
  }
  ...
  while (not que.empty()) {
    ...
    ll xx = divFloor(x, D);
    ll yy = divFloor(y, D);
    REP(dxx, -1, 2) REP(dyy, -1, 2) {
      auto it = mp.find(pll(xx + dxx, yy + dyy));
      if (it == mp.end()) continue;
      vector<pll> tmp;
      for (auto [x0, y0] : it->second) {
        ll dx = x0 - x;
        ll dy = y0 - y;
        if (x == 0 and y == 0 and x0 == 0 and y0 == W) {
          tmp.emplace_back(x0, y0);
        }else if (dx * dx + dy * dy > D * D) {
          tmp.emplace_back(x0, y0);
        }else {
          que.emplace(x0, y0, d + 1);
        }
      }
      it->second = move(tmp);
    }
