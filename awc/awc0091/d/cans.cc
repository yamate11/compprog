...
// @@ !! LIM(f:intDiv intervalSet UnionFind)
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    ll a = A[i];
    ll d = D[i];
    if (a <= K) ev.emplace_back(0, DARKEN, i);
    else if (d == 0) ;
    else {
      ll t = divCeil(a - K, d);
      ev.emplace_back(t, DARKEN, i);
    }
  }
  vector<ll> ans(Q);
  REP(i, 0, Q) {
    ll t; cin >> t;
    ev.emplace_back(t, QUERY, i);
  }
  ranges::sort(ev);
  ll cval = 0;
  UnionFind uf(N);
  ...
  for (auto [t, kd, idx] : ev) {
    if (kd == DARKEN) {
      ...
      auto op = [&](ll j) -> void {
        ...
        if (len[a] == 0) return;
        ll b = uf.leader(idx);
        cval -= len[a] * (len[a] + 1) / 2;
        cval -= len[b] * (len[b] + 1) / 2;
        ll c = uf.merge(a, b);
        len[c] = len[a] + len[b];
        cval += len[c] * (len[c] + 1) / 2;
      };
      op(idx - 1);
      op(idx + 1);
    }else {
