...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> VV(N + 1);
  REP(i, 0, N) VV[i + 1] = VV[i] + V[i];
  ll big = 1LL << 60;
  auto mymax = [](ll x, ll y) -> ll { return max(x, y); };
  auto st = make_seg_tree_lazy(-big, 0LL, mymax, plus<ll>(), plus<ll>(), VV);
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll p; cin >> p; p--;
      ...
      ll b; cin >> b;
      if (d == 'L') b = -b;
      st.update(p + 1, N + 1, b - V[p]);
      V[p] = b;
    }else if (tp == 2) {
      ll K; cin >> K;
      DLOGK(st.vec_view(), K);
      auto check = [&](ll val) -> bool { return val >= K; };
      ll a = st.binsearch_r_from(check, 0);
      if (a == N + 2) a = 0;
      else a--;
      cout << a << "\n";
    }
