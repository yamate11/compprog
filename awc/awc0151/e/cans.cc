...
// @@ !! LIM(sieve cmpNaive forall debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  while (true) {
    ...
    DLOGK(k0);
    if (k0 >= 2 and not is_prime_MR(k0)) {
      DLOGKL("not prime", k0);
    }
    ll k = k0;
    while (true) {
      if (k == 1) k = 2;
        ...
        ll i0 = EXFIND_D(i, k + 1, N + 1, not done[i] and gcd(i, k) >= 2, N + 1);
        if (i0 <= N) k = i0;
        else break;
      }
      DLOGK(k, k0);
      ...
      if (k % k0 != 0) {
        DLOGKL("not divisible", k, k0);
      }
    }
    k0 = EXFIND_D(p, 1, N + 1, not done[p], N + 1);
    if (k0 == N + 1) break;
    cnt++;
  }
  cout << cnt << "\n";
...
int body(istream& cin, ostream& cout) {
  ...
  ll N; cin >> N;
  vector suc(N + 1, vector<ll>());
  REP(i, 2, N) for (ll j = i; j < N; j += i) suc[j].push_back(j + i);
  DLOGK(suc);
  vector<ll> eff(N + 1, true);
  ll cnt = 1;
  eff[1] = false;
  for (ll i = 2; i <= N; i += 2) eff[i] = false;
  ll i0 = 3;
  for (; true; cnt++) {
    for (; i0 <= N and not eff[i0]; i0++);
    if (i0 > N) break;
    DLOGK(i0);
    for (ll k = i0; true; ) {
      auto sch = [&]() -> ll {
        for (ll m : suc[k]) if (eff[m]) return m;
        return -1;
      };
      k = sch();
      if (k < 0) break;
