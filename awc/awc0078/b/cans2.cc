...
int main(/* int argc, char *argv[] */) {
  ...
  CartesianTree ct(A);
  auto dfs = [&](auto rF, ll nd, ll ht) -> ll {
    ll ret = A[nd] - ht;
    if (ct.left[nd] >= 0) ret += rF(rF, ct.left[nd], A[nd]);
    if (ct.right[nd] >= 0) ret += rF(rF, ct.right[nd], A[nd]);
    return ret;
  };
  ll ans = dfs(dfs, ct.root, 0);
  cout << ans << endl;
