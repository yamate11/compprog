...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> EE(N + 1);
  REP(i, 0, N) EE[i + 1] = DD[i] + EE[i];
  auto i0 = ranges::min_element(EE) - EE.begin();
  if (i0 == N) i0 = 0;
  vector<ll> D(N);
  REP(i, 0, N) D[i] = DD[(i0 + i) % N];
  vector<ll> E(N + 1);
  REP(i, 0, N) E[i + 1] = E[i] + D[i];
  DLOGK(i0, D, E);
  ll big = 1LL << 60;
  auto mymin = [&](ll a, ll b) -> ll { return min(a, b); };
  auto st = make_seg_tree(big, mymin, E);
  REP(_q, 0, Q) {
    auto loc_solve = [&]() -> ll {
      ll l, s; cin >> l >> s; l--;
      l = imod(l - i0, N);
      DLOGK(l);
      auto check1 = [&](ll val) -> bool { return E[l] - val > s; };
      ll r = st.binsearch_r_from(check1, l);
      DLOGK(r);
      if (r <= N + 1) return r - l - 1;
      if (E[N] >= 0) return 0;
      ll ans = N - l;
      s += E[N] - E[l];
      assert(s >= 0);
      ...
      ans += y * N;
      s = s % (-E[N]);
      auto check2 = [&](ll val) -> bool { return -val > s; };
      ll r1 = st.binsearch_r_from(check2, 0);
      ans += r1 - 1;
      return ans;
    };
    cout << loc_solve() << "\n";
