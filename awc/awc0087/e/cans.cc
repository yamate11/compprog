...
// @DefStruct(sta, (tot, mT, mL, mR)) [gcKLhaPQ]
struct sta {
  ll tot;
  ll mT;
  ll mL;
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> B(N + 1);
  REP(i, 0, N) B[i + 1] = B[i] + abs(A[i]);
  
  ll big = 1LL << 60;
  sta unit_sta(-big, -big, -big, -big);
  auto add = [&](const sta& x, const sta& y) -> sta {
    if (x == unit_sta) return y;
    if (y == unit_sta) return x;
    sta ret;
    ret.tot = x.tot + y.tot;
    ret.mT = max(x.mT, max(y.mT, x.mR + y.mL));
    ret.mL = max(x.mL, x.tot + y.mL);
    ret.mR = max(x.mR + y.tot, y.mR);
    return ret;
  };
  vector<sta> init_vec(N);
  REP(i, 0, N) init_vec[i] = sta(A[i], A[i], A[i], A[i]);
  auto st = make_seg_tree(unit_sta, add, init_vec);
  REP(_q, 0, Q) {
    ll l, r; cin >> l >> r; l--;
    sta p = st.query(l, r);
    DLOGK(p);
    if (p.mT >= 0)  {
      cout << (B[r] - B[l] + p.tot - 2 * p.mT) / 2 << "\n";
    }else {
      cout << (B[r] - B[l] + p.tot) / 2 << "\n";
    }
