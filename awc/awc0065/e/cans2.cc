...
int main(/* int argc, char *argv[] */) {
  ...
  pll unit_dat{-big, big};
  auto add = [&](pll a, pll b) -> pll { return pll(max(a.first, b.first), min(a.second, b.second)); };
  vector<pll> init_dat(N);
  REP(i, 0, N) init_dat[i] = pll(H[i], H[i]);
  auto st = make_seg_tree(unit_dat, add, init_dat);
#if DEBUG
  REP(i, 1, N + 1) DLOGK(i, st.query(0, i));
#endif
  ll ans = 0;
  REP(i, 0, N) {
    auto check = [&](pll a) { return a.first - a.second <= D; };
    ll y = st.binsearch_r_until(check, i);
    DLOGK(i, y);
    ans = max(ans, y - i);
  }
