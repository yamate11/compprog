...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    auto get_max_j = [&]() -> ll {
      while (true) {
        if (H[maxD.front()] - H[minD.front()] > D) return j - 1;
        if (j == N - 1) return N - 1;
        j++;
        while (not maxD.empty() and H[maxD.back()] <= H[j]) maxD.pop_back();
        maxD.push_back(j);
        while (not minD.empty() and H[minD.back()] >= H[j]) minD.pop_back();
        minD.push_back(j);
      }
    };
    ll val = get_max_j() - i + 1;
    ans = max(ans, val);
