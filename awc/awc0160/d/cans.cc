...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1LL << 60;
  auto unit1 = pll(-big, -1);
  auto add1 = [&](pll a, pll b) -> pll {
    if (a.first < b.first) return b;
    else if (a.first > b.first) return a;
    else if (a.second < b.second) return a;
    return b;
  };
  vector init1(N, unit1);
  REP(i, 0, N) init1[i] = pll(A[i], i);
  auto unit2 = pll(big, -1);
  auto add2 = [&](pll a, pll b) -> pll {
    if (a.first > b.first) return b;
    else if (a.first < b.first) return a;
    else if (a.second < b.second) return a;
    return b;
  };
  vector init2(N, unit1);
  REP(i, 0, N) init2[i] = pll(A[i], i);

  auto segt1 = make_seg_tree(unit1, add1, init1);
  auto segt2 = make_seg_tree(unit2, add2, init2);
  REP(_q, 0, Q) {
    ll t, x, r; cin >> t >> x >> r;
    if (r == 0) {
      ll i, vv;
      if (t == 1) {
        tie(vv, i) = segt1.query(0, N);
      }else {
        tie(vv, i) = segt2.query(0, N);
      }
      segt1.rs(i) = pll(x, i);
      segt2.rs(i) = pll(x, i);
    }
  ...
  ll ans = 0;
  REP(i, 0, N) ans += segt1.at(i).first;
  cout << ans << "\n";
