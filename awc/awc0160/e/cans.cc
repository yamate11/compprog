...
// @@ !! LIM(cartesianTree unordered_map debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpMVec(N, ((C, dec=1), (H, dec=0))) [WMpnnhbw]
  auto C = vector(N, ll());
  auto H = vector(N, ll());
  for (int i = 0; i < N; i++) {
    ll v1; cin >> v1; v1 -= 1; C[i] = v1;
    ll v2; cin >> v2; v2 -= 0; H[i] = v2;
  }
  // @End [WMpnnhbw]
  vector E(N, vector<ll>());
  REP(i, 0, N) E[C[i]].push_back(i);
  ...
  ll ans = 0;
  using mp_tp = safe_umap<ll, ll>;
  auto dfs = [&](auto rF, ll nd, ll lo, ll hi, mp_tp&& exc) -> void {
    ll c = C[nd];
    auto [it, b] = exc.try_emplace(c, 0);
    ll num = ranges::lower_bound(E[c], hi) - ranges::lower_bound(E[c], lo + 1) - 1 - it->second;
    DLOGK(c, E[c], hi, lo, it->second);
    ans += num;
    if (ct.left[nd] >= 0) rF(rF, ct.left[nd], lo, nd, mp_tp());
    if (ll r = ct.right[nd]; r >= 0) {
      if (H[nd] == H[r]) {
        it->second++;
        rF(rF, r, nd, hi, move(exc));
      }else {
        rF(rF, r, nd, hi, mp_tp());
      }
    }
  };
  dfs(dfs, ct.root, -100, N + 100, mp_tp());
  cout << ans << "\n";
