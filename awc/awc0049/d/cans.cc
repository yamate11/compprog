...
int naive(istream& cin, ostream& cout) {
  ...
  vector<ll> P;
  REP(i, 0, N) REP(j, 0, M) P.emplace_back(A[i] * B[j]);
...
int body(istream& cin, ostream& cout) {
  ...
  set<pll> ss;
  pque.emplace(A[0] * B[0], 0, 0);
  ss.emplace(0, 0);
  ll ans = 0;
  while (true) {
    auto [a, i, j] = pque.top(); pque.pop();
    DLOGKL("popped", a, i, j);
    ans += a;
    if (--K == 0) break;
    auto op = [&](ll x, ll y) -> void {
      if (x < N and y < M and not ss.contains(pll(x, y))) {
        ss.emplace(x, y);
        DLOGKL("pushed", x, y, A[x] * B[y]);
        pque.emplace(A[x] * B[y], x, y);
      }
    };
    op(i + 1, j);
    op(i, j + 1);
