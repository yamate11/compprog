...
// published at https://github.com/yamate11/compprog-clib/blob/master/mo.cc
...
int main(/* int argc, char *argv[] */) {
  ...
  struct sta {
    ll base;
    vector<ll> vec;
    sta(ll base_, ll size) : base(base_), vec(size) {}
  };
  vector<ll> ans(Q);
  sta left(N + 2, 2*N + 4);
  sta right(N + 2, 2*N + 4);
  ...
  auto rep = [&](string lab, ll x) -> void {
#if DEBUG    
    DLOGK(lab, x, mo.cl, mo.cr);
    DLOGK(left.base, left.vec);
    DLOGK(right.base, right.vec);
    DLOGK(sat, full);
#endif
  };
  auto modsat = [&](const auto& ss, ll sign) -> void {
    sat += sign * (ss.vec[ss.base - 1] + ss.vec[ss.base] + ss.vec[ss.base + 1]);
  };
  auto in_left = [&](ll x) -> void {
    if (mo.cr - mo.cl != 1) {
      right.vec[right.base + full]++;
      if (-1 <= full and full <= 1) sat++;
    }
    // modsat(left, -1);
    if (S[x] == 'W') {
      left.base--;
      if (mo.cr - mo.cl != 1) left.vec[left.base + 1]++;
      full++;
    }else {
      left.base++;
      if (mo.cr - mo.cl != 1) left.vec[left.base - 1]++;
  ...
  auto in_right = [&](ll x) -> void {
    if (mo.cr - mo.cl != 1) {
      left.vec[left.base + full]++;
      if (-1 <= full and full <= 1) sat++;
    }
    // modsat(right, -1);
    if (S[x] == 'W') {
      right.base--;
      if (mo.cr - mo.cl != 1) right.vec[right.base + 1]++;
      full++;
    }else {
      right.base++;
      if (mo.cr - mo.cl != 1) right.vec[right.base - 1]++;
