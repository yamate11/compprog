...
int main(/* int argc, char *argv[] */) {
  ...
  auto comp = [&](pll x, pll y) -> bool { return x.first * y.second < y.first * x.second; };
  ...
  auto plus = [&](pll x, pll y) -> pll {
    auto [x1, x2] = x;
    auto [y1, y2] = y;
    ll t = lcm(x2, y2);
    ll s = x1 * (t / x2) + y1 * (t / y2);
    ll g = gcd(s, t);
    return pll(s / g, t / g);
  };
  ...
  auto f = [&](ll i) -> pll {
    ...
    REP(j, 0, i) {
      pll x = pll(H[j], i - j);
      if (comp(left, x)) left = x;
    }
    pll right(0, 1);
    REPrev(j, N - 1, i + 1) {
      pll x = pll(H[j], j - i);
      DLOGKL("  ", i, j, x, right);
      if (comp(right, x)) right = x;
      DLOGKL("      ", right);
    }
    pll val = plus(left, right);
    val = plus(val, pll(1, 1));
    DLOGK(i, left, right, val);
    return val;
  };
