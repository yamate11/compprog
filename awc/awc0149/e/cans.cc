...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, K; cin >> N >> M >> K;
  // @InpNbrList(N, M, nbr, dec=1, read=W) [vzI3E2Lm]
  struct nbr_t {
    int nd{};
    ll W{};
    string show() const {
  ...
  ll big = 1LL << 60;
  auto distG = vector(K + 2, vector(K + 2, big));
  auto f = [&](ll i0) -> void {
    vector dist(N, big);
    ...
    REP(k, 0, K + 2) distG[i0][k] = dist[C[k]];
    DLOGK(i0, dist, distG[i0]);
  };
  ...
  tbl[1][0] = 0;
  REP(x, 0, 1LL << (K + 2)) REP(k, 0, K + 2) {
    if (tbl[x][k] == big) continue;
    REP(m, 0, K + 2) {
      if (not (x >> m & 1)) {
        updMin(tbl[x | (1LL << m)][m], tbl[x][k] + distG[k][m]);
      }
    }
  }
  cout << tbl[(1LL << (K + 2)) - 1][K + 1] << "\n";
