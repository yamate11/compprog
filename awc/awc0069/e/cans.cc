...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpMVec(Q, ((L, dec=1), (R, dec=0), T)) [1n4G4Xgq]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  auto T = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  vector<ev_t> ev;
  REP(i, 0, N) ev.emplace_back(D[i], 0, i);
  REP(i, 0, Q) ev.emplace_back(T[i], 1, i);
  ranges::sort(ev);
  auto st = make_seg_tree(0LL, plus<ll>(), vector<ll>(N));
  vector<ll> ans(Q);
  for (auto [t, tp, i] : ev) {
    if (tp == 0) st.rs(i) = A[i] * V[i];
    else ans[i] = st.query(L[i], R[i]);
  }
  REPOUT(i, 0, Q, ans[i], "\n");
