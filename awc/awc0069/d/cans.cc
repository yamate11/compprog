...
// @@ !! LIM(coordCompr segTree f:updMaxMin cmpNaive forall debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  REP(x, 1, 1LL << N) {
    auto check = [&]() -> bool {
      if (EXISTS(i, 0, N, (x >> i & 1) and P[i] < K)) return false;
      REP(i, 0, N) {
        if (FORALL(j, 0, N, IMPLIES(x >> j & 1, abs(X[i] - X[j]) > D))) return false;
      }
      return true;
    };
    if (check()) updMin(ans, (ll)popcount((u64)x));
  }
  if (ans > N) ans = -1;
  cout << ans << "\n";
  return 0;
...
int body(istream& cin, ostream& cout) {
  ...
  vector<pll> Hi;
  REP(i, 0, N) if (XP[i].second >= K) Hi.emplace_back(XP[i].first, i);
  REP(i, 0, N) {
    if (XP[i].second >= K) continue;
    ll i0 = ranges::lower_bound(Hi, pll(XP[i].first, 0)) - Hi.begin();
    ll i1 = i0 - 1;
    bool b0 = i0 == ssize(Hi) or Hi[i0].first - XP[i].first > D;
    bool b1 = i1 < 0 or XP[i].first - Hi[i1].first > D;
    if (b0 and b1) {
      DLOGK(i, b0, b1, i0, i1);
      cout << -1 << endl;
      return 0;
  ...
  REP(i, 0, N) {
    ll x = XP[i].first;
    ll i0 = ranges::lower_bound(XP, pll(x - D, 0)) - XP.begin();
    if (i0 == 0) crit[i] = -1;
    else {
      auto it = ranges::lower_bound(Hi, pll(XP[i0 - 1].first - D, 0));
      crit[i] = it->second;
    }
  }
  ...
  vector<ll> init_dat(N, big);
  auto st = make_seg_tree(big, mymin, init_dat);

  for (auto [x, i] : Hi) {
    ll t0;
    if (crit[i] < 0) t0 = 1;
    else {
      t0 = 1 + st.query(crit[i], i);
    }
    st.rs(i) = t0;
    DLOGK(x, i, t0);
  }
  ll i0 = ranges::lower_bound(Hi, pll(XP[N - 1].first - D, 0)) - Hi.begin();
  ll ans = st.query(Hi[i0].second, N);
  if (ans == big) ans = -1;
  cout << ans << endl;
