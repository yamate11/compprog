...
struct Tree {
  ...
  */
  ...
  struct children_view {
    ...
    /*
    nbr_iterator<get_peer, excl_parent> begin() const {
      return nbr_iterator<get_peer, excl_parent>(body, 0);
    }
    nbr_iterator<get_peer, excl_parent> end() const {
      return nbr_iterator<get_peer, excl_parent>(body, std::ssize(body.pe) - (excl_parent ? 1 : 0));
    }
    */
    auto begin() const { return nbr_iterator<get_peer, excl_parent>(body, 0); }
    auto end()   const { return nbr_iterator<get_peer, excl_parent>(body, ssize(body) - (excl_parent ? 1 : 0)); }
  };
  ...
  auto children_pe(ll nd) {
    mature_check();
    return children_view<false, true>(_nbr[nd]);
  }
  auto children(ll nd) {
    mature_check();
    return children_view<true, true>(_nbr[nd]);
  }
  auto neighbor_pe(ll nd) {
    mature_check();
    return children_view<false, false>(_nbr[nd]);
  }
  auto neighbor(ll nd) {
    mature_check();
    return children_view<true, false>(_nbr[nd]);
  }

  ll stsize(ll nd) {
    static_assert(use_stsize, "use_stsize should be set.");
    mature_check();
    return _stsize[nd];
  }

  ll depth(ll nd) {
    static_assert(use_depth, "use_depth should be set.");
    mature_check();
    return _depth[nd];
  }
  ...
  ll euler_in(ll nd) {
    static_assert(use_euler, "use_euler should be set.");
    return _euler_in[nd];
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  UnionFind uf(N);
  Tree tr(N);
  vector tr_e_w(N - 1, 0LL);
  vector nbr1(N, vector<pll>());
  ll w_t = 0;
  for (auto [w, u, v] : WUV) {
    if (uf.leader(u) != uf.leader(v)) {
      ...
      nbr1[u].emplace_back(v, w);
      nbr1[v].emplace_back(u, w);
    }
  }
  ll smax = 0;
  using pq_t = priority_queue<pll, vector<pll>, greater<pll>>;
  vector<bool> done(N, false);
  auto dfs = [&](auto rF, ll nd) -> pq_t {
    pq_t pque;
    for (auto [peer, w] : nbr1[nd]) {
      pque.emplace(w, tr.lca(nd, peer));
    }
    DLOGKL("initpq", nd, pque);
    ll acc_sts = 1;
    for (ll c : tr.children(nd)) {
      auto pque_c = rF(rF, c);
      if (acc_sts < tr.stsize(c)) swap(pque, pque_c);
      acc_sts += tr.stsize(c);
      while (not pque_c.empty()) {
        auto [w, u] = pque_c.top(); pque_c.pop();
        if (not done[u] and u != nd) pque.emplace(w, u);
      }
      DLOGKL("merged", nd, c, pque);
    }
    if (nd != 0) {
      while (not pque.empty()) {
        auto [w, u] = pque.top();
        if (not done[u] and u != nd) {
          smax = max(smax, w - tr_e_w[tr.edge_idx(nd, tr.parent(nd))]);
          break;
        }else {
          pque.pop();
        }
