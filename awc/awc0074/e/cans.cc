...
// @@ !! LIM(tree segTree sortedIndex UnionFind debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((U, dec=1), (V, dec=1), W)) [H98wmPhU]
  auto U = vector(M, ll());
  auto V = vector(M, ll());
  auto W = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  ll trw = 0;
  vector<ll> oe;
  vector<ll> orig_idx_edge(N - 1);
  REP(ii, 0, M) {
    ll i = ord[ii];
    if (uf.leader(U[i]) != uf.leader(V[i])) {
      uf.merge(U[i], V[i]);
      ll j = tr.add_edge(U[i], V[i]);
      orig_idx_edge[j] = i;
      trw += W[i];
    }else {
      oe.push_back(i);
  ...
  vector<ll> init_dat(2 * N, big);
  auto st = make_seg_tree_lazy(big, big, mymin, mymin, mymin, init_dat);
  for (ll i : oe) {
    for (auto [l, r] : tr.hl_path(U[i], V[i])) st.update(l, r, W[i]);
  }
  ll ans = 0;
  REP(nd, 1, N) {
    ll et_idx = tr.euler_in(nd);
    ll e = tr.euler_elem_edge(et_idx);
    ll cost = trw + st.at(et_idx) - W[orig_idx_edge[e]];
    ans = max(ans, cost);
  }
  cout << ans << endl;
