...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  vector vec = du.to_vector_big_endian(N);
  vector tbl_init(2, vector(2, vector(M, 0LL)));
  auto tbl = tbl_init;
  if (M == 1) tbl[1][1][0] = 1;
  else tbl[1][1][1] = 1;
  for (ll t : vec) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(eq, 0, 2) REP(az, 0, 2) REP(v, 0, M) {
      if (prev[eq][az][v] == 0) continue;
      REP(d, 1, 10) {
        if (eq and d > t) continue;
        int new_eq = eq and d == t;
        int new_az = 0;
        int new_v = (v * d) % M;
        tbl[new_eq][new_az][new_v] += prev[eq][az][v];
      }
      if (M == 1) tbl[0][1][0] = 1;
      else        tbl[0][1][1] = 1;
      DLOGK(t, eq, v, prev[eq][0][v], tbl[0][0]);
      DLOGK(t, eq, v, prev[eq][0][v], tbl[1][0]);
    }
  }
  ll ans = 0;
  REP(eq, 0, 2) {
