...
int main(/* int argc, char *argv[] */) {
  ...
  while (not pque.empty()) {
    auto [d, bi] = pque.top(); pque.pop();
    if (dist[bi.r][bi.c] == d) {
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        ll len = big;
        if (brd.at(bj) == 'X') len = big;
        else if (brd.at(bi) == 'V' and brd.at(bj) == 'V') len = 0;
        else if (brd.at(bj) == 'V') len = 2;
        else len = 1;
        ll new_d = dist[bi.r][bi.c] + len;
        if (new_d < big and new_d < dist[bj.r][bj.c]) {
          dist[bj.r][bj.c] = new_d;
          pque.emplace(new_d, bj);
        }
      }
    }
  }
