...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, K; cin >> N >> M >> K;
  // @InpVec(N, S, type=string) [6gPJxxWi]
  auto S = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; S[i] = v; }
  // @End [6gPJxxWi]

  vector A(N + 1, vector(M + 1, 0LL));
  REP(i, 0, N) REP(j, 0, M) A[i + 1][j + 1] = S[i][j] - '0';
  REP(i, 0, N) REP(j, 0, M) A[i + 1][j + 1] += A[i][j + 1];
  REP(i, 0, N) REP(j, 0, M) A[i + 1][j + 1] += A[i + 1][j];
  
  ll ans = 0;
  bool found = false;
  for (ll d1 : getDivisors(K)) {
    ll d2 = K / d1;
    if (d1 <= N and d2 <= M) {
      found = true;
      REP(i, 0, N - d1 + 1) REP(j, 0, M - d2 + 1) {
        ll t = A[i][j] + A[i + d1][j + d2] - A[i + d1][j] - A[i][j + d2];
        ans = max(ans, t);
      }
    }
  }
