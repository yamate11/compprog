...
// @DefStruct(DAT, (left, right, blockN)) [vwuEbYdd]
struct DAT {
  ll left;
  ll right;
  ll blockN;
  ostream& operator<<(ostream& ostr) const {
    return ostr << "(" << left << ", " << right << ", " << blockN << ")";
  }
  auto operator<=>(const DAT&) const = default;
};
// @End [vwuEbYdd]
...
int main(/* int argc, char *argv[] */) {
  ...
  auto add = [&](const DAT& a, const DAT& b) -> DAT {
    if (unit_DAT == a) return b;
    if (unit_DAT == b) return a;
    ll x = a.blockN + b.blockN;
    if (a.right == b.left) x--;
    return DAT(a.left, b.right, x);
  };
  using OP = optional<ll>;
  auto comp = [](OP f, OP g) -> OP { return f ? f : g; };
  auto appl = [](OP f, const DAT& a) -> DAT { return f ? DAT{*f, *f, 1} : a; };
  vector<DAT> init_dat(N);
  REP(i, 0, N) init_dat[i] = DAT{C[i], C[i], 1};
  auto st = make_seg_tree_lazy(unit_DAT, OP{}, add, comp, appl, init_dat);
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      ll l, r; cin >> l >> r; l--;
      auto [_left, _right, blockN] = st.query(l, r);
      cout << blockN << "\n";
    }else assert(0);
  }
