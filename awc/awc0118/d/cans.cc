...
template<typename T>
struct sd_vector {   // square-root divided vector
  int bsize;
  deque<deque<T>> body;
  sd_vector(int bsz = 500) : bsize(max(4, bsz)) {}
  pair<int, int> get_pos(int idx) const {
    int p = 0;
    int q = idx;
    for ( ; p < ssize(body) and ssize(body[p]) <= q; p++) q -= ssize(body[p]);
    return {p, q};
  }
  void _insert_pq(int p, int q, T t) {
    if (body.empty()) body.push_back(deque<T>{});
    if (q == 0 and p > 0) { p--; q = ssize(body[p]); }
    body[p].insert(body[p].begin() + q, move(t));
    if (ssize(body[p]) > bsize) {
      int old_size = ssize(body[p]);
      int new_size = bsize / 2;
      body.insert(body.begin() + (p + 1), deque<T>{});
      for (int i = new_size; i < old_size; i++) body[p + 1].push_back(body[p][i]);
      body[p].resize(new_size);
    }
  }
  void insert(int idx, T t) {
    auto [p, q] = get_pos(idx);
    _insert_pq(p, q, move(t));
  }
  void push_back(T t) { _insert_pq(ssize(body), 0, move(t)); }
  void _erase_pq(int p, int q) {
    body[p].erase(body[p].begin() + q);
    if (body[p].empty()) body.erase(body.begin() + p);
  }
  void erase(int idx) {
    auto [p, q] = get_pos(idx);
    _erase_pq(p, q);
  }
  void pop_back() { _erase_pq(ssize(body) - 1, ssize(body.back()) - 1); }
...
int main(/* int argc, char *argv[] */) {
  ...
  ll Q; cin >> Q;
  ll bsize = llround(4 + sqrt(Q));
  sd_vector<ll> vec(bsize);
  REP(q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll x; cin >> x;
