...
int main(/* int argc, char *argv[] */) {
  ...
  string S; cin >> S;
  ll i0 = 0, j0 = N;
  for ( ; i0 + 7 <= N and S.substr(i0, 7) == "ATCODER"; i0 += 7);
  if (i0 == N) {
    ll a = i0 / 7;
    if (M >= a - 1) {
      cout << a << endl;
  ...
  for ( ; i0 <= j0 - 7 and S.substr(j0 - 7, 7) == "ATCODER"; j0 -= 7);
  ll a = i0 / 7 + (N - j0) / 7;
  if (a >= M) {
    cout << M << endl;
    return 0;
  ...
  DLOGK(i0, j0);
  while (true) {
    for ( ; i + 7 <= j0 and S.substr(i, 7) != "ATCODER"; i++);
    DLOGK(i);
    if (i + 7 > j0) break;
    ll j = i + 7;
    DLOGK(j);
    for ( ; j + 7 <= j0 and S.substr(j, 7) == "ATCODER"; j += 7);
    DLOGK(j);
    P.push_back((j - i) / 7);
    i = j;
  }
