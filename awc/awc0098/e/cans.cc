...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll nd) -> pll {
    ll numc = tr.num_children(nd);
    ll base = 0;
    vector<ll> ca(numc);
    REP(i, 0, numc) {
      auto [b, a] = rF(rF, tr.child(nd, i));
      base += b;
      ca[i] = a;
    }
    ranges::sort(ca, greater<ll>());
    ll sum1 = 0;
    REP(i, C[nd], numc) sum1 += min(ca[i], W[nd]);
    ll sum2 = 0;
    if (nd == 0) {
      sum2 = sum1;
    }else if (C[nd] == 0) {
      sum2 = W[nd];
      REP(i, 0, numc) sum2 += min(ca[i], W[nd]);
    }else {
      sum2 = 0;
      REP(i, C[nd] - 1, numc) sum2 += min(ca[i], W[nd]);
    }
    return pll{base + sum1, sum2 - sum1};
  };
  auto [cost_base, cost_add] = dfs(dfs, 0);
  cout << N - 1 + cost_base << endl;
