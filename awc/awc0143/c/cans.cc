...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [M22PblB3]
  // @InpNbrList(N, M, fwd, dir=True, dec=1) [7vpKYJgR]
  ...
  auto dfs = [&](auto rF, ll nd) -> void {
    ...
    visited[nd] = true;
    for (ll peer : fwd[nd]) {
      DLOGKL("  checking", peer, nd);
      if (not visited[peer] and C[peer] != C[nd]) rF(rF, peer);
    }
  };
  dfs(dfs, 0);
  vector<ll> ans;
  REP(i, 0, N) if (not visited[i]) ans.push_back(i);
  if (ans.empty()) {
    cout << "COMPLETE\n";
  }else {
