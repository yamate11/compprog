...
int main(/* int argc, char *argv[] */) {
  ...
  REP(_q, 0, Q) {
    ll K; cin >> K;
    // @InpVec(K, V, dec=1) [5tFOT2zM]
    auto V = vector(K, ll());
    for (int i = 0; i < K; i++) { ll v; cin >> v; v -= 1; V[i] = v; }
    // @End [5tFOT2zM]
    if (K == 1) {
      cout << "0 -1\n";
    }else {
      ll td = 0;
      for (ll v : V) td += tr.depth(v);
      vector<ll> vlcaL(K);
      vector<ll> vlcaR(K);
      vlcaL[0] = V[0];
      REP(i, 1, K) {
        vlcaL[i] = tr.lca(vlcaL[i - 1], V[i]);
      }
      vlcaR[K - 1] = V[K - 1];
      REPrev(i, K - 2, 0) {
        vlcaR[i] = tr.lca(vlcaR[i + 1], V[i]);
      }
      ll dep00 = tr.depth(vlcaL[K - 1]);
      ll val1 = td - dep00 * K;
      ll val2 = 1LL << 60;
      if (K == 2) val2 = 0;
      else {
        ...
        REP(k, 0, K) {
          if (k == 0) clca = vlcaR[1];
          else if (k == K - 1) clca = vlcaL[K - 2];
          else clca = tr.lca(vlcaL[k - 1], vlcaR[k + 1]);
          ll cur = td - tr.depth(V[k]) - tr.depth(clca) * (K - 1);
          val2 = min(val2, cur);
        }
      }
      cout << val1 << " " << val2 << "\n";
