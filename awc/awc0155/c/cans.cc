...
// @@ !! LIM(binsearch unordered_map debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](const auto& A, const auto& B) -> bool {
    ll N = ssize(A);
    ll M = ssize(B);
    ll i = 0;
    REP(j, 0, M) {
  ...
  auto check = [&](ll m) -> bool {
    ...
    REP(j, 0, M) {
      ...
      if (A[i] == B[j]) {
        k++;
        DLOGKL("   =", k, i, j, A[i], B[j]);
        if (k == m) {
          REP(ii, i + 1, N) AA.push_back(A[ii]);
          REP(jj, j + 1, M) BB.push_back(B[jj]);
          bool b = f(AA, BB);
          DLOGKL("call f", AA, BB, b);
          return b;
        }
        i++;
      }
