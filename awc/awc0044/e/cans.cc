...
int naive(istream& cin, ostream& cout) {
  ...
  while (idp.get()) {
    ...
    REP(i, 0, N) {
      ll x = idp[i];
      if (app[x] < 0) app[x] = seq++;
      vec[i] = app[x];
    }
    if (FORALL(i, 0, N, D[i] < 0 or D[i] == vec[i])) cnt++;
  }
  cout << cnt << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 1, N) {
    ...
    if (D[i] == -1) {
      tbl[0] = prev[0];
      REP(j, 1, H) tbl[j] = (M - j) * prev[j - 1] + (j + 1) * prev[j];
    }else {
      REP(j, D[i], H) tbl[j] = prev[j];
      if (D[i] > 0 and D[i] < H) tbl[D[i]] += (M - D[i]) * prev[D[i] - 1];
    }
    DLOGK(i, tbl);
  }
  Fp ans = 0;
