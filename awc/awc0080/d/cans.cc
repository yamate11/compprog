...
// @@ !! LIM(UnionFind sortedIndex forall)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, K; cin >> N >> M >> K;
  // @InpMVec(M, ((U, dec=1), (V, dec=1), W)) [qxWIvnT1]
  auto U = vector(M, ll());
  auto V = vector(M, ll());
  auto W = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  // @End [qxWIvnT1]
  // @InpVec(K, E, dec=1) [yMGYiboO]
  auto E = vector(K, ll());
  for (int i = 0; i < K; i++) { ll v; cin >> v; v -= 1; E[i] = v; }
  // @End [yMGYiboO]

  vector<bool> EE(M);
  REP(i, 0, K) EE[E[i]] = true;

  ll totW = 0;
  UnionFind uf(N);
  REP(i, 0, K) {
    if (uf.leader(U[E[i]]) == uf.leader(V[E[i]])) {
      cout << -1 << endl;
      return 0;
    }
    uf.merge(U[E[i]], V[E[i]]);
    totW += W[E[i]];
  }
  ...
  if (FORALL(i, 0, N, uf.leader(i) == uf.leader(0))) {
    cout << totW << endl;
  }else {
    cout << -1 << endl;
  }
