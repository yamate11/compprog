...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  // @InpGrid(H, W, A) [X9Cgt1nu]
  auto A = vector(H, vector(W, ll()));
  for (int i = 0; i < H; i++) for (int j = 0; j < W; j++) { ll v; cin >> v; A[i][j] = v; }
  // @End [X9Cgt1nu]

  auto B = vector(H, vector(W + 1, 0LL));
  REP(r, 0, H) {
    REP(c, 0, W) B[r][c + 1] = B[r][c] + A[r][c];
  }

  UnionFind uf0(W + 1);
  vector uff(H, uf0);
  auto C = vector(H, vector(W + 1, 0LL));
  REP(r, 0, H) REP(c, 0, W + 1) C[r][c] = c;

  ll Q; cin >> Q;
  REP(_q, 0, Q) {
    ll ans = 0;
    ll rQ, cQ, dQ; cin >> rQ >> cQ >> dQ; rQ--; cQ--;
    ll r0 = max(0LL, rQ - dQ);
    ll r1 = min(H - 1, rQ + dQ);
    REP(r, r0, r1 + 1) {
      ll c0 = max(0LL,   cQ - (dQ - abs(r - rQ)));
      ll c1 = min(W - 1, cQ + (dQ - abs(r - rQ)));
      ll c = c0;
      while (c < c1 + 1) {
        c = C[r][uff[r].leader(c)];
        if (c >= c1 + 1) break;
        ans += A[r][c];
        if (c + 1 <= W) {
          ll a = uff[r].leader(c);
          ll b = uff[r].leader(c + 1);
          ll x = uff[r].merge(a, b);
          C[r][x] = C[r][b];
          // DLOGK(c, a, b, x, C[r][x], C[r][b]);
        }
        // DLOGK(r, c, c + 1, uff[r].leader(c), uff[r].leader(c + 1), C);
        c++;
      }
    }
    cout << ans << "\n";
