...
int body(istream& cin, ostream& cout) {
  ...
  string S; cin >> S;
  S = S + S;
  auto root = new Trie<26, 'a', int, string, true, false>();
  REP(i, 0, N) {
    auto p = root->insert(S.substr(i, N));
    for (; p != root; p = p->parent) p->user++;
  }
  vector tbl(N, vector(N, 0LL));
  REP(i, 0, N) {
    auto nd = root;
    ll acc = 0;
    ll acc2 = 0;
    REP(j, 0, N) {
      ...
      ll cur = 0;
      REP(d, 0, d0) {
        auto z = nd->get_child_offset(d);
        if (z) cur += z->user;
      }
      acc += cur;
      acc2 += acc;
      tbl[i][j] = acc2;
      nd = nd->get_child_offset(d0);
      assert(nd);
    }
  }
