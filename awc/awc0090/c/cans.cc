...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&]() -> bool {
    ...
    // @End [4xLfWKKO]
    // @InpMVec(M, ((U, dec=1), (V, dec=1), B)) [WF4DwhCD]
    auto U = vector(M, ll());
    auto V = vector(M, ll());
    auto B = vector(M, ll());
    for (int i = 0; i < M; i++) {
    ...
    /*
    REP(i, 0, N) {
      if (child[i][0] < 0 and child[i][1] >= 0) {
        DLOGKL("child01", i);
        return false;
      }
    }
    */
    ...
    REP(i, 0, M) {
      ...
      bwd[V[i]].push_back(U[i]);
    }
    if (not bwd[0].empty()) {
      DLOG("zero not empty");
      return false;
    }
    if (EXISTS(i, 1, N, ssize(bwd[i]) != 1)) {
      DLOG("ssize not one");
      return false;
    }
    ...
    while (not que.empty()) {
      ...
      ll x = child[nd][0];
      if (x >= 0 and not rble[x]) {
        rble[x] = true;
        que.push(x);
      }
      ll y = child[nd][1];
      if (y >= 0 and not rble[y]) {
        rble[y] = true;
        que.push(y);
      }
    }
    if (EXISTS(i, 0, N, not rble[i])) {
      DLOG("not reachable");
      return false;
    }

    REP(i, 0, M) {
      if (not (L[U[i]] < L[V[i]] and R[V[i]] < R[U[i]])) {
        DLOGKL("LR", i);
        return false;
      }
      if (P[V[i]] != Q[U[i]]) {
    ...
    REP(i, 0, N) {
      ll x = child[i][0];
      ll y = child[i][1];
      if (x >= 0 and y >= 0 and not (R[x] < L[y])) {
        DLOGKL("RL", x, y);
        return false;
      }
    }
