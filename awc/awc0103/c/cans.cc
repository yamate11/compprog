...
int main(/* int argc, char *argv[] */) {
  ...
  if (N % 2 == 0) {
    ll k = N / 2;
    ans = 2 * (S[N] - S[N - (k - 1)]) + A[k] - (2 * (S[k - 1] - S[0]) + A[k - 1]);
  }else {
    if (N == 1) ans = 0;
    else {
      ll k1 = N / 2;
      ...
      ll up1 = S[N] - S[k1];
      ll down1 = S[k1];
      ll xup1 = 2 * up1 - (A[k1] + A[k2]);
      ll xdown1 = 2 * down1;
      ll ans1 = xup1 - xdown1;
      ll up2 = S[N] - S[k2];
      ll down2 = S[k2];
      ll xup2 = 2 * up2;
      ll xdown2 = 2 * down2 - (A[k1] + A[k1 - 1]);
      ll ans2 = xup2 - xdown2;
      DLOGK(up1, down1, xup1, xdown1, ans1, up2, down2, xup2, xdown2, ans2);
      ans = max(ans1, ans2);
    }
  }
  cout << ans << endl;
