...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](ll sgn) -> vector<IPoint> {
    ...
    REP(i, 2, N) {
      while (ssize(stack) >= 2) {
        ll m = ssize(stack);
        IPoint a = stack[m - 1] - stack[m - 2];
        IPoint b = P[i] - stack[m - 1];
        if ((a.x * b.y - a.y * b.x) * sgn > 0) break;
        stack.pop_back();
      }
      stack.push_back(P[i]);
    }
  ...
  DLOGK(down);
  DLOGK(up);
  assert(up.back() == down.back());
  REPrev(i, ssize(up) - 2, 1) down.push_back(up[i]);
  ll ans = 0;
  REP(i, 1, ssize(down) - 1) {
    IPoint a = down[i] - down[0];
    IPoint b = down[i + 1] - down[0];
    ans += a.x * b.y - a.y * b.x;
  }
  cout << ans << endl;
