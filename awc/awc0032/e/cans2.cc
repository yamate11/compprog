...
// @DefStruct(sta, (v, st, len)) [o6IJ8QxD]
struct sta_t {
  ll v;
  ll st;
  ll len;
  sta_t(ll v_ = ll(), ll st_ = ll(), ll len_ = ll()) : v(v_), st(st_), len(len_) {}
...
int main(/* int argc, char *argv[] */) {
  ...
  for (int i = 0; i < N; i++) { ll v; cin >> v; S[i + 1] = v; }

  ll thr = llround(sqrt(N + 10));
  ll numB = divCeil(N + 10, thr);
  vector rec(thr, 0LL);
  auto A = S;
  vector B(numB, 0LL);
  REP(i, 1, N + 1) B[i / thr] += A[i];
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll k, v; cin >> k >> v;
