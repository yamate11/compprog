...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> diff(2 * M);
  auto f = [&](const auto& s) -> ll {
    ll ret = 0;
    REP(i, 0, M) diff[M + i] = s[i] == 0 ? 1 : -1;
    REPrev(i, M - 1, 1) {
      diff[i] = diff[2*i] + diff[2*i + 1];
      if (diff[i] == 0) ret++;
    }
    return ret;
  };
  ...
  {
    string S0; cin >> S0;
    REP(i, 0, M) S[i] = S0[i] - '0';
  }
  ll orig = f(S);
  auto T = S;
  ll cnt0 = 0;
  REP(i, 0, K) cnt0 += (1 - T[i]);
  REP(i, 0, cnt0) T[i] = 0;
  REP(i, cnt0, K) T[i] = 1;
  ll curVal = f(T);
  auto chg = [&](ll p, ll v) -> void {
    p += M;
    diff[p] = v == 0 ? 1 : -1;
    while (true) {
      p /= 2;
      if (p == 0) break;
      if (diff[p] == 0) curVal--;
      diff[p] = diff[2*p] + diff[2*p + 1];
      if (diff[p] == 0) curVal++;
    }
  };

  ll ans = max(orig, curVal);
  DLOGK(orig, curVal, diff);
  REP(i, 0, M - K) {
    ll new_cnt0 = cnt0;
    if (S[i] == 0) new_cnt0--;
    if (S[i + K] == 0) new_cnt0++;
    DLOGK(i, cnt0, new_cnt0, diff);
    ll p0 = i + cnt0;
    ll p1 = p0 + 1;
    ll new_brd = i + 1 + new_cnt0;
    if (p0 < new_brd) {
      chg(p0, 0);
      DLOGKL("p0", p0, new_brd, diff, curVal);
    }
    if (p1 < new_brd) {
      chg(p1, 0);
      DLOGKL("p1", p1, new_brd, diff, curVal);
    }
    chg(i, S[i]);
    DLOGKL("i", curVal);
    if (new_cnt0 < K) {
      chg(i + K, 1);
      DLOGKL("i + K", diff, curVal);
    }
    DLOGK(i, curVal, diff);
    cnt0 = new_cnt0;
    ans = max(ans, curVal);
