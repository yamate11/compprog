...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [HYxTCJj5]
  vector<ll> C(N + 1, 0LL);
  REP(i, 0, N) C[i + 1] = C[i] - A[i] + B[i];
  ll vx = *ranges::max_element(C);
  DLOGK(C, vx);

  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll i, a, b; cin >> i >> a >> b; i--;
      A[i] = a;
      B[i] = b;
      C = vector<ll>(N + 1, 0LL);
      REP(j, 0, N) C[j + 1] = C[j] - A[j] + B[j];
      vx = *ranges::max_element(C);
      DLOGK(_q, tp, C, vx);
    }else {
      ll d; cin >> d;
      d = -d;
