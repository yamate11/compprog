...
int main(/* int argc, char *argv[] */) {
  ...
  auto repr = [&](IPoint vec, IPoint pt) -> sta {
    if (vec.x == 0) {
      assert(vec.y != 0);
      vec.y = 1;
    }else if (vec.y == 0) {
    ...
    ll c = vec.x * pt.y - vec.y * pt.x;
    return {vec.x, vec.y, c};
  };

  ll N; cin >> N;
  vector F(N, vector<IPoint>());
  REP(i, 0, N) {
    ll k; cin >> k;
    REP(j, 0, k) {
      IPoint P; cin >> P;
  ...
  vector<ll> G1, G2, G3;
  REP(i, 0, N) {
    if (ssize(F[i]) == 1) G1.push_back(i);
    else if (ssize(F[i]) == 2) G2.push_back(i);
    else {
      IPoint A = F[i][0];
      IPoint B = F[i][1];
      bool is3 = false;
      ...
      if (not is3) G2.push_back(i);
    }
  }
  map<sta, vector<ll>> mp;
  ll n2 = ssize(G2);
  REP(j, 0, n2) {
    ll i = G2[j];
    auto r = repr(F[i][0] - F[i][1], F[i][0]);
    mp[r].push_back(i);
  }


  vector<IPoint> vecA(n2);
  vector<IPoint> vecB(n2);
  REP(j, 0, n2) {
    ll i = G2[j];
    vecA[j] = F[i][0] - F[i][1];
    vecB[j] = F[i][0];
  ...
  vector<ll> H2(n2, -1);
  vector L2(n2, vector<ll>());
  REP(j, 0, n2) {
    if (H2[j] >= 0) continue;
    H2[j] = j;
    L2[j].push_back(j);
    REP(p, j + 1, n2) {
      if (not para(vecA[j], vecA[p])) continue;
      if (not para(vecA[j], vecB[j] - vecB[p])) continue;
      H2[p] = j;
      L2[j].push_back(p);
    }
  }
