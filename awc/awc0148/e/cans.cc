...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1LL << 60;
  vector C(N + 1, vector(N + 2, big));
  REP(i, 0, N + 1) REP(j, i + 1, N + 2) cin >> C[i][j];
  DLOGK(C);
  vector dist(1LL << M, vector(K + 1, vector(N + 2, big)));
  dist[0][0][0] = 0;
  using sta = tuple<ll, ll, ll, ll>;
  priority_queue<sta, vector<sta>, greater<sta>> pque;
  pque.emplace(0, 0, 0, 0);
  while (not pque.empty()) {
    auto [d, z, k, n] = pque.top(); pque.pop();
    DLOGKL("popped", d, z, k, n, dist[z][k][n]);
    if (dist[z][k][n] == d) {
      REP(m, n + 1, N + 2) {
        ll new_z, new_k;
        if (n == 0) {
        ...
        DLOGK(n, new_z, new_k, m, dist[new_z][new_k][m], d + C[n][m]);
        if (m == N + 1) {
          if (new_k < K) continue;
        }else {
          if (new_k + (N - m) < K) continue;
        }
        if (dist[new_z][new_k][m] > d + C[n][m]) {
          dist[new_z][new_k][m] = d + C[n][m];
          DLOGKL("emplace", d + C[n][m], new_z, new_k, m);
          pque.emplace(d + C[n][m], new_z, new_k, m);
        }
      }
    }
  }
