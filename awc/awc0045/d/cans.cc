...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> tmax(lim + 1, -1LL);
  vector<ll> tmin(lim + 1, -1LL);
  auto f = [&](auto rF, ll a) -> pll {
    ll& vmin = tmin[a];
    ll& vmax = tmax[a];
    if (vmin < 0) {
      if (a == 1) {
        ...
        auto pr = prfacDivSieve(a, divSieve);
        vmin = (ll)1e18;
        vmax = 0;
        for (auto [p, r] : pr) {
          auto [wmin, wmax] = rF(rF, a / p);
          updMin(vmin, 1 + p * wmin);
          updMax(vmax, 1 + p * wmax);
        }
      }
    }
    return pll(vmin, vmax);
  };
