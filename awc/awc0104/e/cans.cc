...
int body(istream& cin, ostream& cout) {
  ...
  ll numB = divCeil(N, bsz) * 8;
  vector diffs(numB, vector<ll>());
  vector a_diffs(numB, vector<ll>());
  vector val(numB, 0LL);
  vector delayed(numB, 0LL);
  auto calc_val = [&](ll p) -> void {
    ll sz = ssize(diffs[p]);
    ll i0 = ranges::lower_bound(diffs[p], -delayed[p]) - diffs[p].begin();
    val[p] = (a_diffs[p][sz] - a_diffs[p][i0]) + delayed[p] * (sz - i0);
  };
  auto calc_vecs = [&](ll p) -> void {
    ...
    REP(q, 0, bsz) {
      ll i = p * bsz + q;
      if (i >= N) break;
      diffs[p].push_back(C[i]);
    }
    ranges::sort(diffs[p]);
    ll sz = ssize(diffs[p]);
    a_diffs[p].resize(sz + 1);
    a_diffs[p][0] = 0;
    REP(i, 0, sz) a_diffs[p][i + 1] = a_diffs[p][i] + diffs[p][i];
    calc_val(p);
  };
