...
// @@ !! LIM(f:intDiv unordered_map cmpNaive debug)
...
int body(istream& cin, ostream& cout) {
  ...
  ll bsz = llround(floor(sqrt(N)));
  ll numB = divCeil(N, bsz);
  vector diff(numB, vector<ll>());
  vector a_cnt(numB, vector<ll>());
  vector a_prod(numB, vector<ll>());
  vector base(numB, 0LL);
  auto calc_vecs = [&](ll p) -> void {
    map<ll, ll> mp;
    REP(q, 0, bsz) {
      ll i = p * bsz + q;
      if (i >= N) break;
    ...
    ll sz = ssize(diff[p]);
    a_cnt[p].resize(sz + 1); a_cnt[p][0] = 0;
    a_prod[p].resize(sz + 1); a_prod[p][0] = 0;
    REP(i, 0, sz) a_cnt[p][i + 1] = a_cnt[p][i] + cnt[i];
    REP(i, 0, sz) a_prod[p][i + 1] = a_prod[p][i] + prod[i];
  };
  REP(p, 0, numB) calc_vecs(p);
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    ll l, r, x; cin >> l >> r >> x; l--;
    if (tp == 2) x = -x;
    ...
    for (ll p : ss) calc_vecs(p);
    ll ans = 0;
    REP(p, 0, numB) {
      ll i0 = ranges::lower_bound(diff[p], base[p]) - diff[p].begin();
      ll sz = ssize(diff[p]);
      DLOGK(p, diff[p], base[p], i0, sz);
      if (i0 < sz) {
        ll v = a_prod[p][sz] - a_prod[p][i0] - base[p] * (a_cnt[p][sz] - a_cnt[p][i0]);
        DLOGK(p, v);
        ans += v;
      }
    }
