...
// @@ !! LIM(cartesianTree segTree debug cmpNaive forall)
...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = 0;
  REP(i, 0, N) {
    ll i0 = EXFIND_D(j, i + 1, N, H[i] < H[j], N);
    if (i0 < N) {
      ll vmin = H[i];
      REP(j, i, i0 + 1) vmin = min(vmin, H[j]);
      ans += vmin;
    }
  }
  cout << ans << "\n";
...
int body(istream& cin, ostream& cout) {
  ...
  auto st = make_seg_tree(big, mymin, H);

  vector<ll> real_target(N);
  ll ans = 0;
  CartesianTree ct(H, greater<ll>());
  auto dfs = [&](auto rF, ll nd, ll target) -> void {
    DLOGK(nd, target);
    ll rt = -1;
    if (target < N and H[nd] == H[target]) {
      rt = real_target[target];
    }else {
      rt = target;
    }
    real_target[nd] = rt;
    if (rt < N) ans += st.query(nd, rt + 1);

    ll l0 = ct.left[nd];
    ll r0 = ct.right[nd];
    if (l0 >= 0) rF(rF, l0, nd);
    if (r0 >= 0) rF(rF, r0, target);
  };
  dfs(dfs, ct.root, N);
  cout << ans << "\n";
