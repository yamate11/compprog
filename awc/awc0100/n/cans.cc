...
int main(/* int argc, char *argv[] */) {
  ...
  CoordCompr cc;
  REP(i, 0, N) cc.add(H[i]);
  ll sz = cc.size();
  auto segt = make_seg_tree(0LL, plus<ll>(), vector<ll>(sz));
  ll cx = sz;
  ll cv = 0;
  REPrev(i, N - 1, 0) {
    ll x = cc.c(H[i]);
    if (x <= cx) {
      segt.rs(x) = segt.at(x) + 1;
      cx = x;
    }else {
      ll tv = H[i] - cc.d(cx);
      assert(tv >= 0);
      cv += tv;
      segt.rs(cx) = segt.at(cx) - 1;
      segt.rs(x)  = segt.at(x)  + 2;
      if (segt.at(cx) == 0) {
        cx = segt.binsearch_r_until([](ll v) { return v == 0; }, 0);
      }
    }
    DLOGK(i, cc.d(cx), cv, segt.vec_view());
  }
  cout << cv << endl;
  return 0;
}
