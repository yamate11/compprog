...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, vec, type=IPoint) [lEjw6fPG]
  auto vec = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; vec[i] = v; }
  // @End [lEjw6fPG]

  auto ord = sortedIndexCFAI(vec,
                             [&](ll i, ll j) -> bool {
                               ll t = vec[i].x * vec[j].y - vec[i].y * vec[j].x;
                               if (t != 0) return t < 0;
                               return i < j;
                             });
  cout << ord[0] + 1 << "\n";
