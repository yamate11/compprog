...
// @@ !! LIM(binsearch f:intDiv f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&](ll K) -> bool {
    DLOGKL("check called", K);
    ll rmin = 0;
    ll rmax = 2e9;
    REP(i, 0, N) {
      ll alpha = V - divCeil(D[i], K);
      DLOGK(K, i, D[i], alpha);
      if (alpha < 0) return false;
      updMax(rmin, X[i] - alpha);
      updMin(rmax, X[i] + alpha);
      DLOGK(K, i, D[i], rmin, rmax);
      if (rmax < rmin) return false;
    }
    return true;
  };
  ll big = 1LL << 60;
  ll K = binsearch<ll>(check, big, 0);
  if (K == big) K = -1;
  cout << K << "\n";
