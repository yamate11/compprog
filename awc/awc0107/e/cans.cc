...
// @DefStruct(sta, (vm, idx)) [GFi5jA2I]
struct sta {
  ll vm;
  ll idx;
  string show() const {
    return "(" + to_string(vm) + ", " + to_string(idx) + ")";
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  ll T, N, D, Q; cin >> T >> N >> D >> Q;
  // @InpMVec(N, ((L, dec=0), (R, dec=0))) [cckk1WGM]
  auto L = vector(N, ll());
  auto R = vector(N, ll());
  for (int i = 0; i < N; i++) {
  ...
  using DAT = sta;
  DAT unit_DAT(2*N, -1LL);
  auto add = [&](const DAT& a, const DAT& b) -> DAT {
    if (a.vm <= b.vm) return a;
    else return b;
  };
  auto appl = [&](ll x, const DAT& a) -> DAT { return DAT(a.vm + x, a.idx); };
  vector<DAT> init_vec(T - D + 1);
  REP(i, 0, T - D + 1) init_vec[i] = DAT(0, i);
  auto st = make_seg_tree_lazy(unit_DAT, 0LL, add, plus<ll>(), appl, init_vec);
  auto getB = [&](ll l, ll r) -> pll {
    ll a = max(0LL, r - D);
    ll b = min(T - D + 1, l + 1);
    return {a, b};
  };
  ...
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll i, l, r; cin >> i >> l >> r; i--;
      auto [a0, b0] = getB(L[i], R[i]);
      st.update(a0, b0, -1);
      L[i] = l;
      R[i] = r;
      auto [a1, b1] = getB(L[i], R[i]);
      st.update(a1, b1, 1);
    }else if (tp == 2) {
      auto v = st.query(0, T - D + 1);
      cout << v.idx << " " << v.vm << "\n";
    }
  }
