...
// @@ !! LIM(coordCompr segTree f:updMaxMin)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, K; cin >> N >> M >> K;
  // @InpMVec(M, ((L, dec=1), (R, dec=0))) [HdiN9UaA]
  auto L = vector(M, ll());
  auto R = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  ll big = 1LL << 60;
  pll unit_dat(big, 0);
  auto add = [&](pll a, pll b) -> pll {
    if (a.first < b.first) return a;
    else if (a.first > b.first) return b;
    else return pll(a.first, a.second + b.second);
  };
  auto apply = [&](ll f, pll a) -> pll {
    return pll(a.first + f, a.second);
  };
  vector<pll> init_dat(sz);
  REP(i, 0, sz) init_dat[i] = pll(0, i + 1 < sz ? cc.d(i + 1) - cc.d(i) : 0);
  auto st = make_seg_tree_lazy(unit_dat, 0LL, add, plus<ll>(), apply, init_dat);
  ll init_v = st.query(0, sz).second;
  ll ans = 0;
  REP(i, K, M) {
    st.update(cc.c(L[i]), cc.c(R[i]), 1);
  }
  updMax(ans, init_v - st.query(0, sz).second);
  REP(i, 0, M - K) {
    st.update(cc.c(L[i]), cc.c(R[i]), 1);
    st.update(cc.c(L[i + K]), cc.c(R[i + K]), -1);
    updMax(ans, init_v - st.query(0, sz).second);
  }
  cout << ans << endl;
  return 0;
