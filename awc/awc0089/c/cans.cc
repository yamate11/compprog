...
int naive(istream& cin, ostream& cout) {
  ...
  // @End [xJfZrwXS]
  // @InpVec(N - 1, C) [JzNoy9SB]
  auto C = vector(N - 1, ll());
  for (int i = 0; i < N - 1; i++) { ll v; cin >> v; C[i] = v; }
  // @End [JzNoy9SB]
  ...
  REP(i, 0, N) REP(j, i + 1, N) {
    ll cost = 0;
    ll sat = 0;
    for (ll k = i; k + 1 <= j; k++) cost += C[k];
    for (ll k = i; k <= j; k++) sat += P[k];
    if (cost <= B) ans = max(ans, sat);
  }
  cout << ans << endl;
