...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, Q; cin >> N >> M >> Q;
  // @InpGrid(N, M, B) [hcSH7inh]
  auto B = vector(N, vector(M, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < M; j++) { ll v; cin >> v; B[i][j] = v; }
  // @End [hcSH7inh]
  // @InpMVec(Q, ((X, dec=1), (Y, dec=1), (Z, dec=1), (W, dec=1))) [IFhtrwJb]
  auto X = vector(Q, ll());
  auto Y = vector(Q, ll());
  auto Z = vector(Q, ll());
  auto W = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
    ...
    ll v3; cin >> v3; v3 -= 1; Z[i] = v3;
    ll v4; cin >> v4; v4 -= 1; W[i] = v4;
  }
  // @End [IFhtrwJb]
  ...
  DLOGK(A);
  REP(i, 1, N + 1) REP(j, 0, M + 1) A[i][j] ^= A[i - 1][j];
  REP(i, 0, N + 1) REP(j, 1, M + 1) A[i][j] ^= A[i][j - 1];
  DLOGK(A);
  ll ans = 0;
  REP(i, 0, N) REP(j, 0, M) {
    ll x;
