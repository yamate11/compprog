...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpNbrList(N, M, nbr, dec=1, read=C) [XAjwC19w]
  struct nbr_t {
    int nd;
    ll C;
  ...
  vector<vector<ll>> distV(K, vector(K, 0LL));
  REP(k, 0, K) {
    auto vec = dij(V[k]);
    REP(i, 0, K) distV[k][i] = vec[V[i]];
  }
  auto dist0 = dij(0);

  vector tbl(1LL << K, vector(K, big));
  REP(k, 0, K) tbl[1LL << k][k] = dist0[V[k]];
  REP(x, 0, 1LL << K) REP(t, 0, K) {
    if (tbl[x][t] < big) {
      REP(i, 0, K) if (not (x >> i & 1)) {
        updMin(tbl[x | (1LL << i)][i], tbl[x][t] + distV[t][i]);
      }
    }
  }
  ll full = (1LL << K) - 1;
  ll ans = big;
  REP(k, 0, K) ans = min(ans, tbl[full][k] + dist0[V[k]]);
  cout << ans << endl;
