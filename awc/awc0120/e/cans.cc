...
// published at https://github.com/yamate11/compprog-clib/blob/master/rollingHash.cc
...
int main(/* int argc, char *argv[] */) {
  ...
  string P; cin >> P;
  ll szP = ssize(P);
  // @InpVec(N, S, type=string) [No5oH3ri]
  ...
  ranges::reverse(revP);
  RollingHash rh;
  auto hs0 = rh.precomp(P);
  auto hs1 = rh.precomp(revP);
  vector vmp(N, safe_umap<RHVal, ll>());
  REP(i, 0, N) {
    auto hs_i = rh.precomp(S[i]);
    REP(j, 0, ssize(S[i]) - szP + 1) {
      RHVal r = hs_i.get(j, szP);
      vmp[i][r]++;
    }
  }

  REP(_q, 0, Q) {
    ll i, l, r; cin >> i >> l >> r; i--; l--;
    RHVal part1 = hs0.get(0, l);
    RHVal part3 = hs0.get(r, szP - r);
    RHVal part2 = hs1.get(szP - r, r - l);
    RHVal tot = rh.hash_concat(rh.hash_concat(part1, part2, r - l), part3, szP - r);
    auto it = vmp[i].find(tot);
    ll ans = 0;
    if (it != vmp[i].end()) ans = it->second;
    cout << ans << "\n";
  }
