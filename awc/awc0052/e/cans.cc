...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [ehHvIOZI]
  // @InpNbrList(N, M, nbr, dec=1, read=W) [zjEhRieX]
  struct nbr_t {
    int nd;
    ll W;
    ...
    REP(x, 0, 1LL << N) REP(i, 0, N) {
      ...
      if (str0 >= 0) {
        for (auto [peer, w] : nbr[i]) {
          if (str0 >= w) {
            ll new_str = str0 - w + ((x >> peer & 1) ? 0 : R[peer]);
            ll new_x = x | (1LL << peer);
            if (new_str > str[new_x][peer]) {
              str[new_x][peer] = new_str;
              DLOGK(x, i, str0, peer, new_x, str[new_x][peer]);
              changed = true;
            }
          }
        }
  ...
  ll ans = -1;
  REP(x, 0, 1LL << N) ans = max(ans, str[x][N - 1]);
  cout << ans << endl;
  return 0;
}
