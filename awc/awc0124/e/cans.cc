...
vector<INT> conv_minplus(const vector<INT>& a, const vector<INT>& b) {
  int m = ssize(a);
  int n = ssize(b);
  if (m == 0 or n == 0) return vector<INT>{};
  INT cur = a[0] + b[0];
  vector<INT> ret{cur};
  int i = 0, j = 0;
  auto proc = [&](const auto& v, int& idx) -> void { cur += v[idx + 1] - v[idx]; ret.push_back(cur); idx++; };
  while (i + 1 < m or j + 1 < n) {
    if      (j + 1 >= n) proc(a, i);
    else if (i + 1 >= m) proc(b, j);
    else if (a[i + 1] - a[i] <= b[j + 1] - b[j]) proc(a, i);
    else proc(b, j);
  }
  DLOGKL("conv_minplus", a, b, ret);
  return ret;
}
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<safe_umap<ll, vector<ll>>> tbl(N);
  auto f = [&](auto rF, ll l, ll r) -> vector<ll> {
    DLOGKL("called", l, r);
    auto [it, b] = tbl[l].try_emplace(r, vector<ll>{0});
    if (b) {
      if (l == r) ; // do nothing
      else if (l + 1 == r) it->second.push_back(A[l]);
      else if (l + 2 == r) it->second.push_back(min(A[l], A[l + 1]));
      else {
        ll m = (l + r) / 2;
        auto v1 = conv_minplus(rF(rF, l, m - 1), rF(rF, m, r));
        auto v2 = conv_minplus(rF(rF, l, m), rF(rF, m + 1, r));
        DLOGK(l, r, v1, v2);
        REP(k, 1, K + 1) {
          if (ssize(v1) <= k and ssize(v2) <= k) break;
          else if (ssize(v1) <= k) it->second.push_back(v2[k]);
          else if (ssize(v2) <= k) it->second.push_back(v1[k]);
          else it->second.push_back(min(v1[k], v2[k]));
        }
      }
      DLOGK(l, r, it->second);
    }
  ...
  auto vec = f(f, 1, N - 1);
  ll ans = tot - *ranges::min_element(vec);
  cout << ans << "\n";
