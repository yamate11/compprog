...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N + 1) init_vec[i] = AC[i].second;
  auto segt = make_seg_tree(big, mymin, init_vec);
  vector<ll> vecA(N + 1);
  REP(i, 0, N + 1) vecA[i] = AC[i].first;
  ll ans = big;
  DLOGK(vecA);
  DLOGK(init_vec);
  REP(i, 0, N + 1) {
    ll j = ranges::lower_bound(vecA, D - vecA[i]) - vecA.begin();
    DLOGK(i, j);
    j = max(j, i + 1);
    if (j < N + 1) {
      ll cost2 = segt.query(j, N + 1);
      ll cost = init_vec[i] + cost2;
      ans = min(ans, cost);
    }
  }
  if (ans == big) ans = -1;
