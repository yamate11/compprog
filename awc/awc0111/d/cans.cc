...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](const auto& nbr) -> vector<string> {
    vector<string> ret;
    IntPerm ip(N, N);
    while (ip.get()) {
      auto dfs = [&](auto rF, ll nd, ll pt) -> string {
        if (nd > 0 and ssize(nbr[nd]) == 1) return "ab";
        vector<ll> cs;
        for (ll peer : nbr[nd]) if (peer != pt) cs.push_back(peer);
        ranges::sort(cs, [&](ll i, ll j) -> bool { return ip[i] < ip[j]; });
        string r = "a";
        for (ll c : cs) r += rF(rF, c, nd);
        r += "b";
        return r;
      };
      ret.push_back(dfs(dfs, 0, -1));
    }
    ranges::sort(ret);
  ...
  // @InpNbrList(N, N - 1, nbrA, dec=1) [3S0b9D6Y]
  auto nbrA = vector(N, vector(0, int()));
  for (int i = 0; i < N - 1; i++) {
    int u, v; cin >> u >> v; u -= 1; v -= 1;
    nbrA[u].emplace_back(v);
    nbrA[v].emplace_back(u);
  }
  // @End [3S0b9D6Y]
  // @InpNbrList(N, N - 1, nbrB, dec=1) [d8IH2kGC]
  auto nbrB = vector(N, vector(0, int()));
  for (int i = 0; i < N - 1; i++) {
    int u, v; cin >> u >> v; u -= 1; v -= 1;
    nbrB[u].emplace_back(v);
    nbrB[v].emplace_back(u);
  }
  // @End [d8IH2kGC]
  ...
  {
    ll i = 0, j = 0;
    while (i < ssize(ssA) and j < ssize(ssB)) {
      if (ssA[i] == ssB[j]) {
        ans.push_back(ssA[i]);
        i++;
        j++;
      }else if (ssA[i] < ssB[j]) i++;
      else j++;
    }
  }
  cout << ssize(ans) << "\n";
