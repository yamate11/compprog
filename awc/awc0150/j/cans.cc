...
// @@ !! LIM(debug f:gcd f:intDiv f:updMaxMin)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((S, dec=1), (D, dec=0))) [EA1YvMkd]
  auto S = vector(M, ll());
  auto D = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  DLOGK(intV);
  ll big = 1LL << 60;
  vector tbl(1LL << M, vector(N, big));
  tbl[0][0] = 1;
  if (N >= 2) tbl[0][1] = 1;
  REP(x, 0, 1LL << M) {
    priority_queue<pll, vector<pll>, greater<pll>> pque;
    REP(p, 0, N) if (tbl[x][p] < big) pque.emplace(tbl[x][p], p);
    DLOGKL("before D", x, tbl[x]);
    while (not pque.empty()) {
      auto [t, p] = pque.top(); pque.pop();
      if (tbl[x][p] == t) {
        auto op = [&](ll p1, ll t1) -> void {
          if (t1 < tbl[x][p1]) {
            tbl[x][p1] = t1;
            pque.emplace(t1, p1);
          }
    ...
    DLOGKL("after D", x, tbl[x]);
    REP(p, 0, N) if (tbl[x][p] < big) REP(a, 0, M) if ((not (x >> a & 1)) and fstV[p][a] >= 0) {
        ll y = x | (1LL << a);
        ll tt = max(0LL, tbl[x][p] - fstV[p][a]);
        bool b = updMin(tbl[y][p], fstV[p][a] + divCeil(tt, intV[a]) * intV[a]);
        if (b) DLOGK(x, p, a, y, tbl[y][p]);
      }
  }
  ll ans = *ranges::min_element(tbl[(1LL << M) - 1]);
  cout << ans << "\n";
