...
// @@ !! LIM(tree segTree f:updMaxMin debug unordered_map cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ll big = 1LL << 60;
  ll N, Q; cin >> N >> Q;
  // @InpMVec(N - 1, ((A, dec=1), (B, dec=1), C)) [CG8GzOoo]
  ...
  REP(i, 0, N - 1) dist[A[i]][B[i]] = dist[B[i]][A[i]] = C[i];
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      ll v0, t; cin >> v0 >> t; v0--;
      ll d = big;
      REP(i, 0, N) if (on[i]) updMin(d, dist[v0][i]);
#if ANS_DIGIT
      cout << d << "\n";
#else
      cout << (d <= t ? "YES\n" : "NO\n");
