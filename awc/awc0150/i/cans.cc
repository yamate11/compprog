...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> valW{0};
  REP(m, 0, M) {
    auto it = ranges::lower_bound(valW, W[m]);
    if (it == valW.end() or *it != W[m]) valW.insert(it, W[m]);
    DLOGK(m, valW);
    auto func = [&](ll p0) -> vector<ll> {
      vector fwd(N, vector<ll>());
      vector numbwd(N, 0LL);
      REP(i, 0, m + 1) if (W[i] > valW[p0]) {
        REP(j, 0, ssize(P[i]) - 1) {
          fwd[P[i][j]].push_back(P[i][j + 1]);
          numbwd[P[i][j + 1]]++;
        }
      }
      vector<ll> ordered;
      vector<ll> stack;
      REP(i, 0, N) if (numbwd[i] == 0) {
        stack.push_back(i);
      }
    ...
    auto check = [&](ll p0) -> bool {
      vector v = func(p0);
      bool b = ssize(v) == N;
      DLOGKL("check", m, p0, v, b);
      return b;
    };
    ll p0 = binsearch<ll>(check, ssize(valW), -1LL);
    DLOGKL("binsearch", m, p0, valW[p0]);
    ans1[m] = valW[p0];
    if (m == M - 1) { ans2 = move(func(p0)); }
  }
  REPOUT(i, 0, M, ans1[i], " ");
  REPOUT(i, 0, N, ans2[i] + 1, " ");
