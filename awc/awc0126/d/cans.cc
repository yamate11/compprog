...
// @@ !! LIM(sieve forall debug unordered_map cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto enc = [&](const string& s) -> ll {
    ll ret = 0;
    for (char c : s) ret = ret * 2 + (c == 'A' ? 0 : 1);
    return ret;
  };
  auto dec = [&](ll e) -> string {
    string ret;
    REP(i, 0, N) {
      ret += (e & 1) ? 'B' : 'A';
      e >>= 1;
  ...
  ll mask = (1LL << N) - 1;
  ll big = 1LL << 60;
  vector dist(1LL << N, big);
  vector prev(1LL << N, 0LL);
  queue<ll> que;
  ll es = enc(S);
  ll et = enc(T);
  dist[es] = 0;
...
int body(istream& cin, ostream& cout) {
  ...
  ranges::sort(divs);
  ll per = N;
  for (ll d : divs) if (FORALL(i, 0, N, T[i] == T[i % d])) { per = d; break; }
  vector<ll> adds{1};
  for (ll d : divs) if (d < per and per % d != 0) adds.push_back(d);
  ll big = 1LL << 60;
  ll ans = big;
  REP(p, 0, N) for (ll a : adds) REP(q, 0, a) REP(r, 0, per) {
      ll cur = 0;
      auto rotate = [&](const string& z, ll k) -> string {
        if (k == 0) return z;
        cur += min(k, ssize(z) - k);
        return (z + z).substr(k, ssize(z));
      };
      auto x = rotate(S, p);
      DLOGK(p, a, q, r, x, cur);
      string y;
      ...
      if (y == T) ans = min(ans, cur);
      else {
        y = y.substr(0, per);
        cur++;
        y = rotate(y, r);
        DLOGK(p, a, q, r, y, cur);
        if (y == T.substr(0, per)) ans = min(ans, cur);
      }
    }
  if (ans == big) ans = -1;
  cout << ans << "\n";
