...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [jXShDPaf]
  // @InpMVec(N - 1, ((A, dec=1), (B, dec=1))) [DIKyJUrZ]
  ...
  REP(i, 0, N - 1) {
    nbr[A[i]].emplace_back(B[i], i);
    nbr[B[i]].emplace_back(A[i], i);
  }
  vector wtE(N - 1, vector(2, vector(N, 0LL)));
  REP(i, 0, N - 1) {
    auto f = [&](auto rF, ll kd, ll nd, ll pt, ll dp) -> void {
      wtE[i][kd][dp] += V[nd];
      for (auto [peer, _dum] : nbr[nd]) {
        if (peer == pt) continue;
        rF(rF, kd, peer, nd, dp + 1);
      }
    };
    f(f, 0, A[i], B[i], 0);
    f(f, 1, B[i], A[i], 0);
  }
  vector wtN(N, vector(N, 0LL));
  REP(nd, 0, N) wtN[nd][0] = V[nd];
  REP(i, 0, N - 1) REP(kd, 0, 2) REP(d, 1, N) {
    if (kd == 0) wtN[B[i]][d] += wtE[i][kd][d - 1];
    else         wtN[A[i]][d] += wtE[i][kd][d - 1];
  }
  REP(nd, 0, N) REP(d, 1, N) wtN[nd][d] += wtN[nd][d - 1];
  REP(i, 0, N - 1) REP(kd, 0, 2) REP(d, 1, N) wtE[i][kd][d] += wtE[i][kd][d - 1];
#if DEBUG
  REP(i, 0, N - 1) REP(kd, 0, 2) DLOGK(i, kd, wtE[i][kd]);
  REP(nd, 0, N) DLOGK(nd, wtN[nd]);
#endif
  ll ans = 1LL << 60;
  REP(i, 0, N - 1) {
    ll cur = 0;
    auto f = [&](auto rF, ll kd, ll nd, ll pt, ll dp) -> void {
      updMax(cur, wtN[nd][D] - (D - (dp + 1) >= 0 ? wtE[i][1 - kd][D - (dp + 1)] : 0));
      for (auto [peer, _dum] : nbr[nd]) {
        if (peer == pt) continue;
        rF(rF, kd, peer, nd, dp + 1);
      }
