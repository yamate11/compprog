...
// ---- inserted library file doublyLinkedList.cc
// published at https://github.com/yamate11/compprog-clib/blob/master/doublyLinkedList.cc
...
int main(/* int argc, char *argv[] */) {
  ...
  myDLL* plo = new myDLL(pll(-1, 0));
  vector<myDLL*> nodes(N - 1);
  myDLL* pp = plo;
  REP(i, 0, N - 1) {
    ll v = S[i + 1] - S[i];
    pp = nodes[i] = pp->insert_after(pll(i, v));
    pque.emplace(v, i);
  }
  pp->insert_after(pll(N - 1, 0));

  ll ans = 0;
  REP(j, 0, M) {
    ll v0, i0;
    while (true) {
      tie(v0, i0) = pque.top(); pque.pop();
      if (nodes[i0]) break;
    }
    ans += v0;
    auto [ip, vp] = nodes[i0]->prv->val;
    auto [in, vn] = nodes[i0]->nxt->val;
    if (ip >= 0) {
      myDLL::erase(nodes[i0]->prv);
      nodes[ip] = nullptr;
    }
    if (in < N - 1) {
      myDLL::erase(nodes[i0]->nxt);
      nodes[in] = nullptr;
    }
    if (ip >= 0 and in < N - 1) {
      ll new_v = vp + vn - v0;
      nodes[i0]->val.second = new_v;
      pque.emplace(new_v, i0);
    }else {
      myDLL::erase(nodes[i0]);
      nodes[i0] = nullptr;
    }
#if DEBUG
    {
      DLOGK(j, ans);
      vector<pll> vnodes(N - 1);
      REP(i, 0, N - 1) vnodes[i] = nodes[i] ? nodes[i]->val : pll(-1, -1);
      DLOGK(vnodes);
      DLOGK(*plo);
      DLOGK(pque);
    }
