...
int main(/* int argc, char *argv[] */) {
  ...
  auto enc = [&](ll x, ll n) -> int { return n * K + x; };
  auto dec = [&](int e) -> pll { return {e % K, e / K}; };

  ll sz = bit_ceil((u64)N);
  ll ht = bit_width((u64)sz) - 1;
  
  using DAT = vector<int>;
  DAT dat_unit(K);
  REP(i, 0, K) dat_unit[i] = enc(i, 0);

  vector<DAT> P(sz, dat_unit);
  REP(i, 0, N) {
    REP(k, 0, K) {
      ll x = k;
      x += A[i];
      x = max(0LL, x - B[i]);
      if (x >= K) P[i][k] = enc(0, 1);
      else        P[i][k] = enc(x, 0);
    }
  }
  ...
  REP(h, 0, ht - 1) {
    ...
    REP(j, 0, bNum) {
      ...
      REP(i, c + 1, c + len) {
        DLOGKL("   a ", h, j, c, i);
        R[h][i] = add(R[h][i - 1], R[h][i]);
      }
      REPrev(i, c - 2, c - len) {
        DLOGKL("   b ", h, j, c, i);
        R[h][i] = add(R[h][i], R[h][i + 1]);
      }
    }
  }
#if DEBUG
  ...
  REP(i, 0, Q) {
    ...
    DAT d;
    ll j = ht - 1;
    while (j >= 0 and (l >> j & 1) == (r >> j & 1)) j--;
    if (j < 0) d = P[l];
    else d = add(R[ht - 1 - j][l], R[ht - 1 - j][r]);
    auto [x, n] = dec(d[0]);
    cout << n << " " << x << "\n";
  }
