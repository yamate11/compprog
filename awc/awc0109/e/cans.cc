...
int main(/* int argc, char *argv[] */) {
  ...
  using DAT = vector<pii>;
  DAT unit_dat(K);
  REP(i, 0, K) unit_dat[i] = {0, i};
  auto add = [&](const DAT& a, const DAT& b) -> DAT {
    DAT ret(K);
    REP(i, 0, K) {
      const auto& [n1, x1] = a[i];
      const auto& [n2, x2] = b[x1];
      ret[i] = {n1 + n2, x2};
    }
    return ret;
  };
  auto idat = [&](ll c) -> DAT {
    ...
    REP(i, 0, K) {
      if (i + c >= K) ret[i] = {1, 0};
      else if (i + c >= 0) ret[i] = {0, i + c};
      else ret[i] = {0, 0};
    }
    return ret;
  };
  vector<DAT> init_vec(N);
  REP(i, 0, N) init_vec[i] = idat(A[i] - B[i]);
  auto segt = make_seg_tree(unit_dat, add, init_vec);
  REP(_q, 0, Q) {
    ll l, r; cin >> l >> r; l--;
    auto [n, x] = segt.query(l, r)[0];
    cout << n << " " << x << "\n";
  }
