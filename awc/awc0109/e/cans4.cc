...
// Do test orig_size == 0 and orig_size == 1!!
...
// @DefStruct(sta, (st, en, tp, x, n)) [rfO2kfbD]
struct sta {
  ll st;
  ll en;
  ll tp;
  ...
  ll n;
  string show() const {
    return "(" + to_string(st) + ", " + to_string(en) + ", " + to_string(tp) + ", " + to_string(x) + ", " + to_string(n) + ")";
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  using DAT = vector<sta>;
  DAT unit{{0, K, TP_LIN, 0, 0}};
  auto add = [&](const DAT& a, const DAT& b) -> DAT {
    DAT ret;
    for (const sta& e : a) {
      if (e.tp == TP_CONST) {
        ll i = binsearch<ll>([&](ll ii) { return e.x < b[ii].en; }, ssize(b), -1);
        if (b[i].tp == TP_CONST) ret.emplace_back(e.st, e.en, TP_CONST, b[i].x, e.n + b[i].n);
        else {
          assert(b[i].n == 0);
          ret.emplace_back(e.st, e.en, TP_CONST, e.x + b[i].x, e.n);
        }
      }else {
        assert(e.n == 0);
        ll u = e.st;
        ll i = binsearch<ll>([&](ll ii) { return u + e.x < b[ii].en; }, ssize(b), -1);
        for (; u < e.en; i++) {
          ll dist_a = e.en - u;
          ll dist_b = b[i].en - (u + e.x);
          ll dist = min(dist_a, dist_b);
          if (b[i].tp == TP_CONST) ret.emplace_back(u, u + dist, TP_CONST, b[i].x, b[i].n);
          else {
            assert(e.n == 0 and b[i].n == 0);
            ret.emplace_back(u, u + dist, TP_LIN, e.x + b[i].x, 0);
          }
          u += dist;
        }
      }
  ...
  vector<DAT> init_dat(N);
  REP(i, 0, N) {
    ll y = A[i] - B[i];
    if (y >= K) init_dat[i] = DAT{{0, K, TP_CONST, 0, 1}};
    else if (y > 0) init_dat[i] = DAT{{0, K - y, TP_LIN, y, 0}, {K - y, K, TP_CONST, 0, 1}};
    else if (y == 0) init_dat[i] = unit;
    else if (y > -K + 1) init_dat[i] = DAT{{0, -y, TP_CONST, 0, 0}, {-y, K, TP_LIN, y, 0}};
    else init_dat[i] = DAT{{0, K, TP_CONST, 0, 0}};
  }

  // DisjSparseTbl dst(unit, add, init_dat);

  vector<DAT> ctbl(N);
  auto set_vals = [&](ll h) {
    ll len = 1 << h;
    ll blen = 2 * len;
    ll numB = (N + blen - 1) / blen;
    for (ll i = 0; i < numB; i++) {
      ll c = i * blen + len;
      if (c >= N) continue;
      ctbl[c - 1] = init_dat[c - 1];
      for (int j = c - 2; j >= c - len; j--) ctbl[j] = add(init_dat[j], ctbl[j + 1]);
      ctbl[c] = init_dat[c];
      for (int j = c + 1; j < min(N, c + len); j++) ctbl[j] = add(ctbl[j - 1], init_dat[j]);
    }
  };

  auto query = [&](int l, int r) -> DAT {  // closed interval [l, r]
    if (l > r) return unit;
    if (l == r) return init_dat[l];
    return add(ctbl[l], ctbl[r]);
  };

  vector perH(20, vector<ll>());
  vector<ll> L(Q);
  vector<ll> R(Q);
  vector<sta> ans(Q);
  REP(q, 0, Q) {
    ...
    L[q] = l;
    R[q] = r;
    if (l == r) ans[q] = init_dat[l][0];
    else {
      ll h = bit_width((unsigned)(l ^ r)) - 1;
      perH[h].push_back(q);
    }
  }
  REP(h, 0, 20) {
    if (not perH[h].empty()) {
      set_vals(h);
      for (ll q : perH[h]) {
        ans[q] = query(L[q], R[q])[0];
      }
    }
  }
  REP(q, 0, Q) {
    cout << ans[q].n << " " << ans[q].x << "\n";
  }
