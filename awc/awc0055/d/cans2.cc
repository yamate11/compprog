...
int main(/* int argc, char *argv[] */) {
  ...
  ll Q; cin >> Q;
  // @InpMVec(Q, ((C, type=char), X)) [FOiInYLe]
  ...
  REP(q, 0, Q) {
    ...
    if (C[q] == '+') {
      ...
      DLOGKL("-", v);
      auto check = [&](ll val) -> bool { return val < (curnum + 1) / 2; };
      ll med = st.binsearch_r_until(check, 0);
      DLOGK(st.vec_view(), med, curnum);
      if (v == med) ans++;
      st.rs(v) = st.at(v) - 1;
      curnum--;
    }else assert(0);
