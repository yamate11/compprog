...
int main(/* int argc, char *argv[] */) {
  ...
  // safe_uset<ll> ss;
  vector<ll> result;
  auto f = [&](ll lo, ll hi, int mode) -> bool {
    ll wid = hi - lo;
    REP(x, 0, 1LL << wid) {
      ll y = x;
      while (true) {
        ...
        REP(t, 0, N) {
          if (y >> t & 1) val += A[lo + t];
          else if (x >> t & 1) val += B[lo + t];
        }
        if (mode == 0) result.push_back(val);
        else {
          auto it = ranges::lower_bound(result, K - val);
          if (it != result.end() and *it == K - val) return true;
        }
        if (y == 0) break;
        y = (y - 1) & x;
      }
