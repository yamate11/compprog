...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [g27DTWIE]
  // @InpMVec(Q, ((L, dec=1), (R, dec=0), T)) [Na0j4utL]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  auto T = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  auto d_ord = sortedIndex(D, greater<ll>());
  auto t_ord = sortedIndex(T, greater<ll>());
  ...
  REP(ii, 0, Q) {
    ll i = t_ord[ii];
    for (; idx < M and D[d_ord[idx]] >= T[i]; idx++) {
      ll p = S[d_ord[idx]];
      st.rs(p) = st.at(p) + 1;
    }
    ans[i] = max(st.query(l, r) - K, 0LL);
  }
  REPOUT(i, 0, Q, ans[i], "\n");
  return 0;
}
