...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  digit_util du(10);
  auto vec = du.to_vector_big_endian(N);

  //                                  a          b
  vector tbl_init(2, vector(2, vector(10, vector(K + 2, 0LL))));
  auto tbl = tbl_init;
  tbl[1][1][0][0] = 1;
  ...
  for (ll t : vec) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(eq, 0, 2) REP(az, 0, 2) REP(a, 0, 10) REP(b, 0, K + 1) {
      if (prev[eq][az][a][b] == 0) continue;
      REP(d, 0, 10) {
        ...
        if (new_az) new_b = 0;
          ...
          new_b = b + abs(a - d);
          if (new_b > K) new_b = K + 1;
        }
        tbl[new_eq][new_az][new_a][new_b] += prev[eq][az][a][b];
      }
    }
  }
  ll ans = 0;
  REP(eq, 0, 2) REP(a, 0, 10) ans += tbl[eq][0][a][K];
  cout << ans << "\n";
