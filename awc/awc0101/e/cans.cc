...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    ...
    auto add = [&](ll t) -> void {
      if (mp[A[t]]++ == 0) var++;
      while (not vmax.empty() and B[vmax.back()] <= B[t]) vmax.pop_back();
      while (not vmin.empty() and B[vmin.back()] >= B[t]) vmin.pop_back();
      vmax.emplace_back(t);
      vmin.emplace_back(t);
    };
    auto rem = [&](ll t) -> void {
      if (--mp[A[t]] == 0) var--;
      while (not vmax.empty() and vmax.front() <= t) vmax.pop_front();
      while (not vmin.empty() and vmin.front() <= t) vmin.pop_front();
    };
    auto cond = [&]() -> bool {
      if (i == j) return true;
      bool b1 = var * (j - i) <= K;
      bool b2 = vmax.front().first - vmin.front().first <= M;
      return b1 and b2;
    };
