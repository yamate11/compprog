...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 0, M) {
    ll p, d; cin >> p >> d;
    ev.emplace_back(p - L, STORE_IN, d);
    ev.emplace_back(max(0LL, p - L), EVAL, 0);
    ev.emplace_back(p + 1, STORE_OUT, d);
    if (p + 1 <= N - L) ev.emplace_back(p + 1, EVAL, 0);
  }
  REP(i, 0, K) {
    ll q; cin >> q;
    ev.emplace_back(q - L, RES_IN, 0);
    ev.emplace_back(max(0LL, q - L), EVAL, 0);
    ev.emplace_back(q + 1, RES_OUT, 0);
    if (q + 1 <= N - L) ev.emplace_back(q + 1, EVAL, 0);
  }
  ev.emplace_back(0, EVAL, 0);
  ev.emplace_back(N - L, EVAL, 0);
  ranges::sort(ev);
  ...
  for (auto [t, kd, x] : ev) {
    if (kd == STORE_IN) {
      ...
      if (not ms.empty()) {
        ll lst = *ms.begin();
        if (lst < -R) cur_mod = cur - lst - R;
      }
      DLOGK(t, kd, x, cur_mod);
      ans = max(ans, cur_mod);
    }
    DLOGK(t, kd, x, cur, ms);
  }
