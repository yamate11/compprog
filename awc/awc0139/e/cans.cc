...
int naive(istream& cin, ostream& cout) {
  ll N, M, K, L, C, R; cin >> N >> M >> K >> L >> C >> R;
  
  // @InpMVec(M, (P, D)) [7WOeAEyg]
  auto P = vector(M, ll());
  auto D = vector(M, ll());
  for (int i = 0; i < M; i++) {
    ll v1; cin >> v1; P[i] = v1;
  ...
  // @InpVec(K, Q) [r05a90Gm]
  auto Q = vector(K, ll());
  for (int i = 0; i < K; i++) { ll v; cin >> v; Q[i] = v; }
  // @End [r05a90Gm]
  ...
  REP(s, 0, N - L + 1) {
    ll cur = 0;
    ll lst = big;
    REP(i, 0, M) if (s <= P[i] and P[i] <= s + L) {
      cur += D[i];
      lst = min(lst, D[i]);
    }
    if (lst < -R) cur += (-lst) - R;
    REP(i, 0, K) if (s <= Q[i] and Q[i] <= s + L) cur -= C;
    ans = max(ans, cur);
  }
  cout << ans << "\n";
  return 0;
...
int body(istream& cin, ostream& cout) {
  ...
  CoordCompr cc;
  REP(i, 0, M) cc.add(P[i]);
  REP(i, 0, M) cc.add(P[i] - L);
  REP(i, 0, K) cc.qdd(Q[i]);
  REP(i, 0, K) cc.qdd(Q[i] - L);
  cc.add(0);
  cc.add(N - L);
  ll sz = cc.size();
