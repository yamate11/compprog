...
struct TS {
  ...
  TS(ll num) {
    sz = bit_ceil((u64)num);
    dat = vector<pbds_set>(2 * sz);
  }
  void add(ll p0, ll x) { 
    for (ll p = p0 + sz; p > 0; p /= 2) dat[p].insert(pll(x, p0));
  }
  void rem(ll p0, ll x) {
    for (ll p = p0 + sz; p > 0; p /= 2) {
      auto it = dat[p].find(pll(x, p0));
      assert(it != dat[p].end());
      dat[p].erase(it);
    }
  }
  ll query(ll left, ll right, ll x) {
    ...
    auto sub = [&](auto rF, ll idx, ll lo, ll hi) -> void {
      DLOGK(left, right, x, idx, lo, hi);
      if (hi <= left or right <= lo) ;
      else if (left <= lo and hi <= right) {
        ll z = dat[idx].order_of_key(pll(x + 1, -1));
        ret += dat[idx].size() - z;
      }else {
        ll mid = (lo + hi) / 2;
        rF(rF, 2*idx,     lo,  mid);
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) { ts.add(tr.euler_in(i), A[i]); }

  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll v, x; cin >> v >> x; v--;
      ts.rem(tr.euler_in(v), A[v]);
      A[v] = x;
      ts.add(tr.euler_in(v), A[v]);
    }else if (tp == 2) {
      ll v; cin >> v; v--;
      cout << ts.query(tr.euler_in(v), tr.euler_out(v), A[v]) << endl;
    }
  }
