...
int main(/* int argc, char *argv[] */) {
  ...
  REP(q, 0, Q) {
    ...
    if (tp == 1) {
      ll v, x; cin >> v >> x; v--;
      QS.emplace_back(1, v, x);
    }else if (tp == 2) {
      ll v; cin >> v; v--;
      QS.emplace_back(2, v, -1);
    }
  }

  TS ts(2*N);

  REP(i, 0, N) ts.decl_use(tr.euler_in(i), A[i]);
  for (auto [tp, v, x] : QS) {
    if (tp == 1) ts.decl_use(tr.euler_in(v), x);
  }
  ts.prepare_fwt();
  REP(i, 0, N) ts.mod(tr.euler_in(i), A[i], -1);
  for (auto [tp, v, x] : QS) {
    if (tp == 1) {
      ts.mod(tr.euler_in(v), x, A[v]);
      A[v] = x;
    }else if (tp == 2) {
      cout << ts.query(tr.euler_in(v), tr.euler_out(v), A[v]) << "\n";
    }
  }
