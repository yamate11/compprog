...
int main(/* int argc, char *argv[] */) {
  ...
  using DAT = pll;
  using OP = ll;
  pll unit_dat = pll(0, 0);
  ll unit_op = 0;
  auto add = [&](DAT x, DAT y) -> DAT { return DAT(x.first + y.first, x.second + y.second); };
  auto apply = [&](OP f, DAT x) -> DAT {
    return DAT(x.first + f * x.second, x.second);
  };
  vector<DAT> init_vec(N);
  REP(i, 0, N) init_vec[i] = pll(C[i], 1);
  auto st = make_seg_tree_lazy(unit_dat, unit_op, add, plus<ll>(), apply, init_vec);
