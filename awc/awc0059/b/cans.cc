...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [gTDQ6o3G]
  // @InpGrid(N, M, S) [kusn0W5g]
  auto S = vector(N, vector(M, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < M; j++) { ll v; cin >> v; S[i][j] = v; }
  // @End [kusn0W5g]

  vector P(N, 0LL);
  REP(i, 0, N) P[i] = accumulate(ALL(S[i]), 0LL);

  vector<ll> vec;
  REP(i, 0, N) {
    if (FORALL(j, 0, M, S[i][j] >= T[j])) vec.push_back(i);
  }
  DLOGK(vec);
  if (ssize(vec) >= K) {
    ranges::sort(vec, [&](ll i, ll j) { return P[i] > P[j]; });
    ll x = P[vec[K - 1]];
    vector<ll> tmp;
    for (ll i : vec) if (P[i] >= x) tmp.push_back(i);
    vec = move(tmp);
  }
  ranges::sort(vec);
