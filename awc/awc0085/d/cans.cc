...
int main(/* int argc, char *argv[] */) {
  ...
  */
  ...
  vector<sta> state(Q + 1);
  state[0] = sta{0, 0, M};
  REP(q, 1, Q + 1) {
    ll p; cin >> p;
    auto [old_b, old_l, old_r] = state[p];
    char c; cin >> c;
    ll d; cin >> d;
    ll b, l, r;
    if (c == 'R') {
  ...
  ll sz = cc.size();
  vector H(sz, 0LL);
  REP(i, 0, N) H[cc.c(A[i])]++;
  vector B(sz + 1, 0LL);
  REP(i, 0, sz) B[i + 1] = B[i] + H[i];
  REP(q, 1, Q + 1) {
    auto [b, l, r] = state[q];
    if (r <= l) cout << N << "\n";
    else {
      cout << N - (B[cc.c(r)] - B[cc.c(l)]) << endl;
    }
  }
