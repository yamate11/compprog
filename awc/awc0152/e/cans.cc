...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, fwd, dir=True, bwd=bwd, dec=1) [yFrQMfwF]
  ...
  // @End [yFrQMfwF]
  REP(i, 0, N) {
    fwd[i].emplace_back(i);
    bwd[i].emplace_back(i);
  }
#if 0
  REP(i, 0, N) {
    DLOGK(i, fwd[i]);
  ...
  vector C(N, vector(N, vector(2, false)));
  vector<sta> stack;
  vector ngcnt(N, vector(N, 0LL));
  REP(i, 0, N) {
    C[i][i][0] = C[i][i][1] = true;
    stack.emplace_back(i, i, 0);
    stack.emplace_back(i, i, 1);
  }
  while (not stack.empty()) {
    auto [i, j, turn] = stack.back(); stack.pop_back();
    if (turn == 0) {
      for (ll x : bwd[j]) {
        if (C[i][x][1] == false) {
          C[i][x][1] = true;
          stack.emplace_back(i, x, 1);
        }
      }
      /*
      if (C[i][j][1] == false) {
      ...
      */
      ...
      for (ll x : bwd[i]) {
        if (C[x][j][0] == false) {
          ngcnt[x][j] ++;
          if (ngcnt[x][j] == ssize(fwd[x])) {
            C[x][j][0] = true;
            stack.emplace_back(x, j, 0);
          }
        }
      }
    }
...
#if DEBUG
  REP(i, 0, N) REP(j, 0, N) REP(a, 0, 2) if (C[i][j][a]) DLOGK(i, j, a);
#endif
