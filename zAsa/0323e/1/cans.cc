...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, nbr, dec=1, read=S) [5K6y40di]
  struct nbr_t {
    int nd;
    ll S;
    nbr_t() {}
    nbr_t(int nd_, ll S_) : nd(nd_), S(S_) {}
  };
  auto nbr = vector(N, vector(0, nbr_t()));
  for (int i = 0; i < M; i++) {
    int u, v; cin >> u >> v; u -= 1; v -= 1;
    ll S; cin >> S;
    nbr[u].emplace_back(v, S);
    nbr[v].emplace_back(u, S);
  }
  // @End [5K6y40di]
  
  vector<ll> depth(N, -1LL);
  vector<ll> alpha(N);
  ll amax = LLONG_MAX;
  ll amin = LLONG_MIN;
  auto dfs = [&](auto rF, ll nd, ll pt, ll acc, ll d) -> void {
    depth[nd] = d;
    alpha[nd] = acc;
    if (d % 2 == 0) {
      amin = max(amin, -acc + 1);
    ...
    DLOGK(nd, acc, d, amin, amax);
    for (auto [cld, w] : nbr[nd]) {
      if (cld == pt) continue;
      if (depth[cld] < 0) {
        rF(rF, cld, nd, -acc + w, d + 1);
      }else {
        if ((depth[cld] + depth[nd]) % 2 != 0) {
          if (alpha[cld] + alpha[nd] == w) {
            ;
          }else {
            amax = -1;
          }
        }else {
          ll z = w - (alpha[nd] + alpha[cld]);
          if (z % 2 != 0) {
            amax = -1;
          }else {
            ll tv = z / 2;
            if (depth[nd] % 2 != 0) tv = -tv;
            amax = min(amax, tv);
            amin = max(amin, tv);
          }
        }
