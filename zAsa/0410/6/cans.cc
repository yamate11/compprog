...
int main(/* int argc, char *argv[] */) {
  ...
  ll A, K; cin >> A >> K;
  auto func = [&](ll lo, ll hi) -> ll {
    string hs = to_string(hi);
    string ls = to_string(lo);
    ll sz = SIZE(hs);
    ll z = SIZE(hs) - SIZE(ls);
    if (z > 1) return 1;
    if (z > 0) {
      string t(sz, '1');
      ll tt = stoll(t);
      if (tt <= hi) {
        DLOGKL("func A", lo, hi, 1);
        return 1;
      }
      string r(sz - 1, '9');
      ll rr = stoll(r);
      if (lo <= rr) {
        DLOGKL("func A2", lo, hi, 1);
        return 1;
      }
      DLOGKL("func B", lo, hi, 2);
      return 2;
    }
    // if (z > 0) ls = string(z, ' ') + ls;
    // DLOGK(hs, ls);
    vector<bool> used(10);
    ll p = 0;
    for (; p < sz and hs[p] == ls[p]; p++) {
      used[hs[p] - '0'] = true;
    }
    ll nu = 0;
    REP(i, 0, 10) if (used[i]) nu++;
    if (p == sz) {
      DLOGKL("func C", lo, hi, nu);
      return nu;
    }
    ll a = ls[p] - '0';
    ll b = hs[p] - '0';
    ...
    REP(c, a, b + 1) {
      REP(i, 0, 10) {
        ...
        if (lo <= ss and ss <= hi) {
          ll e = 0;
          if (not used[c]) e++;
          if (c != i and not used[i]) e++;
          mine = min(mine, e);
        }
      }
    }
    DLOGKL("func D", lo, hi, nu, mine);
    return nu + mine;
  };
  ...
  ll ans = binsearch_i<ll>(check, 1e16, -1);
  cout << ans << endl;
