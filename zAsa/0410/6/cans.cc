...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, P, type=Point) [NwYDkz41]
  ...
  REP(i, 0, N) REP(j, i + 1, N) {
    Point A = (P[i] + P[j]) / 2;
    Real cur = 0;
    REP(k, 0, N) updMax(cur, (P[k] - A).len());
    updMin(ans, cur);
  }
  REP(i, 0, N) REP(j, i + 1, N) REP(k, j + 1, N) {
    Point S = P[j] - P[i];
    Point T = P[k] - P[i];
    if (llround(S.x * T.y - S.y * T.x) == 0) continue;
    Point A = circumcenter(P[i], P[j], P[k]);
    Real cur = 0;
    REP(l, 0, N) updMax(cur, (P[l] - A).len());
    updMin(ans, cur);
  }
  cout << ans << endl;
  return 0;
