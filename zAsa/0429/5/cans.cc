...
int main(/* int argc, char *argv[] */) {
  ...
  ll A, B; cin >> A >> B; A--; B--;
  ll M; cin >> M;
  // @InpNbrList(N, M, nbr, dec=1) [c92b4GXc]
  ...
  vector<optional<Fp>> tbl(N);
  tbl[B] = Fp(1);
  auto g = [&](auto rF, ll nd) -> ll {
    optional<Fp>& ret = tbl[nd];
    if (not ret) {
      Fp v = 0;
      for (ll peer : nbr[nd]) {
        if (distB[peer] == distB[nd] - 1) v += rF(rF, peer);
      }
      ret = v;
    }
    return *ret;
