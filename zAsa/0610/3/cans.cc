...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpVec(M, T) [1MQgkRcD]
  auto T = vector(M, ll());
  for (int i = 0; i < M; i++) { ll v; cin >> v; T[i] = v; }
  // @End [1MQgkRcD]
  auto vec = rle(T);
  
  auto check = [&]() -> ll {
    ll ex0 = EXISTS(i, 0, N, S[i] == 0);
    ll ex1 = EXISTS(i, 0, N, S[i] == 1);

    if (ssize(vec) == 1 and vec[0].first == 0 and not ex0) return -1;
    if (ssize(vec) == 1 and vec[0].first == 1 and not ex1) return -1;
    if (ssize(vec) >= 2 and not ex0) return -1;
    if (ssize(vec) >= 2 and not ex1) return -1;
    
    ll fmove1 = 0, fmove2 = 0;
    if (N >= 2) {
      REP(i, 1, N) {
        fmove1++;
        if (S[i] != S[0]) break;
    ...
    DLOGK(fmove1, fmove2, fmove);
    ll ans = 0;
    if (S[0] == T[0]) {
      ans += vec[0].second + fmove + vec[1].second;
      DLOGK(ans);
      REP(i, 2, ssize(vec)) {
        ans += 1 + vec[i].second;
        DLOGK(i, ans);
