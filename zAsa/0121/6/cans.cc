...
int main(/* int argc, char *argv[] */) {
  ...
  vector ordV_init(N - 1, 0LL);
  REP(i, 0, N - 1) if (S[i] > S[i + 1]) ordV_init[i] = 1;
  auto ordV = make_seg_tree(0LL, plus<ll>(), ordV_init);
  auto dummy = make_seg_tree(0LL, plus<ll>(), vector<ll>(N));
  using sg_t = decltype(dummy);
  vector<sg_t> st(26, dummy);
  vector ccnt(26, 0LL);
  REP(i, 0, N) {
    ll d = S[i] - 'a';
    st[d].rs(i) = 1;
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      // update_ordV;
      if (x > 0)     ordV.rs(x - 1) = (S[x - 1] > c       ) ? 1 : 0;
      if (x < N - 1) ordV.rs(x)     = (c        > S[x + 1]) ? 1 : 0;
      ...
      auto check = [&]() -> bool {
        ...
        if (ordV.query(l, r - 1) > 0) return false;
        ll p = S[l] - 'a';
        ll q = S[r - 1] - 'a';
        REP(t, p + 1, q) {
          if (st[t].query(l, r) < ccnt[t]) return false;
        }
        return true;
      };
      cout << (check() ? "Yes\n" : "No\n");
