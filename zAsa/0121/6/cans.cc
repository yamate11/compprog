...
// @@ !! LIM(digit f:updMaxMin perm forall debug cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = 1e18;
  REP(x, 1, A * 10 + 1) {
    if (numUsed(x) <= K) { updMin(ans, abs(x - A)); }
  }
  cout << ans << endl;
  return 0;
}
...
int body(istream& cin, ostream& cout) {
  ...
  IntComb ic(10, K);
  while (ic.get()) {
    vector enable(10, false);
    REP(i, 0, K) enable[ic[i]] = true;
    auto get_lower = [&](ll t) -> ll {
      REPrev(s, t, 0) if (enable[s]) return s;
      return -1;
    };
    auto get_higher = [&](ll t) -> ll {
      REP(s, t, 10) if (enable[s]) return s;
      return -1;
    };

    auto f = [&](auto rF, auto v) -> ll {
      DLOGKL("called", v);
      ll i = ssize(v);
      if (i == w0) return 0;
      ll ret = 1e18;
      ...
      if (enable[d0]) {
        auto v1 = v;
        v1.push_back(d0);
        updMin(ret, rF(rF, move(v1)));
      }
      ll y = get_lower(d0 - 1);
      if (y >= 0) {
        auto v1 = v;
        v1.push_back(y);
        REP(j, i + 1, w0) v1.push_back(ic[K - 1]);
        DLOGKL("-a-", v, v1);
        updMin(ret, A - du.from_vector(v1));
      }else if (i == 0 and w0 >= 2) {
        vector<ll> v1(w0 - 1);
        REP(j, 0, w0 - 1) v1[j] = ic[K - 1];
        DLOGKL("-b-", v, v1);
        updMin(ret, A - du.from_vector(v1));
      }
      ll z = get_higher(d0 + 1);
      if (z >= 0) {
        auto v1 = v;
        v1.push_back(z);
        REP(j, i + 1, w0) v1.push_back(ic[0]);
        DLOGKL("-c-", v, v1);
        updMin(ret, du.from_vector(v1) - A);
      }
      DLOGKL("ret", v, ret);
      return ret;
    };
    updMin(ans, f(f, vector<ll>{}));
  }
  cout << ans << endl;
