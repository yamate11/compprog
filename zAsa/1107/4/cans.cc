...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpVec(N, S, type=string) [R5Ub2siZ]
  auto S = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; S[i] = v; }
  // @End [R5Ub2siZ]
  
  ll big = 1e18;
  vector dist(N, vector(N, IPoint(big, 0)));

  REP(i, 0, N) dist[i][i] = IPoint(0, 0);
  REP(i, 0, N) REP(j, 0, N) if (S[i][j] == 'Y') dist[i][j] = IPoint(1, 0);
#if DEBUG
  REP(i, 0, N) REP(j, 0, N) DLOGK(i, j, dist[i][j]);
#endif
  REP(k, 0, N) {
    REP(i, 0, N) REP(j, 0, N) {
      if (i == k or j == k) continue;
      updMin(dist[i][j], dist[i][k] + dist[k][j] + IPoint(0, -A[k]));
    }
#if DEBUG
    REP(i, 0, N) REP(j, 0, N) DLOGK(k, i, j, dist[i][j]);
#endif
  }
