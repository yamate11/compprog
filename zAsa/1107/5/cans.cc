...
int main(/* int argc, char *argv[] */) {
  ...
  REP(nd, 0, N) {
    vector<ll> W;
    for (ll c : tr.children(nd)) W.push_back(1 + tr.num_children(c));
    if (nd != 0) {
      ll pt = tr.parent(nd);
      if (pt == 0) W.push_back(1 + tr.num_children(pt) - 1);
      else         W.push_back(1 + tr.num_children(pt) - 1 + 1);
    }
    ranges::sort(W, greater<>());
    ll val = 0;
    REP(i, 0, ssize(W)) val = max(val, 1 + (i + 1) * W[i]);
    DLOGK(nd, W, val);
