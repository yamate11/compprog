...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, L; cin >> N >> M >> L;
  using state = tuple<multiset<ll>, multiset<ll>, multiset<ll>>;
  map<state, bool> tbl;
  auto f = [&](auto rF, const state& s) -> bool {
    auto [it, b] = tbl.try_emplace(s, false);
    if (b) {
      const auto& [msP, msO, msS] = s;
      auto check = [&]() -> bool {
        for (auto it0 = msP.begin(); it0 != msP.end(); it0++) {
          auto msP1 = msP;
          auto msS1 = msS;
          ll x = *it0;
          ll y = -1;
          auto it1 = msP1.find(x);
          auto it2 = msS1.lower_bound(x);
          if (it2 != msS1.begin()) {
            it2--;
            y = *it2;
            msS1.erase(it2);
          ...
          msP1.erase(it1);
          msS1.insert(x);
          if (rF(rF, state{msO, move(msP1), move(msS1)})) return false;
        }
        return true;
      };
      it->second = check();
  ...
  multiset<ll> msT, msA, msS;
  REP(i, 0, N) { ll a; cin >> a; msT.insert(a); }
  REP(i, 0, M) { ll a; cin >> a; msA.insert(a); }
  REP(i, 0, L) { ll a; cin >> a; msS.insert(a); }
  if (f(f, state{msT, msA, msS})) {
    cout << "Aoki\n";
  }else {
    cout << "Takahashi\n";
  }
