...
// @@ !! LIM(debug cmpNaive f:perfmeas sortedIndex f:intDiv binsearch)
...
int naive(istream& cin, ostream& cout) {
  ...
  REP(_q, 0, Q) {
    ...
    ans = f((alpha ^ ans) - 1, beta ^ ans, gamma ^ ans);
    // ans = f(alpha - 1, beta, gamma);
    cout << ans << "\n";
  }
...
int body(istream& cin, ostream& cout) {
  ...
  // ll big = 1e18;
  auto B = sortedIndex(A);

  double t1 = get_time_sec();
  
  ll bsize = (ll)sqrt(N);
  ll numb = divCeil(N, bsize);
  vector C(numb + 1, vector(N + 1, 0LL));
  REP(ii, 0, N) {
    ll i = B[ii];
    ll e = ii / bsize;
    C[e + 1][i + 1] = A[i];
  }
  REP(e, 1, numb + 1) REP(i, 1, N + 1) C[e][i] += C[e][i - 1];
  REP(e, 1, numb + 1) REP(i, 1, N + 1) C[e][i] += C[e - 1][i];

  /*
  vector D(num + 1);
  REP(i, 0, num) D[i] = A[B[i * bsize]];
  D[num] = big;
  */
  ...
  auto f = [&](ll l, ll r, ll x) -> ll {
    auto check = [&](ll e) -> bool { return A[B[e * bsize]] <= x; };
    ll p, q;
    ll e = binsearch<ll>(check, -1, numb);
    if (e < 0) {
      p = q = 0;
    }else {
      p = C[e][r] - C[e][l];
      q = 0;
      REP(ii, e * bsize, (e + 1) * bsize) if (ii < N) {
        ll i = B[ii];
        if (l <= i and i < r and A[i] <= x) q += A[i];
      }
      DLOGK(bsize, e, C[e], p, q);
    }
    return p + q;
  };
