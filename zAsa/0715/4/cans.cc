...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [FYRdDaXU]
  // @InpVec(M, B, dec=0) [BcnNEtUR]
  auto B = vector(M, ll());
  for (int i = 0; i < M; i++) { ll v; cin >> v; v -= 0; B[i] = v; }
  // @End [BcnNEtUR]

  auto mymax = [](ll a, ll b) -> ll { return max(a, b); };
  auto segt = make_seg_tree_lazy(0LL, 0LL, mymax, plus<ll>(), plus<ll>(), A);
  REP(i, 0, M) {
    ll a = segt.at(B[i]);
    segt.rs(B[i]) = 0;
    ll p = a / N;
    ll q = a % N;
    if (p > 0) segt.update(0, N, p);
    ll r = min(N - B[i] - 1, q);
    if (r > 0) segt.update(B[i] + 1, B[i] + 1 + r, 1);
    if (q - r > 0) segt.update(0, q - r, 1);
  }
  REPOUT(i, 0, N, segt.at(i), " ");
