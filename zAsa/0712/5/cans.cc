...
int main(/* int argc, char *argv[] */) {
  ...
  ll M, N; cin >> M >> N;
  // @InpVec(M, X, dec=1) [ZP8inGzZ]
  auto X = vector(M, ll());
  for (int i = 0; i < M; i++) { ll v; cin >> v; v -= 1; X[i] = v; }
  // @End [ZP8inGzZ]
  vector release(M, vector<ll>());
  REP(i, 0, M) release[X[i]].push_back(i);

  vector fnext(1LL << M, vector<ll>());
  REP(x, 0, 1LL << M) {
    REP(i, 0, M) {
      if (x >> i & 1) continue;
      ll y = x | (1LL << i);
      for (ll j : release[i]) y &= ~(1LL << j);
      fnext[x].push_back(y);
    }
  }
  vector tbl_init(1LL << M, Fp(0));
  auto tbl = tbl_init;
  tbl[0] = 1;
  REP(i, 0, N) {
    auto prev = move(tbl);
