...
// @@ !! LIM -- end mark --
  ...
  struct nbr_t {
    ...
    friend ostream& operator<<(ostream& ostr, const nbr_t& t) {
      ostr << "(" << t.nd << ", " << t.C << ")";
      return ostr;
    }
  };
...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpNbrList(N, M, nbr, dir=True, dec=1, read=C, ostr=True) [Oy6nUsA5]
  ...
  while (idp.get()) {
    auto check = [&]() -> ll {
      ...
      ll c = 0;
      REP(i, 0, L) {
        DLOGKL("  ", i, idp[i], c, nbr[c]);
        if (ssize(nbr[c]) <= idp[i]) return -1;
        auto p = nbr[c][idp[i]];
        len += p.C;
        c = p.nd;
        DLOGKL("    ", p, len, c);
