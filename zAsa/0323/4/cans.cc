...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpNbrList(N, M, nbr, dec=1) [9Qecdkcw]
  ...
  vector tbl_init(N + 1, sta(Fp(0), Fp(0)));
  auto tbl = tbl_init;
  tbl[S] = sta(Fp(1), Fp(0));
  REP(i, 0, K) {
    auto prev = move(tbl);
    ...
    REP(j, 0, N) {
      for (ll p : nbr[j]) {
        if (p == X) {
          tbl[p].first += prev[j].second;
          tbl[p].second += prev[j].first;
        }else {
          tbl[p].first += prev[j].first;
          tbl[p].second += prev[j].second;
        }
      }
    }
  }
  cout << tbl[T].first << endl;
