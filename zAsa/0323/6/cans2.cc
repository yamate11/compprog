...
// @@ !! LIM(board forall debug unordered_map)
...
int main(/* int argc, char *argv[] */) {
  ...
  cin >> brd;
  BrdIdx start;
  REP(r, 0, H) REP(c, 0, W) if (brd.at(r, c) == 'T') start = BrdIdx(r, c);
  
  vector goal(H + 1, vector(H + 1, vector(W + 1, vector(W + 1, false))));
  REP(r0, 0, H + 1) REP(r1, 0, H + 1) REP(c0, 0, W + 1) REP(c1, 0, W + 1) {
    goal[r0][r1][c0][c1] = FORALL(r, r1, H - r0, FORALL(c, c1, W - c0, brd.at(r, c) != '#'));
  }

  ll big = 1000;
  vector dist(2*H + 1, vector(2*W + 1, vector(H + 1, vector(H + 1, vector(W + 1, vector(W + 1, big))))));
  using sta = tuple<BrdIdx, ll, ll, ll, ll>;
  queue<sta> que;
  dist[H][W][0][0][0][0] = 0;
  que.emplace(BrdIdx(0, 0), 0, 0, 0, 0);
  while (not que.empty()) {
    auto [bd, r0, r1, c0, c1] = que.front(); que.pop();
    ll d = dist[H + bd.r][W + bd.c][r0][r1][c0][c1];
    auto bi = start + bd;
    DLOGKL("popped", bd, bi, r0, r1, c0, c1, d);
    if (goal[r0][r1][c0][c1]) {
      cout << d << endl;
      return 0;
    }
    for (auto dir : BrdIdx::nbr4) {
      auto bj = bi + dir;
      bool okr = bj.r < r1 or H - r0 <= bj.r;
      bool okc = bj.c < c1 or W - c0 <= bj.c;
      if (okr or okc or brd.at(bj) != '#') {
        DLOGKL("ok", bj);
        ll new_r0 = max(r0, (ll)(start.r - bj.r));
        ll new_r1 = max(r1, (ll)(bj.r - start.r));
        ll new_c0 = max(c0, (ll)(start.c - bj.c));
        ll new_c1 = max(c1, (ll)(bj.c - start.c));
        auto new_bd = bj - start;
        ll& new_d = dist[H + new_bd.r][W + new_bd.c][new_r0][new_r1][new_c0][new_c1];
        DLOGKL("  ", new_bd, new_r0, new_r1, new_c0, new_c1, new_d);
        if (new_d == big) {
          new_d = d + 1;
          que.emplace(new_bd, new_r0, new_r1, new_c0, new_c1);
          DLOGKL("push", bj, new_bd, new_r0, new_r1, new_c0, new_c1, new_d);
        }
      }else {
        DLOGKL("ng", bj);
      }
