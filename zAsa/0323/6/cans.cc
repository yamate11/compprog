...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  auto enc = [&](ll r, ll c) -> ll { return W * r + c; };
  // auto dec = [&](ll e) -> pll { return pll(e / W, e % W); };
  auto goal = [&](const auto& bs) -> bool {
    REP(e, 0, H * W) if (bs[e]) return false;
    return true;
  };
  mybs bs0;
  ll sr, sc;
  REP(i, 0, H) {
    ...
    REP(j, 0, W) {
      if (s[j] == 'T') { sr = i; sc = j; }
      else if (s[j] == '#') bs0[enc(i, j)] = true;
    }
  }
  const vector<pll> dirs{{1, 0}, {-1, 0}, {0, 1}, {0, -1}};
  unordered_map<mybs, ll> dist;
  queue<mybs> que;
  dist[bs0] = 0;
  que.emplace(bs0);
  while (not que.empty()) {
    auto bs = que.front(); que.pop();
    ll d = dist[bs];
    if (goal(bs)) {
      cout << d << endl;
      return 0;
    ...
    for (auto [dr, dc] : dirs) {
      if (0 <= sr - dr and sr - dr < H and bs[enc(sr - dr, sc)]) continue;
      if (0 <= sc - dc and sc - dc < W and bs[enc(sr, sc - dc)]) continue;
      mybs new_bs;
      REP(r, 0, H) REP(c, 0, W) if (0 <= r + dr and r + dr < H and 0 <= c + dc and c + dc < W) {
        new_bs[enc(r + dr, c + dc)] = bs[enc(r, c)];
      }
      auto [it, b] = dist.try_emplace(new_bs, d + 1);
      if (b) {
        que.emplace(new_bs);
      }
    }
