...
int main(/* int argc, char *argv[] */) {
  ...
  auto st1 = make_seg_tree(-big, mymax, iv1);
  auto st2 = make_seg_tree(-big, mymax, iv2);

  ll M; cin >> M;
  REP(_m, 0, M) {
    ll t, p; cin >> t >> p; t--;
    ll u1 = st1.query(0, t + 1) - st1.at(t);
    ll u2 = st2.query(t, N) - st2.at(t);
    U[t] += max(u1, u2) + p;
    st1.rs(t) = U[t] + C * t;
    st2.rs(t) = U[t] - C * t;
    DLOGK(U);
    DLOGK(_m, st1.vec_view());
    DLOGK(_m, st2.vec_view());
  }
  
  ll ans = *ranges::max_element(U);
  cout << ans << endl;
