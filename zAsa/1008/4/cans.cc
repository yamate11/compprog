...
int main(/* int argc, char *argv[] */) {
  ...
  using sta = vector<ll>;
  vector<sta> vec(N);
  REP(i, 0, N) vec[i].push_back(i);
  REP(_q, 0, Q) {
    ll tp; cin >> tp;
    if (tp == 1) {
      ll uu, vv; cin >> uu >> vv; uu--; vv--;
      DLOGKL("merge", uu, vv);
      ll u = uf.leader(uu);
      ll v = uf.leader(vv);
      if (u != v) {
        ll a = uf.merge(u, v);
        ll b = a == u ? v : u;
        while (not vec[b].empty()) {
          ll x = vec[b].back(); vec[b].pop_back();
          vec[a].push_back(x);
        }
        DLOGK(tp, a, b, vec[a]);
      }
      ...
      ll v, k; cin >> v >> k; v--;
      ll a = uf.leader(v);
      DLOGK(tp, v, k, a, vec[a]);
      ranges::sort(vec[a], greater<ll>());
      if (ssize(vec[a]) > 10) vec[a].resize(10);
      if (ssize(vec[a]) >= k) {
        cout << vec[a][k - 1] + 1 << "\n";
      }else {
        cout << -1 << "\n";
