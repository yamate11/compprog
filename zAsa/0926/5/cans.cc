...
// @@ !! LIM(input binsearch forall debug cmpNaive perm)
...
int naive(istream& cin, ostream& cout) {
  ...
  // @End [jiUtCKLM]
  // @InpNbrList(N, M, nbr, dec=1) [NVlc7tgj]
  ...
  while (ip.get()) {
    ...
    REP(ii, 0, N) {
      ...
      ll cost = 0;
      for (ll j : nbr[i]) {
        if (not erased[j]) cost += A[j];
      }
      thiscost = max(cost, thiscost);
      erased[i] = true;
    }
...
int body(istream& cin, ostream& cout) {
  ...
  auto check = [&](ll maxcost) -> bool {
    vector<ll> cost(N);
    REP(i, 0, N) for (ll j : nbr[i]) cost[i] += A[j];
    DLOGK(cost);
    vector<bool> registered(N);
    vector<ll> cands;
    REP(i, 0, N) if (cost[i] <= maxcost) {
      cands.push_back(i);
      registered[i] = true;
    }
    ...
    return FORALL(i, 0, N, registered[i]);
  };
  ll maxcost = binsearch<ll>(check, 1e18, -1);
  cout << maxcost << endl;
