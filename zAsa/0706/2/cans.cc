...
int main(/* int argc, char *argv[] */) {
  ...
  while (true) {
    ...
    char c = brd.at(bi);
    BrdIdx dir;
    if (c == 'U') dir = BrdIdx{-1, 0};
    else if (c == 'D') dir = BrdIdx{ 1, 0};
    else if (c == 'L') dir = BrdIdx{0, -1};
    else if (c == 'R') dir = BrdIdx{0,  1};
    else break;
    if (not brd.in(bi + dir)) {
      cout << bi.r + 1 << " " << bi.c + 1 << endl;
      return 0;
    }
    if (visited.at(bi + dir)) {
      cout << -1 << endl;
      return 0;
    }
    bi = bi + dir;
