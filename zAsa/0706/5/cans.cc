...
int naive(istream& cin, ostream& cout) {
  ...
  // @InpVec(M, X, dec=1) [QZ3Ki2jU]
  auto X = vector(M, ll());
  for (int i = 0; i < M; i++) { ll v; cin >> v; v -= 1; X[i] = v; }
  // @End [QZ3Ki2jU]
  
  vector tbl(M, vector(A + 1, vector(B + 1, vector(1LL << N, -1LL))));
  auto f = [&](auto rF, ll i, ll a, ll b, u64 x) -> ll {
    if (i == M) return popcount(x);
    ll& r = tbl[i][a][b][x];
    if (r < 0) {
      ll j = X[i];
      if (x >> j & 1) {
        auto getOptA2 = [&]() -> ll {
          ll y = x ^ (1LL << j);
          ll s1 = rF(rF, i + 1, a - 1, b, y);
          ll s2 = b == 0 ? 0 : rF(rF, i + 1, a - 1, b - 1, x);
          return max(s1, s2);
        };
        ll r1 = rF(rF, i + 1, a, b, x);
        ll r2 = a == 0 ? N : getOptA2();
        r = min(r1, r2);
      }else {
        r = rF(rF, i + 1, a, b, x);
      }
