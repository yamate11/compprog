...
int naive(istream& cin, ostream& cout) {
  ...
  IntPerm ip(N, 4);
  while (ip.get()) {
    if (FORALL(i, 0, 3, D[ip[i + 1]] >= 2 * D[ip[i]])) ans++;
  }
  cout << ans << endl;
  return 0;
}
int body(istream& cin, ostream& cout) {
  ...
  ranges::sort(D);
  auto f = [&](const auto& vec) -> vector<Fp> {
    vector<Fp> acc(N + 1);
    REPrev(i, N - 1, 0) acc[i] = acc[i + 1] + vec[i];
    vector<Fp> ret(N);
    REP(i, 0, N) {
      ll j = ranges::lower_bound(D, 2 * D[i]) - D.begin();
      ret[i] = acc[j];
    }
    return ret;
  };
