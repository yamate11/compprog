...
// @@ !! LIM(intervalSet cmpNaive debug)
...
itv_set<res_t> itv_apply(auto f, const auto& x, const auto& y) {
  return itv_apply_body<typename remove_reference<decltype(x)>::type::value_type, typename remove_reference<decltype(y)>::type::value_type, res_t, decltype(f)>(f, x, y);
}
*/
...
int naive(istream& cin, ostream& cout) {
  ...
  REP(i, 0, N) {
    REP(j, L[i], R[i]) {
      ll k = lower_bound(ALL(vec), j) - vec.begin();
      if (k == SIZE(vec)) vec.push_back(j);
      else vec[k] = j;
    }
  }
  cout << SIZE(vec) << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  itv_set<ll> is;
  REP(i, 0, N) {
    auto phase_erase = [&]() -> void {
      ll k = R[i] - L[i];
      ll x;
      auto [l, r, b] = is.get(L[i]);
      if (b == 0) {
        if (r >= big) return;
        auto [l1, r1, b1] = is.get(r);
        x = l1;
        r = r1;
      }else {
        x = L[i];
      ...
      while (true) {
        if (k <= r - x) {
          ...
          is.put(x, r, 0);
          k -= r - x;
          auto [l1, r1, b1] = is.get(x);
          if (r1 >= big) return;
          x = r1;
          auto [l2, r2, b2] = is.get(x);
          r = r2;
        }
      }
    };
