...
int main(/* int argc, char *argv[] */) {
  ...
  mp[pll(1, 0)] = 0;
  REP(i, 0, N) {
    if (A[i] == 0 and B[i] == 0) mp[pll(0, 0)]++;
    else if (A[i] == 0) mp[pll(0, 1)]++;
    else if (B[i] == 0) mp[pll(1, 0)]++;
    else {
      ll a = A[i], b = B[i];
      ll g = gcd(a, b);
      a /= g;
  ...
  auto f = [&](ll n, ll m) -> Fp {
    Fp ret = 0;
    if (n == 0 and m == 0) ret = Fp(1);
    else if (n == 0) ret = power<Fp>(2, m);
    else if (m == 0) ret = power<Fp>(2, n);
    else ret = power<Fp>(2, n) + power<Fp>(2, m) - 1;
    DLOGKL("f", n, m, ret);
    return ret;
  };
