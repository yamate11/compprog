...
int main(/* int argc, char *argv[] */) {
  ...
  ll T, M; cin >> T >> M;
  Fp::setMod(M);
  auto primes = sieve(5000 + 2 * sqrt(M));
  auto pr = prfac(M, primes);
  vector<pll> P;
  for (auto [p, r] : pr) {
    ll q = 1;
    REP(z, 0, r) q *= p;
  ...
  auto solve = [&]() -> ll {
    ...
    ll totC = accumulate(ALL(C), 0LL);
    vector<ll> D(totC + 1, 0LL);
    D[totC] = 1;
    REP(i, 0, N) D[C[i]]--;
    REPrev(i, totC - 1, 0) D[i] += D[i + 1];

    vector<ll> E(totC + 1);
    REP(n, 1, totC + 1) E[n] = n;
    vector<ll> cnt(ssize(P));
    REP(i, 0, ssize(P)) {
      auto [p, q] = P[i];
      for (ll z = p; z <= totC; z += p) {
        while (E[z] % p == 0) {
          cnt[i] += D[z];
          E[z] /= p;
        }
    ...
    REP(i, 0, ssize(P)) {
      assert(cnt[i] >= 0);
      REP(j, 0, cnt[i]) ans *= P[i].first;
    }
    REP(n, 2, totC + 1) {
      if (D[n] != 0) {
        Fp x = (D[n] > 0) ? Fp(E[n]) : 1 / Fp(E[n]);
        REP(j, 0, abs(D[n])) ans *= x;
      }
    }
    return ans;
  };
