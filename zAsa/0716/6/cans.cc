...
// @@ !! LIM(f:intDiv forall cmpNaive debug)
...
int body(istream& cin, ostream& cout) {
  ...
  for (int i = 0; i < N; i++) { ll v; cin >> v; A[i] = v; }
  // @End [nKrmsX0m]
  if (A[0] < 0) REP(i, 0, N) A[i] = -A[i];
  ll tot = accumulate(ALL(A), 0LL);
  vector<ll> X(N);
  REP(i, 0, N) X[i] = i;
  ll base = 0;
  REP(i, 0, N) base += X[i] * A[i];
  DLOGK(base);
  if (base != 0) {
    if (base > 0) {
      X[0] -= base;
    }else if (tot != 0) {
      ll d = divCeil(-base, abs(tot));
      ll r = d * abs(tot) - (-base);
      REP(i, 0, N) X[i] += (tot > 0) ? d : -d;
      X[0] -= r;
    }else {
      vector<ll> ht(N + 1);
      REP(i, 0, N) ht[i + 1] = ht[i] + A[i];
      assert(ht[1] > 0 and ht[N] == 0);
      ll i0 = -1, i1 = -1;
      REP(i, 0, N + 1) {
        if (i0 == -1 and ht[i] < 0) i0 = i - 1;
        else if (i0 >= 0 and i1 < 0 and ht[i] == 0) i1 = i - 1;
      }
      if (i0 < 0) {
        cout << "No\n";
        return 0;
