...
int main(/* int argc, char *argv[] */) {
  ...
  auto mygcd = [&](ll a, ll b) -> ll { return gcd(a, b); };
  auto segtA = make_seg_tree(0LL, mygcd, DA);
  auto segtB = make_seg_tree(0LL, mygcd, DB);
  REP(_q, 0, Q) {
    ll h1, h2, w1, w2; cin >> h1 >> h2 >> w1 >> w2; h1--; w1--;
    ll pa = segtA.query(h1, h2 - 1);
    ll pb = segtB.query(w1, w2 - 1);
    ll ans = gcd(A[h1] + B[w1], gcd(pa, pb));
    cout << ans << "\n";
  }
