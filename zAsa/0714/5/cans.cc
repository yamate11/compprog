...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [NS9t29VE]
  vector<Fp> S(N + 1, Fp(0));
  REP(i, 0, N) S[i + 1] = S[i] + A[i];
  DLOGK(S);
  vector T(K + 1, vector(N + 1, Fp(1)));
  REP(k, 1, K + 1) {
    REP(i, 0, N + 1) T[k][i] = T[k - 1][i] * S[i];
    DLOGK(k, T[k]);
  }
  vector U(K + 1, vector(N + 1, Fp(0)));
  REP(k, 0, K + 1) {
    REP(i, 0, N) U[k][i + 1] = U[k][i] + T[k][i];
    DLOGK(k, U[k]);
  }
  
  
  Fp ans = 0;
  REP(r, 1, N + 1) REP(k, 0, K + 1) {
    Fp sgn = (k % 2 == 0) ? Fp(1) : Fp(-1);
    Fp v = cb.binom(K, k) * U[k][r] * T[K - k][r];
    DLOGK(r, k, sgn, v);
    ans += sgn * v;
  }
  cout << ans << endl;
