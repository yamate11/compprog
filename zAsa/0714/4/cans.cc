  ...
  auto solve = [&]() -> void {
    ...
    auto f = [&](ll s) -> vector<ll> {
      ...
      while (true) {
        ...
        ret.push_back(x);
        if (s - x >= K) return ret;
        if (ssize(ret) >= N or x <= 2) return vector<ll>();
      }
    };
    auto check = [&](ll s) -> bool {
      return not f(s).empty();
    };
    ll s = binsearch<ll>(check, 3 * (K + 10), K);
    auto v = f(s);
    if (ll a = N - ssize(v); a > 0) {
      ll z = v.back();
      REP(i, 0, a) v.push_back(z);
    }
    ranges::reverse(v);
    REPOUT(i, 0, N, v[i], " ");
  };
