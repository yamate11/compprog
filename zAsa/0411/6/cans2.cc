...
int body(istream& cin, ostream& cout) {
  ...
  vector tbl(S + 1, vector(S + 1, Fp(0)));
  tbl[0][0] = 1;
  REPrev(n, S, 1) {
    if (cnt[n] == 0) continue;
    ll m = S / n;
    REPrev(k, m, 0) {
      REP(i, 0, S) {
        if (tbl[k][i] == 0) continue;
        ll lim = min((S - i) / n, cnt[n]);
        REP(z, 1, lim + 1) {
          tbl[k + z][i + z * n] += tbl[k][i] * cb.binom(cnt[n], z);
        }
      }
    }
  }
