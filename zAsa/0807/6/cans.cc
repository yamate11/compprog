...
// @@ !! LIM(f:intDiv cmpNaive perm debug unordered_map f:updMaxMin)
...
int body(istream& cin, ostream& cout) {
  ...
  safe_umap<ll, ll> cnt;
  ll mc = 0;
  REP(i, 0, N) updMax(mc, ++cnt[A[i]]);
  vector<ll> B;
  for (auto [i, x] : cnt) if (x == mc) B.push_back(i);
  ranges::sort(B);
  DLOGK(mc, cnt, B);
  ll ans;
  if (ssize(B) == 1) ans = (N - mc) * K;
  else {
    ll gap = B[ssize(B) - 1] - B[0];
    REP(i, 0, ssize(B)) {
      if (i < ssize(B) - 1) gap = min(gap, K - (B[i + 1] - B[i]));
    }
    ans = (N - mc) * K - gap;
  }
  cout << ans << "\n";
