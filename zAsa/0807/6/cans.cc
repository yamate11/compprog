...
int body(istream& cin, ostream& cout) {
  ...
  vector PT(N, vector<ll>());
  REP(i, 0, N) PT[T[i]].push_back(D[i]);
  REP(i, 0, N) sort(ALL(PT[i]), greater<ll>());
  using sta = tuple<ll, ll, ll>;
  vector<sta> A;
  REP(i, 0, N) {
    REP(j, 0, ssize(PT[i])) A.emplace_back(PT[i][j], i, j);
  }
  sort(ALL(A), [&](sta t1, sta t2) -> bool {
    auto [v1, tp1, ord1] = t1;
    auto [v2, tp2, ord2] = t2;
  ...
  ll ans = op + nol * nol;
  DLOGK(A, onlist, op, nol);
  ll p = K, q = K - 1;
  while (true) {
    for (; p < N; p++) {
      auto [v1, tp1, ord1] = A[p];
      if (not onlist[tp1]) {
        onlist[tp1] = true;
        break;
      }
