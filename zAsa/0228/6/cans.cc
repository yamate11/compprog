...
// @@ !! LIM(geometry f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, P, type=Point) [WrfFbhi8]
  ...
  REP(i, 1, N) {
    ...
    REP(j, j0, i) {
      REP(k, 0, K) {
        if (k + i - 1 - j < K) updMin(tbl[i][k + i - 1 - j], tbl[j][k] + (P[i] - P[j]).len());
      }
    }
    DLOGK(i, tbl[i]);
  }
  Real ans = tbl[N - 1][0];
  REP(k, 1, K) updMin(ans, tbl[N - 1][k] + (1LL << (k - 1)));
  cout << ans << endl;
  return 0;
}
