...
int main(/* int argc, char *argv[] */) {
  ...
  ll M = ssize(RA1);
  vector<pll> RA(M + 1);
  RA[0] = pll(RA1[0].first, 0);
  REP(i, 1, M) RA[i] = pll(RA1[i].first, RA[i - 1].second + RA1[i - 1].second);
  RA[M] = pll(big, RA[M - 1].second + RA1[M - 1].second);
  DLOGK(RA1, RA);
  ll ans = 0;
  REP(i, 0, M) {
    auto [a, n] = RA1[i];
    ...
    if (i + 1 < M) {
      ll p = RA[i + 1].second;
      for (ll j = 1; ; j++) {
        auto it = lower_bound(ALL(RA), pll(a * (j + 1), 0));
        auto [_x, y] = *it;
        ans += n * j * (y - p);
        if (it - RA.begin() == M) break;
        p = y;
      }
    }
    DLOGK(i, ans);
