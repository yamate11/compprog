...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [JIO2TBqB]
  // @InpMVec(M, ((A, dec=1), (B, dec=1), Z)) [afBV2eAr]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  auto Z = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  ll big = 1e18;
  auto func = [&](ll mode) -> ll {
    vector<tuple<ll, ll, ll>> vec;
    if (mode >> 0 & 1) {
      REP(i, 0, N) vec.emplace_back(X[i], i, N);
    }
    if (mode >> 1 & 1) {
      REP(i, 0, N) vec.emplace_back(Y[i], i, N + 1);
    }
    REP(i, 0, M) vec.emplace_back(Z[i], A[i], B[i]);
    sort(ALL(vec));
    UnionFind uf(N + 2);
    ll cost = 0;
    for (auto [c, u, v]: vec) {
      if (uf.leader(u) == uf.leader(v)) continue;
      cost += c;
      uf.merge(u, v);
    }
    if (FORALL(i, 0, N, uf.leader(i) == uf.leader(0))) return cost;
    else return big;
  };
  ll ans = min(func(0), min(func(1), min(func(2), func(3))));
  cout << ans << endl;
