...
int main(/* int argc, char *argv[] */) {
  ...
  auto nbr = [&](ll t, ll r, ll c) -> vector<sta> {
    vector<sta> ret;
    if (t == 4) {
      REP(tt, 0, 4) ret.emplace_back(tt, r, c);
    }else {
      BrdIdx bi(r, c);
      BrdIdx bj = bi + BrdIdx::nbr4[t];
      if (brd.at(bj) == '#') ret.emplace_back(4, r, c);
      else ret.emplace_back(t, bj.r, bj.c);
    }
    return ret;
  };

  vector visited(5, vector(N, vector(M, false)));
  visited[4][1][1] = true;
  queue<sta> que;
  que.emplace(4, 1, 1);
  while (not que.empty()) {
    auto [t, r, c] = que.front(); que.pop();
    for (auto [t1, r1, c1] : nbr(t, r, c)) {
      if (not visited[t1][r1][c1]) {
        visited[t1][r1][c1] = true;
        que.emplace(t1, r1, c1);
      }
    }
  }
  ll ans = 0;
  REP(r, 0, N) REP(c, 0, M) {
    if (EXISTS(t, 0, 5, visited[t][r][c])) ans++;
  }
  cout << ans << endl;
