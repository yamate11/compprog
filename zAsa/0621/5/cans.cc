...
int main(/* int argc, char *argv[] */) {
  ...
  ll R, C, K; cin >> R >> C >> K;
  Board<char> brd(R, C, 'x');
  cin >> brd;

  vector L(R, vector(C, 0LL));
  REP(i, 0, R) {
    REP(j, 0, C) {
      ll x = EXFIND(k, 0, C + 1, brd.at(i, j + k) == 'x');
      L[i][j] = x;
    }
    DLOGK(L[i]);
  }
  
  auto check = [&](ll i, ll j, ll d, ll p) -> bool {
    if (d == 0) return L[i][j] >= 2*K - 1;
    ll i1 = i + d * p;
    ll j1 = j + d;
    if (0 <= i1 and i1 < R and 0 <= j1 and j1 < C) {
      return L[i1][j1] >= 2*K - 1 - 2*d;
    }else {
      return false;
    }
  };
  ...
  ll cnt = 0;
  REP(i, 0, R) REP(j, 0, C) {
    if (FORALL(di, 0, K, check(i, j, di, -1) and check(i, j, di, 1))) cnt++;
  }
  cout << cnt << endl;
