...
// @@ !! LIM(f:intDiv binsearch debug power)
...
int main(/* int argc, char *argv[] */) {
  ...
  double gx, gy, gr; cin >> gx >> gy >> gr;
  ll hx = llround(gx * 1e4);
  ll hy = llround(gy * 1e4);
  ll hr = llround(gr * 1e4);
  ll unit = 1e4;
  ll lo = divFloor(hx - hr, unit);
  ll hi = divCeil(hx + hr, unit);

  DLOGK(lo, hi);

  ll ans = 0;
  REP(k, lo, hi + 1) {
    ll v = hr * hr - (k * unit - hx) * (k * unit - hx);
    if (v >= 0) {
      ll hiY = divFloor(hy + llsqrt(v), unit);
      ll loY = divCeil( hy - llsqrt(v), unit);
      ll cur = max(0LL, hiY - loY + 1);
      DLOGK(k, hiY, loY, cur);
      ans += cur;
    }
  }
