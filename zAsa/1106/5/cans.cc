...
int main(/* int argc, char *argv[] */) {
  ...
  auto g = [&](IPoint a, ll x) -> IPoint { return a + IPoint(0, C[x]); };
  auto f = [&](IPoint a, ll c, ll x) -> IPoint { return a + IPoint(a.y, 0); };
  auto vec = reroot(tr, IPoint(0, 0), plus<IPoint>(), f, g);
  ll ans = 1e18;
  REP(i, 0, N) ans = min(ans, vec[i].x);
  cout << ans << endl;
