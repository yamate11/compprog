...
int main(/* int argc, char *argv[] */) {
  ...
  binom[1][0] = binom[1][1] = Fp(1);
  REP(n, 2, N + 1) {
    binom[n][0] = binom[n][n] = 1;
    REP(i, 1, n) binom[n][i] = binom[n - 1][i - 1] + binom[n - 1][i];
  }
  vector<Fp> pow2(N*N + 1);
  pow2[0] = Fp(1);
  REP(i, 0, N*N) pow2[i + 1] = pow2[i] * 2;

  vector tbl(N, vector<ll>(N, Fp(0)));
  tbl[1][1] = 1;
  REP(n, 2, N) {
    REP(nn, 1, n) {
      ll m = n - nn;
      REP(k, 1, nn + 1) {
        tbl[n][m] += tbl[nn][k] * power<Fp>(pow2[k] - 1, m) * pow2[m * (m - 1) / 2] * binom[N - 1 - nn][m];
      }
    }
  }
  Fp ans = 0;
  REP(k, 1, N - 1) ans += tbl[N - 1][k] * (power<Fp>(2, k) - 1);
  cout << ans << endl;
