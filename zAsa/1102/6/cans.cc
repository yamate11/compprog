...
// @@ !! LIM(forall debug f:intDiv perm)
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> U{2, 5, 5, 4, 5, 6, 3, 7, 6};

  ll N, M1; cin >> N >> M1;
  // @InpVec(M1, A1, dec=1) [TjBTuUct]
  auto A1 = vector(M1, ll());
  for (int i = 0; i < M1; i++) { ll v; cin >> v; v -= 1; A1[i] = v; }
  // @End [TjBTuUct]
  ranges::sort(A1, greater<ll>());
  vector<ll> A;
  REP(i, 0, M1) if (FORALL(j, 0, i, U[A1[j]] != U[A1[i]])) A.push_back(A1[i]);
  ll M = ssize(A);
  ll vmin = 10;
  ll idx_min = -1;
  REP(i, 0, M) {
  ...
  DLOGK(A, vmin, idx_min);

  vector lim(M, 0LL);
  REP(i, 0, M) {
    if (i == idx_min) lim[i] = 1;
    else lim[i] = U[A[i]] - 1;
  }
  IntDirProd did(lim);
  ll ans_digits = 0;
  ...
  while (did.get()) {
    ...
    if (s <= N and (N - s) % vmin == 0) {
      vector<ll> chlg = did.vec_view();
      chlg[idx_min] = (N - s) / vmin;
      k += chlg[idx_min];
      auto check = [&]() -> bool {
        if (ans_digits > k) return false;
        if (ans_digits < k) return true;
        REP(i, 0, M) {
          if (ans_vec[i] > chlg[i]) return false;
          if (ans_vec[i] < chlg[i]) return true;
        }
        assert(0);
      };
      if (check()) {
  ...
  string ans;
  REP(i, 0, M) ans += string(ans_vec[i], '1' + A[i]);
  cout << ans << "\n";
  return 0;
}
