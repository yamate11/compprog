...
ll myminA(ll x, ll y) { return min(x, y); }
template<typename T> T myminB(T x, T y) { return min(x, y); }

int func0(int x) { return x; }
int func1(int x) { return x + 1; }
int func1(int x, int y) { return x + y; }

int func2(auto g, int x) { return g(x); }
...
int main(/* int argc, char *argv[] */) {
  ...
  // auto mymin = [](ll x, ll y) { return min(x, y); };
  // auto st = make_seg_tree(1e18, min<ll>);
  // auto st = make_seg_tree(1e18, myminB<ll>);

  cout << func2(func1, 0) << endl;
