...
// @@ !! LIM(rle f:intDiv cmpNaive forall debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  REP(x, 0, 1LL << nq) {
    ...
    auto check = [&]() -> bool {
      ll no = 0;
      REP(i, 0, N) if (t[i] == 'o') no++;
      if (no != K) return false;
      REP(i, 0, N - 1) if (t[i] == 'o' and t[i + 1] == 'o') return false;
      return true;
    };
    if (check()) cands.push_back(t);
  }
  if (cands.empty()) return 0;
  string ans;
  REP(i, 0, N) {
    ll no = 0, nx = 0;
    REP(j, 0, ssize(cands)) if (cands[j][i] == 'o') no++; else nx++;
    if (no == 0) ans += '.';
    else if (nx == 0) ans += 'o';
    else ans += '?';
  }
...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 0, N) {
    if (S[i] == 'o') {
      ...
      if (i - 1 >= 0 and S[i - 1] == 'o') return 0;
      if (i + 1 <  N and S[i + 1] == 'o') return 0;
    }
  }
  DLOGK(S);
  auto vec = rle(S);
  ...
  REP(i, 0, ssize(vec)) {
    auto [c, n] = vec[i];
    if (c == '?') cap += divCeil(n, 2);
  }
  if (K < no or no + cap < K) return 0;
  K -= no;
  string ans;
  REP(i, 0, ssize(vec)) {
    auto [c, n] = vec[i];
    if (c != '?') ans += string(n, c);
      ...
      if (K == 0) {
        ans += string(n, '.');
      }else if (K == cap and n % 2 != 0) {
        string pat(n, 'o');
        REP(j, 0, n) if (j % 2 != 0) pat[j] = '.';
        ans += pat;
      }else {
        ans += string(n, '?');
      }
