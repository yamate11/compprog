...
int main(/* int argc, char *argv[] */) {
  ...
  if (ssize(P) == 1) P.push_back(1);
  vector revP(N, -1LL);
  REP(i, 0, ssize(P)) revP[P[i]] = i;
  vector tbl(K + 1, vector(ssize(P), Fp(0)));
  tbl[0][0] = Fp(1);
  REP(t, 0, K) {
    REP(i, 0, ssize(P)) {
      for (ll q : fwd[P[i]]) tbl[t + 1][revP[q]] += tbl[t][i];
      {
        ll j = (i + 1) % ssize(P);
        ll r = (N + P[j] - P[i]) % N;
        ll t0 = min(t + r, K);
