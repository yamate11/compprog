...
// @@ !! LIM(mod f:updMaxMin debug cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  ll N, M; cin >> N >> M;
  // @InpGrid(N, N, A) [bgqERcQg]
  auto A = vector(N, vector(N, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) { ll v; cin >> v; A[i][j] = v; }
  // @End [bgqERcQg]

  ll ans = 0;
  auto f = [&](auto rF, ll i, ll j, string s) -> void {
    s += (char)('0' + A[i][j]);
    if (i == N - 1 and j == N - 1) {
      updMax(ans, stoll(s) % M);
      return;
    }
    if (i < N - 1) rF(rF, i + 1, j, s);
    if (j < N - 1) rF(rF, i, j + 1, s);
  };
  f(f, 0, 0, "");
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  vector pow10(2*N, Fp(0));
  pow10[0] = 1;
  REP(i, 1, 2*N) pow10[i] = pow10[i - 1] * 10;

  auto A = vector(N, vector(N, Fp()));
  for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) {
      ll v; cin >> v;
      A[i][j] = v * pow10[2 * (N - 1) - (i + j)];
    }
  ...
  vector B(N, vector(N, vector<Fp>()));
  B[0][0] = vector<Fp>{A[0][0]};
  REP(i, 0, N) {
    REP(j, 0, N) {
      if (i + j > N - 1) break;
      if (i > 0) for (auto x : B[i - 1][j]) B[i][j].push_back(x + A[i][j]);
      if (j > 0) for (auto x : B[i][j - 1]) B[i][j].push_back(x + A[i][j]);
    }
  }
  vector C(N, vector(N, vector<Fp>()));
  C[N - 1][N - 1] = vector<Fp>{Fp(A[N - 1][N - 1])};
  REPrev(i, N - 1, 0) {
    REPrev(j, N - 1, 0) {
      if (i + j <= N - 1) break;
      if (i < N - 1) for (auto x : C[i + 1][j]) C[i][j].push_back(A[i][j] + x);
      if (j < N - 1) for (auto x : C[i][j + 1]) C[i][j].push_back(A[i][j] + x);
      DLOGK(i, j, C[i][j]);
    }
  }
  ll ans = 0;
  REP(i, 0, N) {
    vector<ll> vec;
    for (Fp x : B[i][N - 1 - i]) vec.push_back((ll)x);
    sort(ALL(vec));
    DLOGK(i, vec);
    auto f = [&](ll y) -> void {
      ll j0 = lower_bound(ALL(vec), M - y) - vec.begin();
      ll x0;
      if (j0 > 0) x0 = vec[j0 - 1];
      else x0 = vec.back();
      updMax(ans, (x0 + y) % M);
    };
    if (i > 0) for (Fp y : C[i][N - i]) f(y);
    if (i < N - 1) for (Fp y : C[i + 1][N - 1 - i]) f(y);
  }
  cout << ans << endl;
