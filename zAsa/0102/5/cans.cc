...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [9WTeSpvG]
  // @InpNbrList(N, M, nbr, dec=1) [mCxPiUcd]
  ...
  // @End [mCxPiUcd]
  vector W0(N, 0LL);
  REP(i, 0, N) for (ll j : nbr[i]) W0[i] += A[j];
  auto check = [&](ll v) -> bool {
    auto W = W0;
    vector deleted(N, false);
    vector<ll> st;
    REP(i, 0, N) if (W[i] <= v) st.push_back(i);
    while (not st.empty()) {
      ...
      if (not deleted[i]) {
        ...
        for (ll j : nbr[i]) {
          if (not deleted[j]) {
            W[j] -= A[i];
            if (W[j] <= v) st.push_back(j);
          }
        }
      }
    }
    return FORALL(i, 0, N, deleted[i]);
  };
