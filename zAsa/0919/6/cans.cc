...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, nbr, dec=1) [Ms5JBXf1]
  ...
  // @End [Ms5JBXf1]
  ll K; cin >> K;
  // @InpVec(K, C, dec=1) [zKNcWRDQ]
  auto C = vector(K, ll());
  for (int i = 0; i < K; i++) { ll v; cin >> v; v -= 1; C[i] = v; }
  // @End [zKNcWRDQ]
  /*
  safe_umap<ll, ll> revC;
  REP(i, 0, K) revC[C[i]] = i;
  */
  ll big = 1LL << 60;
  ...
  vector dist2(K, vector(K, big));
  REP(i, 0, K) {
    auto dist1 = func(C[i]);
    REP(j, 0, K) dist2[i][j] = dist1[C[j]];
  }
  ...
  REP(x, 0, 1LL << K) {
    REP(i, 0, K) {
      if (tbl[x][i] < big) {
        REP(j, 0, K) {
          if ((not (x >> j & 1)) and dist2[i][j] < big) {
            updMin(tbl[x | (1LL << j)][j], tbl[x][i] + dist2[i][j]);
          }
        }
      }
    }
