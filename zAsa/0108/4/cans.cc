...
int main(/* int argc, char *argv[] */) {
  ...
  Board<bool> light(H, W, false);
  Board<bool> block(H, W, false);
  Board<bool> ans(H, W, false);
  REP(i, 0, N) {
    ll a, b; cin >> a >> b; a--; b--;
    light.at(a, b) = true;
  }
  ...
  REP(i, 0, H) {
    bool b = false;
    REP(j, 0, W) {
      if (light.at(i, j)) b = true;
      if (block.at(i, j)) b = false;
      if (b) ans.at(i, j) = true;
    }
  }
  REP(i, 0, H) {
    bool b = false;
  ...
  DLOGK(ans);
  ll cnt = 0;
  REP(i, 0, H) REP(j, 0, W) if (ans.at(i, j)) cnt++;
  cout << cnt << endl;
