...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, nbr, dec=1, read=((C, dec=1))) [xRGVHuym]
  struct nbr_t {
    int nd;
    ll C;
    // nbr_t() {}
  ...
  auto dfs = [&](auto rF, ll nd, ll col) -> void {
    if (visited[nd]) return;
    visited[nd] = true;
    ans[nd] = col;
    for (auto [peer, ec] : nbr[nd]) {
      DLOGK(nd, col, peer, ec);
      if (ec == col) rF(rF, peer, (ec + 1) % N);
      else rF(rF, peer, ec);
    }
  };
  dfs(dfs, 0, 0);
