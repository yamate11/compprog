...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, C; cin >> N >> C;
  // @InpGrid(C, C, D) [ginL8Nz4]
  auto D = vector(C, vector(C, ll()));
  for (int i = 0; i < C; i++) for (int j = 0; j < C; j++) { ll v; cin >> v; D[i][j] = v; }
  // @End [ginL8Nz4]
  // @InpGrid(N, N, col, dec=1) [S5PFqB1q]
  auto col = vector(N, vector(N, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) { ll v; cin >> v; v -= 1; col[i][j] = v; }
  // @End [S5PFqB1q]

  vector cnt(3, vector(C, 0LL));
  REP(i, 0, N) REP(j, 0, N) {
    ll r = (i + j) % 3;
    cnt[r][col[i][j]]++;
  }
  ...
  while (ip.get()) {
    ll cost = 0;
    REP(r, 0, 3) REP(c, 0, C) cost += cnt[r][c] * D[c][ip[r]];
    ans = min(ans, cost);
  }
  cout << ans << endl;
