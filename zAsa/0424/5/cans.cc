...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl(H, vector(W, optional<Fp>()));
  Fp c23 = Fp(2) / Fp(3);
  auto f = [&](auto rF, ll h, ll w) -> Fp {
    optional<Fp>& v = tbl[h][w];
    if (not v) {
      if (h == H - 1 and w == W - 1) v = power<Fp>(Fp(3), H * W - K);
      else {
        char c = vec[h][w];
        if (c == 'R') {
          if (w == W - 1) v = Fp(0);
          ...
          v = Fp(0);
          if (w == W - 1) v = *v + Fp(0);
          else v = *v + rF(rF, h, w + 1);
          if (h == H - 1) v = *v + Fp(0);
          else v = *v + rF(rF, h + 1, w);
        }else if (c == '.') {
          v = Fp(0);
          if (w == W - 1) v = *v + Fp(0);
          else v = *v + rF(rF, h, w + 1);
