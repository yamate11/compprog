...
int main(/* int argc, char *argv[] */) {
  ...
  vector tblA(C + 1, vector(D + 1, Fp(0)));
  vector tblH(C + 1, vector(D + 1, Fp(0)));
  vector tblV(C + 1, vector(D + 1, Fp(0)));
  tblA[A][B] = Fp(1);
  REP(i, A, C) {
    tblA[i + 1][B] = tblA[i][B] * B;
    tblH[i + 1][B] = tblA[i + 1][B];
  }
  REP(j, B, D) {
    tblA[A][j + 1] = tblA[A][j] * A;
    tblV[A][j + 1] = tblA[A][j + 1];
  }
  REP(i, A + 1, C + 1) REP(j, B + 1, D + 1) {
    tblA[i][j] = tblA[i - 1][j] * j + tblA[i][j - 1] * i - tblV[i - 1][j] * (j - 1) /*- tblH[i][j - 1] * (i - 1)*/;
    tblV[i][j] = tblA[i][j - 1] * i;
    tblH[i][j] = tblA[i - 1][j] * j;
  }
  cout << tblA[C][D] << endl;
