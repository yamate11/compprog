...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N*N, P, dec=1) [bLqcrv0O]
  auto P = vector(N*N, ll());
  for (int i = 0; i < N*N; i++) { ll v; cin >> v; v -= 1; P[i] = v; }
  // @End [bLqcrv0O]
  
  Board<ll> brd(N, N, 0);
  brd.setDispWidth(3);
  Board<bool> ex(N, N, false);
  REP(i, 0, N) REP(j, 0, N) {
    brd.at(i, j) = N + 1;
    ex.at(i, j) = true;
  }
  auto f_init = [&](auto rF, BrdIdx bi) -> ll {
    if (brd.at(bi) > N) {
      for (BrdIdx dir : BrdIdx::nbr4) updMin(brd.at(bi), rF(rF, bi + dir) + 1);
    }
    return brd.at(bi);
  };
  REP(i, 0, N) REP(j, 0, N) f_init(f_init, BrdIdx(i, j));
  auto chg = [&](auto rF, BrdIdx bi, ll v) -> void {
    if (brd.at(bi) <= v) return;
    brd.at(bi) = v;
    for (BrdIdx dir : BrdIdx::nbr4) rF(rF, bi + dir, ex.at(bi + dir) ? v + 1 : v);
  };
  ll ans = 0;
  for (ll p : P) {
    ll i = p / N;
