...
int main(/* int argc, char *argv[] */) {
  ...
  REP(x, 1, (1LL << w) - 1) {
    vector<ll> va, vb;
    REP(i, 0, w) {
      if (x >> i & 1) va.push_back(du.d_at(N, i));
      else vb.push_back(du.d_at(N, i));
    }
    DLOGK(va, vb);
    IntPerm ip(ssize(va), ssize(va));
    while (ip.get()) {
      DLOGK(ip.vec_view());
      if (va[ip[0]] == 0) continue;
      vector<ll> vp(ssize(va));
      REP(i, 0, ssize(va)) vp[i] = va[ip[i]];
      ranges::reverse(vp);
      ll xp = du.from_vector(vp);
      IntPerm iq(ssize(vb), ssize(vb));
      while (iq.get()) {
        if (vb[iq[0]] == 0) continue;
        vector<ll> vq(ssize(vb));
        REP(i, 0, ssize(vb)) vq[i] = vb[iq[i]];
        ranges::reverse(vq);
        ll xq = du.from_vector(vq);
        ans = max(ans, xp * xq);
      }
    }
  }
