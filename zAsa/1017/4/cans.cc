...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpMVec(M, ((X, dec=1), (Y, dec=1))) [TLrY5YEs]
  auto X = vector(M, ll());
  auto Y = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  SCC scc(N);
  REP(i, 0, M) scc.addEdge(X[i], Y[i]);
  scc.build();
  ll nc = scc.numComp();
  
#if DEBUG
  REP(i, 0, N) { DLOGK(i, scc.ccForNode(i)); }
#endif
  

  ll big = 1e18;
  vector sell_nc(nc, -big);
  vector tmp(nc, vector<ll>());
  REP(i, 0, N) { tmp[scc.ccForNode(i)].push_back(A[i]); }
  REP(p, 0, nc) { ranges::sort(tmp[p], greater<ll>()); }
  auto get_sell_nc = [&](auto rF, ll p) -> ll {
    ll& ret = sell_nc[p];
    if (ret == -big) {
      for (ll q : scc.cFwd(p)) updMax(ret, max(rF(rF, q), tmp[q][0]));
      DLOGK(p, ret);
    }
    return ret;
  };
  ll ans = -big;
  REP(i, 0, N) {
    ll p = scc.ccForNode(i);
    ll sell = get_sell_nc(get_sell_nc, p);
    if (A[i] != tmp[p][0]) updMax(sell, tmp[p][0]);
    else if (ssize(tmp[p]) >= 2) updMax(sell, tmp[p][1]);
    updMax(ans, sell - A[i]);
  }
  cout << ans << endl;
