...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [PTSXVg9H]
  auto initA = vector<ll>(N - 1);
  REP(i, 0, N - 1) initA[i] = A[i + 1] - A[i];
  auto initB = vector<ll>(N - 1);
  REP(i, 0, N - 1) initB[i] = B[i + 1] - B[i];
  auto mygcd = [&](ll x, ll y) -> ll { return gcd(x, y); };
  auto stA = make_seg_tree<ll>(0LL, mygcd, initA);
  auto stB = make_seg_tree<ll>(0LL, mygcd, initB);
  REP(_q, 0, Q) {
    ll h1, h2, w1, w2; cin >> h1 >> h2 >> w1 >> w2; h1--; w1--;
    ll eA = stA.query(h1, h2 - 1);
    ll eB = stB.query(w1, w2 - 1);
    ll g = gcd(A[h1] + B[w1], gcd(eA, eB));
    cout << g << "\n";
  }
