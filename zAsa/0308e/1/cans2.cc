...
int main(/* int argc, char *argv[] */) {
  ...
  auto divSieve = divisorSieve(50);

  ll lim = 50;
  vector<vector<pll>> fwdG(lim + 1);
  vector<vector<pll>> bwdG(lim + 1);
  REP(i, 0, lim + 1) REP(j, i + 1, lim + 1) {
    auto divs = getDivisorsDivSieve(j - i, divSieve);
    ll x = 0;
    for (ll d : divs) if (d > i) x |= (1LL << d);
    if (x) {
      fwdG[j].emplace_back(i, x);
      bwdG[i].emplace_back(j, x);
    }
  }
  ...
  vector fwd(N, vector(lim + 1, vector<pll>()));
#if DEBUG
  REP(i, 0, N) DLOGKL("ff", i, fwd[i][A[i]]);
#endif
  vector graph(N, vector(lim + 1, false));
  // vector bwd(N, vector<pll>(lim + 1));
  REP(i, 0, N) {
    auto sub = [&](ll st, auto& rch, const auto& nbrG) -> void {
      rch[st] = true;
      queue<ll> que;
      que.push(st);
      while (not que.empty()) {
        ll j = que.front(); que.pop();
        for (const auto& [k, _] : nbrG[j]) {
          if (not rch[k]) {
            rch[k] = true;
            que.push(k);
          }
    ...
    vector<bool> frch(lim + 1), brch(lim + 1);
    sub(A[i], frch, fwdG);
    sub(B[i], brch, bwdG);
    DLOGK(i, frch);
    DLOGK(i, brch);
    REP(p, 0, lim + 1) graph[i][p] = frch[p] and brch[p];
#if DEBUG
    vector<ll> vvv;
    REP(p, 0, lim + 1) if (graph[i][p]) vvv.push_back(p);
    DLOGK(i, vvv);
#endif
#if DEBUG
  REP(a, 0, N) DLOGKL("gg", a, fwd[a][A[a]]);
#endif
    REP(p, 0, lim + 1) {
      for (const auto& [q, x] : fwdG[p]) {
        if (graph[i][p] and graph[i][q]) fwd[i][p].emplace_back(q, x);
      }
    }
  }
#if DEBUG
  REP(i, 0, N) DLOGK(i, fwd[i][A[i]]);
#endif
  if (EXISTS(i, 0, N, A[i] != B[i] and fwd[i][A[i]].empty())) {
    cout << -1 << endl;
    return 0;
  }
  ...
  ll ans = 0;
  vector fl(N, vector<ll>());
  vector mv(N, 0LL);
  vector done(N, false);
  ll numDone = 0;
  REP(i, 0, N) {
    if (A[i] == B[i]) { done[i] = true; numDone++; }
    fl[i].push_back(A[i]);
    for (auto& [nxt, t] : fwd[i][A[i]]) mv[i] |= t;
#if DEBUG
    vector<ll> www;
    REP(p, 0, lim + 1) if (mv[i] >> p & 1) www.push_back(p);
    DLOGK(i, www);
#endif
  }
  ...
  while (numDone < N) {
    ...
    REP(i, 0, N) {
      ...
      for (ll z : fl[i]) {
        ...
        if (not graph[i][w]) continue;
        a.push_back(w);
        for (auto& [nxt, t] : fwd[i][w]) mv[i] |= t;
      }
      for (ll w : a) fl[i].push_back(w);
      DLOGK(i, fl[i], mv[i]);
    }
  }
  cout << ans << endl;
