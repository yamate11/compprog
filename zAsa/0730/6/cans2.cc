...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1e5;
  string empty = "";
  vector len_add(N + 1, big);
  vector add(N + 1, empty);
  vector len_mult(N + 1, big);
  vector mult(N + 1, empty);
  // vector num(N + 1, empty);

  using sta = pair<ll, string>;
  auto foradd = [&](ll n) -> sta {
    if (len_add[n] >= big and len_mult[n] >= big) return {big, ""};
    if (len_add[n] >= big) return {len_mult[n], mult[n]};
    if (len_mult[n] >= big) return {len_add[n], add[n]};
    if (len_add[n] < len_mult[n]) return {len_add[n], add[n]};
    return {len_mult[n], mult[n]};
  };
  auto formult = [&](ll n) -> sta {
    if (len_add[n] >= big and len_mult[n] >= big) return {big, ""};
    if (len_add[n] >= big) return {len_mult[n], mult[n]};
    if (len_mult[n] >= big) return {len_add[n] + 2, "(" + add[n] + ")"};
    if (len_add[n] + 2 < len_mult[n]) return {len_add[n] + 2, "(" + add[n] + ")"};
    return {len_mult[n], mult[n]};
  };


  REP(n, 1, N + 1) {
    if (n == 1) { len_mult[n] = 1; mult[n] = "1"; }
    else if (n == 11) { len_mult[n] = 2; mult[n] = "11"; }
    else if (n == 111) { len_mult[n] = 3; mult[n] = "111"; }
    else if (n == 1111) { len_mult[n] = 4; mult[n] = "1111"; }
    else {
      REP(x, 1, n) {
        ll y = n - x;
        auto [lenx, sx] = foradd(x);
        auto [leny, sy] = foradd(y);
        if (lenx + leny + 1 < len_add[n]) {
          add[n] = sx + "+" + sy;
          len_add[n] = lenx + leny + 1;
        }
      }
      REP(x, 2, n) {
        if (n % x == 0) {
          ll y = n / x;
          auto [lenx, sx] = formult(x);
          auto [leny, sy] = formult(y);
          if (lenx + leny + 1 < len_mult[n]) {
            mult[n] = sx + "*" + sy;
            len_mult[n] = lenx + leny + 1;
          }
        }
      }
    }
    DLOGK(n, len_add[n], add[n], len_mult[n], mult[n]);
  }
  if (len_add[N] < len_mult[N]) {
    cout << add[N] << endl;
  }else {
    cout << mult[N] << endl;
  }
