...
enum kind {EXPR1, EXPR2, TERM1, TERM2, FACTOR1, FACTOR2, NUMBER};

// @DefStruct(sta, (len, (tp, kind), idx1, idx2)) [xpKCdUW0]
struct sta {
  ll len;
  kind tp;
  ll idx1;
  ll idx2;
  string show() const {
    return "(" + to_string(len) + ", " + g_show(tp) + ", " + to_string(idx1) + ", " + to_string(idx2) + ")";
  }
...
ll big = 1e5;
sta def(big, NUMBER, big, big);

vector expr(N0 + 1, def);
vector term(N0 + 1, def);
vector factor(N0 + 1, def);
vector number(N0 + 1, def);

string build (const sta& s) {
  DLOGKL("  build", s);
  if (s.tp == NUMBER) return to_string(s.idx1);
  if (s.tp == FACTOR1) return build(number[s.idx1]);
  if (s.tp == FACTOR2) return "(" + build(expr[s.idx1]) + ")";
  if (s.tp == TERM1) return build(factor[s.idx1]);
  if (s.tp == TERM2) return build(term[s.idx1]) + "*" + build(factor[s.idx2]);
  if (s.tp == EXPR1) return build(term[s.idx1]);
  if (s.tp == EXPR2) return build(expr[s.idx1]) + "+" + build(term[s.idx2]);
  assert(0);
};
...
int main(/* int argc, char *argv[] */) {
  ...
  number[1] = sta{1, NUMBER, 1, 0};
  number[11] = sta{2, NUMBER, 11, 0};
  number[111] = sta{3, NUMBER, 111, 0};
  number[1111] = sta{4, NUMBER, 1111, 0};
  
  REP(n, 1, N + 1) {
    auto updFactor = [&]() -> bool {
      bool ret = false;
      if (factor[n].len > number[n].len) {
        ret = true;
        factor[n] = {number[n].len, FACTOR1, n, 0};
      }
      if (factor[n].len > expr[n].len + 2) {
        ret = true;
        factor[n] = {expr[n].len + 2, FACTOR2, n, 0};
      }
      return ret;
    };
    auto updTerm = [&]() -> bool {
      bool ret = false;
      if (term[n].len > factor[n].len) {
        ret = true;
        term[n] = {factor[n].len, TERM1, n, 0};
      }
      REP(x, 2, n) {
        if (n % x == 0) {
          ll y = n / x;
          ll new_len = term[x].len + 1 + factor[y].len;
          if (term[n].len > new_len) {
            ret = true;
            term[n] = {new_len, TERM2, x, y};
          }
        }
      }
      return ret;
    ...
    auto updExpr = [&]() -> bool {
      bool ret = false;
      if (expr[n].len > term[n].len) {
        ret = true;
        expr[n] = {term[n].len, EXPR1, n, 0};
      }
      REP(x, 1, n) {
        ll y = n - x;
        ll new_len = expr[x].len + 1 + term[y].len;
        if (expr[n].len > new_len) {
          ret = true;
          expr[n] = {new_len, EXPR2, x, y};
        }
      }
      return ret;
    };
    ...
    ll cnt = 0;
    while (true) {
      if (updFactor()) cnt = 0; else cnt++;
      if (cnt >= 3) break;
      if (updTerm()) cnt = 0; else cnt++;
      if (cnt >= 3) break;
      if (updExpr()) cnt = 0; else cnt++;
      if (cnt >= 3) break;
    }
  }
