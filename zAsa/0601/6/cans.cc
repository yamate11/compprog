...
int main(/* int argc, char *argv[] */) {
  ...
  if (nn < K) {
    ...
    auto check = [&](ll t) -> bool {
      ...
      REP(i, 0, SIZE(Pos)) {
        if (Pos[i] == 0) cnt += i;
          ...
          ll z = lower_bound(ALL(Pos), divCeil(t, Pos[i])) - Pos.begin();
          cnt += min(i, z);
        }
      }
      REP(i, 0, SIZE(Neg)) {
        if (Neg[i] == 0) cnt += i;
          ...
          ll z = lower_bound(ALL(Neg), divCeil(t, Neg[i])) - Neg.begin();
          cnt += min(i, z);
        }
      }
      return cnt <= K - 1;
    };
    ll ans = binsearch_i<ll>(check, 0, (ll)(1e18 + 1));
    cout << ans << endl;
  }else {
    K = nn - K + 1;
    DLOGKL("Negative", K);
    auto check = [&](ll t) -> bool {
      ll cnt = 0;
      for (ll n : Neg) {
        ll z = lower_bound(ALL(Pos), divCeil(t, n)) - Pos.begin();
        cnt += z;
      }
      return cnt <= K - 1;
    };
