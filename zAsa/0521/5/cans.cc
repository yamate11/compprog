...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpVec(N, S, type=string) [iCh6QbML]
  ...
  ll tlen = 0;
  REP(i, 0, N) tlen += ssize(S[i]);
  ll max_us = 16 - tlen;
  ll rem = max_us - (N - 1);
  auto check = [&]() -> string {
    if (N == 1) {
      if (ssize(S[0]) < 3) return "-1";
      if (ss.contains(S[0])) return "-1";
      return S[0];
    }

    IntPerm ip(N, N);
    while (ip.get()) {
      IDCBoxBall ibb(N, rem);
      while (ibb.get()) {
        string cand;
        REP(i, 0, N) {
          cand += S[ip[i]];
          if (i < N - 1) cand += string(1 + ibb[i], '_');
        }
        if (not ss.contains(cand)) return cand;
      }
    }
    return "-1";
  };
