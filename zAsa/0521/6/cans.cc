...
// @@ !! LIM(segTree unordered_map debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W, N; cin >> H >> W >> N;
  vector A(H, vector<ll>());
  REP(i, 0, N) {
    ll r, c; cin >> r >> c; r--; c--;
    A[r].push_back(c);
  }
  REP(i, 0, H) ranges::sort(A[i], greater<ll>());
  auto enc = [&](ll r, ll c) -> ll { return r * W + c; };
  safe_umap<ll, pll> mp;
  using sta = tuple<ll, ll, ll>;
  auto mymax = [](const sta& a, const sta& b) -> sta {
    auto [a1, a2, a3] = a;
    auto [b1, b2, b3] = b;
    if (a1 >= b1) return a;
    else return b;
  };
  auto st = make_seg_tree(sta(0, H - 1, W - 1), mymax, vector(W, sta(0, H - 1, W - 1)));
  REPrev(r, H - 1, 0) {
    for (auto c : A[r]) {
      auto [x, pr, pc] = st.query(c, W);
      mp[enc(r, c)] = pll(pr, pc);
      st.rs(c) = sta(x + 1, r, c);
    }
    DLOGK(r, A[r], st.vec_view());
  }
  auto [aval, fr, fc] = st.query(0, W);
  string route;
  REP(r, 0, fr) route += 'D';
  REP(c, 0, fc) route += 'R';
  ll cr = fr, cc = fc;
  while (not (cr == H - 1 and cc == W - 1)) {
    auto [nr, nc] = mp[enc(cr, cc)];
    REP(r, cr, nr) route += 'D';
    REP(c, cc, nc) route += 'R';
    cr = nr;
    cc = nc;
  }
  cout << aval << endl << route << endl;
