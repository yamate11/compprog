...
int main(/* int argc, char *argv[] */) {
  ...
  using MyTrie = Trie<26, 'a', TRIE_SET_MULTI, bool>;
  MyTrie* root = new MyTrie;
  
  ll Q; cin >> Q;
  ll ans = 0;

  auto dfs1 = [&](auto rF, MyTrie* p) -> void {
    while (p->reside > 0) p->erase();
    if (p->size_st == 0) return;
    REP(d, 0, 26) {
      MyTrie* q = p->get_child_offset(d);
      if (q) rF(rF, q);
    }
  };
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      auto func = [&]() -> void {
        MyTrie* p = root;
        REP(i, 0, ssize(s)) {
          p = p->get_or_create_child_val(s[i]);
          if (p->user) return;
        }
        p->insert("");
        ans++;
