...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, P, type=IPoint) [E2YEjXll]
  ...
  REP(i, 0, N) REP(j, i + 1, N) {
    ...
    UnionFind uf(N);
    REP(k, 0, N) {
      auto it1 = mp.find(P[k] + d);
      if (it1 != mp.end()) uf.merge(k, it1->second);
      auto it2 = mp.find(P[k] - d);
      if (it2 != mp.end()) uf.merge(k, it2->second);
    }
    ll cnt = 0;
    REP(k, 0, N) if (uf.leader(k) == k) cnt++;
    ans = min(ans, cnt);
  }
  cout << ans << endl;
