...
// @@ !! LIM(tree cmpNaive forall perm input debug)
...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ll N; cin >> N;
    // @InpNbrList(N, N - 1, nbr, dec=1) [e1xJ2c1I]
    ...
    // @End [e1xJ2c1I]
    string S; cin >> S;
    string bodyans; cin >> bodyans;
    auto check = [&](ll x) -> bool {
      REP(i, 0, N) {
        ll cntB = 0, cntW = 0;
        for (ll p : nbr[i]) if (x >> p & 1) cntB++; else cntW++;
        if ((cntB > cntW and S[i] == 'W') or (cntW > cntB and S[i] == 'B')) {
          // DLOGK(x, i, cntB, cntW, S, nbr[i]);
          return false;
        }
      }
      return true;
    ...
    if (bodyans != "-1") {
      assert(ssize(bodyans) == N);
      assert(FORALL(i, 0, N, bodyans[i] == 'B' or bodyans[i] == 'W'));
      ll x = 0;
      REP(i, 0, N) if (bodyans[i] == 'B') x |= (1LL << i);
      if (not check(x)) {
        cout << "NG\n";
        return;
      }
  ...
  auto solve = [&]() -> void {
    ...
    auto dfs = [&](auto rF, ll nd) -> char {
      ...
      if (me != '.') {
        ...
        if (pt >= 0) orig[nd] = S[pt];
        else orig[nd] = 'B';
      }
      DLOGK(nd, cntB, cntW, S[nd]);
      if ((cntB > cntW and S[nd] == 'W') or (cntB < cntW and S[nd] == 'B')) {
        DLOGKL("throw", nd, cntB, cntW, S[nd]);
        throw myexc();
      }
      if (pt >= 0 and cntB == cntW) return S[nd];
      else return '.';
    };
    try {
      dfs(dfs, 0);
