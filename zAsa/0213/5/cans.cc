...
int main(/* int argc, char *argv[] */) {
  ...
  auto match = [&](string s, string pat) -> bool {
    REP(i, 0, SIZE(s)) {
      if (pat[i] == '*') continue;
  ...
  auto check3 = [&](string s, string pat) -> bool {
    return s.substr(0, 3) == pat or s.substr(1, 3) == pat;
  };

  auto check4 = [&](string s) -> bool {
    if (match(s, "A*GC")) return false;
    if (match(s, "AG*C")) return false;
    if (check3(s, "AGC")) return false;
    if (check3(s, "GAC")) return false;
    if (check3(s, "ACG")) return false;
    return true;
  };

  vector<char> cs{'A', 'C', 'G', 'T'};
  vector tbl_init(4, vector(4, vector(4, Fp(0))));
  auto tbl = tbl_init;
