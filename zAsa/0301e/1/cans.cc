...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpNbrList(N, N-1, nbr, dec=1) [yF4qICvv]
  ...
  vector rec1(N, vector<Fp>());
  vector rec2(N, vector<Fp>());

  auto vget = [&](const auto& v, ll i) -> Fp {
    if (i >= 0 and i < SIZE(v)) return v[i];
    return Fp(0);
  };
  auto vadd = [&](auto& v, ll i, Fp w) -> void {
    assert(i >= 0);
    if (i >= SIZE(v)) v.resize(i + 1);
    v[i] += w;
  };
  auto vmax = [&](const auto& v) -> ll {
    return SIZE(v) - 1;
  };
  ...
  auto dfs = [&](auto rF, ll nd, ll pt) -> void {
    ...
    for (ll cld : nbr[nd]) {
      if (cld == pt) continue;
      rF(rF, cld, nd);
      auto prev1 = move(rec1[nd]);
      auto prev2 = move(rec2[nd]);
      rec1[nd] = vector<Fp>();
      rec2[nd] = vector<Fp>();
      ll mp1 = vmax(prev1), mp2 = vmax(prev2), mc1 = vmax(rec1[cld]), mc2 = vmax(rec2[cld]);
      REP(i, 1, mp1 + 1) REP(j, 1, mc1 + 1) vadd(rec1[nd], i + j - 1, vget(prev1, i) * vget(rec1[cld], j));
      REP(i, 1, mp1 + 1) REP(j, 0, mc2 + 1) vadd(rec1[nd], i + j,     vget(prev1, i) * vget(rec2[cld], j));
      REP(i, 0, mp2 + 1) REP(j, 1, mc1 + 1) vadd(rec2[nd], i + j,     vget(prev2, i) * vget(rec1[cld], j));
      REP(i, 0, mp2 + 1) REP(j, 0, mc2 + 1) vadd(rec2[nd], i + j,     vget(prev2, i) * vget(rec2[cld], j));
    }
    DLOGK(nd, rec1[nd], rec2[nd]);
  };
  dfs(dfs, 0, -1);
  REPOUT(i, 1, N + 1, vget(rec1[0], i) + vget(rec2[0], i), "\n");
