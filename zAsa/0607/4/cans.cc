...
int body(istream& cin, ostream& cout) {
  ...
  q[1] = 1;
  REP(k, 2, 17) {
    p[k] = du.pow(k - 1) + p[k - 1];
    q[k] = q[k - 1] + p[k];
  }
  DLOGK(p);
  DLOGK(q);
  
  auto f = [&](auto rF, ll t) -> ll {
    if (t == 0) return 0;
    if (t == 1) return 1;
    if (t < 10) return 1;
    ll k = du.width(t);
    ll ret;
    if (du.d_at(t, k - 1) == 1) {
      ll x = t - du.v_at(t, k - 1);
      if (x >= du.pow(k - 2)) {
        ll a = rF(rF, x);
        ret = x + 1 + q[k - 1] + a - q[k - 2];
        DLOGK(t, x, k, q[k - 1], a, q[k - 2], ret);
      }else {
        ret = x + 1 + q[k - 1];
        DLOGK(t, x, k, q[k - 1], ret);
      }
    }else {
      ret = q[k];
    }
