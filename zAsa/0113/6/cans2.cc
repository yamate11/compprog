...
int main(/* int argc, char *argv[] */) {
...
#if DEBUG
  using umap = safe_umap<ll, ll, false>;
#else
  using umap = safe_umap<ll, ll, true>;
  // using umap = safe_umap<ll, ll, false>;
#endif
  auto f = [&](const auto& vec) -> pair<umap, umap> {
    umap mp1, mp2;
    mp2[0] = 1;
    for (ll t : vec) {
      // umap mp(8 * ssize(mp2));
      umap mp;
      for (auto [x, n] : mp2) mp[(x + t) % M] = n;
      for (auto [x, n] : mp1) mp2[x] += n;
      mp1 = move(mp);
      DLOGK(t, mp1, mp2);
    }
    return {move(mp1), move(mp2)};
  };
  auto B = A;
  ranges::reverse(B);
  ll H = N / 2;
  ...
  auto [p1, p2] = f(A);
  auto [q1, q2] = f(B);
  auto get = [&](const auto& mp, ll y) -> ll {
    auto it = mp.find(y);
