...
int main(/* int argc, char *argv[] */) {
  ...
  using vl = vector<ll>;
  auto f = [&](const auto& vec) -> pair<vl, vl> {
    vl mp1, mp2;
    mp2.push_back(0);
    for (ll t : vec) {
      vl mp = mp2;
      REP(i, 0, ssize(mp)) mp[i] = (mp[i] + t) % M;
      for (ll x : mp1) mp2.push_back(x);
      mp1 = move(mp);
      // DLOGK(t, mp1, mp2);
    }
    ranges::sort(mp1);
  ...
  auto [p1, p2] = f(A);
  auto [q1, q2] = f(B);
  auto get = [&](const auto& vec, ll y) -> ll {
    return ranges::lower_bound(vec, y + 1) - ranges::lower_bound(vec, y);
  };
