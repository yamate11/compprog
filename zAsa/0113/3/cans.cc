...
// @@ !! LIM(digit sieve f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    ll w = ssize(vec);
    vector v1(w - 1, 9LL);
    ll ans = du.from_vector(v1);
    for (ll d : getDivisors(w)) {
      ll e = w / d;
      DLOGK(d, e);
      if (e == 1) continue;
      vector<ll> v2(w);
      REP(i, 0, e) REP(j, 0, d) v2[i * d + j] = vec[j];
      ll z = du.from_vector(v2);
      DLOGK(z);
      if (z <= N) {
        updMax(ans, z);
      }else {
        auto v3 = v2;
        v3.resize(d);
        ll y = du.from_vector(v3) - 1;
        DLOGK(v3, y);
        if (du.width(y) == d) {
          v3 = du.to_vector(y);
          vector<ll> v4(w);
          REP(i, 0, e) REP(j, 0, d) v4[i * d + j] = v3[j];
          DLOGK(v4);
          updMax(ans, du.from_vector(v4));
        }
      }
    }
    return ans;
