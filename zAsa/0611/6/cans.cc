...
int body(istream& cin, ostream& cout) {
  ...
  auto myle = [&](ll size1, ll cost1, ll size2, ll cost2) -> bool { return cost1 * size2 <= cost2 * size1; };
  ...
  auto solve = [&]() -> ll {
    ...
    if (N / A <= lim or N / B <= lim) {
      if (N / B <= lim) { swap(A, B); swap(Y, Z); }
      ll ret = 1e18;
      REP(i, 0, N / A + 1) {
        ll cost = i * Y;
        ll rem = N - A * i;
    ...
    REP(z, 0, A) {
      ...
      ll x = r % A;
      ll y = r / A;
      ret = min(ret, z * Z + x * X + y * Y);
      DLOGKL("ret4", ret, x, y, z);
    }
    return ret;
