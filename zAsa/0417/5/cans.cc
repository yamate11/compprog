...
// @@ !! LIM(unordered_map f:updMaxMin forall debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<pll> vR;
  vector<pll> vC;
  for (auto [r, x] : mpR) vR.emplace_back(x, r);
  for (auto [c, x] : mpC) vC.emplace_back(x, c);
  ranges::sort(vR, greater<pll>());
  ranges::sort(vC, greater<pll>());
  ll val1 = 0;
  REP(i, 0, N) updMax(val1, mpR[R[i]] + mpC[C[i]] - X[i]);
  auto f = [&]() -> ll {
    REP(k, 0, 2*N - 1) {
      ll ret = -1;
      REP(i, 0, N) {
        ...
        auto [xr, r] = vR[i];
        auto [xc, c] = vC[j];
        if (not pts.contains(pll(r, c))) updMax(ret, xr + xc);
      }
      if (ret >= 0) return ret;
    }
    return 0;
