...
// @@ !! LIM(rollingHash binsearch f:perfmeas)
...
int main(/* int argc, char *argv[] */) {
  ...
  u64 full = ~u64(0);
  vector cache(N, vector(N + 1, full));
  auto with_memo = [&](ll start, ll len) -> u64 {
    u64& ret = cache[start][len];
    if (ret == full) ret = rh.get(hs, start, len);
    return ret;
  };

  ll ans = 0;
  REP(i, 0, N) REP(j, i + 1, N) {
    auto check = [&](ll len) -> bool { return with_memo(i, len) == with_memo(j, len); };
