...
int main(/* int argc, char *argv[] */) {
  ...
  ll K, N; cin >> K >> N;
  // @InpMVec(N, ((V, type=string), (W, type=string))) [mh9xzs5l]
  ...
  auto check = [&]() -> bool {
    DLOGK(idp.vec_view());
    vector<string> dic(9);
    auto reg = [&](ll e, string u) -> bool {
      if (dic[e] != "") return dic[e] == u;
      dic[e] = u;
      return true;
    };
    auto sub = [&](string v, string w) -> bool {
      ll idx = 0;
      ll vlen = SIZE(v);
      ll wlen = SIZE(w);
      REP(j, 0, vlen) {
        ll e = v[j] - '1';
        ll l = idp.at(e);
        if (idx + l > wlen) return false;
        if (reg(e, w.substr(idx, l))) return false;
        idx += l;
      }
      if (idx != wlen) return false;
      retun true;
    };
    REP(i, 0, N) if (not sub(V[i], W[i])) return false;
    REP(i, 0, K) {
      if (dic[i] != "") cout << dic[i] << "\n";
      else cout << "a\n";
    }
