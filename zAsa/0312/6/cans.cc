...
int main(/* int argc, char *argv[] */) {
  ...
  while (ic.get()) {
    ...
    auto z0 = [&](ll i) -> ll { return i % 2 == 0 ? A[i] : B[i]; };
    auto z1 = [&](ll i) -> ll { return i % 2 == 0 ? B[i] : A[i]; };
    stable_sort(ALL(P), [&](ll i, ll j) -> bool { return z0(i) < z0(j); });
    stable_sort(ALL(Q), [&](ll i, ll j) -> bool { return z1(i) < z1(j); });
    ll m1 = N - m;
    auto check = [&]() -> bool {
      REP(i, 0, m1) {
        if (not (z0(P[i]) <= z1(Q[i]))) return false;
        if (i + 1 < ssize(P) and not (z1(Q[i]) <= z0(P[i + 1]))) return false;
      }
      return true;
    };
    if (check()) {
      ...
      if (m1 < m) R.push_back(P[m1]);
      ll cur = 0;
      REP(i, 0, N) REP(j, i + 1, N) if (R[i] > R[j]) cur++;
      ans = min(ans, cur);
    }
  }
  if (ans == big) ans = -1;
