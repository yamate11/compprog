...
int main(/* int argc, char *argv[] */) {
  ...
  auto st1 = make_seg_tree(0LL, plus<ll>(), vector<ll>(sz));
  auto st2 = make_seg_tree(0LL, plus<ll>(), vector<ll>(sz));
  ll ans = 0;
  REP(i, 0, N) {
    ll ca = cc.c(A[i]);
    ll p = st1.query(0, ca);
    ans += A[i] * p;
    st1.rs(ca) = st1.at(ca) + 1;
  }
  REPrev(i, N - 1, 0) {
    ll ca = cc.c(A[i]);
    ll p = st2.query(ca + 1, sz);
    ans -= A[i] * p;
    st2.rs(ca) = st2.at(ca) + 1;
  }
  cout << ans << endl;
