...
int main(/* int argc, char *argv[] */) {
  ...
  RollingHash rh;
  auto pT = rh.precomp(T);
  auto pRT = rh.precomp(RT);
  REP(i, 0, N + 1) {
    RHVal h0 = pT.get(0, i);
    RHVal h1 = pRT.get(N - i, N);
    RHVal h2 = pT.get(i + N, N - i);
    DLOGK(i, T.substr(0, i), T.substr(i + N, N - i), RT.substr(N - i, N));
    if (rh.hash_concat(h0, h2, N - i) == h1) {
      cout << RT.substr(N - i, N) << endl << i << endl;
      return 0;
    }
  }
  cout << -1 << endl;
