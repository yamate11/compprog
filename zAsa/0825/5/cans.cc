...
int main(/* int argc, char *argv[] */) {
  ...
  string S; cin >> S;
  ll sigma = 26;
  vector tbl_init(sigma, vector(sigma, Fp(0)));
  auto tbl = tbl_init;
  REP(i, 0, sigma) REP(j, 0, sigma) {
    if (i == j) continue;
    if (S[0] == 'a' + i or S[0] == '?') {
      if (S[1] == 'a' + j or S[1] == '?') tbl[i][j] = 1;
    }
  }
  REP(i, 2, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(a, 0, sigma) REP(b, 0, sigma) {
      if (prev[a][b] == Fp(0)) continue;
      if (S[i] == '?') {
        REP(d, 0, sigma) {
          if (a != d and b != d) tbl[b][d] += prev[a][b];
        }
      }else {
        ll d = S[i] - 'a';
        if (a != d and b != d) tbl[b][d] += prev[a][b];
  ...
  Fp ans = 0;
  REP(a, 0, sigma) REP(b, 0, sigma) ans += tbl[a][b];
  cout << ans << "\n";
