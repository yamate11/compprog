...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpMVec(N, ((X, type=double), (Y, type=double), (C, type=double))) [DIqyPzI8]
  auto X = vector(N, double());
  auto Y = vector(N, double());
  auto C = vector(N, double());
  for (int i = 0; i < N; i++) {
    double v1; cin >> v1; X[i] = v1;
    double v2; cin >> v2; Y[i] = v2;
    double v3; cin >> v3; C[i] = v3;
  }
  // @End [DIqyPzI8]
  ...
  auto check = [&](double t) -> bool {
    ...
    REP(i, 0, N) {
      updMax(xmin, X[i] - t / C[i]);
      updMin(xmax, X[i] + t / C[i]);
      updMax(ymin, Y[i] - t / C[i]);
      updMin(ymax, Y[i] + t / C[i]);
      DLOGK(i, xmin, xmax, ymin, ymax);
      if (xmin > xmax or ymin > ymax) return false;
    }
    return true;
  };
  double t = binsearch_r<double>(check, 1e10, 0, 1e-5, true);
  cout << t << endl;
