...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> vector<ll> {
    ll numc = 26;
    // @InpVec(3, S, type=string) [DRneaoId]
    auto S = vector(3, string());
    for (int i = 0; i < 3; i++) { string v; cin >> v; S[i] = v; }
    // @End [DRneaoId]
    vector<bool> C(numc);
    REP(i, 0, 3) {
      for (char c : S[i]) C[c - 'a'] = true;
    }
    ll seq = 0;
    vector<ll> D(numc, -1LL);
    REP(i, 0, numc) if (C[i]) D[i] = seq++;
    vector<vector<ll>> T(3);
    REP(i, 0, 3) {
      for (char c : S[i]) T[i].push_back(D[c - 'a']);
    }
    ...
    while (ip.get()) {
      auto f = [&](ll idx) -> ll {
        if (ip[T[idx][0]] == 0) return -big;
        ll ret = 0;
        for (ll d : T[idx]) ret = 10 * ret + ip[d];
        return ret;
      };
      vector<ll> v(3);
      REP(i, 0, 3) v[i] = f(i);
