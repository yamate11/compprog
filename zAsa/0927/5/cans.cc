...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, C, dec=1) [AiaAuWw6]
  auto C = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; C[i] = v; }
  // @End [AiaAuWw6]
  // @InpVec(N, X, dec=1) [kBP1TP7n]
  auto X = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; X[i] = v; }
  // @End [kBP1TP7n]

  auto st = make_seg_tree<ll>(0LL, plus<ll>(), vector(N, 0LL));

  auto func = [&](auto& A) -> ll {
    ll ret = 0;
    REP(i, 0, SIZE(A)) {
      ll t = st.query(A[i] + 1, N);
      DLOGK(i, t);
      ret += t;
      st.update(A[i], st[A[i]] + 1);
      DLOGK(i, st);
    }
    REP(i, 0, SIZE(A)) st.update(A[i], 0);
    return ret;
  };

  auto vec = vector(N, vector<ll>());
  REP(i, 0, N) vec[C[i]].push_back(X[i]);
