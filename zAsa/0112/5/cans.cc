...
int body(istream& cin, ostream& cout) {
  ...
  auto f = [&](ll p) -> ll {
    ...
    REP(i, 0, N) {
      ...
      REPrev(j, M - 1, 0) {
        if (tbl[j]) {
          ll new_val = *tbl[j] + val;
          tbl[j + 1] = tbl[j + 1] ? max(*tbl[j + 1], new_val) : new_val;
        }
      }
    }
    return *tbl[M];
