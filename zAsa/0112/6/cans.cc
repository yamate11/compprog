...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [pMUQUoId]
  auto px = sortedIndex(X);
  auto py = sortedIndex(Y);
  using sta = tuple<ll, int, int, int>;
  vector<sta> gaps;
  REP(i, 0, N - 1) gaps.emplace_back(X[px[i + 1]] - X[px[i]], 0, px[i], px[i + 1]);
  REP(i, 0, N - 1) gaps.emplace_back(Y[py[i + 1]] - Y[py[i]], 1, py[i], py[i + 1]);
  ranges::sort(gaps);

  UnionFind uf(N);
  ll ans = 0;
  for (auto [d, kd, i, j] : gaps) {
