...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](auto start, auto goal, char pass, char wall) -> ll {
    ll big = 1e18;
    ...
    while (not deq.empty()) {
      ...
      if (dist[bi.r][bi.c] == d) {
        for (auto dir : BrdIdx::nbr4) {
          ...
          if (brd.at(bj) == pass) {
            ll new_d = d;
            if (new_d < dist[bj.r][bj.c]) {
              dist[bj.r][bj.c] = new_d;
              deq.emplace_front(new_d, bj);
            }
          }else if (brd.at(bj) == wall) {
            ll new_d = d + 1;
            if (new_d < dist[bj.r][bj.c]) {
              dist[bj.r][bj.c] = new_d;
              deq.emplace_back(new_d, bj);
            }
          }
        }
      }
  ...
  ll x = f(BrdIdx(0, 0), BrdIdx(N - 1, N - 1), 'R', 'B');
  ll y = f(BrdIdx(N - 1, 0), BrdIdx(0, N - 1), 'B', 'R');
  cout << x + y << endl;
