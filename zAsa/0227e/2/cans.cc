...
struct single_t {
  multi_t* body;
  single_t() : body(nullptr) {}

  vector<Fp> count(ll);
};

struct multi_t {
  vector<single_t*> bodies;
  multi_t() : bodies() {}

  vector<Fp> count(ll K) {
    vector<Fp> ret{1};
    for (single_t* p : bodies) {
      auto vec = p->count(K);
      auto o_ret = move(ret);
      ll new_sz = SIZE(o_ret) + SIZE(vec) - 1;
      ret = vector<Fp>(new_sz);
      REP(i, 0, SIZE(vec)) REP(j, 0, SIZE(o_ret)) ret[i + j] += vec[i] * o_ret[j];
      DLOGK(o_ret, vec, ret);
    }
    return ret;
  }
...
vector<Fp> single_t::count(ll K) {
  auto p = body->count(K);
  vector<Fp> ret(SIZE(p) + 2);
  REP(i, 0, SIZE(p)) {
    ret[i + 1] += 2 * p[i];
    ret[i + 2] += p[i];
    ret[i] += p[i];
  ...
  DLOGKL("s1", ret);
  if (SIZE(ret) > 2 * K + 1) {
    ll shift = (SIZE(ret) - (2 * K + 1)) / 2;
    copy(ret.begin() + shift, ret.begin() + shift + (2 * K + 1), ret.begin());
    ret.resize(2 * K + 1);
    DLOGKL("s2", ret);
  }
  return ret;
...
pair<single_t*, ll> parse_single(string, ll);

pair<multi_t*, ll> parse_multi(string S, ll i) {
  multi_t* rs = new multi_t();
  while (i < SIZE(S) and S[i] == '(') {
    single_t* ss;
    tie(ss, i) = parse_single(S, i);
    rs->bodies.push_back(ss);
  }
  return {rs, i};
}

pair<single_t*, ll> parse_single(string S, ll i) {
  single_t* rs = new single_t();
  multi_t* ss;
  tie(ss, i) = parse_multi(S, i + 1);
  assert(S[i] == ')');
  rs->body = ss;
  return {rs, i + 1};
}
...
int main(/* int argc, char *argv[] */) {
  ...
  ll KK; cin >> KK;
  ll K = KK / 2;
  auto [p, _] = parse_multi(S, 0);
  DLOGK(SIZE(p->bodies));
  auto v = p->count(K);
  Fp ans = accumulate(ALL(v), Fp(0));
  cout << ans << endl;
