...
int main(/* int argc, char *argv[] */) {
  ...
  ll T = min(K + 1, H*W + 1);
  vector B(T, vector(H, vector(W, -1LL)));
  B[0][si][sj] = 0;
  REP(t, 0, T - 1) {
    REP(r, 0, H) REP(c, 0, W) {
      if (B[t][r][c] >= 0) {
        updMax(B[t + 1][r][c], B[t][r][c] + brd.at(r, c));
        BrdIdx bi(r, c);
        for (auto dir : BrdIdx::nbr4) {
          auto bj = bi + dir;
          if (brd.in(bj)) {
            updMax(B[t + 1][bj.r][bj.c], B[t][r][c] + brd.at(bj));
          }
        }
      }
    }
  ...
  ll ans = 0;
  REP(r, 0, H) REP(c, 0, W) {
    updMax(ans, B[t0][r][c] + (K - t0) * brd.at(r, c));
  }
  cout << ans << endl;
