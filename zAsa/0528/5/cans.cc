...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  vector<ll> D(N + 1);
  // @InpMVec(M, ((S, dec=1), (T, dec=0))) [eSPkllPi]
  auto S = vector(M, ll());
  auto T = vector(M, ll());
  for (int i = 0; i < M; i++) {
    ll v1; cin >> v1; v1 -= 1; S[i] = v1;
    ll v2; cin >> v2; v2 -= 0; T[i] = v2;
  }
  // @End [eSPkllPi]
  REP(i, 0, M) {
    D[S[i]]++;
  ...
  REP(i, 0, N) {
    cur += D[i];
    if (cur == 1) zero.push_back(i);
  }
  zero.push_back(N);
  DLOGK(zero);
  vector<ll> ans;
  REP(i, 0, M) {
    ll x = ranges::lower_bound(zero, S[i]) - zero.begin();
    ll y = ranges::lower_bound(zero, T[i]) - zero.begin();
