...
// @@ !! LIM(UnionFind mod debug forall)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  // @InpGrid(N, N, A) [ZK4aW7Kb]
  auto A = vector(N, vector(N, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) { ll v; cin >> v; A[i][j] = v; }
  // @End [ZK4aW7Kb]
  UnionFind ufR(N), ufC(N);
  REP(r1, 0, N) REP(r2, r1 + 1, N) if (FORALL(c, 0, N, A[r1][c] + A[r2][c] <= K)) ufR.merge(r1, r2);
  REP(c1, 0, N) REP(c2, c1 + 1, N) if (FORALL(r, 0, N, A[r][c1] + A[r][c2] <= K)) ufC.merge(c1, c2);
  Comb<Fp> cb(N);
  Fp ans = 1;
  REP(r, 0, N) if (ufR.leader(r) == r) ans *= cb.fact(ufR.groupSize(r));
  REP(c, 0, N) if (ufC.leader(c) == c) ans *= cb.fact(ufC.groupSize(c));
