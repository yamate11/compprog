...
int main(/* int argc, char *argv[] */) {
  ...
  */
  CoordCompr cc;
  REP(i, 0, N) cc.add(B[i]);
  ll sz = cc.size();

  ll ans = 0;
  auto st = make_seg_tree<ll>(0LL, plus<ll>(), vector<ll>(sz, 0LL));
  for (auto& [a, bs] : mp) {
    for (auto& [b, v] : bs) {
      ll ccb = cc.c(b);
      st.update(ccb, st[ccb] + v);
      ans += v * st.query(ccb, sz);
    }
  }
  cout << ans << endl;
