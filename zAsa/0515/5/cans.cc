...
int main(/* int argc, char *argv[] */) {
  ...
  string S, T; cin >> S >> T;
  vector<ll> cands;
  REP(i, 0, N) cands.push_back(i);
  auto match = [&](ll i) -> bool {
    if (i < 0) return false;
    if (i + M > N) return false;
    if (FORALL(j, 0, M, S[i + j] == '#')) return false;
    REP(j, 0, M) {
      char c = S[i + j];
      if (not (c == T[j] or c == '#')) return false;
    }
    return true;
  };
  while (not cands.empty()) {
    ...
    if (match(i)) {
      DLOGKL("match", S, i);
      REP(j, 0, M) S[i + j] = '#';
      REP(j, i - M + 1, i + M - 1 + 1) cands.push_back(j);
    }else {
      DLOGKL("not match", S, i);
    }
  }
  cout << (FORALL(i, 0, N, S[i] == '#') ? "Yes\n" : "No\n");
