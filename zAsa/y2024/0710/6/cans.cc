...
int main(/* int argc, char *argv[] */) {
  ...
  vector<BrdIdx> P;
  REP(i, 0, N) REP(j, 0, M) {
    if (brd.at(i, j) == 'o') P.emplace_back(i, j);
  }
  ...
  REP(i, 0, N) REP(j, 0, M) {
    BrdIdx bi(i, j);
    if (brd.at(bi) != '#') {
      if (brd.at(bi + fdir1) != '#') graph.add_edge(brd.enc(bi), brd.enc(bi + fdir1), big, 0);
      if (brd.at(bi + fdir2) != '#') graph.add_edge(brd.enc(bi), brd.enc(bi + fdir2), big, 0);
      graph.add_edge(brd.enc(bi), g_sink, 1, (N - i) + (M - j));
    }
  }
  for (BrdIdx bi : P) {
    graph.add_edge(g_src, brd.enc(bi), 1, 0);
  }
  auto [flow, cost] = graph.flow(g_src, g_sink);
  DLOGK(flow);
#if DEBUG
  auto es = graph.edges();
  for (auto& e : es) {
    DLOGK(e.from, e.to, e.cap, e.flow, e.cost);
  }
#endif
  // assert(flow == ssize(P));
  ll base = 0;
