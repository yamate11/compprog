...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, H) REP(j, 0, W) {
    ...
    if (brd.at(bi) == '#') {
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (brd.at(bj) == '#') uf.merge(brd.enc(bi), brd.enc(bj));
      }
    }
  }
  ...
  REP(i, 0, H) REP(j, 0, W) {
    BrdIdx bi(i, j);
    if (brd.at(bi) == '#' and uf.leader(brd.enc(bi)) == brd.enc(bi)) numcc++;
  }
  ll num_red = 0;
  ll diff = 0;
  REP(i, 0, H) REP(j, 0, W) {
    ...
    if (brd.at(bi) == '.') {
      ...
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (brd.at(bj) == '#') ss.insert(uf.leader(brd.enc(bj)));
      }
      diff += 1 - ssize(ss);
    }
  }
  cout << Fp(numcc) + Fp(diff) / Fp(num_red) << endl;
