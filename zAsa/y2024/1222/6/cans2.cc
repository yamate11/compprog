...
int main(/* int argc, char *argv[] */) {
  ...
  itv_set<ll> cur(-1, 2*N + 2);
  cur.put(N, 1);
  for (auto& [x, ys] : mp) {
    vector<ll> add;
    vector<ll> rem;
    for (ll y : ys) {
      if (cur.get_val(y) == 1 and cur.get_val(y - 1) == 0 and cur.get_val(y + 1) == 0) rem.push_back(y);
      if (cur.get_val(y) == 0 and (cur.get_val(y - 1) == 1 or cur.get_val(y + 1) == 1)) add.push_back(y);
    }
    for (ll y : add) cur.put(y, 1);
    for (ll y : rem) cur.put(y, 0);
  }
  cout << cur.sum(-1, 2*N + 2) << endl;
