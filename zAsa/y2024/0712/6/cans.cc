...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&](ll r, bool b0, bool b1, bool b2) -> bool {
    REP(c, 0, W) {
      BrdIdx bi(r, c);
      auto sub = [&](auto dir) -> bool {
        auto bj = bi + dir;
        if (not brd.in(bj)) return false;
        bool rr = bj.r == r - 1 ? b0 : bj.r == r ? b1 : b2;
        return (brd.at(bi) == b1) == (brd.at(bj) == rr);
      };
      if (not EXISTS_C(dir, BrdIdx::nbr4, sub(dir))) return false;
    }
    return true;
  };

  ll big = 1e4;
  vector<ll> tbl_init(4, big);
  auto tbl = tbl_init;
  REP(x, 0, 4) {
    if (check(0, 0, x >> 1 & 1, x & 1)) tbl[x] = popcount((u64)x);
    else tbl[x] = big;
  }
  REP(i, 1, H - 1) {
    auto prev = move(tbl);
    ...
    REP(x, 0, 4) REP(b, 0, 2) {
      if (prev[x] < big) {
        if (check(i, x >> 1 & 1, x & 1, b)) updMin(tbl[((x << 1) & 3) | b], prev[x] + b);
      }
    }
  }
  ll ans = big;
  REP(x, 0, 4) {
    if (check(H - 1, x >> 1 & 1, x & 1, 0)) updMin(ans, tbl[x]);
  }
  if (ans == big) ans = -1;
  cout << ans << endl;
