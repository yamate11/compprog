...
int body(istream& cin, ostream& cout) {
  ...
  string S; cin >> S;
  vector tbl_init(2, vector(2, vector(2, pll())));
  auto tbl = tbl_init;
  tbl[1][1][1] = pll(1, 0);

  auto myadd = [&](ll eq, ll az, ll eff, ll v_inst, ll v_ones) -> void {
    pll& p = tbl[eq][az][eff];
    p.first += v_inst;
    p.second += v_ones;
  };
  ...
  for (char c : S) {
    ...
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(eq, 0, 2) REP(az, 0, 2) REP(eff, 0, 2) {
      if (prev[eq][az][eff].first == 0) continue;
      REP(x, 0, 10) {
        if (eq and x > d) continue;
        ll new_eq = eq and x == d;
        ll new_az = az and x == 0;
        ll new_eff = new_az or (eff and x == 1);
        auto [num_inst, num_ones] = prev[eq][az][eff];
        if (eff and x == 1) myadd(new_eq, new_az, new_eff, num_inst, num_ones + num_inst);
        else myadd(new_eq, new_az, new_eff, num_inst, num_ones);
        DLOGK(eq, az, eff, x, new_eq, new_az, new_eff, tbl[new_eq][new_az][new_eff]);
      }
    }
  }
  
  ll ans = 0;
  REP(eq, 0, 2) REP(eff, 0, 2) ans += tbl[eq][0][eff].second;
  cout << ans << endl;
