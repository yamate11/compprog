...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) REP(j, 0, N) {
    if (brd.at(i, j) == 'P') {
      if (initA.r < 0) initA = BrdIdx(i, j);
      else initB = BrdIdx(i, j);
      brd.rs(i, j) = '.';
    }
  }
  ll big = 1e18;
  ...
  while (not que.empty()) {
    ...
    for (auto dir : BrdIdx::nbr4) {
      REP(x, 0, 2) {
        auto p1 = p - dir;
        if (x == 1) {
          if (brd.at(p + dir) != '#') continue;
          p1 = p;
        }
        if (brd.at(p1) != '.') continue;
        REP(y, 0, 2) {
          auto q1 = q - dir;
          if (y == 1) {
            if (brd.at(q + dir) != '#') continue;
            q1 = q;
          }
          if (brd.at(q1) != '.') continue;
          if (dist[p1.r][p1.c][q1.r][q1.c] < big) continue;
          dist[p1.r][p1.c][q1.r][q1.c] = dist[p.r][p.c][q.r][q.c] + 1;
          que.emplace(p1, q1);
        }
      }
    }
  }
  ll ans = dist[initA.r][initA.c][initB.r][initB.c];
  if (ans == big) ans = -1;
  cout << ans << endl;
