...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> V{2,5,5,4,5,6,3,7,6};

  ll N, M; cin >> N >> M;
  // @InpVec(M, A, dec=1) [CH3MJ5yM]
  auto A = vector(M, ll());
  ...
  vector<ll> B;
  for (ll a : A) {
    if (FORALL_C(b, B, V[a] != V[b])) B.push_back(a);
  }
  sort(ALL(B), [&](ll x, ll y) { return V[x] < V[y]; });
  ll sz = ssize(B);
  DLOGK(B);
  vector lims(sz - 1, 0LL);
  REP(i, 1, sz) {
    lims[i - 1] = lcm(V[B[0]], V[B[i]]);
  }

  auto better = [&](auto& c, auto& a) -> bool {
    if (ssize(a) == 0) return true;
    assert(ssize(c) == ssize(a));
    REP(i, 0, ssize(c)) {
      auto [cb, cm] = c[i];
      auto [ab, am] = a[i];
      assert(cb == ab);
      if (cm != am) return cm > am;
    }
    assert(0);
  };
  ...
  while (did.get()) {
    ll cnt = 0;
    REP(i, 0, sz - 1) cnt += did.at(i) * V[B[i + 1]];
    ll rem = N - cnt;
    if (rem >= 0 and rem % V[B[0]] == 0) {
      DLOGK(did.vec_view(), cnt, rem, V[B[0]]);
      vector<pll> cand;
      ll cand_len = rem / V[B[0]];
      cand.emplace_back(B[0], rem / V[B[0]]);
      REP(i, 0, sz - 1) {
        cand_len += did.at(i);
        cand.emplace_back(B[i + 1], did.at(i));
      }
      sort(ALL(cand), greater<pll>());
      if (cand_len > ans_len or (cand_len == ans_len and better(cand, ans))) {
        DLOGK(cand_len, cand);
        ans_len = cand_len;
        ans = move(cand);
      }
