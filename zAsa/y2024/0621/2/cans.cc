...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpGrid(9, 9, A, dec=1) [ErW3qLdu]
  auto A = vector(9, vector(9, ll()));
  for (int i = 0; i < 9; i++) for (int j = 0; j < 9; j++) { ll v; cin >> v; v -= 1; A[i][j] = v; }
  // @End [ErW3qLdu]
  
  vector<bool> row(9), col(9), blk(9);
  auto dec = [&](ll e) -> pll { return pll(e / 3, e % 3); };
  auto f = [&](ll i, ll j, ll n) -> bool {
    auto [ix, iy] = dec(i);
    auto [jx, jy] = dec(j);
    return A[3 * ix + jx][3 * iy + jy] == n;
  };

  REP(i, 0, 9) row[i] = FORALL(n, 0, 9, EXISTS(j, 0, 9, A[i][j] == n));
  REP(i, 0, 9) col[i] = FORALL(n, 0, 9, EXISTS(j, 0, 9, A[j][i] == n));
  REP(i, 0, 9) blk[i] = FORALL(n, 0, 9, EXISTS(j, 0, 9, f(i, j, n)));
  bool ans = FORALL(i, 0, 9, row[i]) and FORALL(i, 0, 9, col[i]) and FORALL(i, 0, 9, blk[i]);
