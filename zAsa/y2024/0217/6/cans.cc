...
// @@ !! LIM(input binsearch UnionFind ftwo)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, nbr, dec=1, read=W) [JcT8Zwmo]
  struct nbr_t {
    int nd;
    ll W;
    // nbr_t() {}
  ...
  REP(i, 0, N) {
    vector<ll> V;
    if (ssize(nbr[i]) >= 2) {
      for (const auto& p : nbr[i]) V.push_back(p.W);
      sort(ALL(V));
      vmin = min(vmin, V[0] + V[1]);
    }
  }
  auto check = [&](ll X) -> bool {
    UnionFind<Ftwo> uf(N);
    REP(i, 0, N) {
      for (const auto& p : nbr[i]) {
        if (p.W < X) uf.merge(i, p.nd, Ftwo(1));
      }
    }
    REP(i, 0, N) {
      if (not uf.pot(i)) return false;
    }
    return true;
  };
  ll ans = binsearch_i<ll>(check, 1, vmin + 1);
  cout << ans << endl;
