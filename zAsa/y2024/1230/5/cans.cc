...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  string S; cin >> S;
  vector<bool> par(1LL << K);
  REP(i, 0, 1LL << K) {
    if (FORALL(j, 0, K, (i >> j & 1) == (i >> (K - 1 - j) & 1))) par[i] = true;
    else par[i] = false;
  }
  DLOGK(par);
  vector tbl_init(1LL << K, Fp(0));
  auto tbl = tbl_init;
  REP(i, 0, 1LL << (K - 1)) {
    if (FORALL(j, 0, K - 1, IMPLIES(i >> j & 1, S[K - 2 - j] != 'A') and IMPLIES(not (i >> j & 1), S[K - 2 - j] != 'B'))) {
      tbl[i] = Fp(1);
    }
  }
  DLOGKL("init", tbl);
