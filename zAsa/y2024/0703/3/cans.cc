...
// @@ !! LIM(cmpNaive perm forall debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto possible = [&]() -> bool {
    ...
    while (ip.get()) {
      vector<ll> S(N + 1);
      REP(i, 0, N) S[i + 1] = S[i] + A[ip.at(i)];
      ll lo_max = -1;
      ll hi_min = N + 2;
      REP(i, 0, N + 1) {
        if (S[i] < K) lo_max = max(lo_max, i);
        else hi_min = min(hi_min, i);
      }
      if (lo_max < hi_min) {
        DLOGKL("possible", ip.vec_view());
        return true;
      }
    }
    return false;
