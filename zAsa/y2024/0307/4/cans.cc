...
int main(/* int argc, char *argv[] */) {
  ...
  using node_t = vector<ll>;
  map<node_t, ll> mp;
  node_t init_node(K, 0LL);
  mp[init_node] = 0;
  node_t goal(K, P);
  REP(i, 0, N) {
    IntDupPerm dip(P + 1, K);
    while (dip.get()) {
      node_t nd = goal;
      REP(j, 0, K) nd[j] -= dip.at(j);
      auto it = mp.find(nd);
      if (it != mp.end()) {
        ll new_val = it->second + C[i];
        node_t peer(K);
        REP(j, 0, K) peer[j] = min(P, nd[j] + A[i][j]);
        auto it2 = mp.find(peer);
        if (it2 == mp.end()) mp[peer] = new_val;
        else updMin(it2->second, new_val);
      }
    }
  }
  auto it = mp.find(goal);
  ll ans = it == mp.end() ? -1 : it->second;
  cout << ans << endl;
