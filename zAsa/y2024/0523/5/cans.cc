...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  // @InpVec(H, S, type=string) [n4LK3EI4]
  auto S = vector(H, string());
  for (int i = 0; i < H; i++) { string v; cin >> v; S[i] = v; }
  // @End [n4LK3EI4]
  
  vector tbl(H, vector(W, Fp(0)));
  vector vert(H + 1, vector(W + 1, Fp(0)));
  vector horiz(H + 1, vector(W + 1, Fp(0)));
  vector diag(H + 1, vector(W + 1, Fp(0)));
  REPrev(i, H - 1, 0) {
    REPrev(j, W - 1, 0) {
      if (i == H - 1 and j == W - 1) {
        tbl[i][j] = Fp(1);
        vert[i][j] = vert[i + 1][j] + tbl[i][j];
        horiz[i][j] = horiz[i][j + 1] + tbl[i][j];
        diag[i][j] = diag[i + 1][j + 1] + tbl[i][j];
      }else if (S[i][j] == '#') {
        ;
      }else {
        tbl[i][j] = vert[i + 1][j] + horiz[i][j + 1] + diag[i + 1][j + 1];
        vert[i][j] = vert[i + 1][j] + tbl[i][j];
        horiz[i][j] = horiz[i][j + 1] + tbl[i][j];
        diag[i][j] = diag[i + 1][j + 1] + tbl[i][j];
      }
