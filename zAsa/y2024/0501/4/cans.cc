...
// @@ !! LIM(forall binsearch perm cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  REP(k, 1, N + 1) {
    ...
    while (dip.get()) {
      ...
      REP(i, 0, k) {
        string v;
        v = u.substr(1, dip.at(i));
        v += u[0];
        v += u.substr(dip.at(i) + 1);
        u = v;
      }
      if (u == T) {
        cout << k << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  if (EXISTS(i, 0, 26, numsS[i] != numsT[i])) {
    cout << -1 << endl;
    return 0;
  }
  
  auto check = [&](ll i0) -> bool {
    ll p = 0;
    REP(i, i0, N) {
      while (p < N and T[p] != S[i]) p++;
      if (p == N) return false;
      p++;
    }
    return true;
