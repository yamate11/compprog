...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpNbrList(N, N - 1, nbr, dec=1) [B5cRnfrd]
  ...
  REP(k, 0, lim) {
    ...
    auto dfs = [&](auto rF, ll nd, ll pt) -> void {
      ...
      for (ll cld : nbr[nd]) {
        ...
        F[nd][0] = F[nd][1] = Fp(0);
        rF(rF, cld, nd);
        F[nd][1] += prev[1] * F[cld][0] + prev[0] * F[cld][1] + prev[1] * F[cld][1];
        F[nd][0] += prev[0] * F[cld][0] + prev[1] * F[cld][1] + prev[0] * F[cld][1];
      }
    };
    dfs(dfs, 0, -1);
    ans += F[0][1] * Fp(1LL << k);
