...
int main(/* int argc, char *argv[] */) {
  ...
  auto flip(IPoint p) -> IPoint { return IPoint(p.y, p.x); };

  auto op1 = [&](IPoint p1, IPoint p2, IPoint p3) -> ll {
    auto sub = [&](IPoint q1, IPoint q2, IPoint q3) -> ll {
      if (not (q1.x == q2.x and q2.x == q3.x)) return false;
      return (q1.y < q3.y) == (q3.y < q2.y);
    };
    ll add = sub(p1, p2, p3) or sub(flip(p1), flip(p2), flip(p3)) ? 2 : 0;
    return abs(p1.x - p2.x) + abs(p1.y - p2.y) + add;
  };

  auto g = [&](IPoint pos, IPoint start, IPoint goal) -> pair<ll, IPoint> {
    if (start == goal) return {0, pos};
    IPoint dir = goal - start;
    ll e = max(abs(dir.x), abs(dir.y));
    IPoint edir = dir / e;
    IPoint pos2 = start - edir;
    ll x = op1(pos, pos2, start) + e;
    return {x, goal - edir};
  }

  auto f = [&](IPoint pos, IPoint start, IPoint mid, IPoint goal) -> ll {
    auto [x, pos2] = g(pos, start, mid);
    auto [y, pos3] = g(pos2, mid, goal);
    return x + y;
  };

  IPoint pos, start, goal; cin >> pos >> start >> goal;
  IPoint mid1(start.x, goal.y);
  IPoint mid2(start.y, goal.x);
  ll ans = min(f(pos, start, mid1, goal), f(pos, start, mid2, goal));
  cout << ans << endl;
