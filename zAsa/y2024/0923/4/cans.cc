...
int main(/* int argc, char *argv[] */) {
  ...
  string W;
  cin >> x >> y >> W;
  // @InpVec(9, S, type=string) [vl8Ai8jJ]
  auto S = vector(9, string());
  for (int i = 0; i < 9; i++) { string v; cin >> v; S[i] = v; }
  // @End [vl8Ai8jJ]

  IPoint p(y - 1, x - 1);
  IPoint dir(0, 0);
  for (char c : W) {
    if (c == 'R') dir += IPoint(0,  1);
    if (c == 'L') dir += IPoint(0, -1);
    if (c == 'U') dir += IPoint(-1, 0);
    if (c == 'D') dir += IPoint( 1, 0);
  }
  auto nxt = [&](ll a, ll b) -> pll {
    if (b == 0) return pll(a, b);
    else if (b == 1) {
      if (a == 8) return pll(7, -1);
      else return pll(a + 1, 1);
    }
    else if (b == -1) {
      if (a == 0) return pll(1, 1);
      else return pll(a - 1, -1);
    }
    else assert(0);
  };
  ...
  REP(i, 0, 4) {
    ...
    IPoint q = p;
    IPoint dd = dir;
    tie(p.x, dir.x) = nxt(q.x, dd.x);
    tie(p.y, dir.y) = nxt(q.y, dd.y);
  }
  cout << ans << endl;
