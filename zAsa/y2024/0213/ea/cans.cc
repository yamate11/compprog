...
bool is_prime(ll x) {
  ...
  REP(k, 0, K) {
    ll a = rand.range(1, x);
    ll t = d;
    ll y = (ll)power<Fp>(a, t);
    while (t != x - 1 and y != 1 and y != x - 1) {
      y = (y * y) % x;
      t <<= 1;
    }
    if (y != x - 1 and (t & 1) == 0) return false;
  }
  return true;
}

ll get_prime(ll from, ll to) {
  static Random rand;
  while (true) {
    ll p = rand.range(from, to);
...
int main(/* int argc, char *argv[] */) {
  ...
  /*
  ll lim = 1000000;
  auto primes = sieve(lim + 1);
  vector<bool> vec(lim + 1, false);
  for (int p : primes) vec[p] = true;
  REP(x, 1, lim) {
    if (vec[x] != is_prime(x)) {
      cout << x << " " << vec[x] << endl;
      return 0;
  ...
  */
  ...
  ll from = (ll)1e10;
  ll to = (ll)1e11;
  ll p1 = get_prime(from, to);
  ll p2 = get_prime(from, to);
  cout << p1 << " " << p2 << endl;
