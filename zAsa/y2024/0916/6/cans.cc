...
int main(/* int argc, char *argv[] */) {
  ...
  auto check0 = [&](double x) -> bool {
    double alpha = 100.0 / x - 1.0;
    vector vec(M, 0.0);
    REP(i, 0, M) vec[i] = alpha * C[i] - D[i];
    sort(ALL(vec));
    ll cnt = 0;
    REP(i, 0, N) {
      ll k = vec.end() - lower_bound(ALL(vec), -alpha * A[i] + B[i]);
      cnt += k;
    }
    return cnt >= K;
  };
  double ans = binsearch<double>(check0, 0.0, 100.0, 1e-10);
  cout << ans << endl;
