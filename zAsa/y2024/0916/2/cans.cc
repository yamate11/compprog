...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  // @InpGrid(N, N, T) [FW2gYUgX]
  auto T = vector(N, vector(N, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) { ll v; cin >> v; T[i][j] = v; }
  // @End [FW2gYUgX]
  ...
  while (ip.get()) {
    DLOGK(ip.vec_view());
    ll s = T[0][ip.at(0) + 1] + T[ip.at(N - 2) + 1][0];
    DLOGK(s);
    REP(i, 0, N - 2) s += T[ip.at(i) + 1][ip.at(i + 1) + 1];
    DLOGK(s);
    if (s == K) cnt++;
  }
  cout << cnt << endl;
