...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpNbrList(N, N - 1, nbr, dec=1, read=C) [1Leyb5U9]
  struct nbr_t {
    int nd;
    ll C;
    // nbr_t() {}
  ...
  REP(i, 0, N) for (auto [j, c] : nbr[i]) dist[i][j] = c;

  Tree tr(N);
  REP(i, 0, N) {
    for (auto [j, c] : nbr[i]) if (i < j) tr.add_edge(i, j);
  }
  auto myadd = [&](ll a, ll b) -> ll { return max(a, b); };
  auto mod = [&](ll a, ll nd, ll cld) -> ll { return max(a, D[cld]) + dist[nd][cld]; };
  auto result = reroot(tr, 0LL, myadd, mod);
