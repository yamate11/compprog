...
// @@ !! LIM(segTree coordCompr mod debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Fp> pow2(N + 1);
  pow2[0] = 1;
  REP(i, 1, N + 1) pow2[i] = pow2[i - 1] * Fp(2);

  auto apply = [&](int f, Fp x) -> Fp { return pow2[f] * x; };
  auto st = make_seg_tree_lazy(Fp(0), 0, plus<Fp>(), plus<int>(), apply, vector(sz, Fp(0)));
  st.rs(cc.c(A[0])) = 1;
  Fp ans = 0;
  REP(i, 1, N) {
    ll p = cc.c(A[i]);
