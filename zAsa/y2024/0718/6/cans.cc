...
// @@ !! LIM(f:updMaxMin debug cmpNaive perm forall)
...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = -1e18;
  while(ic.get()) {
    bool b = FORALL(i, 0, M - 1, ic.at(i) + 1 < ic.at(i + 1));
    if (b) {
      ll s = 0;
      REP(i, 0, M) s += A[ic.at(i)];
      ans = max(ans, s);
    }
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  ll t = (N % 2 == 0) ? 2 : 3;
  ll big = 1e18;
  vector tbl_init(2, vector(t, -big));
  auto tbl = tbl_init;
  tbl[0][0] = 0;
  REP(i, 0, N) {
    auto prev = move(tbl);
    ...
    // prev skip
    // this skip
    REP(j, 0, t - 1) updMax(tbl[0][j + 1], prev[0][j]);
    // this do
    REP(j, 0, t) updMax(tbl[1][j], prev[0][j] + A[i]);
    // prev do
    // this skip
    REP(j, 0, t) updMax(tbl[0][j], prev[1][j]);
  }
  cout << max(tbl[0][t - 1], max(tbl[1][t - 1], max(tbl[0][t - 2], tbl[1][t - 2]))) << endl;
