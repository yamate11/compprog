...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W, N; cin >> H >> W >> N;
  IPoint start; cin >> start;
  IPoint goal; cin >> goal;
  // @InpVec(N, P, type=IPoint ) [5GGViDBL]
  auto P = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; P[i] = v; }
  // @End [5GGViDBL]

  map<ll, vector<ll>> Horiz;
  map<ll, vector<ll>> Vert;
  REP(i, 0, N) {
    Horiz[P[i].x].push_back(P[i].y);
    Vert[ P[i].y].push_back(P[i].x);
  }
  for (auto& [i, v] : Horiz) {
    v.push_back(-1);
    v.push_back(W + 1);
  ...
  while (not que.empty()) {
    ...
    vector<IPoint> cands;
    {
      auto it = Horiz.find(p.x);
      if (it != Horiz.end()) {
        auto& vec = it->second;
        ll k = lower_bound(ALL(vec), p.y) - vec.begin();
        if (vec[k] != W + 1)  cands.emplace_back(p.x, vec[k    ] - 1);
        if (vec[k - 1] != -1) cands.emplace_back(p.x, vec[k - 1] + 1);
      }
    }
    {
      auto it = Vert.find(p.y);
      if (it != Vert.end()) {
        auto& vec = it->second;
        ll k = lower_bound(ALL(vec), p.x) - vec.begin();
        if (vec[k] != H + 1)  cands.emplace_back(vec[k    ] - 1, p.y);
        if (vec[k - 1] != -1) cands.emplace_back(vec[k - 1] + 1, p.y);
      }
    }
    for (auto q : cands) {
      if (not dist.contains(q)) {
        dist[q] = cur_dist + 1;
        que.push(q);
      }
    }
