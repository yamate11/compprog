...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [sUDRWG6a]
  // @InpMVec(M, ((A, dec=1), (B, dec=1))) [9ZTFET2E]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  REP(i, 0, N) {
    if (D[i] > 0) F[uf.leader(i)].push_back(i);
  }
  vector<ll> st1, st2;
  REP(i, 0, N) if (uf.leader(i) == i) {
    if (E[i] == 1) st1.push_back(i);
    else if (E[i] >= 2) st2.push_back(i);
    else assert(0);
  }
  DLOGK(st1, st2);
  vector<pll> ans;
  ...
  while (true) {
    if (st2.empty()) {
      if (not st1.empty()) {
        assert(ssize(st1) == 2);
        ans.emplace_back(F[st1[0]].back(), F[st1[1]].back());
      }
      break;
    }
    ll b = st2.back();
    ll a = st1.back(); st1.pop_back();
    assert(ssize(F[a]) == 1);
    ll x = F[a].back();
    ll y = F[b].back();
    ans.emplace_back(x, y);
    D[y]--;
    if (D[y] == 0) F[b].pop_back();
    E[b]--;
    if (E[b] == 1) {
      st1.push_back(b);
      st2.pop_back();
