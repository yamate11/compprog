...
int main(/* int argc, char *argv[] */) {
  ...
  vector<double> pow(N * M + 1);
  pow[0] = 1.0;
  REP(i, 1, N * M + 1) pow[i] = pow[i - 1] * 0.99;
  
  auto bright = [&](BrdIdx bi, ll d) -> double {
    char c = brd.at(bi);
    if (c == 's' or c == 'g') return 10.0;
    if (c == '#') return -10.0;
    double x = c - '0';
    return x * pow[d];
  };
  ...
  ll big = 1e9;
  auto check = [&](double th) -> bool {
    Board<ll> dist(N, M, big);
    dist.rs(start) = 0;
    queue<BrdIdx> que;
    que.push(start);
    while (not que.empty()) {
      BrdIdx bi = que.front(); que.pop();
      ll curdist = dist.at(bi);
      for (BrdIdx dir : BrdIdx::nbr4) {
        BrdIdx bj = bi + dir;
        if (dist.at(bj) >= big and bright(bj, curdist + 1) >= th) {
          dist.rs(bj) = curdist + 1;
          que.push(bj);
        }
      }
    }
    return dist.at(goal) < big;
  };
  double ans = binsearch<double>(check, -1, 10, 1e-11);
  if (ans < 0) {
    cout << "-1\n";
  }else {
    cout << ans << endl;
