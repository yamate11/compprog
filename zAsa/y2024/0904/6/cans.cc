...
int main(/* int argc, char *argv[] */) {
  ...
  Board<double> ans(N, M, -1.0);
  ans.rs(goal) = 10.0;
  using sta = pair<double, BrdIdx>;
  priority_queue<sta> pque;
  pque.emplace(10.0, goal);
  while (not pque.empty()) {
    auto [a, bi] = pque.top(); pque.pop();
    for (auto dir : BrdIdx::nbr4) {
      auto bj = bi + dir;
      char c = brd.at(bj);
      if (c != '#' and ans.at(bj) < 0.0) {
        double v = (c == 's' or c == 'g') ? 10.0 : (double)(c - '0');
        double val = min(0.99 * a, v);
        ans.rs(bj) = val;
        pque.emplace(val, bj);
      }
    }
