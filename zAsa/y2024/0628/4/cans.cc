...
int main(/* int argc, char *argv[] */) {
  ...
  Board<ll> brd(H, W, -1);
  REP(i, 0, H) REP(j, 0, W) {
    ll a; cin >> a; brd.rs(i, j) = a;
  }
  Board<BrdIdx> dir(H, W, BrdIdx(0, 0));
  REP(i, 0, H) REP(j, 0, W) {
    if (i % 2 == 0) dir.rs(i, j) = BrdIdx(0, 1);
    else dir.rs(i, j) = BrdIdx(0, -1);
  }
  REP(i, 0, H) {
    if (i % 2 != 0) dir.rs(i, 0) = BrdIdx(1, 0);
    else dir.rs(i, W - 1) = BrdIdx(1, 0);
  }
  if (H % 2 == 0) dir.rs(H - 1, 0) = BrdIdx(0, 0);
  else dir.rs(H - 1, W - 1) = BrdIdx(0, 0);
  BrdIdx cur(0, 0);
  BrdIdx target(-1, -1);
  vector<BrdIdx> asrc;
  vector<BrdIdx> adest;
  while (true) {
    ...
    if (dir.at(cur) == BrdIdx(0, 0)) break;
    cur = cur + dir.at(cur);
  }
  cout << ssize(asrc) << "\n";
  REP(i, 0, ssize(asrc)) {
    cout << asrc[i].r + 1 << " " << asrc[i].c + 1 << " " << adest[i].r + 1 << " " << adest[i].c + 1 << "\n";
  }
