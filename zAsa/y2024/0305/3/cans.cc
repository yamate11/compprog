...
int main(/* int argc, char *argv[] */) {
  ...
  map<ll, ll> mp;
  REP(i, 0, ssize(primes)) mp[primes[i]] = i;
  ...
  auto g = [&](ll x) -> ll {
    ll ret = 1;
    REP(i, 0, ssize(primes)) if (x >> i & 1) ret *= primes[i];
    return ret;
  };
  ...
  vector A(N, 0LL);
  REP(i, 0, N) A[i] = func(X[i]);
  ll ans = 1e18;
  REP(x, 0, 1LL << ssize(primes)) {
    if (FORALL(i, 0, N, A[i] & x)) ans = min(ans, g(x));
  }
  cout << ans << endl;
