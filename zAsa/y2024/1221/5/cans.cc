...
int main(/* int argc, char *argv[] */) {
  ...
  vector B(H + 1, vector(W + 1, 0LL));
  REP(i, 0, H) REP(j, 0, W) B[i + 1][j + 1] = A[i][j];
  REP(i, 1, H + 1) REP(j, 1, W + 1) B[i][j] += B[i][j - 1];
  REP(i, 1, H + 1) REP(j, 1, W + 1) B[i][j] += B[i - 1][j];
  ll ans = 0;
  REP(i, 0, H) {
    ll x = 0;
    REP(j, 0, W) {
      if (x > 0) x--;
      while (true) {
        if (i + x == H or j + x == W) break;
        if (B[i][j] + B[i + x + 1][j + x + 1] - B[i + x + 1][j] - B[i][j + x + 1] > 0) break;
        x++;
      }
      ans += x;
    }
