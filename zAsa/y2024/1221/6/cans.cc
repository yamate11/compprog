...
int main(/* int argc, char *argv[] */) {
  ...
  auto enc1 = [&](ll i, ll j) -> u64 { return 1ULL << brd.enc(i, j); };
  auto enc2 = [&](BrdIdx bi) -> u64 { return enc1(bi.r, bi.c); };

  auto f = [&](auto& cur) -> set<u64> {
    set<u64> ret;
    for (u64 t : cur) {
      REP(i, 0, N) REP(j, 0, N) {
        if (brd.at(i, j) != '.' or (enc1(i, j) & t) != 0) continue;
        auto tmp = [&]() -> bool {
          BrdIdx bi(i, j);
          for (auto dir : BrdIdx::nbr4) {
            auto bj = bi + dir;
            if (brd.in(bj) and (enc2(bj) & t) != 0) return true;
          }
          return false;
        };
        if (tmp()) ret.insert(t | enc1(i, j));
      }
    }
    return ret;
  };
  ...
  set<u64> coll;
  REP(i, 0, N) REP(j, 0, N) {
    if (brd.at(i, j) == '.') coll.insert(enc1(i, j));
  }
  REP(k, 1, K) coll = f(coll);
  cout << ssize(coll) << endl;
