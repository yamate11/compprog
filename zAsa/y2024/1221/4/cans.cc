...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpVec(N + 1, A) [i1I4mzEe]
  auto A = vector(N + 1, ll());
  for (int i = 0; i < N + 1; i++) { ll v; cin >> v; A[i] = v; }
  // @End [i1I4mzEe]
  // @InpVec(N + M + 1, C) [hMQY5tEq]
  auto C = vector(N + M + 1, ll());
  for (int i = 0; i < N + M + 1; i++) { ll v; cin >> v; C[i] = v; }
  // @End [hMQY5tEq]

  PolyLL pa(A);
  PolyLL pc(C);
  auto [pb, pr] = pc.divmod(pa);
  REPOUT(i, 0, M + 1, pb.getCoef(i), " ");
