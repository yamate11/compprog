...
// @@ !! LIM(cmpNaive perm f:updMaxMin debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  REP(t, 0, min(N, M) + 1) {
    ...
    while (ip.get()) {
      ...
      while (iq.get()) {
        ll y = 0;
        REP(i, 0, t) if (A[ip.at(i)] != B[iq.at(i)]) y++;
        ll x = N - t + M - t;
        updMin(ans, x + y);
        if (x + y == 1) DLOGK(t, ip.vec_view(), iq.vec_view());
      }
    }
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 0, N) REP(j, 0, M) {
    ...
    ll y = tbl[i + 1][j];
    ll z = tbl[i][j + 1];
    if (A[i] == B[j]) tbl[i + 1][j + 1] = min(x, min(y + 1, z + 1));
    else tbl[i + 1][j + 1] = min(x + 1, min(y + 1, z + 1));
  }
  DLOGK(tbl);
  cout << tbl[N][M] << endl;
