...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((U, dec=1), (V, dec=1))) [cTUKXwr1]
  auto U = vector(M, ll());
  auto V = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  REP(i, 0, N) {
    ...
    else if (nn[i] == 1) leaf.push_back(i);
  }
  while (not leaf.empty()) {
    ll i = leaf.back(); leaf.pop_back();
    eff[i] = false;
    for (ll j : nbr[i]) {
      if (eff[j]) {
        nn[j]--;
