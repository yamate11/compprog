...
int main(/* int argc, char *argv[] */) {
  ...
  ll sz = ssize(v);
  vector<ll> num(10001, -1LL);
  vector<vector<ll>> eg(10001);
  REP(c, 1, 7) {
    IntDupComb ic(sz, c);
    while (ic.get()) {
      ll z = 0;
      REP(j, 0, c) z += v[ic.at(j)];
      if (z < 10001) {
        if (num[z] < 0) {
          num[z] = c;
          REP(j, 0, c) eg[z].push_back(v[ic.at(j)]);
          sort(ALL(eg[z]));
        }
      }
    }
  ...
  REP(x, 1, 10001) {
    cout << x << " " << num[x] << "    ";
    REP(j, 0, num[x]) cout << eg[x][j] << " ";
    cout << endl;
  }
  */
    
  vector tbl(10, vector(20, vector<pll>()));
  vector eff(10, vector(20, false));
  const auto& partition = [&](ll k, ll c) {
    if (not eff[k][c]) {
      eff[k][c] = true;
      auto g = [&](auto rF, ll acc, ll pnum, ll cur, ll d) -> void {
        if (d == c) {
          if (acc > 0 and acc % 10 == k) {
            tbl[k][c].emplace_back(acc, pnum);
          }
        }else {
          REP(z, cur, 4) {
            ll new_pnum = (z == 0) ? pnum : pnum + 1;
            rF(rF, acc + z, new_pnum, z, d + 1);
          }
        }
      };
      g(g, 0, 0, 0, 0);
  ...
  auto f = [&](auto rF, ll n, ll c, bool smaller) -> bool {
    if (n == 0) return true;
    ll k = n % 10;
    const auto& vs = partition(k, c);
    for (const auto& v : vs) {
      auto [m, c1] = v;
      if ((c == c1 or smaller) and n >= m) {
        if (rF(rF, (n - m) / 10, c1, true)) return true;
      }
    }
    return false;
  };
