...
int main(/* int argc, char *argv[] */) {
  ...
  REP(q, 0, Q) {
    ll l, r; cin >> l >> r; l--; r--;
    RLI.emplace_back(r, l, q);
  }
  sort(ALL(RLI), greater<sta>());
  ...
  auto st = make_seg_tree_lazy(big, 0LL, mymin, plus<ll>(), plus<ll>(), vector<ll>(N, 0LL));

  vector<ll> ans(Q);
  ll j = 0;
  REPrev(i, N - 1, 0) {
    while (true) {
      if (j == ssize(RLI)) break;
      auto [r, l, q] = RLI[j];
      if (r != i) break;
      j++;
      ans[q] = st.at(l);
    }
    st.update(corr[i], i, 1);
  }
  REPOUT(q, 0, Q, ans[q], "\n");
