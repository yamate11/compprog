...
int main(/* int argc, char *argv[] */) {
  ...
  map<ll, ll> mp;
  REP(i, 2, N + 1) {
    for (auto [p, r] : prfac(i, primes)) mp[p] += r;
  }
  vector<pll> vec;
  for (auto [p, n] : mp) vec.emplace_back(p, n);
  ll sz = ssize(vec);
  ll ans = 0;
  REP(i, 0, sz) {
    if (vec[i].second >= 74) ans++;
  }
  REP(i, 0, sz) REP(j, 0, sz) {
    if (i == j) continue;
    if (vec[i].second >= 24 and vec[j].second >= 2) ans++;
    if (vec[i].second >= 14 and vec[j].second >= 4) ans++;
  }
  REP(i, 0, sz) REP(j, i + 1, sz) {
    if (vec[i].second >= 4 and vec[j].second >= 4) {
      REP(k, 0, sz) if (k != i and k != j and vec[k].second >= 2) ans++;
    }
  }
  cout << ans << endl;
