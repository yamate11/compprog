...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(2*N, A) [d127YnAJ]
  auto A = vector(2*N, ll());
  for (int i = 0; i < 2*N; i++) { ll v; cin >> v; A[i] = v; }
  // @End [d127YnAJ]

  auto sia = sortedIndex(A);
  vector<ll> rev(2*N);
  REP(i, 0, 2*N) rev[sia[i]] = i;
  
  vector record(2, vector<bool>());
  string ans;
  REP(i, 0, 2*N) {
    bool cur = rev[i] < N;
    int parity = i % 2;
    auto& me = record[parity];
    auto& you = record[1 - parity];
    if (not you.empty() and you.back() != cur) {
      ans += ')';
      you.pop_back();
    }else {
      ans += '(';
  ...
  assert(record[0].empty() and record[1].empty());
  cout << ans << endl;
