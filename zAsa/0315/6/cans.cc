...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1e18;
  vector tbl(1LL << N, mgs<map<ll, ll>>(big));
  tbl[0][0] = 0;
  REP(x, 1, 1LL << N) {
    ...
    REP(t, 0, N) if (x >> t & 1) {
      auto [a, b] = vec[t];
      ll y = x ^ (1LL << t);
      ll face = ((k - 1 - t) % 2 == 0) ? a : b;
      ll sn = k - cnt - 1;
      for (auto [f, s] : tbl[y]) {
        if (f <= face) tbl[x][face] = min(tbl[x].get(face), s + sn);
      }
      cnt++;
    }
    DLOGK(x, tbl[x]);
  }
  const auto& mp = tbl[(1LL << N) - 1];
  if (mp.empty()) {
    cout << -1 << endl;
  }else {
    ll ans = big;
    for (auto [_, s] : mp) ans = min(ans, s);
    cout << ans << endl;
  }
