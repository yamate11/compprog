...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [07sDqYQR]
  // @InpMVec(M, ((A, dec=1), (B, dec=1), Z)) [GNBc2VFI]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  auto Z = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  ll big = 1LL << 60;
  auto f = [&](ll plane, ll ship) -> ll {
    using conn_tp = tuple<ll, ll, ll>;
    vector<conn_tp> conn;
    REP(i, 0, M) conn.emplace_back(Z[i], A[i], B[i]);
    if (plane) {
      REP(i, 0, N) conn.emplace_back(X[i], i, N);
    }
    if (ship) {
      REP(i, 0, N) conn.emplace_back(Y[i], i, N + 1);
    }
    ranges::sort(conn);
    UnionFind uf(N + 2);
    ll ret = 0;
    ...
    if (plane == 0 and ship == 0) {
      if (uf.group_size(0) == N) return ret;
      else return big;
    }else return ret;
  };
  ...
  ll cost2 = f(1, 0);
  ll cost3 = f(1, 1);
  ll ans = min(min(cost0, cost1), min(cost2, cost3));
  cout << ans << endl;
