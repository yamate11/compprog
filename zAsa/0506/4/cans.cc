...
int naive(istream& cin, ostream& cout) {
  ...
  while (ibb.get()) {
    ...
    ranges::sort(v);
    ans += v[0] + v[1] + 4;
    if (v[0] == v[1] and v[1] == v[2]) { cnt3 += v[2]; }
    else if (v[1] == v[2]) { cnt2 += v[2]; }
    else { cnt1 += v[2]; }
  }
...
int body(istream& cin, ostream& cout) {
  ...
  if (L % 3 == 0) {
    Fp lmax = L / 3;
    cnt3 = (lmax + 1) * (lmax + 1) * (lmax + 1) * 1 * lmax;
  }
  Fp cnt2 = 0;
  if (L >= 2) {
    REP(r, 0, L + 1) {
  ...
  REPrev(x, L, 0) {
    ...
    REP(y, 0, L - x + 1) {
      ...
      if (z < y) break;
      Fp lmax = x;
      Fp c = Fp(lmax + 1) * Fp(y + 1) * Fp(z + 1) * (y == z ? 3 : 6) * lmax;
      cnt1 += c;
      DLOGK(x, y, z, c);
    }
  }
  DLOGK(cnt1, cnt2, cnt3);
  Fp cnt = cnt1 + cnt2 + cnt3;
  Fp ans = (K - 2) * cb.binom(K - 1, 5) - cnt;
  cout << ans << endl;
