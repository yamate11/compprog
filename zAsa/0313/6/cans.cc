...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  // @InpMVec(Q, ((L, dec=1), (R, dec=0), D)) [nx4h8LcH]
  auto L = vector(Q, ll());
  auto R = vector(Q, ll());
  auto D = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  auto myadd = [&](sta p1, sta p2) -> sta {
    auto [x1, c1] = p1;
    auto [x2, c2] = p2;
    return {x1 + x2, c1 + c2};
  };
  auto mycomp = [&](ll h, ll g) -> ll {
    if (h == 0) return g;
    else return h;
  };
  auto myappl = [&](ll h, sta p) -> sta {
    if (h == 0) return p;
    auto [x, c] = p;
    return {c * h, c};
  };
  ...
  auto st = make_seg_tree_lazy<sta, ll>({0,0}, 0LL, myadd, mycomp, myappl, init_vec);
  
  REP(q, 0, Q) {
    st.update(L[q], R[q], D[q]);
