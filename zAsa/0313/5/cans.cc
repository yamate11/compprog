...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [t3Mpl95Q]
  ll voted = accumulate(ALL(A), 0LL);
  ll rest = K - voted;

  auto ord = sortedIndex(A, greater<ll>());
  vector B = A;
  ranges::sort(B, greater<ll>());
  vector S(N + 1, 0LL);
  ...
  REP(ii, 0, N) {
    ...
    auto check = [&](ll x) -> bool {
      ll t = B[ii] + x;
      ll j = lower_bound(ALL(B), t, greater<ll>()) - B.begin();
      if (ii < M) {
        ll y = (t + 1) * (M + 1 - j) - (S[M + 1] - S[j]) - 1;
        return y > rest;
      }else {
        if (j >= M) return false;
        ll y = (t + 1) * (M - j) - (S[M] - S[j]) + x;
        return y > rest;
      }
    };
    ll x = binsearch<ll>(check, rest + 1, -1);
    if (x > rest) x = -1;
    ans[i] = x;
  }
  REPOUT(i, 0, N, ans[i], " ");
