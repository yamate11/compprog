...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, H) REP(j, 0, W) {
    BrdIdx bi(i, j);
    if (brd.at(bi) == 'S') start = bi;
    if (brd.at(bi) == 'G') goal = bi;
  }
  auto closed = [&](ll sw, BrdIdx bi) -> bool {
    if (sw) return brd.at(bi) == 'o';
    else    return brd.at(bi) == 'x';
  };
  ...
  que.emplace(0, start);
  while (not que.empty()) {
    auto [sw, bi] = que.front(); que.pop();
    ll d = dist[sw][bi.r][bi.c];
    for (auto dir : BrdIdx::nbr4) {
      auto bj = bi + dir;
      if (brd.at(bj) == '#' or closed(sw, bj)) continue;
      ll new_sw = sw;
      if (brd.at(bj) == '?') new_sw = 1 - new_sw;
      ll old_d = dist[new_sw][bj.r][bj.c];
      if (old_d == big) {
        dist[new_sw][bj.r][bj.c] = d + 1;
        que.emplace(new_sw, bj);
      }
    }
  }
