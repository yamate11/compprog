...
int main(/* int argc, char *argv[] */) {
  ...
  ll R, C, K; cin >> R >> C >> K;
  // @InpVec(R, S, type=string) [dNLQ0spZ]
  ...
  REP(i, 0, R) REP(j, 0, C) {
    if (S[i][j] == 'x') {
      REP(d, -(K - 1), K) {
        ...
        if (brd.in(i + d, 0)) {
          BrdIdx bi0, bi1;
          if (brd.in(i + d, j - s)) bi0 = BrdIdx(i + d, j - s);
          else bi0 = BrdIdx(i + d, 0);
          if (brd.in(i + d, j + s + 1)) bi1 = BrdIdx(i + d, j + s + 1);
          else bi1 = BrdIdx(i + d, C);
          brd.rs(bi0) = brd.at(bi0) + 1;
          brd.rs(bi1) = brd.at(bi1) - 1;
          DLOGK(i, j, d, bi0, bi1);
        }
      }
    }
  ...
  REP(i, K - 1, R - (K - 1)) {
    ...
    REP(j, 0, C) {
      s += brd.at(i, j);
      if (K - 1 <= j and j < C - (K - 1) and s == 0) cnt++;
    }
  }
  cout << cnt << endl;
