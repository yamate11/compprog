...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&]() {
    ...
    ll big = 2 * N;
    auto stP = make_seg_tree_lazy(big, big, mymin, mymin, mymin, vector<ll>(N, big));
    auto stN = make_seg_tree_lazy(big, big, mymin, mymin, mymin, vector<ll>(N, big));

    REP(i, 0, N) {
      ll vP = P[i] + stP.at(P[i]);
      ll vN = -P[i] + stN.at(P[i]);
      ret[i] = i + min(vP, vN);
      stP.update(P[i], N, -P[i] - i);
      stN.update(0, P[i] + 1, P[i] - i);
    }
    return ret;
  };
  auto vec1 = f();
  ...
  reverse(ALL(vec2));
  vector<ll> ans(N);
  REP(i, 0, N) ans[i] = min(vec1[i], vec2[i]);
  REPOUT(i, 0, N, ans[i], " ");
