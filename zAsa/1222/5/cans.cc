...
// @@ !! LIM(doubling FuncGraph f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll ans = 0;
  REP(i, 0, N) {
    if (done[fg.ccid(i)]) continue;
    done[fg.ccid(i)] = true;
    ll sz = fg.cycle_size(i);
    ...
    vector<ll> S(2 * sz + 1, 0LL);
    REP(j, 0, 2*sz) S[j + 1] = S[j] + vec[j];
    ll tot = S[sz];
    ll ansL = 0;
    REP(j, 0, sz) {
      REP(k, 0, min(sz, K + 1)) {
