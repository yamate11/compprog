...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpGrid(3, 3, C) [k7Z8A9YF]
  auto C = vector(3, vector(3, ll()));
  for (int i = 0; i < 3; i++) for (int j = 0; j < 3; j++) { ll v; cin >> v; C[i][j] = v; }
  // @End [k7Z8A9YF]
  ...
  while (ip.get()) {
    auto check = [&]() -> bool {
      vector opened(3, vector(3, false));
      REP(i, 0, 9) {
        ll x = ip.at(i) / 3;
        ll y = ip.at(i) % 3;
        auto f = [&](ll x0, ll y0, ll dx, ll dy) -> bool {
          ll num_same = 0;
          ll num_open = 0;
          REP(k, 0, 3) {
            ll xx = x0 + k * dx;
            ...
            if (opened[xx][yy]) {
              num_open++;
              if (C[xx][yy] == C[x][y]) num_same++;
            }
          }
          return num_open == 1 and num_same == 1;
        };
        if (f(x, 0, 0, 1)) return false;
        if (f(0, y, 1, 0)) return false;
        if (x == y and f(0, 0, 1, 1)) return false;
        if (x + y == 2 and f(0, 2, 1, -1)) return false;
        opened[x][y] = true;
      }
      return true;
    };
