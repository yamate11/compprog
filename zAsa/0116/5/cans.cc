...
int main(/* int argc, char *argv[] */) {
  ...
  ll A, B; cin >> A >> B;
  ll ans = 0;
  auto pos = [&](ll i, ll j) -> ll { return (1LL << (i * W + j)); };
  auto f = [&](auto rF, ll x, ll a, ll b) -> void {
    DLOGKL("called", x, du2.to_string(x), a, b);
    if (x == (1LL << (H * W)) - 1) {
      ans++;
      DLOGKL("count", ans);
      return;
    ...
    auto [i0, j0] = [&]() {
      REP(i, 0, H) REP(j, 0, W) {
        if (not (x & pos(i, j))) return pll(i, j);
      }
      assert(0);
    }();
    ll y = x | pos(i0, j0);
    DLOGK(i0, j0, y, du2.to_string(y));
    if (b > 0) rF(rF, y, a, b - 1);
    DLOGKL("posA", x, du2.to_string(x), y, du2.to_string(y));
    if (a > 0 and j0 + 1 < W and (not (x & pos(i0, j0 + 1)))) rF(rF, y | pos(i0, j0 + 1), a - 1, b);
    DLOGKL("posB", x, du2.to_string(x), y, du2.to_string(y));
    DLOGK(i0, j0, y, du2.to_string(y));
    if (a > 0 and i0 + 1 < H and (not (x & pos(i0 + 1, j0)))) rF(rF, y | pos(i0 + 1, j0), a - 1, b);
    DLOGKL("posC", x, du2.to_string(x), y, du2.to_string(y));
  };
  f(f, 0, A, B);
  cout << ans << endl;
  return 0;
