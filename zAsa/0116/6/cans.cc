...
// @@ !! LIM(ipoint debug cmpNaive input)
...
int naive(istream& cin, ostream& cout) {
  ll N; cin >> N;
  // @InpVec(N, P, type=IPoint) [PbGCpWOX]
  auto P = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; P[i] = v; }
  // @End [PbGCpWOX]
  auto ffc = [&](IPoint p, IPoint q) -> ll { return p.x * q.y - p.y * q.x; };
  auto compP = [&](IPoint p, IPoint q) -> bool {
    return ffc(p, q) > 0;
  };
  ...
  REP(x, 0, 1LL << N) {
    if ([&]() {
      REP(i, 0, N) REP(j, i + 1, N) {
        if ((x >> i & 1) and (x >> j & 1)) {
          IPoint is = P[i] + IPoint(0, -1);
          IPoint ie = P[i] + IPoint(-1, 0);
          IPoint js = P[j] + IPoint(0, -1);
          IPoint je = P[j] + IPoint(-1, 0);
          if ((not compP(js, ie)) or (not compP(is, je))) {
            // ok
          }else {
            return false;
          }
...
int body(istream& cin, ostream& cout) {
  ...
  ll start = 1;
  ll end = 0;
  using ev_t = tuple<IPoint, ll, ll>;
  vector<ev_t> ev;
  REP(i, 0, N) {
    ev.emplace_back(P[i] + IPoint(0, -1), start, i);
    ev.emplace_back(P[i] + IPoint(-1, 0), end, i);
  }
  auto comp = [&](ev_t x, ev_t y) -> bool {
    auto [xp, xk, xi] = x;
    auto [yp, yk, yi] = y;
    if (compP(xp, yp)) return true;
    if (compP(yp, xp)) return false;
