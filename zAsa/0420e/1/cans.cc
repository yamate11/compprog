...
int main(/* int argc, char *argv[] */) {
  ...
  vector num_perm(N + 1, vector(B + 1, vector<Fp>()));
  REP(n, 1, N + 1) REP(b, 1, B + 1) {
    ll lim = n / b;
    num_perm[n][b].resize(lim + 1);
    num_perm[n][b][0] = Fp(1);
    REP(k, 1, lim + 1) num_perm[n][b][k] = num_perm[n][b][k - 1] * cb.binom(n - b * (k - 1), b);
    REP(k, 1, lim + 1) num_perm[n][b][k] /= cb.fact(k);
  }
  DLOGK(num_perm);

  vector tbl(N + 1, vector(B + 1, optional<Fp>()));
  auto f = [&](auto rF, ll n, ll b) -> Fp {
    auto& ret = tbl[n][b];
    if (not ret) {
      if (n == 0) ret = Fp(1);
      else if (b < A) ret = Fp(0);
      else {
        ret = rF(rF, n, b - 1);
        REP(k, C, D + 1) {
          if (k * b > n) break;
          Fp np = num_perm[n][b][k];
          DLOGK(n, b, k, np);
          ret = *ret + np * rF(rF, n - k * b, b - 1);
        }
        DLOGK(n, b, *ret);
      }
    }
