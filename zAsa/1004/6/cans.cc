...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) REP(j, 0, M) {
    if (brd.at(i, j) == '#') {
      brd.rs(i + 1, j) = brd.rs(i + 1, j + 1) = '#';
    }
  }
  DLOGK(brd);
  vector tbl(M + 1, vector(N + 1, Fp(0)));
  tbl[M][0] = Fp(1);
  REPrev(j, M - 1, 0) {
    vector acc(N + 2, Fp(0));
    REP(i, 0, N + 1) acc[i + 1] = acc[i] + tbl[j + 1][i];
    REP(i, 0, N + 1) {
      DLOGK(j, i, brd.at(i - 1, j));
      if (i > 0 and brd.at(i - 1, j) == '#') tbl[j][i] = 0;
      else if (i == N) tbl[j][i] = acc[N + 1];
      else tbl[j][i] = acc[i + 2];
    }
    DLOGK(j, tbl);
  }
  cout << accumulate(ALL(tbl[0]), Fp(0)) << endl;
