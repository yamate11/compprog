...
// @@ !! LIM(sieve unordered_map random)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, S, type=string) [CXJ5nR2M]
  ...
  auto f = [&](ll p) -> vector<ll> {
    auto g = [&](auto& s) -> ll {
      ll r = 0;
      ll t = 1;
      REPrev(i, ssize(s) - 1, 0) {
        r = (r + t * (s[i] - '0')) % p;
        t = (t * 10) % p;
      }
      return r;
  ...
  ll ans = 0;
  REP(i, 0, N) REP(j, 0, N) {
    ll x1 = (vec1[i] * vec1[j]) % p1;
    ll x2 = (vec2[i] * vec2[j]) % p2;
    auto it = mp.find(pll(x1, x2));
