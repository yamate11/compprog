...
vector<IPoint> sgns{{-1, -1}, {-1, 1}, {1, -1}, {1, 1}};
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<IPoint> vec;
  ll lim = (ll)sqrt(M) + 10;
  REP(x, 0, lim) REP(y, 0, lim) {
    if (x * x + y * y == M) vec.emplace_back(x, y);
  }
  ll big = 1e18;
  ...
  while (not que.empty()) {
    ...
    for (auto dp : vec) {
      for (auto sgn : sgns) {
        auto q = p + IPoint(dp.x * sgn.x, dp.y * sgn.y);
        if (0 <= q.x and q.x < N and 0 <= q.y and q.y < N and dist[q.x][q.y] == big) {
