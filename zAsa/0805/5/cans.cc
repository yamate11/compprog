...
// @DefStruct(sta, (l, r, i), ord=(l, r, i)) [0XGt11Xs]
struct sta {
  ll l;
  ll r;
  ll i;
  string show() const {
    return "(" + to_string(l) + ", " + to_string(r) + ", " + to_string(i) + ")";
  }
  strong_ordering operator<=>(const sta& o) const {
    if (auto r_ = l <=> o.l; r_ != 0) return r_;
    if (auto r_ = r <=> o.r; r_ != 0) return r_;
    if (auto r_ = i <=> o.i; r_ != 0) return r_;
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> vector<pll> {
    ranges::sort(P);
    ll i0 = EXFIND_D(i, 0, M, P[i].l == 1 and P[i].r == N, -1LL);
    if (i0 >= 0) return vector<pll>{{i0, 1}};
    if (M == 1) return vector<pll>{};
    ll i1 = EXFIND_D(i, 0, M - 1, P[i].l == P[i + 1].l, -1LL);
    if (i1 >= 0) return vector<pll>{{i1, 2}, {i1 + 1, 1}};
    ll i2 = EXFIND_D(i, 0, M - 1, P[i].r >= P[i + 1].r, -1LL);
    if (i2 >= 0) return vector<pll>{{i2, 1}, {i2 + 1, 2}};
    if (P[0].l == 1 and P[M - 1].r == N and P[M - 1].l <= P[0].r + 1 ) return vector<pll>{{0, 1}, {M - 1, 1}};
    if (P[0].r + 1 <= P[M - 1].l) return vector<pll>{{0, 2}, {M - 1, 2}};
    if (M == 2) return vector<pll>{};
    return vector<pll>{{0, 1}, {1, 2}, {2, 1}};
  };
  ...
  if (vec.empty()) {
    ...
    cout << ssize(vec) << "\n";
    vector ans(M, 0LL);
    for (auto [i, op] : vec) ans[P[i].i] = op;
    REPOUT(i, 0, M, ans[i], " ");
  }
