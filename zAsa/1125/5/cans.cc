...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    auto f = [&](IPoint P, IPoint Q, IPoint H) -> ll {
      assert(P != H and Q != H);
      if (P == Q) return 0;
      if (P.x != Q.x and P.y != Q.y) return abs(P.x - Q.x) + abs(P.y - Q.y);
      if (P.x == Q.x) { swap(P.x, P.y); swap(Q.x, Q.y); swap(H.x, H.y); }
      if (P.x > Q.x) { P.x = -P.x; Q.x = -Q.x; H.x = -H.x; }
      assert(P.y == Q.y and P.x < Q.x);
      if (P.x < H.x and H.x < Q.x) return Q.x - P.x + 2;
      else return Q.x - P.x;
    };

    IPoint A, B, C; cin >> A >> B >> C;
    if (B.x == C.x) { swap(A.x, A.y); swap(B.x, B.y); swap(C.x, C.y); }
    if (C.x < B.x) { A.x = -A.x; B.x = -B.x; C.x = -C.x; }
    if (C.y < B.y) { A.y = -A.y; B.y = -B.y; C.y = -C.y; }
    ...
    DLOGK(A, B, C, f(A, B - X10, B), f(A, B - X01, B));
    return min(f(A, B - X10, B), f(A, B - X01, B)) + C.x - B.x + C.y - B.y + 2;
  };
  cout << solve() << endl;
  return 0;
}
