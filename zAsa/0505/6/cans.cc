...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  vector tbl_init(M + 1, vector(M + 1, vector(M + 1, Fp(0))));
  auto tbl = tbl_init;
  tbl[M][M][M] = Fp(1);
  REP(i, 0, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(x, 0, M) {
      REP(i0, 0, M + 1) REP(i1, 0, M + 1) REP(i2, 0, M + 1) {
        Fp tp = prev[i0][i1][i2];
        if (tp == Fp(0)) continue;
        if      (x < i0)  tbl[x ][i1][i2] += tp;
        else if (x == i0) tbl[i0][i1][i2] += tp;
        else if (x < i1)  tbl[i0][x ][i2] += tp;
        else if (x == i1) tbl[i0][i1][i2] += tp;
        else if (x < i2)  tbl[i0][i1][x ] += tp;
        else if (x == i2) tbl[i0][i1][i2] += tp;
      }
    }
  }
  Fp ans = 0;
  REP(i0, 0, M) REP(i1, 0, M) REP(i2, 0, M) ans += tbl[i0][i1][i2];
  cout << ans << endl;
