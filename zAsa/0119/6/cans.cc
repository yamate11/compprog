...
int main(/* int argc, char *argv[] */) {
  ...
  REP(x, 1, 1LL << 16) {
    auto check = [&]() -> bool {
      ...
      REP(i, 0, 4) REP(j, 0, 4) {
        ll e = i * 4 + j;
        if (x >> e & 1) cur.rs(i + 1, j + 1) = 1;
        else cur.rs(i + 1, j + 1) = 0;
      }
      REP(i, 0, 4) REP(j, 0, 4) {
        if (brd0.at(i + 1, j + 1) == 1 and cur.at(i + 1, j + 1) == 0) return false;
      }
      DLOGK(x, brd0, cur);
      UnionFind uf(36);
      REP(i, 0, 6) REP(j, 0, 6) {
        ...
        for (auto dir : BrdIdx::nbr4) {
          auto bj = bi + dir;
          if (cur.in(bj) and cur.at(bi) == cur.at(bj)) uf.merge(e, bj.r * 6 + bj.c);
        }
      }
      return uf.num_groups() == 2;
    };
    if (check()) ans++;
  }
  cout << ans << endl;
