...
int main(/* int argc, char *argv[] */) {
  ...
  auto dist = [&](ll i, ll j, ll ii, ll jj) -> ll {
  ...
  Board<char> ans(R, C, '#');
  cin >> brd;
  REP(i, 0, R) REP(j, 0, C) ans.at(i, j) = brd.at(i, j);
  REP(i, 0, R) REP(j, 0, C) {
    REP(ii, 0, R) REP(jj, 0, C) {
      ll d = brd.at(ii, jj) - '0';
      if (1 <= d and d <= 9 and dist(i, j, ii, jj) <= d) ans.at(i, j) = '.';
    }
  }
  REP(i, 0, R) {
    REP(j, 0, C) cout << ans.at(i, j);
    cout << "\n";
  }
