...
int main(/* int argc, char *argv[] */) {
  ...
  vector<BrdIdx> dirs{{1, 0}, {0, 1}, {1, 1}};

  Board<int> cache(H, W, -1);
  auto f = [&](auto rF, const auto& bi) -> bool {
    if (cache.at(bi) >= 0) return (bool)cache.at(bi); 
    bool result = true;
    for (const auto& d : dirs) {
      auto bj = bi + d;
      if (brd.at(bj) == '.' and rF(rF, bj)) {
        result = false;
        break;
      }
    }
  ...
  cout <<  (f(f, BrdIdx(0, 0)) ? "Second" : "First") << endl;
