...
// @@ !! LIM(matrix ftwo debug random forall)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto ask = [&](const auto& vec) -> ll {
    string s = "?";
    for (ll v : vec) s += " " + to_string(v + 1);
    cout << s << endl;
    ll t; cin >> t;
    return t;
  };

  auto fin = [&](const auto& vec) -> void {
    string s = "!";
    for (ll v : vec) s += " " + to_string(v);
    cout << s << endl;
    exit(0);
  };
  ...
  vector A(N, vector(N, Ftwo(0)));
  vector B(N, Ftwo(0));
  vector main_idx(N, -1LL);
  REP(i, 0, N) {
    using ret_t = tuple<vector<Ftwo>, vector<Ftwo>, Ftwo, ll>;
    auto get_cand = [&]() -> ret_t {
      while (true) {
        vector cand(N, Ftwo(0));
        if (K == 1) {
          REP(j, 0, N) cand[j] = Ftwo(i == j);
        }else if (K == N - 1) {
          REP(j, 0, N) cand[j] = Ftwo(i != j);
        }else {
          vector<ll> ord(N);
          REP(j, 0, N) ord[j] = j;
          REPrev(j, N - 1, 1) {
            ll k = rand.range(0, j + 1);
            if (k < j) swap(ord[k], ord[j]);
          }
          REP(j, 0, K) cand[ord[j]] = 1;
        }
        Ftwo acc_b = 0;
        auto n_cand = cand;
        REP(m, 0, i) {
          if (n_cand[main_idx[m]] == Ftwo(1)) {
            REP(j, main_idx[m], N) n_cand[j] += A[m][j];
            acc_b += B[m];
          }
        }
        ll x = EXFIND_D(j, 0, N, n_cand[j] != Ftwo(0), -1LL);
        if (x >= 0) return {cand, n_cand, acc_b, x};
      }
    };

    auto [cand, n_cand, acc_b, x] = get_cand();
    vector<ll> vec;
    REP(j, 0, N) if (cand[j]) vec.push_back(j);
    Ftwo t = ask(vec);
    A[i] = move(n_cand);
    B[i] = acc_b + t;
    main_idx[i] = x;
    DLOGK(i, cand, n_cand, acc_b, x);
    DLOGKL("  ", A, B, main_idx);
  }
  vector<Ftwo> ans(N);
  REPrev(i, N - 1, 0) {
    Ftwo a = B[i];
    REP(j, main_idx[i] + 1, N) a += A[i][j] * ans[j];
    ans[main_idx[i]] = a;
  }
  fin(ans);
