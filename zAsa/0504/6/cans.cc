...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  auto g = [&](ll p, ll q) -> Fp {
    Fp a = Fp(q) * Fp(M) * (Fp(p) * Fp(p + 1) / 2 - Fp(p)) + Fp(q) * Fp(q + 1) * Fp(p) / 2;
    DLOGKL("g", p, q, a);
    return a;
  };
  auto f = [&](ll p, ll q) -> Fp {
    if (p % 2 == 0 and q % 2 == 0) return g(p, q) / Fp(2);
    else if (p % 2 == 1 and q % 2 == 0) return (g(p, q) - Fp(q / 2)) / Fp(2);
    else if (p % 2 == 0 and q % 2 == 1) return (g(p, q) - M * Fp(p / 2)) / Fp(2);
    else if (p % 2 == 1 and q % 2 == 1) return g(p, q) - (g(p, q) - M * Fp((p - 1) / 2) - Fp((q - 1) / 2) - 1) / Fp(2);
    else assert(0);
  };
  ...
  */
  ...
  REP(i, 0, Q) {
    ...
    ll ans = f(b, d) + f(a, c) - (f(a, d) + f(b, c));
    cout << ans << "\n";
  }
