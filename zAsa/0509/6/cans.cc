...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  vector cdef(2, vector(2, ' '));
  cin >> cdef[0][0] >> cdef[0][1] >> cdef[1][0] >> cdef[1][1];
  ...
  auto f = [&]() -> void {
    ...
    REP(i, 2, 10) {
      ...
      for (string s : prev) {
        REP(j, 1, i) {
          string t = "";
          REP(k, 0, j) t.push_back(s[k]);
          t.push_back(cdef[s[j - 1] - 'A'][s[j] - 'A']);
          REP(k, j, i) t.push_back(s[k]);
          ss.insert(t);
        }
      }
      // DLOGK(SIZE(ss), ss);
