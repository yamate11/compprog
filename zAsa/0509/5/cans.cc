...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [W4LTiHDM]
  ll M; cin >> M;
  vector hist(3, vector(M + 1, IPoint()));
  vector<IPoint> cur{{1, 0}, {0, 1}, {0, 0}};
  REP(i, 0, 3) hist[i][0] = cur[i];
  REP(i, 0, M) {
    ll tp; cin >> tp;
    if (tp == 1) REP(j, 0, 3) cur[j] = cur[j].rotate(-1);
    else if (tp == 2) REP(j, 0, 3) cur[j] = cur[j].rotate(1);
    else if (tp == 3) {
      ll p; cin >> p;
      REP(j, 0, 3) cur[j] = IPoint(2*p - cur[j].x, cur[j].y);
    }else if (tp == 4) {
      ll p; cin >> p;
      REP(j, 0, 3) cur[j] = IPoint(cur[j].x, 2*p - cur[j].y);
    }else assert(0);
    REP(j, 0, 3) hist[j][i + 1] = cur[j];
  }
  DLOGK(hist[0]);
  DLOGK(hist[1]);
  DLOGK(hist[2]);
  ...
  REP(_q, 0, Q) {
    ...
    vector<IPoint> z(3);
    IPoint w{X[b], Y[b]};
    IPoint ans = X[b] * hist[0][a] + Y[b] * hist[1][a] + (1 - (X[b] + Y[b])) * hist[2][a];
    cout << ans.x << " " << ans.y << "\n";
  }
