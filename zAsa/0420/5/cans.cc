...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](ll i) -> ll { return (i + A[i]) % N; };
  auto g = [&](ll i) -> ll { return A[i]; };
  DoublingFRel dR(K + 1, N, f);
  DoublingCum<ll> dC(dR, g);
#if DEBUG
  REP(i, 0, K + 1) DLOGK(i, dR.val(i, 0), dC.val(i, 0));
#endif
  cout << dC.val(K, 0) << endl;
