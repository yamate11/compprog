...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> brd(H, W, '@');
  cin >> brd;
  Board<bool> seen(H, W, false);
  ll ans = 0;
  auto parity = [&](BrdIdx bi) -> bool {
    return ((bi.r + bi.c) % 2 == 0) == (brd.at(bi) == '#');
  };
  auto func = [&](BrdIdx bi0) -> void {
    bool par_orig = parity(bi0);
    ll black = 0;
    ll white = 0;
    auto dfs = [&](auto rF, BrdIdx bi) -> void {
      if (not brd.in(bi)) return;
      if (parity(bi) != par_orig) return;
      if (seen.at(bi)) return;
      seen.at(bi) = true;
      if (brd.at(bi) == '#') black++;
      else white++;
      for (BrdIdx d : BrdIdx::nbr4) rF(rF, bi + d);
    };
    dfs(dfs, bi0);
    ans += black * white;
  };
