...
int main() {
  ll N, A, B; cin >> N >> A >> B;
  if (A * B < N or N + 1 < A + B) cout << -1 << endl;
  else {
    CoordCompr ccx, ccy;
    auto f = [&](ll x) -> ll { return B * (x / B) + (B - 1) - (x % B); };
    auto g = [&](ll x) -> void { ccx.add(x); ccy.add(f(x)); };
    REP(i, 0, B) g(i);
    REP(i, 1, A) g(i * B);
    for (ll j = B + 1, rem = N - B - A - 1; rem > 0; j++) if (j % B != 0) { g(j); rem--; }
    REP(i, 0, N) cout << 1 + ccy.c(f(ccx.d(i))) << " ";
    cout << endl;
  }
  return 0;
}
