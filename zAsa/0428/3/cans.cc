...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](bool is_odd) -> void { // 本体
    for (ll n = 1; true; n++) {
      string s = dut.to_string(n);
      string rev_s = s; ranges::reverse(rev_s);
      if (is_odd) s.pop_back();
      ll x = dut.from_string(s + rev_s);
      if (x > N) break;
      string sA = duA.to_string(x);
      if (FORALL(i, 0, ssize(sA) / 2, sA[i] == sA[ssize(sA) - i - 1])) ans += x;
    }
  };
