...
int main(/* int argc, char *argv[] */) {
  ...
  Fp tot = cb.binom(H * W, K);
  auto q = [&](ll a, ll b) -> Fp {
    return cb.binom(a * b, K) / tot;
  };
  ...
  REP(i, 0, H) REP(j, 0, W) {
    ...
    ll j1 = j;
    ll j2 = W - j - 1;
    DLOGK(i, j, i1, i2, j1, j2, H, W);
    Fp bp1 = q(i1, W) + q(i2, W) + q(H, j1) + q(H, j2);
    DLOGK(i, j, q(i1, W), q(i2, W), q(H, j1), q(H, j2), bp1);
    Fp bp2 = q(i1, j1) + q(i1, j2) + q(i2, j1) + q(i2, j2);
    DLOGK(i, j, q(i1, j1), q(i2, j2), q(i2, j1), q(i2, j2), bp2);
    Fp bp = bp1 - bp2;
    DLOGK(i, j, bp);
    ans += Fp(1) - bp;
  }
