...
int main(/* int argc, char *argv[] */) {
  ...
  ll N = ssize(S);
  vector<ll> vec;
  auto bc = [&](ll i) -> bool { return i + 1 < N and S[i] == 'B' and S[i + 1] == 'C'; };
  ll ans = 0;
  for (ll i = 0; i < N + 1; ) {
    if (i == N or (S[i] != 'A' and not bc(i))) {
      if (not vec.empty()) {
        ll sz = ssize(vec);
        vector<ll> w(sz + 1, 0LL);
        REPrev(j, sz - 1, 0) w[j] = w[j + 1] + (ll)(vec[j] == 1);
        REP(j, 0, sz) if (vec[j] == 0) ans += w[j];
        DLOGK(vec, w, ans);
        vec.resize(0);
      }
      if (i == N) break;
