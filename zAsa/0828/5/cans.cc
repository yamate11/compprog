...
int main(/* int argc, char *argv[] */) {
  ...
  vector fwd(N, vector(sigma, vector<ll>()));
  vector bwd(N, vector(sigma, vector<ll>()));
  REP(i, 0, N) REP(j, 0, N) if (C[i][j] != '-') {
    ll d = C[i][j] - 'a';
    fwd[i][d].push_back(j);
    bwd[j][d].push_back(i);
  }
  ...
  REP(i, 0, N) REP(j, 0, N) if (i != j and C[i][j] != '-') {
    dist[i][j] = 1;
    que.emplace(i, j);
  }
  while (not que.empty()) {
    auto [i0, j0] = que.front(); que.pop();
    ll len = dist[i0][j0];
    REP(d, 0, sigma) for (ll i : bwd[i0][d]) for (ll j : fwd[j0][d]) {
        if (len + 2 < dist[i][j]) {
          dist[i][j] = len + 2;
          que.emplace(i, j);
        }
  ...
  REP(i, 0, N) {
    REPOUT(j, 0, N, dist[i][j] == big ? -1 : dist[i][j], " ");
  }
