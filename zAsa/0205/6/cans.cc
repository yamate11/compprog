...
int main(/* int argc, char *argv[] */) {
  ...
  REP(x, 1, 1LL << t) {
    ll a = N;
    REP(i, 0, t) {
      if (x >> i & 1) a /= pr[i].first;
    }
    vector absent(a, false);
    REP(i, 0, N) {
      if (S[i] == '.') absent[i % a] = true;
    }
    ll sa = 0;
    REP(i, 0, a) if (absent[i]) sa++;
    Fp pat = power<Fp>(2, a - sa);
    if (popcount((u64)x) % 2 == 0) ans -= pat;
    else ans += pat;
    DLOGK(x, a, absent, sa, pat, ans);
  }
  cout << ans << endl;
  return 0;
}
