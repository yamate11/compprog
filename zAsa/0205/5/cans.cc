...
int main(/* int argc, char *argv[] */) {
  ...
  auto root = new Trie<26, 'a', pll>();
  
  auto is_X_prefix = [&](string s) -> bool {
    DLOGKL("is_X_prefix", s);
    for (auto [p, i] = make_pair(root, 0); p ; p = p->get_child_val(s[i++])) {
      if (p->user.first) return true;
      if (i == ssize(s)) break;
    }
    return false;
  };
  ...
  REP(_q, 0, Q) {
    ...
    if (t == 1) {
      bool b = is_X_prefix(s);
      auto p = root->insert(s);
      p->user.first = 1;
      if (not b) {
        ll v = p->user.second;
        for (auto q = p; q; q = q->parent) q->user.second -= v;
      }
    }else if (t == 2) {
      if (not is_X_prefix(s)) {
        auto p = root->insert(s);
        for (auto q = p; q; q = q->parent) q->user.second++;
      }
    }
    cout << root->user.second << "\n";
  }
