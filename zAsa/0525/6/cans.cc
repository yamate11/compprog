...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((A, dec=1), (B, dec=1), (C, type=char))) [JVLRQnUe]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  ...
  vector<vector<bool>> conn(N, vector(N, false));
  REP(i, 0, M) {
    nbr[A[i]].emplace_back(B[i], C[i]);
    nbr[B[i]].emplace_back(A[i], C[i]);
    conn[A[i]][B[i]] = conn[B[i]][A[i]] = true;
  }
  ll big = 1LL << 60;
  ...
  while (not que.empty()) {
    ...
    DLOGKL("popped", s, t);
    if (s == t) {
      updMin(ans, dist[s][t] * 2);
    }else if (conn[s][t]) {
      updMin(ans, dist[s][t] * 2 + 1);
    }else {
      for (auto [ns, cs] : nbr[s]) for (auto [nt, ct] : nbr[t]) {
          if (dist[ns][nt] == big and cs == ct) {
            dist[ns][nt] = dist[s][t] + 1;
            que.emplace(ns, nt);
            DLOGKL("pushed", ns, nt);
          }
        }
