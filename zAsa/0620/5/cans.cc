...
// @@ !! LIM(f:intDiv cmpNaive perm forall debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto check = [&]() -> bool {
    vector found(Z, false);
    REP(len, 1, N + 1) {
      IntComb ic(N, len);
      while (ic.get()) {
        ll s = 0;
        REP(i, 0, len) s += A[ic.at(i)];
        s = s % Z;
  ...
  if (x == y) {
    if (FORALL(i, 0, x, B[i] == C[i])) {
      cout << "NG\n";
      return 0;
    }
  }
  if (EXISTS(i, 0, x - 1, B[i] >= B[i + 1]) or (EXISTS(i, 0, y - 1, C[i] >= C[i + 1]))) {
    cout << "NG\n";
    return 0;
  }

  ll sx = 0, sy = 0;
  REP(i, 0, x) sx += A[B[i] - 1];
  REP(i, 0, y) sy += A[C[i] - 1];
  if ((sx - sy) % Z != 0) {
    cout << "NG\n";
  }
  cout << "OK\n";
...
int body(istream& cin, ostream& cout) {
  ...
  vector tbl(Z, vector<ll>());

  auto show_ans = [&](ll k) -> void {
    cout << ssize(tbl[k]) << " ";
    REPOUT(m, 0, ssize(tbl[k]), tbl[k][m] + 1, " ");
  };
