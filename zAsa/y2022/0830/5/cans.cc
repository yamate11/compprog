...
int main(/* int argc, char *argv[] */) {
  ...
  tbl[0][0] = power<Fp>(3, H * W - K);
  REP(h, 0, H) REP(w, 0, W) {
    if (h > 0) {
      char c = sym[h - 1][w];
      if (c == ' ') tbl[h][w] += tbl[h - 1][w] * 2 / 3;
      else if (c == 'X' or c == 'D') tbl[h][w] += tbl[h - 1][w];
    }
    if (w > 0) {
      char c = sym[h][w - 1];
      if (c == ' ') tbl[h][w] += tbl[h][w - 1] * 2 / 3;
      else if (c == 'X' or c == 'R') tbl[h][w] += tbl[h][w - 1];
    }
    DLOGK(tbl[h][w]);
  }
  cout << tbl[H - 1][W - 1] << endl;
