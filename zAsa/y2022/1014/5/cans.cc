...
int main(/* int argc, char *argv[] */) {
  ...
  Board brd(H, W, '#');
  cin >> brd;
  vector<BrdIdx> D = {{0, 1}, {1, 0}, {1, 1}};
  Board tbl(H, W, Fp(0));
  Board acc(H, W, vector<ll>(3, Fp(0)));
  tbl.at(0, 0) = Fp(1);
  acc.at(0, 0) = {Fp(1), Fp(1), Fp(1)};
  REP(r, 0, H) REP(c, 0, W) {
    ...
    REP(i, 0, 3) {
      auto bi2 = bi - D[i];
      tbl.at(bi) += acc.at(bi2)[i];
    }
    REP(i, 0, 3) {
      auto bi2 = bi - D[i];
      acc.at(bi)[i] = tbl.at(bi) + acc.at(bi2)[i];
    }
    DLOGK(bi, brd.at(bi), acc.at(bi));
  }
  cout << tbl.at(H - 1, W - 1) << endl;
