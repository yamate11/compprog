...
int main(/* int argc, char *argv[] */) {
  ...
  CoordCompr cc;
  REP(i, 0, N) { cc.add(L[i]); cc.add(R[i]); }
  ll lim = 1e9;
  cc.add(0), cc.add(lim + 1);
  ll sz = cc.size();

  auto add = [&](pll a, pll b) -> pll { return pll(a.first + b.first, a.second + b.second); };
  auto st = make_seg_tree_lazy<pll, pll>(pll(0, 0), pll(0, 0), add, add, add, vector<pll>(sz));
  ll z = 1;
  REP(i, 0, N) {
    ll r = cc.c(R[i]);
    ll l = cc.c(L[i]);
    st.update(r, sz, pll(1, -R[i]));
    st.update(0, l, pll(-1, L[i]));
    auto [al, bl] = st[l - 1];
    auto [ar, br] = st[r];
    ll ans;
    if (ar == 0) ans = br;
