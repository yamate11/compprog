...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, D; cin >> N >> D;
  // @InpVec(N, A, dec=1) [4bzDJoYm]
  auto A = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; A[i] = v; }
  // @End [4bzDJoYm]
  vector<bool> appear(N);
  REP(i, 0, N) if (A[i] >= 0) appear[A[i]] = true;
  ll P = 1 << (2 * D + 1);
  vector tbl_init(P, Fp(0));
  auto tbl = tbl_init;
  ll x0 = 0;
  REP(i, 0, D) x0 |= 1LL << i;
  REP(i, 0, D + 1) if (appear[i]) x0 |= 1LL << (D + i);
  tbl[x0] = 1;
  DLOGK(tbl);
  REP(i, 0, N) {
    ll b =  (i + D + 1 >= N or appear[i + D + 1]) ? 1LL << (2 * D) : 0;
    DLOGK(i, b);
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(x, 0, P) {
