...
int main(/* int argc, char *argv[] */) {
  ...
  string S; cin >> S;
  ll N = SIZE(S);
  vector tbl_init(8, Fp(0));
  auto tbl = tbl_init;
  REP(i, N) {
    auto prev = tbl;
    if (S[i] == 'c') tbl[0] += 1;
    if (S[i] == 'h') tbl[1] += prev[0];
    if (S[i] == 'o') tbl[2] += prev[1];
    if (S[i] == 'k') tbl[3] += prev[2];
    if (S[i] == 'u') tbl[4] += prev[3];
    if (S[i] == 'd') tbl[5] += prev[4];
    if (S[i] == 'a') tbl[6] += prev[5];
    if (S[i] == 'i') tbl[7] += prev[6];
  }
  cout << tbl[7] << endl;
