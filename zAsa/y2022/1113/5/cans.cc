...
int main(/* int argc, char *argv[] */) {
  ...
  ll rn = 0;
  {
    auto st = make_seg_tree<ll>(0LL, add, vector<ll>(N));
    for (ll i : ord) {
      rn += st.query(i + 1, N);
      st.update(i, 1);
    }
  ...
  REP(i, 0, N) rec[C[i]].push_back(i);
  auto st2 = make_seg_tree<ll>(0LL, add, vector<ll>(N));
  ll rn2 = 0;
  REP(c, 1, N + 1) {
    auto& vec = rec[c];
