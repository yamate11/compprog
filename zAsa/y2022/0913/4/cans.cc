...
int main(/* int argc, char *argv[] */) {
  ...
  ll K = SIZE(sN);
  ll lim = K + 1;
  vector tbl_init(2, vector(2, vector(2, vector(lim, 0LL))));
  auto tbl = tbl_init;
  tbl[1][1][1][0] = 1;
  for (ll k = 0; k < K; k++) {
    ll d = sN[k] - '0';
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(eq, 0, 2) REP(az, 0, 2) REP(ao, 0, 2) REP(olen, 0, lim) {
      ll val = prev[eq][az][ao][olen];
      if (val == 0) continue;
      REP(x, 0, 10) {
        if (eq && x > d) continue;
        ll new_eq = eq && x == d;
        ll new_az = az && x == 0;
        ll new_ao = new_az || (ao && x == 1);
        ll new_olen = ao && x == 1 ? olen + 1 : olen;
        tbl[new_eq][new_az][new_ao][new_olen] += val;
      }
    }
  }
  ll ans = 0;
  REP(eq, 0, 2) REP(ao, 0, 2) REP(olen, 0, lim) {
    ans += tbl[eq][0][ao][olen] * olen;
  }
  cout << ans << endl;
