...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  ll Ax, Ay, Bx, By; cin >> Ax >> Ay >> Bx >> By; Ax--; Ay--; Bx--; By--;
  Board<char> brd(N, N, '#');
  vector<BrdIdx> dirs{{1,1}, {1,-1}, {-1,-1}, {-1,1}};

  cin >> brd;
  auto enc = [&](BrdIdx bi, ll e) -> ll { return 4 * (N * bi.r + bi.c) + e; };
  auto start = BrdIdx(Ax, Ay);
  auto goal = BrdIdx(Bx, By);
  if (start == goal) {
    cout << 0 << endl;
    return 0;
  }
  ll big = 1e18;
  vector dist(N * N * 4, big);
  auto mydist = [&](BrdIdx bi, ll e) -> ll& { return dist[enc(bi, e)]; };
  REP(e, 4) mydist(start, e) = 1;
  deque<sta> deq;
  REP(e, 4) deq.emplace_back(1, start, e);
  while (not deq.empty()) {
    auto [d, bi, e] = deq.front(); deq.pop_front();
    DLOGKL("out", d, bi, e, mydist(bi, e));
    if (d > mydist(bi, e)) continue;
    if (bi == goal) {
      cout << d << endl;
      return 0;
    }
    REP(ee, 4) {
      if (ee == e) {
        BrdIdx bd = bi + dirs[ee];
        if (brd.at(bd) == '#') continue;
        if (mydist(bd, ee) <= d) continue;
        mydist(bd, ee) = d;
        deq.emplace_front(d, bd, ee);
        DLOGKL("pushed 0", d, bd, ee);
      }else {
        if (mydist(bi, ee) <= d + 1) continue;
        mydist(bi, ee) = d + 1;
        deq.emplace_back(d + 1, bi, ee);
        DLOGKL("pushed 1", d + 1, bi, ee);
      }
    }
  }
  cout << -1 << endl;
