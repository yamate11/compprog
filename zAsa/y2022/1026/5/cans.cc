...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, K; cin >> N >> M >> K;
  // @InpNbrList(N, M, fr, dec=1) [9cJBNbZf]
  auto fr = vector(N, vector(0, int()));
  for (int i = 0; i < M; i++) {
  ...
  // @End [9cJBNbZf]
  // @InpNbrList(N, K, bl, dec=1) [9cJBNbZf]
  auto bl = vector(N, vector(0, int()));
  for (int i = 0; i < K; i++) {
    int u, v; cin >> u >> v; u -= 1; v -= 1;
    bl[u].emplace_back(v);
    bl[v].emplace_back(u);
  ...
  // @End [9cJBNbZf]
  UnionFind uf(N);
  REP(i, 0, N) for (ll j : fr[i]) uf.merge(i, j);
  REP(i, 0, N) {
    ll l = uf.leader(i);
    ll num = uf.groupSize(l) - 1;
    for (ll j : fr[i]) if (uf.leader(j) == l) num--;
    for (ll j : bl[i]) if (uf.leader(j) == l) num--;
    cout << num << "\n";
  }
