...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(p, 0, M + 1) REP(q, 0, M + 1) REP(r, 0, M + 1) {
      Fp z = prev[p][q][r];
      if (z == Fp(0)) continue;
      REP(m, 0, M) {
        if (m < p) tbl[m][q][r] += z;
        else if (m == p) tbl[p][q][r] += z;
        else if (p < m and m < q) tbl[p][m][r] += z;
        else if (m == q) tbl[p][q][r] += z;
        else if (q < m and m < r) tbl[p][q][m] += z;
        else if (m == r) tbl[p][q][r] += z;
      }
    }
  }
  Fp ans = 0;
  REP(p, 0, M) REP(q, 0, M) REP(r, 0, M) ans += tbl[p][q][r];
  cout << ans << endl;
