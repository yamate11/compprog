...
int main(/* int argc, char *argv[] */) {
  ...
  cin >> orig;
  vector<BrdIdx> dots;
  REP(i, 0, H) REP(j, 0, W) if (orig.at(i, j) == '.') dots.push_back(BrdIdx(i, j));
  ll sz = SIZE(dots);
  ll ans = 0;
  DLOGK(sz);
  REP(pp, 0, sz) {
    BrdIdx start = dots[pp];
    /*
    Board brd(H, W, '#');
    REP(i, 0, H) REP(j, 0, W) brd.at(i, j) = orig.at(i, j);
    */
    ll big = 1e18;
    ...
    while (not que.empty()) {
      ...
      for (BrdIdx d : BrdIdx::nbr4) {
        BrdIdx bb = bi + d;
        if (orig.at(bb) == '.' and dist.at(bb) == big) {
          dist.at(bb) = dist.at(bi) + 1;
          ans = max(ans, dist.at(bb));
          que.push(bb);
        }
      }
    }
