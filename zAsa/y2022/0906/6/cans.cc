...
ll lim = 1e9 + 1;
ll enc(ll r, ll c) { return r * lim + c; };
ll enc(const IPoint& p) { return enc(p.x, p.y); };
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> ans(10);
  REP(i, 0, N) {
    IPoint p; cin >> p; p.x--; p.y--;
    P[i] = p;
    brd.insert(enc(p));
  }
  auto op = [&](ll x, ll y) -> void {
    if (not (0 <= x and x + 2 < H and 0 <= y and y + 2 < W)) return;
    auto it = done.find(enc(x, y));
    if (it != done.end()) return;
    done.insert(enc(x, y));
    ll t = 0;
    REP(dx, 0, 3) REP(dy, 0, 3) {
      ll e = enc(x + dx, y + dy);
      if (brd.find(e) != brd.end()) {
        DLOGKL("  in: ", x+dx, y+dy);
        t++;
      }
    }
  ...
  for (IPoint& p : P) {
    REP(dx, -2, 1) REP(dy, -2, 1) op(p.x + dx, p.y + dy);
  }
  ll s = 0;
  REP(t, 1, 10) s += ans[t];
  ans[0] = (H - 2) * (W - 2) - s;
