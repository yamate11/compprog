...
// @@ !! LIM(board forall mod debug topord)
...
int main(/* int argc, char *argv[] */) {
  ...
  Board<ll> brd(H, W, 0);
  cin >> brd;
  auto enc = [&](BrdIdx bi) -> ll { return bi.r * W + bi.c; };
  auto dec = [&](ll e) -> BrdIdx { return BrdIdx(e / W, e % W); };
  TopOrd to(H * W);
  REP(i, H) REP(j, W) {
    BrdIdx bi(i, j);
    for (auto d : BrdIdx::nbr4) {
      auto bd = bi + d;
      if (brd.in(bd) and brd.at(bi) < brd.at(bd)) to.add_edge(enc(bd), enc(bi));
    }
  }
  ...
  for (ll x : to.sorted()) {
    Fp t = 1;
    for (ll e : to.backward(x)) t += val.at(dec(e));
    val.at(dec(x)) = t;
    ans += t;
  }
  cout << ans << endl;
