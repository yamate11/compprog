...
int main(/* int argc, char *argv[] */) {
  ...
  ll p = N / 8;
  vector<ll> ans;
  REP(i, 0, p) REP(j, 1, 13) if (j % 2 == 0 or j % 3 == 0) ans.push_back(12 * i + j);
  ll q = N % 8;
  ll r = q / 2;
  ll s = q % 2;
  if (r >= 1) { ans.push_back(12 * p + 3); ans.push_back(12 * p + 9); }
  if (r >= 2) { ans.push_back(12 * p + 2); ans.push_back(12 * p + 4); }
  if (r >= 3) { ans.push_back(12 * p + 8); ans.push_back(12 * p + 10); }
  if (s == 1) { ans.push_back(12 * p + 6); }
  REPOUT(i, 0, N, ans[i], " ");
