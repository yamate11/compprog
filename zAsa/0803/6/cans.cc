...
int main(/* int argc, char *argv[] */) {
  ...
  ll K, N; cin >> K >> N;
  // @InpMVec(N, ((V, type=string), (W, type=string))) [yTGCgFDK]
    ...
    auto check = [&]() {
      ...
      REP(i, 0, N) {
        ...
        REP(j, 0, ssize(v)) {
          ...
          ll len = dip.at(d) + 1;
          if (p + len > ssize(w)) {
            DLOGKL("over", i, j, p, len);
            return emp;
          }
          string tt = w.substr(p, len);
          if (ssize(ret[d]) == 0) {
            ret[d] = tt;
          }else if (ret[d] != tt) {
            DLOGKL("inconsis", i, j, ret[d], tt, p, len);
            return emp;
          }
          p += len;
        }
        if (p != ssize(w)) {
          DLOGKL("size mismatch", i, p);
          return emp;
        }
      }
      return ret;
