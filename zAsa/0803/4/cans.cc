...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, K; cin >> N >> M >> K;
  // @InpNbrList(N, M, nbr, dec=1, read=A) [SvVhIKdv]
  struct nbr_t {
    int nd;
    ll A;
    string show() const {
      return "(" + to_string(nd) + ", " + to_string(A) + ")";
    }
  ...
  auto enc = [&](ll nd, ll mode) -> ll { return 2 * nd + mode; };
  ...
  while (not deq.empty()) {
    auto [d, nd, mode] = deq.front(); deq.pop_front();
    DLOGKL("popped", d, nd, mode);
    DLOGKL(dist[enc(nd, mode)], d);
    if (dist[enc(nd, mode)] == d) {
      if (S[nd]) {
        ll e_peer = enc(nd, 1 - mode);
        if (dist[e_peer] > d) {
          dist[e_peer] = d;
          deq.emplace_front(d, nd, 1 - mode);
          DLOGKL("push front", d, nd, 1 - mode);
        }
      }
      DLOGK(nd, nbr[nd]);
      for (auto [peer, a] : nbr[nd]) {
        if (a == mode) {
          ll e_peer = enc(peer, mode);
          if (dist[e_peer] > d + 1) {
            dist[e_peer] = d + 1;
            deq.emplace_back(d + 1, peer, mode);
            DLOGKL("push back", d + 1, peer, mode);
          }
        }
      }
    }
  }
  ll ans = min(dist[enc(N - 1, 0)], dist[enc(N - 1, 1)]);
  if (ans >= big) ans = -1;
  cout << ans << endl;
