...
int main(/* int argc, char *argv[] */) {
  ...
  safe_umap<ll, ll> sumR;
  safe_umap<ll, ll> sumC;
  safe_umap<pll, ll> mp;
  ...
  vector<pll> vecR, vecC;
  for (auto [r, s] : sumR) vecR.emplace_back(s, r);
  for (auto [c, s] : sumC) vecC.emplace_back(s, c);
  ranges::sort(vecR, greater<pll>());
  ranges::sort(vecC, greater<pll>());
  ll j0 = big;
  ll ans = 0;
  DLOGK(vecR);
  DLOGK(vecC);
  for (auto [sr, r] : vecR) {
    if (j0 == 0) break;
    REP(j, 0, min(j0, (ll)ssize(vecC))) {
      auto [sc, c] = vecC[j];
      auto it = mp.find(pll{r, c});
      if (it == mp.end()) {
        ans = max(ans, sr + sc);
        j0 = j;
        break;
      }else {
        ans = max(ans, sr + sc - X[it->second]);
      }
    }
  }
  cout << ans << "\n";
