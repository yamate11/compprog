...
int main(/* int argc, char *argv[] */) {
  ...
  ll big = 1e18;
  ll ans = big;
  auto ordidx = sortedIndex(A);
  vector<int> idxord(N);
  REP(i, 0, N) idxord[ordidx[i]] = i;
  REP(ii, 0, N) {
    ll i = ordidx[ii];
    ll val = A[i];
    vector<ll> cands;
    for (ll j = 0; true; ) {
      for (; j < N and A[j] < val; j++);
      if (j == N) break;
      vector<ll> tmp;
      ll k = j;
      for (; k < N and A[k] >= val; k++) tmp.push_back(A[k]);
      DLOGKL("  ", i, val, j, k, tmp);
      ranges::sort(tmp);
      REP(p, 0, ssize(tmp) - K + 1) cands.push_back(tmp[p]);
      j = k;
    }
    DLOGK(val, cands);
    if (ssize(cands) < Q) break;
    ranges::sort(cands);
    updMin(ans, cands[Q - 1] - cands[0]);
  }
  cout << ans << endl;
  return 0;
}
