...
// @@ !! LIM(forall f:updMaxMin cmpNaive perm)
...
int naive(istream& cin, ostream& cout) {
  ll N; cin >> N;
  // @InpMVec(2*N, (A, (C, type=char))) [Gj1NIIIg]
...
int body(istream& cin, ostream& cout) {
  ...
  ll N; cin >> N;
  unordered_map<char, int> code{{'R', 0}, {'G', 1}, {'B', 2}};
  vector vals(3, vector<ll>());
  REP(i, 0, 2*N) {
    ll a; cin >> a;
    char c; cin >> c;
    vals[code[c]].push_back(a);
  }
  if (ssize(vals[0]) % 2 == 0) swap(vals[0], vals[2]);
  if (ssize(vals[1]) % 2 == 0) swap(vals[1], vals[2]);
  if (ssize(vals[0]) % 2 == 0) {
    cout << 0 << endl;
    return 0;
  }
  REP(i, 0, 3) sort(ALL(vals[i]));
  auto func = [&](ll c1, ll c2) -> ll {
    ll j = 0;
    ll ret = 1e18;
    if (vals[c1].empty() or vals[c2].empty()) return ret;
    REP(i, 0, ssize(vals[c1])) {
      while (j + 1 < ssize(vals[c2]) and vals[c2][j + 1] < vals[c1][i]) j++;
      updMin(ret, abs(vals[c1][i] - vals[c2][j]));
      if (j + 1 < ssize(vals[c2])) updMin(ret, abs(vals[c1][i] - vals[c2][j + 1]));
    }
    return ret;
  };
  ll ans1 = func(0, 1);
  ll ans2 = func(0, 2) + func(1, 2);
