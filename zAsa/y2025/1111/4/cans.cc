...
int main(/* int argc, char *argv[] */) {
  ...
  cin >> brd;
  vector<BrdIdx> cands;
  REP(i, 0, H) REP(j, 0, W) if (brd.at(i, j) == '#') cands.emplace_back(i, j);
  while (true) {
    vector<BrdIdx> stage;
    for (auto bi : cands) {
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (brd.in(bj) and brd.at(bj) == '.') {
          ll cnt = 0;
          for (auto dir2 : BrdIdx::nbr4) {
            auto bk = bj + dir2;
            if (brd.in(bk) and brd.at(bk) == '#') cnt++;
          }
          if (cnt == 1) stage.push_back(bj);
        }
      }
    }
    if (stage.empty()) break;
    for (auto bi : stage) brd.rs(bi) = '#';
    cands = move(stage);
  }
  ll ans = 0;
  REP(i, 0, H) REP(j, 0, W) if (brd.at(i, j) == '#') ans++;
  cout << ans << endl;
