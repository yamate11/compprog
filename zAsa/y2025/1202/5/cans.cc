...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) A[i] = A[i] % M;
  vector<ll> B(N + 1);
  REP(i, 0, N) B[i + 1] = (B[i] + A[i]) % M;
  vector<ll> C(M);
  REP(i, 0, N) C[B[i + 1]]++;
  ll base = 0;
  REP(i, 0, M) base += C[i] * i;
  ll ans = base;

  auto st = make_seg_tree(0LL, plus<ll>(), C);

  REP(i, 0, N) {
    base -= B[i + 1];
    st.rs(B[i + 1]) = st.at(B[i + 1]) - 1;
    ll v1 = st.query(0, B[i + 1]);
    ll v2 = st.query(B[i + 1], M);
    ans += base + (M - B[i + 1]) * v1 - B[i + 1] * v2;
  }
  cout << ans << endl;
