...
// @@ !! LIM(perm forall cmpNaive debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  ll l0 = lmax * (N - 1);
  IntDupPerm dip(l0 + 1, N);
  ll big = 1e18;
  ll ans = big;
  while (dip.get()) {
    vector<ll> A(l0 + 1);
    REP(i, 0, N) A[dip.at(i)] += W[i];
    vector<ll> S(l0 + 2);
    REP(i, 0, l0 + 1) S[i + 1] = S[i] + A[i];
    auto check = [&]() -> bool {
      REP(i, 0, M) {
        REP(a, 0, l0 + 1) {
          if (a + L[i] < l0 + 2 and S[a + L[i]] - S[a] > V[i]) return false;
        }
      }
      return true;
    };
    ...
    ll y1 = dip.at(0);
    REP(i, 0, N) {
      y0 = min(y0, (ll)dip.at(i));
      y1 = max(y1, (ll)dip.at(i));
    }
    if (check()) ans = min(ans, y1 - y0);
  }
  if (ans == big) ans = -1;
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  sort(ALL(VL));
  if (EXISTS(i, 0, N, W[i] > VL[0].first)) {
    cout << -1 << endl;
    return 0;
  }
  map<ll, ll> mp;
