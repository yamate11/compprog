...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl_init(a0 + 1, vector(b0 + 1, vector(c0 + 1, Fp(0))));
  auto tbl = tbl_init;
  tbl[0][0][0] = Fp(1);
  while (true) {
    bool remained = false;
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(a, 0, a0 + 1) REP(b, 0, b0 + 1) REP(c, 0, c0 + 1) {
      if (a == a0 and b == b0 and c == c0) tbl[a][b][c] += prev[a][b][c];
      else {
        if (prev[a][b][c] != Fp(0)) {
          remained = true;
          auto op = [&](ll p, ll q, ll r) -> void {
            if (a + p > a0 or b + q > b0 or c + r > c0) return;
            tbl[a + p][b + q][c + r] += prev[a][b][c] * prob0;
          };
          op(1, 0, 0);
          op(0, 1, 0);
          op(2, 0, 0);
    ...
    if (not remained) break;
  }
  cout << tbl[a0][b0][c0] << endl;
  return 0;
}
