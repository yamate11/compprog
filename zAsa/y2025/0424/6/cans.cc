...
int main(/* int argc, char *argv[] */) {
  ...
  auto divSieve = divisorSieve(K);
  vector P(K + 1, 0LL);
  REP(k, 1, K + 1) {
    auto pr = prfacDivSieve(k, divSieve);
    if (FORALL(i, 0, ssize(pr), pr[i].second == 1)) {
      if (ssize(pr) % 2 == 0) P[k] = 1;
      else P[k] = -1;
    }else {
      P[k] = 0;
    }
  ...
  REPrev(k, K, 1) {
    ll m = K / k;
    Fp w = power<Fp>(Fp(m), N);
    // REP(i, 2, m + 1) w += P[i] * vec[k * i];
    REP(i, 2, m + 1) w -= vec[k * i];
    vec[k] = w;
  }
  DLOGK(vec);
  DLOGK(P);
  Fp ans = 0;
  REP(k, 1, K + 1) ans += k * vec[k];
  cout << ans << endl;
