...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, TD, type=pll) [C9gNGMBo]
  auto TD = vector(N, pll());
  for (int i = 0; i < N; i++) { pll v; cin >> v; TD[i] = v; }
  // @End [C9gNGMBo]
  ranges::sort(TD);
  ...
  ll idx = 0;
  while (true) {
    if (idx < N and TD[idx].first == curT) {
      pque.push(curT + TD[idx].second);
      idx++;
    }else if (get_one()) {
      ans++;
      curT++;
