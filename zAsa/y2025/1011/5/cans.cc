...
// @@ !! LIM(UnionFind sortedIndex tree doubling debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, Q; cin >> N >> M >> Q;
  // @InpMVec(M, ((A, dec=1), (B, dec=1), C)) [uRa9UtKH]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  auto C = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  REP(ii, 0, M) {
    ...
    if (uf.leader(A[i]) != uf.leader(B[i])) {
      uf.merge(A[i], B[i]);
      ll e = tr.add_edge(A[i], B[i]);
      weight[e] = C[i];
    }
  }
  vector<ll> pp(N, 0LL), ww(N, 0LL);
  REP(i, 1, N) {
    pp[i] = tr.parent(i);
    ww[i] = weight[tr.edge_idx(i, pp[i])];
  }
  auto dobj = make_doubling_with_monoid_unit_add(N, N, pp, ww, 0LL, [](ll x, ll y) { return max(x, y); });
  REP(_q, 0, Q) {
    ...
    if (u == v) ans = false;
      ...
      ll a = tr.lca(u, v);
      auto [_i, x] = dobj.val(tr.depth(u) - tr.depth(a), u);
      auto [_j, y] = dobj.val(tr.depth(v) - tr.depth(a), v);
      ans = w < max(x, y);
      DLOGK(_q, u, v, w, a, _i, x, _j, y, ans);
      DLOGK(tr.depth(u), tr.depth(v), tr.depth(a));
    }
    cout << (ans ? "Yes\n" : "No\n");
  }
