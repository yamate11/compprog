...
int main(/* int argc, char *argv[] */) {
  ...
  REPrev(i, H - 1, 0) REPrev(j, W - 1, 0) {
    if (i == H - 1 and j == W - 1) ;
      ...
      ll val = -big;
      if (i < H - 1) updMax(val, v(brd.at(i + 1, j)) - tbl[i + 1][j]);
      if (j < W - 1) updMax(val, v(brd.at(i, j + 1)) - tbl[i][j + 1]);
      tbl[i][j] = val;
    }
  }
  if (tbl[0][0] > 0) cout << "Takahashi\n";
  else if (tbl[0][0] == 0) cout << "Draw\n";
  else cout << "Aoki\n";
