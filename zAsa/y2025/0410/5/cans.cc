...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, red, type=IPoint) [SaPF2iFw]
  auto red = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; red[i] = v; }
  // @End [SaPF2iFw]
  // @InpVec(N, blue, type=IPoint) [WHplWBV5]
  auto blue = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; blue[i] = v; }
  // @End [WHplWBV5]
  ...
  REP(i, 0, N) REP(j, 0, N) {
    if (red[i].x < blue[j].x and red[i].y < blue[j].y) graph.add_edge(i, N + j, 1);
  }
  ll ans = graph.flow(2*N, 2*N + 1);
  cout << ans << endl;
