...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpMVec(M, ((U, dec=1), (V, dec=1))) [bUvyLTVr]
  auto U = vector(M, ll());
  auto V = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  /*
  // @InpNbrList(N, M, nbr, dec=1) [HiANIv1a]
  ...
  */
  ...
  vector tbl_init(N, vector(2, Fp(0)));
  auto tbl = tbl_init;
  tbl[S][0] = 1;
  REP(i, 0, K) {
    auto prev = move(tbl);
    tbl = tbl_init;
    auto op = [&](ll par, ll u, ll v) -> void {
      ll new_par = (v == X) ? 1 - par : par;
      tbl[v][new_par] += prev[u][par];
    };
    REP(e, 0, M) REP(par, 0, 2) {
      op(par, U[e], V[e]);
      op(par, V[e], U[e]);
    }
  }
