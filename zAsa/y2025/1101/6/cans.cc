...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> bool {
    ...
    if (FORALL(i, 0, N, B[i] != B[(i + 1) % N])) {
      return FORALL(i, 0, N, A[i] == B[i]);
    }

    ll i0 = EXFIND_D(i, 1, N, B[i - 1] != B[i], -1LL);
    if (i0 >= 0) {
      auto tmp = move(B);
      B = vector<ll>();
      REP(i, i0, N) B.push_back(tmp[i]);
      REP(i, 0, i0) B.push_back(tmp[i]);
    }
    ...
    auto check = [&](ll p0) -> bool {
      if (B[0] != A[p0]) return false;
      ll j = p0;
      REP(i, 1, ssize(C)) {
        auto [v, n] = C[i];
        for (; j < p0 + N and A[j] != v; j++);
        if (j == p0 + N) return false;
      }
      return true;
    };
    return EXISTS(p0, 0, N, check(p0));
  };
