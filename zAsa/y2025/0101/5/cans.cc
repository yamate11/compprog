...
int main(/* int argc, char *argv[] */) {
  ...
  Board<Fp> spR(H, W, 0);
  Board<Fp> spD(H, W, 0);
  REPrev(i, H - 2, 0) REP(j, 0, W) spD.rs(i, j) = spD.at(i + 1, j) + (com.at(i + 1, j) == '.' ? 1 : 0);
  REPrev(j, W - 2, 0) REP(i, 0, H) spR.rs(i, j) = spR.at(i, j + 1) + (com.at(i, j + 1) == '.' ? 1 : 0);
  DLOGK(spR);
  DLOGK(spD);
  ll M = max(H, W) + 1;
  vector<Fp> pow3(M);
  pow3[0] = Fp(1);
  REP(i, 1, M) pow3[i] = 3 * pow3[i - 1];
  REPrev(i, H - 1, 0) REPrev(j, W - 1, 0) {
    if (i == H - 1 and j == W - 1) {
      if (com.at(i, j) == '.') tbl.rs(i, j) = Fp(3);
      else tbl.rs(i, j) = Fp(1);
    }else {
      Fp vD = tbl.at(i + 1, j) * pow3[spR.at(i, j)];
      Fp vR = tbl.at(i, j + 1) * pow3[spD.at(i, j)];
      if (     com.at(i, j) == 'R') tbl.rs(i, j) = vR;
      else if (com.at(i, j) == 'D') tbl.rs(i, j) = vD;
      else if (com.at(i, j) == 'X') tbl.rs(i, j) = vR + vD;
      else if (com.at(i, j) == '.') tbl.rs(i, j) = 2 * (vR + vD);
      DLOGK(i, j, spR.at(i, j), spD.at(i, j), com.at(i, j), vD, vR, tbl.at(i, j));
    }
  }
  cout << tbl.at(0, 0) << endl;
