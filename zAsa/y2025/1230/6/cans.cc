...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](ll th) -> ll {
    ...
    ll hp = 0;
    REP(i, 0, N) {
      ll x = ranges::lower_bound(A, th - A[i]) - A.begin();
      ll y = ranges::lower_bound(A, th - A[i] + 1) - A.begin();
      cntLess += (N - y);
      cntEq += y - x;
      hp += B[N] - B[y] + A[i] * (N - y);
    }
    DLOGK(th, cntLess, cntEq, hp);
    if (cntLess > M) {
      return -1;
    }
    ll t = min(M - cntLess, cntEq);
    hp += th * t;
    return hp;
  };
  ...
  ll th = binsearch<ll>(check, (ll)1e6, -1);
  cout << f(th) << endl;
