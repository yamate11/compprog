...
int main(/* int argc, char *argv[] */) {
  ...
  DLOGK(P, Q, val);
  vector<ll> vec(N + 3, 0LL);
  vector<ll> vec1(N + 3, 0LL);
  vector<ll> vec2(N + 3, 0LL);

  REP(i, 0, N) {
    vec2[1] += val[i];
    vec2[P[i] + 2] -= val[i];
    vec2[Q[i] + 2] -= val[i];
    vec2[P[i] + Q[i] + 3] += val[i];
  }
  REP(i, 1, N + 3) vec1[i] = vec1[i - 1] + vec2[i];
  REP(i, 1, N + 3) vec[i] = vec[i - 1] + vec1[i];
  REPOUT(i, 1, N + 1, vec[i], "\n");
