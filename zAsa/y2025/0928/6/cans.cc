...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll nd) -> pll {
    ...
    ll tot = 0;
    REP(i, 0, numC) {
      auto [base, add] = rF(rF, tr.child(nd, i));
      tot += base;
      vec.push_back(add);
    }
    ...
    if (D[nd] == 0) {
      ...
      REP(i, 0, min(D[nd], numC)) tot += vec[i];
      ll nonsel = tot;
      if (nd == 0) {
        return {tot, 0};
      }else {
        auto [p, e] = tr.parent_pe(nd);
        ll sel = tot - (D[nd] <= numC ? vec[D[nd] - 1] : 0) + weight[e];
        return {nonsel, max(sel - nonsel, 0LL)};
      }
    }
  };
  auto [base, add] = dfs(dfs, 0);
  cout << base << endl;
