...
// @@ !! LIM(unordered_map debug forall)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  // @InpVec(H, S, type=string) [n3HBXqe6]
  ...
  vector info(H + W, vector(nChar, 0LL));
  auto mysize(ll e) = { return e < H ? W : H; };
  auto brd(ll e, ll j) -> char& { return e < H ? S[e][j] : S[j][e - H]; };
  auto f = [&](ll e) -> void {
    REP(j, 0, mysize(e)) info[e][brd(e, j) - 'a']++;
  };
  REP(e, 0, H + W) f(e);
  ll ans = H * W;
  while (true) {
    ...
    REP(e, 0, H + W) {
      ll d0 = -1;
      REP(d, 0, nChar) {
        if (info[e][d] == 1) return;
        else if (info[e][d] >= 2) {
          if (d0 >= 0) return;
          d0 = d;
        }
      }
      if (d0 >= 0) to_erase.emplace_back(e, d0);
    }
    if (to_erase.empty()) break;
    auto eidx = [&](ll tp, ll j) { return tp == 0 ? j : H + j; };
    auto tp = [&](ll e) { return e < H ? 0 : 1; };
    for (auto [e, d] : to_erase) {
      REP(j, 0, mysize(e)) {
        if (brd(e, j) != '.') {
          brd(e, j) = '.';
          ans--;
          info[eidx(1 - tp(e), j)][d]--;
        }
      }
      info[e][d] = 0;
    }
