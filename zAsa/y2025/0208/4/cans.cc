...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((A, dec=1), (B, type=char), (C, dec=1), (D, type=char))) [S4UxP8lO]
  auto A = vector(M, ll());
  auto B = vector(M, char());
  auto C = vector(M, ll());
  auto D = vector(M, char());
  for (int i = 0; i < M; i++) {
    ll v1; cin >> v1; v1 -= 1; A[i] = v1;
    char v2; cin >> v2; B[i] = v2;
    ll v3; cin >> v3; v3 -= 1; C[i] = v3;
    char v4; cin >> v4; D[i] = v4;
  }
  // @End [S4UxP8lO]
  ...
  ll cycle = 0;
  REP(i, 0, M) {
    if (uf.leader(A[i]) == uf.leader(C[i])) cycle++;
    else uf.merge(A[i], C[i]);
  }
  ll cc = 0;
  REP(i, 0, N) {
    if (uf.leader(i) == i) cc++;
  }
  cout << cycle << " " << cc - cycle << endl;
