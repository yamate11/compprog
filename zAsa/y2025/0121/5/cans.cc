...
int main(/* int argc, char *argv[] */) {
  ...
  Board<ll> horizLo(H, W, 0);
  Board<ll> horizHi(H, W, 0);
  Board<ll> vertLo(H, W, 0);
  Board<ll> vertHi(H, W, 0);
  REP(i, 0, H) {
    {
      ll c = -1;
      REP(j, 0, W) {
        if (brd.at(i, j) == '.') horizLo.rs(i, j) = c;
        else c = j;
      }
    } {
      ll c = W;
      REPrev(j, W - 1, 0) {
        if (brd.at(i, j) == '.') horizHi.rs(i, j) = c;
        else c = j;
      }
    }
  }
  REP(j, 0, W) {
    {
      ll c = -1;
      REP(i, 0, H) {
        if (brd.at(i, j) == '.') vertLo.rs(i, j) = c;
        else c = i;
      }
    } {
      ll c = H;
      REPrev(i, H - 1, 0) {
        if (brd.at(i, j) == '.') vertHi.rs(i, j) = c;
        else c = i;
      }
    }
  }
  ...
  DLOGK(vertHi);
  ll K = 0;
  REP(i, 0, H) REP(j, 0, W) if (brd.at(i, j) == '.') K++;
  vector<Fp> pow2(K + 1);
  pow2[0] = 1;
  REP(k, 1, K + 1) pow2[k] = 2 * pow2[k - 1];
  Fp ans = 0;
  REP(i, 0, H) REP(j, 0, W) {
    if (brd.at(i, j) == '.') {
      ll x = (horizHi.at(i, j) - horizLo.at(i, j) - 1) + (vertHi.at(i, j) - vertLo.at(i, j) - 1) - 1;
      DLOGK(i, j, x, pow2[K - x], pow2[K] - pow2[K - x]);
      ans += pow2[K] - pow2[K - x];
    }
  }
  cout << ans << endl;
