...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> brd(H, W, '#');
  REP(i, 0, H) REP(j, 0, W) brd.rs(i, j) = '.';
  REP(i, 0, N) {
    ll a, b; cin >> a >> b; a--; b--;
    brd.rs(a, b) = '@';
  }
  ...
  auto op = [&](const auto& start, const auto& dir, ll t) -> void {
    DLOGK(start, dir, t);
    int st = 0;
    REP(k, 0, t) {
      BrdIdx bi = start + k * dir;
      DLOGKL("  ", bi, st);
      if (brd.at(bi) == '@') st = 1;
      else if (brd.at(bi) == '#') st = 0;
      else if (st == 1) brd.rs(bi) = '%';
    }
  };

  REP(i, 0, H) {
    op(BrdIdx(i, 0), BrdIdx(0, 1), W);
    op(BrdIdx(i, W - 1), BrdIdx(0, -1), W);
  }
  REP(j, 0, W) {
    op(BrdIdx(0, j), BrdIdx(1, 0), H);
    op(BrdIdx(H - 1, j), BrdIdx(-1, 0), H);
  }
  ...
  ll ans = 0;
  REP(i, 0, H) REP(j, 0, W) {
    if (brd.at(i, j) == '@' or brd.at(i, j) == '%') ans++;
  }
  cout << ans << endl;
