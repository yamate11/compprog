...
int main(/* int argc, char *argv[] */) {
  ...
  IPoint Pt, Pa; cin >> Pt >> Pa;
  ll N, M, L; cin >> N >> M >> L;
  vector<IPoint> cp(128);
  cp['U'] = IPoint(-1,  0);
  cp['D'] = IPoint( 1,  0);
  cp['L'] = IPoint( 0, -1);
  cp['R'] = IPoint( 0,  1);
  ll Tak = 0;
  ll Aok = 1;
  using ev_t = tuple<ll, ll, IPoint>;
  vector<ev_t> ev;

  auto myread = [&](ll sz, ll person) -> void {
    ll t = 0;
    REP(i, 0, sz) {
      char c; cin >> c;
      ll a; cin >> a;
      ev.emplace_back(t, person, cp[c]);
      t += a;
    }
  };
  myread(M, Tak);
  myread(L, Aok);
  ev.emplace_back(N, Tak, IPoint(-1, -1));
  ranges::sort(ev);

  auto f = [&](IPoint P, IPoint V, ll t) -> ll {
    if (t == 0) return 0;
    if (V == IPoint(0, 0) and P == IPoint(0, 0)) return t;
    if (V.y == 0) { swap(V.x, V.y); swap(P.x, P.y); }
    if (V.y == 0) return P == IPoint(0, 0) ? t : 0;
    ll t1 = -P.y / V.y;
    return P.y % V.y == 0 and 0 < t1 and t1 <= t and P.x + t1 * V.x == 0 ? 1 : 0;
  };
  IPoint Vt = IPoint(-1, -1);
  IPoint Va = IPoint(-1, -1);
  ll ans = 0;
  REP(i, 0, ssize(ev) - 1) {
    auto [tc, mc, vc] = ev[i];
    auto [tn, mn, vn] = ev[i + 1];
    if (mc == Tak) Vt = vc;
    else Va = vc;
    ans += f(Pa - Pt, Va - Vt, tn - tc);
    Pt += (tn - tc) * Vt;
    Pa += (tn - tc) * Va;
  }
  cout << ans << endl;
