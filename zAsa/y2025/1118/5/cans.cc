...
int naive(istream& cin, ostream& cout) {
  ...
  REP(t, 0, T) {
    ...
    ll a = max(A1, A2);
    ll b = min(A1, A2);
    vector<ll> vec(a + b + 10, 0LL);
    for (ll i = du.nd_min(A1); i <= du.nd_max(A1); i++) {
      for (ll j = du.nd_min(A2); j <= du.nd_max(A2); j++) {
        vec[du.width(i + j)]++;
      }
    }
    cout << vec[A3] << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  REP(t, 0, T) {
    ...
    ll a = max(A1, A2);
    ll b = min(A1, A2);
    Fp aam = power<Fp>(10, a - 1);
    Fp bbm = power<Fp>(10, b - 1);
    Fp aa = aam * 10;
    Fp bb = bbm * 10;
    Fp xh = (bb - 1) * bb / Fp(2) - (bbm - 1) * bbm / Fp(2);
    Fp x;
    if (a == b) {
      x = xh - (bbm - 1) * bbm / Fp(2);
    }else {
