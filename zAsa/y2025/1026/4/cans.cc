...
int main(/* int argc, char *argv[] */) {
  ...
  cin >> brd;
  BrdIdx start, goal;
  auto f = [&](BrdIdx bi, BrdIdx dir) -> void {
    auto stop = [&](char c) -> bool {
      if (c == '#') return true;
  ...
  REP(i, 0, H) REP(j, 0, W) {
    ...
    else if (brd.at(bi) == 'G') goal = bi;
    else if (brd.at(bi) == '>') f(bi, BrdIdx(0, 1));
    else if (brd.at(bi) == '<') f(bi, BrdIdx(0, -1));
    else if (brd.at(bi) == '^') f(bi, BrdIdx(-1, 0));
    else if (brd.at(bi) == 'v') f(bi, BrdIdx(1, 0));
  }
  ll big = 1e18;
  ...
  while (not que.empty()) {
    ...
    for (auto dir : BrdIdx::nbr4) {
      auto bj = bi + dir;
      if (road(brd.at(bj)) and dist[bj.r][bj.c] >= big) {
