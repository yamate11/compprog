...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](ll x, ll y, ll w) -> ll { return x * du.pow(w) + y; };
  auto g = [&](ll a, ll b) -> ll {
    return llsqrt(b) - llsqrt(a - 1);
  };
  ...
  auto solve = [&]() -> ll {
    ...
    ll ans = 0;
    for (ll w = w0; 1; w++) {
      ll lo = max(C + 1, du.nd_min(w));
      ll hi = min(C + D, du.nd_max(w));
      ans += g(f(C, lo, w), f(C, hi, w));
      if (C + D <= du.nd_max(w)) break;
    }
    return ans;
  };
