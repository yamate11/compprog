...
int main(/* int argc, char *argv[] */) {
  ...
  cin >> brd;
  BrdIdx start;
  REP(i, 0, H) REP(j, 0, W) if (brd.at(i, j) == 'S') start = BrdIdx(i, j);
  ll big = 1e9;
  ...
  while (not que.empty()) {
    ...
    if (dist[bi.r][bi.c] > K) continue;
    vmin = min(vmin, min((ll)bi.r, min(H - 1 - bi.r, min((ll)bi.c, W - 1 - bi.c))));
