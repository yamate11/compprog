...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](auto& vec, ll Z) -> ll {
    DLOGK(vec, Z);
    ll t = accumulate(ALL(vec), 0LL);
    if (not (-t <= Z and Z <= t)) throw myexc();
    if ((t + Z) % 2 != 0) throw myexc();
    ll p = (t + Z) / 2;
    DLOGK(p);
    ll sz = ssize(vec);
    if (sz == 1) {
      if (vec[0] == Z) return 1;
      else if (vec[0] == -Z) return 0;
      throw myexc();
    }
    auto g = [&](ll lo, ll hi) {
      ll len = hi - lo;
