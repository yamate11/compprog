...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl(N, my_umap<ll, Fp>());
  auto f = [&](auto rF, ll k, ll carry) -> Fp {
    if (k == N) return carry + 1;
    auto [it, b] = tbl[k].try_emplace(carry, 0);
    Fp& r = it->second;
    if (b) {
      ll x = A[k] + carry;
      ll y = x / 10;
