...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  // @InpVec(H, S, type=string) [lFdcivx4]
  auto S = vector(H, string());
  for (int i = 0; i < H; i++) { string v; cin >> v; S[i] = v; }
  // @End [lFdcivx4]

  vector<vector<ll>> rows(H);
  vector<vector<ll>> cols(W);
  ll numn = 0;
  REP(i, 0, H) REP(j, 0, W) {
    if (S[i][j] == '#') {
      rows[i].push_back(j);
  ...
  auto f = [&](const auto& vec, ll x, ll vmax) -> ll {
    ll hi = lower_bound(ALL(vec), x) - vec.begin();
    ll lo = hi - 1;
    ll xhi = hi == ssize(vec) ? vmax : vec[hi];
    ll xlo = lo == -1 ? -1 : vec[lo];
    ll ret = xhi - xlo - 1;
    DLOGK(vec, x, vmax, hi, lo, xhi, xlo, ret);
    return ret;
  };

  vector<Fp> pow2(numn + 1);
  pow2[0] = 1;
  REP(i, 1, numn + 1) pow2[i] = Fp(2) * pow2[i - 1];
