...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  string S; cin >> S;
  vector tbl_init(N + 1, vector(N + 1, vector(N + 1, vector(K + 1, Fp(0)))));
  auto tbl = tbl_init;
  tbl[0][0][0][0] = Fp(1);
  REP(n, 0, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    char c = S[n];
    REP(p, 0, n + 1) REP(q, 0, n + 1 - p) REP(r, 0, n + 1 - p - q) REP(k, 0, K + 1) {
      Fp z = prev[p][q][r][k];
      if (z == Fp(0)) continue;
      ll s = n - (p + q + r);
      if (c == 'A' or c == '?') {
        tbl[s][r][q][min(K, k + s)] += z;
        ll new_p{0}, new_q{0}, new_r{0}, new_k{0}, new_s{0};
        new_s += p;
        new_r += q;
        new_q += r;
        new_p += s;
  ...
  Fp ans = 0;
  REP(p, 0, N + 1) REP(q, 0, N + 1) REP(r, 0, N + 1) ans += tbl[p][q][r][K];
  cout << ans << endl;
