...
int main(/* int argc, char *argv[] */) {
  ...
  auto check1 = [&](double m) -> bool {
    vector<double> B(N);
    REP(i, 0, N) B[i] = A[i] - m;
    DLOGK(B);
    vector<double> vec(N);
    vec[0] = B[0];
    vec[1] = max(B[0] + B[1], B[1]);
    REP(i, 2, N) {
      vec[i] = max(vec[i - 1] + B[i], vec[i - 2] + B[i]);
    }
    bool b = vec[N - 2] >= 0 or vec[N - 1] >= 0;
    DLOGK(m, vec, b);
    return b;
  };
  double mean = binsearch<double>(check1, vmin, vmax + 1, 1e-4);

  auto check2 = [&](ll m) -> bool {
    vector<ll> B(N);
    REP(i, 0, N) B[i] = A[i] >= m ? 1 : -1;
    vector<ll> vec(N);
    vec[0] = B[0];
    vec[1] = max(B[1], B[0] + B[1]);
    REP(i, 2, N) {
      vec[i] = max(vec[i - 1] + B[i], vec[i - 2] + B[i]);
    }
    return vec[N - 2] >= 1 or vec[N - 1] >= 1;
  };
  ll median = binsearch<ll>(check2, vmin, vmax + 1);

  cout << mean << endl << median << endl;
