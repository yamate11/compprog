...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpGrid(N, M, A, dec=1) [k6jlxU8J]
  auto A = vector(N, vector(M, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < M; j++) { ll v; cin >> v; v -= 1; A[i][j] = v; }
  // @End [k6jlxU8J]
  REP(i, 0, N) ranges::reverse(A[i]);

  auto check = [&](ll cap) -> bool {
    vector<ll> toassign;
    REP(i, 0, N) toassign.push_back(i);
    auto cands = A;
    vector sport(M, vector<ll>());
    vector live(M, true);
    while (not toassign.empty()) {
      ll i = toassign.back(); toassign.pop_back();
      ll a;
      while (true) {
        if (cands[i].empty()) return false;
        a = cands[i].back(); cands[i].pop_back();
        if (live[a]) break;
      }
      sport[a].push_back(i);
      if (ssize(sport[a]) > cap) {
        live[a] = false;
        ranges::copy(sport[a], back_inserter(toassign));
      }
    }
    return true;
  };
