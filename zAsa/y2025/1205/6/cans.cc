...
int main(/* int argc, char *argv[] */) {
  ...
  vector B(N, vector(N, vector<ll>()));
  B[0][0].push_back(A[0][0]);
  B[N - 1][N - 1].push_back(A[N - 1][N - 1]);
  REP(k, 1, N) REP(i, 0, k + 1) {
    ll j = k - i;
    ll p = N - 1 - i;
    ll q = 2*N - 2 - k - p;
    if (i - 1 >= 0) for (ll x : B[i - 1][j]) B[i][j].push_back(x ^ A[i][j]);
    if (j - 1 >= 0) for (ll x : B[i][j - 1]) B[i][j].push_back(x ^ A[i][j]);
    if (k < N - 1) {
      if (p + 1 < N)  for (ll x : B[p + 1][q]) B[p][q].push_back(x ^ A[p][q]);
      if (q + 1 < N)  for (ll x : B[p][q + 1]) B[p][q].push_back(x ^ A[p][q]);
    }
    DLOGK(i, j, B[i][j]);
    DLOGK(p, q, B[p][q]);
  }
  ...
  REP(i, 0, N) {
    ...
    */
    auto getnum = [&](ll x) -> ll {
      ...
      /*
      return ranges::lower_bound(B[i][j], x + 1) - ranges::lower_bound(B[i][j], x);
      */
    };
    if (i + 1 < N) for (ll x: B[i + 1][j]) ans += getnum(x);
    if (j + 1 < N) for (ll x: B[i][j + 1]) ans += getnum(x);
  }
  cout << ans << endl;
  return 0;
}
