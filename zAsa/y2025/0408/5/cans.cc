...
int main(/* int argc, char *argv[] */) {
  ...
  vector in_child(N, vector<ll>());
  vector rev(N, false);

  REP(_q, 0, Q) {
    ll M; cin >> M;
    // @InpVec(M, V, dec=1) [1b1pZBN8]
    auto V = vector(M, ll());
    for (int i = 0; i < M; i++) { ll v; cin >> v; v -= 1; V[i] = v; }
    // @End [1b1pZBN8]
    
    auto dfs = [&](auto rF, ll nd) -> ll {
      ll ret = tr.num_children(nd) - ssize(in_child[nd]);
      while (not in_child[nd].empty()) {
        ll c = in_child[nd].back(); in_child[nd].pop_back();
        ret += rF(rF, c);
      }
      return ret;
    };
    ...
    ll ans = 0;
    vector<ll> top;
    for (ll v : V) rev[v] = true;
    for (ll v : V) {
      if (v == 0 or not rev[tr.parent(v)]) top.push_back(v);
      else in_child[tr.parent(v)].push_back(v);
    }
    for (ll v : top) {
      ans += 1 + dfs(dfs, v);
    }
