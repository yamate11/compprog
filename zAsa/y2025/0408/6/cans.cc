...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  ll asz = 26;
  vector nbr(N, vector(asz, vector<ll>()));
  REP(i, 0, M) {
    ll a, b; cin >> a >> b; a--; b--;
    char c; cin >> c;
    ll d = c - 'a';
  ...
  unordered_map<IPoint, ll> dist;
  using sta = pair<ll, IPoint>;
  priority_queue<sta, vector<sta>, greater<sta>> pque;
  IPoint p0(0, N - 1);
  dist[p0] = 0;
  pque.emplace(0, p0);
  while (not pque.empty()) {
    auto [dd, p] = pque.top(); pque.pop();
    DLOGKL("popped", dd, p);
    if (dist[p] < dd) continue;
    if (p.x == p.y) {
      cout << dd << endl;
      return 0;
    }
    auto reg = [&](ll u, ll v, ll new_dist) -> void {
      if (u > v) swap(u, v);
      IPoint q(u, v);
      auto [it, b] = dist.emplace(q, big);
      if (new_dist < it->second) {
        it->second = new_dist;
        pque.emplace(new_dist, q);
        DLOGKL("pushed", new_dist, q);
      }
    };
    REP(d, 0, asz) {
      for (ll u : nbr[p.x][d]) {
