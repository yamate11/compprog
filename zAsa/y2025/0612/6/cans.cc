...
int main(/* int argc, char *argv[] */) {
  ...
  REP(k, 1, N + 1) {
    ...
    Fp fp = f;
    /*
    Fp a = Fp(1) - Fp(N - k) * Fp(N - k) / (Fp(N) * Fp(N));
    Fp b = Fp(k) / Fp(N) * fp + Fp(N - k) / Fp(N) * (Fp(1) + Fp(k) / Fp(N) * ep);
    e = b / a;
    f = Fp(k) / Fp(N) * ep + Fp(N - k) / Fp(N) * e;
    */
    Fp a = Fp(N) * Fp(N) - Fp(N - k) * Fp(N - k);
    Fp b = fp * N * k + (ep * k + N) * (N - k);
    e = b / a;
    f = (ep * k + e * (N - k)) / N;
  }
  cout << e << " " << f << endl;
