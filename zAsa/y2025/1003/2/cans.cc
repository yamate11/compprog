...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N - 1) REP(j, 0, N - 1) {
    if (brd.at(i, j) == '.' and brd.at(i, j + 1) == '.' and brd.at(i + 1, j) == '.' and brd.at(i + 1, j + 1) == '.') {
      A[i + 1][j + 1] = 1;
    }
  }
  REP(i, 0, N) REP(j, 1, N) A[i][j] += A[i][j - 1];
  REP(i, 1, N) REP(j, 0, N) A[i][j] += A[i - 1][j];
  REP(_q, 0, Q) {
    ll u, d, l, r; cin >> u >> d >> l >> r; u--; l--; d--; r--;
    ll ans = A[u][l] + A[d][r] - A[u][r] - A[d][l];
    cout << ans << "\n";
