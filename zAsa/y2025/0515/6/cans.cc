...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, S, type=string) [KfF66vdC]
  auto S = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; S[i] = v; }
  // @End [KfF66vdC]
  sort(ALL(S), [&](string& s1, string& s2) -> bool { return ssize(s1) < ssize(s2); });
  ...
  REP(i, 0, N) {
    ...
    vector<bool> app(sigma);
    REP(j, 1, sz + 1) {
      app[s[j - 1] - 'a'] = true;
      u64 e = rh.get(hs, j, sz - j);
      auto it = mp.find(e);
      if (it != mp.end()) {
        for (ll idx : it->second) {
          if (app[S[idx][0] - 'a']) ans++;
        }
      }
    }
    u64 e2 = rh.get(hs, 1, sz - 1);
    mp[e2].push_back(i);
  }
  cout << ans << "\n";
