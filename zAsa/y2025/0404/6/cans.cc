...
// @@ !! LIM(f:updMaxMin debug cmpNaive perm random segTree)
...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 0, N) init_vec[i] = DAT(N - i, i);
  auto st = make_seg_tree_lazy(DAT(big, -1), 0LL, add, plus<ll>(), apply, init_vec);
  */
  auto mymin = [&](ll x, ll y) -> ll { return min(x, y); };
  vector<ll> init_vec(N);
  REP(i, 0, N) init_vec[i] = N - i;
