...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  string S; cin >> S;
  vector tbl_init(4, vector(N + 2, vector(N + 2, vector(N + 2, Fp(0)))));
  auto tbl = tbl_init;
  tbl[0][1][0][0] = 1;
  REP(i, 0, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    char c = S[i];
    REP(cur, 0, 4) REP(x0, 0, i + 2) REP(x1, 0, i + 2) REP(x2, 0, i + 2) if (prev[cur][x0][x1][x2] != 0) {
      auto op = [&](ll pat) -> void {
        ll new_cur = cur ^ pat;
        ll new_x0 = x0, new_x1 = x1, new_x2 = x2;
        if (new_cur == 0) new_x0++;
        else if (new_cur == 1) new_x1++;
        else if (new_cur == 2) new_x2++;
        tbl[new_cur][new_x0][new_x1][new_x2] += prev[cur][x0][x1][x2];
        DLOGK(i, new_cur, new_x0, new_x1, new_x2, cur, x0, x1, x2);
      };
      if (c == 'A' or c == '?') op(3);
      if (c == 'B' or c == '?') op(1);
      if (c == 'C' or c == '?') op(2);
  ...
  auto g = [&](ll m) -> ll { return m * (m - 1) / 2; };
  Fp ans = 0;
  REP(cur, 0, 4) REP(x0, 0, N + 2) REP(x1, 0, N + 2) REP(x2, 0, N + 2) {
    ll x3 = N + 1 - (x0 + x1 + x2);
    if (g(x0) + g(x1) + g(x2) + g(x3) >= K) ans += tbl[cur][x0][x1][x2];
  }
  cout << ans << endl;
  return 0;
}
