...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, H) REP(j, 0, W) {
    ...
    for (auto dir : BrdIdx::nbr4) {
      auto bj = bi + dir;
      if (brd.at(bi) == '.' and brd.at(bj) == '.') {
        uf.merge(brd.enc(bi), brd.enc(bj));
      }
    }
  }
  vector<ll> cnt(H * W, 0LL);
  REP(i, 0, H * W) if (uf.leader(i) == i) cnt[i] = uf.group_size(i);
  DLOGK(cnt);
  REP(i, 0, H) REP(j, 0, W) {
    auto bi = BrdIdx(i, j);
    if (brd.at(bi) == '@') {
      ...
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (brd.at(bj) == '.') {
          ll p = uf.leader(brd.enc(bj));
          DLOGK(bi, bj, p, ss);
          if (not ss.contains(p)) {
            ss.insert(p);
            cnt[p]++;
            DLOG("inc");
          }
