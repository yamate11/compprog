...
int main(/* int argc, char *argv[] */) {
  ...
  if (N == 2) {
    if (A[0] == B[0] and A[1] == B[1]) cout << "Yes\n0\n";
    else if (B[0] == A[1] - 1 and B[1] == A[0] + 1) cout << "Yes\n1\n1 2\n";
    else cout << "No\n";
    return 0;
  }
  ...
  REP(i, 0, N - 1) {
    while (A[i] > B[i]) {
      ll j0 = (A[i] - B[i] <= N - 1 - i) ? i + A[i] - B[i] : N - 1;
      if (i == 0 and j0 == N - 1) j0--;
      opswap(i, j0);
      REPrev(j, j0 - 1, i) op(j, j + 1);
    }
    while (A[i] < B[i]) {
      ll j0 = (B[i] - A[i] <= N - 1 - i) ? i + (B[i] - A[i]) : N - 1;
      if (i == 0 and j0 == N - 1) j0--;
      REP(j, i, j0) op(j, j + 1);
      opswap(i, j0);
    }
