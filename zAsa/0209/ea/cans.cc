...
size_t safe_hash_fn(uint64_t x) {   // Note that uint64_t == size_t in our environment
  static const uint64_t FIXED_RANDOM = chrono::steady_clock::now().time_since_epoch().count();
  return splitmix64(x + FIXED_RANDOM);
}
...
namespace std {
  ...
  struct hash<IPoint> {
    size_t operator()(const IPoint& p) const {
      uint64_t a, b;
      a = splitmix64((uint64_t)(p.x));
      b = splitmix64((uint64_t)(p.y));
      return a ^ ((b >> 32) | (b & ((1ULL << 32) - 1) << 32));
    }
  };
}
...
/*
size_t my_pair_hash(const pll& p) {
...
int main(/* int argc, char *argv[] */) {
  ...
  gp_hash_table<IPoint, int> mp1;
  unordered_map<IPoint, int> mp2;
    
  double t1 = get_time_sec();
  REP(i, 0, N) mp2[A[i]] = B[i];
  double t2 = get_time_sec();
  REP(i, 0, N) mp1[A[i]] = B[i];
  ...
  gp_hash_table<int, int, safe_custom_hash<int>> mp3(safe_custom_hash<int>());
