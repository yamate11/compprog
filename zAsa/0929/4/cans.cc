...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, P, type=IPoint) [YRDX2uRI]
  ...
  vector<ll> vec1, vec2;
  REP(i, 0, N) {
    vec1.push_back(P[i].x + P[i].y);
    vec2.push_back(P[i].x - P[i].y);
  }
  ll a1 = *max_element(ALL(vec1));
  ll b1 = *min_element(ALL(vec1));
  ll a2 = *max_element(ALL(vec2));
  ll b2 = *min_element(ALL(vec2));
