...
int main(/* int argc, char *argv[] */) {
  ...
  vector horiz(H, vector<ll>{-1, W});
  vector vert(W, vector<ll>{-1, H});
  REP(i, 0, H) REP(j, 0, W) {
    if (brd.at(i, j) == '#') {
      horiz[i].push_back(j);
      vert[j].push_back(i);
  ...
  REP(i, 0, H) sort(ALL(horiz[i]));
  REP(i, 0, W) sort(ALL(vert[i]));
  
  ll ans = 0;
  REP(i, 0, H) REP(j, 0, W) {
    if (brd.at(i, j) == '.') {
      ll p = lower_bound(ALL(horiz[i]), j) - horiz[i].begin();
      ll q = lower_bound(ALL(vert[j] ), i) - vert[j].begin();
      ll dp = horiz[i][p] - horiz[i][p - 1] - 1;
      ll dq = vert[ j][q] - vert[ j][q - 1] - 1;
      ans = max(ans, dp + dq - 1);
    }
  }
  cout << ans << endl;
