...
// @@ !! LIM -- end mark --
  ...
  struct fwd_t {
    ...
    friend ostream& operator<<(ostream& ostr, const fwd_t& t) {
      ostr << "(" << t.nd << ", " << t.C << ")";
      return ostr;
    }
  };
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpNbrList(N, M, fwd, dir=True, bwd=bwd, dec=1, read=C, ostr=True) [tPiJNsQt]
  ...
  ll big = 1e18;
  auto func = [&](ll st, const auto& nbr) {
  ...
  auto rble1 = func(0,     fwd);
  auto rble2 = func(N - 1, bwd);
  vector<bool> eff(N);
  REP(i, 0, N) eff[i] = rble1[i] and rble2[i]; 
