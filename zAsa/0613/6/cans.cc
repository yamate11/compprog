...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = 0;
  REP(i, 0, N) REP(j, i + 1, N + 1) if (check(i, j)) ans++;
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  auto f = [&](auto rF, ll i) -> pair<ll, ll> {
    ...
    if (ns < 0) {
      if (i == 0) {
        ...
        while (true) {
          ...
          auto [p_pos, p_ns] = rF(rF, p);
          DLOGKL("loop", i, p, p_pos, p_ns);
          if      (A[i] == 1) {
            pos = i;
            ns = p_ns + 1;
            break;
    ...
    DLOGKL("ret", i, pos, ns, _pos, _ns);
    return {pos, ns};
  };
  f(f, N - 1);
  ll ans = accumulate(ALL(_ns), 0LL);
  cout << ans << endl;
