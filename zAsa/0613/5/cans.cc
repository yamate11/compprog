...
int main(/* int argc, char *argv[] */) {
  ...
  REPrev(i, H - 1, 0) REPrev(j, W - 1, 0) {
    ...
    bool b1 = brd.at(i + 1, j) == '.' and tbl.at(i + 1, j) == 1;
    bool b2 = brd.at(i, j + 1) == '.' and tbl.at(i, j + 1) == 1;
    bool b3 = brd.at(i + 1, j + 1) == '.' and tbl.at(i + 1, j + 1) == 1;
    if (b1 or b2 or b3) tbl.rs(i, j) = 0;
    else tbl.rs(i, j) = 1;
  }
  string ans = tbl.at(0, 0) ? "Second" : "First";
  cout << ans << endl;
  return 0;
}
