...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, X; cin >> N >> X;
  vector A(3, vector<pll>());
  REP(i, 0, N) {
    ll v, a, c; cin >> v >> a >> c; v--;
    A[v].emplace_back(a, c);
  }
  ...
  REP(v, 0, 3) {
    ...
    for (auto [a, c] : A[v]) {
      REPrev(k, X, 0) {
        if (tbl[v][k] >= 0 and k + c <= X) updMax(tbl[v][k + c], tbl[v][k] + a);
      }
    }
    REP(k, 1, X + 1) updMax(tbl[v][k], tbl[v][k - 1]);
    DLOGK(v, tbl[v]);
  }
  auto check = [&](ll th) -> bool {
    ll tot = 0;
    REP(v, 0, 3) {
      if (th > tbl[v][X]) return false;
      ll c = ranges::lower_bound(tbl[v], th) - tbl[v].begin();
      tot += c;
      DLOGKL("  ", th, v, c);
    }
    DLOGK(th, tot);
  ...
  ll lim = N * 2e5 + 1;
  ll ans = binsearch<ll>(check, 0, lim);
  cout << ans << endl;
  return 0;
}
