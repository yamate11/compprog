...
// @@ !! LIM(cmpNaive digit forall debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  digit_util du(2);
  ll t = *ranges::max_element(A);
  ll k = du.width(t);
  ll ans = 1e18;
  REP(x, 0, du.nd_max(k) + 1) {
    ll vm = 0;
    for (ll a : A) vm = max(vm, a ^ x);
    DLOGK(x, vm);
    ans = min(ans, vm);
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  auto f = [&](auto rF, const auto& V, ll k) -> ll {
    if (k == 0) {
      if (FORALL(i, 0, ssize(V), V[i] == V[0])) return 0;
      else return 1;
    }
    vector<ll> vec0;
    vector<ll> vec1;
    for (ll v : V) {
      if (v >> k & 1) vec1.push_back(v ^ (1LL << k));
      else vec0.push_back(v);
    }
    if (ssize(vec0) == 0) return rF(rF, vec1, k - 1);
    if (ssize(vec1) == 0) return rF(rF, vec0, k - 1);
    ll x0 = rF(rF, vec0, k - 1);
    ll x1 = rF(rF, vec1, k - 1);
    if (x0 <= x1) return (1LL << k) + x0;
    else          return (1LL << k) + x1;
  };
  cout << f(f, A, K) << endl;
