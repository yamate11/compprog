...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    {
      string S; cin >> S;
      REP(i, 0, N) colA[i] = S[i] == 'B' ? 0 : 1;
    }
    vector colB(N, 0);
    auto dfs = [&](auto rF, ll nd) -> int {
      vector<ll> cnt(2);
      for (int c : tr.children(nd)) {
        char x = rF(rF, c);
        if (x >= 0) cnt[x]++;
      }
      if (cnt[0] > 0 and cnt[1] > 0) throw myexc();
      else if (cnt[0] > 0) colB[nd] = 0;
      else if (cnt[1] > 0) colB[nd] = 1;
      else if (nd != 0) colB[nd] = colA[tr.parent(nd)];
      else colB[nd] = 0;
      vector<ll> cum(2);
      for (int c : tr.children(nd)) cum[colB[c]]++;
      ll col_for = cum[colA[nd]], col_against = cum[1 - colA[nd]];
      if (col_for < col_against) throw myexc();
      if (nd != 0) {
        if (col_for == col_against) return colA[nd];
        else return -1;
      }else {
        return -1;
      }
