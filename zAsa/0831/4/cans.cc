...
int main(/* int argc, char *argv[] */) {
  ...
  REP(q, 0, Q) {
    ...
    if (bS[x]) {
      ...
      bS[x] = true;
      num[q] = q == 0 ? 1 : num[q - 1] + 1;
    }
  }
  REP(i, 0, N) if (bS[i]) app[i].push_back(Q);

  vector accnum(Q + 1, 0LL);
  REP(q, 0, Q) accnum[q + 1] = accnum[q] + num[q];
  DLOGK(num);
  DLOGK(accnum);
  DLOGK(app);
  vector<ll> ans(N);
  REP(i, 0, N) {
    for (int j = 0; j < ssize(app[i]); j += 2) {
      ans[i] += accnum[app[i][j + 1]] - accnum[app[i][j]];
    }
  }
  REPOUT(i, 0, N, ans[i], " ");
