...
// @@ !! LIM(debug mod sortedIndex segTree)
...
int main(/* int argc, char *argv[] */) {
  ...
  // @End [HxmWwv0f]
  auto ord = sortedIndexCFAI(A, [&](ll i, ll j) { return A[i] != A[j] ? A[i] > A[j] : i > j; });
  vector pow2(N + 1, Fp(1));
  REP(i, 1, N + 1) pow2[i] = pow2[i - 1] * 2;
  auto segt = make_seg_tree(Fp(0), plus<Fp>(), vector<Fp>(N));
  Fp ans = 0;
  REP(ii, 0, N) {
    ll i = ord[ii];
    ll x = segt.query(i + 1, N);
    ans += x / pow2[i + 1];
    segt.rs(i) = pow2[i];
  }
  cout << ans << "\n";
