...
int main(/* int argc, char *argv[] */) {
  ...
  ll vmax = 0;
  REP(i, 0, H) REP(j, 0, W) vmax = max(vmax, brd.at(i, j));
  Board<bool> top(H, W, false);
  REP(i, 0, H) REP(j, 0, W) if (brd.at(i, j) == vmax) top.rs(i, j) = true;

  Board<ll> val(H, W, -big);
  val.rs(start) = 0;
  ll k = 0;
  for (; k < min(K, H * W + 10); k++) {
    auto prev = val;
    REP(i, 0, H) REP(j, 0, W) val.rs(i, j) = val.at(i, j) + brd.at(i, j);
    REP(i, 0, H) REP(j, 0, W) {
      BrdIdx bi(i, j);
      for (auto dir : BrdIdx::nbr4) {
        auto bj = bi + dir;
        if (brd.in(bj) and val.at(bj) < prev.at(bi) + brd.at(bj)) {
          val.rs(bj) = prev.at(bi) + brd.at(bj);
        }
      }
    }
  }
  ll ans = 0;
  REP(i, 0, H) REP(j, 0, W) {
    ll cur = val.at(i, j) + (K - k) * brd.at(i, j);
    ans = max(ans, cur);
  }
  cout << ans << endl;
  return 0;
