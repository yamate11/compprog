...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl(N + 1, vector(N + 1, vector(2, optional<Fp>())));
  auto f = [&](auto rF, ll a, ll b, bool tak) -> Fp {
    auto& r = tbl[a][b][tak];
    if (not r) {
      if (tak) {
        Fp prob = 0;
        REP(p, 1, P + 1) {
          if (a + p >= N) prob += Fp(1) / P;
          else prob += rF(rF, a + p, b, false) / P;
        }
        r = prob;
      }else {
        Fp prob = 0;
