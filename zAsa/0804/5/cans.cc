...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> bool {
    ...
    auto read_brd = [&]() -> Board<char> {
      ll H, W; cin >> H >> W;
      Board<char> ret(H, W, '.');
      cin >> ret;
      ll r0 = 0, r1 = H, c0 = 0, c1 = W;
      while (FORALL(j, 0, W, ret.at(r0, j) == '.')) r0++;
      while (FORALL(j, 0, W, ret.at(r1 - 1, j) == '.')) r1--;
      while (FORALL(i, 0, H, ret.at(i, c0) == '.')) c0++;
      while (FORALL(i, 0, H, ret.at(i, c1 - 1) == '.')) c1--;
      Board<char> ans(r1 - r0, c1 - c0, '.');
      REP(i, 0, r1 - r0) REP(j, 0, c1 - c0) ans.rs(i, j) = ret.at(r0 + i, c0 + j);
      return ans;
    };
    ...
    ll ar = brdA.numRows(), ac = brdA.numCols();
    ll br = brdB.numRows(), bc = brdB.numCols();
    ll xr = brdX.numRows(), xc = brdX.numCols();
  
    DLOG(brdA);
    DLOG(brdB);
    DLOG(brdX);
    DLOGK(ar, br, xr, ac, bc, xc);

    if (ar > xr or br > xr or ac > xc or bc > xc) return false;

    auto check = [&](ll dar, ll dac, ll dbr, ll dbc) -> bool {
      Board<char> brdY(xr, xc, '.');
      REP(i, 0, ar) REP(j, 0, ac) if (brdA.at(i, j) == '#') brdY.rs(i + dar, j + dac) = '#';
      REP(i, 0, br) REP(j, 0, bc) if (brdB.at(i, j) == '#') brdY.rs(i + dbr, j + dbc) = '#';
      bool b = FORALL(i, 0, xr, FORALL(j, 0, xc, brdY.at(i, j) == brdX.at(i, j)));
      DLOGK(dar, dac, dbr, dbc);
      DLOG(brdY);
      return b;
    };

    REP(i0, 0, xr - ar + 1) REP(j0, 0, xc - ac + 1) REP(i1, 0, xr - br + 1) REP(j1, 0, xc - bc + 1) {
      if (check(i0, j0, i1, j1)) return true;
    }
    return false;
  };
