...
// @@ !! LIM(sortedIndex cmpNaive f:updMaxMin debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = 0;
  REP(x, 0, 1LL << N) {
    if (x == 0 or x + 1 == (1LL << N)) continue;
    ll lo0 = 0, hi0 = big, lo1 = 0, hi1 = big;
    REP(i, 0, N) {
      if (x >> i & 1) {
        updMax(lo0, L[i]);
        updMin(hi0, R[i]);
    ...
    ll val = max(0LL, hi0 - lo0 + 1) + max(0LL, hi1 - lo1 + 1);
    updMax(ans, val);
  }
  cout << ans << endl;
  return 0;
...
int body(istream& cin, ostream& cout) {
  ...
  if (N == 2) {
    cout << R[0] - L[0] + 1 + R[1] - L[1] + 1 << endl;
    return 0;
  }
  auto [ord, inv] = sortedIndex2(L);
  ll i0 = ord[N - 1];
  ll i1 = ranges::min_element(R) - R.begin();
  DLOGK(i0, i1);
  ll vmax = 0;
  REP(i, 0, N) {
    if (i == i0 or i == i1) continue;
    vmax = max(vmax, R[i] - L[i] + 1);
  }
  ll conj = max(0LL, R[i1] - L[i0] + 1);
  if (i0 == i1) {
    cout << vmax + conj << endl;
    return 0;
  }
  ...
  REP(ii, 0, N) {
    ll i = ord[ii];
    L0[ii] = max(0LL, R[i1] - max(L[i], L[i1]) + 1);
  }
  ll rr = R[i0];
  DLOGKL("init rr", rr);
  L1[N - 1] = R[i0] - L[i0] + 1;
  REPrev(ii, N - 2, 0) {
    ll i = ord[ii];
    rr = min(rr, R[i]);
    L1[ii] = max(0LL, rr - L[i0] + 1);
    DLOGK(ii, i, R[i], rr, L[i0], L1[ii]);
  }
  DLOGK(L0);
  DLOGK(L1);
  ll wmax = 0;
  REP(ii, 0, N - 1) wmax = max(wmax, L0[ii] + L1[ii + 1]);
  ll ans = max(vmax + conj, wmax);
  cout << ans << endl;
