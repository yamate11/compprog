...
// @DefStruct(P, ((ok, bool), (left, int), (right, int))) [W9qqLiZe]
struct P_t {
  bool ok;
  int left;
  int right;
  P_t(bool ok_ = bool(), int left_ = int(), int right_ = int()) : ok(ok_), left(left_), right(right_) {}
  friend istream& operator>>(istream& istr, P_t& t) {
    istr >> t.ok >> t.left >> t.right;
    return istr;
  }
  friend ostream& operator<<(ostream& ostr, const P_t& t) {
    ostr << "(" << t.ok << ", " << t.left << ", " << t.right << ")";
    return ostr;
  }
};
// @End [W9qqLiZe]
...
int main(/* int argc, char *argv[] */) {
  ...
  using DAT = P_t;
  using OP = int;
  auto add = [&](DAT x, DAT y) -> DAT {
    if (x.left == -1) return y;
    else if (y.left == -1) return x;
    else if (x.ok and y.ok and x.right != y.left) return DAT(true, x.left, y.right);
    else return DAT(false, 0, 0);
  };
  auto comp = [&](OP f, OP g) -> OP { return f ^ g; };
  auto appl = [&](OP f, DAT x) -> DAT {
    if (f == 0) return x;
    else return {x.ok, 1 - x.left, 1 - x.right};
  };
  vector<DAT> init_vec(N);
  REP(i, 0, N) init_vec[i] = DAT(true, S[i] - '0', S[i] - '0');
  auto st = make_seg_tree_lazy(DAT(true, -1, -1), 0, add, comp, appl, init_vec);
