...
int naive(istream& cin, ostream& cout) {
  ...
  auto satisfies = [&](const auto& V) -> bool {
    ll sz = ssize(V);
    if (sz <= 1) return false;
    REP(i, 0, sz) {
      REP(j, 0, sz) {
        if (j == i) continue;
        if ((j == (i + 1) % sz) != conn[V[i]][V[j]]) return false;
      }
    }
    return true;
  };
...
int body(istream& cin, ostream& cout) {
  ...
  vector<bool> onhist(N);
  vector<ll> hist;
  vector<ll> cyc;
  auto dfs = [&](auto rF, ll nd) -> void {
    if (onhist[nd]) {
      while (true) {
        ll x = hist.back(); hist.pop_back();
        cyc.push_back(x);
        if (x == nd) break;
      }
      reverse(ALL(cyc));
