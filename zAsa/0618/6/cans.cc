...
enum kind {PERSON, ROAD};

// @DefStruct(ev, (tm, (k, kind), a, b, i, v), ord=((tm, True), k), istr=False) [o8fzdjDr]
struct ev_t {
  ll tm;
  kind k;
  ll a;
  ...
  ll v;
  // ev_t() {}
  ev_t(ll tm_ = ll(), kind k_ = kind(), ll a_ = ll(), ll b_ = ll(), ll i_ = ll(), ll v_ = ll()) : tm(tm_), k(k_), a(a_), b(b_), i(i_), v(v_) {}
  friend ostream& operator<<(ostream& ostr, const ev_t& t) {
    ostr << "(" << t.tm << ", " << t.k << ", " << t.a << ", " << t.b << ", " << t.i << ", " << t.v << ")";
    return ostr;
  }
  bool operator<(const ev_t& o) const {
    if (tm > o.tm) return true;
    if (tm < o.tm) return false;
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, M) {
    ...
    ev.emplace_back(y, ROAD, a, b, -1, -1);
  }
  ll Q; cin >> Q;
  REP(i, 0, Q) {
    ll v, w; cin >> v >> w; v--;
    ev.emplace_back(w, PERSON, -1, -1, i, v);
  }
  sort(ALL(ev));
  ...
  UnionFind uf(N);
  for (auto e : ev) {
    if (e.k == ROAD) uf.merge(e.a, e.b);
    else             ans[e.i] = uf.group_size(e.v);
  }
