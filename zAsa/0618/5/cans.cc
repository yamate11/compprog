...
// @DefStruct(sta, ((a, Fp), (b, Fp))) [TGdTcpf0]
struct sta {
  Fp a;
  Fp b;
  string show() const {
    return format("({}, {})", (ll)a, (ll)b);
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  itv_map<sta> is(0, N + 2);
  REP(i, 0, N + 1) is.put(i, sta(0, SA[i]));
  REP(m, 0, M) {
    ll l, r; cin >> l >> r; l--;
    auto [lo_l, hi_l, xl] = is.get(l);
    auto [lo_r, hi_r, xr] = is.get(r);
    Fp vl = xl.a * (l - lo_l) + xl.b;
    Fp vr = xr.a * (r - lo_r) + xr.b;
    is.put(l, r, sta(Fp(vr - vl) / Fp(r - l), vl));
    is.put(r, hi_r, sta(xr.a, vr));
  }
  vector<Fp> V(N + 1);
  for (ll i = 0; i < N + 1; ) {
    auto [lo, hi, x] = is.get(i);
    REP(j, lo, hi) {
      V[j] = x.a * (j - lo) + x.b;
    }
    i = hi;
  }
  REPOUT(i, 0, N, V[i + 1] - V[i], " ");
