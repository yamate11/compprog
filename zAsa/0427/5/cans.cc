...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> posS;
  REP(i, 0, N) {
    acc1[i + 1] = acc1[i] + (ll)(S[i] == '1');
    acc2[i + 1] = acc2[i] + (ll)(S[i] == '2');
    if (S[i] == '/') posS.push_back(i);
  }
  DLOGK(acc1, acc2, posS);
  
  REP(_q, 0, Q) {
    ll l, r; cin >> l >> r; l--;
    auto num1 = [&](ll i) -> ll { return acc1[posS[i]] - acc1[l]; };
    auto num2 = [&](ll i) -> ll { return acc2[r] - acc2[posS[i]]; };
    auto check = [&](ll i) -> bool {
      DLOGK(i, num1(i), num2(i));
      if (posS[i] < l) return true;
      if (r <= posS[i]) return false;
      return num1(i) < num2(i);
    };
    auto valid = [&](ll i) -> bool { return 0 <= i and i < ssize(posS) and l <= posS[i] and posS[i] < r; };
    ll i0 = binsearch<ll>(check, -1, ssize(posS));
    ll ans = -1;
    if (valid(i0    )) ans = max(ans, num1(i0));
    if (valid(i0 + 1)) ans = max(ans, num2(i0 + 1));
    cout << (ans < 0 ? 0 : ans * 2 + 1) << "\n";
  }
