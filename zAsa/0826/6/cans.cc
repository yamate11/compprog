...
int main(/* int argc, char *argv[] */) {
  ...
  REP(_q, 0, Q) {
    ...
    auto f = [&](ll ww) -> pll {
      ll w = ww / 2;
      ll b = ww % 2;
      ll i2 = ranges::lower_bound(A, b ? x + w + 1 : x + w) - A.begin();
      ll i1 = ranges::lower_bound(A, x - w) - A.begin();
      return {i1, i2};
    };
    auto check = [&](ll ww) -> bool {
      auto [i1, i2] = f(ww);
      return i2 - i1 <= N - i2;
    };

    ll ww = binsearch<ll>(check, -1, 2.1e9);
    auto [i1, i2] = f(ww);
    DLOGK(ww, i1, i2);
    ll ans = lastV;
    if (i2 - i1 == N - i2) {
      ans += SA[N] - SA[i2];
      if (i1 - 2 >= 0) ans += B[i1 - 2];
    }else if (i2 - i1 + 1 == N - i2) {
      ans += SA[N] - SA[i2 + 1];
      if (i1 - 1 >= 0) ans += B[i1 - 1];
    }else assert(0);
    cout << ans << "\n";
  }
