...
// @@ !! LIM(f:intDiv mod forall cmpNaive debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  REP(x, 0, 1LL << N) {
    auto check = [&]() -> bool {
      if (EXISTS(i, 0, N, S[i] != '?' and ((x >> i & 1) != (S[i] == 'R')))) return false;
      vector<ll> spoon(N, true);
      REP(ii, 0, N) {
        ll i = P[ii];
        ll j = imod(i - 1, N);
        if (not spoon[i] and not spoon[j]) return false;
        else if (not spoon[i]) spoon[j] = false;
        else if (not spoon[j]) spoon[i] = false;
        else if (x >> i & 1) spoon[i] = false;
        else spoon[j] = false;
      }
      return true;
    };
...
int body(istream& cin, ostream& cout) {
  ll N; cin >> N;
  // @InpVec(N, rP, dec=1) [ms9SNJTs]
  auto rP = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; rP[i] = v; }
  // @End [ms9SNJTs]
  vector<ll> P(N);
  REP(i, 0, N) P[rP[i]] = i;
  string S; cin >> S;
  vector<bool> itop(N, false);
  vector<bool> ibot(N, false);
  vector<ll> tops;
  vector<ll> bots;
  REP(i, 0, N) {
    ll j = imod(i - 1, N);
    ll k = imod(i + 1, N);
    if (P[j] > P[i] and P[i] < P[k]) {
      itop[i] = true;
      tops.push_back(i);
    }
    if (P[j] < P[i] and P[i] > P[k]) {
