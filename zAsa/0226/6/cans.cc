...
// @@ !! LIM(binsearch debug cmpNaive perm f:intDiv)
...
int naive(istream& cin, ostream& cout) {
  ...
  while (ip.get()) {
    ll cur = 0;
    REP(i, 0, N - 1) {
      cur += imod(A[ip.at(i + 1)] - A[ip.at(i)], K);
    }
    ans = max(ans, cur);
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  auto check = [&](ll m) -> bool {
    ...
    REP(i, 0, N) {
      ll j = i % m;
      if (not vec[j].empty() and vec[j].back() == A[i]) return false;
      vec[j].push_back(A[i]);
    }
    return true;
  };
  ll m = binsearch<ll>(check, N, 0);
  DLOGK(m);
  cout << (N - m) * K + A[N - 1] - A[0] << endl;
  */

  map<ll, ll> mp;
  REP(i, 0, N) mp[A[i]]++;
  ll m = 0;
  for (auto [a, x] : mp) m = max(m, x);
  vector<ll> vms;
  for (auto [a, x] : mp) {
    if (x == m) vms.push_back(a);
  }
  ll z = ssize(vms);
  ll ans = (N - m) * K - (vms[z - 1] - vms[0]);
  REP(i, 0, z - 1) ans = max(ans, (N - m - 1) * K + vms[i + 1] - vms[i]);
  cout << ans << endl;
  return 0;
}
