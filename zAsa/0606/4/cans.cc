...
// @@ !! LIM(forall debug perm f:updMaxMin)
...
int main(/* int argc, char *argv[] */) {
  ...
  while (ipH.get()) {
    ll cntH = 0;
    REP(i, 0, H) REP(j, i + 1, H) if (ipH[i] > ipH[j]) cntH++;
    IntPerm ipW(W, W);
    while (ipW.get()) {
      if (FORALL(i, 0, H, FORALL(j, 0, W, A[i][j] == B[ipH[i]][ipW[j]]))) {
        ll cntW = 0;
        REP(i, 0, W) REP(j, i + 1, W) if (ipW[i] > ipW[j]) {
          DLOGK(i, j);
          cntW++;
        }
        DLOGK(ipH.vec_view(), ipW.vec_view(), cntH, cntW);
        updMin(ans, cntH + cntW);
      }
    }
  }
  if (ans == big) ans = -1;
