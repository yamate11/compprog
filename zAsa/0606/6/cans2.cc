...
// @@ !! LIM(UnionFind board f:updMaxMin)
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, H) REP(j, 0, W) {
    BrdIdx bi0(i, j);
    if (visited.at(bi0)) continue;
    if (brd.at(bi0) == 'o') {
      
      int rmax = bi0.r, cmax = bi0.c, rmin = bi0.r, cmin = bi0.c;
      auto dfs = [&](auto rF, BrdIdx bi) -> void {
        if (brd.at(bi) == '.') return;
        if (visited.at(bi)) return;
        visited.at(bi) = true;
        updMax(rmax, bi.r); updMin(rmin, bi.r); updMax(cmax, bi.c); updMin(cmin, bi.c);
        for (auto d : BrdIdx::nbr8) rF(rF, bi + d);
      };
      dfs(dfs, bi0);
      ll s = (rmax - rmin + 1) / 5;
