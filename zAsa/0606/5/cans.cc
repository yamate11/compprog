...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N; cin >> N;
    // @InpVec(1LL << N, P) [oFOw4vPU]
    auto P = vector(1LL << N, ll());
    for (int i = 0; i < 1LL << N; i++) { ll v; cin >> v; P[i] = v; }
    // @End [oFOw4vPU]
    ll M0 = 1LL << N;
    ll M1 = 1LL << (N + 1);
    vector vec(M1, 0LL);
    REP(i, 0, M0) vec[M0 + i] = P[i];
    REPrev(i, M0 - 1, 1) vec[i] = min(vec[2*i], vec[2*i + 1]);
    DLOGK(vec);
    DLOGK(P);
