...
int main(/* int argc, char *argv[] */) {
  ...
  Real x = (Real)B / A;
  if (x >= 2.0 / sqrt(3.0)) {
    cout << 2.0 / sqrt(3.0) * (Real)A;
    return 0;
  }
  auto check = [&](Real th) -> bool {
    Point O(0, 0);
    Real y = x * tan(th);
    Point P(x, y);
    Point dir1 = Point::polar(1.0, th + G_PI * 2.0 / 3.0);
    Line lPQ(dir1, P);
    Point dir2 = Point::polar(1.0, th + G_PI * 1.0 / 3.0);
    Line lOQ(dir2, O);
    Point Q = lPQ.unsafe_intersect(lOQ);
    return Q.y <= 1;
  };
  Real th = binsearch_r<Real>(check, 0, G_PI / 6.0, 1e-10, true);
  cout << x / cos(th) * (Real)A << endl;
