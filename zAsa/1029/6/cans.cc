...
int main(/* int argc, char *argv[] */) {
  ...
  vector<sta> tbl_init(2*N + 1);
  auto tbl = tbl_init;
  tbl[N + 0] = sta(Fp(1), Fp(0));
  DLOGK(tbl);
  vector<ll> vL0(2*N + 1), vR0(2*N + 1), vL1(2*N + 1), vR1(2*N + 1), vL(2*N), vR(2*N);
  REP(i, 1, 2*N + 1) {
    if (T[i - 1] == '+') { vL0[i] = vL0[i - 1] + 1; vR0[i] = vR0[i - 1] + 1; }
    if (T[i - 1] == '-') { vL0[i] = vL0[i - 1] - 1; vR0[i] = vR0[i - 1] - 1; }
    if (T[i - 1] == '?') { vL0[i] = vL0[i - 1] - 1; vR0[i] = vR0[i - 1] + 1; }
  }
  REPrev(i, 2*N - 1, 0) {
    if (T[i] == '-') { vL1[i] = vL1[i + 1] + 1; vR1[i] = vR1[i + 1] + 1; }
    if (T[i] == '+') { vL1[i] = vL1[i + 1] - 1; vR1[i] = vR1[i + 1] - 1; }
    if (T[i] == '?') { vL1[i] = vL1[i + 1] - 1; vR1[i] = vR1[i + 1] + 1; }
  }
  REP(i, 0, 2*N) {
    vL[i] = max(vL0[i + 1], vL1[i + 1]);
    vR[i] = min(vR0[i + 1], vR1[i + 1]);
  }
  DLOGK(vL, vR);
  REP(i, 0, 2*N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    if (T[i] == '+') {
      REP(j, vL[i], vR[i] + 1) if (N + j - 1 >= 0) {
        auto [x, y] = prev[N + j - 1];
        tbl[N + j] = sta(x, y + x * abs(j));
      }
    }else if (T[i] == '-') {
      REP(j, vL[i], vR[i] + 1) if (N + j + 1 <= 2*N) {
        auto [x, y] = prev[N + j + 1];
        tbl[N + j] = sta(x, y + x * abs(j));
      }
    }else {
