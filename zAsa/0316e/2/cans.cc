...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) cnt2[C[i]]++;
  map<ll, ll> cnt;
  for (auto [_, n] : cnt2) cnt[n]++;
  vector<pll> vec;
  for (auto [n, dup] : cnt) vec.emplace_back(n, dup);
  ll sz = SIZE(vec);
  vector g(sz, vector(N + 1, Fp(1)));

  REP(K, 1, N + 1) {
    Fp e = 0;
    REP(i, 0, sz) {
      g[i][K] = g[i][K - 1] * (N - vec[i].first - K + 1) / (N - K + 1);
      e += vec[i].second * (Fp(1) - g[i][K]);
    }
    cout << e << "\n";
  }
