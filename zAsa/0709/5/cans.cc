...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, Q; cin >> N >> M >> Q;
  // @InpMVec(M, ((L, dec=1), (R, dec=1))) [30q3NE8a]
  auto L = vector(M, ll());
  auto R = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  // @End [30q3NE8a]
  // @InpMVec(Q, ((vecP, dec=1), (vecQ, dec=1))) [RaxHPunX]
  auto vecP = vector(Q, ll());
  auto vecQ = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
    ll v1; cin >> v1; v1 -= 1; vecP[i] = v1;
    ll v2; cin >> v2; v2 -= 1; vecQ[i] = v2;
  }
  // @End [RaxHPunX]

  vector<ll> ans(Q);
  
  enum kind { TRAIN, QUERY };
  using ev_t = tuple<ll, kind, ll, ll>;
  vector<ev_t> ev;
  REP(i, 0, M) ev.emplace_back(R[i], TRAIN, L[i], i);
  REP(i, 0, Q) ev.emplace_back(vecQ[i], QUERY, vecP[i], i);
  sort(ALL(ev));
  
  auto st = make_seg_tree(0LL, plus<ll>(), vector<ll>(N));
  for (auto [_r, kd, lo, idx] : ev) {
    if (kd == TRAIN) {
      st.rs(lo) = st.at(lo) + 1;
    }else {
      ans[idx] = st.query(lo, N);
    }
  }
  REPOUT(i, 0, Q, ans[i], "\n");
