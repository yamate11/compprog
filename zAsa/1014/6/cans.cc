...
int naive(istream& cin, ostream& cout) {
  ...
  while (dip.get()) {
    ...
    double ssd = 0.0;
    REP(i, 0, D) ssd += (vec[i] - meanW) * (vec[i] - meanW);
    vmin = min(vmin, ssd / D);
  }
  cout << vmin << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  ll vmin = 1e18;
  double ans = 1e18;
  auto sch = [&](auto rF, ll i, ll acc) -> void {
    DLOGKL("in", i, curW, acc);
    if (acc >= vmin) return;
    if (i == N) {
      vmin = acc;
      double ssd = 0;
      REP(j, 0, D) ssd += (curW[j] - meanW) * (curW[j] - meanW);
      ans = ssd / D;
      return;
    }
    vector<ll> cands;
    set<ll> tmp;
    REP(j, 0, D) {
      if (curW[j] * D >= totW) continue;
      if (tmp.contains(curW[j])) continue;
      tmp.insert(curW[j]);
      cands.push_back(j);
    }
    DLOGK(i, curW, cands);
    for (ll j : cands) {
      ll newAcc = acc - curW[j] * curW[j] + (curW[j] + W[i]) * (curW[j] + W[i]);
      curW[j] += W[i];
      rF(rF, i + 1, newAcc);
      curW[j] -= W[i];
    }
  ...
  sch(sch, 0, 0);
  DLOGK(vmin);
  // cout << (vmin - (double)totW * totW / D) / (double)D << endl;
  cout << ans << endl;
