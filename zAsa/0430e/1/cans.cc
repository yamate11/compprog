...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&](ll s) -> bool {
    ll b_better = lower_bound(ALL(B), -s) - B.begin();
    ll b_worse = N - b_better;
    ll c_better = lower_bound(ALL(C), s, greater<ll>()) - C.begin();
    ll c_worse = N - c_better;
    return b_better + c_better > b_worse + c_worse;
  };
  ll s = binsearch_i<ll>(check, -(abs(B[N - 1]) + abs(C[N - 1]) + 1), abs(B[0]) + abs(C[0]) + 1);
  ll ans = 0;
  REP(i, 0, N) ans += abs(B[i] + s + 1) + abs(C[i] - (s + 1));
  cout << ans << endl;
