...
int main(/* int argc, char *argv[] */) {
  ...
  auto cvtv = [&](ll p, ll q) -> bool {
    if ((p + q) % 2 != 0) return false;
    ll r = (p + q) / 2;
    ll c = (p - q) / 2;
    return brd.at(r, c) == 'x';
  };
  ...
  REP(q, -(C - 1), R) {
    ll start = -1;
    REP(p, 0, R + C) {
      if (getV(p) == 0 and getV(p - 1) > 0) start = p;
      else if (getV(p) > 0 and getV(p - 1) == 0) {
        if ((start + q) % 2 != 0) start++;
        ll end = p - 1;
        if ((end + q) % 2 != 0) end--;
        ll td = (end - start) / 2 - (K - 2);
        if (td >= 0) {
          cnt += td;
          DLOGK(q, p, end, start, td);
        }
      }
    }
    if (q + 2*K - 1 >= R) break;
