...
int main(/* int argc, char *argv[] */) {
  ...
  while (ipR.get()) {
    ...
    while (ipC.get()) {
      auto check = [&]() -> bool {
        REP(i, 0, H) REP(j, 0, W) {
          if (A[i][j] != B[ipR.at(i)][ipC.at(j)]) return false;
        }
        return true;
      };
      auto func1 = [&](ll sz, const auto& ip) -> ll {
        ll ret = 0;
        REP(i, 0, sz) REP(j, i + 1, sz) if (ip.at(i) > ip.at(j)) ret++;
        return ret;
      };

      if (check()) {
        ll t = func1(H, ipR) + func1(W, ipC);
        ans = min(ans, t);
      }
    }
  }
