...
int naive(istream& cin, ostream& cout) {
  ...
  for (ll vlim : B) {
    ...
    REP(_q, 0, Q) {
      ...
      REP(i, 0, SIZE(C)) {
        if (i + K > SIZE(C)) break;
        auto it = min_element(C.begin() + i, C.begin() + i + K);
        if (*it < vlim) continue;
        if (*it < v0) {
          v0 = *it;
          idx = it - C.begin();
...
struct myexc : exception {};   // 試すものが無くなったら例外を投げて終了
...
int body(istream& cin, ostream& cout) {
  ...
  set<ll> removed{-1, N};  // 既に試した最小値の集合
  set<pll> viset;  // (v, i) \in viset <=> 添字 i の値 v はまだ最小値や最大値になり得る
  REP(i, 0, N) viset.emplace(A[i], i);
  auto mymax = [&](pll p1, pll p2) -> pll { // 値の大きい方．値が同じなら添字が大きい方
    auto [v1, i1] = p1;
    auto [v2, i2] = p2;
    if (v1 < v2) return p2;
    else if (v1 > v2) return p1;
    else if (i1 < i2) return p2;
    else return p1;
  };

  // st1: 区間 [a, b) でまだ有効な値のうち最も大きいものを見つけるためのセグメント木
  vector<pll> tmp; REP(i, 0, N) tmp.emplace_back(A[i], i);
  auto st1 = make_seg_tree<pll>(pll(LLONG_MIN, -1), mymax, tmp);
  // st2: 区間 [a, b) でまだ有効な値がいくつかるか数えるためのセグメント木
  auto st2 = make_seg_tree<ll>(0LL, plus<ll>(), vector(N, 1LL));
  auto itLo = viset.begin();  // itLo .. 有効な値の最小値
  auto itHi = itLo;           // itHi .. 常に itHi = itLo + (Q - 1) になるようにする
  REP(i, 0, Q - 1) itHi++;
  ll ans = LLONG_MAX;

  // func(p, q) ... 半開区間 [p, q) の，上位 K-1 個の値を無効にする
  auto func = [&](ll p, ll q) -> void {
    ll t = st2.query(p, q);
    REP(j, 0, min(t, t - (q - p - (K - 1)))) {
      auto [w, k] = st1.query(p, q);
      if (pll(w, k) <= *itHi) {
        itHi++;
        if (itHi == viset.end()) throw myexc();
      }
      viset.erase({w, k});
      st1.update(k, pll(-1LL, k));
      st2.update(k, 0LL);
    }
  };
  ...
  try {
    while (true) {
      auto [v, i] = *itLo;   // 今回試す最小値
      auto itr = removed.lower_bound(i);
      ll b = *itr;
      itr--;
      ll a = *itr + 1;    // i は区間 [a, b) に入っている．
      auto [w, j] = *itHi;   // 最大値
      ans = min(ans, w - v);
      removed.insert(i);
      st1.update(i, pll(-1LL, i));
      st2.update(i, 0LL);
      func(a, i);
      func(i + 1, b);
      itLo++;
