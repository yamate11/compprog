...
int main(/* int argc, char *argv[] */) {
  ...
  auto mymax = [&](ll x, ll y) -> ll { return max(x, y); };
  {
    ...
    REP(i, 0, N) {
      ll tv = P[i] - i;
      ll v = st1.query(P[i], N);
      if (v < big) updMin(ans[i], v - tv);
      st1.update(P[i], tv);
    }
  }
  {
    ...
    REP(i, 0, N) {
      ll tv = P[i] + i;
      ll v = st2.query(0, P[i]);
      if (v > -big) updMin(ans[i], tv - v);
      st2.update(P[i], tv);
    }
  }
  {
