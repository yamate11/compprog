...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  ll m = 2 * N + 1;
  auto pow = vector(m, Fp(0));
  pow[0] = Fp(1);
  REP(i, 1, m) pow[i] = pow[i - 1] * 10;
  auto con = vector(m, Fp(0));
  con[0] = Fp(0);
  REP(i, 1, m) con[i] = con[i - 1] * 10 + 1;

  using sta = pair<Fp, ll>;
  using OP = optional<ll>;
  auto myadd = [&](sta x1, sta x2) -> sta {
    auto [r1, l1] = x1;
    auto [r2, l2] = x2;
    return sta(r1 * pow[l2] + r2, l1 + l2);
  };
  auto mycomp = [](OP f, OP g) -> OP { return f ? f : g; };
  auto myapply = [&](OP f, sta x) -> sta {
    auto [r, l] = x;
    if (f) return sta(con[l] * Fp(*f), l);
    else return x;
  };
  auto init_vec = vector(N, sta(Fp(1), 1));
  auto st = make_seg_tree_lazy<sta, OP>(sta(0, 0), OP(), myadd, mycomp, myapply, init_vec);
