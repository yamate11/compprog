...
int main(/* int argc, char *argv[] */) {
  ...
  vector<string> S(H);
  REP(i, 0, H) cin >> S[i];
  vector tf(H, vector(W, Fp(0)));
  vector tg(H, vector(W, Fp(0)));

  auto dg = [&](ll i1, ll j1, ll i2, ll j2) -> Fp {
    Fp np = cb.binom((H - i2) + (W - j2) - 2, H - i2 - 1);
    if (S[i1][j1] == 'Y' and S[i2][j2] == 'Y') {
      return tg[i2][j2] + np;
    }else {
      return tg[i2][j2];
    }
  };
  auto df = [&](ll i1, ll j1, ll i2, ll j2) -> Fp {
    Fp np = cb.binom((H - i2) + (W - j2) - 2, H - i2 - 1);
    if (S[i1][j1] == 'Y' and S[i2][j2] == 'Y') {
      return tf[i2][j2] + 2 * tg[i2][j2] + np;
    }else {
      return tf[i2][j2];
    }
  };
  ...
  REPrev(i, H - 1, 0) REPrev(j, W - 1, 0) {
    if (i == H - 1 and j == W - 1) {
      ...
      tg[i][j] = dg(i, j, i + 1, j) + dg(i, j, i, j + 1);
      tf[i][j] = df(i, j, i + 1, j) + df(i, j, i, j + 1);
    }
  }
  cout << tf[0][0] << endl;
