...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl_init(2, vector(sz + 1, Fp(0)));
  auto tbl = tbl_init;
  ...
  for (char c : L) {
    ...
    tbl = tbl_init;
    ll d = c - '0';
    REP(eq, 0, 2) REP(k, 0, sz + 1) {
      if (prev[eq][k] == 0) continue;
      REP(x, 0, 2) {
        if (eq && x > d) continue;
        ll new_eq = eq and x == d;
        ll new_k = k + x;
        tbl[new_eq][new_k] += prev[eq][k];
      }
    }
    DLOGK(c, tbl);
  }
  Fp ans = 0;
  REP(k, 0, sz + 1) ans += power<Fp>(2, k) * (tbl[0][k] + tbl[1][k]);
  cout << ans << endl;
