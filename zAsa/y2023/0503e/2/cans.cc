...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W, K; cin >> H >> W >> K;
  // @InpGrid(H, W, A) [X1AnAei3]
  auto A = vector(H, vector(W, ll()));
  for (int i = 0; i < H; i++) for (int j = 0; j < W; j++) { ll v; cin >> v; A[i][j] = v; }
  // @End [X1AnAei3]

  ll coef = 10000;
  REP(i, 0, H) REP(j, 0, W) A[i][j] = coef * A[i][j] + i + j;

  ll big = 1e18;
  ll ans = big;
  auto check = [&](ll th) -> bool {
    vector tbl(H, vector(W, vector(K + 1, big)));
    if (A[0][0] >= th) tbl[0][0][1] = A[0][0];
    else tbl[0][0][0] = 0;
    REP(i, 0, H) REP(j, 0, W) REP(k, 0, K + 1) {
      if (A[i][j] >= th) {
        if (i > 0) updMin(tbl[i][j][min(K, k + 1)], tbl[i - 1][j][k] + A[i][j]);
        if (j > 0) updMin(tbl[i][j][min(K, k + 1)], tbl[i][j - 1][k] + A[i][j]);
      }else {
    ...
    ll v = tbl[H - 1][W - 1][K];
    bool ret = v < big;
    if (ret) updMin(ans, v / coef);
    DLOGK(th, tbl, v, ret);
    return ret;
  };
  binsearch_i<ll>(check, 1, big);
  cout << ans << endl;
  return 0;
}
