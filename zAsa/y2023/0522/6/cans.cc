...
int main(/* int argc, char *argv[] */) {
  ...
  vector acc(N + 2, vector<optional<Fp>>(2 * N + 1));
  auto g = [&](auto rF, auto rG, ll n, ll a) -> Fp {
    if (n < 0) return Fp(0);
    optional<Fp>& ret = acc[n][N + a];
    if (not ret) {
      if (n == 0) ret = Fp(0);
      else {
        ret = rG(rF, rG, n - 1, a) + rF(rF, n, a);
      }
    }
    return *ret;
  };
  vector tbl(N + 1, vector<optional<Fp>>(2 * N + 1));
  auto f = [&](auto rF, ll n, ll a) -> Fp {
    optional<Fp>& ret = tbl[n][N + a];
    if (not ret) {
      ret = (g(rF, g, n -    1 + 1, -(a - 2)) - g(rF, g, n -    9, -(a - 2)))
        +   (g(rF, g, n -   10 + 1, -(a - 3)) - g(rF, g, n -   99, -(a - 3)))
        +   (g(rF, g, n -  100 + 1, -(a - 4)) - g(rF, g, n -  999, -(a - 4)))
        +   (g(rF, g, n - 1000 + 1, -(a - 5)) - g(rF, g, n - 9999, -(a - 5)));
    }
    return *ret;
  };
