...
int main(/* int argc, char *argv[] */) {
  ...
  vector<string> S(H);
  REP(i, 0, H) cin >> S[i];
  vector sRow(H, vector(W + 1, Fp(0)));
  vector sCol(H + 1, vector(W, Fp(0)));
  vector sDiag(H + 1, vector(W + 1, Fp(0)));
  REP(i, 0, H) REP(j, 0, W) {
    if (S[i][j] == '#') {
      sRow[i][j + 1] = 0;
      sCol[i + 1][j] = 0;
      ...
      Fp t;
      if (i == 0 and j == 0) t = 1;
      else t = sRow[i][j] + sCol[i][j] + sDiag[i][j];
      if (i == H - 1 and j == W - 1) {
        cout << t << endl;
      }
      sRow[i][j + 1] = sRow[i][j] + t;
      sCol[i + 1][j] = sCol[i][j] + t;
      sDiag[i + 1][j + 1] = sDiag[i][j] + t;
    }
  }
