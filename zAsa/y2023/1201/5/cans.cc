...
int main(/* int argc, char *argv[] */) {
  ...
  Board<char> brd(H, W, '#');
  cin >> brd;
  Board<Fp> brdH(H, W, Fp(0));
  Board<Fp> brdV(H, W, Fp(0));
  Board<Fp> brdD(H, W, Fp(0));
  Board<Fp> val(H, W, Fp(0));
  ...
  REP(r, 0, H) REP(c, 0, W) {
    ...
    if (brd.at(r, c) == '.') {
      if (r > 0) val.at(r, c) += brdV.at(r - 1, c);
      if (c > 0) val.at(r, c) += brdH.at(r, c - 1);
      if (r > 0 and c > 0) val.at(r, c) += brdD.at(r - 1, c - 1);
      brdV.at(r, c) = brdV.at(r - 1, c) + val.at(r, c);
      brdH.at(r, c) = brdH.at(r, c - 1) + val.at(r, c);
      brdD.at(r, c) = brdD.at(r - 1, c -1) + val.at(r, c);
    }
  }
  DLOGK(val);
  cout << val.at(H - 1, W - 1) << endl;
