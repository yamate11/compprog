...
int naive(istream& cin, ostream& cout) {
  ll N; cin >> N;
  vector C(2, vector(2, (char)0));
  REP(i, 0, 2) REP(j, 0, 2) cin >> C[i][j];
  ...
  REP(k, 2, N) {
    ...
    for (const auto& s : prev) {
      REP(i, 0, SIZE(s) - 1) {
        char c = C[s[i] - 'A'][s[i + 1] - 'A'];
        string t = s.substr(0, i + 1);
        t.push_back(c);
        for (char cc : s.substr(i + 1)) t.push_back(cc);
        rec.insert(t);
      }
    }
  }
...
int body(istream& cin, ostream& cout) {
  ...
  auto any = [&](ll N) -> Fp { return power<Fp>(Fp(2), N - 3); };
  ...
  auto solve = [&]() -> Fp {
    ...
    if (C[0][1] == 'A') {
      if (C[0][0] == 'A') return 1;
      else if (C[1][0] == 'A') return single(N);
      else return any(N);
    }else if (C[0][1] == 'B') {
      if (C[1][1] == 'B') return 1;
      else if (C[1][0] == 'B') return single(N);
      else return any(N);
    }else assert(0);
  };
  cout << solve() << endl;
