...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    UnionFind uf(N);
    REP(i, 0, N) uf.merge(i, P[i]);
    ll a = 0, b = 0;
    ll seen = 0;
    while (true) {
      while (b < N and uf.leader(b) == uf.leader(0)) b++;
      if (b == N) break;
      for (; true; a++) {
        if (uf.leader(a) == uf.leader(0)) {
          seen++;
          ll gs = uf.groupSize(0);
          if (P[a] > b or seen == gs) break;
        }
      }
      ll c = revP[b];
      ll d = P[a];
