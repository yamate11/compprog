  ...
  while (ip.get()) {
    auto func = [&]() -> bool {
      ...
      while (ic.get()) {
        ...
        ll j = 0;
        REP(i, 0, 2*N) {
          if (j < N and ic.at(j) == i) {
            v1.push_back(ip.at(i) + 1);
            j++;
          }else {
            v2.push_back(ip.at(i) + 1);
          }
        }
        ll v = 0;
        REP(i, 0, N) v += v1[i] * v2[i];
        assert(v <= vm);
        // DLOGK(ip.vec_view(), ic.vec_view(), v, v1, v2);
        if (v == vm) {
          DLOGK(ip.vec_view());
          return true;
        }
...
int body(istream& cin, ostream& cout) {
  ...
  Fp pow2 = 1;
  REP(i, 0, N) pow2 *= 2;
  Fp ans = pow2 * cb.fact(N) * cb.binom(2*N, N) / (N + 1);
  cout << ans << endl;
