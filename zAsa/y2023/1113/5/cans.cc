...
int main(/* int argc, char *argv[] */) {
  ...
  auto violate = [&](ll x, ll i, ll j, ll k) -> bool {
    if (i == 0 and j == 1 and k == 2) return true;
    else if (i == 0 and j == 2 and k == 1) return true;
    else if (i == 1 and j == 0 and k == 2) return true;
    else if (x == 0 and j == 1 and k == 2) return true;
    else if (x == 0 and i == 1 and k == 2) return true;
    else return false;
  };

  ll N; cin >> N;
  auto tbl_init = vector(4, vector(4, vector(4, Fp(0))));
  auto tbl = tbl_init;
  REP(i, 0, 4) REP(j, 0, 4) REP(k, 0, 4) {
    if (not violate(3, i, j, k)) tbl[i][j][k] = Fp(1);
  }
  REP(ii, 3, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(i, 0, 4) REP(j, 0, 4) REP(k, 0, 4) REP(x, 0, 4) {
      if (not violate(i, j, k, x)) tbl[j][k][x] += prev[i][j][k];
    }
  }
  Fp ans = Fp(0);
  REP(i, 0, 4) REP(j, 0, 4) REP(k, 0, 4) ans += tbl[i][j][k];
  cout << ans << endl;
