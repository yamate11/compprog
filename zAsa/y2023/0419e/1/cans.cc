...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, S, type=Point) [CO1D3ylN]
  auto S = vector(N, Point());
  for (int i = 0; i < N; i++) { Point v; cin >> v; S[i] = v; }
  // @End [CO1D3ylN]
  // @InpVec(N, T, type=Point) [p83pTQPb]
  auto T = vector(N, Point());
  for (int i = 0; i < N; i++) { Point v; cin >> v; T[i] = v; }
  // @End [p83pTQPb]

  auto f = [&](auto& vec) -> void {
    Point G(0, 0);
    REP(i, 0, N) G += vec[i];
    G.x /= N; G.y /= N;
    REP(i, 0, N) vec[i] -= G;
  ...
  f(S);
  f(T);
  ll i0 = EXFIND(i, 0, N, S[i].len() > 0.1);
  REP(j, 0, N) if (abs(S[i0].len() - T[j].len()) < eps) {
    Real th = S[i0].angle(T[j]);
    if (FORALL(k, 0, N, EXISTS(l, 0, N, abs((S[k].rotate(th) - T[l]).len()) < eps))) {
      cout << "Yes\n";
      return 0;
    }
  }
