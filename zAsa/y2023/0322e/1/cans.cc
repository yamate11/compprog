...
int main(/* int argc, char *argv[] */) {
  ...
  vector<pll> XD(N);
  REP(i, 0, N) XD[i] = pll(X[i], D[i]);
  sort(ALL(XD));

  auto mymax = [&](ll x, ll y) -> ll { return max(x, y); };
  auto st = make_seg_tree<ll>(0, mymax, vector<ll>(N));
  REPrev(i, N-1, 0) {
    auto [x, d] = XD[i];
    ll p = lower_bound(ALL(XD), pll{x + d, 0}) - XD.begin();
    ll q = (p == i + 1) ? i + 1 : st.query(i + 1, p);
    st.update(i, q);
  }
  auto f = [&](auto rF, ll bg, ll en) -> Fp {
    if (bg == en) return Fp(1);
    Fp ret = 1;
    ll p = bg;
    while (true) {
      ll q = st[p];
