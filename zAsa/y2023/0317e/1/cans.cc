...
// @@ !! LIM(geometry binsearch UnionFind debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, P, type=Point) [pL2NpTHr]
  ...
  auto check = [&](double t) -> bool {
    ...
    UnionFind uf(N + 2);
    REP(i, 0, N) {
      if (ghi - P[i].y < 2*t) uf.merge(i, hi);
      if (P[i].y - glo < 2*t) uf.merge(i, lo);
      REP(j, i + 1, N) if ((P[i] - P[j]).len() < 2*t) uf.merge(i, j);
    }
    return uf.leader(hi) != uf.leader(lo);
  };
  double ans = binsearch_r<double>(check, 0, 101, 1e-5, true);
  cout << ans << endl;
