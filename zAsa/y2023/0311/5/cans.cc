...
int main(/* int argc, char *argv[] */) {
  ...
  ll szx = ccx.size();
  ll szy = ccy.size();
  vector A(szx, vector(szy, 0LL));
  REP(i, 0, N) A[ccx.c(X[i])][ccy.c(Y[i])] = 1;
  vector S(szx + 1, vector(szy + 1, 0LL));
  REP(i, 0, szx) REP(j, 0, szy) S[i + 1][j + 1] = A[i][j];
  REP(j, 1, szy + 1) REP(i, 1, szx + 1) S[i][j] += S[i-1][j];
  REP(i, 1, szx + 1) REP(j, 1, szy + 1) S[i][j] += S[i][j-1];
  DLOGK(A);
  DLOGK(S);
  ll ans = LLONG_MAX;
  REP(i0, 0, szx) REP(j0, 0, szy) REP(i1, i0 + 1, szx + 1) REP(j1, j0 + 1, szy + 1) {
    ll n = S[i1][j1] + S[i0][j0] - (S[i0][j1] + S[i1][j0]);
    if (n < K) continue;
    ll a = (ccx.d(i1 - 1) - ccx.d(i0)) * (ccy.d(j1 - 1) - ccy.d(j0));
    ans = min(ans, a);
  }
  cout << ans << endl;
