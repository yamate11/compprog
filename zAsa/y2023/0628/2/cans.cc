...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, K; cin >> N >> K;
  // @InpMVec(K, ((vc, type=char), (vk, dec=1))) [PIkvTwM9]
  auto vc = vector(K, char());
  auto vk = vector(K, ll());
  for (int i = 0; i < K; i++) {
    char v1; cin >> v1; vc[i] = v1;
    ll v2; cin >> v2; v2 -= 1; vk[i] = v2;
  }
  // @End [PIkvTwM9]
  vector tbl(K, vector(N, true));
  REP(i, 0, K) {
    REP(j, 0, K) if (j != i) tbl[j][vk[i]] = false;
    if (vc[i] == 'L') REP(j, 0, vk[i]) tbl[i][j] = false;
    else              REP(j, vk[i] + 1, N) tbl[i][j] = false;
  }
  Fp ans = 1;
  REP(i, 0, N) {
    ll cnt = 0;
    REP(j, 0, K) if (tbl[j][i]) cnt++;
    ans *= Fp(cnt);
  }
  cout << ans << endl;
  return 0;
