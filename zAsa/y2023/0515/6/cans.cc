...
int main(/* int argc, char *argv[] */) {
  ...
  auto myadd = [&](ll x, ll y) -> ll { return min(x, y); };
  auto mycomp = [&](ll f, ll g) -> ll { return f + g; };
  auto myapply = [&](ll f, ll x) -> ll { return f + x; };
  vector init_vec(N + 1, 0LL);
  REP(i, 0, N) init_vec[i + 1] = init_vec[i] + (S[i] == '(' ? 1 : -1);
  auto st = make_seg_tree_lazy<ll, ll>(LLONG_MAX, 0LL, myadd, mycomp, myapply, init_vec);
  REP(_q, 0, Q) {
    ll tp; ll l; ll r; cin >> tp >> l >> r; l--;
    DLOGK(tp, l, r);
    if (tp == 1) {
