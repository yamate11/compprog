...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((A, dec=1), (B, dec=1))) [L2GgqLGH]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  auto info = vector(2*N, vector(2*N, false));
  REP(i, 0, M) info[A[i]][B[i]] = true;

  Comb<Fp> cb(N);

  auto tbl = vector(2*N, vector<optional<Fp>>(2*N));
  auto func = [&](auto rF, ll a, ll b) -> Fp {
    if (a > b) return Fp(1);
    if (not tbl[a][b]) {
      ...
      Fp val = 0;
      if (info[a][b]) val += rF(rF, a + 1, b - 1);
      for (ll c = a + 1; c < b; c += 2) {
        ll p = (c + 1 - a) / 2;
        ll q = (b - c) / 2;
        Fp bb = cb.binom(p + q, p);
        Fp vv = rF(rF, a, c) * rF(rF, c + 1, b) * bb;
        DLOGK(bb, vv);
        val += vv;
      }
      tbl[a][b] = val;
