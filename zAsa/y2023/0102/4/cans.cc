...
int main(/* int argc, char *argv[] */) {
  ...
  for (auto [a, b] : mp) {
    aa.push_back(a);
    bb.push_back(b);
    pb.push_back(power<Fp>(2, b));
  }
  ll sz = SIZE(aa);
  DLOGK(aa, bb, pb, sz);
  vector<ll> sb(sz + 1);
  REP(i, 0, sz) sb[i + 1] = sb[i] + bb[i];
  DLOGK(sb);
  Fp c = 0;
  REP(j, 1, sz) {
    c += Fp(aa[j]) * (pb[j] - 1) * power<Fp>(2, sb[j] - sb[1]);
  }
  DLOGKL("init_c", c);
  Fp ans = 0;
  REP(i, 0, sz) ans += aa[i] * aa[i] * (pb[i] - 1);
  DLOGKL("first ans", ans);
  REP(i, 0, sz - 1) {
    Fp x = aa[i] * (pb[i] - 1);
    if (i > 0) {
      DLOGK(i, c, aa[i + 1], pb[i + 1]);
      c = (c - aa[i] * (pb[i] - 1)) / pb[i];
      DLOGK(i, c);
    }
    DLOGK(i, x, c, x*c);
    ans += x * c;
