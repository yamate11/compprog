...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpNbrList(N, N - 1, nbr, dec=1, read=(C)) [n89XMhNl]
  struct nbr_t {
    int nd;
    ll C;
    nbr_t() {}
  ...
  vector<ll> vec(N);
  ll seq = 0;
  auto dfs1 = [&](auto rF, ll nd, ll pt, ll dist) -> void {
    in[nd] = seq;
    vec[nd] = dist + D[nd];
    for (auto [c, d] : nbr[nd]) {
      if (c == pt) continue;
  ...
  auto myapply = [](ll h, ll x) -> ll { return h + x; };
  vector<ll> v_init(N);
  REP(i, 0, N) v_init[in[i]] = vec[i];
  auto st = make_seg_tree_lazy<ll, ll>(0LL, 0LL, mymax, plus<ll>(), myapply, v_init);
  vector<ll> ans(N);
  auto dfs2 = [&](auto rF, ll nd, ll pt) -> void {
    DLOGK(nd, st);
    st.update(in[nd], in[nd] + 1, -D[nd]);
    ans[nd] = st.query(0, N);
    st.update(in[nd], in[nd] + 1, D[nd]);
    for (auto [c, d] : nbr[nd]) {
      if (c == pt) continue;
