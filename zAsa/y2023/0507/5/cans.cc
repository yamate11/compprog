...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpVec(M, P, type=IPoint) [EvNJyhIh]
  auto P = vector(M, IPoint());
  for (int i = 0; i < M; i++) { IPoint v; cin >> v; P[i] = v; }
  // @End [EvNJyhIh]
  ...
  vector<ll> vec;
  for (auto [_x, y] : P) {
    ll i = lower_bound(ALL(vec), y) - vec.begin();
    if (i >= SIZE(vec)) vec.push_back(y);
    else vec[i] = y;
  }
  cout << SIZE(vec) << endl;
