...
int body(istream& cin, ostream& cout) {
  ...
  REPrev(l, L - 1, 0) {
    ...
    auto C = vector<ll>();
    for (ll a : A) {
      if (a >> l & 1) B.push_back(a & ((1LL << l) - 1));
      else C.push_back(a);
    }
    DLOGK(l, B, C);
    ll sz = SIZE(B);
    if (sz < K) {
      ...
      ll n = 0;
      REP(i, 0, K - sz) n += (1LL << l) - C[i];
      if (n <= M) {
        REP(i, 0, K - sz) B.push_back(0);
        M -= n;
        ans |= 1LL << l;
      }else {
        for (ll c : C) B.push_back(c);
      }
    }else {
      ans |= 1LL << l;
    }
