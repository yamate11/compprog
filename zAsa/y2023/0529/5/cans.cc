...
int main(/* int argc, char *argv[] */) {
  ...
  REP(x, 0, sz) {
    ...
    if (FORALL(i, 0, L, cnt[rep[i]][i] + 1 <= N)) {
      REP(i, 0, L) cnt[rep[i]][i]++;
      vec.push_back(move(rep));
    }
  }
  DLOGK(vec);
  assert(SIZE(vec) == 3*N);
  REP(i, 0, 3*N) vec[i][0] = (vec[i][0] + 2) % 3;
  REP(i, 0, 3*N) {
    REP(j, 0, L) cout << vec[i][j];
    cout << "\n";
  }
