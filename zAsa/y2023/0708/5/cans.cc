...
int main(/* int argc, char *argv[] */) {
  ...
  BrdIdx start, goal;
  REP(r, 0, H) REP(c, 0, W) {
    if (brd.at(r, c) == 's') start = BrdIdx(r, c);
    if (brd.at(r, c) == 'g') goal  = BrdIdx(r, c);
  }
  ...
  while (not deq.empty()) {
    ...
    if (dist.at(bi) < d) continue;
    for (auto dir : BrdIdx::nbr4) {
      auto bnew = bi + dir;
      if (not brd.in(bnew)) continue;
      ll dd = brd.at(bnew) == '#' ? 1 : 0;
      ll dnew = d + dd;
      if (dnew < dist.at(bnew)) {
        dist.at(bnew) = dnew;
        if (dd == 0) deq.emplace_front(dnew, bnew);
        else         deq.emplace_back(dnew, bnew);
      }
    }
  }
  DLOGK(dist, goal, dist.at(goal));
  if (dist.at(goal) <= 2) cout << "YES\n";
  else cout << "NO\n";
