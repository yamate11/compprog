...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  ll pn = 1LL << N;
  // @InpVec(pn - 1, C, base=1) [enDLWKo5]
  auto C = vector(pn - 1 + 1, ll());
  for (int i = 0; i < pn - 1; i++) { ll v; cin >> v; C[i + 1] = v; }
  // @End [enDLWKo5]
  ll big = 1e18;
  C[0] = big;
  vector<ll> ord(pn);
  ...
  ll ans = 0;
  for (ll x : ord) {
    vector<ll> newb = basis;
    newb.push_back(x);
    auto v = getF2Basis<0>(newb);
    if (SIZE(v) > SIZE(basis)) {
      basis = move(v);
      ans += C[x];
      if (SIZE(v) == N) break;
    }
