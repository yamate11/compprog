...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M, Q; cin >> N >> M >> Q;
  // @InpMVec(Q, (T, (X, dec=1), Y)) [RlVuf9BR]
  auto T = vector(Q, ll());
  auto X = vector(Q, ll());
  auto Y = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  CoordCompr cc;
  cc.add(0);
  REP(i, 0, Q) cc.add(Y[i]);
  ll sz = cc.size();
  auto stAcnt = make_seg_tree<ll>(0LL, plus<ll>(), vector<ll>(sz));
  auto stAsum = make_seg_tree<ll>(0LL, plus<ll>(), vector<ll>(sz));
  auto stBcnt = make_seg_tree<ll>(0LL, plus<ll>(), vector<ll>(sz));
  auto stBsum = make_seg_tree<ll>(0LL, plus<ll>(), vector<ll>(sz));
  stAcnt.update(cc.c(0), N);
  stBcnt.update(cc.c(0), M);
  ll cur = 0;
  REP(q, 0, Q) {
    auto func = [&](auto& vec, auto& stTcnt, auto& stTsum, auto& stOcnt, auto& stOsum) -> void {
      ll old_dy = cc.c(vec[X[q]]);
      ll old_upper = stOsum.query(old_dy, sz);
      ll old_lower = stOcnt.query(0, old_dy) * vec[X[q]];
      ll new_dy = cc.c(Y[q]);
      ll new_upper = stOsum.query(new_dy, sz);
      ll new_lower = stOcnt.query(0, new_dy) * Y[q];
      stTcnt.update(old_dy, stTcnt[old_dy] - 1);
      stTcnt.update(new_dy, stTcnt[new_dy] + 1);
      stTsum.update(old_dy, stTsum[old_dy] - cc.d(old_dy));
      stTsum.update(new_dy, stTsum[new_dy] + cc.d(new_dy));
      vec[X[q]] = Y[q];
      DLOGK(q, old_upper, old_lower, new_upper, new_lower);
      DLOGK(stAcnt, stAsum);
      DLOGK(stBcnt, stBsum);
      cur = cur - (old_upper + old_lower) + (new_upper + new_lower);
      cout << cur << "\n";
    };
    if (T[q] == 1) func(A, stAcnt, stAsum, stBcnt, stBsum); 
    else if (T[q] == 2) func(B, stBcnt, stBsum, stAcnt, stAsum);
    else assert(0);
  }
