...
// @@ !! LIM(forall f:intDiv power debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  if (N == 1) {
    if (T[0] == '0') cout << lim << endl;
    if (T[0] == '1') cout << 2 * lim << endl;
    return 0;
  }
  if (N == 2) {
    if (T == "01") cout << lim - 1 << endl;
    if (T == "00") cout << 0 << endl;
    if (T == "11") cout << lim << endl;
    if (T == "10") cout << lim << endl;
    return 0;
  }

  ll fidx = 0;
  for (; fidx < 3; fidx++) {
    auto correct = [&](ll j) -> bool {
      return (imod(j + fidx, 3) == 0 and T[j] == '1') or        \
        (imod(j + fidx, 3) == 1 and T[j] == '1') or             \
        (imod(j + fidx, 3) == 2 and T[j] == '0');
    };
    bool b = FORALL(j, 0, N, correct(j));
    if (b) break;
  }
  if (fidx == 3) {
    cout << 0 << endl;
