...
int main(/* int argc, char *argv[] */) {
  ...
  vector<pll> Pe, Po;
  REP(i, 0, N) {
    if (i % 2 == 0) Pe.emplace_back(P[i], i);
    else Po.emplace_back(P[i], i);
  }

  auto mymin = [&](pll a, pll b) -> pll {
    if (a.first < b.first) return a;
    else return b;
  };
  pll unit(LLONG_MAX, -1);
  auto ste = make_seg_tree<pll>(unit, mymin, Pe);
  auto sto = make_seg_tree<pll>(unit, mymin, Po);

  using sta = tuple<ll, ll, ll, ll, ll, ll>;
  auto f = [&](ll a, ll b) -> sta {
    assert(a < b);
    ll p, q, i, j;
    if (a % 2 == 0) {
      tie(p, i) = ste.query(a / 2, b / 2);
      tie(q, j) = sto.query(i / 2, b / 2);
    }else {
      tie(p, i) = sto.query(a / 2, b / 2);
      tie(q, j) = ste.query(i / 2 + 1, b / 2 + 1);
    }
    return sta(p, q, a, i, j, b);
    /*
    auto vec1 = rF(rF, a, i);
    auto vec2 = rF(rF, i + 1, j);
    auto vec3 = rF(rF, j + 1, b);
    vector<pll> vec{{p, q}};
    ll i1 = 0, i2 = 0, i3 = 0;
    ll sz1 = SIZE(vec1), sz2 = SIZE(vec2), sz3 = SIZE(vec3);
    while (not (i1 == sz1 and i2 == sz2 and i3 == sz3)) {
      if (i1 < sz1 and (i2 == sz2 or vec1[i1].first < vec2[i2].first) and (i3 == sz3 or vec1[i1].first < vec3[i3].first)) {
        vec.push_back(vec1[i1]);
        i1++;
      }
      else if (i2 < sz2 and (i3 == sz3 or vec2[i2].first < vec3[i3].first) and (i1 == sz1 or vec2[i2].first < vec1[i1].first)) {
        vec.push_back(vec2[i2]);
        i2++;
      }
      else if (i3 < sz3 and (i1 == sz1 or vec3[i3].first < vec1[i1].first) and (i2 == sz2 or vec3[i3].first < vec2[i2].first)) {
        vec.push_back(vec2[i3]);
        i3++;
      }
    }
    return vec;
  ...
  while (not pque.empty()) {
    auto [p, q, a, i, j, b] = pque.top(); pque.pop();
    cout << p << " " << q << " ";
    if (a < i) pque.push(f(a, i));
    if (i + 1 < j) pque.push(f(i + 1, j));
    if (j + 1 < b) pque.push(f(j + 1, b));
  }
  cout << endl;
