...
int main(/* int argc, char *argv[] */) {
  ...
  auto myadd = [&](ll a, ll b) -> ll { return a | b; };
  auto initA = vector(N, 0LL); REP(i, 0, N) initA[i] = 1LL << (S[i] - 'a');
  auto stApp = make_seg_tree<ll>(0LL, myadd, initA);
  auto initD = vector(N - 1, 0LL); REP(i, 0, N - 1) initD[i] = S[i] > S[i + 1] ? 1 : 0;
  auto stDec = make_seg_tree<ll>(0LL, myadd, initD);
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      char c; cin >> c;
      S[x] = c;
      stApp.update(x, 1LL << (c - 'a'));
      if (x - 1 >= 0) stDec.update(x - 1, S[x - 1] > S[x] ? 1 : 0);
      if (x < N - 1) stDec.update(x, S[x] > S[x + 1] ? 1 : 0);
    }else if (tp == 2) {
      ll l, r; cin >> l >> r; l--;
      bool judge;
      if (stDec.query(l, r - 1) != 0) judge = false;
      else {
        ull t = stApp.query(l, r);
        ull yL = stApp.query(0, l);
        ull yR = stApp.query(r, N);
        ll a = countr_zero(t);
        ll b = 63 - countl_zero(t);
        ll mask = ((1LL << b) - 1) & ~( (1LL << (a + 1)) - 1 );
        DLOGK(l, r, t, yL, yR, a, b, mask);
        if ((yL | yR) & mask) judge = false;
        else judge = true;
      }
      cout << (judge ? "Yes\n" : "No\n");
    }else assert(0);
