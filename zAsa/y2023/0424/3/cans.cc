...
int main(/* int argc, char *argv[] */) {
  ...
  const ll big = 1e18;
  vector<BrdIdx> dirs{BrdIdx{0, 1}, BrdIdx{1, 0}};
  ...
  ll ans = 0;
  while (not que.empty()) {
    const auto& bi = que.front(); que.pop();
    for (const auto& d : dirs) {
      auto bnew = bi + d;
      if (brd.at(bnew) == '#' or dist.at(bnew) < big) continue;
      dist.at(bnew) = dist.at(bi) + 1;
      ans = max(ans, dist.at(bnew));
      que.push(bnew);
    }
  }
  cout << ans + 1 << endl;
