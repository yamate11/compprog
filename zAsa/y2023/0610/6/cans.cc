...
int main(/* int argc, char *argv[] */) {
  ...
  REP(s, 0, sz) {
    dist[s][s] = 0;
    vector locDist(H * W, big);
    queue<BrdIdx> que;
    locDist[enc(P[s].r, P[s].c)] = 0;
    que.push(P[s]);
    while (not que.empty()) {
      auto bi = que.front(); que.pop();
      for (BrdIdx dir : BrdIdx::nbr4) {
        BrdIdx p = bi + dir;
        if (brd.at(p) == '#') continue;
        if (locDist[enc(p.r, p.c)] < big) continue;
        locDist[enc(p.r, p.c)] = locDist[enc(bi.r, bi.c)] + 1;
        que.push(p);
      }
    }
    REP(t, 0, sz) dist[s][t] = locDist[enc(P[t].r, P[t].c)];
  }
  vector tbl(1LL << sz, vector(sz, big));
  tbl[1LL << numCake][numCake] = 0;
  REP(x, 0, 1LL << sz) {
    REP(t, 0, sz) {
      ...
      REP(u, 0, sz) {
        if (t == u) continue;
        if (x >> u & 1) continue;
        updMin(tbl[x | (1LL << u)][u], tbl[x][t] + dist[t][u]);
      }
    }
  }
  ll ans = -1;
  REP(x, 0, 1LL << sz) {
    if (tbl[x][sz - 1] <= T) updMax(ans, ll(__builtin_popcountll(x) - 2));
  }
  cout << ans << endl;
