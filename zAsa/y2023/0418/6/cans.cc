...
Fp func(ll n) {
  if (n == 1) return Fp(1);
  if (n == 2) return Fp(3);
  vector tbl_init(2, vector(n + 1, Fp(0)));
  Fp ret(0);
  REP(b, 0, 2) {
    auto tbl = tbl_init;
    tbl[b][0] = 1;
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, P, dec=1) [HXX7j7YP]
  auto P = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; P[i] = v; }
  // @End [HXX7j7YP]
  // @InpVec(N, Q, dec=1) [DYOx2WWf]
  auto Q = vector(N, ll());
  for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; Q[i] = v; }
  // @End [DYOx2WWf]
  
  vector<pll> PQ;
  REP(i, 0, N) PQ.emplace_back(P[i], Q[i]);
  sort(ALL(PQ));
  vector<ll> A(N);
  REP(i, 0, N) A[i] = PQ[i].second;
  vector<bool> B(N);
  Fp ans = 1;
  REP(i, 0, N) {
    if (not B[i]) {
      ll c = 0;
      for (ll j = i; not B[j]; j = A[j]) {
        B[j] = true;
        c++;
      }
      ans *= func(c);
