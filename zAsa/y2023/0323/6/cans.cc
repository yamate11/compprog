...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = 0;
  REP(a, 0, B + 1) REP(b, 0, R + 1) {
    if (a * x + b <= R and a + b * y <= B) ans = max(ans, a + b);
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  auto check = [&](ll k) -> bool {
    ll p = divFloor(R - k, x - 1);
    ll q = k + divCeil(k - B, y - 1);
    return max(0LL, q) <= min(p, k);
  };
  ll ans = binsearch_i<ll>(check, 0, B + R + 1);
  cout << ans << endl;
  return 0;
}
