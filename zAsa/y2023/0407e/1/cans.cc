...
int main(/* int argc, char *argv[] */) {
  ...
  auto check = [&](ll k) -> bool {
    DLOGKL("check", k);
    vector<pll> v;
    v.emplace_back(0, A[0]);
    DLOGK(v);
    REP(i, 1, N) {
      if (A[i - 1] < A[i]) v.emplace_back(0, A[i] - A[i - 1]);
      else {
        ll red = A[i - 1] - A[i];
        while (red > 0) {
          auto [c, t] = v.back(); v.pop_back();
          if (red >= t) red -= t;
          else {
            v.emplace_back(c, t - red);
            break;
          }
        }
        DLOGKL("adjust", v);
        ...
        while (true) {
          if (v.empty()) {
            DLOGKL("failed check", k);
            return false;
          }
          auto [c, t] = v.back(); v.pop_back();
          if (c + 1 < k) {
            if (t > 1) v.emplace_back(c, t - 1);
            v.emplace_back(c + 1, 1);
            if (m > 0) v.emplace_back(0, m);
            break;
          }else {
            m += t;
          }
  ...
  ll ans = binsearch_i<ll>(check, N, 0);
  cout << ans << endl;
