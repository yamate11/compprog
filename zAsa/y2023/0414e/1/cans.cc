...
int main(/* int argc, char *argv[] */) {
  ...
  string S; cin >> S;
  ll curX = 0, curY = 0;
  map<ll, itv_set<int>> mapH, mapV;
  mapH[0].put(0, 1);
  mapV[0].put(0, 1);
  for (auto& [y, is] : mapH) DLOGKL("mapH", y, is.impl);
  for (auto& [x, is] : mapV) DLOGKL("mapV", x, is.impl);

  auto f = [&](auto& mapT, auto& mapO, ll dif, ll& cT, ll& cO) {
    auto itT = mapT.find(cO);
    assert(itT != mapT.end());
    auto& isT = itT->second;
    auto [l, r] = isT.get_itvl(cT);
    if (dif == 1) cT = r;
    else          cT = l - 1;
    isT.put(cT, 1);
    mapO[cT].put(cO, 1);
  ...
  REP(i, 0, K) {
    if      (S[i] == 'L') f(mapH, mapV, -1, curX, curY);
    else if (S[i] == 'R') f(mapH, mapV,  1, curX, curY);
    else if (S[i] == 'U') f(mapV, mapH,  1, curY, curX);
    else if (S[i] == 'D') f(mapV, mapH, -1, curY, curX);
    else assert(0);
    DLOGK(i);
#if DEBUG
    for (auto& [y, is] : mapH) DLOGKL("mapH", y, is.impl);
