...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  string S; cin >> S;
  vector tbl_init(10, vector(1LL << 10, Fp(0)));
  auto tbl = tbl_init;
  REP(i, 0, N) {
    ll d = S[i] - 'A';
    DLOGK(i, S[i], d);
    ...
    REP(x, 0, 1LL << 10) {
      REP(dd, 0, 10) {
        if (dd == d) tbl[d][x] += 2 * prev[d][x];
        else {
          tbl[dd][x] += prev[dd][x];
          if (not (x >> d & 1)) {
            tbl[d][x | (1LL << d)] += prev[dd][x];
          }
        }
      }
    }
    ...
    REP(x, 0, 1LL << 10) {
      REP(e, 0, 10) {
        if (tbl[e][x] != Fp(0)) {
          string s;
          REP(m, 0, 10) if (x >> m & 1) s += (char)('A' + m);
          char c = (char)('A' + e);
          DLOGK(c, s, tbl[e][x]);
        }
      }
  ...
  Fp ans = 0;
  REP(d, 0, 10) REP(x, 0, 1LL << 10) ans += tbl[d][x];
  cout << ans << endl;
  return 0;
}
