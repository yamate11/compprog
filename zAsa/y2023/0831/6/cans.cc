...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, Q; cin >> N >> Q;
  string S; cin >> S;
  auto init_vec = vector(N + 1, 0LL);
  REP(i, 0, N) init_vec[i + 1] = init_vec[i] + (S[i] == '(' ? 1 : -1);
  auto mymin = [](ll a, ll b) -> ll { return min(a, b); };
  auto st = make_seg_tree_lazy<ll, ll>(LLONG_MAX, 0LL, mymin, plus<ll>(), plus<ll>(), init_vec);
  DLOGK(init_vec);
  REP(_q, 0, Q) {
    ll tp, l, r; cin >> tp >> l >> r; l--; r--;
    if (tp == 1) {
      if (S[l] == '(' and S[r] == ')') st.update(l + 1, r + 1, -2);
      else if (S[l] == ')' and S[r] == '(') st.update(l + 1, r + 1, +2);
      swap(S[l], S[r]);
      DLOGK(_q, st);
    }else {
      ll start = st.query(l, l + 1);
      ll x = st.query(l, r + 1);
      ll y = st.query(r + 1, r + 2);
      bool b = x == start and y == start;
      DLOGK(l, r, start, x, y);
      cout << (b ? "Yes\n" : "No\n");
    }
  }
