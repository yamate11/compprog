...
int main(/* int argc, char *argv[] */) {
  ...
  REPrev(i, N - 1, 0) {
    if (i == N - 1) {
      ...
      if (AB[i].first == AB[i + 1].first) limA[i] = limA[i + 1];
      else limA[i] = i;
    }
  }
  vector<ll> ordB(N);
  REP(i, 0, N) ordB[i] = i;
  sort(ALL(ordB), [&](ll i, ll j) -> bool { return AB[i].second > AB[j].second; });
  vector<ll> limB(N);
  REPrev(i, N - 1, 0) {
    if (i == N - 1) {
      limB[i] = i;
    }else {
      if (AB[ordB[i]].second == AB[ordB[i + 1]].second) {
        limB[i] = limB[i + 1];
        limB[i + 1] = -1;
      }
      else limB[i] = i;
  ...
  auto st = make_seg_tree<ll>(0LL, plus<ll>(), vector<ll>(N, 0LL));
  ll ans = 0;
  REP(i, 0, N) {
    if (limB[i] >= 0) {
      REP(i0, i, limB[i] + 1) st.update(ordB[i0], 1);
    }
    ll j = ordB[i];
    ans += st.query(0, limA[j] + 1);
  }
  cout << ans << endl;
