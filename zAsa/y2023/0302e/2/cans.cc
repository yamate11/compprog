...
// @@ !! LIM(tree binsearch forall debug)
...
template<typename T, typename U = vector<T>, typename C = less<T>>
struct mypque {
  ll size;
  priority_queue<T, U, C> pque;
  multiset<T> ms;
  mypque() : size(0), pque(), ms() {}
  void push(const T& t) {
    pque.push(t);
    size++;
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  mypque<ll> pqLo;
  mypque<ll, vector<ll>, greater<ll>> pqHi;
  auto func = [&](auto rF, ll nd, bool forMax) -> ll {
    DLOGK(nd, forMax);
    DLOGK(pqLo.pque, pqLo.ms);
    DLOGK(pqHi.pque, pqHi.ms);
    ll v = A[nd];
    ll push_type = 0;
    if (pqLo.size == 0) {
      assert(pqHi.size == 0);
    ...
    ll rval;
    if (tr.children(nd).empty()) {
      if (pqLo.size == pqHi.size) rval = (pqLo.top() + pqHi.top()) / 2;
      else rval = pqLo.top();
    }else {
      bool first = true;
      for (ll cld : tr.children(nd)) {
        ll cv = rF(rF, cld, not forMax);
        if (first) {
          rval = cv;
          first = false;
        }else if (forMax) rval = max(rval, cv);
        else              rval = min(rval, cv);
      }
    }
    if (push_type == 0) {
      pqLo.erase(v);
