...
int main(/* int argc, char *argv[] */) {
  ...
  auto st0 = make_seg_tree_lazy<Fp, Fp>(Fp(0), Fp(0), plus<Fp>(), plus<Fp>(), plus<Fp>(), vector<Fp>(N));
  auto st1 = make_seg_tree_lazy<Fp, Fp>(Fp(0), Fp(0), plus<Fp>(), plus<Fp>(), plus<Fp>(), vector<Fp>(N));
  auto st2 = make_seg_tree_lazy<Fp, Fp>(Fp(0), Fp(0), plus<Fp>(), plus<Fp>(), plus<Fp>(), vector<Fp>(N));
  auto upd = [&](ll i, Fp v) -> void {
    st2.update(i, N, v * 1);
    st1.update(i, N, v * (3 - 2 * i));
    st0.update(i, N, v * (i * i - 3 * i + 2));
  };
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      ll x; cin >> x; x--;
      DLOGK(tp, x);
      Fp v2 = st2.query(x, x + 1);
      Fp v1 = st1.query(x, x + 1);
      Fp v0 = st0.query(x, x + 1);
      Fp a = (v2 * x * x + v1 * x + v0) / 2;
      DLOGK(st0, st1, st2);
      DLOGK(x, v2, v1, v0, a);
      cout << a << "\n";
    }else assert(0);
