...
int main(/* int argc, char *argv[] */) {
  ...
  reverse(ALL(S));
  auto vec = vector(N, Fp(0));
  vec[0] = 1;
  REP(i, 1, N) vec[i] = vec[i - 1] * 10;
  REP(i, 0, N) vec[i] = vec[i] * (S[i] - '0');
  auto acc = vector(N + 1, Fp(0));
  REP(i, 0, N) acc[i + 1] = acc[i] + vec[i];
  auto cnt = vector(P, 0LL);
  REP(i, 0, N + 1) cnt[ll(acc[i])]++;
  ll ans = 0;
  REP(i, 0, N) {
    ans += cnt[acc[i]] - 1;
    cnt[acc[i]]--;
