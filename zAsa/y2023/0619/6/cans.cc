...
int main(/* int argc, char *argv[] */) {
  ...
  REP(_q, 0, Q) {
    ...
    ll dep = tr.depth(u);
    if (dep >= k) {
      ans = tr.ancestorDep(u, dep - k);
    }else if (u == ct0) {
      if (tr.depth(nd1) >= k) ans = tr.ancestorDep(nd1, k);
      else ans = -1;
    }else {
      // ll v0 = tr.ancestorDep(nd0, 1);
      ll v1 = tr.ancestorDep(nd1, 1);
      ll v = tr.ancestorDep(u, 1);
      if (v != v1) {
        if (tr.depth(nd1) >= k - dep) ans = tr.ancestorDep(nd1, k - dep);
        else ans = -1;
      }else {
        if (tr.depth(nd0) >= k - dep) ans = tr.ancestorDep(nd0, k - dep);
        else ans = -1;
      }
    }
    if (ans < 0) cout << -1 << "\n";
