...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) REP(j, 0, N) {
    if (brd.at(i, j) == 'P') {
      if (p1.r < 0) p1 = BrdIdx(i, j);
      else p2 = BrdIdx(i, j);
      brd.rs(i, j) = '.';
    }
  }
  assert(p1.r >= 0 and p2.r >= 0);
  ll big = 1e18;
  vector<ll> dist(N*N*N*N, big);
  auto genc = [&](BrdIdx b1, BrdIdx b2) -> ll { return brd.enc(b1) * N*N + brd.enc(b2); };
  dist[genc(p1, p2)] = 0;
  using sta = pair<BrdIdx, BrdIdx>;
  queue<sta> que;
  que.emplace(p1, p2);
  while (not que.empty()) {
    auto [q1, q2] = que.front(); que.pop();
    DLOGKL("pop", q1, q2);
    ll d = dist[genc(q1, q2)];
    if (q1 == q2) {
      cout << d << endl;
      return 0;
    }
    for (auto dir : BrdIdx::nbr4) {
      auto op = [&](BrdIdx r1, BrdIdx r2) -> void {
        ll f = dist[genc(r1, r2)];
        if (f >= big) {
          dist[genc(r1, r2)] = d + 1;
          DLOGKL("emplace", r1, r2);
          que.emplace(r1, r2);
        }
      };
      auto r1 = q1 + dir;
