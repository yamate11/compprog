...
// @@ !! LIM(board forall f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  // @InpGrid(H, W, A) [NqfWFwlU]
  auto A = vector(H, vector(W, ll()));
  for (int i = 0; i < H; i++) for (int j = 0; j < W; j++) { ll v; cin >> v; A[i][j] = v; }
  // @End [NqfWFwlU]

  Board<int> brd(H, W, -1);
  REP(i, 0, H) REP(j, 0, W) brd.rs(i, j) = A[i][j];

  vector allow(H, vector<bool>(8));
  REP(i, 0, H) {
    REP(x, 0, 8) {
      auto check = [&](ll j) -> bool {
        BrdIdx bi(i, j);
        ll p = brd.at(bi) ^ (x >> 1 & 1);
        for (auto dir : BrdIdx::nbr4) {
          auto bj = bi + dir;
          if (brd.in(bj)) {
            ll t = 1 + bj.r - i;
            if ((brd.at(bj) ^ (x >> t & 1)) == p) return true;
          }
        }
        return false;
      };
      allow[i][x] = FORALL(j, 0, W, check(j));
    }
  }
  ...
  REP(i, 0, H) {
    ...
    REP(x, 0, 4) {
      if (prev[x] < big) {
        REP(b, 0, 2) {
          ll y = (b << 2) | x;
          if (allow[i][y]) updMin(tbl[y >> 1], prev[x] + b);
        }
      }
    }
    DLOGK(i, tbl);
