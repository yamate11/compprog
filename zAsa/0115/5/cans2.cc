...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](ll A1, ll A2, ll A3) ->  pll {
    if (A3 > A1 + A2 or A3 < max(A1, A2)) return {-1, -1};
    if (A3 == A1 + A2) {
      REP(d, 0, 100) {
        REP(i, 0, d + 1) {
          ll j = d - i;
          ll a = du.nd_max(A1) - i;
          ll b = du.nd_max(A2) - j;
          if (a % 2 != 0 and a % 5 != 0 and b % 2 != 0 and b % 5 != 0 and gcd(a, b) == 1) return {a, b};
        }
      }
      assert(0);
    }
    ...
    REP(d, 0, 100) {
      REP(i, 0, d + 1) {
        ...
        ll a = du.nd_min(x1) + i;
        ll b = du.nd_min(x2) + j;
        if (a % 2 != 0 and a % 5 != 0 and b % 2 != 0 and b % 5 != 0 and gcd(a, b) == 1) {
          return {a * du.pow(x), b * du.pow(x)};
        }
      }
    }
    assert(0);
