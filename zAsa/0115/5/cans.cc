...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 1, 39) pow3[i] = pow3[i - 1] * 3;

  auto solve = [&](ll x, ll y, ll z) -> pll {
    if (x + y < z or z < max(x, y)) return {-1, -1};
    if (x + y == z) {
      ll a = *prev(ranges::lower_bound(pow2, du.nd_min(x + 1)));
      ll b = *prev(ranges::lower_bound(pow3, du.nd_min(y + 1)));
      return {a, b};
    }
    bool swapped = false;
    if (x < y) {
    ...
    ll a = *ranges::lower_bound(pow2, du.nd_min(x));
    ll b = *ranges::lower_bound(pow2, du.nd_min(y));
    ll lcm = max(a, b);
    // DLOGK(x, y, z, a, b, lcm);
    while (du.width(lcm) < z) {
      b *= 3;
      lcm *= 3;
      while (du.width(b) > y) {
        assert(b % 2 == 0);
    ...
    return swapped ? pll(b, a) : pll(a, b);
  };

#if DEBUG
  REP(i, 1, 18) REP(j, 1, 18) REP(k, 1, 18) {
    if (k <= i + j and k >= max(i, j)) {
      auto [a, b] = solve(i, j, k);
      if (a < 0 or du.width(a) != i or du.width(b) != j or du.width(lcm(a, b)) != k) {
        DLOGK(i, j, k, a, b);
        return 0;
      }
    }
