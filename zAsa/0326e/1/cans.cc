...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, P, type=Point) [1enE0jKR]
  ...
  REP(i, 0, N) REP(j, i + 1, N) {
    Point dir = (P[j] - P[i]).rotate(G_PI / 2.0);
    vec.emplace_back(dir.arg(), j, i);
    vec.emplace_back(dir.rotate(G_PI).arg(), i, j);
  }
  sort(ALL(vec));
  ll hsz = SIZE(vec) / 2;
  ll sz = SIZE(vec);
  DLOGK(vec);
  vector eff(sz, vector(N, true));
  REP(k, 0, hsz) {
    auto [a, i, j] = vec[k];
    REP(n, 0, k) eff[n][j] = false;
    REP(n, k, k + hsz) eff[n][i] = false;
    REP(n, k + hsz, sz) eff[n][j] = false;
  }
  DLOGK(eff);
  vector<Real> prob(N);
  REP(k, 0, sz) {
    ll i0 = EXFIND_D(i, 0, N, eff[k][i], -1LL);
    auto [a, _i, _j] = vec[k];
    double p;
    if (k + 1 == sz) {
      auto [b, _ii, _jj] = vec[0];
      p = 2 * G_PI + b - a;
    }else {
      auto [b, _ii, _jj] = vec[(k + 1) % sz];
      p = b - a;
    }
    if (i0 >= 0) prob[i0] += p;
  }
  REP(i, 0, N) cout << prob[i] / (2 * G_PI) << "\n";
