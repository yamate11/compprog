...
// @@ !! LIM(tree debug cmpNaive forall)
...
int naive(istream& cin, ostream& cout) {
  ...
  // @InpMVec(N - 1, ((U, dec=1), (V, dec=1), W)) [VfiZ29Xf]
  ...
  REP(x, 0, 1LL << (N - 1)) {
    ...
    if (FORALL(i, 0, N, E[i] <= D[i])) ans = max(ans, totw);
  }
  cout << ans << endl;
...
int body(istream& cin, ostream& cout) {
  ...
  auto dfs = [&](auto rF, ll n) -> pll {
    vector<ll> ds;
    ll base = 0;
    for (ll cld : tr.children(n)) {
      ll w = weight[tr.edge_idx(n, cld)];
      auto [x1, y1] = rF(rF, cld);
      base += x1;
      ll d = w + y1 - x1;
      if (d > 0) ds.push_back(d);
    }
