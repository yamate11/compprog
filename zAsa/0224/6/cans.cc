...
int main(/* int argc, char *argv[] */) {
  ...
  ll d0 = 0;
  auto f = [&](ll N, auto& tr) -> vector<ll> {
    auto [diam, nda, ndb, cta, ctb] = tr.diameter();
    d0 = max(d0, diam);
    tr.change_root(cta);
    vector<ll> ret(N);
    REP(i, 0, N) ret[i] = (diam + 1) / 2 + tr.depth(i);
    if (diam % 2 != 0) {
  ...
  for (ll x : vec1) {
    auto it = ranges::lower_bound(vec2, d0 - (x + 1));
    ll n = vec2.end() - it;
    sum1 += n * x;
    sum3 += n;
    sum2 += (N2 - n) * d0;
  ...
  for (ll x : vec2) {
    auto it = ranges::lower_bound(vec1, d0 - (x + 1));
    ll n = vec1.end() - it;
    sum1 += n * x;
    sum3 += n;
    sum2 += (N1 - n) * d0;
  }
  ll ans = sum1 + sum2 / 2 + sum3 / 2;
  cout << ans << endl;
  return 0;
}
