...
// @@ !! LIM(cmpNaive UnionFind tree debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  // @InpMVec(2*N - 1, ((A, dec=1), (B, dec=1))) [iTOtAKCI]
  auto A = vector(2*N - 1, ll());
  auto B = vector(2*N - 1, ll());
  ...
  UnionFind uf(2 * N);
  REP(i, 0, 2 * N - 1) {
    if (not (0 <= A[i] and A[i] < 2 * N and 0 <= B[i] and B[i] < 2 * N and B[i] != A[i])) {
      DLOGKL("early", i, A[i], B[i]);
      cout << "NG\n";
      return 0;
    }
    uf.merge(A[i], B[i]);
  }
  if (uf.groupSize(0) != 2 * N) {
    DLOGK(uf.groupSize(0));
    cout << "NG\n";
    return 0;
  }
  Tree tr(2*N);
  REP(i, 0, 2*N - 1) tr.add_edge(A[i], B[i]);
  REP(i, 0, N) {
    ll t = 0;
    for (ll x : tr.nnpath(i, N + i)) {
      ll w = (x >= N) ? x - N + 1 : x + 1;
      t ^= w;
    }
    if (t != i + 1) {
...
int body(istream& cin, ostream& cout) {
  ...
  ll m = (1LL << k) - 1;
  vector<pll> ans;
  REP(i, 1, m) { ans.emplace_back(i, i + 1); ans.emplace_back(N + i, N + i + 1); }
  ans.emplace_back(m, N + 1);
  ll p = m + 2;
  for (; p <= N; p += 2) {
    ans.emplace_back(p - 1, p);
    ans.emplace_back(p, 1);
    ans.emplace_back(1, N + p - 1);
    ans.emplace_back(N + p - 1, N + p);
  }
  if (p - 1 <= N) {
    ans.emplace_back(p - 1, p - 2);
    ll x = (p - 1) ^ (p - 2);
    ans.emplace_back(x - 1, N + p - 1);
  }
  for (auto [a, b] : ans) {
    cout << a << " " << b << "\n";
  }
