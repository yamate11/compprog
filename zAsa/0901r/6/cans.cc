...
// @@ !! LIM(mod debug segTree coordCompr)
...
int main(/* int argc, char *argv[] */) {
  ...
  using OP = pair<ll, Fp>;   // {-1,0,1} \times Fp
  auto myadd = [&](const DAT& a1, const DAT& a2) -> DAT {
    const auto& [b1, c1] = a1;
    const auto& [b2, c2] = a2;
    return DAT(b1 + b2, c1 + c2);
  };
  auto mycomp = [&](const OP& f1, const OP& f2) -> OP {
    const auto& [k1, x1] = f1;
    const auto& [k2, x2] = f2;
    if (k1 == 0) return f1;
    else if (k1 == 1) return OP(k2, x1 + x2);
    else if (k1 == -1) return OP(-k2, x1 - x2);
    else assert(0);
  };
  auto myappl = [&](const OP& f, const DAT& a) -> DAT {
    const auto& [k, x] = f;
    const auto& [b, c] = a;
    return DAT(k * b + x * c, c);
  };
  auto init_vec = vector<DAT>(sz);
  REP(i, 0, sz - 1) init_vec[i] = DAT(0, cc.d(i + 1) - cc.d(i));
  init_vec[sz - 1] = DAT(0, 0);
  auto st = make_seg_tree_lazy<DAT, OP>(DAT(0, 0), OP(1, 0), myadd, mycomp, myappl, init_vec);
  Fp sum = Fp(1);
  REP(i, 0, N) {
    st.update(0, sz, OP(-1, sum));
    st.update(cc.c(A[i]), sz, OP(0, 0));
    sum = st.query(0, sz).first;
  }
  cout << sum << endl;
