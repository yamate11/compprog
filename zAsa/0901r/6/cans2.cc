...
int main(/* int argc, char *argv[] */) {
  ...
  auto idx = vector<ll>{0, A[0]};
  auto val = vector<Fp>{Fp(1)};
  auto acc = vector<Fp>{Fp(0), Fp(A[0])};
  Fp base = 0;
  ...
  REP(i, 1, N) {
    ...
    if (A[i - 1] < A[i]) {
      idx.push_back(A[i]);
      val.push_back(-sign * base);
      acc.push_back(old_sum - sign * base * (A[i] - A[i - 1]));
    }else if (A[i - 1] > A[i]) {
      ll j = lower_bound(ALL(idx), A[i]) - idx.begin();
      assert(0 < j and j < SIZE(idx));
      if (A[i] < idx[j]) {
        idx[j] = A[i];
        acc[j] = acc[j - 1] + val[j - 1] * (A[i] - idx[j - 1]);
      }
      idx.resize(j + 1);
      val.resize(j);
      acc.resize(j + 1);
    }
    base = base * (A[i - 1] - 1) + sign * old_sum;
    DLOGK(i, idx, val, acc, base);
  }
  ll ts = N % 2 != 0 ? 1 : -1;
  Fp ans = 0;
  REP(i, 0, SIZE(idx) - 1) {
    ans += (base + ts * val[i]) * (idx[i + 1] - idx[i]);
  }
  cout << ans << endl;
