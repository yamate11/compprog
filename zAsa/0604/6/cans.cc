...
// @@ !! LIM(forall digit debug binsearch)
...
int main(/* int argc, char *argv[] */) {
  ...
  string s; cin >> s;
  ll i0 = EXFIND(i, 0, ssize(s), s[i] == '/');
  ll x = du.from_string(s.substr(0, i0));
  ll y = du.from_string(s.substr(i0 + 1));
  ll g = gcd(x, y);
  x /= g;
  y /= g;
  double t = (double)x / y;
  ll lo = llround(floor(2 * t - 1)) - 1;
  ll hi = llround(ceil(2 * t + 1)) + 1;
  if (lo < 1) lo = 1;
