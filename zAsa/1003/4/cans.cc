...
int naive(istream& cin, ostream& cout) {
  int N; cin >> N;
  // @InpVec(N, S, type=string) [SBGgXqRX]
  ...
  while (ip.get()) {
    string t = "";
    REP(i, 0, N) t += S[ip.at(i)];
    ll cnt = 0;
    REP(i, 0, SIZE(t) - 1) if (t[i] == 'A' and t[i + 1] == 'B') cnt++;
    ans = max(ans, cnt);
  }
  cout << ans << endl;
  return 0;
...
int body(istream& cin, ostream& cout) {
  ...
  REP(j, 0, N) {
    string s; cin >> s;
    REP(i, 0, SIZE(s) - 1) if (s[i] == 'A' and s[i + 1] == 'B') cnt++;
    if (s[0] == 'B' and s[SIZE(s) - 1] == 'A') both++;
    else if (s[SIZE(s) - 1] == 'A') enda++;
    else if (s[0] == 'B') startb++;
  }
  DLOGK(cnt, both, enda, startb);
  if (both >= 1) {
    cnt += both - 1;
    if (enda > 0) {
      enda--;
