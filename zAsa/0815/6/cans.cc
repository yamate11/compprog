...
// @@ !! LIM(ipoint unordered_map f:updMaxMin cmpNaive debug)
...
int body(istream& cin, ostream& cout) {
  ...
  REP(x, 1, 1LL << H) {
    ...
    ll v = Mb * sa - Ma * sb;
    auto [it, b] = mp.try_emplace(v, sc);
    if (not b and it->second > sc) it->second = sc;
    if (v == 0) updMin(ans, sc);
  }
  DLOGK(mp);
  ll H2 = N - H;
  REP(x, 1, 1LL << H2) {
    ...
    ll v = Ma * tb - Mb * ta;
    if (v == 0) updMin(ans, tc);
    auto it = mp.find(v);
    if (it != mp.end()) {
      updMin(ans, it->second + tc);
    }
  }
  if (ans == big) ans = -1;
  cout << ans << endl;
