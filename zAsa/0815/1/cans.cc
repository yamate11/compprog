...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpVec(N, D, type=IPoint) [Vnz3QaDw]
  auto D = vector(N, IPoint());
  for (int i = 0; i < N; i++) { IPoint v; cin >> v; D[i] = v; }
  // @End [Vnz3QaDw]

  bool b = EXISTS(i, 0, N - 2, D[i].x == D[i].y and D[i + 1].x == D[i + 1].y and D[i + 2].x == D[i + 2].y);
  cout << (b ? "Yes" : "No") << endl;
