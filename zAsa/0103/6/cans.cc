...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N) rev[A[i]].push_back(i);
  
  vector acc(N, vector(M + 1, Fp(0)));
  auto g = [&](auto rF, ll nd) -> void {
    REP(m, 1, M + 1) acc[nd][m] = Fp(1);
    for (ll n : rev[nd]) {
      rF(rF, n);
      REP(m, 1, M + 1) acc[nd][m] *= acc[n][m];
    }
    REP(m, 2, M + 1) acc[nd][m] += acc[nd][m - 1];
  };

  auto f = [&](ll i0) -> Fp {
    ll nd = fg.node_on_cycle(i0);
    vector<ll> vec;
    REP(j, 0, fg.cycle_size(i0)) {
      for (ll n : rev[nd]) {
        if (not fg.is_on_cycle(n)) {
          vec.push_back(n);
          g(g, n);
        }
      }
