...
int main(/* int argc, char *argv[] */) {
  ...
  cin >> brd;
  BrdIdx start, goal;
  vector alph(na, vector<BrdIdx>());
  REP(i, 0, H) REP(j, 0, W) {
    BrdIdx bi(i, j);
    char c = brd.at(bi);
    if (c == 'S') start = bi;
    else if (c == 'G') goal = bi;
    else if ('a' <= c and c <= 'z') alph[c - 'a'].push_back(bi);
  }
  ll big = 1e18;
  vector dist(H + 1, vector(max(W, na), big));
  using sta = pair<ll, BrdIdx>;
  deque<sta> deq;
  dist[start.r][start.c] = 0;
  deq.emplace_back(0, start);
  while (not deq.empty()) {
    auto [d, bi] = deq.front(); deq.pop_front();
    if (dist[bi.r][bi.c] == d) {
      if (bi.r == H) {
        for (const auto& bj : alph[bi.c]) {
          ll new_dist = d + 1;
          if (new_dist < dist[bj.r][bj.c]) {
            dist[bj.r][bj.c] = new_dist;
            deq.emplace_back(new_dist, bj);
          }
        }
      }else {
        for (const auto& dir : BrdIdx::nbr4) {
          auto bj = bi + dir;
          ll new_dist = d + 1;
          if (brd.at(bj) != '#' and new_dist < dist[bj.r][bj.c]) {
            dist[bj.r][bj.c] = new_dist;
            deq.emplace_back(new_dist, bj);
          }
        }
        ...
        if ('a' <= c and c <= 'z') {
          ll new_dist = d;
          if (new_dist < dist[H][c - 'a']) {
            dist[H][c - 'a'] = new_dist;
            deq.emplace_front(new_dist, BrdIdx(H, c - 'a'));
          }
        }
      }
    }
  }
  ll ans = dist[goal.r][goal.c];
  if (ans == big) ans = -1;
  cout << ans << endl;
