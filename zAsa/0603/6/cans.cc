...
int main(/* int argc, char *argv[] */) {
  ...
  auto st_ord = make_seg_tree(0LL, plus<ll>(), vector<ll>(N));
  using st_t = decltype(make_seg_tree(0LL, plus<ll>()));
  vector<st_t> st_char(26);
  REP(i, 0, 26) {
    st_char[i] = make_seg_tree(0LL, plus<ll>(), vector<ll>(N));
  }
  vector cnt(26, 0LL);
  REP(i, 0, N) {
    int d = S[i] - 'a';
  ...
  REP(_q, 0, Q) {
    ...
    if (tp == 1) {
      ...
      st_char[d_old].rs(x) = 0;
      st_char[d_new].rs(x) = 1;

      if (x - 1 >= 0) st_ord.rs(x - 1) = S[x - 1] > S[x    ] ? 1 : 0;
      if (x + 1 <  N) st_ord.rs(x    ) = S[x    ] > S[x + 1] ? 1 : 0;
      ...
      auto check = [&]() -> bool {
        ...
        if (st_ord.query(l, r - 1) > 0) {
          return false;
        }
        ll d_lo = S[l] - 'a';
        ll d_hi = S[r - 1] - 'a';
        if (d_lo == d_hi or d_lo + 1 == d_hi) {
          return true;
        }
        REP(d, d_lo + 1, d_hi) {
          ll z = st_char[d].query(l, r);
          if (z < cnt[d]) {
            return false;
          }
        }
