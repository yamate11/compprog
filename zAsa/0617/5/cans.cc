...
int main(/* int argc, char *argv[] */) {
  ...
  vector ndid(N + 1, -1LL);
  ndid[0] = 0;
  vector child(N + 1, map<ll, ll>());
  REP(i, 1, N + 1) {
    ll x = X[i - 1], y = Y[i - 1];
    ll s = ndid[x];
    auto it = child[s].find(y);
    if (it == child[s].end()) {
      ndid[i] = i;
      child[s][y] = i;
    }else {
  ...
  vector rev(N + 1, vector<ll>());
  REP(i, 0, N + 1) rev[ndid[i]].push_back(i);
  vector<ll> ans;
  auto dfs = [&](auto rF, ll nd) -> void {
    for (ll x : rev[nd]) ans.push_back(x);
    for (auto [y, c] : child[nd]) {
      rF(rF, c);
    }
  };
  dfs(dfs, 0);
  DLOGK(ans);
  assert(ssize(ans) == N + 1);
