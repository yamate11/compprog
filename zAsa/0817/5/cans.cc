...
int main(/* int argc, char *argv[] */) {
  ...
  vector tbl(c - a + 1, vector(d - b + 1, optional<Fp>()));
  auto f = [&](auto rF, ll p, ll q) -> Fp {
    optional<Fp>& r = tbl[p][q];
    if (not r) {
      if (p == 0 and q == 0) r = Fp(1);
      else if (p == 0) r = rF(rF, 0, q - 1) * a;
      else if (q == 0) r = rF(rF, p - 1, 0) * b;
      else {
        Fp r1 = rF(rF, p - 1, q) * (b + q);
        Fp r2 = rF(rF, p, q - 1) * (a + p);
        Fp r3 = rF(rF, p - 1, q - 1) * (a + p - 1) * (b + q - 1);
        r = r1 + r2 - r3;
      }
    }
    return *r;
