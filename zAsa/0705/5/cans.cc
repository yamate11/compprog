...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, N + 1) {
    ...
    func(A[i]);
    DLOGKL("f", i, vacant, que, ans);
    if (vacant >= B[N + 1] - B[i + 1]) {
      REP(j, i + 1, N + 1) ans += A[j] * (j - i);
      DLOGK(ans, B[N + 1] - B[i + 1]);
      func(B[N + 1] - B[i + 1]);
      break;
    }
    que.push(vacant);
    vacant *= 2;
    DLOGK(i, vacant, que, ans);
  }
  cout << ans << endl;
