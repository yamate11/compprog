...
int body(istream& cin, ostream& cout) {
  ...
  REP(i, 0, N) {
    C[i + 1] = C[i] + A[i];
    if (i % 2 == 0) B[i + 1] = B[i] + A[i];
    else B[i + 1] = B[i];
  }
  DLOGK(B);
  if (N % 2 == 0) REP(i, 0, N + 1) B[i] = C[i] - B[i];
  ...
  REP(_q, 0, Q) {
    ll X; cin >> X;
    auto f = [&](ll h) -> pll {
      auto it1 = upper_bound(ALL(A), X + h);
      auto it2 = lower_bound(ALL(A), X - h);
      ll m = A.end() - it1;
      ll n = A.end() - it2 - m;
      return {m, n};
    };
    ...
    ll h = binsearch_i<ll>(check, -1, 1e9 + 10);
    auto [m, n] = f(h);
    DLOGK(h, m, n);
    ll ans;
    if (m == n + 2) {
      ans = C[N] - C[N - m] + B[N - m - n - 1];
    }else {
      ans = C[N] - C[N - m] + B[N - m - n];
    }
    cout << ans << endl;
  }
