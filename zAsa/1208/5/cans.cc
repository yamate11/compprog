...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  string S; cin >> S;
  // @InpVec(N, T, type=string) [LtZF2BNn]
  auto T = vector(N, string());
  for (int i = 0; i < N; i++) { string v; cin >> v; T[i] = v; }
  // @End [LtZF2BNn]

  vector<Fp> tbl(ssize(S) + 1, Fp(0));
  tbl[0] = Fp(1);
  REP(i, 0, ssize(S)) {
    if (tbl[i] == Fp(0)) continue;
    REP(j, 0, N) {
      if (S.substr(i, ssize(T[j])) == T[j]) {
        tbl[i + ssize(T[j])] += tbl[i];
        DLOGK(i, T[j], ssize(T[j]), tbl[i + ssize(T[j])]);
      }
    }
  }
  cout << tbl[ssize(S)] << endl;
  return 0;
}
