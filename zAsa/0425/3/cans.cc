...
int naive(istream& cin, ostream& cout) {
  ...
  auto check = [&](const auto B) -> bool {
    vector<ll> S(N + 1);
    REP(i, 0, N) S[i + 1] = S[i] + B[i];
    auto b = EXISTS(i, 0, N + 1, EXISTS(j, i + 1, N + 1, S[i] >= K and S[j] < K));
    return not b;
  };
  ...
  if (ans1 == "Yes") {
    // @InpVec(N, ans2) [wwhoyniR]
    auto ans2 = vector(N, ll());
    for (int i = 0; i < N; i++) { ll v; cin >> v; ans2[i] = v; }
    // @End [wwhoyniR]
