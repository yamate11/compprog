...
struct GenSegTree {
  ...
  DAT query(int l, int r) {
    ...
    while (l < r) {
      if (l & 1) {
        DLOGKL("  ", ret_l, l, node[l]);
	ret_l = add(ret_l, node[l]);
	l++;
      }
      if (r & 1) {
        DLOGKL("  ", r -1, node[r - 1], ret_r);
...
struct DAT {
  ...
  ll cR0 = 0;
  ll cR1 = 0;
  friend ostream& operator<<(ostream& ostr, const DAT& t) {
    ostr << "(" << t.len << " " << t.cM0 << ", " << t.cM1 << ", " << t.cL0 << ", " << t.cL1 << ", " << t.cR0 << ", " << t.cR1 << ")";
    return ostr;
  }
  void flip() {
    swap(cM0, cM1);
  ...
  static DAT forInt(int i) {
    if (i == 0) { return DAT(1, 1, 0, 1, 0, 1, 0); }
    if (i == 1) { return DAT(1, 0, 1, 0, 1, 0, 1); }
    assert(0);
  }
};
...
int main(/* int argc, char *argv[] */) {
  ...
  auto add = [&](const DAT& a, const DAT& b) -> DAT {
    ll cM0 = max(a.cM0, max(b.cM0, a.cR0 + b.cL0));
    ll cM1 = max(a.cM1, max(b.cM1, a.cR1 + b.cL1));
    ll cL0 = a.cL0 == a.len ? a.cL0 + b.cL0 : a.cL0;
    ll cL1 = a.cL1 == a.len ? a.cL1 + b.cL1 : a.cL1;
    ll cR0 = b.cR0 == b.len ? b.cR0 + a.cR0 : b.cR0;
    ll cR1 = b.cR1 == b.len ? b.cR1 + a.cR1 : b.cR1;
    auto ret = DAT(a.len + b.len, cM0, cM1, cL0, cL1, cR0, cR1);
    DLOGK(a, b, ret);
    return ret;
  };
  auto comp = [&](OP a, OP b) -> OP { return a ^ b; };
  auto appl = [&](OP f, const DAT& d) -> DAT {
    if (not f) return d;
    DAT ret(d);
    ret.flip();
    return ret;
  ...
  REP(i, 0, N) init_vec[i] = DAT::forInt(S[i] - '0');
  auto st = make_seg_tree_lazy(DAT(), false, add, comp, appl, init_vec);
