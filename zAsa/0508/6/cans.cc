...
int main(/* int argc, char *argv[] */) {
  ...
  vector<pll> incint{{-N - N, 0}};
  vector<bool> tmp(N - 1);
  REP(i, 0, N - 1) tmp[i] = P[i] < P[i + 1];
  ll idx = 0;
  for (auto [b, len] : rle(tmp)) {
    if (b) incint.emplace_back(idx, len);
    idx += len;
  }
  incint.emplace_back(N + N, 0);
  auto mymin = [](ll a, ll b) -> ll { return min(a, b); };
  auto mymax = [](ll a, ll b) -> ll { return max(a, b); };
  auto stMin = make_seg_tree(N, mymin, P);
  auto stMax = make_seg_tree(0LL, mymax, P);

  ll no_change = 0;
  set<pll> ss;
  REP(i, 0, N - K + 1) {
    auto pp = ranges::lower_bound(incint, pll(i + 1, -1));
    auto [tt, mm] = *std::prev(pp);
    if (tt <= i and i + K - 1 <= tt + mm) {
      no_change = 1;
      continue;
    }
    ll mn = stMin.query(i, i + K);
    ll mx = stMax.query(i, i + K);
    DLOGK(mn, mx);
    ll left, right;
    if (i == 0 or P[i - 1] > mn) left = i;
    else {
      auto p = ranges::lower_bound(incint, pll(i + 1, -1));
      auto [t, l] = *std::prev(p);
      assert(t >= 0 and i - 1 <= t + l);
      left = t;
    }
    if (i + K - 1 == N - 1 or mx > P[i + K - 1 + 1]) right = i + K - 1;
    else {
      auto p = ranges::lower_bound(incint, pll(i + K + 1, -1));
      auto [t, l] = *std::prev(p);
      assert(t >= 0 and i + K - 1 <= t + l);
      right = t + l;
    }
    DLOGK(i, left, right);
    ss.emplace(left, right);
  }
  cout << ssize(ss) + no_change << endl;
