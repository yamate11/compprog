...
int main(/* int argc, char *argv[] */) {
  ...
  vector<Fp> tbl_init(64, Fp(0));
  vector<Fp> tbl(64, Fp(1));
  REP(i, 3, N) {
    auto prev = move(tbl);
    tbl = tbl_init;
    REP(a, 0, 4) REP(b, 0, 4) REP(c, 0, 4) REP(d, 0, 4) {
      auto match = [&](ll na, ll nb, ll nc, ll nd) -> bool {
        if (not (na == a or na == -1)) return false;
        if (not (nb == b or nb == -1)) return false;
        if (not (nc == c or nc == -1)) return false;
        if (not (nd == d or nd == -1)) return false;
        return true;
      };

      if      (match(0, 2, 1, -1)) ;
      else if (match(2, 0, 1, -1)) ;
      else if (match(0, 1, 2, -1)) ;
      else if (match(-1, 0, 2, 1)) ;
      else if (match(-1, 2, 0, 1)) ;
      else if (match(-1, 0, 1, 2)) ;
      else if (match(0, 3, 2, 1)) ;
      else if (match(0, 2, 3, 1)) ;
      else if (match(0, 2, 2, 1)) ;
      else tbl[b * 16 + c * 4 + d] += prev[a * 16 + b * 4 + c];
    }
  }
  Fp ans = 0;
  REP(j, 0, 64) ans += tbl[j];
  cout << ans << endl;
