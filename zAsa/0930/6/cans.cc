...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W; cin >> H >> W;
  ll empty = H * W - 1;
  string goal0(H * W, (char)0);
  REP(i, 0, H * W) goal0[i] = (char)i;
  string start0(H * W, (char)0);
  Board<ll> brd(H, W, -1LL);
  cin >> brd;
  BrdIdx posEmpty0;
  REP(i, 0, H) REP(j, 0, W) {
    ll x;
  ...
  using mytp = unordered_map<string, sta>;

  auto f = [&](string start, string goal, auto posEmpty) -> pair<ll, mytp> {
    mytp mp;
    queue<string> que;
    if (start == goal) return {0, mytp()};
    mp[start] = sta(0, posEmpty);
    que.push(move(start));
    while (not que.empty()) {
      string s = move(que.front()); que.pop();
      auto& [dist, bi] = mp[s];
      if (dist >= 12) break;
      ll e = brd.enc(bi);
      for (auto& dir : BrdIdx::nbr4) {
        ...
        if (brd.in(bj)) {
          ...
          string t = s;
          swap(t[e], t[e2]);
          if (t == goal) return {dist + 1, mytp()};
          if (not mp.contains(t)) {
            mp[t] = sta(dist + 1, bj);
            que.push(move(t));
          }
        }
  ...
  auto [ans1, mp1] = f(start0, goal0, posEmpty0);
  if (ans1 >= 0) {
    cout << ans1 << endl;
    return 0;
  }
  auto [ans2, mp2] = f(goal0, start0, BrdIdx(H - 1, W - 1));
  assert(ans2 < 0);
  ll ans = 1e18;
  for (auto& [s, p] : mp1) {
    auto it = mp2.find(s);
    if (it != mp2.end()) {
      auto& [d2, pos2] = it->second;
      ans = min(ans, p.first + d2);
    }
  }
  cout << ans << endl;
