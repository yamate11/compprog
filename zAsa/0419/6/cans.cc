...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 0, M) {
    ll a = gcd(A[i], N);
    auto [it, b] = mp.try_emplace(a, C[i]);
    if (not b) updMin(it->second, C[i]);
  }
  map<ll, ll> tbl;
  tbl[N] = 0;
  while (true) {
    ...
    for (auto [a, c] : mp) {
      ll g = gcd(x, a);
      if (g < x) {
        ll cost = cost_x + c * (x - g);
        auto [it, b] = tbl.try_emplace(g, cost);
        if (not b) updMin(it->second, cost);
      }
    }
  }
