...
int main(/* int argc, char *argv[] */) {
  ...
  ll H, W, K; cin >> H >> W >> K;
  auto pw = vector(H, vector<char>(W, ' '));
  REP(i, 0, K) {
    ll h, w; char c; cin >> h >> w >> c; h--; w--;
    pw[h][w] = c;
  }
  vector cntD(H + 1, vector<ll>(W, 0LL));
  vector cntR(H, vector<ll>(W + 1, 0LL));
  REP(j, 0, W) REP(i, 0, H) cntD[i + 1][j] = cntD[i][j] + (pw[i][j] == ' ' ? 1 : 0);
  REP(i, 0, H) REP(j, 0, W) cntR[i][j + 1] = cntR[i][j] + (pw[i][j] == ' ' ? 1 : 0);

  vector tbl(H, vector(W, Fp(0)));
  REPrev(h, H - 1, 0) REPrev(w, W - 1, 0) {
    if (h == H - 1 and w == W - 1) {
      tbl[H - 1][W - 1] = Fp(1);
      continue;
    }
    Fp vd = Fp(0);
    Fp vr = Fp(0);
    if (h + 1 < H) vd = tbl[h + 1][w] * power<Fp>(3, cntR[h][W] - cntR[h][w + 1]);
    if (w + 1 < W) vr = tbl[h][w + 1] * power<Fp>(3, cntD[H][w] - cntD[h + 1][w]);
    if (pw[h][w] == 'D') {
      tbl[h][w] = vd;
    }else if (pw[h][w] == 'R') {
      tbl[h][w] = vr;
