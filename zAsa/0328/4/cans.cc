...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  vector A(2*N, vector<ll>(2*N, 0LL));
  REP(i, 0, 2*N - 1) {
    REP(j, i + 1, 2*N) {
      ll a; cin >> a;
      A[i][j] = A[j][i] = a;
  ...
  ll full = (1LL << (2 * N)) - 1;
  ll ans = 0;
  auto f = [&](auto rF, ll x, ll score) -> void {
    if (x == full) updMax(ans, score);
    else {
      ll i = countr_one((u64)x);
      REP(j, i + 1, 2*N) if (not (x >> j & 1)) {
        rF(rF, x | (1LL << i) | (1LL << j), score ^ A[i][j]);
      }
    }
  };
  f(f, 0, 0);
