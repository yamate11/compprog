...
int main(/* int argc, char *argv[] */) {
  ...
  ll hx = llround(gx * unit);
  ll hy = llround(gy * unit);
  ll hr = llround(gr * unit);
  ll st = divCeil(hx - hr, unit);
  ll en = divFloor(hx + hr, unit);
  ll ans = 0;
  REP(i, st, en + 1) {
    ll hi = i * unit;
    auto check = [&](ll h) -> bool {
      return (hi - hx) * (hi - hx) + (h - hy) * (h - hy) <= hr * hr;
    };
    ll hu = binsearch_i<ll>(check, hy, hy + hr * 2);
    ll hl = hy - (hu - hy);
    ans += divFloor(hu, unit) - divCeil(hl, unit) + 1;
  }
  cout << ans << endl;
  return 0;
}
