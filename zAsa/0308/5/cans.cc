...
// @@ !! LIM(unordered_map forall cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  ll N; cin >> N;
  safe_umap<ll, pll> mp;
  vector PE(N, vector<pll>());
  REP(i, 0, N) {
    ll m; cin >> m;
    REP(j, 0, m) {
      ll p, e; cin >> p >> e;
      PE[i].emplace_back(p, e);
      auto [it, b] = mp.try_emplace(p, pll(e, 1));
      if (not b) {
        if (it->second.first == e) it->second.second++;
        else if (it->second.first < e) it->second = pll(e, 1);
      }
    }
  }
  ll ans = 0;
  ll unchange = 0;
  REP(i, 0, N) {
    if (EXISTS(j, 0, ssize(PE[i]), mp[PE[i][j].first] == pll(PE[i][j].second, 1))) {
      ans++;
    }else {
      unchange++;
    }
  }
  cout << ans + (unchange >= 1 ? 1 : 0) << endl;
  return 0;
}
