...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpGrid(N, 5, P) [Blhc7T6r]
  auto P = vector(N, vector(5, ll()));
  for (int i = 0; i < N; i++) for (int j = 0; j < 5; j++) { ll v; cin >> v; P[i][j] = v; }
  // @End [Blhc7T6r]
  vector Q(N, vector(32, 0LL));
  ll big = 1e18;
  REP(i, 0, N) {
    REP(x, 0, 32) {
      Q[i][x] = big;
      REP(j, 0, 5) if (x >> j & 1) updMin(Q[i][x], P[i][j]);
    }
  }
  vector<ll> R(32, 0LL);
  REP(x, 0, 32) {
    REP(i, 0, N) updMax(R[x], Q[i][x]);
  }
  IntDupPerm dip(3, 5);
  ll ans = 0LL;
  while (dip.get()) {
    vector y(3, 0LL);
    REP(i, 0, 5) y[dip.at(i)] |= 1LL << i;
    ll v = min(R[y[0]], min(R[y[1]], R[y[2]]));
    updMax(ans, v);
  }
  cout << ans << endl;
