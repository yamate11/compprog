...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  // @InpMVec(N - 1, ((A, dec=1), (B, dec=1))) [HnlliqwR]
  ...
  vector Z(N, 0LL);
  REP(_q, 0, Q) {
    ll tp, e, x; cin >> tp >> e >> x; e--;
    bool dab;
    ll nd;
    if (tr.parent(A[e]) == B[e]) {      dab = true;  nd = A[e]; }
    else if (tr.parent(B[e]) == A[e]) { dab = false; nd = B[e]; }
    else assert(0);
    if ((dab and tp == 1) or (not dab and tp == 2)) {
      Z[nd] += x;
    }else {
      base += x;
      Z[nd] -= x;
  ...
  vector<ll> ans(N);
  auto dfs = [&](auto rF, ll nd, ll val) -> void {
    val += Z[nd];
    ans[nd] = val;
    for (ll cld : tr.children(nd)) rF(rF, cld, val);
  };
  dfs(dfs, 0, base);
  REPOUT(i, 0, N, ans[i], "\n");
