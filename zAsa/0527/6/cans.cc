...
int main(/* int argc, char *argv[] */) {
  ...
  /*
  ll tlim = N * M + 1;
  vector<double> B(tlim);
  B[0] = 1;
  REP(i, 1, tlim) B[i] = B[i - 1] * 0.99;
  */

  vector pb(N, vector(M, -1.0));
  using sta = tuple<double, BrdIdx>;
  priority_queue<sta> pque;
  pb[goal.r][goal.c] = 100.0;
  pque.emplace(100.0, goal);
  while (not pque.empty()) {
    auto [br, x] = pque.top(); pque.pop();
    DLOGKL("popped", br, x);
    if (pb[x.r][x.c] != br) continue;
    for (auto dir : BrdIdx::nbr4) {
      ...
      if (y == start) {
        ...
        double newbr = min(br * 0.99, double(brd.at(y) - '0'));
        if (newbr > pb[y.r][y.c]) {
          pb[y.r][y.c] = newbr;
          pque.emplace(newbr, y);
        }
      }
    }
  }
