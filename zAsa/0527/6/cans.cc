...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpMVec(N, ((M, dec=1), S, E)) [6DjVoty6]
  auto M = vector(N, ll());
  auto S = vector(N, ll());
  auto E = vector(N, ll());
  for (int i = 0; i < N; i++) {
    ll v1; cin >> v1; v1 -= 1; M[i] = v1;
    ll v2; cin >> v2; S[i] = v2;
    ll v3; cin >> v3; E[i] = v3;
  }
  // @End [6DjVoty6]

  vector A(N, vector<pll>());
  REP(i, 0, N) A[M[i]].emplace_back(E[i], S[i]);
  REP(i, 0, N) ranges::sort(A[i]);

  ll lim = 1e6 + 1;
  vector st(lim, vector<ll>());
  REP(i, 0, N) st[S[i]].emplace_back(M[i]);

  vector<ll> tbl(lim, 0LL);
