...
int main(/* int argc, char *argv[] */) {
  ...
  Fp::setMod(P);
  vector<Fp> fact(N + 1);
  fact[0] = 1; REP2(i, 1, N + 1) fact[i] = fact[i - 1] * i;
  vector binom(N + 1, vector(N + 1, Fp(0)));
  REP(n, N + 1) {
    binom[n][0] = binom[n][n] = Fp(1);
    REP2(r, 1, n) binom[n][r] = binom[n-1][r-1] + binom[n-1][r];
  }
  vector tbl(M + 1, vector(N + 1, vector(K + 1, Fp(0))));
  REP2(n, 1, N + 1) tbl[1][n][1] = fact[n];
#if DEBUG
    REP2(n, 1, N + 1) REP(d, min(n + 1, K + 1)) DLOGK("z", n, d, tbl[1][n][d]);
#endif
  REP2(m, 2, M + 1) {
    REP2(n, 1, m) tbl[m][n][0] = fact[n];
    REP2(n, m, N + 1) REP(d, min(n + 1, K + 1)) tbl[m][n][d] += 2 * tbl[m-1][n-1][d];
    REP2(a, 1, N + 1) REP2(b, max(1LL, m - a - 1), N - a) REP(d1, min(a + 1, K + 1)) {
      Fp val = tbl[m-1][a][d1];
      if (val == Fp(0)) continue;
      REP(d2, min(b + 1, K + 1 - d1)) {
        DLOGK(m, a, b, d1, d2);
        tbl[m][a + b + 1][d1 + d2] += binom[a + b][a] * val * tbl[m-1][b][d2];
        DLOGK(tbl[m][a+b+1][d1+d2]);
      }
    }
#if DEBUG
    REP2(n, m, N + 1) REP(d, min(n + 1, K + 1)) DLOGK("z", n, m, d, tbl[m][n][d]);
#endif
  }
  cout << tbl[M][N][K] << endl;
  return 0;
