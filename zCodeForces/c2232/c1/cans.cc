...
// @@ !! LIM(f:updMaxMin cmpNaive forall debug)
...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ...
    auto f = [&](ll x) -> ll {
      ...
      REP(i, 0, N) {
        if (A[i] == 'A') aidx++;
        if (A[i] == 'I' or (A[i] == 'A' and (x >> aidx & 1))) {
          ll p = EXFIND_D(j, 0, X, F[j] == 0, -1LL);
          if (p == -1) continue;
          F[p] = 1;
          seated++;
        }else if (A[i] == 'E' or (A[i] == 'A' and not (x >> aidx & 1))) {
          ll p = EXFIND_D(j, 0, X, 0 < F[j] and F[j] < S, -1LL);
          if (p == -1) continue;
          F[p]++;
          seated++;
        }
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ll N, X, S; cin >> N >> X >> S;
    string A; cin >> A;
    vector<ll> tbl(X + 1, -1LL);
    tbl[0] = 0;
    REP(i, 0, N) {
      if (A[i] == 'I') {
        REPrev(j, X, 1) {
          if (tbl[j - 1] >= 0) updMax(tbl[j], tbl[j - 1] + 1);
        }
        tbl[0] = -1;
      }else if (A[i] == 'E') {
        REP(j, 0, X + 1) {
          if (tbl[j] >= 0 and tbl[j] < S * j) tbl[j]++;
        }
      }else {
        REPrev(j, X, 0) {
          ll old_val = tbl[j];
          if (j >= 1 and tbl[j - 1] >= 0) updMax(tbl[j], tbl[j - 1] + 1);
          if (tbl[j] >= 0 and tbl[j] < S * j) updMax(tbl[j], old_val + 1);
        }
      }
      DLOGK(i, tbl);
    }
