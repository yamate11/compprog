...
// @@ !! LIM(sieve mod input unordered_map)
...
// @DefStruct(sta, ((x0, ll), (v, vector<ll>))) [hNj8wNMN]
struct sta {
  ll x0;
  vector<ll> v;
  /*
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    Fp ans = 1;
    for (auto& [p, st] : mp) {
      if (st.x0 > accumulate(ALL(st.v), 0LL)) {
        cout << 0 << "\n";
        return;
      }
      ll z = 0;
      ...
      ll sz = z + 2 + st.x0;
      vector tbl_init(sz, vector(sz, Fp(0)));
      auto tbl = tbl_init;
      tbl[0][0] = Fp(1);
      for (ll r : st.v) {
        auto prev = move(tbl);
        tbl = tbl_init;
        REP(mx, 0, sz) REP(e, 0, sz) {
          if (st.x0 + mx < e) break;
          if (prev[mx][e] == Fp(0)) continue;
          REP(i, 0, r + 1) {
            if (st.x0 + max(mx, i) >= e + i) {
              tbl[max(mx, i)][e + i] += prev[mx][e];
            }
          }
        }
      }
