...
// @@ !! LIM(binsearch unordered_map debug cmpNaive forall random)
...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ...
    REPrev(len, N / 2, 1) {
      REP(st1, 0, N - 2 * len + 1) {
        auto V = A;
        sort(V.begin() + st1, V.begin() + st1 + len);
        if (FORALL(i, 0, len - 1, V[st1 + i] + 1 == V[st1 + i + 1])) {
          REP(st2, st1 + len, N - len + 1) {
            auto W = V;
            sort(W.begin() + st2, W.begin() + st2 + len);
            if (FORALL(i, 0, len - 1, W[st2 + i] + 1 == W[st2 + i + 1])) {
              if (W[st1] == W[st2] + len or W[st1] + len == W[st2]) {
                cout << len << "\n";
                return;
              }
            }
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    ll mask = big - 1;
    vector hash(N, 0LL);
    REP(i, 0, N) hash[i] = myrand.range(0, big);
    // vector itvl(maxlen + 1, safe_umap<ll, ll>());
    vector itvl(N, vector(maxlen + 1, 0LL));
    REP(i, 0, N) {
      ll h = 0;
      REP(j, 1, maxlen + 1) {
        if (i + j - 1 == N) break;
    ...
    DLOGK(itvl);
    vector cands(maxlen + 1, vector<ll>());
    REP(i, 0, N) {
      ll h = 0;
      ll vmin = A[i];
      REP(j, 1, maxlen + 1) {
        if (i + j - 1 == N) break;
        h = (h + hash[A[i + j - 1]]) & mask;
        vmin = min(vmin, A[i + j - 1]);
        DLOGK(i, j, h, vmin);
        if (itvl[vmin][j] == h) cands[j].push_back(vmin);
      }
    }
    DLOGK(cands);
    REPrev(len, maxlen, 1) {
      ranges::sort(cands[len]);
      for (ll x : cands[len]) {
        auto it = ranges::lower_bound(cands[len], x + len);
        if (it != cands[len].end() and *it == x + len) {
          cout << len << "\n";
          return;
        }
      }
