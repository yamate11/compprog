...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N, start, goal; cin >> N >> start >> goal; start--; goal--;
    Tree tr(N, start);
    REP(i, 0, N - 1) {
      ll u, v; cin >> u >> v; u--; v--;
      tr.add_edge(u, v);
    ...
    vector<ll> vec(d0 + 1);
    vec[d0] = goal;
    REPrev(d, d0 - 1, 0) vec[d] = tr.parent(vec[d + 1]);
    vector<ll> ans;
    vector<pll> tmp;
    auto f = [&](auto rF, ll nd) -> void {
      tmp.emplace_back(tr.depth(nd), nd);
      for (ll cld : tr.children(nd)) rF(rF, cld);
    };
    REP(d, 0, d0) {
      tmp.clear();
      for (ll cld : tr.children(vec[d])) {
        if (cld != vec[d + 1]) {
          f(f, cld);
        }
      }
      tmp.emplace_back(d, vec[d]);
      sort(ALL(tmp), greater<pll>());
      for (auto [_d, nd] : tmp) ans.push_back(nd);
    }
    tmp.clear();
    for (ll cld : tr.children(vec[d0])) f(f, cld);
    tmp.emplace_back(d0, vec[d0]);
    sort(ALL(tmp), greater<pll>());
    for (auto [_d, nd] : tmp) ans.push_back(nd);
