...
// @@ !! LIM(f:updMaxMin UnionFind debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  using ret_t = vector<pll>;
  auto solve = [&]() -> ret_t {
    ret_t ans;

    ll N; cin >> N;
    // @InpVec(N, S, type=string) [B5p71eB4]
    auto S = vector(N, string());
    for (int i = 0; i < N; i++) { string v; cin >> v; S[i] = v; }
    // @End [B5p71eB4]

    vector<ll> rank(N, 0LL);
    vector<ll> nbwd(N);
    REP(i, 0, N) REP(j, 0, N) {
      if (j == i) {
        if (S[i][j] != '1') return ret_t();
      }else {
        if (S[i][j] == '1') {
          nbwd[j]++;
        }
    ...
    ll cnt = 0;
    vector<ll> cands;
    REP(i, 0, N) if (nbwd[i] == 0) {
      cands.push_back(i);
      cnt++;
    }
    while (not cands.empty()) {
      ll nd = cands.back(); cands.pop_back();
      REP(peer, 0, N) if (peer != nd and S[nd][peer] == '1') {
        updMax(rank[peer], rank[nd] + 1);
        if (--nbwd[peer] == 0) {
    ...
    if (cnt != N) return ret_t();

    vector edges_rd(N, vector<pll>());
    REP(i, 0, N) REP(j, 0, N) if (i != j and S[i][j] == '1') edges_rd[rank[j] - rank[i]].emplace_back(i, j);
    
    UnionFind uf(N);
    vector children(N, vector<ll>());
    REP(diff, 0, N) {
      for (auto [i, j] : edges_rd[diff]) {
    ...
    ll lcnt = 0;
    REP(i, 0, N) if (uf.leader(i) == i) lcnt++;
    if (lcnt > 1) return ret_t();

    vector cl(N, vector(N, false));
    auto g = [&](auto rF, ll nd) -> void {
      if (cl[nd][nd]) return;
      cl[nd][nd] = true;
      for (ll c : children[nd]) {
        rF(rF, c);
        REP(i, 0, N) if (cl[c][i]) cl[nd][i] = true;
      }
    };
    REP(i, 0, N) g(g, i);

    REP(i, 0, N) REP(j, 0, N) if (cl[i][j] != (S[i][j] == '1')) return ret_t();
    return ans;
  };
