...
// @@ !! LIM(cmpNaive f:updMaxMin debug)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<ll> cnt(amax + 1);
    REP(i, 0, N) cnt[A[i]]++;
    auto cnt_save = cnt;
    vector<ll> ans(M + 1);

    REP(k, 1, M + 1) {
      if (k >= 2 and ans[k - 1] == tot) {
        ans[k] = tot;
        continue;
      }
      REP(goal, 1, amax + 1) {
        ...
        REP(i, 0, k) {
          ...
          cnt = move(new_cnt);
          DLOGK(k, goal, i, cnt);
          if (ssize(cnt) <= goal) break;
        }
        if (ssize(cnt) <= goal) break;
        updMax(ans[k], cnt[goal]);
        if (ans[k] == tot) break;
      }
    }
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    ll amax = *ranges::max_element(A);
    vector<ll> nxt(M + 1, M + 1);
    REPrev(i, M - 1, 0) nxt[i] = cnt[i + 1] == 0 ? nxt[i + 1] : i + 1;

    vector<ll> ans(M + 1);
    REP(k, 1, M + 1) {
      ll powk = 1LL << k;
      if (powk >= amax) {
        REP(kk, k, M + 1) ans[kk] = tot;
        break;
      }
      else {
        ll score = 0;
        for (ll a = 1; a <= M; a = nxt[a]) {
          if (a <= powk) score += a * cnt[a];
          else           score += (powk - 1) * cnt[a];
        }
        ll cmax = cnt[powk];
        for (ll a0 = 2 * powk; a0 <= amax; a0 += powk) {
          if (cnt[a0] > cmax) {
            cmax = cnt[a0];
            ll sc = 0;
            ll x0 = a0 / powk;
            for (ll a = 1; a <= M; a = nxt[a]) {
              ll tv;
              if (a <= a0) tv = (a / x0) * cnt[a];
              else         tv = (powk - 1) * cnt[a];
              DLOGK(k, a0, a, tv);
              sc += tv;
            }
            DLOGK(k, a0, sc);
