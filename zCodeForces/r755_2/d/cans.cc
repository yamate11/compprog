...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    auto isc = [&](ll x) -> ll {
      auto check = [&](ll t) -> bool { return t * (t - 1) / 2 <= x; };
      ll t = binsearch_i<ll>(check, 0, N + 1);
    ...
    auto op2 = [&]() -> void {
      ...
      assert(y2 >= 0);
      ll kk = mid + y2;
      auto check3 = [&](ll j) -> bool {
        cout << "? " << j << " " << kk << endl;
        ll x; cin >> x;
        return (kk - j + 1) * (kk - j) / 2 == x;
      };
      ll jj = binsearch_i<ll>(check3, mid, l);
      ll ss = tot - (kk - jj + 1) * (kk - jj) / 2;
      ll yy = isc(ss);
      cout << "! " << jj - yy << " " << jj << " " << kk << endl;
    };
    auto op3 = [&](ll y1) -> void {
      DLOG("op3");
      ll ii = mid - y1 + 1;
      auto check2 = [&](ll j) -> bool {
        cout << "? " << ii << " " << j << endl;
        ll x; cin >> x;
        return (j - ii + 1) * (j - ii) / 2 == x;
      };
      ll jj = binsearch_i<ll>(check2, mid, N);
      jj++;
      ll s = tot - (jj - ii) * (jj - ii - 1) / 2;
      ll y = isc(s);
      DLOGK(tot, jj, ii, s, y);
      cout << "! " << ii << " " << jj << " " << jj + y - 1 << endl;
    };
