...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N; cin >> N;
    // @InpMVec(N - 1, ((U, dec=1), (V, dec=1))) [IUwFOr4F]
    ...
    ll K; cin >> K;
    // @InpMVec(K, ((A, dec=1), (B, dec=1))) [1z8GNKD9]
    auto A = vector(K, ll());
    auto B = vector(K, ll());
    for (int i = 0; i < K; i++) {
    ...
    vector cov(N - 1, 0LL);
    REP(i, 0, K) {
      const auto& nnp = tr.nnpath(A[i], B[i]);
      REP(j, 0, ssize(nnp) - 1) {
        ll e = tr.edgeIdx(nnp[j], nnp[j + 1]);
        cov[e] |= (1LL << i);
      }
    }
    sort(ALL(cov));
    cov.erase(unique(ALL(cov)), cov.end());

    ll big = 1e18;
    vector tbl_init(1LL << K, big);
    auto tbl = tbl_init;
    tbl[0] = 0;
    for (ll c : cov) {
      auto prev = tbl;
