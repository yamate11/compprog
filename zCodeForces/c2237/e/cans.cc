...
// published at https://github.com/yamate11/compprog-clib/blob/master/FuncGraph.cc
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> vector<ll> {
    ll N; cin >> N;
    // @InpVec(N, A, dec=1) [nkDRzOza]
    auto A = vector(N, ll());
    for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; A[i] = v; }
    // @End [nkDRzOza]
    // @InpVec(N, B, dec=1) [iO9HQkO6]
    auto B = vector(N, ll());
    ...
    auto cycsize = [&](ll nd) -> ll { return fg.cycle_size(fg.cc(nd)); };

    vector persize(N + 1, vector<ll>());
    REP(i, 0, N) {
      persize[cycsize(i)].push_back(i);
    }
    vector psIdx(N + 1, 0LL);

    vector<bool> checked(N, false);
    vector<bool> transed(N, false);
    REP(i, 0, N) {
      if (B[i] >= 0 and not checked[i]) {
        DLOGK(i, B[i]);
        checked[i] = true;
        transed[B[i]] = true;
        if (cycsize(i) != cycsize(B[i])) return emptyVec;
        ll x = i;
        ll y = B[i];
        DLOGK(x, y);
        REP(j, 1, cycsize(i)) {
          ...
          DLOGK(j, x, y);
          if (B[x] < 0) {
            if (transed[y]) return emptyVec;
            B[x] = y;
          }
          else if (B[x] != y) return emptyVec;
          checked[x] = true;
          transed[y] = true;
        }
      }
    ...
    REP(i, 0, N) {
      if (not checked[i]) {
        auto get_free = [&]() -> ll {
          ll z = cycsize(i);
          for (ll j = psIdx[z]; j < ssize(persize[z]); j++) {
            ll w = persize[z][j];
            if (not transed[w]) return w;
          }
          return -1;
        };
        ll w = get_free();
        if (w < 0) return emptyVec;
        B[i] = w;
        checked[i] = true;
        transed[w] = true;
        ll x = i;
