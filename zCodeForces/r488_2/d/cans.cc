...
int main(/* int argc, char *argv[] */) {
  ...
  auto match1 = [](const auto& P, const auto& Q) -> ll {
    if (P.x == Q.x && P.y != Q.y) return P.x;
    if (P.y == Q.y && P.x != Q.x) return P.y;
    if (P.x == Q.y && P.y != Q.x) return P.x;
    if (P.y == Q.x && P.x != Q.y) return P.y;
    return -1;
  };

  ll n, m; cin >> n >> m;
  vector<IPoint> P(n), Q(m);
  ...
  auto pknow = [&](const auto& X, const auto& Y) -> bool {
