...
int main(/* int argc, char *argv[] */) {
  ...
  auto f = [&](const vector<IPoint>& X, IPoint V) -> bool {
    ll x0 = X[0].x, x1 = X[0].x;
    ll y0 = X[0].y, y1 = X[0].y;
    for (ll i = 0; i < 4; i++) {
      x0 = min(x0, X[i].x);
      x1 = max(x1, X[i].x);
      y0 = min(y0, X[i].y);
