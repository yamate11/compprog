...
// @@ !! LIM(binsearch f:power binsearch debug)
// --> binsearch f:power f:<< debug
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    auto check = [&](ll t) -> bool {
      return k <= (power<ll>(4, t) - 1) / 3;
    };
    ll thr = binsearch_i<ll>(check, 31, 0);
    DLOGK(thr, n);
    if (thr > n) return -1;
    ll rem = (power<ll>(4, thr) - 1) / 3 - k;
    ll edge = 2 * power<ll>(2, thr - 1) - 1;
    DLOGK(rem, edge);
    if (rem >= edge) return n - thr + 1;
    else return n - thr;
