...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> Fp {
    ...
    ll state = 0;
    REP(i, 0, N - 1) {
      if (not (1 <= A[i] and A[i] < N)) return Fp(0);
      cnt[A[i]]++;
      if (state == 0) {
        if (A[i] == N - 1) {
          state = 1;
        }else {
          if (i > 0 and A[i - 1] > A[i]) return Fp(0);
          left[A[i]] = true;
        }
      }else if (state == 1) {
        if (A[i] == N - 1) {
        }else {
          if (left[A[i]]) return Fp(0);
          state = 2;
        }
      }else if (state == 2) {
        if (A[i - 1] < A[i]) return Fp(0);
        if (left[A[i]]) return Fp(0);
      }
    }
    if (state == 0) return Fp(0);
    Fp ans = Fp(2); // for N and N - 1
    ll possible = cnt[N - 1] - 1;
    REPrev(i, N - 2, 1) {
      if (cnt[i] >= 1) {
        possible += cnt[i] - 1;
      }else {
        if (possible <= 0) return Fp(0);
        ans *= Fp(possible);
        possible--;
      }
    }
