...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ll ans = 0;
    auto isComplSquare = [&](ll x) -> bool {
      ll y = llround(sqrt(x));
      return y * y == x;
    };
    auto dfs = [&](auto rF, ll nd) -> void {
      if (isComplSquare(A[nd])) {
        ll cur = 0;
        vector<ll> vec;
        for (ll c : tr.children(nd)) vec.push_back(tr.stsize(c));
        if (nd != tr.root) vec.push_back(N - tr.stsize(nd));
        ll ord = ssize(vec);
        if (ord == 1) cur = 0;
        else if (ord == 2) cur = vec[0] * vec[1];
        else {
          ll s1 = 0, s2 = 0, s3 = 0;
          for (ll x : vec) {
            s1 += x;
