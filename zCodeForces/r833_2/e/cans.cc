...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<ll> childL(N + 1, -1LL), childR(N + 1, -1LL);
    vector<ll> ord(N);
    iota(ALL(ord), 1);
    sort(ALL(ord), [&](ll a, ll b) -> bool { return A[a] == A[b] ? a < b : A[a] > A[b]; });
    set<ll> ss{0, N + 1};
    for (ll i : ord) {
      auto itU = ss.lower_bound(i);
      auto itL = prev(itU);
      if (A[*itL] < A[*itU]) {
        parent[i] = *itL;
        childR[*itL] = i;
    ...
    vector tbl1(N + 1, vector(M + 1, Fp(0)));
    vector tbl2(N + 1, vector(M + 1, Fp(0)));
    auto dfs = [&](auto rF, ll nd) -> void {
      ll cL = childL[nd], cR = childR[nd];
      if (cL >= 0) rF(rF, cL);
      if (cR >= 0) rF(rF, cR);
      REP(m, 1, M + 1) {
        Fp vL = cL < 0 ? Fp(1) : tbl2[cL][m - 1];
        Fp vR = cR < 0 ? Fp(1) : tbl2[cR][m];
        tbl1[nd][m] = vL * vR;
      }
      REP(m, 1, M + 1) tbl2[nd][m] = tbl2[nd][m - 1] + tbl1[nd][m];
    };
    ll root = childR[0];
    dfs(dfs, root);
    cout << tbl2[root][M] << endl;
