...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    vector<vector<ll>> tbl(n, vector(m, 0LL));
    vector<ll> orig(3);
    for (ll i = 0; i < n; i++) {
      for (ll j = 0; j < m; j++) {
	char c = S[i][j];
	if ('0' <= c && c <= '9') tbl[i][j] = 0;
	else if ('a' <= c && c <= 'z') tbl[i][j] = 1;
	else tbl[i][j] = 2;
      }
      orig[tbl[i][0]]++;
    }
    /*
    ll sat = 0;
    for (ll i = 0; i < 3; i++) sat += (orig[i] > 0 ? 1 : 0);
    if (sat == 3) return 0;
    */
    vector<vector<ll>> near(3, vector<ll>(n));
    ll big = 1e10;
    for (ll c = 0; c < 3; c++) {
      for (ll i = 0; i < n; i++) {
	near[c][i] = big;
	for (ll j = 0; j < m; j++) {
	  if (tbl[i][j] == c || (j > 0 && tbl[i][m - j] == c)) {
	    near[c][i] = j;
	    break;
	  }
	}
    ...
    do {
      ll cnt = 0;
      for (ll i = 0; i < 3; i++) cnt += near[i][v[i]];
      ans = min(ans, cnt);
    }while (ip.next());
    return ans;
