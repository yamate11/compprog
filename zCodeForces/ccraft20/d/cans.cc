...
int main(/* int argc, char *argv[] */) {
  ...
  BrdIdx dummy(0, 0);
  BrdIdx bloop(-2, -2);
  Board<BrdIdx> brd(n, n, dummy);
  vector<vector<ll>> fcnt(n, vector<ll>(n));
  for (ll r = 0; r < n; r++) {
    for (ll c = 0; c < n; c++) {
      ll x, y; cin >> x >> y; x--; y--;
      brd.at(r, c) = BrdIdx(x, y);
    }
  }
  Board<int> visited(n, n, 0);
  vector<vector<char>> ans(n, vector(n, 'a'));
  vector<vector<bool>> onStack(n, vector(n, false));
  auto gc = [](BrdIdx bi) -> char {
    if (bi.r == 1 && bi.c == 0) return 'D';
    if (bi.r == -1 && bi.c == 0) return 'U';
    if (bi.r == 0 && bi.c == 1) return 'R';
    if (bi.r == 0 && bi.c == -1) return 'L';
    return 'X';
  };

  for (ll r = 0; r < n; r++) {
    for (ll c = 0; c < n; c++) {
      if (visited.at(r, c) == 1) continue;
      BrdIdx thisBI(r, c);
      auto recdfs = [&](auto rF, BrdIdx bi, BrdIdx par) -> void {
	if (visited.at(bi) == 1) return;
	if (brd.at(bi) != brd.at(par)) return;
	visited.at(bi) = 1;
	ans[bi.r][bi.c] = gc(par - bi);
	for (BrdIdx bd : BrdIdx::nbr4) {
	  if (brd.in(bi + bd)) rF(rF, bi + bd, bi);
	}
      };
      if (brd.at(r, c) == bloop) {
	BrdIdx inLoop;
	BrdIdx inLoopPar;
	auto chkLoop = [&](auto rF, BrdIdx bi, BrdIdx par) -> bool {
	  DLOG("  chkLoop", bi.r, bi.c, par.r, par.c);
	  if (onStack[bi.r][bi.c]) {
	    inLoop = bi;
	    inLoopPar = par;
	    return true;
	  }
	  if (visited.at(bi) >= 1) return false;
	  if (brd.at(bi) != brd.at(par)) return false;
	  visited.at(bi) = 2;
	  for (BrdIdx bd : BrdIdx::nbr4) {
	    if (brd.in(bi + bd)) {
	      onStack[bi.r][bi.c] = true;
	      bool b = rF(rF, bi + bd, bi);
	      onStack[bi.r][bi.c] = false;
	      if (b) return true;
	    }
	  }
	  return false;
	};
	if (!chkLoop(chkLoop, thisBI, thisBI)) {
	  DLOG("chkLoop returns false");
	  cout << "INVALID\n";
	  return 0;
	}
	recdfs(recdfs, inLoop, inLoop);
	ans[inLoop.r][inLoop.c] = gc(inLoopPar - inLoop);
      }else if (brd.at(r, c) == BrdIdx(r, c)) {
	recdfs(recdfs, thisBI, thisBI);
	ans[r][c] = 'X';
      }
    }
  }
