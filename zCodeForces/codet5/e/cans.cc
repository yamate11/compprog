...
int main(/* int argc, char *argv[] */) {
  ...
  vector pts(K, vector<pll>());
  REP(i, 0, N) pts[X[i]].emplace_back(Y[i], C[i]);
  
  ll big = 1e18;
  using OP = ll;
  auto add = [](ll x, ll y) -> ll { return min(x, y); };
  auto comp = [](OP f, OP g) -> OP { return f + g; };
  auto appl = [](OP f, ll x) -> ll { return f + x; };
  auto st = make_seg_tree_lazy<ll, OP>(LLONG_MAX, OP(), add, comp, appl, vector<ll>(K + 1, big));
  st.update(0, 1, -big);
  REPrev(i, K - 1, 0) {
    st.update(K - i, K - i + 1, -big + st.query(0, K - i));
    st.update(0, K - i, A);
    for (auto [y, c] : pts[i]) st.update(y + 1, K - i + 1, c);
  }
  ll ans = st.query(0, K + 1);
  cout << ans << endl;
