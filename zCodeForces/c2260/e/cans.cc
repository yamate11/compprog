...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N, Q; cin >> N >> Q;
    string S; cin >> S;
    vector cntF(4, vector<ll>(N));
    REP(i, 0, N - 1) {
      ll x0 = ((S[i] - '0') << 1) + (S[i + 1] - '0');
      REP(x, 0, 4) cntF[x][i + 1] = cntF[x][i] + (x == x0 ? 1 : 0);
    }
    DLOGK(cntF);
    REP(_q, 0, Q) {
      ll l, r; cin >> l >> r; l--;
      vector C(4, 0LL);
      REP(x, 0, 4) C[x] = cntF[x][r - 1] - cntF[x][l];
      ll w = ((S[r - 1] - '0') << 1) + (S[l] - '0');
      C[w]++;
      DLOGK(_q, l, r, w, C);
      assert(C[1] == C[2]);
      auto f = [&](ll x, ll y, ll z) -> ll {
        if (z >= x and z >= y) return (z - x) + (z - y);
        if (z >= y or x - y >= y - z) {
          ll k = divCeil(x - z, 2);
          x -= k;
          z += k;
        ...
        z += 2 * m;
        if (x == z) return k + 2 * m;
        else if (x == z + 1) return k + 2 * m + 2;
        else if (x == z + 2) return k + 2 * m + 4;
        else assert(0);
      };
      ll ans = C[0] >= C[3] ? f(C[0], C[3], C[1]) : f(C[3], C[0], C[1]);
      cout << ans << "\n";
    }
