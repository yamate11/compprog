...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    vector<ll> C(N);
    REP(i, 0, N) {
      ll g1 = i == 0 ? 1 : gcd(A[i - 1], A[i]);
      ll g2 = i + 1 == N ? 1 : gcd(A[i + 1], A[i]);
      ll l = lcm(g1, g2);
      C[i] = l <= B[i] ? l : A[i];
    }
    ll proh = -1;
    REP(i, 0, N) {
      ...
      for (auto p : primes) {
        ...
        if (okL() and okR()) cands.push_back(p);
        if (ssize(cands) == 2) break;
      }
      DLOGK(i, cands);
      if (cands.empty()) proh = -1;
      else if (ssize(cands) == 1) {
        proh = cands[0];
        ans++;
      }else {
        proh = -1;
