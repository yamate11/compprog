...
int main(/* int argc, char *argv[] */) {
  ...
  ll n, m; cin >> n >> m;
  // @InpMVec(n, ((l, dec=1), r, pp, qq)) [NwVmb300]
  auto l = vector(n, ll());
  auto r = vector(n, ll());
  auto pp = vector(n, ll());
  auto qq = vector(n, ll());
  for (int i = 0; i < n; i++) {
    ...
    ll v2; cin >> v2; r[i] = v2;
    ll v3; cin >> v3; pp[i] = v3;
    ll v4; cin >> v4; qq[i] = v4;
  }
  // @End [NwVmb300]

  auto sidx = sortedIndex(l);

  vector<Fp> prob(n);
  REP(i, 0, n) prob[i] = Fp(pp[i]) / Fp(qq[i]);
  auto dummy = [](Fp x, Fp y) -> Fp { return x; };
  vector<Fp> init_vec(m + 1, Fp(0));
  init_vec[0] = Fp(1);
  auto st = make_seg_tree_lazy(Fp(0), Fp(1), dummy, multiplies<Fp>(), multiplies<Fp>(), init_vec);
  
  REP(ii, 0, n) {
    ll i = sidx[ii];
    Fp p0 = st.at(l[i]);
    st.update(0, m + 1, Fp(1) - prob[i]);
    Fp p1 = st.at(r[i]);
    st.rs(r[i]) = p1 + p0 * prob[i];
  }
  cout << st.at(m) << endl;
