...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  vector<ll> D(N), acc(N+1);
  Comb cb(N);
  for (ll i = 0; i < N; i++) {
    cin >> D[i];
  }
  sort(D.begin(), D.end());
  for (ll i = 0; i < N; i++) acc[i+1] = acc[i] + D[i];
  for (ll i = 0; i < M; i++) {
    ll a, b; cin >> a >> b;
    auto check = [&](ll n) -> bool {return D[n] >= b;};
    ll n = binsearch<ll>(check, N, -1);
    ll numBig = N - n;
    ll sumSmall = acc[n];
    ll sumBig = acc[N] - acc[n];
    if (numBig < a) {
      cout << 0 << "\n";
      continue;
    }
    Fp eBig = Fp(sumBig) * Fp(numBig - a) / Fp(numBig);
    Fp eSmall = Fp(sumSmall) * Fp(numBig + 1 - a) / Fp(numBig + 1);
    cout << eBig + eSmall << "\n";
  }
