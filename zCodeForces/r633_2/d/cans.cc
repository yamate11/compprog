...
int main(int argc, char *argv[]) {
  ...
  ll root = 0;
  while (chk.at(root) == 1) root++;
  Tree tr(move(ve), root);
  bool even_leaf = false;
  bool odd_leaf = false;
  vector<ll> num_leaf_children(N);
  auto dfs = [&](const auto& recF, ll n) -> void {
    if (tr.child.at(n).size() == 0) {
      if (tr.depth.at(n) % 2 == 0) even_leaf = true;
      else                         odd_leaf  = true;
      num_leaf_children.at(tr.parent.at(n))++;
    }else {
      for (ll c : tr.child.at(n)) recF(recF, c);
    }
  };
  dfs(dfs, root);
  ll ans_least = even_leaf && odd_leaf ? 3 : 1;
  ll cnt = N-1;
  for (ll i = 0; i < N; i++) {
    ll x = num_leaf_children.at(i);
    if (x > 0) cnt -= x - 1;
  }
  cout << ans_least << " " << cnt << "\n";
