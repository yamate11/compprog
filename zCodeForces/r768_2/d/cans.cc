...
int main(/* int argc, char *argv[] */) {
  ...
  auto pout = [&](ll x, ll y) -> void { cout << x << " " << y << "\n"; };

  auto solve = [&]() -> void {
    ll N, K; cin >> N >> K;
    vector<ll> A(N); REP(i, N) cin >> A[i];
    vector<ll> C(N + 1); REP(i, N) C[A[i]]++;
    auto check_d = [&](ll d) -> ll {
      ll v = 0;
      ll i = 0;
      for (; i < d; i++) v += C[i];
      for (;; i++ ) {
        if (v >= N - v + K) return i - d;
        if (i >= N + 1) break;
        v += C[i] - C[i - d];
      }
      return -1;
    };
    auto check = [&](ll d) -> bool { return check_d(d) >= 0; };
    ll d0 = binsearch_i<ll>(check, N, 0);
    ll x = check_d(d0);
    ll y = x + d0 - 1;
    cout << x << " " << y << "\n";
    vector<ll> rec;
    ...
    if (K == 1) pout(1, N);
      ...
      REP(i, K - 1) {
        if (i == 0) pout(1,              rec[i] + 1);
        else        pout(rec[i - 1] + 2, rec[i] + 1);
      }
      pout(rec[K - 2] + 2, N);
    }
  };
