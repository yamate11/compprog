...
int main(/* int argc, char *argv[] */) {
  ...
  for (ll n = 0; n < N; n++) {
    ...
    dist[n][n] = 0;
    for (ll i = 0; i <= n; i++) pque.emplace(dist[i][n], i);
    while (!pque.empty()) {
      auto [d, x] = pque.top(); pque.pop();
      if (dist[n][x] < d) continue;
      for (auto [y, w] : nbr[x]) {
	if (dist[n][y] <= d + w) continue;
	dist[n][y] = dist[y][n] = d + w;
	pque.emplace(d + w, y);
      }
    }
  }
  ...
  for (auto [u, v, w] : edges) {
    ...
    for (ll i = 0; i < K; i++) {
      ll x = dist[A[i]][B[i]];
      ll y = dist[A[i]][u] + dist[v][B[i]];
      ll z = dist[A[i]][v] + dist[u][B[i]];
      ll d = min(x, min(y, z));
      s += d;
    }
    ans = min(ans, s);
