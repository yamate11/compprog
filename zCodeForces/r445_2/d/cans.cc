...
int main(/* int argc, char *argv[] */) {
  ...
  ll n; cin >> n;
  vector<char> nxt(127, -1);
  vector<char> prv(127, -1);
  vector<string> S(n);
  vector<bool> used(127);
  for (ll i = 0; i < n; i++) {
    cin >> S[i];
  ...
  vector<bool> follow(127);
  for (char c = 'a'; c <= 'z'; c++) {
    if (nxt[c] >= 0) follow[nxt[c]] = true;
  }
  vector<ll> tops;
  for (char c = 'a'; c <= 'z'; c++) {
    if (used[c] && !follow[c]) tops.push_back(c);
  }
  sort(tops.begin(), tops.end());
  string ans;
  for (char c : tops) {
    for (char d = c; d >= 0; d = nxt[d]) ans += d;
  }
  vector<bool> inans(127);
  for (char c: ans) inans[c] = true;
  for (char c = 'a'; c <= 'z'; c++) {
    if (used[c] && !inans[c]) {
      DLOGKL("NO", c);
      cout << "NO\n";
