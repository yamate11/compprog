...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ...
    while (ic.get()) {
      vector dry(N, true);
      vector vec(M, true);
      REP(i, 0, K) vec[ic.at(i)] = false;
      REP(i, 0, M) if (vec[i]) REP(j, L[i], R[i]) dry[j] = false;
      ll cnt = 0;
      REP(i, 0, N) if (dry[i]) cnt++;
      ans = max(ans, cnt);
    }
    cout << ans << "\n";
  };
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    // @End [o0ereLF4]
    vector P(N + 1, vector<ll>());
    REP(i, 0, M) P[R[i]].push_back(L[i]);
    REP(i, 0, N + 1) sort(ALL(P[i]));
    vector tbl(N + 1, vector(K + 1, 0LL));
    REP(i, 1, N + 1) {
      ll m = SIZE(P[i]);
      DLOGK(i, P[i], m);
      REP(k, 0, K + 1) {
        if (k - m >= 0) updMax(tbl[i][k], 1 + tbl[i - 1][k - m]);
        REP(l, 0, m) {
          if (k - l >= 0) updMax(tbl[i][k], tbl[P[i][l]][k - l]);
          else break;
        }
      }
      DLOGK(i, tbl[i]);
