...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    auto satpat = [&](ll n, ll p) -> bool { return popcount((u64)(n & p)) % 2 == 0; };

    auto ask = [&](const auto& vec) -> ll {
      if (--rem_budget < 0) throw myexc();
      cout << "? " << ssize(vec);
      for (ll n : vec) cout << " " << n + 1;
      cout << endl;
      ll x; cin >> x;
      if (x < 0) {
        cerr << "Response negative";
        assert(0);
      }
      return x;
    };

    auto report = [&](ll x, ll y, ll z) -> void {
      cout << "! " << x + 1 << " " << y + 1 << " " << z + 1 << endl;
    };

    auto getinfo = [&](ll p) -> ll {
      try {
        vector<ll> s_in;
        vector<ll> s_out;
        REP(n, 0, 2*N + 1) {
          if (satpat(n, p)) s_in.push_back(n);
          else s_out.push_back(n);
        }
        if (s_in.empty()) return 0;
        if (s_out.empty()) return 3;
        ll a_in = ask(s_in);
        if ((ssize(s_in) - a_in) % 2 != 0) return 3;
        ll a_out = ask(s_out);
        if ((ssize(s_out) - a_out) % 2 != 0) return 0;
        assert(a_in != a_out);
        if (a_in > a_out) return 1;
        else return 2;
      }catch(myexc& e) {
        return -1;
      }
    ...
    vector<ll> pat;
    REP(x, 1, 1LL << W) pat.push_back(x);
    rand.shuffle(ALL(pat));
    vector<ll> has(ssize(pat), -1LL);
    ll effsz = ssize(pat);
    REP(i, 0, ssize(pat)) {
      if (ll rc = getinfo(pat[i]); rc >= 0) {
        has[i] = rc;
      }else {
        effsz = i;
        break;
    ...
    REP(i, 0, effsz) {
      ll p = pat[i];
      if (has[i] == 0) has_zero.emplace_back(p, 0);
      else if (has[i] == 3) has_zero.emplace_back(p, 1);
    }
    DLOGK(pat);
    DLOGK(has);
    DLOGK(has_zero);
    ...
    REP(n, 0, 2*N + 1) {
      auto check = [&]() -> bool {
        for (auto [p, b] : has_zero) {
          if (b == 0 and     satpat(n, p)) return false;
          if (b == 1 and not satpat(n, p)) return false;
        }
        return true;
      };
      if (check()) one_cand.push_back(n);
    }
    vector<pll> two_cand;
    REP(a, 0, ssize(one_cand)) REP(b, a + 1, ssize(one_cand)) {
      ll n = one_cand[a];
      ll m = one_cand[b];
      auto check = [&]() -> bool {
        REP(i, 0, effsz) {
          ll p = pat[i];
          if (has[i] == 1 and     satpat(n, p) and     satpat(m, p)) return false;
          if (has[i] == 2 and not satpat(n, p) and not satpat(m, p)) return false;
        }
        return true;
      };
      if (check()) two_cand.emplace_back(n, m);
    }
    DLOGK(one_cand);
    DLOGK(two_cand);
    ll x0 = -1, y0 = -1, z0 = -1;
    for (ll x : one_cand) {
      for (auto [y, z] : two_cand) {
        if (x >= y) continue;
        auto check = [&]() -> bool {
          REP(i, 0, effsz) {
            ll p = pat[i];
            ...
            if (satpat(y, p)) cnt++;
            if (satpat(z, p)) cnt++;
            if (has[i] != cnt) return false;
          }
          return true;
        };
        if (check()) {
          if (x0 >= 0) {
            cerr << "budget is too small";
            assert(0);
          }
          x0 = x;
          y0 = y;
