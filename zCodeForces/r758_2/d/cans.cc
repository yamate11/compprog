...
int main(/* int argc, char *argv[] */) {
  ...
  ll N; cin >> N;
  Comb cb(2*N);
  ll BB = 0, BW = 0, BQ = 0, WB = 0, WW = 0, WQ = 0, QB = 0, QW = 0, QQ = 0;
  REP(i, N) {
    string s; cin >> s;
    if (s == "BB") BB++;
    else if (s == "BW") BW++;
  ...
  Fp ans = 0;
  if (BB + WW + WB + WQ + QB == 0) ans += 1;
  if (BB + WW + BW + BQ + QW == 0) ans += 1;
  
  vector<Fp> vb(BQ + QB + 1), vw(WQ + QW + 1);
  REP(i, BQ + QB + 1) vb[i] = cb.comb(BQ + QB, i);
  REP(i, WQ + QW + 1) vw[i] = cb.comb(WQ + QW, i);
  Pol pb(move(vb)), pw(move(vw));
  auto p1 = pb * pw;
  vector<Fp> vd(2 * QQ + 1);
  REP(i, 2*QQ + 1) vd[i] = cb.comb(2*QQ, i);
  Pol pd(move(vd));
  auto p2 = p1 * pd;
  ll idx = WQ + QW + QQ + WW - BB;
  if (idx >= 0 and idx <= p2.degree()) {
    Fp z = p2.getCoef(idx);
    ans += z;
    if (BB + WW == 0) {
      ans -= power<Fp>(Fp(2), QQ);
    }
  }
  cout << ans << endl;
