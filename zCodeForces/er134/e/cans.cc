...
// @@ !! LIM(strSearch debug f:updMaxMin)
...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, 1, N) {
    if (i + info[i] == N) {
      REP(j, 1, 11) {
        if (info[i] + j > SIZE(S)) break;
        string z = S.substr(info[i], j);
        mp[z] = max(mp[z], info[i] + j);
      }
    }
  }
  DLOGK(mp);

  auto isRep = [&](string s, ll k) -> bool {
    REP(i, k, SIZE(s)) if (s[i] != s[i - k]) return false;
    return true;
  };
  ...
  REP(_q, 0, Q) {
    ...
    REP(i, 1, SIZE(t) + 1) {
      ...
      DLOGK(x);
      REP(j, 1, i + 1) {
        if (tt.substr(i - j, j) == S.substr(0, j)) {
          updMax(x, j);
          DLOGK(x, j);
        }
      }
