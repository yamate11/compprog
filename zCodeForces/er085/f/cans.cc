...
// @@ !! LIM(segTree binsearch debug)
// --> f:<< segTree binsearch debug
...
int main(int argc, char *argv[]) {
  ...
  DLOG("start");
  for (ll z = 0; z <= M; z++) {
    DLOG(z, st.query(z, z+1), st.query(0, z+1));
  }
#endif
  for (ll i = 0; i < N; i++) {
    DLOG("**** i=", i);
    auto check = [&](ll x) -> bool { return A.at(i) <= B.at(x); };
    ll k = binsearch<ll>(check, M, -1);
    bool upd = false;
    ll s = 0;
    if (k < M && B.at(k) == A.at(i)) {
      s = st.query(k+1, k+2);
      if (-s < min(0LL, P.at(i))) upd = true;
    }
    DLOG("k=", k, "upd=", upd);

    st.update(0, 1, P.at(i));
    if (upd) {
      st.update(k+1, k+2, -(s+P.at(i)));
      if (k+2 < M+1) {
	if (P.at(i) > 0) st.update(k+2, k+3, s);
	else             st.update(k+2, k+3, s+P.at(i));
      }
    }else {
      if (k+1 < M+1) {
	if (P.at(i) > 0) st.update(k+1, k+2, -P.at(i));
      }
    }
#ifdef DEBUG
    for (ll z = 0; z <= M; z++) {
