...
int main(/* int argc, char *argv[] */) {
  ...
  auto g = [&](ll x) -> Fp { return power<Fp>(2, x) - 2; };
  auto f = [&](ll i, u64 X) -> Fp {
    Fp val = 0;
    if (i >= (ll)bit_width(X)) val = 0;
    else {
      u64 lomask = (1ULL << i) - 1;
      u64 himask = (~lomask) ^ (1ULL << i);
      u64 upper = himask & X;
      u64 lower = lomask & X;
      u64 thebit = X & (1ULL << i);
      if (thebit) {
        val = (((upper >> 1) | lower) + 1) * 2;
      }else {
        val = upper;
      }
    }
  ...
  auto solve = [&]() -> void {
    ...
    Fp ans1a = Fp(A + 1) * Fp(B + 1);
    Fp ans1b = Fp(A + 1) * Fp(B + 1) * Fp(B) * g(m) / 2;
    Fp ans2 = 0;
    Fp wa = 0;
    REP(i, 0, 30) {
      Fp ua = f(i, A);
      Fp ub = f(i, B);
      ans2 += ua * g(n) * ub * g(m) / 4;
      ans2 += ua * g(n) / 2 * (B + 1);
      wa += ua;
    }
    Fp ans3 = (Fp(A + 1) * Fp(A) - wa) * g(n) * Fp(B + 1) / 2;
    Fp ans = ans1a + ans1b + ans2 + ans3;
    DLOGK(ans1a, ans1b, ans2, ans3);
    cout << ans << "\n";
  };
