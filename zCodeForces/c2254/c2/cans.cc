...
int main(/* int argc, char *argv[] */) {
  ...
  auto g = [&](const string& s, string& t) -> ll {
    ll i0 = 0, i1 = 0;
    ll m = ssize(s);
    vector<ll> vec(m);
    REP(i, 0, m) {
    ...
    DLOGK(s, t, vec);
    auto segt = make_seg_tree(0LL, plus<ll>(), vector<ll>(m));
    ll ret = 0;
    REP(i, 0, m) {
      ll e = segt.query(vec[i], m);
      ret += e;
      segt.rs(vec[i]) = 1;
    }
    return ret;
  ...
  auto solve = [&]() -> void {
    ...
    auto [ae, ao] = f(a);
    auto [be, bo] = f(b);
    auto aee = ae, aoo = ao, bee = be, boo = bo;
    ranges::sort(ae);
    ranges::sort(ao);
    ranges::sort(be);
    ranges::sort(bo);
    if (ae == be and ao == bo) {
      ll ans = g(aee, bee) + g(aoo, boo);
      cout << ans << "\n";
    }else {
      cout << "-1\n";
    }
