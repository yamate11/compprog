...
// @@ !! LIM(coordCompr f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N; cin >> N;
    // @InpMVec(N, ((X, dec=1), (Y, dec=1))) [MCK4BK7s]
    auto X = vector(N, ll());
    auto Y = vector(N, ll());
    for (int i = 0; i < N; i++) {
    ...
    REP(i, 0, N) cntX[X[i]]++;
    vector<ll> ecX(N);
    REP(i, 1, N) ecX[i] = ecX[i - 1] + (cntX[i - 1] == 0 ? 0 : 1);
    vector<bool> exY(N, false);
    REP(i, 0, N) exY[Y[i]] = true;
    vector<ll> dl(N, N);
    vector<ll> dr(N, -1);
    vector<ll> ul(N, N);
    vector<ll> ur(N, -1);
    ...
    REP(i, 1, N)        { updMin(dl[i], dl[i - 1]); updMax(dr[i], dr[i - 1]); }
    REPrev(i, N - 2, 0) { updMin(ul[i], ul[i + 1]); updMax(ur[i], ur[i + 1]); }
    DLOGK(dl);
    DLOGK(dr);
    DLOGK(ul);
    DLOGK(ur);
    ...
    ll ans = 0;
    REP(y, 0, N - 1) {
      if (ul[y + 1] >= ur[y + 1]) break;
      if (dl[y] >= dr[y]) continue;
      if (not exY[y]) continue;
      ll i0 = max(dl[y], ul[y + 1]);
      ll j0 = min(dr[y], ur[y + 1]);
      if (i0 >= j0) continue;
      ans += ecX[j0] - ecX[i0];
    }
    cout << ans << "\n";
