...
// @@ !! LIM(forall digit f:updMaxMin debug cmpNaive perm)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> ll {
    ...
    // @End [Qei0JkUQ]
    vector<bool> ex(10, false);
    for (ll d : D) ex[d] = true;

    ll ans = big;
    auto check = [&](ll x) -> bool {
      if (x == 0) return ex[0];
      while (x > 0) {
        if (not ex[x % 10]) return false;
        x /= 10;
      }
      return true;
    };
    ll lim = max(A * 10 + 1, 101LL);
    REP(x, 0, lim) {
      if (check(x)) updMin(ans, abs(A - x));
    }
    return ans;
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> ll {
    ...
    if (FORALL(i, 0, w, ex[vecA[i]])) return 0;
    if (w == 1) {
      ll ans = 100;
      for (ll d : D) updMin(ans, abs(A - d));
      REP(i, 0, N) REP(j, 0, N) {
        ll x = 10 * D[i] + D[j];
        updMin(ans, abs(A - x));
      }
    ...
    auto get_up = [&](ll d) -> ll {
      REP(e, d + 1, 10) if (ex[e]) return e;
      return -1;
    };
    auto get_down = [&](ll d) -> ll {
      REPrev(e, d - 1, 0) if (ex[e]) return e;
      return -1;
    };
    {
      REPrev(i, w - 1, 0) {
