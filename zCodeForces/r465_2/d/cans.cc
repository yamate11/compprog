...
int main(/* int argc, char *argv[] */) {
  ...
  auto func = [&](auto rF, ll i) -> Fp {
    if (i == n) return 0;
    if (S1[i] != 0 && S2[i] != 0) {
      if (S1[i] == S2[i]) return rF(rF, i + 1);
    ...
    if (S1[i] == 0 && S2[i] != 0) {
      DLOGK(i, p, m, S2[i]);
      return p / Fp(m) + Fp(m - S2[i]) / Fp(m);
    }else if (S1[i] != 0 && S2[i] == 0) {
      return p / Fp(m) + Fp(S1[i] - 1) / Fp(m);
    }else if (S1[i] == 0 && S2[i] == 0) {
      return p / Fp(m) + (Fp(1) - 1/Fp(m)) / Fp(2);
    }else assert(0);
  };
  cout << func(func, 0) << endl;
