...
int main(/* int argc, char *argv[] */) {
  ...
  BCB.emplace_back(lim, 1e10);

  auto arrange = [&](auto& vx) -> vector<sta> {
    DLOG("vx=", vx);
    sort(vx.begin(), vx.end(), greater<sta>());
    DLOG("sorted vx=", vx);
    ll prev = ll(1e11);
    vector<sta> ret;
    for (auto [x, cx] : vx) {
      if (cx >= prev) continue;
      ret.emplace_back(x, cx);
      prev = cx;
    }
  ...
  for (ll i = 0; i < P; i++) {
    ll x, y, z; cin >> x >> y >> z;
    if (x >= vA[vA.size() - 1].first || y >= vB[vB.size() - 1].first) continue;
  ...
  vector<DAT> init_vec(lim);
  {
    init_vec[0] = -vB[0].second;
    ll q = -vB[0].second;
    ll j = 0;
    for (ll i = 1; i < lim; i++) {
      if (j < (ll)vB.size() && i == vB[j].first) {
	j++;
	q = -vB[j].second;
      }
      init_vec[i] = q;
  ...
  ll weapcost = vA[0].second;
  ll weap = vA[0].first;
  ll i = 0;
  ll ans = -weapcost - vB[0].second;
#if DEBUG
  vector<ll> tmp2(lim);
  for (ll k = 0; k < lim; k++) tmp2[k] = st.query(k, k+1);
  DLOG("tmp2=", tmp2);
#endif
  for (auto [x, y, z] : XYZ) {
    if (x >= weap) {
      i++;
      weap = vA[i].first;
      weapcost = vA[i].second;
    }
    st.update(y, lim, z);
    ll t = st.query(0, lim);
#if DEBUG
    vector<ll> tmp(lim);
    for (ll k = 0; k < lim; k++) tmp[k] = st.query(k, k+1);
    DLOG("x,y,z=", x, y, z, "t=", t, "weapcost=", weapcost);
    DLOG("tmp=", tmp);
#endif
    ans = max(ans, t - weapcost);
  }
  cout << ans << endl;
