...
int main(/* int argc, char *argv[] */) {
  ...
  for (ll i = 0; i < n; i++) {
    ...
    for (ll j = i - 1; j >= 0; j--) {
      a = min(a, A[s[j] - 'a']);
      valid = valid && c_min[j] >= 0 && i - j < a;
      if (valid) {
	t_num += c_num[j];
	t_min = min(t_min, c_min[j]);
    ...
    DLOG("i=", i, "t_num=", t_num, "t_min=", t_min, "longest=", longest);
    DLOG("c_num=", c_num);
    DLOG("c_min=", c_min);
  }
  cout << t_num << endl << longest << endl << t_min << endl;
