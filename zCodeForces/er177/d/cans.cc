...
#endif
  auto solve = [&]() -> void {
    // @InpVec(asz, C) [QXAXAr2A]
    auto C = vector(asz, ll());
    for (int i = 0; i < asz; i++) { ll v; cin >> v; C[i] = v; }
    // @End [QXAXAr2A]
    ...
    REP(i, 0, asz) {
      ...
      tbl = tbl_init;
      REP(m, 0, M + 1) {
        if (m + c <= M) tbl[m + c] += prev[m] * cb.binom(m + c, c);
        ll n = csum - m;
        if (n + c <= sum - M) tbl[m] += prev[m] * cb.binom(n + c, c);
      }
      csum += c;
      DLOGK(i, tbl);
    }
