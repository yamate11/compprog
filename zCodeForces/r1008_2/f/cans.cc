...
// @@ !! LIM(mod f:intDiv convolution debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<Fp> vec1(N + 1);
    REP(r, 0, N + 1) vec1[r] = cb.binom(N, r);
    vector<Fp> vec2(2*N + 1);
    REP(k, 0, N + 1) {
      vec2[N + k] = vec2[N - k] = Fp(divFloor(k, 2)) * Fp(divCeil(k, 2));
    }
    DLOGK(vec1);
    DLOGK(vec2);
    auto vec0 = convolution(vec1, vec2);
    DLOGK(vec0);
