...
struct TrieInfo {
  ...
  /** コンストラクタ */
  TrieInfo(char from_, int br_size_) : from(from_), br_size(br_size_), root(nullptr) {}
};
...
struct Trie {
  ...
  /** コンストラクタ */
  Trie(TrieInfo<T>* info_, Trie* parent_ = nullptr, int depth_ = 0, int cidx_ = -1)
  ...
  static Trie* create_root(char from_, char to_) {
    auto info = new TrieInfo<T>(from_, to_ - from_ + 1);
  ...
  */
  ...
  /** 文字列の存在個数を増減する．負にはならない．その文字列を表すノードを返す */
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N; cin >> N;
    // @InpVec(N, S, type=ull) [u0UXWeDi]
    auto S = vector(N, ull());
    for (int i = 0; i < N; i++) { ull v; cin >> v; S[i] = v; }
    // @End [u0UXWeDi]

    Fp sum;
    auto root = Trie<>::create_root('0', '1');
    DLOGK(sizeof(*root));
    ll K = 0;
    REP(i, 0, N) K = max(K, (ll)(countr_zero(bit_ceil(S[i] + 1))));
    DLOGK(K);
    REP(i, 0, N) root->insert(ull2binstr(S[i], K));
    REP(i, 0, N) {
      string s = ull2binstr(S[i], K);
      DLOGK(i, s);
      ll ones = 0;
      auto p = root;
      REP(k, 0, K) {
        ll thidx = s[k] - '0';
        ll otidx = 1 - thidx;
        auto th = p->children[thidx];
        auto ot = p->children[otidx];
        ll sz = ot == nullptr ? 0 : ot->size;
        sum += (ones + otidx + 1) * sz;
        DLOGK(k, sum);
        if (s[k] == '1') ones++;
        p = th;
