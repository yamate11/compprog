...
int main(/* int argc, char *argv[] */) {
  ...
  ll d; cin >> d;
  Comb cb(d);
  // @InpVec(d + 1, A) [OpVN8LKm]
  auto A = vector(d + 1, ll());
  for (int i = 0; i < d + 1; i++) { ll v; cin >> v; A[i] = v; }
  // @End [OpVN8LKm]
  // @InpVec(d + 1, B) [IAltlgc3]
  auto B = vector(d + 1, ll());
  for (int i = 0; i < d + 1; i++) { ll v; cin >> v; B[i] = v; }
  // @End [IAltlgc3]
  DLOGK(A, B);

  auto func = [&](auto& vec) -> Fp {
    Fp t = 0;
    ll sign = 1;
    REP(i, 0, d) {
      Fp x = cb.binom(d - 1, i) * Fp(vec[d - 1 - i]) * Fp(sign);
      t = t + x;
      sign = -sign;
    }
    return t;
  ...
  auto g = [&](auto& vec) -> Fp {
    ...
    ll sign = 1;
    REP(i, 0, d + 1) {
      Fp x = cb.binom(d, i) * Fp(vec[d - i]) * Fp(sign);
      t = t + x;
      sign = -sign;
    }
    return t;
