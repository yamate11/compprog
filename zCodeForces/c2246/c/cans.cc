...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    if (B[0].first == -1) {
      numNeg = B[0].second;
      REP(i, 1, ssize(B)) C.push_back(B[i]);
    }else {
      numNeg = 0;
      C = move(B);
    }
    Fp posE = 1;
    for (auto [x, n] : C) if (n >= 1) posE *= pow2[n - 1];
    Fp cNeg1 = (numNeg == 0) ? Fp(1) : pow2[numNeg - 1];
    Fp ans1 = cNeg1 * posE;
    ll k = 0;
    REP(i, 0, ssize(C) - 1) {
      if (C[i].first + 1 == C[i + 1].first) k++;
    }
    Fp cNeg2 = (numNeg == 0) ? Fp(0) : cNeg1;
    Fp ans2 = k * cNeg2 * posE;
    Fp ans = ans1 + ans2;
    cout << ans << "\n";
  };
