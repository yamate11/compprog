...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N, M, Q; cin >> N >> M >> Q;
    // @InpMVec(M, ((U, dec=1), (V, dec=1), W)) [NjOSqqrC]
    auto U = vector(M, ll());
    auto V = vector(M, ll());
    auto W = vector(M, ll());
    for (int i = 0; i < M; i++) {
    ...
    // @End [NjOSqqrC]
    // @InpMVec(Q, ((A, dec=1), (B, dec=1), K)) [YWOsMBZs]
    auto A = vector(Q, ll());
    auto B = vector(Q, ll());
    auto K = vector(Q, ll());
    for (int i = 0; i < Q; i++) {
    ...
    auto show_dist = [&]() -> void {
#if DEBUG
      REP(i, 0, N) REP(j, 0, N) cerr << i << " " << j << " " << dist[i][j] << "\n";
#endif      
    };
    ...
    map<tpA, vector<ll>> ump;
    REP(i, 0, Q) ump[tpA(A[i], B[i], K[i])].push_back(i);
    ...
    REP(ii, 0, M) {
      ...
      dist[u][v] = dist[v][u] = 0;
      REP(j, 0, N) REP(k, 0, N) {
        ll x = min(dist[j][u] + dist[v][k], dist[j][v] + dist[u][k]);
        ll new_d = min(dist[j][k], x);
        dist[j][k] = dist[k][j] = new_d;
        auto it = ump.find(tpA(j, k, new_d + 1));
        if (it != ump.end()) {
          for (ll q : it->second) {
            ans[q] = w;
            DLOGK(q, ans[q]);
