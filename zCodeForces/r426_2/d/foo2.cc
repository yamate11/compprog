...
struct SegTree {
  ...
  SegTree(const SegTree& st)
    : unit_dat(st.unit_dat), unit_op(st.unit_op), add(st.add), comp(st.comp),
      appl(st.appl), range_update(st.range_update) {}
...
int main(/* int argc, char *argv[] */) {
  ...
  auto xAppl = [](int kk, OP f, DAT x) -> DAT { return f + x; };
  auto xComp = [](OP h, OP g) -> OP { return h + g; };
  vector<DAT> init_vec(n + 1, 0LL);
  auto _st = make_seg_tree(unit_dat, unit_op, xAdd, xComp, xAppl, true);
  vector<decltype(_st)> sts(k + 1, _st);
  for (ll i = 0; i <= k; i++) sts[i].set_data(init_vec);

  vector<bool> app(n + 1);
  ll app_cnt = 0;
  vector<ll> last_pos(n + 1, -1);
  for (ll i = 0; i < n; i++) {
    ll t = A[i];
    if (!app[t]) {
      app[t] = true;
    ...
    ll lp = last_pos[t];
    for (ll j = 1; j <= k; j++) {
      sts[j].update(lp + 1, i + 1, 1);
    }
    sts[1].update(i, i+1, app_cnt);
    for (ll j = 1; j <= k - 1; j++) {
      ll m = sts[j].query(0, i + 1);
      sts[j + 1].update(i + 1, i + 2, m);
    }
#if DEBUG
    for (ll j = 1; j <= k; j++) {
      cerr << "j=" << j << "  ";
      for (ll ii = 0; ii <= i; ii++) {
        cerr << sts[j].query(ii, ii + 1) << " ";
      }
      cerr << endl;
    }
#else
  ...
  void foo = [&](ll x) -> vector<ll> {
    hello();
  };

  void bar = [&](ll x, ll y) -> ll {
    foo();
  };

  foid foo = [&](ll x, ll y, ll z) -> map<ll, ll> {
    baz();
  };
