...
int main(/* int argc, char *argv[] */) {
  ...
  auto divSieve = divisorSieve(2e6);
  ...
  auto solve =
    [&]() -> bool {
      ...
      if (a == b) {
        ll t = ll(floor(pow((double)a, 0.3333333) + 0.4));
        if (t * t * t == a) return true;
        return false;
      }
      if (a > b) swap(a, b);
      ll t = ll(floor(pow((double)a, 0.25) + 0.4));
      if (t * t * t * t == a && t * t * t * t * t == b) return true;
      ll g = gcd(a, b);
      if (g > ll(2e6) - 1000) {
        for (ll p : primes) {
          ll ca = 0, cb = 0;
  ...
  ll T; cin >> T;
  for (ll t = 0; t < T; t++) cout << (solve() ? "Yes" : "No") << endl;
