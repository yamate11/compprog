...
int main(/* int argc, char *argv[] */) {
  ...
  priority_queue<ll, vector<ll>, decltype(f)> pque(f);
  ll vmax = 0;
  ll maxarg = k-1;
  vector<ll> hist(n, -1);
  for (ll i = 0; i < n; i++) {
  ...
  vector<bool> bag(maxarg + 1, true);
  for (ll i = 0; i <= maxarg; i++) {
    if (hist[i] >= 0) { bag[hist[i]] = false; }
  }
