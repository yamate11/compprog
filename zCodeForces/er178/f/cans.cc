...
int main(/* int argc, char *argv[] */) {
  ...
  auto reg = [&](ll x) -> void {
    ...
    auto it = mp.find(s);
    if (it == mp.end()) mp[s] = x;
    else if (x < it->second) it->second = x;
  };

  REP(k, 0, 8 + 1) {
    IntDupComb dic(10, k);
    while (dic.get()) {
      vector<int> vec = dic.vec_view();
      ll j0 = EXFIND_D(j, 0, k, vec[j] != 0, -1LL);
      if (j0 >= 1) swap(vec[0], vec[j0]);
      ll x = 0;
      REP(i, 0, k) x = x * 10 + vec[i];
      REP(m, 0, 8 + 1) {
        ll y = x * 10 + m;
        REP(p, 0, 9 - (k + 1) + 1) {
          reg(y);
  ...
  vector<ll> tbl;
  for (auto& [_s, x] : mp) if (x > 0) tbl.push_back(x);
  sort(ALL(tbl));
  ...
  REP(t, 0, T) {
    ll n; cin >> n;
    ll ans = lower_bound(ALL(tbl), n + 1) - tbl.begin();
    cout << ans << "\n";
  }
