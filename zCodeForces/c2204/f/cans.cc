...
int main(/* int argc, char *argv[] */) {
  ...
  // @InpVec(M, K) [H2W3qHog]
  auto K = vector(M, ll());
  for (int i = 0; i < M; i++) { ll v; cin >> v; K[i] = v; }
  // @End [H2W3qHog]

  Fp base = 0;
  REP(i, 0, N) { base += Fp(1) / Fp(A[i]) * (i + 1) * (N - i); }
  
  CartesianTree ct(A);
  map<ll, ll> mp;
  auto dfs = [&](auto rF, ll nd, ll lo, ll hi) -> void {
    mp[A[nd]] += (nd - lo + 1) * (hi - nd + 1);
    if (ct.left[ nd] >= 0) rF(rF, ct.left[nd], lo, nd - 1);
    if (ct.right[nd] >= 0) rF(rF, ct.right[nd], nd + 1, hi);
  };
  dfs(dfs, ct.root, 0, N - 1);
  Fp incL = 0;
  Fp incH = 0;
  for (auto [a, v] : mp) {
    if (a == 1) incH += Fp(v);
    else incL += v * (Fp(1) / Fp(a));
  }
  DLOGK(mp);
  Fp ans = base;
  auto it = mp.begin();
  ...
  DLOGK(incL, incH);
  for (ll k : K) {
    for (; it != mp.end() and it->first - 1 <= k; it++) {
      ll k0 = it->first - 1;
      ll dk = k0 - prev_k;
      ans += (incL + incH) * dk;
      incH += Fp(it->second) * Fp(1);
      incL -= Fp(it->second) * (Fp(1) / Fp(k0 + 1));
      prev_k = k0;
      DLOGK(k0, ans, incL, incH);
    }
    ans += (incL + incH) * (k - prev_k);
    prev_k = k;
    DLOGK(k, ans);
    cout << ans << "\n";
  }
