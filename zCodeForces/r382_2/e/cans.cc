...
int main(/* int argc, char *argv[] */) {
  ...
  using sta = pair<vector<Fp>, vector<Fp>>;
  auto dfs = [&](auto rF, ll nd, ll pt) -> sta {
    auto black = vector<Fp>(k + 1);
    auto white = vector<Fp>(k + 1);
    black[0] = 1; white[0] = 1;
    for (ll cld : nbr[nd]) {
      if (cld == pt) continue;
      auto [blc, whc] = rF(rF, cld, nd);
      auto blp = move(black);
      auto whp = move(white);
      black = vector<Fp>(k + 1);
      white = vector<Fp>(k + 1);
      for (ll i = 0; i <= k; i++) {
        for (ll j = 0; j <= k; j++) {
          black[min(i, j + 1)] += blp[i] * blc[j];
          ll p = max(i, j + 1);
          if (p <= k) white[p] += whp[i] * whc[j];
          if (i + j + 1 <= k) black[i] += blp[i] * whc[j];
          else if (j + 1 <= k) white[j + 1] += blp[i] * whc[j];
          if (i + j + 1 > k) white[i] += whp[i] * blc[j];
          else if (j + 1 <= k) black[j + 1] += whp[i] * blc[j];
        }
        white[k] = 0;
      }
    }
    DLOGK(nd, black, white);
    return {move(black), move(white)};
  };
