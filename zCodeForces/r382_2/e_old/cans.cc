...
int main(/* int argc, char *argv[] */) {
  ...
  vector init2(k + 2, vector(k + 1, Fp(0)));
  vector _tbl(n, init2);
  auto atbl = [&](ll nd, ll b, ll w) -> Fp& { return _tbl[nd][b + 1][w + 1]; };
  auto _tmp = init2;
  auto atmp = [&](ll b, ll w) -> Fp& { return _tmp[b + 1][w + 1]; };
  ll big = 1000;
  auto dfs = [&](auto rF, ll nd, ll pt) -> void {
    ...
    for (ll m : nbr[nd]) {
      ...
      for (ll i = -1; i <= k; i++) {
	for (ll j = -1; j < k; j++) {
	  atbl(nd, 0, -1) += atmp(0, -1) * atbl(m, i, j);
	  for (ll p = -1; p <= k; p++) {
	    if (p == 0) continue;
	    for (ll q = -1; q < k; q++) {
	      ll p0 = p == -1 ? big : p;
	      ...
	      ll black = min(p0, i0);
	      if (black > k) black = -1;
	      ll q1 = (q == -1 || q + i0 <= k) ? -1 : q;
	      ll j1 = (j == -1 || j + 1 + p0 <= k) ? -1 : j + 1;
	      ll white = max(q1, j1);
	      if (white < k) {
		atbl(nd, black, white) += atmp(p, q) * atbl(m, i, j);
	      }
	    }
	  }
	}
  ...
  dfs(dfs, 0, -1);
  Fp ans = 0;
  for (ll i = -1; i <= k; i++) ans += atbl(0, i, -1);
  cout << ans << endl;
