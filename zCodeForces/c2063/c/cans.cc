...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    vector deg(N, pll());
    DLOGK(tr.num_children(0));
    deg[0] = pll(tr.num_children(0), 0);
    REP(i, 1, N) deg[i] = pll(tr.num_children(i) + 1, i);
    sort(ALL(deg), greater<pll>());
    ll maxd = deg[0].first;

    auto conn = [&](ll u, ll v) -> bool { return tr.parent(u) == v or tr.parent(v) == u; };

    ll u = deg[0].second, v = deg[1].second;
    DLOGK(deg, maxd, u, v);
    ll ans;
    if (N == 2) {
      ans = 0;
    }else if (deg[2].first == maxd) {
      DLOG("A");
      ans = 1 + 2 * (maxd - 1);
    }else if (deg[1].first == maxd) {
      if (conn(u, v)) {
        DLOG("B");
        ans = 1 + (maxd - 1) + (maxd - 2);
      }else {
      ...
      ll t = 1;
      ll nxtmaxd = deg[1].first;
      while (t < N and nxtmaxd == deg[t].first) t++;
      if (EXISTS(x, 1, t, not conn(u, deg[x].second))) {
        DLOG("D");
        ans = 1 + (maxd - 1) + (nxtmaxd - 1);
      }else {
        DLOG("E");
        ans = 1 + (maxd - 1) + (nxtmaxd - 2);
      }
    }
    cout << ans << "\n";
  };
