...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N; cin >> N;
    safe_umap<ll, Fp> tblF;
    safe_umap<ll, Fp> tblG;
    tblF[0] = tblF[1] = tblF[2] = tblG[0] = tblG[1] = tblG[2] = 0;
    tblF[3] = tblG[3] = 1;
    auto f = [&](auto f0, auto g0, ll n) -> Fp {
      auto [it, b] = tblF.try_emplace(n, 0);
      Fp& r = it->second;
      if (b) r = g0(g0, (n - 3) / 4) + g0(g0, (n - 1) / 4) + 1;
      return r;
    };
    auto g = [&](auto g0, ll n) -> Fp {
      auto [it, b] = tblG.try_emplace(n, 0);
      Fp& r = it->second;
      if (b) {
        r = Fp(0);
        for (ll k = 1; n / k >= 3; k *= 2) r += f(f, g0, n / k);
      }
      return r;
    };
    cout << g(g, N) << "\n";
    DLOGK(ssize(tblF), ssize(tblG));
  };
