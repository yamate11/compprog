...
// @DefStruct(sta, (v, n, i), ord=(v, n, i)) [w4B4NlNd]
struct sta {
  ll v{};
  ll n{};
  ll i{};
  string show() const {
    return "(" + to_string(v) + ", " + to_string(n) + ", " + to_string(i) + ")";
  }
  strong_ordering operator<=>(const sta& o) const {
    if (auto r_ = v <=> o.v; r_ != 0) return r_;
    if (auto r_ = n <=> o.n; r_ != 0) return r_;
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    // @End [NLoEsmFu]
    // @InpGrid(N, M, A) [iypN6pK5]
    auto A = vector(N, vector(M, ll()));
    for (int i = 0; i < N; i++) for (int j = 0; j < M; j++) { ll v; cin >> v; A[i][j] = v; }
    // @End [iypN6pK5]
    vector<sta> B(N*M);
    REP(i, 0, N) REP(j, 0, M) B[i*M + j] = sta(A[i][j], 1, i);
    ranges::sort(B, greater<sta>());
    vector pos(N, vector<ll>());
    REP(i, 0, N*M) pos[B[i].i].push_back(i);
    /*
    vector<ll> init_vec(N*M);
    REP(i, 0, N*M) init_vec[i] = B[i].first;
    */
    auto unit = sta{0, 0, 0};
    auto add = [&](const sta& a, const sta& b) -> sta { return sta(a.v + b.v, a.n + b.n, a.i + b.i); };
    auto segt = make_seg_tree(unit, add, B);
    ll ans = M;
    REP(i, 0, N) {
      auto check = [&](const sta& sum) -> bool { return sum.v >= V[i]; };
      auto x = segt.binsearch_r_from(check, 0);
      ans = min(ans, segt.query(0, x).n);
      for (ll p : pos[i]) segt.rs(p) = unit;
    }
    cout << ans << "\n";
