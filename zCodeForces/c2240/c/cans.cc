...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ...
    ll N = 4;
    ll M = 10;
    vector tbl(M, vector(M, vector(M, vector(M, false))));
    IntDupPerm idp(M, N);
    while (idp.get()) {
      ll cnt = 0;
      REP(i, 0, N) if (idp[i] > 0) cnt++;
      if (cnt <= 1) {
        tbl[idp[0]][idp[1]][idp[2]][idp[3]] = true;
      }else {
        auto check = [&]() -> bool {
          vector v = idp.vec_view();
          REP(j, 0, N) v[j]++;
          IntDirProd iii(v);
          while (iii.get()) {
            ll s = 0;
            ...
            if (s > 0 and t == 0) {
              auto w = idp.vec_view();
              REP(i, 0, N) w[i] -= iii[i];
              if (tbl[w[0]][w[1]][w[2]][w[3]]) return false;
            }
          }
          return true;
        };
        tbl[idp[0]][idp[1]][idp[2]][idp[3]] = check();
      }
      if (tbl[idp[0]][idp[1]][idp[2]][idp[3]] == 1) {
        cerr << idp[0] << " " <<  idp[1] << " " <<  idp[2] << " " << idp[3] << ": " <<  tbl[idp[0]][idp[1]][idp[2]][idp[3]] << endl;
      }
    }
  };
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    ll t = 64 - countl_zero((u64)s) - 1;
    DLOGK(t);
    Fp ans = 0;
    REP(i, 0, N) if (A[i] >> t & 1) ans += 1;
    cout << ans << "\n";
  };
