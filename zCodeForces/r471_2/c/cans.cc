...
// @@ !! LIM(sieve debug binsearch f:power)
// --> sieve f:<< debug binsearch f:power
...
int main(/* int argc, char *argv[] */) {
  ...
  auto func = [&vec](ll n) -> ll {
    if (n == 0) return 0;
    ll ans = 0;
    for (auto [x, k] : vec) {
      double tt = pow(n, 1.0 / x);
      ll q = llround(tt);
      ll t;
      if (abs(tt - (double)q) > 0.01) {
        t = llround(floor(tt));
      }else {
        if (power<ll>(q, x) <= n) {
          t = q;
  ...
  for (ll x = 2; x <= 60; x++) {
    ...
    if (succ) vec.emplace_back(x, (ll)pr.size());
  }
