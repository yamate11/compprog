...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    vector<ll> revA(N); REP(i, N) revA[A[i]] = i;

    if (N == 1) return 0;
    if (N == 2) return 1;

    vector<pll> stackLow;
    stackLow.emplace_back(N, -1);
    vector<ll> recLow(N);
    REP2R(i, N - 1, 0) {
      while (true) {
        auto [j, v] = stackLow.back();
        if (A[i] > v) {
          recLow[i] = j;
          stackLow.emplace_back(i, A[i]);
          break;
        }else {
          stackLow.pop_back();
        }
    ...
    vector<pll> stackHigh;
    stackHigh.emplace_back(N, N);
    vector<ll> recHigh(N);
    REP2R(i, N - 1, 0) {
      while (true) {
        auto [j, v] = stackHigh.back();
        if (A[i] < v) {
          recHigh[i] = j;
          stackHigh.emplace_back(i, A[i]);
          break;
        }else {
          stackHigh.pop_back();
        }
    ...
    const DAT unit_dat_min = LLONG_MAX;
    const OP unit_op = nullopt;
    auto xAddMin = [](DAT x, DAT y) -> DAT { return min(x, y); };
    auto xApplMin = [](int k, OP f, DAT x) -> DAT { return f.value_or(x); };
    auto xCompMin = [](OP h, OP g) -> OP { return h.has_value() ? h : g; };
    auto stMin = make_seg_tree(unit_dat_min, unit_op, xAddMin, xCompMin, xApplMin, false);
    stMin.set_data(A);
    
    const DAT unit_dat_max = LLONG_MIN;
    auto xAddMax = [](DAT x, DAT y) -> DAT { return max(x, y); };
    auto xApplMax = [](int k, OP f, DAT x) -> DAT { return f.value_or(x); };
    auto xCompMax = [](OP h, OP g) -> OP { return h.has_value() ? h : g; };
    auto stMax = make_seg_tree(unit_dat_max, unit_op, xAddMax, xCompMax, xApplMax, false);
    stMax.set_data(A);
    ...
    while (i < N - 1) {
      if (A[i] < A[i + 1]) {
        ll j = recLow[i];
        ll v = stMax.query(i, j);
        i = revA[v];
      }else {
        ll j = recHigh[i];
        ll v = stMin.query(i, j);
        i = revA[v];
      }
      ans++;
    }
