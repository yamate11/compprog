...
// @@ !! LIM(debug sieve cmpNaive forall)
...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> bool  {
    ...
    sort(ALL(B));
    vector<ll> vec;
    for (ll a : A) vec.push_back(a % ans);
    sort(ALL(vec));
    if (FORALL(i, 0, N, vec[i] == B[i])) return true;
    else return false;
  };

  ll T; cin >> T;
  if (FORALL(t, 0, T, solve())) cout << "OK\n";
  else cout << "NG\n";
...
int body(istream& cin, ostream& cout) {
  ...
  auto divSieve = divisorSieve((ll)1e6);
  ...
  auto solve = [&]() -> ll  {
    ...
    ll minB = mpB.begin()->first;
    DLOGK(mpA);
    DLOGK(mpB);
    if (mpA.empty()) return lo;
    ll hi = mpA.begin()->first;
    ll gap = 1000;
    ll lim = min(lo + gap, hi);
    ...
    auto check = [&](ll k) -> bool {
      ...
      for (auto [a, c] : mpA) {
        ...
        auto it = cpB.find(x);
        if (it == cpB.end()) {
          DLOGKL("check fails A", k, a);
          return false;
        }
        it->second -= c;
        if (it->second < 0) {
          DLOGKL("check fails B", k, a, it->second);
          return false;
        }
      }
      return true;
    ...
    for (auto [a, c] : mpA) {
      if (a - pr > gap - 10) {
        for (ll d : getDivisorsDivSieve(a - minB, divSieve)) {
          if (lo <= d and d <= hi and check(d)) return d;
        }
      }
      pr = a;
    }
