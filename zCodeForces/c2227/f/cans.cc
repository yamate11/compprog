...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    auto appl = [](OP f, DAT x) -> DAT { return DAT(x.first + f * x.second, x.second); };
    vector<DAT> dat_init(N, pll(0, 1));
    auto stA = make_seg_tree_lazy(DAT(), 0LL, add, comp, appl, dat_init);
    ll orig_move = 0;
    REPrev(i, N - 1, 0) {
      ll s = stA.query(0, A[i]).first;
      orig_move += (N - 1 - i) * A[i] - s;
      stA.update(0, A[i], 1);
    }
    ...
    vector<ll> B(N + 1);
    B[N] = big;
    REPrev(i, N - 1, 0) B[i] = min(A[i], B[i + 1]);
    */
    auto stB = make_seg_tree_lazy(DAT(), 0LL, add, comp, appl, dat_init);
    ll vmax = 0;
    REP(i, 0, N) {
      ll x = stB.at(A[i] - 1).first;
      vmax = max(vmax, x);
      stB.update(0, A[i], 1);
    }
    cout << orig_move + vmax << "\n";
