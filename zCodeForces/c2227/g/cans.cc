...
// @@ !! LIM(debug cmpNaive segTree coordCompr)
...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ...
    auto check_sub = [&](const auto rF, const auto& vec) -> bool {
      if (ssize(vec) == 3) return vec[0] - vec[1] + vec[2] > 0;
      REP(i, 0, ssize(vec) - 2) {
        ll a = vec[i] - vec[i + 1] + vec[i + 2];
        if (a > 0) {
          vector<ll> w;
          REP(j, 0, i) w.push_back(vec[j]);
          w.push_back(a);
          REP(j, i + 3, ssize(vec)) w.push_back(vec[j]);
          if (rF(rF, w)) return true;
        }
      }
      return false;
    };

    auto check = [&](ll st, ll len) -> bool {
      if (len == 1) return true;
      vector<ll> vec;
      REP(i, 0, len) vec.push_back(A[st + i]);
      return check_sub(check_sub, vec);
    };

    ll ans = 0;
    REP(i, 0, N) {
      for (ll len = 1; i + len <= N; len += 2) {
        if (check(i, len)) ans++;
      }
    }
    cout << ans << "\n";
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    auto f = [&](ll st) -> ll {
      ...
      ll sz = cc.size();
      vector<ll> init_vec(sz);
      for (auto x : B) init_vec[cc.c(x)]++;
      auto segt = make_seg_tree(0LL, plus<ll>(), init_vec);
      csum = 0;
      REP(i, 0, ssize(B)) {
        ll val = segt.query(cc.c(csum) + 1, sz);
        DLOGK(st, i, A[i], val, B[i], csum);
        ret += val;
        if (i == ssize(B) - 1) break;
        segt.rs(cc.c(B[i])) = segt.at(cc.c(B[i])) - 1;
        csum += A[st + 2 * i] - A[st + 2 * i + 1];
      }
      return ret;
    };
