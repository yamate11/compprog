...
// @DefStruct(elem, (a, b), idx=idx) [fMzauviX]
struct elem_t {
  ll a;
  ll b;
  int idx;
  // elem_t() {}
  elem_t(ll a_ = ll(), ll b_ = ll()) : a(a_), b(b_) {}
  elem_t(ll a_, ll b_, int idx_) : a(a_), b(b_), idx(idx_) {}
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    sort(ALL(vec), [&](elem_t p, elem_t q) -> bool {
      ll pt = p.a < p.b ? 1 : p.a == p.b ? 0 : -1;
      ll qt = q.a < q.b ? 1 : q.a == q.b ? 0 : -1;
      if (pt != qt) return pt > qt;
      else if (pt == 0) return false;
      else if (pt == 1) return p.a < q.a;
      else if (pt == -1) return p.b > q.b;
      assert(false);
    });
    vector<ll> vals(N);
    {
    ...
    REP(i, 0, N) {
      DLOGK(i);
      if (vec[i].b <= sum_b - sum_a) {
        DLOGK(vec[i]);
        ll d = vec[i].b - vec[i].a;
        ll orig = st.at(i);
        st.update(i, N, -d);
        st.rs(i) = big;
        ll alpha = st.query(0, N);
        DLOGK(alpha);
        vmin = min(vmin, -alpha);
        st.update(i, N, d);
        st.rs(i) = orig;
    ...
    ll ans = (vmin == big) ? -1LL : sum_a + vmin;
    cout << ans << "\n";
  };
