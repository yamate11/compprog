...
auto solve1(ll N, auto& S, ll rx, ll cx) -> tuple<ll, ll, ll, ll> {
  ll cntW = 0, cntB = 0;
  REP(i, 0, N) REP(j, 0, N) {
    if (S[i][j] == '.') cntW++;
    else cntB++;
  ...
  char c0;
  if (cntW < cntB) c0 = '.';
  else if (cntB < cntW) c0 = '#';
  else assert(0);
  ll cnt = min(cntW, cntB);
  DLOGK(cnt);
  Fp::setMod(N);
  Fp rt = 0, ct = 0;
  REP(i, 0, N) REP(j, 0, N) {
  ...
  Fp diff_r = cnt * rx - rt;
  Fp diff_c = cnt * cx - ct;
  DLOGK(rx, rt, cx, ct, diff_r, diff_c);
  if (diff_r == 0 and diff_c == 0) return {0, 0, 0, 0};
  REP(i, 0, N) REP(j, 0, N) {
    ll i0 = ll(diff_r + Fp(i));
    ll j0 = ll(diff_c + Fp(j));
    if (S[i][j] == c0 and S[i0][j0] != c0) {
      DLOGK(i, j, i0, j0);
      return {i, j, i0, j0};
    }
  }
...
auto solve2(ll N, auto& S) -> pair<ll, ll> {
  ll cntW = 0, cntB = 0;
  REP(i, 0, N) REP(j, 0, N) {
    if (S[i][j] == '.') cntW++;
    else cntB++;
  ...
  Fp rx = rt / cnt;
  Fp cx = ct / cnt;
  DLOGK(rt, ct, cnt, rx, cx);
  return {rx, cx};
}

bool myjudge(ll N, auto& S, ll rx, ll cx) {
  auto [a1, a2, a3, a4] = solve1(N, S, rx, cx);
  swap(S[a1][a2], S[a3][a4]);
  auto [b1, b2] = solve2(N, S);
  return rx == b1 and cx == b2;
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 2 and strcmp(argv[1], "comp") == 0) {
    ll N; cin >> N;
    vector<string> S(N);
    REP(i, 0, N) cin >> S[i];
    ll rx, cx; cin >> rx >> cx; rx--; cx--;
    bool b = myjudge(N, S, rx, cx);
    assert(b);
  }

  string phase; cin >> phase;
  ll T; cin >> T;
  for (ll _t = 0; _t < T; _t++) {
    if (phase == "first") {
      ll N; cin >> N;
