...
int main(/* int argc, char *argv[] */) {
  ...
  auto report1 = [&](ll a1, ll a2, ll a3, ll a4) {
    cout << a1 + 1 << " " << a2 + 1 << " " << a3 + 1 << " " << a4 + 1 << endl;
  };
  auto report2 = [&](ll a1, ll a2) {
    cout << a1 + 1 << " " << a2 + 1 << endl;
  };
  ...
  auto solve1 = [&]() -> void {
    ...
    DLOGK(rt, ct, last_r, last_c, cnt, rx, cx);
    rt -= last_r;
    ct -= last_c;
    Fp mod_r = cnt * rx - rt;
    Fp mod_c = cnt * cx - ct;
    report1(last_r, last_c, mod_r, mod_c);
  };
  ...
  ll T; cin >> T;
  for (ll _t = 0; _t < T; _t++) {
    if (phase == "first") solve1();
    else if (phase == "second") solve2();
    else assert(0);
  }
