...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = 0;
  REP(x, 0, 1LL << N) {
    if (__builtin_popcountll(x) != num1) continue;
    ll k = 0;
    ll p = 0, q = 0;
    REP(i, 0, num1) {
      while (A[p] == 0) p++;
...
int body(istream& cin, ostream& cout) {
  ...
  vector<ll> pos1; REP(i, 0, N) if (A[i] == 1) pos1.push_back(i);
  ll num1 = SIZE(pos1);
  vector rmax(num1, 0LL);
  vector lmin(num1, 0LL);
  REP(i, 0, num1) {
    ll c = 0;
    for (ll p = pos1[i], x = pos1[i]; true; ++x, ++p) {
      for (; x < N and A[x] == 1; ++x);
      c += x - p;
      if (x == N or c > K) {
        rmax[i] = p;
  ...
  DLOGK(lmin, rmax);
  vector tbl(num1 + 1, vector(K + 1, Fp(0)));
  tbl[0][0] = Fp(1);
  REP(n, 0, N) {
    REPrev(i, num1 - 1, 0) {
      if (not (lmin[i] <= n and n <= rmax[i])) continue;
      REP(k, 0, K + 1) {
        ll pk = k - abs(pos1[i] - n);
        if (pk >= 0) tbl[i + 1][k] += tbl[i][pk];
      }
    }
  }
  Fp ans = 0;
  for (ll k = K; k >= 0; k -= 2) ans += tbl[num1][k];
  cout << ans << endl;
