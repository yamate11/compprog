...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> hcol(n);  // 0 .. uncolored, -1 .. mixed, >=0 .. that color
  vector<ll> nnc(n); // number of nodes colored in hcol(i)
  vector<ll> cut(k+1);
  auto dfs1 = [&](auto rF, ll nd, ll p) -> void {
	...
	hcol[nd] = -1;
      ...
      }else if (cut[hcol[c]] > 0) {
	if (pending == -1) pending = c;
	else pending = -2;
      }else if (hcol[nd] == 0) {
	hcol[nd] = hcol[c];
	nnc[nd] = nnc[c];
      }else if (hcol[nd] == hcol[c]) {
	nnc[nd] += nnc[c];
      }else {
	hcol[nd] = -1;
      }
    }
    DLOG("nd=", nd, "hcol=", hcol[nd], "nnc=", nnc[nd]);
    if (hcol[nd] == 0 && pending >= 0) {
      hcol[nd] = hcol[pending];
      nnc[nd] = nnc[pending];
    }
    ll cc = hcol[nd];
    if (cc > 1 && nnc[nd] == ccnt[cc]) {
      cut[cc]++;
      DLOG("  cut=", cut[cc]);
    }
  };
