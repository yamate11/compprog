...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ll val = -1;
    vector<ll> D(N + 1);
    auto add = [&](ll p, ll diff) -> void {
      if (p > N) return;
      else if (p <= m - 1) val += diff;
      else D[p] += diff;
    };

    REP(i, 0, N + 1) D[i] = 1;
    REP(k, 0, N) {
      if (A[k] < m and not app[A[k]]) {
        add(0, 1);
        add(A[k], -1);
      }else {
        add(0, 1);
      ...
      if (A[k] <= N) app[A[k]] = true;
      DLOGK(k, m, val, app[m - 1], D);
      while (val >= 0 or app[m - 1]) {
        if (m - 1 >= k + 1) break;
        if (app[m]) {
          add(0, -1);
          add((m - 1) / 2 + 1, 1);
