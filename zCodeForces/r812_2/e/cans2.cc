...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    UnionFind uf(N);
    vector<ll> partner(N, -1LL);
    REP(i, N) REP2(j, i + 1, N) {
      if (uf.leader(i) == uf.leader(j)) continue;
      if (partner[uf.leader(i)] == uf.leader(j)) continue;
      if (A[i][j] == A[j][i]) continue;
      bool swapP = A[i][j] > A[j][i];
      ll i0 = uf.leader(i);
      ll j0 = uf.leader(j);
      ll p = partner[i0];
      ll q = partner[j0];
      ...
      if (p < 0 and q < 0) {
        ...
        if (swapP) {
          uf.merge(i0, q);
          partner[uf.leader(i0)] = j0;
          partner[j0] = uf.leader(i0);
        }else {
          uf.merge(i0, j0);
          partner[uf.leader(i0)] = q;
          partner[q] = uf.leader(i0);
        }
      }else if (p >= 0 and q < 0) {
        if (swapP) {
          uf.merge(j0, p);
          partner[uf.leader(j0)] = i0;
          partner[i0] = uf.leader(j0);
        }else {
          uf.merge(j0, i0);
          partner[uf.leader(j0)] = p;
          partner[p] = uf.leader(j0);
        }
      }else if (p >= 0 and q >= 0) {
        if (swapP) {
          uf.merge(i0, q);
...
#if DEBUG
      DLOGK(i, j);
      REP(ii, N) DLOGK(ii, uf.leader(ii));
      REP(ii, N) DLOGK(ii, partner[ii]);
#endif    
    }
    vector<ll> parity(N, -1);
#if DEBUG
    REP(i, N) DLOGK(i, uf.leader(i));
    REP(i, N) DLOGK(i, partner[i]);
#endif    
    REP(i, N) if (parity[i] < 0) {
      ll a = uf.leader(i);
      ll b = partner[a];
      for (int j : uf.group(a)) parity[j] = 0;
      if (b >= 0) {
        for (int j : uf.group(b)) parity[j] = 1;
      }
    }
    DLOGK(parity);
    vector ans(N, vector(N, 0LL));
    REP(i, N) ans[i][i] = A[i][i];
    REP(i, N) REP2(j, i + 1, N) {
      if ((parity[i] + parity[j]) % 2 == 0) {
        ans[i][j] = A[i][j];
        ans[j][i] = A[j][i];
      }else {
        ans[i][j] = A[j][i];
