...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    UnionFind uf(2*N);
    vector<ll> _partner(2*N);
    REP(i, N) { _partner[i] = N + i; _partner[N + i] = i; }
    auto partner(ll i) -> ll { return _partner[uf.leader(i)]; };
    auto pmerge(ll i, ll j) -> void {
      ll a = uf.merge(i, j);
      ll b = uf.merge(partner(i), partner(j));
      partner[a] = b;
      partner[b] = a;
    };
    REP(i, N) REP2(j, i + 1, N) {
      if (uf.leader(i) == uf.leader(j)) continue;
      if (partner(i) == uf.leader(j)) continue;
      if (A[i][j] == A[j][i]) continue;
      if (A[i][j] < A[j][i]) pmerge(i, j);
      else                   pmerge(i, partner(j));
    }
    vector<ll> parity(N, -1);
    REP(i, N) if (ur.leader(i) == i and parity[i] < 0) {
      parity[i] = 0;
      parity[partner(i)] = 1;
    }
    REP(i, N) {
      REP(j, N) cout << ((i != j and (parity[i] + parity[j]) % 2 != 0) ? A[j][i] : A[i][j]) << " ";
      cout << "\n";
    }
  };
