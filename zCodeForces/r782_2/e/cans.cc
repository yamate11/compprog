...
// @@ !! LIM(input UnionFind forall debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  // @InpMVec(M, ((A, dec=1), (B, dec=1), W)) [vFPivjyx]
  auto A = vector(M, ll());
  auto B = vector(M, ll());
  auto W = vector(M, ll());
  for (int i = 0; i < M; i++) {
  ...
  /*
  // @InpNbrList(N, M, nbr, dec=1, read=W) [AiSJyKwo]
  struct nbr_t {
    int nd;
    ll W;
    nbr_t() {}
  ...
  */
  ...
  ll Q; cin >> Q;
  // @InpMVec(Q, ((U, dec=1), (V, dec=1))) [YrCliGXN]
  auto U = vector(Q, ll());
  auto V = vector(Q, ll());
  for (int i = 0; i < Q; i++) {
  ...
  ll K = 30;
#endif
  vector uf(K, UnionFind(N));
  REP(k, 0, K) {
    REP(i, 0, M) {
      if (W[i] >> k & 1) uf[k].merge(A[i], B[i]);
    }
  }
  vector good(K, vector(N, false));
  REP(i, 0, M) {
    DLOGK(i, W[i], W[i] & 1);
    if (not (W[i] & 1)) {
      REP(k, 1, K) {
        good[k][uf[k].leader(A[i])] = true;
        good[k][uf[k].leader(B[i])] = true;
      }
    }
  }
  DLOGK(good);
  ...
  vector<ll> ans(Q, -1LL);
  REP(i, 0, Q) {
    if (EXISTS(k, 0, K, uf[k].leader(U[i]) == uf[k].leader(V[i]))) {
      cout << 0 << "\n";
    }else if (EXISTS(k, 1, K, good[k][uf[k].leader(U[i])])) {
      cout << 1 << "\n";
    }else {
      cout << 2 << "\n";
    }
