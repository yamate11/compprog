...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ll s = 0; REP(i, 0, N) s += C[i];
    assert(s % N == 0);
    ll t = s / N;

    auto myadd = [&](pll p, pll q) -> pll { return pll(p.first + q.first, p.second + q.second); };
    auto myapply = [&](ll h, pll p) -> pll { return pll(p.first + h, p.second); };
    vector<pll> init_vec(N, pll(0, 1));
    auto st = make_seg_tree_lazy<pll, ll>(pll(0, 0), 0LL, myadd, plus<ll>(), myapply, init_vec);

    vector<ll> A(N, 0LL);
    ll s_one = N - t;
    REPrev(i, N - 1, 1) {
      if (s_one < i + 1) st.update(s_one, i + 1, 1);
      ll w = st.query(i, i + 1).first;
      assert((C[i] - w) % i == 0);
      A[i] = (C[i] - w) / i;
      if (A[i] == 0) s_one--;
    }
