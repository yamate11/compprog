  ...
  auto solve = [&]() -> void {
    ...
    while (ip.get()) {
      auto check = [&]() -> bool {
        REP(i, 0, N) {
          ll j = i;
          while (j - 1 >= 0 and ip[j - 1] > ip[i]) j--;
          ll k = i;
          while (k + 1 < N and ip[k + 1] > ip[i]) k++;
          ll v = (i - j + 1) * (k - i + 1);
...
int body(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ...
    auto f = [&](auto rF, ll st, ll lim) -> Fp {
      
      if (st == lim) {
        DLOGKL("trivial", st, lim);
        return Fp(1);
      }
      Fp nSeq = 1;
      ll left = 1;
      ...
      while (j < lim) {
        if (A[j] % left != 0) {
          DLOGKL("throw: mod not zero", st, j, left);
          throw myexc();
        }
        ll right = A[j] / left;
        ll j1 = j + right;
        DLOGKL("  ", st, j, right, j1);
        if (j1 > lim) {
          DLOGKL("throw: lim over", st, j, lim, j1, left);
          throw myexc();
        }
        auto locNSeq = rF(rF, j + 1, j1);
        nSeq *= cb.binom(j1 - (j + 1) + j - st, j - st) * locNSeq;
        DLOGKL("  ", st, j, locNSeq, j1 - (j + 1), j - st, nSeq);
        left += right;
        j = j1;
      }
      DLOGKL("return", st, lim, nSeq);
      return nSeq;
    };
