...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    // @End [SSgpFFyi]
    ll i0 = ranges::max_element(H) - H.begin();

    H.resize(2*N);
    REP(i, 0, N) H[N + i] = H[i];
    vector<ll> HS(2*N + 1);
    REP(i, 0, 2*N) HS[i + 1] = HS[i] + H[i];
    vector<ll> A(2*N);
    A[i0 + N] = 0;
    vector<ll> stA{i0 + N};
    REPrev(i, i0 + N - 1, i0 + 1) {
      while (H[i] > H[stA.back()]) stA.pop_back();
      A[i] = H[i] * (stA.back() - i) + A[stA.back()];
      stA.push_back(i);
    }
    vector<ll> B(2*N);
    B[i0 + 1] = 0;
    vector<ll> stB{i0};
    REP(i, i0 + 1, i0 + N) {
      while (H[i] > H[stB.back()]) stB.pop_back();
      B[i + 1] = H[i] * (i - stB.back()) + B[stB.back() + 1];
      stB.push_back(i);
    }
    DLOGK(A);
    DLOGK(B);
