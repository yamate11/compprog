...
UnionFind::UnionFind(int s) {
  ...
  for (int i = 0; i < size; i++) { _leader[i] = i; }
...
int UnionFind::merge(int i0, int j0) {
  ...
  else if (ri == rj) _rank[rep]++;
  _leader[other] = rep;
  _gsize[rep] += _gsize[other];
  return rep;
}

int UnionFind::groupSize(int i) {
  return _gsize[leader(i)];
}

const vector<vector<int>>& UnionFind::groups() {
  if (!_groups_built) {
    _groups_built = true;
    _groups = vector(size, vector<int>());
    for (int i = 0; i < size; i++) _groups[leader(i)].push_back(i);
  }
  return _groups;  
}

const vector<int>& UnionFind::group(int i) {
  return groups()[leader(i)];
}
...
int main(/* int argc, char *argv[] */) {
  ...
  for (ll i = 0; i < n; i++) {
    ...
    map<ll, ll> cnt;
    ll vmax = 0;
    for (int x : uf.group(i)) {
      ll v = ++cnt[C[x]];
      vmax = max(vmax, v);
    }
    ll chg = uf.groupSize(i) - vmax;
    ans += chg;
  }
  DLOG("finished");
  cout << ans << endl;
