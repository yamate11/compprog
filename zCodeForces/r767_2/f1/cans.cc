...
int main(/* int argc, char *argv[] */) {
  ...
  vector etbl(lim + 1, vector(lim + 1, false));
  vector vtbl(lim + 1, vector(lim + 1, Fp(0)));

  auto func = [&](auto rF, ll a, ll s) -> Fp {
    bool eff = etbl[a][s];
    Fp& ret = vtbl[a][s];
    if (not eff) {
      etbl[a][s] = true;
