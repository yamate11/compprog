...
int main(/* int argc, char *argv[] */) {
  ...
  Board<ll> cnt(N, M, -1);
  REP(i, 0, N) REP(j, 0, M) {
    if (brd.at(i, j) == '*' or cnt.at(i, j) >= 0) continue;
    ll cc = 0;
    vector<BrdIdx> rec;
    auto dfs = [&](auto rF, BrdIdx bi) -> void {
      if (brd.at(bi) == '*' or cnt.at(bi) == 0) return;
      cnt.at(bi) = 0;
      rec.push_back(bi);
      ll pict = 0;
      for (auto dir : BrdIdx::nbr4) {
        if (brd.at(bi + dir) == '*') pict++;
      }
      cc += pict;
      for (auto dir : BrdIdx::nbr4) rF(rF, bi + dir);
    };
    dfs(dfs, BrdIdx(i, j));
    for (auto bi : rec) cnt.at(bi) = cc;
  }
  REP(k, 0, K) {
    ll x, y; cin >> x >> y; x--; y--;
    cout << cnt.at(x, y) << "\n";
