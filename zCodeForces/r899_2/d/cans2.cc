...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ll K = 20;
    REP(k, 0, K) {
      auto myadd = [&](pll a, pll b) -> pll { return pll(a.first + b.first, a.second + b.second); };
      auto mod = [&](const pll& t, int node, int child) -> pll {
        const auto& [v, ss] = t;
        if ((A[node] >> k & 1) == (A[child] >> k & 1)) return pll(v, ss + 1);
        else return pll(v + ss + 1, ss + 1);
      };
      vector<pll> result = reroot(tr, pll(0, 0), myadd, mod);
      REP(i, 0, N) ans[i] += result[i].first << k;
    }
    REPOUT(i, 0, N, ans[i], " ");
