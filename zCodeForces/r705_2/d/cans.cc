...
// @@ !! LIM(mod sieve debug f:power)
// --> f:gcd f:intDiv mod sieve f:<< debug f:power
...
int main(/* int argc, char *argv[] */) {
  ...
  const ll lim = 5;
#else
  const ll lim = ll(2e5) + 1;
#endif
  ll N, Q; cin >> N >> Q;
  vector<ll> A(N);
  vector<map<ll, ll>> vmp(N);
  vector<multiset<ll>> mset(lim);
  for (ll i = 0; i < N; i++) {
    ll a; cin >> a;
    A[i] = a;
    auto pr = prfac(a, primes);
    for (auto [p, r] : pr) {
      vmp[i][p] = r;
      mset[p].insert(r);
    }
  ...
  for (ll _q = 0; _q < Q; _q++) {
    ...
    for (auto [p, r] : prfac(x, primes)) {
      ...
      ll cur = vmp[i][p];
      if (cur > 0) {
        auto it = mset[p].find(cur);
        assert(it != mset[p].end());
        mset[p].erase(it);
      }
      vmp[i][p] += r;
      mset[p].insert(vmp[i][p]);
      if ((ll)mset[p].size() == N) {
        ll newv = *(mset[p].begin());
        if (newv > old) {
          g = g * power<ll>(p, newv - old);
        }
      }
    }
    DLOGK(vmp);
