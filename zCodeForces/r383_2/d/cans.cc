...
// @@ !! LIM(UnionFind f:updMaxMin debug)
// --> UnionFind f:updMaxMin f:<< debug
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> cur(w + 1, -1LL);
  cur[0] = 0;
  for (ll i = 0; i < n; i++) {
    if (uf.leader(i) != i) continue;
    auto prev = cur;
    auto put = [&](ll ww, ll bb) -> void {
      for (ll j = 0; j + ww <= w; j++) {
	if (prev[j] < 0) continue;
	updMax(cur[j + ww], prev[j] + bb);
      }
      DLOG("ww=", ww, "bb=", bb, "cur=", cur);
    };
    ll sumB = 0, sumW = 0;
    for (ll x : uf.group(i)) {
      sumB += B[x];
    ...
    if (uf.groupSize(i) > 1) put(sumW, sumB);
    DLOG("end of i=", i, "cur=", cur);
  }
  ll ans = 0;
  for (ll i = 0; i <= w; i++) updMax(ans, cur[i]);
  cout << ans << endl;
