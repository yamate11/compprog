...
// @@ !! LIM(f:gcd f:intDiv debug)
// --> f:gcd f:intDiv f:<< debug
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> pair<ll, ll> {
    ...
    if (vx == 0) {
      if (x == 0 && vy > 0) { return {0, m}; }
      if (x == 0 && vy < 0) { return {0, 0}; }
      if (x == n && vy > 0) { return {n, m}; }
      if (x == n && vy < 0) { return {n, 0}; }
      return {-1, -1};
    }
    if (vy == 0) {
      if (y == 0 && vx > 0) { return {n, 0}; }
      if (y == 0 && vx < 0) { return {0, 0}; }
      if (y == m && vx > 0) { return {n, m}; }
      if (y == m && vx < 0) { return {0, m}; }
      return {-1, -1};
    }
    ll ax = vx > 0 ? -x : x;
    ll ay = vy > 0 ? -y : y;
    ...
    if (r == -1) return {-1, -1};
    DLOGK(ax, ay, r);
    ll xx = n * abs(((x + vx * r) / n) % 2);
    ll yy = m * abs(((y + vy * r) / m) % 2);
    return {xx, yy};
  };
