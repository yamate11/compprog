...
// @@ !! LIM(cmpNaive debug UnionFind)
// --> cmpNaive f:<< debug UnionFind
...
int body(istream& cin, ostream& cout) {
  ...
  using c_t = pair<ll, ll>;
  vector<c_t> C(n);
  for (ll i = 0; i < n; i++) C[i] = c_t(A[i], i);
  sort(C.begin(), C.end());
  map<ll, ll> mp;
  ll numLoc = 0;
  ll ans = -1;
  ...
  auto append = [&](ll x) -> void {
    auto it = mp.find(x);
    if (it == mp.end()) mp.emplace_hint(it, x, 1);
    else it->second++;
  };
  ...
  for (ll i = 0; i < n; i++) {
    ...
    sz[j] = 1;
    append(1);
    if (j-1 >= 0) t_merge(j-1, j);
    if (j+1 < n) t_merge(j, j+1);
    DLOGK(i, k, j, sz, mp);
    if (mp.size() == 1) {
