...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll nd) -> void {
    ...
    stack.pop_back();
    stackIdx[nd] = -1;
    DLOGKL("popped", nd, stack);
  };
  vector<bool> visited2(n + 1);
  vector<bool> onstack2(n + 1);
  auto dfs2 = [&](auto rF, ll nd) -> void {
    if (onstack2[nd]) throw MyExc();
    if (nd == ans) return;
    if (visited2[nd]) return;
    visited2[nd] = true;
    onstack2[nd] = true;
    for (ll cld : nxt[nd]) rF(rF, cld);
    onstack2[nd] = false;
