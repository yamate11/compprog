...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    // @End [zbsrbNyV]
    // @InpVec(N, P, dec=1) [zSQxBojX]
    auto P = vector(N, ll());
    for (int i = 0; i < N; i++) { ll v; cin >> v; v -= 1; P[i] = v; }
    // @End [zSQxBojX]
    
    auto segt = make_seg_tree(0LL, plus<ll>(), vector<ll>(N, 0LL));
    vector<ll> ans(N);
    ans[N - 1] = 0;
    segt.rs(P[N - 1]) = A[P[N - 1]];
    set<ll> FF{P[N - 1]};

    REPrev(ii, N - 2, 0) {
      ll i = P[ii];
      segt.rs(i) = A[i];
      auto it = FF.lower_bound(i);
      if (it == FF.begin()) {
        auto [tmp_it, tmp_b] = FF.insert(i);
        assert(tmp_b);
        it = tmp_it;
      }else {
        it--;
      ...
      DLOGK(ii, i, segt.vec_view(), FF, *it);
      ll i0 = *it;
      it++;
      for (; it != FF.end(); ) {
        ll j0 = *it;
        if (segt.query(i0, j0) >= A[j0]) {
          it = FF.erase(it);
        }else {
          break;
        }
