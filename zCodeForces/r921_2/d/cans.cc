...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N, M, K; cin >> N >> M >> K;
    // @InpMVec(M, ((A, dec=1), (B, dec=1), F)) [F5cVxAXF]
    auto AA = vector(M, ll());
    auto BB = vector(M, ll());
    auto F = vector(M, ll());
    for (int i = 0; i < M; i++) {
      ll v1; cin >> v1; v1 -= 1; AA[i] = v1;
      ll v2; cin >> v2; v2 -= 1; BB[i] = v2;
      ll v3; cin >> v3; F[i] = v3;
    }
    // @End [F5cVxAXF]
    ...
    Fp sum = 0;
    Fp third = 0;
    REP(m, 0, M) third += Fp(K) * Fp(2 * F[m] + K - 1) / Fp(2);
    Fp second = 1;
    sum += cb.binom(K, 0) * second * third;
    Fp sf = Fp(0);
    REP(m, 0, M) sf += F[m];
    REP(k, 1, K) {
      Fp diff = sf + Fp(K - k) * Fp(M);
      third -= diff;
      second *= Fp(B - 1);
      sum += cb.binom(K, k) * second * third;
    }
    cout << sum / power<Fp>(B, K) << "\n";
  };
