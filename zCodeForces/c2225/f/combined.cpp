...
// AtCoder Library: https://atcoder.github.io/ac-library
...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> string {
    ...
    while (bb.get()) {
      ...
      ll p = 0;
      REP(i, 0, K) {
        vs.push_back(S.substr(p, L + bb[i]));
        p += L + bb[i];
      }
      DLOGK(bb.vec_view(), vs);
      ranges::sort(vs);
      ans = max(ans, vs.back());
      DLOGK(ans);
    }
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> string {
    ...
    auto func = [&](ll p) -> ll {
      if (p + L > N) return -1;
      if (0 < p and p < L) return -1;
      ll before = p / L;
      ll after = (N - (p + L)) / L;
      if (before + after < K - 1) return -1;
      ll a = K - 1 - before;
      ll b = a >= 0 ? a : 0;
      return (N - p) - b * L;
    };
    auto find_one = [&]() -> pll {
      REPrev(i, N - 1, 0) {
        ll len = func(sa[i]);
        if (len >= 0) return pll(i, len);
      }
      return pll(-1, -1);
    };
    auto [i0, len] = find_one();
    if (i0 < 0) return "";
    ll i_ans = i0;
    REPrev(i, i0 - 1, 0) {
      if (lcp[i] < len) break;
