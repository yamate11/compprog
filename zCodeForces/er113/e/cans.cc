...
// @@ !! LIM(f:power perm mod debug f:updMaxMin)
...
int main(/* int argc, char *argv[] */) {
  ...
  ll tmax = ((1 << (K - 1)) + 1) * (1 << (K - 1));
  vector rec(K, vector(tmax + 1, vector<ll>()));
  vector<ll> lo(K, LLONG_MAX), hi(K);
  REP(k, K) {
    // DLOGK(k);
    DupIntPerm dip(1 << (K - k), 1 << k);
    auto& v = dip.get();
    do {
      ll x = 0;
      ll co = 0;
      ...
      // DLOGK(v, co, x);
      rec[k][co].push_back(x);
    }while (dip.next());
    DLOGK(k, rec[k]);
#if 0
    ll cnt = 0;
    REP(t, tmax + 1) if (not rec[k][t].empty()) cnt++;
    DLOGK(k, cnt);
#endif
  }
  ...
  auto dfs2 = [&](auto rF, ll k, ll x) -> void {
    if (k == K + 1) {
      REP(t, 1 << K) {
        ll c = [&]() {
          REP(kk, K+1) if ((stack2[kk] >> t) & 1) return kk == 0 ? 1 : (1 << (kk - 1)) + 1;
          assert(0);
        }();
        cout << c << " ";
      }
    ...
    DLOGKL("dfs2", k, x, stack2);
    ll kk = k == 0 ? 0 : k - 1;
    for (ll y : rec[kk][stack[k]]) {
      DLOGK(y);
      if (not (x & y)) {
        stack2.push_back(y);
        rF(rF, k + 1, x | y);
  ...
  auto dfs = [&](auto rF, ll k, Fp acc) -> void {
    DLOGK(k, acc, stack);
    if (k == K + 1) {
      if (acc == Fp(H)) {
        stack2.resize(0);
    ...
    ll kk = k == 0 ? 0 : k - 1;
    REP2(co, lo[kk], hi[kk] + 1) {
      stack.push_back(co);
      rF(rF, k + 1, acc + B[k] * co);
      stack.pop_back();
    }
  };
