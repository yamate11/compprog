...
int main(int argc, char *argv[]) {
  ...
  vector tbl(vmax + 1, vector(3, Fp(0)));
  auto build = [&]() -> void {
    for (ll i = 1; i <= vmax; i++) {
      for (ll tp = 0; tp < 3; tp++) {
	ll ans = 0;
	if (tp == 0) {
	  ll case1 = 0;
	  if (i >= 3) case1 += 4;
	  if (i-1 >= 1) case1 += tbl.at(i-1).at(1);
	  if (i-2 >= 1) case1 += 2 * tbl.at(i-2).at(2);
	  ll case2 = 0;
	  if (i-1 >= 1) case2 += tbl.at(i-1).at(0);
	  if (i-2 >= 1) case2 += 2 * tbl.at(i-2).at(0);
	  ans = max(case1, case2);
	}else {
	  if (i-1 >= 1) ans += tbl.at(i-1).at(0);
	  if (i-2 >= 1) ans += 2 * tbl.at(i-2).at(0);
	}
	tbl.at(i).at(tp) = ans;
      }
    }
