...
struct Trie {
  ...
  /** 文字列ノードの添字 */
  int string_index(const string& s, bool create = false) {
    int cur = 0;
  ...
  void erase(const string& s) {
    ...
    if (nodes[idx].num == 0) throw runtime_error("Trie: tried to remove a number from zero.");
    nodes[idx]._num--;
    for (int i = idx; i >= 0; i = _parent_index(i)) nodes[i]._size--;
  }
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    Fp sum;
    auto trie = create_trie<>('0', '1');
    ll K = 0;
    REP(i, 0, N) K = max(K, (ll)(countr_zero(bit_ceil(S[i] + 1))));
    DLOGK(K);
    REP(i, 0, N) trie.insert(ull2binstr(S[i], K));
    REP(i, 0, N) {
      ...
      REP(k, 0, K) {
        ll thidx = s[k] - '0';
        ll otidx = 1 - thidx;
        int th = trie.child_index(idx, s[k]);
        int ot = trie.child_index(idx, '0' + (1 - (s[k] - '0')));
        ll sz = trie.size(ot);
        sum += (ones + otidx + 1) * sz;
        DLOGK(k, sum);
        if (s[k] == '1') ones++;
