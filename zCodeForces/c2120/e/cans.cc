...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ll x0 = binsearch<ll>(check1, 0, amax + 1);

    auto spV = [&](ll x) -> ll {
      ll i = lower_bound(ALL(A), x) - A.begin();
      ll ret = 0;
      REP(j, i, N) ret += (x + A[j] - 1) * (A[j] - x) / 2;
      return ret;
    };
    auto vcV = [&](ll x) -> ll {
      ll i = lower_bound(ALL(A), x) - A.begin();
      ll ret = 0;
      REP(j, 0, i) ret += (A[j] + x - 1) * (x - A[j]) / 2;
      return ret;
    };

    ll r1 = vacant(x0), s1 = spill(x0 + K);
    DLOGK(x0, r1, s1);
    ll orig = spV(0) + B[N];
    ll diff;
    if (r1 <= s1) {
      diff = spV(x0 + K) - (vcV(x0) + (s1 - r1) * x0) - K * s1;
    }else {
      diff = spV(x0 + K) + (r1 - s1) * (x0 + K - 1) - vcV(x0) - K * r1;
    }
    DLOGK(orig, diff);
    cout << orig - diff << "\n";
