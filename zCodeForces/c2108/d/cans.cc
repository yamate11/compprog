...
// @@ !! LIM(forall binsearch f:intDiv debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> pll {
    ll N, K; cin >> N >> K;
    if (N == 2 * K) return pll(K, K);
    vector<ll> A(K), B(K);
    REP(i, 0, K) A[i] = ask(i);
    REP(i, 0, K) B[K - 1 - i] = ask(N - 1 - i);
    ll d = N % K;
    ll i0 = EXFIND_D(i, 0, K, A[(d + i) % K] != B[i], -1LL);
    if (i0 < 0) return pll(-1, -1);
    ll j0 = (d + i0) % K;
    DLOGK(A, B, i0, j0, d);
    ...
    ll x = binsearch<ll>(check, 0, divCeil(N - j0, K));
    ll j1 = j0 + x * K;
    DLOGK(x, j1);
    vector<ll> Z(K + 1);
    REP(r, 0, K + 1) {
    ...
    DLOGK(Z);
    auto mustL = [&](ll r) -> bool {
      return j1 + r < K or Z[r] != B[(i0 + r) % K];
    };
    auto mustR = [&](ll r) -> bool {
      return j1 + r >= N - K or Z[r] != A[(j0 + r) % K];
    };
    REP(r, 0, K) {
      if (mustL(r) and mustR(r + 1)) {
        return pll(j1 + r + 1, N - (j1 + r + 1));
      }
    }
    return pll(-1, -1);
  };
