...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<ll> a0(na);
    vector<ll> b0(nb);
    REP(i, 0, na) a0[i] = a[i] - '0';
    REP(i, 1, na) a0[i] = (a0[i - 1] + a0[i]) % 10;
    REP(i, 0, nb) b0[i] = b[i] - '0';
    REP(i, 1, nb) b0[i] = (b0[i - 1] + b0[i]) % 10;
    DLOGK(a0, b0);
    if (a0[na - 1] != b0[nb - 1]) {
      cout << -1 << "\n";
    }else {
      vector tbl(na + 1, vector(nb + 1, 0LL));
      REP(i, 0, na) REP(j, 0, nb) {
        if (a0[i] == b0[j]) tbl[i + 1][j + 1] = tbl[i][j] + 1;
