...
int main(/* int argc, char *argv[] */) {
  ...
  auto dfs = [&](auto rF, ll nd) -> void {
    In[nd] = seq++;
    for (ll cld : child[nd]) rF(rF, cld);
    Out[nd] = seq;
  };
  dfs(dfs, 1);
  vector<ll> T(n + 1), init(n);
  for (ll i = 1; i <= n; i++) {
    cin >> T[i];
    init[In[i]] = T[i];
  }
  ...
  ll q; cin >> q;
  for (ll _q = 0; _q < q; _q++) {
    string com; ll i; cin >> com >> i;
    if (com == "get") {
      ll x = st.query(In[i], Out[i]);
      cout << x << "\n";
    }else if (com == "pow") {
      st.update(In[i], Out[i], true);
    }
  }
