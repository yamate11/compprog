...
int main(/* int argc, char *argv[] */) {
  ...
  BrdIdx start(x1, y1), goal(x2, y2);
  if (goal == start) {
    cout << 0 << endl;
    return 0;
  }
  using que_t = tuple<ll, BrdIdx, BrdIdx>;
  queue<que_t> que;
  Board<int> visited(n, m, 0);
  visited.at(start) = 1;
  que.emplace(0LL, start, BrdIdx(1, 0));
  que.emplace(0LL, start, BrdIdx(-1, 0));
  que.emplace(0LL, start, BrdIdx(0, 1));
  que.emplace(0LL, start, BrdIdx(0, -1));
  while (!que.empty()) {
    auto [d, bi, dir] = que.front(); que.pop();
    for (ll i = 1; i <= k; i++) {
      BrdIdx newBi = bi + i * dir;
      if (newBi == goal) {
        cout << d + 1 << endl;
        return 0;
      }
      if (brd.at(newBi) == '#') break;
      if (visited.at(newBi)) continue;
      visited.at(newBi) = 1;
      que.emplace(d + 1, newBi, dir.rotateQ());
      que.emplace(d + 1, newBi, -dir.rotateQ());
      if (i == k) que.emplace(d + 1, newBi, dir);
    }
  }
  cout << -1 << endl;
