...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ll maxdepth = 0;
    REP(i, 0, N) maxdepth = max(maxdepth, (ll)tr.depth(i));
    vector dn(maxdepth + 1, vector<ll>());
    REP(i, 0, N) dn[tr.depth(i)].push_back(i);
    vector<Fp> cnt(N);
    DLOGK(maxdepth, dn);
    for (ll nd : dn[maxdepth]) cnt[nd] = 1;
    REPrev(d, maxdepth - 1, 0) {
      DLOGK(d);
      Fp an = 0;
      for (ll nd : dn[d + 1]) an += cnt[nd];
      DLOGK(d, an);
      if (d == 0) cnt[0] = 1 + an;
      else {
        for (ll nd : dn[d]) {
          Fp z = 0;
          for (ll cld : tr.children(nd)) z += cnt[cld];
          cnt[nd] = 1 + an - z;
          DLOGK(d, nd, z, cnt[nd]);
        }
      }
