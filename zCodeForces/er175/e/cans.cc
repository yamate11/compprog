...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> dd{-1, -1, 0, 1};

  ll N; cin >> N;
  string s; cin >> s;
  vector A(N + 1, 0LL);
  REP(i, 0, N) A[i + 1] = A[i] + (s[i] - '0');
  ll ans = 0;
  REP(k, 0, 4) {
    vector<ll> cnt(2*N + 1);
    vector B(N + 1, 0LL);
    ...
    DLOGK(k, B, cnt);
    auto st = make_seg_tree(0LL, plus<ll>(), cnt);
    ll thr = N + 1;
    ll locans = st.query(0, thr);
    for (ll i = 1; true; i++) {
      if (k + i * 4 >= N) break;
      REP(j, 0, 4) {
        ll p = k + (i - 1) * 4 + j;
        st.rs(N + B[p]) = st.at(N + B[p]) - 1;
      }
      ll e = A[k + i * 4] - A[k + (i - 1) * 4];
      thr = thr + (e - 1);
      ll z = st.query(0, thr);
      DLOGK(k, i, z);
      locans += z;
