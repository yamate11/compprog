...
int body(istream& cin, ostream& cout) {
  ...
  ll N; cin >> N;
  vector<string> S(N); REP(i, N) cin >> S[i];
  vector vis(N, vector<ll>());
  vector<ll> dep(N), bal(N);
  REP(i, N) {
    ll d = 0;
    for (ll c : S[i]) {
      if (c == '(') {
...
#if DEBUG
  REP(i, N) {
    DLOGK(i, bal[i], dep[i], vis[i]);
  }
#endif
  const ll big = 1e10;
  vector<ll> live(1LL << N, -big), dead(1LL << N, -big), totbal(1LL << N);
  live[0] = 0;
  REP2(x, 1, 1LL << N) {
    REP(k, N) if (x >> k & 1) {
      auto v_vis = [&](ll t) -> ll { return (0 <= t and t < SIZE(vis[k])) ? vis[k][t] : 0; };
      ll y = x ^ (1LL << k);
      totbal[x] = totbal[y] + bal[k];
      updMax(dead[x], dead[y]);
      DLOGKL("  1", x, y, dead[x]);
      ll& ud = totbal[y] + dep[k] > 0 ? dead[x] : live[x];
      updMax(ud, live[y] + v_vis(-totbal[y]));
      DLOGKL("  2", x, y, totbal[y] + dep[k], ud);
    }
    DLOGK(x, live[x], dead[x]);
  }
  ll full = (1LL << N) - 1;
  cout << max(live[full], dead[full]) << endl;
  return 0;
}
