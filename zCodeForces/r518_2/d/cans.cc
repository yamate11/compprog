...
int main(/* int argc, char *argv[] */) {
  ...
  ll INC = 0;
  ll DECEQ = 1;
  vector<vector<Fp>> cur(2, vector<Fp>(lim + 2));
  for (ll n = 0; n < lim + 2; n++) cur[DECEQ][n] = 1;
  for (ll i = 0; i < N; i++) {
    auto prev(move(cur));
    cur = vector(2, vector<Fp>(lim + 2));
    for (ll n = 1; n <= lim; n++) {
      cur[INC][n] = cur[INC][n-1];
      cur[DECEQ][n] = cur[DECEQ][n-1];

      DLOGKL(" ??", i, A[i], n);
      if (A[i] == -1 || A[i] == n) {
        cur[INC][n] += prev[INC][n-1];
        cur[INC][n] += prev[DECEQ][n-1];
        cur[DECEQ][n] += prev[INC][n] - prev[INC][n-1];
        DLOGKL("  inside", i, n, prev[INC][n], prev[INC][n-1]);
        cur[DECEQ][n] += prev[DECEQ][lim+1] - prev[DECEQ][n-1];
      }
    }
    cur[INC][lim + 1] = cur[INC][lim];
    cur[DECEQ][lim + 1] = cur[DECEQ][lim];
    DLOGK(i, cur);
  }
  cout << cur[DECEQ][lim] << endl;
