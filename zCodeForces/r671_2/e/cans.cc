...
// @@ !! LIM(sieve f:power debug)
// --> sieve f:power f:<< debug
...
int main(/* int argc, char *argv[] */) {
  ...
  auto primes = sieve(ll(1e5));

  auto solve = [&]() -> void {
    ll n; cin >> n;
    auto prs = prfac(n, primes);
    if (prs.size() == 2) {
      auto [p0, r0] = prs[0];
      auto [p1, r1] = prs[1];
      if (r0 == 1 && r1 == 1) {
	...
	for (ll j = 0; j <= r1; j++) {
	  if (i == 1 && j == 1) continue;
	  if (i == 2 && j == 1) continue;
	  vec.push_back(power<ll>(p0, i) * power<ll>(p1, j));
	}
      }
      vec.push_back(p0 * p0 * p1);
      for (ll j = 1; j <= r1; j++) {
	vec.push_back(power<ll>(p1, j));
      }
      bool first = true;
      for (ll v : vec) {
	if (first) first = false;
	...
	for (ll i = 0; i < k; i++) {
      ...
      vector<ll> vec;
      for (ll i = 0; i < k; i++) {
	ll d1 = prs[i].first * prs[(i-1 + k) % k].first;
	ll d2 = prs[i].first * prs[(i+1) % k].first;
	vec.push_back(d1);
	for (ll d : cls[i]) {
