...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  ll st_r, st_c; cin >> st_r >> st_c; st_r--; st_c--;
  ll lmax, rmax; cin >> lmax >> rmax;
  Board<char> brd(cin, N, M, '*');
  BrdIdx start(st_r, st_c);
#if DEBUG
  const ll big = 100;
#else
  const ll big = 1e15;
#endif
  Board<ll> exc(N, M, big);
  exc.setDispWidth(4);
  using pq_t = tuple<ll, ll, ll, BrdIdx>; // excess_value, left, right, brdidx
  priority_queue<pq_t, vector<pq_t>, greater<pq_t>> pque;
  exc.at(start) = 0;
  pque.emplace(0, 0, 0, start);
  while (!empty(pque)) {
    auto [e, l, r, bi] = pque.top(); pque.pop();
    if (exc.at(bi) < e) continue;
    for (auto bd : BrdIdx::nbr4) {
      BrdIdx new_bi = bi + bd;
      if (brd.at(new_bi) == '*') continue;
      ll new_r = r + (bd.c == 1 ? 1 : 0);
      ll new_l = l + (bd.c == -1 ? 1 : 0);
      if (new_r > rmax) continue;
      if (new_l > lmax) continue;
      ll rel_c = new_bi.c - st_c;
      ll new_e;
      if (rel_c >= 0) new_e = new_r - rel_c;
      else            new_e = new_l + rel_c;
      if (exc.at(new_bi) <= new_e) continue;
      exc.at(new_bi) = new_e;
      pque.emplace(new_e, new_l, new_r, new_bi);
    }
  }
  DLOGK(exc);
  ll cnt = 0;
