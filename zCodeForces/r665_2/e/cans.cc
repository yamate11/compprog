...
int main(/* int argc, char *argv[] */) {
  ...
  const ll HORIZ_START = 0;
  const ll HORIZ_END = 2;
  const ll VERT = 1;
  // event  type 0: horizontal start, 1: horizontal end
  //             2: vertical
  //     detail
  //        horizontal ... y coord
  //        vertical ... end points (small, large)
  using sta = tuple<ll, ll, ll, ll>;
  //               x coord, type, detail1, detail2

  ll pcs = 1;
  vector<sta> events;
  ll n, m; cin >> n >> m;
  for (ll i = 0; i < n; i++) {
    ll y, lx, rx; cin >> y >> lx >> rx;
    events.emplace_back(lx, HORIZ_START, y, 0);
    events.emplace_back(rx, HORIZ_END, y, 0);
    if (lx == 0 && rx == ll(1e6)) pcs++;
  }
  for (ll i = 0; i < m; i++) {
    ll x, ly, ry; cin >> x >> ly >> ry;
    events.emplace_back(x, VERT, ly, ry);
    if (ly == 0 && ry == ll(1e6)) pcs++;
  }
  ...
  vector<ll> init_vec(ll(1e6) + 1);
  st.set_data(init_vec);

  for (auto [x, tp, d1, d2] : events) {
    if (tp == HORIZ_START) {
      st.update(d1, d1 + 1, 1);
    }else if (tp == HORIZ_END) {
      st.update(d1, d1 + 1, 0);
    }else if (tp == VERT) {
      pcs += st.query(d1, d2 + 1);
    }else assert(0);
  }
  cout << pcs << endl;
