...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> Fp {
    ...
    ll nk; cin >> nk;
    vector<ll> ps(nk);
    for (ll i = 0; i < nk; i++) cin >> ps[i];
    sort(ps.begin(), ps.end());
    if (nk >= n-1) {
      Fp t = ps[n-2];
      for (ll j = n - 1; j < nk; j++) t *= ps[j];
      ps[n-2] = ll(t);
    }else {
      ps.resize(n-1);
      for (ll j = 0; j < nk; j++) ps[n-2 - j] = ps[n-2 - j - (n-1 - nk)];
      for (ll j = 0; j < n-1 - nk; j++) ps[j] = 1;
    }

    vector<ll> wts;
    auto dfs = [&](auto rF, ll m, ll p) -> ll {
      ll sum = 1;
      for (ll c : nbr[m]) {
	if (c == p) continue;
	ll x = rF(rF, c, m);
