...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ll seq = N + 10;
    vector ans(N, 0LL);
    vector P(N, vector(2, vector<ll>()));
    vector E(N, vector(2, 0LL));
    REP(i0, 0, M) {
      ll o, i, j; cin >> o >> i >> j; o--; i--; j--;
      P[i][o].push_back(j);
      P[j][o].push_back(i);
      E[i][o]++;
      E[j][o]++;
    ...
    queue<pll> que;
    auto fix = [&](ll nd, ll o) -> void {
      que.emplace(nd, o);
      ans[nd] = (o == 0) ? -seq : seq;
      seq--;
      DLOGKL("fixed", nd, o, ans[nd]);
    };
    REP(i, 0, N) {
      if      (E[i][0] == 0) fix(i, 0);
      else if (E[i][1] == 0) fix(i, 1);
    }
    DLOG("entering loop");
    while (not que.empty()) {
      auto [nd, o] = que.front(); que.pop();
      for (ll peer : P[nd][1 - o]) {
        if (ans[peer] == 0) {
          if (--E[peer][1 - o] == 0) fix(peer, 1 - o);
        }
      }
    }
    if (EXISTS(i, 0, N, ans[i] == 0)) {
      cout << "NO\n";
    }else {
      cout << "YES\n";
      REPOUT(i, 0, N, ans[i], " ");
