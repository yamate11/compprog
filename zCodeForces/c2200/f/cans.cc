...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N, M; cin >> N >> M;
    // @InpMVec(N, (X1, Y1)) [Bntp0qM8]
    auto X1 = vector(N, ll());
    auto Y1 = vector(N, ll());
    for (int i = 0; i < N; i++) {
      ll v1; cin >> v1; X1[i] = v1;
      ll v2; cin >> v2; Y1[i] = v2;
    }
    // @End [Bntp0qM8]
    // @InpMVec(M, (X2, Y2)) [Bntp0qM8]
    auto X2 = vector(M, ll());
    auto Y2 = vector(M, ll());
    for (int i = 0; i < M; i++) {
      ll v1; cin >> v1; X2[i] = v1;
      ll v2; cin >> v2; Y2[i] = v2;
    }
    // @End [Bntp0qM8]
    
    auto ord = sortedIndex(Y1);
    ll ii = N - 1;
    ll a = 0;
    vector<ll> A(N + 1), B(N + 1);
    priority_queue<ll, vector<ll>, greater<ll>> pque;
    REP(i, 0, N) pque.push(0);
    REPrev(y, N, 0) {
      for (; ii >= 0 and Y1[ord[ii]] == y; ii--) {
        ll x = X1[ord[ii]];
        pque.push(x);
        a += x;
        if (ssize(pque) > y + 1) {
          ll t = pque.top(); pque.pop();
          a -= t;
        }
      }
      DLOGK(y, pque);
      A[y] = a;
      B[y] = pque.empty() ? 0 : pque.top();
      if (ssize(pque) == y + 1) {
        a -= pque.top();
        pque.pop();
      }
      DLOGK(y, A[y], B[y], pque);
    }
    
    ll omax = *ranges::max_element(A);

    auto mymax = [&](ll s, ll t) -> ll { return max(s, t); };
    vector<ll> init_vec(N + 1);
    REP(i, 0, N + 1) init_vec[i] = A[i] - B[i];
    auto st = make_seg_tree(0LL, mymax, init_vec);
    
    DLOGK(st.vec_view());
    vector<ll> ans(M);
    REP(i, 0, M) {
      ll y0 = ranges::lower_bound(B, X2[i], greater<ll>()) - B.begin();
      ll v = st.query(y0, Y2[i] + 1);
      DLOGK(i, y0, Y2[i] + 1, v);
      ans[i] = max(omax, X2[i] + v);
    }
    REPOUT(i, 0, M, ans[i], " ");
  };
