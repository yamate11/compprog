...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    REP(i, 0, N) {
      string s; cin >> s;
      Fp y = Fp(strtoll(s.substr(1)));
      if (s[0] == '+') { op[i] = sta(y, 0); }
      else if (s[0] == '-') { op[i] = sta(-y, 0); }
      else if (s[0] == '*') { op[i] = sta(0, y - 1); }
      else if (s[0] == '/') { op[i] = sta(0, 1 / y - 1); }
      else assert(0);
      tot.first += op[i].first;
      tot.second += op[i].second;
    }
  };
  vector tbl1_init(N, Fp(0));
  vector tbl2_init(N, Fp(0));
  auto tbl1 = tbl1_init;
  auto tbl2 = tbl1_init;
  REP(j, 0, N) tbl1[j] = Fp(1);
  Fp nd = 1;
  REP(i, 0, N) {
