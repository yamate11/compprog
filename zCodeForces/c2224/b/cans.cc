...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ...
    while (ip.get()) {
      vector<ll> vec(N);
      REP(i, 0, N) vec[i] = A[ip[i]];
      vector<ll> pmax(N);
      vector<ll> pmex(N);
      pmax[0] = vec[0];
      REP(i, 1, N) pmax[i] = max(pmax[i - 1], vec[i]);
      vector<ll> cnt(N + 1);
      REP(i, 0, N) {
        if (vec[i] < N + 1) cnt[vec[i]]++;
        REP(j, 0, N + 1) {
          if (cnt[j] == 0) {
            pmex[i] = j;
            break;
      ...
      ll val = 0;
      DLOGK(ip.vec_view(), vec, pmax, pmex);
      REP(i, 0, N) val += pmax[i] + pmex[i];
      ans = max(ans, val);
    }
    cout << ans << "\n";
  };
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    DLOGK(mx, mex);
    vector<ll> vec1(N);
    REP(i, 0, mex) vec1[i] = i;
    REP(i, mex, N) vec1[i] = mx;
    vector<ll> vec2;
    vec2.push_back(mx);
    REP(i, 0, mex) {
      if (i == mx and cnt[i] == 1) continue;
      vec2.push_back(i);
    }
    ll sz = ssize(vec2);
    REP(i, 0, N - sz) vec2.push_back(mx);
