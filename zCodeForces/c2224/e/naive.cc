...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    auto f = [&](auto rF, ll nd, ll tm, vector<int> vec) -> void {
      vector<ll> vc;
      if (tr.num_children(nd) == 0) {
        for (ll i : vec) ans[i] = nd;
        return;
      }
      for (ll c : tr.children(nd)) vc.push_back(c);
      ll n = ssize(vc);
      ranges::sort(vc);
      ...
      vec = vector<int>();
      REP(i, 0, n) if (not W[i].empty()) {
        ll c = vc[i];
        rF(rF, c, tm + L[c], move(W[i]));
      }
    };
    vector<int> QI(Q);
    REP(i, 0, Q) QI[i] = i;
    f(f, 0, 0, move(QI));
    REPOUT(i, 0, Q, ans[i] + 1, " ");
