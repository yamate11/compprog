...
struct JudgeLoc : Judge {
  ...
  void answer() {
    if (finished) {
      DLOG("OK, correct answer.");
      ac();
    }
    else {
      DLOG("answer() is called, but not finished");
      wa();
    }
  }
};
  ...
  for (;;) {
    ...
    for (ll i = st; ; i += step) {
      if (P[i] < 0) {
        ...
        break;
      }else if ((step == 1 && P[i] > x) || (step == -1 && P[i] < x)) {
        DLOGKL("overwrite", i);
        P[i] = x;
        judge.ask_q(i);
        break;
