...
// @@ !! LIM(f:power mod debug)
// --> f:power f:gcd f:intDiv mod f:<< debug
...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> a(n + 1);
  using ss_t = pair<ll, ll>;   // upto, value
  map<ll, ss_t> ss;
  for (ll i = 1; i <= n; i++) {
    a[i] = (rnd() % vmax) + 1;
    ss[i] = ss_t(i + 1, a[i]);
  ...
  for (ll ii = 1; ii <= m; ii++) {
    ...
    auto divide = [&ss](ll t) -> void {
      auto it = ss.lower_bound(t);
      if (t == it->first) return;
      it--;
      auto& [j, v] = it->second;
      ss[t] = ss_t(j, v);
      j = t;
    ...
    if (op == 1) {
      ...
      for (auto it = ss.begin(); it != ss.end() && it->first != r; ) {
        if (it->first < l) it++;
        else               it = ss.erase(it);
      }
      ss[l] = ss_t(r, x);
      ...
      for (auto [i, p] : ss) {
        ...
        if (r <= i) break;
        auto [j, v] = p;
        s += power<Fp>(Fp(v), x) * (j - i);
      }
      cout << s << "\n";
    }
    // DLOGK(ss);
    DLOGK(ii, ss.size());
    if (ii % 1000 == 0) DLOGK(ss);
  }
