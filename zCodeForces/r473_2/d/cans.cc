...
int main(/* int argc, char *argv[] */) {
  ...
  bool rem_debug = false;

  ll n; cin >> n;
  // if (n == 85) rem_debug = true;
  // if (n == 73043) rem_debug = true;
  vector<ll> A(n);
  for (ll i = 0; i < n; i++) cin >> A[i];
  auto primes = sieve(1e7);
  DLOGK(primes.size());
  vector<ll> seen(2e5 + 1);
  ll pidx = 0;
  bool coprime = true;
  vector<ll> ans;
  ll change = -1;

  auto still_cop = [&](const auto& prs) -> bool {
    for (auto [p, r] : prs) if (seen[p]) return false;
    return true;
  };

  auto find_cop = [&](ll x, const auto& ps) -> ll {
    for (; true; x++) {
      bool succ = true;
      for (ll q : ps) {
        if (x % q == 0) {
  ...
  for (ll i = 0; i < n; i++) {
    if (rem_debug && i == 9542) cout << "#1 ans[1]=" << ans[1] << " ";
    if (coprime) {
      auto prs = prfac(A[i], primes);
      if (still_cop(prs)) {
        if (rem_debug && i == 9542) cout << "#2 ans[1]=" << ans[1] << " ";
        for (auto [p, r] : prs) seen[p] = true;
      }else {
        if (rem_debug && i == 9542) cout << "#3 ans[1]=" << ans[1] << " ";
        vector<ll> tmp;
        for (ll q = 0; q < (ll)seen.size(); q++) {
          if (seen[q]) tmp.push_back(q);
        }
        if (rem_debug && i == 9542) cout << "#4 ans[1]=" << ans[1] << " ";
        change = find_cop(A[i] + 1, tmp);
        if (rem_debug && i == 9542) cout << "#5 ans[1]=" << ans[1] << " ";
        for (auto [q, rr] : prfac(change, primes)) seen[q] = true;
        coprime = false;
        DLOGK(change, coprime);
      }
    }
    if (rem_debug && i == 9542) cout << "#6 ans[1]=" << ans[1] << " ";
    DLOGK(i, coprime);
    if (coprime) {
      if (rem_debug && i == 1) cout << " cop " << A[i];
      ans.push_back(A[i]);
      if (rem_debug && i == 9542) cout << "#7 ans[1]=" << ans[1] << " ";
    }else if (change > 0) {
      if (rem_debug && i == 1) cout << " change " << change;
      ans.push_back(change);
      change = -1;
      if (rem_debug && i == 9542) cout << "#8 ans[1]=" << ans[1] << " ";
    }else {
      for (; primes[pidx] < (ll)seen.size() && seen[primes[pidx]]; pidx++);
      if (rem_debug && i == 1) cout << " primes " << primes[pidx];
      ans.push_back(primes[pidx++]);
      if (rem_debug && i == 9542) cout << "#9 ans[1]=" << ans[1] << " ";
    }
    // DLOGK(ans);
  }
  if (rem_debug) cout << " fin ans[1]=" << ans[1] << " //// ";
