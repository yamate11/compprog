...
int main(/* int argc, char *argv[] */) {
  ...
  const ll eb = 1e9 + 10;
  auto enc = [&](ll x, ll y) -> ll { return x * eb + y; };
  using sta = pair<ll, ll>;
  unordered_map<ll, vector<sta>> mp;
  ll N; cin >> N;
  for (ll i = 1; i <= N; i++) {
    ll a, b, c, d; cin >> a >> b >> c >> d;
    auto foo = [&](ll x, ll y) -> ll {
      ll g = gcd(x, y);
      x /= g;
      y /= g;
      DLOGK(i, x, y);
    ...
    ll e1 = foo(d * (a + b), b * c);
    ll e2 = foo(a * d, b * (c + d));
    mp[e1].emplace_back(e2, i);
    mp[e2].emplace_back(e1, i);
  }
  CoordCompr cc;
  for (auto& [e, v] : mp) cc.add(e);
  ll sz = cc.size();
  vector<vector<sta>> nbr(sz);
  for (auto& [e, v] : mp) {
    for (auto& [ee, i] : v) {
      nbr[cc.c(e)].emplace_back(cc.c(ee), i);
    }
  }
  ...
  auto dfs = [&](auto rF, ll nd, ll pt, ll pidx) -> void {
    ...
    for (auto [cld, idx] : nbr[nd]) {
      ...
      rF(rF, cld, nd, idx);
      if (! paired[idx]) to_op.push_back(idx);
    }
    ll tsz = to_op.size();
    auto mkpair = [&](ll idx1, ll idx2) -> void {
      ans.emplace_back(idx1, idx2);
      paired[idx1] = true;
      paired[idx2] = true;
    };
    for (ll i = 0; i+1 < tsz; i += 2) mkpair(to_op[i], to_op[i+1]);
    if (tsz % 2 == 1 && pidx >= 0) mkpair(to_op[tsz - 1], pidx);
  };
  for (ll p = 0; p < sz; p++) {
    if (! visited[p]) dfs(dfs, p, -1, -1);
  }
  cout << ans.size() << endl;
