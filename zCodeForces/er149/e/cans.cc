...
// @@ !! LIM(mod power debug cmpNaive perm forall)
...
int naive(istream& cin, ostream& cout) {
  ...
  ll ans = 0;
  while (ip.get()) {
    if (EXISTS(i, 0, N, A[i] >= 0 and ip.at(i) != A[i])) continue;
    vector vec(N, 0LL);
    REP(i, 0, N) vec[i] = ip.at(i);
    bool fail = false;
    for (ll m = N / 2; m >= 1; m /= 2) {
      REP(i, 0, m) {
        ll w = min(vec[2 * i], vec[2 * i + 1]);
        if (w >= m) {
          fail = true;
          break;
        }
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    for (ll m = N / 2; m >= 1; m /= 2) {
      ...
      DLOGK(m, z, r, vec);
      ans = ans * power<Fp>(Fp(2), z) * cb.fact(r);
      k++;
    }
