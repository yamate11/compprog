...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> string {
    ...
    ll numAlice = 0;
    if (lcm < lim) {
      for (ll b : B) if (b % lcm == 0) numAlice++;
    }

    vector<ll> CB(N + M + 1, 0LL);
    for (ll b : B) CB[b]++;
    for (ll a : A) {
      for (ll k = a; k <= N + M; k += a) CB[k] = 0;
    }
    ll numBob = 0;
    REP(b, 0, N + M + 1) numBob += CB[b];
    ll numComm = M - (numAlice + numBob);
    if (numAlice + (numComm % 2) > numBob) return "Alice";
    else return "Bob";
  };
