...
int main(/* int argc, char *argv[] */) {
  ...
  Circle cc0(p0, R);
  using tevt = pair<Real, int>;
  vector<tevt> evt;
  auto interval = [&](Real t1, Real t2) -> void {
    assert(t1 <= t2);
    DLOG("interval, t1=", t1, "t2=", t2);
    ll m = 0;
    if (t1 <= -G_PI) {
      m = ceil((-G_PI - t1) / (2 * G_PI));
      t1 += m * 2 * G_PI;
      t2 += m * 2 * G_PI;
    }else if (t1 > G_PI) {
      m = ceil((t1 - G_PI) / (2 * G_PI));
      t1 -= m * 2 * G_PI;
      t2 -= m * 2 * G_PI;
    }
    evt.emplace_back(t1, 1);
    if (t2 > G_PI) {
      evt.emplace_back(G_PI, -1);
      evt.emplace_back(-G_PI, 1);
      t2 -= 2 * G_PI;
    }
    evt.emplace_back(t2, -1);
  };

  for (ll i = 0; i < n; i++) {
    Real d = qq[i].len();
    Real r = rr[i];
    DLOG("i=", i, "r=", r, "d=", d, "R=", R);
    if (R + r <= d) continue;
    Real th = qq[i].arg();
    Real t = sqrt(d * d - r * r);
    DLOG("  t=", t, "R=", T);
    if (t <= R) {
      Real th1 = atan2(r, t);
      interval(th - th1, th + th1);
    }else {
      Circle cc(qq[i], r);
      auto [b, p1, p2] = cc0.intersect(cc);
      assert(b);
      Real th1 = p1.arg();
      Real th2 = p2.arg();
      if (th1 > th2) swap(th1, th2);
      if (th2 - th1 > G_PI) interval(th2, th1 + 2 * G_PI);
      else interval(th1, th2);
    }
  }
  ...
  for (auto [th, tp] : evt) {
    if (lev == 0 && tp == 1) st = th;
    if (lev == 1 && tp == -1) sum += th - st;
    lev += tp;
  }
  cout << sum / (2 * G_PI) << endl;
