...
int main(/* int argc, char *argv[] */) {
  ...
  auto is_prime = [&](ll p) -> bool {
    auto prs = prfac(p, primes);
    if (prs.size() == 1 && prs[0].second == 1) return true;
    return false;
  };

  auto solve = [&]() -> void {
    ll n; cin >> n;
    vector<vector<ll>> A(n, vector(n, 1LL));
    {
      ll i = 1;
      while (true) {
	if (!is_prime(i) && is_prime(n - 1 + i)) break;
	i++;
      }
      ll j = 1;
      while (true) {
	if (!is_prime(j) && is_prime((n - 1) * i + j)) break;
	j++;
      }
      for (ll x = 1; x < n; x++) A[0][x] = A[x][0] = i;
      A[0][0] = j;
    }
