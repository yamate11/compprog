...
int naive(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    ...
    REP(x, 0, 1LL << n) {
      string t;
      REP(i, 0, n) if (not (x >> i & 1)) t += s[i];
      if (check(t)) {
        ll a = (ll)popcount((u64)x);
        if (a < ans) {
          ans = a;
          DLOGK(ans, t, x);
        }
...
int body(istream& cin, ostream& cout) {
  auto solve = [&]() -> void {
    string s; cin >> s;
    ll cur = 0;
    REP(i, 0, ssize(s)) if (s[i] == '4' or s[i] == '2') cur++;
    ll ans = cur;
    REP(i, 0, ssize(s)) {
      if (s[i] == '1' or s[i] == '3') cur++;
      else if (s[i] == '2') cur--;
      ans = min(ans, cur);
    }
    cout << ans << "\n";
