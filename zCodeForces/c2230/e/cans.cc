...
// @@ !! LIM(sortedIndex f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    // @End [PgTRcWbv]
    ll M; cin >> M;
    // @InpVec(M, TP) [bomGKVxk]
    auto TP = vector(M, ll());
    for (int i = 0; i < M; i++) { ll v; cin >> v; TP[i] = v; }
    // @End [bomGKVxk]
    // @InpVec(M, TC) [QByREdVX]
    auto TC = vector(M, ll());
    for (int i = 0; i < M; i++) { ll v; cin >> v; TC[i] = v; }
    // @End [QByREdVX]
    // @InpVec(M, D) [DP1jqMap]
    auto D = vector(M, ll());
    ...
    vector<ll> ans(M);
    REP(j, 0, M) ans[j] = TP[j] + D[j] + TC[j] + D[j];

    auto infl = [&](ll i, ll j) -> ll {
      ll ret = 0;
      if (P[i] < TP[j]);
      else if (P[i] < TP[j] + D[j]) ret += P[i];
      else ret += TP[j] + D[j];
      if (C[i] < TC[j]);
      else if (C[i] < TC[j] + D[j]) ret += C[i];
      else ret += TC[j] + D[j];
      return ret;
    };
    ...
    {
      vector<sta> evP;
      REP(i, 0, N) evP.emplace_back(P[i], NEWS, i);
      REP(j, 0, M) evP.emplace_back(TP[j], PERSON, j);
      ranges::sort(evP);
      vector<ll> W;
      ll loC = big;
      for (auto [_x, tp, idx] : evP) {
        DLOGKL("evP - C", _x, tp, idx);
        if (tp == NEWS) {
          updMin(loC, C[idx]);
          DLOGK(loC);
          while (not W.empty()) {
            ll j = W.back(); W.pop_back();
            updMin(ans[j], infl(idx, j));
          }
        }else {
          if (loC < big) {
            ll lim1 = TC[idx], lim2 = TC[idx] + D[idx];
            ll val = (loC < lim1) ? 0LL : (loC < lim2) ? loC : lim2;
            updMin(ans[idx], val);
            DLOGK(idx, ans[idx], val);
          }
          W.push_back(idx);
        }
      }
    ...
    {
      vector<sta> evC;
      REP(i, 0, N) evC.emplace_back(C[i], NEWS, i);
      REP(j, 0, M) evC.emplace_back(TC[j], PERSON, j);
      ranges::sort(evC);
      vector<ll> W;
      ll loP = big;
      for (auto [_x, tp, idx] : evC) {
        DLOGKL("evC - P", _x, tp, idx);
        if (tp == NEWS) {
          updMin(loP, P[idx]);
          DLOGK(loP);
          while (not W.empty()) {
          ...
          if (loP < big) {
            ll lim1 = TP[idx], lim2 = TP[idx] + D[idx];
            ll val = (loP < lim1) ? 0LL : (loP < lim2) ? loP : lim2;
            updMin(ans[idx], val);
            DLOGK(idx, ans[idx], val);
          }
          W.push_back(idx);
    ...
    {
      vector<sta> evB;
      REP(i, 0, N) evB.emplace_back(P[i] + C[i], NEWS, i);
      REP(j, 0, M) evB.emplace_back(TP[j] + TC[j], PERSON, j);
      ranges::sort(evB);
      vector<ll> W;
      for (auto [_x, tp, idx] : evB) {
        DLOGKL("- B", _x, tp, idx);
        if (tp == NEWS) {
          while (not W.empty()) {
            ll j = W.back(); W.pop_back();
