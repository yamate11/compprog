...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<vector<pll>> tbl(2);
    REP(i, 0, N + 1) tbl[0][i] = pll(i, 0);
    tbl[1][0] = pll(0, 0);
    REP(i, 1, N + 1) tbl[1][i] = pll(tr.parent(i), 2 * (tr.stsize(i) - 1) + 1);
    for (ll h = 2; true; h++) {
      tbl.push_back(vector<pll>(N + 1));
      REP(i, 0, N + 1) {
        auto [j, s] = tbl[h - 1][i];
        auto [k, t] = tbl[h - 1][j];
        tbl[h][i] = pll(k, s + t);
      }
      if (FORALL(i, 0, N + 1, tbl[h].first == 0)) break;
    }
    ll H = ssize(tbl);
    vector<ll> ans(Q);
    REP(_q, 0, Q) {
      ll v, k; cin >> v >> k; v--;
      assert(tbl[H - 1][v].second > k);
      if (k == 0) {
        ans[_q] = v;
        continue;
      }
      ll h = H - 1;
      while (tbl[h][v].second > k) h--;
      if (h > 0) {
        auto [w0, s0] = tbl[h][v];
        auto [w1, s1] = tbl[h][w0];
        if 
