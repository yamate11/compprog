...
// @@ !! LIM(forall mod UnionFind debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ll N, M, K; cin >> N >> M >> K;
    // @InpMVec(K + 1, ((X, dec=1), (Y, dec=1))) [aTidHrZP]
    auto X = vector(K + 1, ll());
    auto Y = vector(K + 1, ll());
    for (int i = 0; i < K + 1; i++) {
    ...
    auto enc = [&](ll x, ll y) -> ll { return x * M + y; };
    UnionFind uf(N * M);

    vector nbr(N*M, vector<ll>());
    vector A(N*M, false);
    REP(k, 0, K) {
      ll dx = X[k + 1] - X[k];
      ll dy = Y[k + 1] - Y[k];
      if (dx == 0) {
        ...
        A[e0] = true;
      }else if (not (abs(dx) == 1 and abs(dy) == 1)) {
        DLOG("ee");
        return 0;
      }else {
        ll x0 = X[k] + dx, y0 = Y[k];
    ...
    REP(i, 0, N*M) if (uf.leader(i) != i) nume[uf.leader(i)] += nume[i];
    Fp ans = 1;
    REP(i, 0, N*M) {
      ll num_edge = nume[i] / 2;
      if (uf.leader(i) == i) {
        ll gs = uf.group_size(i);
        if (gs < 2) continue;
        if (num_edge < gs) ans *= Fp(gs);
        else if (num_edge == gs) ans *= 2;
        else {
          DLOGK(gs, num_edge);
          return 0;
        }
