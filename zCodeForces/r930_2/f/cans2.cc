...
// @@ !! LIM(segTree f:updMaxMin cmpNaive)
...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    REP(q, 0, Q) {
      ll a, b, c; cin >> a >> b >> c;
      if (a == 1) quest.emplace_back(1, b - 1, c);
      else if (a == 2) quest.emplace_back(2, b - 1, c - 1);
      else assert(0);
    }

    auto stA = make_seg_tree(0LL, [](ll a, ll b) -> ll { return max<ll>(a, b); }, A);
...
#endif
    vector ans(Q, big);
    vector hasbit(bitlen, set<ll>{-1, N});
    REP(i, 0, N) REP(j, 0, bitlen) if (B[i] >> j & 1) hasbit[j].insert(i);
    auto search_right = [&](ll i) -> pll {
      vector<pll> vec;
      REP(j, 0, bitlen) {
        ll p = *(hasbit[j].lower_bound(i));
        if (p < N) vec.emplace_back(p, 1LL << j);
      }
      sort(ALL(vec));
      ll z = 0;
      ll p0;
      ...
      if (z < V) return pll(N, N);
      vector<pll> vec2;
      REP(j, 0, bitlen) {
        ll p = *(prev(hasbit[j].upper_bound(p0)));
        if (-1 < p) vec2.emplace_back(p, 1LL << j);
      }
      sort(ALL(vec2), greater<pll>());
      z = 0;
      ll p1;
      for (auto [p, t] : vec2) {
        z |= t;
    ...
    auto stZ = make_seg_tree(big, [](ll a, ll b) -> ll { return min<ll>(a, b); }, vector<ll>(N, big));
    map<ll, ll> giL{{-1, -1}, {N, N}};
    map<ll, ll> giR{{-1, -1}, {N, N}};
    for (ll i = 0; i < N; ) {
      auto [s, t] = search_right(i);
      if (s == N) break;
      stZ.set_single(s, stA.query(s, t + 1));
      giL[s] = t;
      giR[t] = s;
      i = s + 1;
    }
    REP(q, 0, Q) {
      ...
      if (tp == 1) {
        REP(j, 0, bitlen) {
          if ((B[a] >> j & 1) and not (b >> j & 1)) {
            hasbit[j].erase(a);
          }else if (not (B[a] >> j & 1) and (b >> j & 1)) {
            hasbit[j].insert(a);
          }
        }
        B[a] = b;

        auto itR1 = giR.lower_bound(a);
        auto itR2 = prev(itR1);
        auto itL1 = giL.find(itR1->second);
        assert(itL1 != giL.end());
        while (itL1->first <= a and itL1->second >= a) {
          stZ.set_single(itL1->first, big);
          itL1 = giL.erase(itL1);
          itR1 = giR.erase(itR1);
        }
        ll x = itR2->second + 1;
        while (true) {
          auto [c, d] = search_right(x);
          if (not (c <= a and a <= d)) break;
          stZ.set_single(c, stA.query(c, d + 1));
          giL[c] = d;
          giR[d] = c;
          x = c + 1;
        }
...
#if 0
        auto erase_left = [&](ll i) -> void {
          auto it = giL.find(i);
          if (it != giL.end()) {
            giR.erase(it->second);
            giL.erase(it);
            stZ.set_single(i, big);
          }
        };
        erase_left(a);
        REP(j, 0, bitlen) {
          ll c = *prev(hasbit[j].lower_bound(a));
          if (c >= 0) erase_left(c);
        }

        auto build_left = [&](ll i) -> void {
          auto [c, d] = search_right(i);
          if (i == c) {
            giL[c] = d;
            giR[d] = c;
            stZ.set_single(c, stA.query(c, d + 1));
        ...
        ll c = prev(giR.upper_bound(b))->second;
        if (c >= 0) updMin(ans[q], stZ.query(a, c + 1));
      }else assert(0);
    ...
    vector<ll> pans;
    REP(q, 0, Q) if (get<0>(quest[q]) == 2) pans.push_back(ans[q]);
    REPOUT(i, 0, ssize(pans), pans[i] == big ? -1 : pans[i], " ");
  };
