...
int body(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<ll> num({nabc,na,nb,nc,nbc,nca,nab});

    vector<bool> visited(1LL << 14);
    vector<ll> va({1,5,6,0});
    vector<ll> vb({2,4,6,0});
    vector<ll> vc({3,4,5,0});
    ll ans = 0;
    for (ll ia : va) {
      for (ll ib : vb) {
        for (ll ic : vc) {
          ll sel = 0;
          sv_set(2, sel, ia, sv_get(2, sel, ia) + 1);
          sv_set(2, sel, ib, sv_get(2, sel, ib) + 1);
          sv_set(2, sel, ic, sv_get(2, sel, ic) + 1);
          ...
          for (ll i = 0; i < 7; i++) {
            ll m = sv_get(2, sel, i);
            if (m == 0) continue;
            else if (m == 1) cnt *= num[i];
            else if (m == 2) cnt *= dcmb2(num[i]);
            else if (m == 3) cnt *= dcmb3(num[i]);
          }
          ans += cnt;
        }
      }
