...
int main(/* int argc, char *argv[] */) {
  ...
  auto primes = sieve(1.1e6);

  auto solve = [&]() -> void {
    ll S, Q; cin >> S >> Q;
    auto divs = getDivisors(S, primes);
    ranges::sort(divs);
    ll K = ssize(divs);
    vector A(K, 0LL);
    REP(i, 0, K) {
      if (i < K - 1) A[i] = divs[i] * (divs[K - 1 - i] - divs[K - 1 - i - 1]);
      else           A[i] = divs[i] * divs[0];
    }
    vector B(K + 1, 0LL);
    REP(i, 0, K) B[i + 1] = B[i] + A[i];
    vector C(K + 1, 0LL);
    REP(i, 0, K) C[i] = S - divs[K - 1 - i];
    C[K] = S;
    REP(_q, 0, Q) {
      ll x, y; cin >> x >> y;
      ll j = (ranges::lower_bound(C, S - x + 1) - C.begin()) - 1;
      if (y <= divs[j]) {
        cout << x * y << "\n";
      }else {
        ll m = ranges::lower_bound(divs, y) - divs.begin();
        ll ans1 = (S - C[m]) * y;
        ll ans2 = B[m] - B[j + 1];
        ll ans3 = (C[j + 1] - (S - x)) * divs[j];
