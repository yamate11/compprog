...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ll M; cin >> M;
    // @InpVec(M, A, dec=1) [KiGj8DCm]
    auto A = vector(M, ll());
    for (int i = 0; i < M; i++) { ll v; cin >> v; v -= 1; A[i] = v; }
    // @End [KiGj8DCm]
    vector<bool> dam(N, false);
    REP(i, 0, M) dam[A[i]] = true;
    vector<ll> numDs(N);
    auto dfs1 = [&](auto rF, ll nd) -> ll {
      ll ret = dam[nd] ? 1 : 0;
      for (ll c : tr.children(nd)) ret += rF(rF, c);
      numDs[nd] = ret;
      return ret;
    };
    dfs1(dfs1, 0);
    ...
    auto dfs2 = [&](auto rF, ll nd) -> void {
      if (dam[nd]) {
        ...
        ll k = 0;
        for (ll c : tr.children(nd)) if (numDs[c] > 0) k++;
        if (k > 0) {
          for (ll c : tr.children(nd)) {
            if (numDs[c] > 0) {
              k--;
              if (k > 0) { ans.push_back(c); }
              rF(rF, c);
            }
          }
        }
    ...
    if (ssize(ans) == 0) {
      ...
      cout << ssize(ans);
      REP(i, 0, ssize(ans)) cout << " " << ans[i] + 1;
      cout << "\n";
    }
  };
