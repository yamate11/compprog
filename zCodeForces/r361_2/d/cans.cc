...
// @@ !! LIM(debug segTree binsearch cmpNaive)
// --> f:<< debug segTree binsearch cmpNaive
...
int body(istream& cin, ostream& cout) {
  ...
  const DAT unit_dat_B = LLONG_MAX;
  const DAT unit_dat_A = LLONG_MIN;
  const OP unit_op = nullopt;
  auto xAddB = [](DAT x, DAT y) -> DAT { return min(x, y); };
  auto xAddA = [](DAT x, DAT y) -> DAT { return max(x, y); };
  auto xAppl = [](int k, OP f, DAT x) -> DAT { return f.value_or(x); };
  auto xComp = [](OP h, OP g) -> OP { return h.has_value() ? h : g; };

  auto stA = make_seg_tree(unit_dat_A, unit_op, xAddA, xComp, xAppl, false);
  auto stB = make_seg_tree(unit_dat_B, unit_op, xAddB, xComp, xAppl, false);
  ...
  ll ans = 0;
  for (ll i = 0; i < n; i++) {
    auto check1 = [&](ll j) -> bool {
      return stB.query(i, j) > stA.query(i, j);
    };
    auto check2 = [&](ll j) -> bool {
      return stB.query(i, j) < stA.query(i, j);
    };

    ll j1 = binsearch<ll>(check1, i, n+1);
    ll j2 = binsearch<ll>(check2, n+1, i);
    DLOG("j1=", j1, "j2=", j2);
    ans += j2 - j1 - 1;
  }
  cout << ans << endl;
