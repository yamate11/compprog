...
int main(/* int argc, char *argv[] */) {
  ...
  vector<ll> val(N - 1, 1);
  ll M; cin >> M;
  vector<ll> A(M), B(M), G(M);
  for (ll i = 0; i < M; i++) {
    ll a, b, g; cin >> a >> b >> g; a--; b--;
    A[i] = a; B[i] = b; G[i] = g;
    vector<int> nnp = tr.nnpath(a, b);
    for (ll j = 0; j < (ll)nnp.size() - 1; j++) {
      ll x = nnp[j], y = nnp[j + 1];
      ll k = tr.node2edgeID(x, y);
      updMax(val[k], g);
    }
  }
  for (ll i = 0; i < M; i++) {
    ll a = A[i], b = B[i], g = G[i];
    vector<int> nnp = tr.nnpath(a, b);
    ll ok = false;
    for (ll j = 0; j < (ll)nnp.size() - 1; j++) {
      ll x = nnp[j], y = nnp[j + 1];
