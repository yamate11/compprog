...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N; cin >> N;
    vector<ll> vec(4 * N);
    REP(i, 0, N) REP(j, 0, 4) vec[4*i + j] = i;

    auto check = [&]() -> bool {
      vector pos(N, vector<ll>());
      REP(i, 0, 4 * N) pos[vec[i]].push_back(i);
      REP(i, 0, N) {
        ll a = pos[i][1] - pos[i][0];
        ll b = pos[i][2] - pos[i][1];
        ll c = pos[i][3] - pos[i][2];
    ...
    while (true) {
      cnt++;
      myrand.shuffle(ALL(vec));
      if (check()) break;
    }
    DLOGK(cnt);

    REPOUT(i, 0, 4*N, vec[i] + 1, " ");
