...
// @@ !! LIM(unordered_map debug cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> bool {
    ...
    auto check = [&]() -> bool {
      ...
      REP(i, 1, N) {
        DLOGKL("  ", i, fin, A[i]);
        if (fin.contains(A[i])) return false;
        if (A[i - 1] != A[i]) fin.insert(A[i - 1]);
      }
      return true;
    };
    ...
    for (auto& [c, v] : mp) {
      ...
      for (ll i = 0; true; i++) {
        if (i == 0) {
          cur_st = v[0];
          cur_en = v[0] + 1;
        }else if (i < ssize(v) and v[i] == cur_en) {
          cur_en++;
        }else {
          result.emplace_back(cur_st, cur_en);
          if (i == ssize(v)) break;
          cur_st = v[i];
          cur_en = cur_st + 1;
        }
      }
      if (ssize(result) >= 4) return false;
      if (ssize(result) >= 2) havePl.push_back(c);
      mp2[c] = move(result);
    }
    DLOGK(mp2, havePl);
    auto make_cands = [&](const auto& pv) -> vector<pll> {
      vector<pll> cands;
      if (ssize(pv) == 2) {
        auto [a_st, a_en] = pv[0];
        auto [b_st, b_en] = pv[1];
        if (a_en - a_st == 1) {
          if (b_st - 1 >= 0) cands.emplace_back(a_st, b_st - 1);
          if (b_en < N) cands.emplace_back(a_st, b_en);
        }
        if (b_en - b_st == 1) {
          if (a_st - 1 >= 0) cands.emplace_back(b_st, a_st - 1);
          if (a_en < N) cands.emplace_back(b_st, a_en);
        }
        if (a_en + 1 == b_st) {
          cands.emplace_back(a_st, a_en);
          cands.emplace_back(b_en - 1, a_en);
        }
      }else if (ssize(pv) == 3) {
        auto [a_st, a_en] = pv[0];
        auto [b_st, b_en] = pv[1];
        auto [c_st, c_en] = pv[2];
        if (a_en - a_st == 1 and c_st - b_en == 1) cands.emplace_back(a_st, b_en);
        if (c_en - c_st == 1 and b_st - a_en == 1) cands.emplace_back(c_st, a_en);
      }else assert(0);
      return cands;
    };
    if (ssize(havePl) >= 3) return false;
    else if (ssize(havePl) == 2) {
      auto cands0 = make_cands(mp2[havePl[0]]);
      auto cands1 = make_cands(mp2[havePl[1]]);
      DLOGK(havePl);
      DLOGK(cands0);
      DLOGK(cands1);
      for (auto [i0, j0] : cands0) for (auto [i1, j1] : cands1) if (i0 == j1 and j0 == i1) return true;
      return false;
    }else if (ssize(havePl) == 1) {
      auto cands = make_cands(mp2[havePl[0]]);
      for (auto [i, j] : cands) {
        ll col = A[j];
        auto [a_st, a_en] = mp2[col][0];
        if (a_en - a_st == 1) return true;
        if (a_st == j and a_en == i) return true;
        if (a_en - 1 == j and a_st - 1 == i) return true;
      }
      return false;
    }else if (ssize(havePl) == 0) return true;
    else assert(0);
  };
