...
int main(/* int argc, char *argv[] */) {
  ...
  Fp ans = 0;
  ll qAll = Q[N];
  Fp pow0 = power<Fp>(3, qAll);
  Fp pow1 = qAll >= 1 ? pow0 / 3 : Fp(0);
  Fp pow2 = qAll >= 2 ? pow1 / 3 : Fp(0);
  Fp pow3 = qAll >= 3 ? pow2 / 3 : Fp(0);
  for (ll i = 0; i < N; i++) {
    ll aL = A[i];
    // ll aR = A[N] - A[i+1];
    // ll cL = C[i];
	...
	+ aL * qR * pow1
	+ qL * qR * pow2;
      DLOG("i=", i, "S[i]=", S[i], "nums=", aL, qL, cR, qR, "pows=", pow0, pow1, pow2, pow3, "t=", t);
      ans += t;
    }else if (S[i] == '?') {
      ll t = aL * cR * pow1
	+ qL * cR * pow2
