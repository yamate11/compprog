...
// @@ !! LIM(binsearch debug geometry)
// --> binsearch f:<< debug rerror f:updMaxMin geometry
...
int main(int argc, char *argv[]) {
  ...
  ll N; cin >> N;
  vector<ll> X(N), Y(N), DX(N), DY(N), S(N);
  for (ll i = 0; i < N; i++) {
    cin >> X.at(i) >> Y.at(i) >> DX.at(i) >> DY.at(i) >> S.at(i);
  }

  auto check = [&](Real t) -> bool {
    DLOG("===== calling check, t=", t);
    using StA = tuple<Real, char, Line>;
    vector<StA> event;
    for (ll i = 0; i < N; i++) {
      Point odir(DX.at(i), DY.at(i));
      Point p0(X.at(i), Y.at(i));
      Point dir = odir / odir.len() * S.at(i) * t;
      Line l0 = dir.x > 0 ? Line(dir, p0) : Line(-dir, p0 + dir);
      event.emplace_back(l0.base.x, 1, l0);
      event.emplace_back(l0.base.x + l0.dir.x, 2, l0);
    }
    sort(event.begin(), event.end(),
	 [](const StA& st1, const StA& st2) -> bool {
	   auto& [r1, tp1, ln1] = st1;
	   auto& [r2, tp2, ln2] = st2;
    ...
    auto cmp = [](const Line& ln1, const Line& ln2) -> bool {
      Line const * p1 = &ln1;
      Line const * p2 = &ln2;
      bool norm = true;
      if (p1->base.x > p2->base.x) {
	norm = false;
	p1 = &ln2;
	p2 = &ln1;
      }
      Line vert2(Point(0, 1), p2->base);
      Point p12 = p1->unsafe_intersect(vert2);
      bool judge = p12.y < p2->base.y;
      return judge == norm;
    };
    set<Line, decltype(cmp)> vert(cmp);

    auto myIntersect = [](const Line& ln1, const Line& ln2) -> bool {
      auto in01 = [](Real r) -> bool { return r_le(0, r) && r_le(r, 1); };
      auto [rc, t1, t2] = ln1.intersect_coeff(ln2);
      if (rc == Line::IST_NONE) return false;
      if (rc == Line::IST_ALL) {
	Real t1b = ln1.perpend_foot_coeff(ln2.base);
	Real t1e = ln1.perpend_foot_coeff(ln2.base + ln2.dir);
	Real t2b = ln2.perpend_foot_coeff(ln1.base);
	Real t2e = ln2.perpend_foot_coeff(ln1.base + ln1.dir);
	return in01(t1b) || in01(t1e) || in01(t2b) || in01(t2e);
      }
      return in01(t1) && in01(t2);
    };

    for (auto& [x0, tp, ln] : event) {
      DLOG("event extracted: x0=", x0, "tp=", (int)tp, "ln=", ln);
      if (tp == 1) {
	auto it = vert.upper_bound(ln);
	if (it != vert.end()) {
	  DLOG("upper element: ", *it);
	  if (myIntersect(ln, *it)) {
	    DLOG("intersected");
	    return true;
	  }else {
	    DLOG("not intersected");
	  }
	}else {
	  DLOG("last element");
	}
...
#if DEBUG
	DLOG("** vert");
	for (const Line& lnx : vert) {
	  DLOG(lnx);
	}
#endif
      }else {
	auto it = vert.find(ln);
	DLOG("*it=", *it);
	auto it2 = it; it2++;
	if (it != vert.begin() && it2 != vert.end()) {
	  auto it1 = it; it1--;
	  if (myIntersect(*it1, *it2)) return true;
	}
	vert.erase(it);
      }
    }
  ...
  Real t = binsearch<Real>(check, 1e10 + 10, 0, 1e-7);
  if (t > 1e10) {
    cout << "No show :(" << endl;
  }else {
    cout << t << endl;
