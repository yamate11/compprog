...
Fp func(auto& B, ll N, ll M, ll allW) {
  Fp ret = 0;
  vector<ll> rec(M + 1);
  ll numSeq = 0;
  for (ll i = 0; i < N; i++) {
  ...
  numSeq -= rec[1];
  Fp pow = 1;
  Fp rest = power<Fp>(2, allW - 2);
  for (ll i = 0; 2 + i < M + 1; i++) {
    ll p = i / 2;
    Fp t;
    if (i == 2 * p) {
      t = pow - ((pow - 1) / 3);
...
int main(/* int argc, char *argv[] */) {
  ...
  ll N, M; cin >> N >> M;
  auto B = vector(N, vector(M, 0));
  auto Btr = vector(M, vector(N, 0));
  ll allW = 0;
  for (ll i = 0; i < N; i++) {
    string s; cin >> s;
    for (ll j = 0; j < M; j++) {
  ...
  for (ll i = 0; i < N; i++) for (ll j = 0; j < M; j++) Btr[j][i] = B[i][j];
  Fp s1 = func(B, N, M, allW);
  Fp s2 = func(Btr, M, N, allW);
  cout << s1 + s2 << endl;
