...
// @@ !! LIM(digit rle f:updMaxMin debug)
...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ll N, K; cin >> N >> K;
    auto nvec = du.to_vector(N);
    auto evec = rle(nvec);
    ll st = evec[0].first == 1 ? 0 : 1;
    ll en = evec.back().first == 1 ? ssize(evec) : ssize(evec) - 1;
    DLOGK(evec, st, en);
    
    ll lim = min(32LL, K);
    vector tbl_init(2, vector(lim + 1, -1LL));
    auto tbl = tbl_init;
    tbl[0][0] = 0;
    REP(i, st, en) {
      auto [ch, num] = evec[i];
      ...
      tbl = tbl_init;
      if (ch == 1) {
        REP(k, 0, lim + 1) { if (k + 1 <= lim) updMax(prev[1][k + 1], prev[0][k]); }
      }else {
        REP(k, 0, lim + 1) { updMax(prev[0][k], prev[1][k]); }
      }
      REP(b, 0, 2) REP(k, 0, lim + 1) {
        auto update = [&](ll new_b, ll new_k, ll val) -> void {
          if (new_k <= lim) updMax(tbl[new_b][new_k], val);
        };
        if (prev[b][k] < 0) continue;
        if (b == 0) update(b, k, prev[b][k]);
        else {
          if (ch == 0) update(b, k + num, prev[b][k] + num);
          else         update(b, k,       prev[b][k] + num);
        }
      }
    }
    ll ans = 0;
    REP(b, 0, 2) REP(k, 0, lim + 1) {
      updMax(ans, tbl[b][k] + (K - k));
    }
    cout << ans << "\n";
  };
