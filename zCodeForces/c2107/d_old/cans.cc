...
int main(/* int argc, char *argv[] */) {
  ...
  using sdp_t = tuple<ll, ll, ll>;
  using myset = set<sdp_t, greater<sdp_t>>;
  ...
  auto solve = [&]() -> void {
    ...
    auto dfs1 = [&](auto rF, ll nd) -> void {
      if (tr.num_children(nd) == 0) {
        sdp[nd].insert(sdp_t(tr.depth(nd), nd, -1));
      }else {
        for (ll cld : tr.children(nd)) {
          rF(rF, cld);
          auto [d, i, _dummy] = *sdp[cld].begin();
          sdp[nd].emplace(d, i, cld);
        }
      }
    };
    dfs1(dfs1, 0);

    vector<string> tops(N);
    vector<vector<ll>> subs(N);

    auto dfs2 = [&](auto rF, ll nd0) -> ll {
      auto [d0, idx0, cld0] = *sdp[nd0].begin();
      DLOGKL("dfs2", nd0, sdp[nd0]);
      if (cld0 == -1) {
        tops[nd0] = "1 " + to_string(nd0 + 1) + " " + to_string(nd0 + 1);
        return nd0;
      }
      using sta = tuple<ll, ll, ll, ll>;
      sta cand(-1, -1, -1, -1);
      for (ll i = idx0; true; i = tr.parent(i)) {
        if (ssize(sdp[i]) >= 2) {
          auto it = sdp[i].begin();
          auto [d1, idx1, cld1] = *it;
          auto [d2, idx2, cld2] = *(next(it));
          ll len = d1 - tr.depth(i) + d2 - tr.depth(i);
          if (idx1 < idx2) swap(idx1, idx2);
          updMax(cand, sta(len, idx1, idx2, i));
        }else if (i == nd0) {
          updMax(cand, sta(d0 - tr.depth(nd0), max(idx0, nd0), min(idx0, nd0), nd0));
        }
        DLOGK(i, cand);
        if (i == nd0) break;
      }
      auto [len0, idxA, idxB, idxT] = cand;
      tops[idxT] = to_string(len0 + 1) + " " + to_string(idxA + 1) + " " + to_string(idxB + 1);
      auto func2 = [&](ll i0) -> void {
        ll z = rF(rF, i0);
        subs[idxT].push_back(z);
      };
      auto func1 = [&](auto rG, ll i0) -> void {
        ll jj = 0;
        for (auto [d4, idx4, cld4] : sdp[i0]) {
          if (cld4 < 0) break;
          if (jj == 0) rG(rG, cld4);
          else func2(cld4);
          jj++;
      ...
      ll jj = 0;
      for (auto [d3, idx3, cld3] : sdp[idxT]) {
        if (jj <= 1) func1(func1, cld3);
        else func2(cld3);
        jj++;
      }
      if (idxT != nd0) {
        ll nd1 = tr.parent(idxT);
        sdp[nd1].erase(sdp[nd1].begin());
        if (sdp[nd1].empty()) sdp[nd1].emplace(tr.depth(nd1), nd1, -1);
        auto [d6, idx6, _dummy] = *sdp[nd1].begin();
        for (ll i = nd1; i != nd0; i = tr.parent(i)) {
          ll j = tr.parent(i);
          sdp[j].erase(sdp[j].begin());
          sdp[j].emplace(d6, idx6, i);
        }
        func2(nd0);
      }
      return idxT;
    };
    ll entry = dfs2(dfs2, 0);
    vector<string> ans;
    priority_queue<pair<string, ll>> pque;
    pque.emplace(tops[entry], entry);
    while (not pque.empty()) {
      auto [s, i] = pque.top(); pque.pop();
      ans.push_back(s);
      for (ll j : subs[i]) pque.emplace(tops[j], j);
    }
