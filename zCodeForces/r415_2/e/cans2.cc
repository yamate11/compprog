...
Fp func(ll x0, ll y0, ll k0) {
  x0--; y0--; k0--;
  if (x0 < 0 || y0 < 0 || k0 < 0) return 0;

  auto cC = vector(2, vector(2, vector(2, Fp(0))));
  auto cS = vector(2, vector(2, vector(2, Fp(0))));
  cC[1][1][1] = 1;
  for (ll r = lim; r >= 0; r--) {
    auto pC = move(cC);
    auto pS = move(cS);
    cC = vector(2, vector(2, vector(2, Fp(0))));
    cS = vector(2, vector(2, vector(2, Fp(0))));
    ll bx = (x0 >> r) & 1;
    ll by = (y0 >> r) & 1;
    ll bk = (k0 >> r) & 1;
    for (ll a = 0; a < 2; a++) {
      for (ll b = 0; b < 2; b++) {
        for (ll c = 0; c < 2; c++) {
          for (ll x = 0; x < 2; x++) {
            for (ll y = 0; y < 2; y++) {
              if (a == 1 && bx < x) continue;
              if (b == 1 && by < y) continue;
              if (c == 1 && bk < (x ^ y)) continue;
              ll na = (a == 1 && bx == x) ? 1 : 0;
              ll nb = (b == 1 && by == y) ? 1 : 0;
              ll nc = (c == 1 && bk == (x ^ y)) ? 1 : 0;
              cC[na][nb][nc] += pC[a][b][c];
              cS[na][nb][nc] += pS[a][b][c] * 2;
              cS[na][nb][nc] += (x ^ y) * pC[a][b][c];
            }
          }
        }
      }
  ...
  for (ll a = 0; a < 2; a++) {
    for (ll b = 0; b < 2; b++) {
      for (ll c = 0; c < 2; c++) ret += cS[a][b][c] + cC[a][b][c];
    }
  }
  DLOGKL("**** ", x0, y0, k0, ret);
  return ret;
