...
// @@ !! LIM(f:intDiv segTree debug)
// --> f:intDiv f:<< segTree debug
...
int main(/* int argc, char *argv[] */) {
  ...
  ll ran_len = 31;
#endif
  vector<set<ll>> ran(ran_len); // i \in ran[t] <=> 2**(t-1) <= A[i] < 2**t
  auto ran_elem = [&](ll x) -> set<ll>& {
    if (x == 0) return ran[0];
    return ran[64 - __builtin_clzll(x)];
  };
  for (ll i = 0; i < n; i++) ran_elem(A[i]).insert(i);
#if DEBUG
  for (ll i = 0; i < ran_len; i++) DLOGK(i, ran[i]);
#endif
  ...
  for (ll _q = 0; _q < q; _q++) {
    ll p, x; cin >> p >> x; p--;
    st.update(p, p + 1, x);
    ran_elem(A[p]).erase(p);
    ran_elem(x).insert(p);
    A[p] = x;
    bool succ = false;
#if DEBUG
    for (ll ii = 0; ii < ran_len; ii++) DLOGK(ii, ran[ii]);
#endif
    for (ll b = 0; b < ran_len; b++) {
      auto it = ran[b].begin();
      for (ll i = 0; i < 2; i++) {
        if (it == ran[b].end()) break;
        ll idx = *it;
        ll v1 = st.query(0, idx);
        ll v2 = A[idx];
        DLOGK(_q, p, x, idx, v1, v2);
        if (v1 == v2) {
          succ = true;
          cout << idx + 1 << "\n";
          break;
