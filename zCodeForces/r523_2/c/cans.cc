...
int main(/* int argc, char *argv[] */) {
  ...
  auto divSieve = divisorSieve(ll(1e6) + 10);

  auto cur = vector(n + 1, Fp(0));
  cur[0] = 1;
  for (ll i = 0; i < n; i++) {
    auto divs = getDivisorsDivSieve(A[i], divSieve);
    sort(divs.begin(), divs.end(), greater<ll>());
    for (ll d : divs) {
      if (d <= n) cur[d] += cur[d-1];
    }
  }
  Fp ans = 0;
  for (ll i = 1; i <= n; i++) ans += cur[i];
  cout << ans << endl;
