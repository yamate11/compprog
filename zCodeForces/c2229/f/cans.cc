...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> ll {
    ...
    ranges::sort(A);
    ll m0 = A.back(); A.pop_back();
    N--;

    auto myUM = [&](pll& p, const pll& q) -> void {
      if (p.first < q.first or (p.first == q.first and p.second < q.second)) p = q;
    };

    auto check = [&](ll th) -> bool {
      vector<pll> tbl(1LL << N);
      REP(x, 0, 1LL << N) {
        auto [num, part] = tbl[x];
        REP(i, 0, N) {
          if (not (x >> i & 1)) {
            pll new_val = (part + A[i] >= th) ? pll(num + 1, 0) : pll(num, part + A[i]);
            myUM(tbl[x | (1LL << i)], new_val);
          }
        }
      }
      return tbl[(1LL << N) - 1].first >= K;
    };
    
    ll vmin = binsearch<ll>(check, 0, m0 * N + 1);
    return vmin + m0;
  };
