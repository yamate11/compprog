...
// @@ !! LIM(mod UnionFind f:power)
// --> f:gcd f:intDiv mod UnionFind f:power
...
int main(/* int argc, char *argv[] */) {
  ...
  for (const auto& [x, is] : mp) {
    ...
    {
      ll i = 0;
      for (ll u : nds) mp2[u] = i++;
    }
    UnionFind uf(nn);
    for (ll i : is) uf.merge(mp2[U[i]], mp2[V[i]]);
    ll cnt = 0;
    for (ll i = 0; i < nn; i++) if (uf.leader(i) == i) cnt++;
    Fp ok = power<Fp>(2, n - nn + cnt);
    Fp ng = power<Fp>(2, n) - ok;
    tot_ng += ng;
  }
  Fp tot_ok = power<Fp>(2, n + k) - tot_ng;
  cout << tot_ok << endl;
