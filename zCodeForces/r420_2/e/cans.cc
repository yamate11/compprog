...
int main(/* int argc, char *argv[] */) {
  ...
  for (ll i = 0; i < n; i++) {
    if (i == 0) {
      cv = Matrix<Fp>(C[i] + 1, 1);
      cv.at(0, 0) = 1;
    }else {
      auto prev = move(cv);
      cv = Matrix<Fp>(C[i] + 1, 1);
      for (ll p = 0; p <= min(C[i-1], C[i]); p++) {
        cv.at(p, 0) = prev.at(p, 0);
      }
    }
    Matrix<Fp> mat(C[i] + 1, C[i] + 1);
    for (ll p = 0; p <= C[i]; p++) {
      if (p - 1 >= 0) mat.at(p - 1, p) = 1;
      mat.at(p, p) = 1;
      if (p + 1 <= C[i]) mat.at(p + 1, p) = 1;
    }
    DLOGK(mat);
    DLOGK(A[i], B[i]);
    auto matp = mat.matpower(min(k, B[i]) - A[i]);
    DLOGK(matp);
    DLOGKL("bef", i, cv);
    cv = matp * cv;
    DLOGKL("aft", i, cv);
