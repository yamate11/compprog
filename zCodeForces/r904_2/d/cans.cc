...
// @@ !! LIM(sieve debug forall cmpNaive)
...
int naive(istream& cin, ostream& cout) {
  ...
  auto solve = [&]() -> ll {
    ...
    REP(i, 0, N) REP(j, i + 1, N) {
      if (not EXISTS(k, 0, N, A[i] % A[k] == 0 and A[j] % A[k] == 0)) cnt++;
    }
    return cnt;
  };
...
int body(istream& cin, ostream& cout) {
...
#if DEBUG
  auto divSieve = divisorSieve((ll)(100));
#else
  auto divSieve = divisorSieve((ll)(1.1e6));
#endif
  ...
  auto solve = [&]() -> ll {
    ...
    for (int i = 0; i < N; i++) { ll v; cin >> v; A[i] = v; }
    // @End [0yPjDkRa]
    if (EXISTS(i, 0, N, A[i] == 1)) return 0;
    vector cnt(N + 1, 0LL);
    vector P(N + 1, 0LL);
    vector S(N + 1, 0LL);
    REP(i, 0, N) cnt[A[i]]++;
    REP(i, 2, N + 1) {
      if (cnt[i] > 0) {
        auto divs = getDivisorsDivSieve(i, divSieve);
        sort(ALL(divs));
        bool first = true;
        REP(j, 1, SIZE(divs)) {
          ll d = divs[j];
