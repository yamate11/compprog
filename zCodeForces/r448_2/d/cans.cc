...
int main(/* int argc, char *argv[] */) {
  ...
  auto func = [&](auto rF, ll i, bool chkA, bool chkB) -> ll {
    if (i == n) return 0;
    if (chkA && chkB && a[i] == b[i]) {
      cnt[a[i]]--;
      Fp ret = rF(rF, i + 1, true, true);
      DLOGKL("P1", i, ret);
      return ret;
    }
    Fp ret = 0;
    char st = chkA ? a[i] + 1 : 'a';
    char en = chkB ? b[i] - 1 : 'z';

    auto calc = [&]() -> Fp {
      ll m = n - i - 1;
      Fp r = 1;
      for (char c = 'a'; c <= 'z'; c++) r *= cb.fact(cnt[c]);
      return cb.fact(m) / r;
    };
    ...
    if (chkA) {
      cnt[a[i]] --;
      ret += rF(rF, i + 1, true, false);
      cnt[a[i]] ++;
    }
    if (chkB && cnt[b[i]] > 0) {
      cnt[b[i]]--;
      ret += rF(rF, i + 1, false, true);
      cnt[b[i]]++;
    }
    DLOGK(i, ret);
    return ret;
  };
  Fp x = func(func, 0, true, true);
  cout << x << endl;
