...
// @@ !! LIM(sieve debug mod f:power)
// --> sieve f:<< debug f:gcd f:intDiv mod f:power
...
int main(/* int argc, char *argv[] */) {
  ...
  auto primes = sieve(70);
  DLOGK(primes, primes.size());
  vector<ll> pidx(32);
  for (ll i = 0; primes[i] <= 31; i++) {
    pidx[primes[i]] = i;
  }
  ll psz = 11;
  ...
  vector<vector<Fp>> tbl(71, vector(1LL<<psz, Fp(0)));
  tbl[0][0] = 1;
  for (ll i = 1; i <= 70; i++) {
    Fp mult0, mult1;
    if (B[i] == 0) {
    ...
    if (i > 1) {
      ...
      if (pr[0].first >= 32) {
        ...
        for (auto [p, r] : pr) {
          assert(p <= 31);
          if (r % 2 == 1) t = t | (1LL << pidx[p]);
        }
      }
    }
    for (ll x = 0; x < (1LL << psz); x++) {
      tbl[i][x]     += mult0 * tbl[i - 1][x];
      tbl[i][x ^ t] += mult1 * tbl[i - 1][x];
    }
  }
  cout << tbl[70][0] - 1 << endl;
