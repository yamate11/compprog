...
int main(/* int argc, char *argv[] */) {
  ...
  map<ll, ll> mp;
  {
    ...
    for (ll i = 0; i < n; i++) {
      if (A[i] == prev) {
        ...
        idx++;
        assert(idx == (ll)cnt.size() && idx == (ll)B.size());
        B.push_back(A[i]);
        cnt.push_back(1);
        mp[A[i]] = idx;
        prev = A[i];
  ...
  const OP unit_op = 0;
  auto xAdd = [](DAT xx, DAT y) -> DAT { return xx + y; };
  auto xAppl = [](int kk, OP f, DAT xx) -> DAT { return kk * f + xx; };
  ...
  */
  ...
  for (ll i = n - 1; i >= 0; i--) {
    ...
    auto lidx = lower_bound(A.begin(), A.end(), left);
    auto ridx = upper_bound(A.begin(), A.end(), right);
    ll num = ridx - lidx;
    ans += num;
  }
  cout << ans << endl;
