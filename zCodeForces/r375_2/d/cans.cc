...
int main(/* int argc, char *argv[] */) {
  ...
  vector<sta> sizes;
  auto dfs = [&](auto rF, const auto& bi, ll idx) -> ll {
    if (brd.at(bi) != '.') return 0;
    if (vis.at(bi) >= 0) return 0;
    vis.at(bi) = idx;
    ll cnt = 1;
    for (auto bd : BrdIdx::nbr4) {
      ll x = rF(rF, bi + bd, idx);
      if (x < 0 || cnt < 0) cnt = -1;
      else cnt += x;
    }
    if (bi.r == 0 || bi.c == 0 || bi.r == n - 1 || bi.c == m - 1) return -1;
    return cnt;
  };
  {
    ll idx = 0;
	...
	if (vis.at(i, j) >= 0) continue;
	ll x = dfs(dfs, BrdIdx(i, j), idx++);
	if (x > 0) sizes.emplace_back(x, i, j);
      }
    }
  }
  auto dfs2 = [&](auto rF, const BrdIdx& bi, ll idx) -> void {
    DLOG("dfs2 bi=", bi, "idx=", idx);
    if (vis.at(bi) != idx) return;
    vis.at(bi) = -1;
    brd.at(bi) = '*';
    for (const auto& bd : BrdIdx::nbr4) rF(rF, bi + bd, idx);
  };
  ll num = sizes.size();
  sort(sizes.begin(), sizes.end());
  ll ans = 0;
  for (ll t = 0; t < num - k; t++) {
    auto [x, i, j] = sizes[t];
    ans += x;
    dfs2(dfs2, BrdIdx(i, j), vis.at(i, j));
  }
  cout << ans << endl;
  cout << brd << endl;
