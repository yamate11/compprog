...
int main(/* int argc, char *argv[] */) {
  ...
  ll n, m; cin >> n >> m;
  string s; cin >> s;
  vector pos(128, set<ll>());
  for (ll i = 0; i < n; i++) pos[s[i]].insert(i);
  ...
  st.set_data(vector<DAT>(n, 1LL));
  for (ll _m = 0; _m < m; _m++) {
    ll l, r; char c; cin >> l >> r >> c; l--;
    ll t = -1;
    auto check1 = [&](DAT v) -> bool { return v >= t; };
    t = l;
    ll l0 = st.binsearch_l(check1, 0);
    t = r;
    ll r0 = st.binsearch_l(check1, 0);
    DLOGK(l, r, c, l0, r0);
    auto& set1 = pos[c];
    auto it = set1.lower_bound(l0);
    while (it != set1.end() && *it < r0) {
      st.update(*it, *it + 1, -1);
      it = set1.erase(it);
    }
  }
