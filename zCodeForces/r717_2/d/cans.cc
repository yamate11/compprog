...
int main(/* int argc, char *argv[] */) {
  ...
  REP(i, N) {
    ...
    ll idx = -1;
    for (auto [p, r] : pr) {
      if (app[p] >= 0) idx = max(idx, app[p]);
    }
    if (idx >= 0 and nxt[idx] < 0) nxt[idx] = i;
    for (auto [p, r] : pr) app[p] = i;
  }
  vector<pair<ll, ll>> ivl;
  REP(i, N) {
    if (nxt[i] == -1) continue;
    while (not ivl.empty() and ivl.back().second >= nxt[i]) ivl.pop_back();
    ivl.emplace_back(i, nxt[i]);
  }
  vector<vector<ll>> cp(1, vector<ll>(N, N));
  {
    ll i = 0;
    for (auto [p, q] : ivl) {
      for (; i <= p; i++) cp[0][i] = q;
