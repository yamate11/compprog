...
int main(/* int argc, char *argv[] */) {
  ...
  auto G = vector(2 * K + 1, vector(2 * K + 1, Fp(0)));
  auto effG = vector(2 * K + 1, vector(2 * K + 1, false));
  auto calcG = [&](auto rF, ll n, ll j) -> Fp {
    Fp& ret = G[n][j];
    if (not effG[n][j]) {
      effG[n][j] = true;
      if (j == 0) ret = 1;
      else if (n <= j) ret = 0;
      else ret = rF(rF, n - 1, j) + (n - 1) * rF(rF, n - 1, j - 1);
    }
    return ret;
  };

  auto calcH = [&](ll i, ll j) -> Fp {
    Fp ret = 0;
    REP(r, i - j + 1) {
      ll sign = (r % 2 == 0) ? 1 : -1;
      ret += sign * cb.comb(i, r) * calcG(calcG, i - r, j);
    }
    return ret;
  };
  auto binomN = vector(2 * K + 1, Fp(0));
  binomN[0] = 1;
  REP2(r, 1, 2*K + 1) {
    binomN[r] = binomN[r - 1] * (N + 1 - r) / r;
  }
  
  auto valG = [&](ll j) -> Fp {
    Fp ret = 0;
    REP(i, 2*j + 1) ret += binomN[i] * calcH(i, j);
    return ret;
  };
  vector<Fp> ans(K + 1);
  ans[0] = 1;
  ans[1] = valG(1);
  REP2(j, 2, K + 1) ans[j] = ans[j - 2] + valG(j);
  REP2(j, 1, K + 1) cout << ans[j] << " ";
  cout << endl;
