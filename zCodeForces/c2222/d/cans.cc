...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ordered_set_ll ss_perm;
    REP(i, 0, N) ss_perm.insert(i);
    
    ordered_set_pll ss_sum;
    REP(i, 0, N) ss_sum.insert(pll(S[i], i));

    DLOGK(S);
    vector<ll> ans(N);
    REP(i, 0, N) {
      ll ord = ss_sum.order_of_key(pll(S[i], i));
      ll t = N - i - 1 - ord;
      auto it = ss_perm.find_by_order(t);
      DLOGK(i, ord, t, *it);
      ans[i] = *it;
      ss_sum.erase(pll(S[i], i));
      ss_perm.erase(it);
    }
