...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    vector<pll> D;
    {
      ll j = 0;
      REP(i, 0, X) {
        for (; j < Y and B[j] < A[i]; j++) {
          D.emplace_back(B[j], 1);
        }
        if (j < Y and B[j] == A[i]) {
          D.emplace_back(A[i], 2);
          j++;
        }else {
          D.emplace_back(A[i], 0);
        }
      }
      for (; j < Y; j++) D.emplace_back(B[j], 1);
    }
    DLOGK(D);
    ranges::sort(D, greater<pll>());
    auto func = [&](ll n, ll m) -> ll {
      ll ret = 0;
      vector<ll> num(3);
      for (auto [t, p] : D) {
        num[p]++;
        if (num[0] <= n and num[1] <= m and num[0] + num[1] + num[2] <= n + m) {
          ret += t;
        }else {
          num[p]--;
        }
