...
int main(/* int argc, char *argv[] */) {
  ...
  auto solve = [&]() -> void {
    ...
    ll d0 = (diam - 1) / 2;
    tr.change_root(ct0);
    auto f = [&](ll top, ll dp0) -> vector<ll> {
      vector<ll> Z(N);
      auto dfs1 = [&](auto rF, ll nd) -> ll {
        ll ret = tr.depth(nd) == dp0 ? 1 : 0;
        for (ll c : tr.children(nd)) if (c != ct1) ret += rF(rF, c);
        Z[nd] = ret;
        return ret;
      };
      vector<ll> result{dp0};
      auto dfs2 = [&](auto rF, ll nd) -> void {
        ll cnt = 0;
        for (ll c : tr.children(nd)) if (c != ct1) {
            cnt += (Z[c] > 0) ? 1 : 0;
            rF(rF, c);
          }
        if (cnt >= 2) result.push_back(tr.depth(nd));
      };
      dfs1(dfs1, top);
      dfs2(dfs2, top);
      ranges::sort(result);
    ...
    auto W1 = f(ct1, d0 + 1);
    vector<ll> ans;
    for (ll w0 : W0) for (ll w1 : W1) ans.push_back(w0 + w1);
    ranges::sort(ans);
